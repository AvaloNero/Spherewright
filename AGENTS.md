...
无异常时持续使用同一存档。每完成一种产物或持续供应流水线，先证明实际物流/生产，正常保存，再把实现与经验作为单一 Git 里程碑提交；推送须遵守当前用户授权，本批只保留本地 commit。

## 11. Git 与文件安全

- 在 `main` 开发并按单一目的本地提交，保留已有改动。2026-09-06 本批用户明确禁止 push、tag 和发布；该较新限制优先于此前的持续推送授权，未经后续明确授权不得恢复推送。
