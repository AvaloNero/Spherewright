...
2026-09-24 用户明确选择“适配新版”：允许为已确认的 `0.10.34.28529 → 0.10.35.29057` 单向版本对实现受限原档恢复，先以当前 DLL 编译及测试证明，再冷部署/实机核销。仅扩展既有原 owned primary 重新授权路径；旧票据、旧存档与 Journal 来源版本不伪改，必须保留版本迁移证据、身份及完整历史，未知版本/降级仍拒绝。普通恢复及任意存档访问边界不放宽；不以离线兼容分支推断整版本实机通过。沿用本轮同一原档的直接加载确认，完成 fresh 预检后不再次询问相同加载许可。
