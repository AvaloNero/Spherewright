...
| 角色 | 主要职责 | 保留边界 |
|---|---|---|
| 主会话 | 核心接口与安全代码；unknown/quarantine和疑难故障；关键原回执独立审核、最终版本门与 Git 提交/推送 | 不常规重算材料/候选或重写子 Agent 草稿；未知结果/隔离立即接手 |
| Sol Max | 新布局、新连接策略、跨链取舍；Terra 模板不适用或两次同类失败后的重设计 | 不作为每个成熟工序的默认中转；不直接提交游戏写入，不自行放宽规则或宣布版本门通过 |
...
成熟模板的常规链为 **Terra 准备 → Luna 执行 → Terra 证据整理 → root 核关键原回执、验收与提交**，不强制 Sol 加入；独立工作可并行。只有新布局/连接策略、跨链取舍、模板不适用或两次同类失败才交 Sol 重设计；同一问题经三轮有证据记录仍未解决，再将尝试、失败证据和最小 blocker 交 root 重设计。不是允许 Luna 重放失败目标或盲试三次。固定算术优先复用已验证脚本而非模型手算；辅助任务失败不自动升级为游戏试错或改变游戏失败次数。主会话仍即时处理不确定结果、隔离、需新增原语和代码问题，不等待三轮。

每次委派只传固定短任务包：本批目标、必要规则、输入快照/证据引用、数量/时限、完成及停止条件；返回仅差量、证据索引和未证明项，不默认复制整段历史。可用既有审计器做确定性比对，但主会话保留关键原回执核验和最终独立结论，不能仅凭另一个 Agent 说“通过”。不把所有步骤强制串成四级审批，不让非阻塞整理拖住已满足条件的工序；独立工作可以并行，游戏读取/预检与写入按约定窗口协调，所有角色共享同一 accepted 计数。已读且未变的背景大文档不为每个普通工序重复全读；按任务、DTO/实现变化和失败证据补读必要部分，未变的共享测试复用结论，规定的指令/技能阅读与版本完整回归不省略。异常仍按原 stop 规则。既有成功前缀不重做，fresh 预检、terminal、材料/连接读回、全厂阶段审计、十写冻结、正常保存和及时小步提交推送规则不变。

下一批 2–3 个闭合成熟工序试行 token 分布目标：以未缓存输入＋输出统计，主会话约 10–15%、Sol 约 10–20%、两者合计争取不高于 35%，并列 raw/cached 口径。它不是硬指标、账单或工作价值判断；不得为凑比例拆造低层任务、削减安全核验或延后异常接手。
...
- **按有限业务工序规划**：成熟模板由 Terra 在已批准边界内说明材料、供电、实际端口、输入输出、完成判据及停止条件，并复用现有计算/入口；Luna 用已审阅的共享执行入口推进明确后缀，root 处理代码、异常和最终独立核验。仅新布局/连接、跨链取舍、模板不适用或两次同类失败才由 Sol 设计/重设计。不再为普通取料、单件建造或保存各自重建一套 caller、全流模拟与原记录重放器。
...
- **交接到可运行入口，不再交接重复编程任务**：同类固定成熟工序反复被调用方字段或外层字符串插值错误阻断时，由 Terra 准备最小参数/短文件、主会话审核，Luna仅执行已审核的明确入口；新布局或模板不适用时才由 Sol 准备。优先现有共享client，不每阶段建立新模拟框架。多行PowerShell不嵌入另一语言的可插值字符串；以真实`pwsh -File`形态验证无显式执行参数时在导入前拒绝，再交执行者。Git/远端/CI由主会话单点核对并传完整SHA与run ID，不让每个执行叶子重复查短SHA、把查不到误判为CI失败。此收敛不减少fresh预检、材料/终态核验或未知结果停止条件。
