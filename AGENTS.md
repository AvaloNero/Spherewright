...
2026-09-15 用户确立Sol规划、Luna执行、主会话代码/独立审计的分工；2026-09-18加入Terra Max；2026-09-19用户进一步明确：本次只把主会话原有工作下放给Sol，不能把Terra的工作上移给Sol算作下放。以下现行职责保留Terra/Luna既有分工，不改变完整0.4目标、正常游戏规则或发行门。

| 角色 | 主要职责 | 保留边界 |
|---|---|---|
| 主会话 | 核心接口与安全边界；unknown/quarantine、三轮仍未解决的故障；最终版本门与 Git 提交/推送 | 不重做已交给Sol的常规审核；安全异常立即核原回执，必要抽查不能变成每批完整重审 |
| Sol Max | 保留新布局、连接策略和跨链取舍；新增承接root原有的执行入口审核、正常阶段独立验收、普通产品代码评审/非安全缺陷定位、验收结论与下一工序建议 | 不接管Terra现有实现/计算/测试/取证/草稿；不扩大目标、原语、白名单或安全语义，不直接游戏写入，不自签本人实现/执行，不宣布最终版本门通过 |
| Terra Max | 在 root 已批准的有限模板、数量、目标和停止边界内复用现有计算/入口：常规材料与电力预算、模板参数化计划、有限候选筛查、固定测试、一次证据采集/确定性全量比对与文档草稿 | 默认离线/只读；可将已批准工序直接交 Luna；不能自授权新写入、扩大搜索、放宽 native 规则或声明最终验收 |
| Luna Max | 唯一游戏 writer：执行已审核入口、fresh prepare/commit、同 action 终态、常规批内读回和正常保存 | 游戏 commit 保持单流；不重设计方案或反复自写脚本，不能自签独立审计 |

新增委派必须标出其原归属为root：入口实现与测试仍由Terra完成，Sol只接替root审核；证据采集和确定性全量比对仍由Terra完成，Sol只接替root核关键原回执、覆盖和差异并作独立结论；文档草稿仍由Terra完成，Sol接替root检查验收表述、剩余门和下一工序依赖。普通产品代码的评审/非安全缺陷定位原属root，可交Sol；Terra已承担的脚本修复、固定测试、材料/电力计算和有限候选筛查不得上移。Sol原有规划也不算此次新增下放。

成熟模板的常规链改为 **Terra准备 → Luna执行 → Terra一次取证/比对 → Sol接替root独立验收 → root提交**。实质变化的入口在执行前由Sol接替root审核，未变的已批准入口不增加审批；同一事项不再走“Terra审核→Sol重审→root重审”。Sol只复核既有全量比对的覆盖/差异并核关键原回执，不重新采集或重跑无变化的整批证据。root审范围、未证明项及待提交diff，不重复正常验收；安全异常、最终版本门及必要抽查除外。Sol可把通过审核且已在root明确批准的目标/数量/原语/停止边界内的固定入口交Luna，不自授权新增工序。作者/执行者不能自签独立验收；Sol参与被审实现或执行时由root或其他未参与者独立审核。十写冻结仍须审计、文档、push、绿CI和root明确交接后才解除。

新布局/连接、跨链取舍、模板不适用或两次同类失败仍由Sol设计/重设计；同一问题经三轮有证据记录仍未解决，再将尝试、失败证据和最小blocker交root，不因下放重开Sol第四轮。固定算术仍由Terra复用已验证脚本；辅助任务失败不算游戏试错。不确定结果、隔离、需新增原语、改变安全/协议/保存语义立即交root，不等待三轮。

每次委派只传固定短任务包：本批目标、必要规则、输入快照/证据引用、数量/时限、完成及停止条件；返回仅差量、证据索引和未证明项，不默认复制整段历史。可用既有审计器做确定性比对，但独立审核者必须核原始回执、检查覆盖范围并给可复核结论，不能仅转述“通过”；主会话保留安全异常、最终版本门及抽查权。不把所有步骤强制串成四级审批，不让非阻塞整理拖住已满足条件的工序；独立工作可以并行，游戏读取/预检与写入按约定窗口协调，所有角色共享同一 accepted 计数。已读且未变的背景大文档不为每个普通工序重复全读；按任务、DTO/实现变化和失败证据补读必要部分，未变的共享测试复用结论，规定的指令/技能阅读与版本完整回归不省略。异常仍按原 stop 规则。既有成功前缀不重做，fresh 预检、terminal、材料/连接读回、全厂阶段审计、十写冻结、正常保存和及时小步提交推送规则不变。

下一批 2–3 个闭合成熟工序沿用token分布目标：以未缓存输入＋输出统计，主会话约10–15%、Sol约10–20%、两者合计争取不高于35%，并列raw/cached口径；本次不为提高Sol占比而上移Terra任务。它不是硬指标、账单或工作价值判断；不得为凑比例拆造任务、削减安全核验或延后异常接手，同时比较总用量、工序耗时与返工，不能只看角色占比。
...
2026-09-24 用户继续并授权补齐 IFX-170 的原档重新授权入口；游戏执行角色改为 **gpt-6-luna / max**，不再沿用旧 Luna 实例。仅允许在既有 resume prepare/commit 中为过期、未消费、健康且 Journal 完整的精确 owned primary 创建短时重新授权计划：旧票据只是来源证明，不恢复其有效期；不选择 LastExit、任意文件或导入副本。prepare 必须先核原档完整文件证据、精确保存 tick、内嵌身份、版本、和平与原 Journal，展示 disclosure，commit 等用户随后明确确认并再次核验。加载前 durable 消费旧凭据；成功采用及原 Journal 连续性通过后才正常保存同一身份、签发新票据。异常仍 fail-closed，不绕过 tombstone、篡改 expiry 或补造历史。

## 1. 权威文档

2026-09-26 用户授权按复核结论优化后继续。复核引用的 `b849587` 是旧截面，不能据此重做已完成的纳米管施工、蓝图生命周期或Governor两倍验收。先看 [当前状态](docs/current-status.md)，再按所需证据索引读取历史；状态必须标注采样日期、源码/安装/正式包三种版本和未验边界，不采用主观百分比替代验收门。

从本轮起，每个闭合阶段只在 `docs/evidence/YYYY-MM-DD/` 写一份脱敏证据摘要（提交、安装批次、输入边界、action终态、tick/Journal/accepted、原证明哈希、通过与未通过项）。当前状态只更新下一工序和链接；Roadmap只保留目标/验收门与当前摘要；存档日记保留该档时间线短记录和链接；只有新故障/可迁移经验才更新incident/ledger，不再同时复制整段流水。既有历史不删除，旧计划/token不能因文档归档重新成为执行授权。历史原记录的更正另写补充并链接，不伪改当时结论。

普通只读与已批准候选的 `prepare-only` 可直接复用既有审计客户端/公开工具，不为零commit观察另造一次性执行器、FileSmoke或多级审批。保留方法/候选数量界限、原回执、明确不提交与停止条件；观察产生的token不得被后来施工沿用。含写的有限批次仍须审核实际差量并逐动作fresh prepare/terminal/readback。执行入口绑定自身内容、真实已安装批次与fresh现场；不额外用整个仓库HEAD相等门把无关文档/源码提交变成游戏阻塞，已有强制版本门不能静默绕过。

审计窗口的 accepted 计数是外部工作流台账，不是 `SessionState` 的游戏字段。root在审计、提交、推送和绿CI后封窗，只开始一个新的计数窗口；前批回执永久保留，绝不修改或期待游戏revision、tick、身份或原生计数归零。授权从prepare-only变为执行时，root须直接向实际执行者说明覆盖哪条旧限制、固定目标/数量与停止条件，不能只通知中间规划者。少量已审核的有限动作可逐调用复用既有审计客户端/公开原语，不强制新写一次性脚本；fresh预检、唯一commit、同action终态、材料/连接读回和停止边界全部保留，未测试草稿不准执行。

本轮工程修复限定已有能力的安装完整性、自由带端完成证明、Harvest接近停滞和动作历史有界化；按实际审计结果拆分测试/提交，未修部分明确保留。不借机增工具、玩法类型、任意布局或跨恒星动作。安装器完整预检不是事务升级完成；跨目录切换、失败回滚/断电恢复与最终同提交双包仍需各自实证。六项原先暂留的打包/CI修改继续保留，不混入无关切片；分支保护等远端治理设置不在本轮自动修改。

2026-09-24 用户明确选择“适配新版”：允许为已确认的 `0.10.34.28529 → 0.10.35.29057` 及 Steam 随后更新的 `0.10.34.28529 → 0.10.35.29088` 两个独立单向版本对（不开放29057→29088链式迁移）实现受限原档恢复，先以当前 DLL 编译及测试证明，再冷部署/实机核销。仅扩展既有原 owned primary 重新授权路径；旧票据、旧存档与 Journal 来源版本不伪改，必须保留版本迁移证据、身份及完整历史，未知版本/降级仍拒绝。普通恢复及任意存档访问边界不放宽；不以离线兼容分支推断整版本实机通过。沿用本轮同一原档的直接加载确认，完成 fresh 预检后不再次询问相同加载许可。

2026-09-24 本次执行的后续明确授权：用户在获知原档保存点 `73573789 / J91` 和重新授权范围后，要求“直接加载，别再停下来问我了”。仅对此同一原 owned 保存点，本轮无需重复询问；仍须先通过新模式完整预检、核相同身份/精确tick/Journal，再提交其fresh计划与确认摘要。任何证据不符仍拒绝，不选择别的文件/LastExit、不回退、不延长旧票据。此一次性明确指令不变成对未来存档或未来会话的默认代确认规则。
...
- **按有限业务工序规划**：成熟模板由 Terra 在已批准边界内说明材料、供电、实际端口、输入输出、完成判据及停止条件，并复用现有计算/入口；Luna 用已审阅的共享执行入口推进明确后缀，Sol可审核实质变化的入口与正常阶段证据，root处理核心安全代码、异常和最终版本验收。新布局/连接、跨链取舍、模板不适用或两次同类失败才触发 Sol 设计/重设计。不再为普通取料、单件建造或保存各自重建一套 caller、全流模拟与原记录重放器。
...
- **执行叶子的准备也设墙钟界限**：已批准且只需既有入口的固定执行任务，委派两分钟仍未发出首个实际请求时，先回报准备卡点、是否启动命令及PTY/action；五分钟仍无必要新证据时收回准备。成熟模板由 Terra 给精确数据/短命令；新布局/模板不适用才交 Sol 或主会话，随后再交 Luna 运行，不让执行者重做方案或增加包装。收回前后必须核明是否已有运行命令、commit-intent或accepted；存在已接受动作时只核同action终态，不能中断后重放。该界限不缩短正常施工、恢复加载或原生action的执行等待期限，也不擅自更换模型/思考强度；离线语法错误不算游戏原生失败，准备时长不能藏在几秒的执行耗时之外。
- **交接到可运行入口，不再交接重复编程任务**：同类固定成熟工序反复被调用方字段或外层字符串插值错误阻断时，由 Terra 准备最小参数/短文件、Sol常规审核（安全边界/未知结果交root），Luna仅执行已审核的明确入口；新布局或模板不适用时才由 Sol 准备。优先现有共享client，不每阶段建立新模拟框架。多行PowerShell不嵌入另一语言的可插值字符串；以真实`pwsh -File`形态验证无显式执行参数时在导入前拒绝，再交执行者。Git/远端/CI由主会话单点核对并传完整SHA与run ID，不让每个执行叶子重复查短SHA、把查不到误判为CI失败。此收敛不减少fresh预检、材料/终态核验或未知结果停止条件。
