...
> 当前里程碑：**v0.3.0 Logistics Towers（黄糖闭环后的同星系行星/星际物流塔阶段）**。

本文档是编码 Agent 的执行约束、实施顺序和验收标准。M0 已完成并冻结为历史验收基线；本文开头的 Post-M0 覆盖条款高于后文保留的 M0 基线。若 README、实现与本文的**当前阶段条款**冲突，应先消除冲突；不得再用历史 M0 的“禁止飞行/跨星球/物流塔”条款阻止当前物流阶段。
...
M0 — First Red Matrix 已完成并冻结；蓝糖、红糖和黄糖的自动生产闭环已有同一 owned world 的结构化实机证据。后文 M0 Gate、工具清单和完成定义保留为安全与回归基线，不再是阻止扩展的阶段门槛。

当前阶段是 v0.3.0 Logistics Towers：先完善基础产线和科技，再按正常科技/配方/物品/能源规则建设行星物流与同星系星际物流能力。允许实现和使用同星系原生飞行、独立飞行检查点、跨星球资源运输，以及在科技解锁并真实持有建筑后对行星/星际物流塔进行结构化观察、普通施工和配置。站内物品、无人机和运输船只能通过已验证的正常转移或传送带路径装载，禁止直接写塔库存或内部缓冲；本版禁止跨恒星曲速航线。

游戏内操作仍必须完全通过 Spherewright 的结构化 Bridge/MCP 完成。禁止调用 Computer Use，禁止视觉识别和键鼠宏；不得把它们作为启动、兜底或验收的一部分。

必须使用普通游戏规则：`isSandboxMode=false`、`GameMain.sandboxToolsEnabled=false`、正常资源倍率、正常配方/科技解锁、正常物品/能源/时间消耗。禁止物品注入、凭空补充背包、直接解锁科技、瞬间完成建筑、直接填写生产缓冲、修改游戏速度、修改程序集或存档。

本轮禁止实现：
- 内置 Agent/LLM、自主 Goal Planner 和蓝图批量施工；
- 黑雾、战斗、多人、Nebula、跨星系曲速自动驾驶；
...
- 创建时关闭黑雾的存档。
- 原版物品、配方、建筑、科技和生产规则。
- Windows 为当前阶段的唯一正式运行平台。
- 当前阶段在同一台本机完成开发、构建和 DSP 运行验证。局域网独立游戏验证机仍属于延期架构，本轮不得实现远程部署、远程启动、网络 Bridge 或远程测试编排。
...
以下内容不进入当前 Post-M0 Logistics，也不应为其提前建立复杂抽象：
...
- 蓝图批量施工、跨星系曲速规划和任意未解锁物流塔写入。

当前 v0.3.0 阶段继续纳入 M0 的全部普通玩法原语，并新增：同星系原生飞行、受保护的失败重试、跨星球携货，以及行星/星际物流塔的结构化观察与正常施工/配置。物流塔装载必须复用已验证的正常转移、传送带或分拣器路径，不得直接写槽位库存；跨恒星曲速留待 v0.7.0。外部 Agent 负责规划，Spherewright 不内置自主规划循环。
...
### 2.9 冻结的 M0 原语基线与当前阶段扩展

以下列表是 M0 当时主动收窄的历史写入范围；其中“禁止飞行和跨星球”只约束旧的地表 move 原语，不再禁止当前独立的 interplanetary-flight 与 checkpoint-reload 原语：
...
Post-M0 Logistics 在不放宽结果注入禁令的前提下，额外允许经独立 prepare/commit、能源预算、原生运动状态和检查点生命周期证明的同星系飞行，以及经科技/库存/施工/槽位复读证明的物流塔动作。
...
### C2. 地表移动（冻结的 M0 Gate C 基线）

- 此处只定义旧 `move` 原语：它仅允许当前星球地表步行目标，禁止直接写 `position/uPosition` 或瞬移；跨星球由独立的 Post-M0 原生 flight 原语承担。
...
- 配方配置、科研选择、矩阵产出、飞行与物流路径：...

## 风险
- ...

## 下一步
- 列出当前 Post-M0 Logistics 最早未完成工作项及其第一项具体动作。
- 不得把已冻结完成的 M0 重新当作扩展门禁。
```
...
## 14. 历史路线图与 Post-M0 能力切片

Spherewright 保持 **tool-first**。下列 M1/M2/M4 名称是历史能力分组，不再要求严格串行；当前 Post-M0 Logistics 已获准组合使用其中的生产摘要、合法扩建、同星系飞行和物流塔能力。是否加入内置 LLM 是更晚的可选产品层，不属于底层控制协议的前置条件。
