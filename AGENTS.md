...
2026-09-07 原生端点复核后的临时执行边界：普通路径当前仍把每个点作为NEW对象，未实现旧belt的原生non-removing cover复用。全部新点（含绑定源/末端）必须通过完整当前工厂有界占位检查，不再沿用EXP-149的源点豁免/原生转接层推断；旧belt锚点续接在正确复用验证前明确拒绝。既有对象不自动拆除或迁移，不能删掉endpoint参数规避；原生cmd.stage条件依赖也须独立解决，不改玩家命令伪造prepare通过。该遏制不算完整续接修复或版本验收。
