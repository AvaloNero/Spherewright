...
> 当前里程碑：**Post-M0 Logistics（黄糖闭环后的行星/星际物流阶段）**。
...
当前阶段是 Post-M0 Logistics：先完善基础产线和科技，再按正常科技/配方/物品/能源规则建设行星物流与星际物流能力。允许实现和使用同星系原生飞行、独立飞行检查点、跨星球资源运输，以及在科技解锁并真实持有建筑后对行星/星际物流塔进行结构化观察、施工和配置。
...
当前阶段继续纳入 M0 的全部普通玩法原语，并新增：同星系原生飞行、受保护的失败重试、跨星球携货，以及行星/星际物流塔的结构化观察与正常施工/配置。外部 Agent 负责规划，Spherewright 不内置自主规划循环。
