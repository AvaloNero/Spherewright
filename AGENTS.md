...
当前版本是已通过项目所有者发布审核的 **v0.3.3 — Gameplay Mode Compatibility**，它从不可变的 `v0.3.2` 发布基线回移一项窄范围兼容性修复：沙盒状态和资源倍率继续被观察、报告和保留，但不再作为普通动作、导入或恢复的授权门禁。M0 首颗红色矩阵和 `v0.3.0` 物流塔能力只是历史锚点；补丁不引入 v0.4 Overseer 能力或新动作域。本分支在发布后只作 0.3.3 维护与发行证据封存。
...
完整版本门以 Roadmap 为准。`v0.3.3` 继承已发布 `v0.3.2` 的能力与发行证据；本补丁已以下列证据通过发布门：

- 两个星球各有一座普通施工、正常供电的 ILS；运输船真实搬运远端钛和硅。
- 母星钛/硅输出通过正常物流接入生产，临时石转硅安全退役或明确转为备用。
- 黄糖链在跨星物流供料下出现新的持续运行窗口，不依赖伊卡洛斯人工携货。
...
- `v0.3.3` 发行 Plugin 在本机已完成新建非沙盒 1×、导入非沙盒 100×、导入沙盒 1× 三条 live 路径；全部完成普通采集、手搓、科研、首建筑和正常保存，原档不变。项目所有者明确接受该本机证据作为此补丁的发布门；文档仍不声称完成了独立异机验证。
...
运行时 Journal 是逐存档的机器可读原始证据。每个 owned save 使用独立的首次手搓计数、首次流水线产出寄存器、科技/升级首次选择记录和持久化文件；必须公开 `durableThroughSequence`、`persistencePending` 与 `persistenceError`。不得跨档继承首次状态，也不得在旧档迁移时补造未知历史。
...
不得只说“完成”，也不得把历史 M0 基线、一次 prepare 成功、塔内一次收货或配方已设置冒充当前版本完成。
