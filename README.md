...
- Overseer multi-planet diagnostics: v0.4.0 work in progress; six live implementation slices page all already-created owned factories, reporting save-persisted 600-game-tick automatic production/consumption rates, current-version theoretical output capacity/utilization, per-planet power/logistics, one global current-research summary, and bounded findings from real assembler/lab/miner buffers, power, veins, physically traced station routes, and same-planet upstream producers reached only through item-admitting sorter/splitter paths. The sixth slice adds a current-user-protected per-save temporal window for truly missing consumer inputs on routes with positive demand reservations, source inventory, and carrier fleets; active-carrier motion, delivery, order reduction, or a refilled consumer buffer reset the stagnant baseline, while rollback, topology change, and sparse sampling invalidate it. Each read durably commits its route batch once before exposing temporal evidence. A real active/stalled shipment trial, cross-planet producer recursion, controlled fault trials, and the final release gate remain open.
