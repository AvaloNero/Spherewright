...
- Overseer multi-planet diagnostics: v0.4.0 work in progress; four live slices page all already-created owned factories, reporting save-persisted 600-game-tick automatic production/consumption rates, current-version theoretical output capacity/utilization, per-planet power/logistics, one global current-research summary, and bounded direct-device findings from real assembler/lab/miner buffers, power, veins, and physically traced station routes. Recursive upstream producer paths, temporal logistics-stall proof, controlled fault trials, and the final release gate remain open.
