...
The project is experimental and under active development. The original **M0 — First Red Matrix** milestone is complete; the current development save has also validated automatic power-engine, plastic, and titanium-ingot production plus same-star interplanetary flight. Work is now advancing toward the yellow-matrix production chain.
...
- Exact owned-world restart handoff through DSP's fixed LastExit flow; Spherewright never exposes a save picker or enumerates unrelated saves.
...
All DSP and Unity access runs on Unity's main thread. Only deep-copied DTOs leave that thread. Ambiguous write outcomes quarantine further commits until the exact retained action can be proved or the same owned world is safely restarted.
...
- automatic power engine, plastic, and titanium ingot: complete;
