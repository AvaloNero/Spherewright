...
- Overseer multi-planet diagnostics: v0.4.0 work in progress; three live slices page all already-created owned factories, reporting save-persisted 600-game-tick automatic production/consumption rates, current-version theoretical output capacity/utilization, per-planet power/logistics, and one global current-research summary. Runtime fault-classifier wiring, upstream root-cause paths, controlled fault trials, and the final release gate remain open.
