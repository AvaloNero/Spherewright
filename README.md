...
- Structured reads for the player, progression, recipes, build catalog, resources, factory entities—including detailed logistics-station state—power, the local star system, actions, the per-save gameplay journal, a bounded v0.4 multi-planet native production window with independently recomputed theoretical capacity/utilization, and cursor-stable per-planet power/logistics plus global-research summaries.
...
- Overseer multi-planet diagnostics: v0.4.0 work in progress; the live slices page all already-created owned factories, reporting save-persisted 600-game-tick automatic production/consumption rates, current-version theoretical output capacity/utilization, per-planet power/logistics, one global current-research summary, and bounded findings from real assembler/lab/miner buffers, power, veins, and item-admitting physical cargo paths. The protected per-save logistics window distinguishes an active or warming shipment from 600 continuous game ticks without carrier/order/delivery progress. Recursive diagnosis crosses one exact demand/supply route and continues behind the supply station's real input belt: the titanium-ingot chain on planet `104` reaches station `44` and miner `1` on unloaded planet `102`. A real `102 → 104` titanium shipment now covers dispatch, more than 2,100 moving ticks without a false stall, pickup, delivery, and restored `12 min⁻¹` titanium-ingot production. Complete one-page snapshots no longer consume unreachable continuation capacity, while real pagination keeps its hard bound. A controlled stalled-shipment/recovery trial, the three controlled fault trials, and the final release gate remain open.
