...
The 0.4 authorization now includes bounded native module construction; it does not authorize an internal expansion planner. The first data slice routes explicit blueprint code and fresh owned-entity selections through a Core bounded reader plus native header/signature/import/export evidence in Plugin. Its MCP methods remain read-only and non-executable. It reuses Foundry inventory DTOs rather than creating another recipe/cost planner. Native paste, per-object durable progress and cancel/restart reconciliation remain separate unfinished slices.
