...
2026-09-14 / EXP-300 — 跨纬线蓝图必须复用完整原生游标吸附（`validated`限源码、编译和离线，live pending）：IFX-130的原生UI证据显示RecalculateCursorPos之后需要SnapTropic，再生成分区/预览。漏掉此步时，原位置网格吸附和游标跨区重算可造成比例/锚点错误；raw-5d3ef722与raw-081e2e27是两次零写负例，不代表真实施工尝试。适配器现在按同一顺序调用原生方法，只修改工具的一点数组；既有8区/每区高≤256使纬度扫描≤2049次，后续SnapLongitude固定≤160轮/每轮≤8区，不新增目标规划循环。审批必须使用返回的逐对象姿态，既有64m跨度、玩家范围、碰撞、科技、材料、电力和哈希全部保持。1852测试、完整Release零警告错误、编译后调用顺序与源码64工具/1资源/71120字符指南验证通过；六新增回归不执行Unity，不当作原生布局实测。最近复验同日；当前仍为旧安装态、accepted9/J88/primary57109484。冷部署后对同一源代码fresh检查，保存恢复和真实施工分别验收；游戏DLL、吸附规则、源分区、现场或姿态变化时重验，不重放成功前缀、清除原生错误或伪造新增容量。
