...
- 状态：`observed`；日期/最近复验：2026-09-09；范围：当前DLL及0.4源码只读适配，离线已验证、冷部署/live待验。
...
- 复验触发：同批冷部署后fresh空/非空罐、正常保存恢复、当前版本变化；原黄糖31、钛晶石净−1、原矿保守下界41.949的完整联合失败保持，后两项须独立解释，不用观察修复掩盖供给。accepted4保留。
