...
- 最近复验：2026-09-03（里程碑提前审计确认单一 network `1`、17 节点、10 风机、8 消费者、`55000 J/tick`；当时 ratio `0.15708`，钛硅三只出料/汇流 sorter 都已实际工作，无线回充至满核心）。
...
- 限制或反例：本次已经证明钛/硅各一轮母星入库，但母星 ILS 输出尚未接入钛晶石与高纯硅生产，不能把“塔内收到原矿”写成“黄糖原料持续闭环”，也不能提前发布 v0.3。源库存取货后迅速回补，不能用任意两个相隔很久的计数做差；必须保留订单阶段和紧邻下降窗口。调度阈值、订单符号和来船计数均属于当前版本，版本变化后需重新复核程序集和 live 行为。
...
- 最近复验：2026-09-03（钛、硅各完成一次 `remoteOrder=-200 -> 源库存显著下降 -> order=0` 的独立取货序列；返航后母星需求塔两槽均为 `100/100`、订单归零、运输船归队，随后普通保存）。
