...
- 有限待验方案：fresh确认空载且双端完整后，仅配置763的filter0；若仍满格阻水，最多一次从761精确取20油，原封留存到已有2268并证明两次transfer守恒，为水腾一格。不拆楼、不新增仓、不丢弃、不手工灌水、不循环移出库存；原库存变化不能计作生产，必须另看760实际制造、输出仓和下游窗口。此前“冻结转移/不清仓”不允许擅自扩展为全仓清空；本方案只明确这一格的留存式互锁修复。
- 限制或反例：本条原生语义已核对，修复尚未执行。共享带其他物品造成的头阻塞、供给失配和持续产率不能靠这个单步方案声明已解决；若不恢复，停止而非重复腾位。带→仓无消费者配方needs，EXP-065/217的初始过滤规则照常适用。
- 复验触发：sorter目标类型/过滤、仓格内容、消费者配方/needs、原生取货实现变化，或一次修复后的持续运行。
- 关联：EXP-056/065/217/218、原生API研究、存档日记001。
- 最近复验：2026-09-07（主会话核对现存调用链和逐格证据；有限执行待Luna预检/终态与持续产出证明）。
