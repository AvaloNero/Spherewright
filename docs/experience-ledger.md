...
- 日期/最近复验：2026-09-09；状态`validated`，限DLL/离线语义、原失败实验及19c9069同批冷部署/恢复；新60秒读数及稳定基线仍待live。
...
- 限制/复验：无新增通用读取工具、无游戏写入/加速、无新类型或布局；已有失败数据保持。适配必须验证对齐、陈旧/畸形/负值、手搓隔离、源/session/周期切换、恢复声明和缺口；冷部署及新的固定实机实验另验。关联日记001和game-api-overseer原生窗口证据。
