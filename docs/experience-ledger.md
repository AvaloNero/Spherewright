...
- 最近复验：2026-09-03（粒子容器既有 recipe `99`、三过滤和满电端点保持有效；续入电磁涡轮后输入同步消耗、专仓 `1 -> 12`，既有首次事件仍保持 sequence `38`，没有用“无重复 journal”否定再次自动产出）。
...
- 最近复验：2026-09-03（80 个电磁涡轮从静态仓 `827` 经玩家精确转入活跃仓 `884`；审计时目标分布为仓 51、制造台 2、已新增 11 个粒子容器消耗 22，另有生产过程中的瞬时差额，动作两端均 terminal/completed 且玩家归零，未因聚合仓低于 80 而重放）。
...
- 最近复验：2026-09-03（再次按运行时比例守恒投入 180 油/240 石矿/120 水；首个审计窗已见三种输入同步下降，设备内有 2 酸、专仓由原有 20 增至 21，三过滤、配方、端点与满电状态均未漂移）。
...
- 最近复验：2026-09-03（连续三批各 10 项结果在审计时全部仍可查询为 terminal/completed/succeeded；这些未淘汰对照不改变运行时保留仍有界、不得依赖晚查必然成功的结论）。
...
- 最近复验：2026-09-03（侧偏 28 m 候选一次进入自动铁仓交互范围，300 铁转移与反向返回均成功并停稳）。
