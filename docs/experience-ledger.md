...
2026-09-19 / EXP-299、IFX-121 后置日记断言续接复验（`observed`，限当前私有手搓入口与已落盘 r6 终态）：`Assert-FusionStartupCraftJournal` 已正确把历史排除视为合法的 0 或 1 条新事件；调用方不得在其后再强制计数＋1。续接时须精确保全 J91 身份/完整前缀、durable/零 pending/error；仅剩 r56，因此最终只能保持 J91 或新增 1 条对应 `2302` 的 durable 首件事件，不能补写/迟到 `1202`。已 accepted 的 Move、取料和 r6 固定为不可重放前缀。当前仅有 r6 成功事实；r56、保存、新炉与持续供给仍待，见[本档停止/续接记录](./gameplay-timeline.md#2026-09-19--高纯硅炉手搓前缀在-r6-后本地断言停止精确后缀待验)。
