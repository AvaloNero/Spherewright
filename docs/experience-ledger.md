...
- 最近复验：2026-09-03（原三过滤、设备配方、双端连接与四只 sorter 满电状态均未漂移；按完整 160 硫酸预算分批守恒投入 240 油/320 石矿/160 水后，三种输入同步下降、化工厂持续工作、输出 sorter 实际携酸，专仓再次从 0 增长到 38+）。
...
- 最近复验：2026-09-03（第二批先让 PLS 制造台保持缺处理器/粒子容器，再向共享仓精确补 68 钛/68 钢和分批 140 硫酸；651 硅石不变，钛合金专仓从 8 增至 80。该排程避免本批被 PLS 支路消费，但仍不把过滤误称为通用优先级控制）。
...
- 最近复验：2026-09-03（第一次扫描后的新增 belt `1534` 反证临时起点；无人机清零后的 22 格完整有向遍历确认稳定首尾及合法中段注入）。
...
- 最近复验：2026-09-03（两端满电 sorter 实际携铁，蓝矩阵输出 sorter 实际携货，科技五个窗口单调增长并完成正常保存）。
...
- 最近复验：2026-09-03（九项结果仍可晚查，一项只由已保存即时终态与 fresh 拓扑/库存/保存边界核销）。
