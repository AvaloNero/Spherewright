...
- 最近复验：2026-09-01（转移已执行但空聚合后处理报错；fresh 双边复读阻止后续盲猜，并将 action client 收紧为错误即停）。
...
- 最近复验：2026-09-01（钛锭与金刚石两条锁定配方预建线均在解锁后完成自动产出、日记与保存闭环）。
...
- 最近复验：2026-09-01（`163/557/558` 三仓聚合复读）。
...
- 最近复验：2026-09-01（`133 -> 120` 反向五锚点全程 Walk，液罐外缘 5 m 防碰撞，并保持 98% 以上核心能量）。
...
- 直接证据：修复源码把 `slot < 0` 的验收候选限定为当前 16 个真实连接槽，具体非负槽仍只验自身；新增 3 个候选选择测试后，Contracts/Core/MCP 共 `4 + 51 + 13 = 68` 项测试通过，完整 solution build 为 0 warning/0 error。当前进程仍运行旧 DLL，故在重启部署和下一条 live sorter 正常 completed 前保持 observed。
...
- 复验触发：修复版单元/完整构建、正常重启后实体 `721` 持久化读回、下一条 belt↔sorter 正常 completed、隔离核销再次遇到虚拟槽或连接槽上限变化。
- 关联：EXP-012、EXP-028、EXP-038、EXP-062、EXP-068、`src/Spherewright.Plugin/Game/NormalGameActionCoordinator.StructuredActions.cs`。
- 最近复验：2026-09-01（实体 `721` 双端互指、精确物品差量与 reconciliation 同源失败的现场对照）。
