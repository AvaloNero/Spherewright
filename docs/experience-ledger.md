...
2026-09-17 / EXP-077、EXP-236补塔预检复验（`validated`限两个真实拒绝）：电塔覆盖/连网几何合格仍不证明其整座碰撞体和地面可放置。本次2201两候选分别撞3050、被原生NeedGround拒绝，0commit；两点退役并交回规划，保留已保存线路和材料。下一只基于实际陆侧/碰撞证据提出少量不同点，再fresh原生预检，不用移动地表剖面代替建筑放置门；见[本档两拒绝](./gameplay-timeline.md#2026-09-17--纳米管两处补塔候选被真实放置检查拒绝)。最近复验同日，候选、吸附、地形或旧设备变化时重验；这不是两塔已建、供电恢复或整条纳米管施工通过。
