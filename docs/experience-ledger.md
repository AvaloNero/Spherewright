...
- 2026-09-05：新增 EXP-181。从不可变 `v0.3.2` 建立 `v0.3.3` 维护候选，保留沙盒/倍率观察证据，移除普通写入、导入、计划重启和飞行检查点的对应门禁；不开放沙盒工具或任何旁路写入。初始离线 158 项测试与完整 Release build 通过，同日后续本机三档 live 证据将本条升级为 validated；未宣称独立异机验证。
