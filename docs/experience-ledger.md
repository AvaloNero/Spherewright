...
- 最近复验：2026-09-02（Move 已终止后才发生 session 字段展示错误；fresh 位置、Walk/速度、revision 和写健康核销，未重复移动）。
...
- 最近复验：2026-09-02（两处新增密集基座夹缝均由 180-tick 物理进展窗口在满电时早停；fresh 6 m 自由候选脱困，未重放失败长订单）。
...
- 最近复验：2026-09-02（钛锭、金刚石、钛晶石和粒子容器四条锁定配方预建线均在解锁后完成自动产出、日记与保存闭环）。
...
- 最近复验：2026-09-02（至少 8.17 m 非带实体净空的新候选仍落入 Drift；立即返回上一精确 Walk 落点并完全停稳，碰撞净空与陆地身份继续分开判定）。
...
- 最近复验：2026-09-02（既有四过滤共享仓 `723` 在过滤/双端不变时再次分批补入约 400 磁铁、500 铁和 200 铜，线圈制造台恢复工作且未观察到串料）。
...
- 最近复验：2026-09-02（有机晶体活跃目标仓由 58 仓存 + 2 制造台输入闭合全部 60 个；源端并发补货、目标端即时消费与生产净增三类边界均未触发重放）。
...
- 最近复验：2026-09-01（高纯硅/铜双过滤在装料与 recipe 启用前完成，微晶元件连续增长并越过 durable journal/正常保存边界）。
...
- 最近复验：2026-09-01（纯石矿带建立自动缓冲，三过滤空仓完成后装料，硫酸产出越过 durable journal/正常保存边界）。
...
- 最近复验：2026-09-02（纯几何落点被 Drift 反证后，草地 Detail 节点作为对岸陆地锚点使 109 m 单订单稳定落地，并完成钛仓往返）。
...
- 最近复验：2026-09-02（新 DLL 实机先拒绝带货窗口，随后在空载 Returning 窗口成功配置 `551` 并双端复读；环带继续守恒排空）。
...
- 限制或反例：若机甲研究功率大于 0、所有必需矩阵齐全且有足够能量，points 会正常消耗并上传 hash，不能要求其静态不变。科技完成后的自动退回行为仍需下一次 live 终态复读。
...
- 最近复验：2026-09-02（同一行星物流站产线第二批完整消耗并产出，普通保存 tick 9783554；随后成品守恒施工为实体 918）。
...
- 最近复验：2026-09-03（第二个样本证明 `3 m` 回退可在邻接水面提前完成；同一坐标收紧到 `0.5 m` 后恢复 Walk/0，所有 stale 均无 action ID）。
...
- 最近复验：2026-09-03（更多非单调 ID 样本；238 格完整单链拓扑复读，末端 `1471` 只入无出）。
...
- 当前结论：原生 prepare 成功只表示候选可解释为合法带路，不保证它是在向新的空网格延伸；球面量化和转角可能让 plannedPath 的后续点回到该末端的既有输入带。候选工具必须忽略第一个 source 点后，将其余 plannedPath 与全部既有 belt 坐标复核，排除重叠候选；同时显式报告相对目标的净前进量，避免把回折误选为“高净空”。最终 commit 仍必须 fresh 读取 source/player 哈希。
- 直接证据：从自由末端 `1455` 扫描时，原生 prepare 接受了 planned end `(-100.510437,-43.672276,-167.5423)`，该坐标实际已经属于上游 belt `1443`；从 `1462` 的另一个候选也量化回既有带段。两者都没有 commit。只读脚本 `scripts/find-belt-route-candidates.ps1` 随后增加 `0.25 m` 既有 belt 重叠排除、`minimumNewEntityCenterClearance` 与 `destinationProgress`，并继续保留 endpoint 无输出前置检查。
- 限制或反例：`0.25 m` 只用于识别同一网格中心附近的既有基础带，不是通用碰撞半径；合法并线、分流器、垂直带或显式连接既有 belt 的方案需要不同语义，不能直接复用这个排除规则。候选器仍只读，不替代 DSP commit 后拓扑验收。
...
- 最近复验：2026-09-03（两个 prepare-only 回折样本均未提交；候选脚本已加入旧带重叠和净前进量复核）。
...
- 最近复验：2026-09-03（电塔 `1490` 使 sorter `1489` 从无电变为满供电并实际携磁铁；线圈、蓝糖、研究逐层恢复，科技完成且正常保存）。
...
- 最近复验：2026-09-03（未达无线塔但 fresh Walk 停点已进入两个材料仓操作范围；直接守恒取钛成功）。
...
- 最近复验：2026-09-03（三条新 filter 防止 651 硅石串料；钛/钢按两台合法配方分流，钛合金首产 8 并正常保存）。
