...
### EXP-090 — 旧矩阵带再次堵塞时，可用混合仓到研究站的原生需求直供先恢复科技

- 状态：`validated`
- 日期：2026-09-01
- 适用范围：研究站缺少当前科技矩阵、旧矩阵生产/运输支路存在历史混料和回收竞争、普通仓仍保有足量自动矩阵。
- 当前结论：若旧矩阵带的永久修复会延误当前生产科技，而普通混合仓里已有足量自动矩阵，可以先对“普通仓 -> 当前研究站”做独立 sorter prepare/commit。DSP 研究站的当前矩阵需求会约束实际取料；完成后必须读回 sorter 双端、供电、研究站目标矩阵增长和 `hashUploaded` 连续增长，不能仅凭连接成功推断恢复。这个旁路只恢复科研消费，不会自动洗净旧混料带。
- 直接证据：处理器科技 `1302` 长时间停在 `44100/144000`；研究站 `84/679` 都有红矩阵但蓝矩阵为 0，蓝矩阵站 `76` 则因电路板输入为 0 停机。旧终端 `300` 同时接研究 sorter `312` 和回收 sorter `563`，`563` 实际携带电路板进入几乎满载的混料回收仓 `562`，上游 `311` 也停在携板插入态。普通仓 `26` 仍有 600 个既有自动蓝矩阵；新增 sorter `860` 经端点 prepare 后直接连接 `26 -> 84`，满供电运行。8 秒后研究站蓝矩阵内部缓冲由 0 增至 15700、设备恢复工作，科技由 `44100 -> 45479`，随后连续增至 `88189/144000`，证明旁路实际消费而不是静态连接。
- 限制或反例：当前样本只证明研究站在科技 `1302` 需求下从混合仓取得 item `6001`；不得据此宣称任意制造设备会替上游混料仓筛选，也不得撤销 EXP-058。旧 `551 -> 542…300 -> 563 -> 562` 混线仍需在处理器里程碑之后单独清理，旁路 `860` 也应在科技需求变化时重新复读实际矩阵类型。
- 复验触发：当前科技完成并选择红/黄矩阵科技、仓 `26` 蓝矩阵耗尽、sorter `860` 携非矩阵物料、研究站再次停工或旧蓝矩阵产线被永久重构。
- 关联：EXP-049、EXP-054、EXP-058、EXP-059、EXP-073、EXP-079。
- 最近复验：2026-09-01（混合仓直供研究站后蓝矩阵缓冲与处理器 hash 连续恢复，旧混料带仍明确保留为待修问题）。
