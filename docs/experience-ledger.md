...
- 现行短跨越解释：完整observed、零unknown、≤32m、实际点距≤1m，并逐点核验末端至少4m的groundHit/waterHit有效、有限非空groundBelowWaterMetres≤−0.1且无风险；结合当前障碍、≤0.5m容差和有限跨越/恢复能量，外部Agent可选择一次fresh normal Move。浅水not_detected、partial/unavailable/缺失样本不满足；不扩大射线预算、不选择下一目标或更改Plugin准入。终态后真正Walk/低速度/能量仍须实读，不重试失败目标，不把指南条件当作保证或自动循环。长距离/不完整预览沿用EXP-093历史Walk要求。
