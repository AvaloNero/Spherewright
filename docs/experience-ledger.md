...
- 日期/最近复验：2026-09-09；状态`observed`，当前原生窗口和一次失败实验已独立复核，长窗适配尚未实现/live。
...
- 下一最小修复边界：仅Governor显式选择600或3600tick，默认与已有声明仍600；测量周期绑定系列、提案和持久声明，基线前固定，锁后不可更改。三个独立稳定窗口、10%误差、准确2×及至少36000tick持续覆盖不放宽；长窗不保证真实产量稳定，不把短窗故障/目标链健康证据隐藏。
