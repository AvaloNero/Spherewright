...
- 限制或反例：两种物料共用末段会共享带宽；任一 ILS 槽满后是否会让该物品占住站口并挡住另一种，当前尚未跨过首次满槽/派船周期。在真实运输循环验收前，不宣称该布局具有长期吞吐保证。
...
- 最近复验：2026-09-03（普通保存 tick `12278450` 后的 fresh 审计确认 44 格新带完整、两只 sorter 端点/受电正确，ILS 两种物料继续增长）。
