...
- 状态：`validated`（仅当前两负例的离线数值复现；未来施工准入未验）
- 日期：2026-09-08
...
- 限制：只证明反例复现和两NEW场地预检，不证明未来分拣器、油/黄糖持续产出或Governor。更优的离线角度本身不授权试建多个候选。
- 复验触发：DSP哈希、段边界、邻居/倾斜、原生投影实现、插值精度或现场改变。
- 关联：EXP-255、包内明确单带段/禁止重试规则、game-api-foundry、存档日记001。
- 最近复验：2026-09-08（两实机负例及段长离线复现、两NEW非移除cover预检；accepted8冻结）。
