...
2026-09-19 / EXP-299 C 阶段的 prepared/Session DTO 与 post-terminal 无人机边界复验（`validated`，限5115的2302建造、recipe59配置及独立6NEW input）：prepared build/configure DTO 不得硬要求其未公开的 target 字段，公开 Session 也不得假设存在`activeActionId`；应以请求的 state hash、同 action terminal 与 fresh 实体 readback 闭合。C.1 首次本地字段断言发生在5次读取/prepare、0 commit之后，不能由缺 stdout/runId 写成0调用；实际 build terminal 成功后，`working>0`但无 pending build/repair 只允许有界 fresh 等待自然 idle，不能重放。C.2 的5115 recipe59 readback应接受原生生成的零 count/zero inc input1003/output1105缓冲条目，而不是要求数组为空。C.1 terminal、fresh/root closure及C.2 completion分别为 SHA-256 `30111B…E71DD6E87`、`069095…7607BDE1`/`062505…F27A36507`、`D44932…2BFAE90`。C.3 terminal `336fe5a6-210c-4aaa-9b49-307bef42fc46` 唯一形成5116→5121六段内部有向链、传送带−6；短等待仍working时停止不重放，之后10读closure在R99/save72577830/J91/accepted6观察到自然 idle、5115 recipe59及六段逐点链保全（SHA-256 `E846DB…F4DBF92`、`BADE09…B5419C`）。这些边界均未保存、未接料且不证明施工后的吞吐、持续电力/燃料或恢复。DTO、动作回执或无人机语义变化时重验。

2026-09-19 / EXP-299 C.4 source-cover 的终态与等待边界复验（`validated`，限5121→5122的4NEW non-removing cover）：同 action terminal 已成功时，短 idle 等待超时只能停止后续写入并保全前缀，不能反推 native 失败或重放。9读、0写 closure 在R101/save72577830/J91/accepted7确认5121未移除并唯一接5125→5124→5123→5122（四NEW内部顺序不变）、传送带−4；原 terminal/stopped、closure及Sol末审分别为 SHA-256 `808F47…05353D`、`89352A…AF7CE2`、`136587…C74B9`、`F62197…EC425`（tick72948771）。这不证明后续附件、C.6 11NEW上坡、保存、吞吐、持续电力/燃料或恢复；无人机或动作语义变化时重验。

2026-09-19 / EXP-299 封窗后有界手搓备料的静态比对复验（`validated`，限西侧 B 段的`r88×2→r7×2→normal save`）：后续只消耗玩家库存的手搓与保存，应把原 commit/terminal、完整玩家净额、J91和全厂静态配置/姿态/有向拓扑逐项对齐；不得因工厂数量未变而省略完整快照，也不得把buffer、progress、瞬时generation或 live consumer.requiredEnergy 跨 tick 当静态恒等。两次手搓仅形成高速分拣器×4和风力涡轮机×2，保留普通分拣器×2；这只是材料就绪，不证明施工、连接、持续电力/燃料、吞吐或恢复。批准 leaf SHA 必须逐字取自 `Get-FileHash` 原输出，不能从摘要手抄变体；本次 hash 前置拒绝为0游戏调用，不计 accepted 或原生失败。执行 SHA-256 `F92D53…97211F`、收集 `210CE3…7D962`、Terra 比对 `60F5E6…B4637A`、Sol 末审 `A0AFC2…338F63`（tick 72586876）；后续动作、DTO或场景变化时重验。

2026-09-19 / EXP-299 十写续接后的保护证明、显示与动态字段边界复验（`validated`，限西侧 A 段的既定十动作与当前 FactoryConfiguration 域）：一旦唯一 commit、同 action terminal 与最终 session 已证明成功，完整保护 proof 必须优先保留；随后私有 stdout 对整数键 Hashtable 的 JSON 序列化异常不能反推世界未写、不得触发重放或补交 save。展示层应只投影标量、proof叶名与显式字符串键净额，守恒内部仍可使用整数键。当前FactoryConfiguration包含buffer：4942/2203的瞬时generation **322→294** 会改变configuration hash，故该域的hash差异本身不等于静态配置变化，不能泛化到所有hash。4742/2011的`powerDemandPerTick`取自live consumer.requiredEnergy，随`Inserting→Sending`和progress变为**150→300**，也不得跨tick钉成静态额定；姿态、filter、有向双端、拓扑及网络成员关系仍由`Assert-AuditConfig`保全。leaf SHA-256 `D4C37D…AFC99D`以真实 complete proof、16项纯检查及真实 File 零调用 smoke 复验，Sol末审 SHA-256 `BCF12A…864451`；不把显示兼容性当 native 失败、quarantine、施工或持续供给证明。显示/证据序列化、DTO或动作终态形状变化时重验。
...
2026-09-19 / EXP-188、EXP-224、EXP-299 硅石送料固定结构预检复验（`observed`，限两条新接口与2022升级的当前 native prepare）：1976→1980、2018→2020 的两只新增2012及2022的2011→2012均获固定原生预检；两接口封存 SHA-256 `252AEB…A11CEE7`，升级封存 SHA-256 `99EBED…28275BE0`，后者为6次总调用（含1次prepare）、0 commit，R67/save71701504/accepted5不变。计划仅预算2012×3、未来返2011×1及新增1500 J/t；排除间歇氢电的条件规划口径1560000下仍条件缺7900 J/t，后续两风机另行通过才条件余2100 J/t，四聚变燃料连续性未证，A135/B120均只是额定容量。升级预检时目标为空载，不能把离线非空保全推成带货实测；同一 R73 的三条独立空段（6/11/14）另以8次调用、3次prepare、0写入返回`full_path_stage1`，原证明 `2AAC52…6EA51EC`、Sol复核 `51414C…58CDA9B`；它们仍不覆盖4 NEW source-cover、2 NEW L3 dual-cover或4个附件。施工、持续电力/燃料与持续产率仍须在实际阶段后fresh复验。[当档边界](./gameplay-timeline.md#2026-09-19--硅石送料的固定结构原生预检通过施工仍待)；接口、材料、功率或场景变化时重验。

2026-09-19 / EXP-188、EXP-224、EXP-299 硅石串行额定边界复验（`validated`，限当前 Core 计时和该条已读路由）：600 tick 窗口的远端 30/min 与高纯硅 12/min 都是短窗观测，不能唯一归因；但 1981→2022→2030 的串行额定为 45/30/90 硅石/min，当前硬上限为 30/min。两只普通分拣器若均升现有快速等级，路由仍仅 60/min，低于双炉 120/min 设计需求，须另行证明并行/容量变更，不能用更多短窗观察消除该缺口。[短窗分析与容量证明](./gameplay-timeline.md#2026-09-19--高纯硅上游存在额定串行瓶颈短窗不归因)分别为 SHA-256 `3AF65B…9506A3B`、`84631B…C4DF603`；不证明持续供给、唯一故障因果或施工可行性。现场路由、计时规则或设计需求变化时重验。

2026-09-19 / EXP-077、EXP-299 单炉选址门复验（`validated`限当前源码与三条本机只读回执）：Foundry包围球间距只比较本次新计划机器；单炉对既有工厂仍走普通放置的有向collider体积检查及DSP原生检查，不能把纸面包围球相交当原生拒绝。两个紧邻炉位分别被既有4460/4668占位挡住，停止同类候选；西侧位置只到`NotEnoughItem`、缺1炉，不能据“未发现重叠”宣布地形/科技/全场放置通过。正常制炉后须fresh完整预检，输送、附件、材料与持续电力另验，不拆改成功线、不放松任何原生门。[三条原回执、tick与SHA](./research/game-api-foundry.md#machine-site-rejection-boundaries-2026-09-19-local-read-only-evidence)已离线逐一核验，回放零游戏调用；不证明施工或产率。最近复验同日，场景、姿态、材料或安装适配器变化时重验。

2026-09-19 / EXP-299、IFX-121 后置日记断言与 revision 续接复验（`validated`，限当前私有手搓后缀）：`Assert-FusionStartupCraftJournal` 已正确把历史排除视为合法的 0 或 1 条新事件；调用方不得在其后再强制计数＋1。旧 R62 仅是调用方最后本地已知值；续接必须绑定实际 fresh boundary，并在 craft、save 后读取严格前进的实际 revision，不能推算＋1。续接精确保全 J91 身份/完整前缀、durable/零 pending/error；只允许对应 `2302` 的 0 或 1 条新增 durable 首件事件，不能补写/迟到 `1202`。本次 `r56×1 → normal save` 已在 R67/save71701504 唯一成功并经 61+4 只读比对；已 accepted 的 Move、取料和 r6 固定为不可重放前缀。首次比较误以较早材料封窗作 r56 前玩家基线，只离线更正并补四个未读项目，未重采工厂或产生游戏异常。见[本档完成/边界记录](./gameplay-timeline.md#2026-09-19--高纯硅炉-r561-后缀与正常保存完成施工仍待)；不证明新炉施工、持续供给或恢复，入口/DTO/现场变化时重验。

2026-09-19 / EXP-299 只下放root职责（`observed`，效果待验）：用户纠正了把Terra工作转给Sol也算“下放”的草案。现仅将root原有的入口审核、正常阶段关键原回执独立验收、普通产品代码评审/非安全缺陷定位、验收表述与下一工序依赖检查交Sol；Terra的实现、计算、固定测试、一次取证/确定性全量比对及文档草稿不变，Luna仍是唯一writer。Sol替代root原审核位置，不重复Terra整批工作，root也不再完整重审；作者不得自签。root保留核心安全、unknown/quarantine、三轮未解、最终版本门及Git/CI；十写冻结/绿CI/root封存不变。调整前短窗口为本地9月18日23:32:29–19日00:23:13，未缓存输入＋输出占比root24.41%、Sol9.05%、Terra65.70%、Luna0.84%；两者合计33.46%，总量仍约108万/小时，不构成节省证明。沿用root10–15%、Sol10–20%、合计≤35%的试行目标，不为比例上移任务；下一批2–3个闭合工序同时复核总用量/耗时/返工。三轮失败已转root的布局不重开Sol第四轮。具体边界见[AGENTS现行职责](../AGENTS.md)，最近复核同日。
...
2026-09-18 / EXP-188、EXP-224、EXP-246、EXP-253、EXP-299复验（`validated`限本次原生升级/保存/独立批审）：2030送料由额定45提升至90/min，实际ID不变仅为本次观察，升级瞬间为空载；同步原生保全与稍后自然流动物料分开核验。6紫糖/min共享硅需求必须拆成48＋12，不把各支名义容量相加当可分配供给。Move的not_detected不能替代末端干地证据，按实际点距算连续后缀（本次6点4.995m），只核并提交同一fresh计划。执行前70项Core、真实入口及8项纯检查通过，实机五动作和完整5114对象独立审计见[本档记录](./gameplay-timeline.md#2026-09-18--硅石送料分拣器原生升级并保存五动作独立审计通过)。准备墙钟与13.899秒执行分别记录；私有入口复发IFX-160/168不能记为原生失败或让成功前缀重放。最近复验同日，现场/目录/调用方式变化重验；持续产率、重启与新炉接口另验，accepted5保留。

2026-09-18 / EXP-188、EXP-224、EXP-299硅送料预算复验（`validated`限现有目录、原生计时读数和Core算术）：`ProgressRequired`对分拣器对应native stt，2030的400000按现有BasicSorterCycleTicks规则为45/min，不能只核炉30/min而漏掉其60/min硅石输入需求。先算每条实际输送边，再选升级/增建；当前目录r88一次产两只高速，按正常返还计算后续可用材料，不能凭名称猜配方。证据与6/min预声明见[本档核算](./gameplay-timeline.md#2026-09-18--六紫糖目标核算硅石入炉输送上限已定位)。跨星球来料不能拿本星生产0替代，名义矿速/站存量也不是持续供给；瞬时火电容量不替代持续燃料证明。最近复验同日，实际升级、完整物流、持续产能和保存恢复另验，旧Governor门不重做。

2026-09-18 / EXP-237、EXP-299导入隔离复验（`validated`限本地审计helper）：同名参数污染已能用零游戏检查复现，私有helper内部改任务专属参数并保留旧命令参数别名，7项兼容/可见性检查通过，见IFX-079。这只消除该一个公共变量名的覆盖，不能据此取消其他调用者的导入后绑定或将所有私有入口称为安全；最近复验同日，helper接口/导入链变化时重验。不改变游戏动作、审计要求或公开工具。

2026-09-18 / EXP-242、EXP-299、EXP-303出口激活与十写复验（`validated`限两端正常施工、到仓、保存和完整批审）：目的先接、源最后，合接后的实际端点重新prepare，两只6004过滤分拣器保持十三带原路径及已建输入。十终态、5114实体/9968边、完整库存与J91独立核验见[本档出口激活](./gameplay-timeline.md#2026-09-18--紫糖出口接通并保存十写独立审计通过)。仓内73件只证明到货；供给不足时不据此签持续生产或最终科研消费。最近复验同日，accepted10仍须绿CI及明确封窗后才归零。IFX-079再次说明任务参数应在helper导入后赋值；外层执行转发须保留完整返回对象/进程句柄，空stdout不证明命令未执行，应先核精确落盘证据，不能重采或重放。现场、契约或程序集变化仍须fresh核验。

2026-09-18 / EXP-242、EXP-299、EXP-303合接复验（`validated`限当前固定七NEW双cover施工/保存）：源3＋新7＋目的3可保全身份并合为13成员空载路径，旧两cover端原生朝向变化与插件终态、后续完整工厂配置一致；不能把整个旧厂rotation都豁免。复用本次施工终态后的13份详情，不重复采同一组实体，独立核全部5112对象/9960边，见[本档合接](./gameplay-timeline.md#2026-09-18--紫糖出口中间合接保存十三段线路独立核验通过)。最近复验同日；accepted7保留，合接改变端点几何后附件必须重新预检，目的先接、源最后；不据空载合路宣布供料、持续量产或恢复通过。

2026-09-18 / EXP-242、EXP-299、EXP-303目的短段复验（`validated`限正常施工/保存、实际附件及双cover预检）：目的5105→5104→5103在材料与全厂独立核验后保持空载，真实tail→仓slot4通过native exact_slots；后续5102→5105的7NEW合接另做完整原生预检，不能用两短段许可代替。详见[目的段与证据边界](./gameplay-timeline.md#2026-09-18--紫糖出口目的段保存中间合接预检通过)。实体ID顺序、旧action、详情数量不能靠裸数字全局替换；读取失败保留成功前缀，只补未完成采集。最近复验同日；accepted5不归零，合接后仍需fresh实际附件/供电与真实供料，缓冲仓到货不等于最终消费或持续生产；现场/输入或程序集改变重新验证，不删安全检查以缩短时间。

2026-09-18 / EXP-242、EXP-299、EXP-303最小源段复验（`validated`限3NEW普通施工/保存及真实附件预检）：先建5100→5101→5102，立即用真实head证明lab slot0可接，再扩大依赖线路的顺序在本场景通过；附件仍不提交，保留后续空载双cover条件。独立全5102实体/9940边审计、材料和三终态见[当档源段](./gameplay-timeline.md#2026-09-18--紫糖出口源段保存真实实验室附件预检通过)。recipe批数与产物数分开报告；当前执行分支封存Generic.List用ToArray并做真实序列化smoke，自由路径同时核无目标复用。最近复验同日；accepted3不清零，目的端、合接后重验附件/供电及持续供给另验，现场或执行入口变化仍fresh预检。

2026-09-18 / EXP-242、EXP-299、EXP-303出口前缀与十写复验（`validated`限两段native预览、正常补料保存及完整批审）：目的短段缩短一格可改变中间轨迹，但两端native通过仍不授权整路或未来附件；下一按最小源段→真实接口预检推进。accepted8时把仅剩两次用于3铁取料与正常保存，达到10后严格冻结；全窗材料、5099实体/9936边、107详情和十终态独立通过，见[本档证据](./gameplay-timeline.md#2026-09-18--出口补料保存与十写审计通过两端短带原生预检通过)。采集参数必须在导入后用不冲突名称绑定；缺失DTO字段不能以`@($null).Count`推断现场pending。只读本地失败保留已完成原回执，补未读后缀，不重采快照或把本地错误称为native失败。最近复验同日；关联IFX-168，持续产量/重启未验，现场、契约、材料或程序集变化后fresh复验，绿CI与明确封窗前不归零。

2026-09-18 / EXP-188、EXP-224、EXP-299硅链供需复验（`validated`限当前拓扑和基础容量算术）：旧线路通畅、仓储预约恢复与单点working仍可能同时存在上游容量不足。独立逐槽核完整248节点/247边及842的微晶/晶格硅两支后，结合已观察速度与配方得单炉30高纯硅/分、两消费者满负需求75/分；目标紫糖的两支都必须展开，不漏掉宽带经晶格硅消耗的同种共享上游，基础供料上界3紫糖/分不是实际产率或目标达标。见[供需证据](./gameplay-timeline.md#2026-09-18--硅链连接完整确认高纯硅共享产能缺口)。范围计数须明确是否含消费者；历史目录可支持固定配方，不能支持当前unlock准入，未知lab速度保持未知。最近复验同日；当前负载、增产、配方/设备/输入来源或程序集改变时重算，再以实际生产、消费、库存及独立窗口验收，不据理论上限直接扩产或宣布配平。

2026-09-18 / EXP-242、EXP-299、EXP-303出口布局停止边界复验（`validated`限有限离线否决和升级分工）：完整带路native许可不证明未来附件可接；设备朝向优先的两端各自清空，也不证明中段无碰撞。三轮出口方案按上限停止，root复用已有Core/helper独立核四端与中段共5项一致，见[布局blocker](./gameplay-timeline.md#2026-09-18--紫糖出口三轮布局收口主会话接手)；近似CargoPath角度不是原生失败，未穷尽全部路线。摘要功率必须按真实J/t标注并乘60转W，原证据文件名必须精确；只离线更正，不重采或增加现场失败次数。最近复验同日；accepted8保留，改由root设计，不让执行者盲试第四轮或用新仓掩盖几何问题。新方案仍受原生、材料、双端与保存/十写边界约束。

2026-09-18 / EXP-224、EXP-299出口备料与诊断解释复验（`validated`限当前三动作、独立核验和原生带路预检）：复用既有入口取12铁、递归制造12带、保存70419300，accepted8保留；全5099实体配置不变，30执行回复与一次77读供独立核验，见[本档材料与证据边界](./gameplay-timeline.md#2026-09-18--紫糖出口材料正常制造保存完整带路预检通过)。源仓持续补货只与同步terminal守恒分开解释，不能要求后读等于扣后库存。材料齐后必须fresh完整预检；12NEW带的stage1通过仍不能代替未来附件可接性。诊断关联不同DTO前先核索引语义：站beltSlot.StorageIndex原样保留native一基值（0未选），storageSlots.Index/配置请求为零基；不得按同名字段直接关联，或将有界追踪截断称为完整链。1793错槽假设已撤销，未改正确配置；这不是硅石持续供给证明。最近复验同日；关联IFX-168，字段契约/现场/安装变化时重新核验，不新增重复框架或降低十写边界。

2026-09-18 / EXP-221、EXP-224、EXP-299共享硅仓复验（`validated`限配置修复/即时守恒及正常保存）：843保留五边和晶格硅不代表高纯硅仍可入仓，bans29与唯一1113预约实际切断旧1105输入。先用原生完整投影证明保留占用首格、预约全部29空格1105，再以bans28只开放前两格；两动作即时有序库存/玩家不变，跨tick允许可解释的正常物流。相关48项测试、三终态和独立5099实体/9936边审计通过，见[本档修复与证据边界](./gameplay-timeline.md#2026-09-18--共享硅仓重新开放高纯硅输入并正常保存)。现行经验仍要求改共享仓前枚举所有需要继续服务的输入/输出及合法可用格，不新增泛用工具；本次微晶工作和处理器收到1微晶不证明持续硅石供给。最近复验同日；普通保存70330468已覆盖首紫糖，accepted5保留，连续产量/恢复另验。关联IFX-169；原生过滤规则、现场、库存、输入集合或安装变化时fresh复验。

2026-09-18 / EXP-242、EXP-299、EXP-303源最后与首产复验（`validated`限正常接源/保存及首批到货，不含持续产能）：854.slot7→5021的分拣器5099正常施工、实际带槽4、网3满供；两端connections和原生终态可证明互返边，分拣器自身unavailable的sorterEndpoints不能冒充可用证据。源激活后Journal从90增至91，保全旧durable条目而不强求序号不变；逐事件tick与primary save tick分别记录，后发生的首产不属于早先保存。见[接源、首批紫糖与供给缺口](./gameplay-timeline.md#2026-09-18--处理器高架接源保存首次自动紫糖已观察到)。最近复验同日；验收连续产能前同时核成品去路和上游实际生产，不能以目标缓冲10件或耗历史库存代替；当前缺出口/硅原料，不启动注定不合格的稳定窗。accepted2保留，重启与持续生产仍待，新现场按原门fresh复验。

2026-09-18 / EXP-242、EXP-299、EXP-303实验室接线与十写复验（`validated`限本次正常施工/保存/批审）：把原生preview的实际位置纳入第二次fresh prepare的提交前比较，不能只在扣料后发现供电见证已失效；完工另核真实net/ratio与双向槽。5022虚拟槽−1实际落slot4，lab slot2持有5098，旧slot1/5000不变。末尾working1但pending0、前build已终态时正常保存成功，再次支持返航不等于未完成施工。十写收集159个原回复供独立5098实体/9932互返边及10终态核验，不重复采集；完整事实与证据见[当档接线与批审](./gameplay-timeline.md#2026-09-18--处理器实验室接线保存与十写审计通过)。最近复验同日；accepted10仍待推送/CI后明确交接，854源最后连接前须fresh现场/预算预检，当前空载路径不外推持续供给或恢复验收。

2026-09-18 / EXP-242、EXP-299、EXP-303高位合接复验（`validated`限当前DLL的固定空载双cover正常施工/保存）：两个已建坡段可在实际同高度平接端以既有native_grid另做完整预检；不能把native_elevated_grid的单跨度限额混用于该绑定路径，也不能去掉端点绑定冒充空地。5010→5030实际67NEW已正常建成，与原21条组成唯一88成员空载开放路径，源输入保持最后施工。新实体ID不保证沿路径递增或递减，按原生计划点唯一映射并核互返边；同一终态后的67份新带详情可复用于完整88成员核验，独立整厂快照另证静态一致，不要求再次采相同详情。见[合接/保存/独立审计](./gameplay-timeline.md#2026-09-18--处理器高架水平合接施工保存与独立核验通过)。最近复验同日；accepted8保留，附件/持续供料/重启仍待，各后缀fresh预检。IFX-168的调用层字段/标量错误仍只属mitigated，不能据本次成功推定全部私有调用方已消除该问题。
...
2026-09-18 / EXP-001、EXP-002、EXP-299、EXP-303部署恢复复验（`validated`限2649aad同批安装与同档十写审计）：228文件逐一同源hash、Steam单次启动、真实MCP64/1和新Plugin认证闭环；一次exact-primary恢复至69694711/J90，原宽带源及全部5009实体/9754互返边/64带保留，十个accepted逐项核销后才封存计数窗。见[本档完整证据与哈希](./gameplay-timeline.md#2026-09-18--高架适配冷部署与同档恢复十写审计通过)。恢复后玩家位置允许既有0.0001m浮点边界，实体配置/材料/Journal不放宽；辅助恢复入口无显式execute先于导入拒绝。最近复验同日；新DLL、部署批次或恢复触发重验。此结论不外推具体坡段、高位合接、持续供料、最终包或异机实测。

2026-09-18 / EXP-299、EXP-303调用字段及部署前复验（`validated`限本次保存与独立原证据核验）：数值0不能经`@(...).Count`判为非零任务；完成tick读action的`completedAtGameTick`，存档水位读session的`lastOwnedSaveGameTick`，模式比较用实际契约枚举，不猜字段或缩写。辅助摘要失败在原回复已受保护落盘时仅离线补齐，不重采/重放；本次128回复供独立核5009实体、9754互返边、70详情及完整J90，见[部署前日记](./gameplay-timeline.md#2026-09-18--高架适配部署前保存与独立核验)、IFX-168。保存69694680/accepted9保留；尚非冷部署、恢复或持续供料通过。最近复验同日；DTO、调用入口或现场变化触发对应重验，不能把这些辅助错误归为原生场地失败。

2026-09-18 / EXP-299、EXP-303显式高度实现复核（`validated`限当前DLL/离线源码与测试；live待）：一次原生坡段须先证明active grid、Snap实际层级、完整端点和短段清理no-op，再完整stage1；不能靠拼接三次结果或覆盖末点掩盖截断，也不能将只对两点适用的.5短段门误加到4点以上路径（应为.28f及相同float运算）。原生预建筑旋转是球面基，完工pitch来自实际3D CargoPath，分阶段证明，不能互比或只看拓扑。空路径cover helper允许外部消费者/声明槽变化，新建free坡段必须额外收紧所有16槽及精确成员；旧cover条件不放宽。见[源码切片与验证层级](./gameplay-timeline.md#2026-09-18--显式单跨度高架带源码闭环实机待验)、IFX-167；最近复验同日。cold deploy、具体场地/材料/科技、水平合接、送料及保存恢复仍须实机证明，不能从58项Core合成测试推定整桥可行；DLL/模式/现场变化触发重验。

2026-09-18 / EXP-299、EXP-303处理器能力边界复核（`observed`限当前DLL/源码/固定离线否决）：地表候选反复失败后先核当前动作族；自由端onTerrain吸附会丢失输入高度，等半径cover不是坡段能力，解析蓝图tilt也不表示现场存在可复制模块。三次SnapLine拼接未被原生单次调用证明，不能据每段点数通过直接签发整桥token。采用显式、全NEW、单次原生跨度研究，保留后续合接与实机门，见[固定路线与研究边界](./gameplay-timeline.md#2026-09-18--处理器固定路线停止并定位显式高架能力缺口)。无新施工，accepted8/保存69380216不变；安装200b4bc不支持该新模式，不把三轮停止当作穷尽所有地表路线。最近复验同日；DLL、方案或现场变化时重新审查，未完成原生/部署证明前不交Luna施工。

2026-09-18 / EXP-299、EXP-303源接料及动态审计复验（`validated`限5009正常施工/保存与1402真实到货）：2255槽10→5005虚拟端正常生成实际slot4，不能把计划−1与真实槽号混为一谈；lab4743读到1402×6，但1303/6004均零，不能宣称紫糖或持续产量通过。127条原回复一次采集后供独立核5009实体/9754互返边、70详情和64带，见[本档阶段证据](./gameplay-timeline.md#2026-09-18--宽带源接入保存与实验室到货通过)。原生`energyCapacity`逐tick重算，不是静态配置；节点/发电机/消费者与配置严格核对，动态量按当前供需、全负载预算和声明窗口核验，不要求跨时点逐值相等，不凭增加值臆断原因或持续余量。manifest本地字段错误只恢复已落盘原证据，不重采全厂，实际字段`lastOwnedSaveGameTick`未变。IFX-166；最近复验同日。保存69380216/accepted8，持续供料、燃料与重启恢复仍待；现场、DLL、端点或材料变化须fresh完整预检。

2026-09-18 / EXP-299、EXP-303施工后快照复核（`validated`限C1双带合接与保存）：完工新边必须以after快照验证，before只用于核原配置/旧边；调用方错用旧快照的异常不等于原生失败。先核原action和fresh唯一实体/双边、材料与健康状态，再只执行尚未完成的保存，不重放已成功前缀。126次只读的一次证据集可供主会话独立核5008实体/9750互返边、69详情和精确42成员路径；不把辅助封存参数错误当游戏失败或重采全厂理由。[阶段事实及证明](./gameplay-timeline.md#2026-09-18--宽带入口双带合接保存与独立核验通过)、IFX-165。合接后源接口须重新prepare，预检通过仍不是送料；保存69297240/accepted6保留。最近复验同日；实际朝向/端点或现场变化、非唯一读回、unknown/quarantine仍停止并独立核销，不放宽原生路径/collider证明。

2026-09-18 / EXP-299、EXP-303有限入口与证据续核（`validated`限C1四NEW正常施工/保存、源接口预检）：先施工已核首段并用真实head5005预检，2255槽10以exact0/0首次通过；保持未送料，后续双cover通过后仍须重新取计划。[本档阶段证据](./gameplay-timeline.md#2026-09-18--宽带入口四带施工保存与真实源接口通过)。调用方丢失进程内分页变量不等于原证据丢失：审计Bridge已逐回复落盘，按确切prefix恢复，再只补未完成后缀；root独立核原页与新详情，不重读全厂或重跑已成功施工。保存请求须用fresh实际revision，不以accepted代替；缺参prepare拒绝只修保存后缀。最近复验同日，尚不证明源料接通/持续产出或重启恢复。

2026-09-18 / EXP-188、EXP-299、EXP-303源入口朝向复核（`validated`限原生路径回执与离线否决）：free-source→target-cover的grid吸附可改变第二NEW及首带切向，必须先核actual plannedPath，再重算未来分拣器，不沿用旧纸面角度。固定B原生带路通过但入口seed先验拒绝，故零施工；唯一C1的4NEW原生预检符合预定转向，尚不构成后续双cover或分拣器许可。[事实与边界](./gameplay-timeline.md#2026-09-18--宽带源入口先验否决与四带短段原生预检)。无输入/空载合接要求意味着源分拣器只能先prepare、不commit，合接后重新取得真实路径与新计划。最近复验同日；原生点、拓扑或现场变化须重新核验。

2026-09-18 / EXP-299、EXP-303执行准备延迟复核（`observed`限本次墙钟证据）：方案已定的Luna任务曾超过七分钟仍零游戏请求，固定短命令接手后才进入9.8458582秒建设段。[时间边界与改进](./gameplay-timeline.md#2026-09-18--成熟执行步骤的准备耗时单独限界)。准备延迟必须计入工序，不能只报工具运行秒数；规范现以2分钟回报、5分钟无必要新证据交回准备，先核后台命令/意图/accepted，不重放。阈值是流程约束而非原生超时，后续用同类完整工序检验，暂不宣称整体提速比例。最近复验同日；执行已进入游戏、结果未知或恢复加载时不套准备限界。

2026-09-18 / EXP-299、EXP-303双空带路合接实机复验（`validated`限本机正常施工、材料与保存）：安装态`200b4bc`以fresh双cover/geodesic完整预检和普通无人机施工，将4984→5002→5001→4978接为精确36成员开放空路径，仅扣2001×2。完整阶段原回执证明5002实体/9738互返边/58空带，旧路径中仅4984/4978发生可归因的原生rotation变化；不能继续套只准源/前驱旋转的旧source-only审计器，也不能因此忽略其他旧实体朝向。Terra采集一次完整证据，主会话独立离线复核同一原始集；普通保存69115512、R4、accepted2保留。[存档证据](./gameplay-timeline.md#2026-09-18--双空带路首次正常合接并保存)。源仓未接、持续产出与合接后的恢复仍待，不以施工成功代替业务送料。最近复验同日；现场/路径、DLL/cohort或外部输入变化须重新预检。
...
2026-09-18 / EXP-299、EXP-303接点拓扑替换核算（`validated`限离线停止边界）：当原1只源分拣器改为2只时，必须以净替换差额重算中桥和处理器预留，不能把新增600 J/t塞进含旧300的历史余量。被否决B的条件总数为8只、2400 J/t，容量余10400但库存7只短1；它既非当前负载也不授权施工。A/B均为0游戏、非原生近似且无完整方案，见[两接口离线停止](./gameplay-timeline.md#2026-09-18--两接口离线替代无完整方案成功前缀冻结)；保留56带与4983/5000，下一只能重设计最小入口/中继并fresh核验。最近复验同日。

2026-09-18 / EXP-299、EXP-303实验室附件实机复验（`validated`限4964→4743 slot1/filter1402）：`exact_slots` offset0/0可正常施工2011 **5000**、双端读回与普通保存；recipe55及4743 slot2给1303的保留均在独立审计后保持。该阶段只扣一只2011，背包42条带与全部旧4999实体保留，网3的条件余量仍为10700，见[本档实验室附件通过](./gameplay-timeline.md#2026-09-18--宽带实验室附件正常施工保存与独立审计通过)。它不证明料流、持续生产/燃料或重启恢复；两处上游pair禁止重放且9+3邻段无候选，下一连接方案须重新设计最小入口/中继并fresh预检。最近复验同日。

2026-09-18 / EXP-299、EXP-303宽带part0精确链施工复验（`validated`限4999→…→4984的16 NEW与普通保存/审计）：belt-only cargo检查必须绑定明确成员集**4943..4982**，不能以`id <= 4982`替代；后者会误纳2255、4743、854。该守卫在Bridge前、0游戏调用时修正，因此不是原生负例。随后正常施工、16带物料读回、56开放空带和旧4983双带桥配置保持均获独立审计，见[本档part0通过](./gameplay-timeline.md#2026-09-18--宽带-part0-的16条带正常施工保存与独立审计通过)。后置两附件已分别得到无facing pair/无finite projection的零写拒绝，进一步证明带路可建不等于附件可接；不证明送料、持续生产/燃料或重启恢复。保留成功前缀，仅重设计未完成连接；新几何仍须fresh native预检，禁止重放两组失败端点。最近复验同日。

2026-09-18 / EXP-188、EXP-299、EXP-303双带内段正常施工复验（`validated`限4981→4947这一精确桥接）：`native_two_belt_segments`可按正常2011施工、双端读回和普通保存；安装态完成回执可核绑定的offset3/0与pose，而公开inspect DTO不暴露offset，不能臆造公共观察字段。独立审计确认材料仅2011 9→8、新4983及双向边，40带/旧配置/网4保持，见[本档桥接通过](./gameplay-timeline.md#2026-09-18--双带内段桥接正常施工保存与独立审计通过)。此正例不证明送料、持续产出、燃料连续性或重启恢复；下一新段/附件仍须fresh native预检，不能重放旧失败端点或已完成前缀。最近复验同日。

2026-09-18 / EXP-188、EXP-299、EXP-303已建内段接点正例（`validated`限完整原生prepare与功率见证，施工待验）：补齐Core seed前置后，四个NEW对旧入口4949均未取得纸面可行；对明确选定的4947/4945内段做八对离线检查，root只选择最近4981→4947，fresh返回`native_two_belt_segments`、signed offset3/0、filter1402及正常2011×1预算。未隐式换带、未重试原失败对，也未再造中继。功率见证分别核实际点和吸附代理，网3按全部九附件满负载保留10700余量；不能按瞬时需求或全星球总容量替代。见[本档首次原生正例](./gameplay-timeline.md#2026-09-18--内段双带-fallback-首次原生预检正例尚未施工)。最近复验同日；实际材料/无人机施工、偏移/双端/供电读回、持续送料和保存恢复仍需各自证明，所有执行必须重新prepare。

2026-09-18 / EXP-188、EXP-299、EXP-303前置准入经验产品化（`validated`限当前原生顺序及源码指南/离线回归）：真实4981→4949的折叠朝向42.23°/对向26.76°在DSP和Core都未满足<40°seed门。此前约8.18°的私有插值结果位于不可达分支，不能再用来规划依赖施工；核验顺序必须是实际quarter-turn→folded seed→destination/source投影→最终朝向/tilt→完整native placement。playbook已明确这一顺序、最小接口验证片和停止同目标重试；66 Core＋1 MCP定向Release通过，运行c5f3542未替换、新指南待后续同批部署。不新增读字段/动作、不改角度阈值；最近复验同日，关联IFX-162与本档四格阶段。

2026-09-18 / EXP-188、EXP-299、EXP-303真实转角复验（`validated`限四NEW正常施工/保存，附件仍负例）：Luna复用同一入口27.80秒完成四带及保存，root完整4982实体/9694互返边/40空带与材料/日记审计通过，见[本档四格阶段](./gameplay-timeline.md#2026-09-18--宽带四格南转通过完整预检先验证转角再扩展)。真实4981→4949却在`no_admissible_seed`停止，原生放置尚未调用；不能由曲线中存在近似<11°插值点跳过种子姿态的原生准入，也不能把带路通过、纸面最优角或正常保存称为附件/生产成功。先核真实seed及精确插值，再考虑明确未完成后缀；保留成功40带，不重放相同端点或先铺依赖上游。27.80秒仅该执行脚本，不含规划/审核，不是总开发提速倍数。最近复验同日；真实姿态、段形、安装或材料变化重新预检。

2026-09-18 / EXP-188、EXP-299、EXP-303转角最小验证片（`validated`限四NEW带路原生预检）：原直带接点无有限投影后，先按当前原生Bezier规则比较固定局部南/北转，只有南转进入完整带路预检，见[本档四格预检](./gameplay-timeline.md#2026-09-18--宽带四格南转通过完整预检先验证转角再扩展)。纸面方向插值不是原生Quaternion.Slerp，也不替代真实角段附件；候选通过只授权有限正常施工、保存和实际接点复验，不授权依赖上游16带。明确选择第二NEW角段，不将尾端或邻带自动替代。全网总容量与选中网分量须区分，满基础预约也不同于瞬时需求；持续燃料仍另验。最近复验同日；真实对象、段形、现场、材料或安装变化后重新预检。
...
2026-09-18 / EXP-299、EXP-303观察开销收敛（`validated`限离线调用方；实机新节奏待验）：真实14带批次735回复中625次为同action查询，固定250ms轮询对长施工过密。共享Wait改为250/500/1000/2000ms封顶且不越原deadline，保留同action、终态、单次prepare/commit和异常核销；54项PowerShell5/7离线检查及9项Bridge检查通过，见[IFX-161](./incident-fix-log.md#ifx-161--长施工期间共享调用方固定高频轮询)。不得把少查询当作无人机或整体进度加速。另一零写、仅首尾各一次session读取的30.103秒窗口推进473tick（15.713tick/实际秒），只能证明该窗口有效游戏时间推进偏慢，不能区分后台限速、负载或其他原因，亦不授权修改游戏倍率。最近复验同日；后续分别测实际调用量、工序墙钟和game tick，不能由一次窗口外推所有阶段。

2026-09-18 / EXP-299、EXP-303单线工序复用（`validated`限离线边界/几何）：剩余宽带14/16带共用同一受限执行分支，由新封存数据限定先1后0、每片一次build及一次save；不能通过重用旧seal、改变索引或重放完整线路继续。326纯检查、真实shell/全参数/导入smoke及原保护数据边界检查通过，未改14/16同action观察上限。固定两段30NEW对最新4964实体及真实两格转向的Core占位复算通过，材料76带/9分拣器足够余30/4；原纸面1547800只含四宽带附件，执行仍fresh核包括处理器预留的1549300条件预算，不冒充持续燃料。保存证明先落盘，再预检实际桥；附件未过时保留已保存成功前缀、停止依赖线路。最近复验同日，本段不是新施工或附件实机许可；现场、材料、依赖桥或封存边界变化时重新验证，不为下一段再复制执行/审计框架。

同日后继实机复验（EXP-124/259/299/303，`validated`限14带同handle观察/保存及桥接负例）：原action在初始30秒后仍正常施工，六个预建筑窗口11→9→8→5→3→2后完成，不把调用方等待边界当原生失败，也不重放。14NEW材料、全4978实体独立核验及save67943806通过；随后4965→4949首次真实桥预检在16候选/0 nativeChecks处TooSkew拒绝，已保存前缀保留，见[本档中段结果](./gameplay-timeline.md#2026-09-18--宽带中段十四带保存同action观察通过真实桥接方向拒绝)。geodesic路径端点的实际朝向仍不能由距离/Core无占位替代；belt-device专用fallback的unsupported_endpoint_family不代表exact-slot的belt-belt能力不存在。桥未过时下一单线入口必须拒绝part0，不能因完整材料或上一条lab附件正例放行。最近复验同日；16带观察、附件施工、持续供给和恢复未验证，不把212.48秒执行段当全流程提速。

2026-09-18 / EXP-188、EXP-299有限续接保存顺序（`validated`限离线调用方）：复用既有送料执行器，窄模式只准4962的2NEW source-cover及一次普通保存；完整原20带保留，只有原源/直接前驱在产品精确native旋转证明范围内可变朝向。299送料纯检查（原276+23）、21共享客户端及实际shell/参数/导入smoke通过，0游戏调用。原action观察只为这项精确两目标计划增加显式准入，原14/16及各时间/进展上限不变。构建与材料/旧链读回通过后先保存并写独立保护证明，再作真实尾端到lab槽1的只读预检；后者失败必须引用已保存前缀，不能让失败覆盖成功证据或触发整批重放。两目标终态测试首次漏填completed，修正的是合成夹具，不是游戏/执行器故障。最近复验同日；本段只记录施工前离线检查，后继实机结果见下。

同日后继实机复验（EXP-188/208/299/303，`validated`限两NEW施工、旧链保全、保存和真实附件预检）：4963/4964正常施工，仅扣两带，原源和直接前驱朝向与产品原生证明相符，其余旧配置/货物及双向边保持；save67906656/R244/J90后，4964→lab4743槽1真实预检通过。完整4964实体独立核验及[本档两格闭环](./gameplay-timeline.md#2026-09-18--宽带两格转向正常施工保存真实尾端附件预检通过)证明最小施工取得真实端点后再扩大依赖线路的顺序可行，不是允许绕过原生方向门或重放失败端点。分拣器尚未施工、持续产出/恢复另验；不能从本次成功推断同handle超时分支实际触发。预检证明至独立审计约24分06秒，而执行至保存35.83秒、审计30.96秒；后两者不能代替端到端效率。后续复用稳定执行/审计机制，现场变化仅替换明确计划数据并fresh预检；最近复验同日，源身份/货物/朝向、现场、材料或当前DLL变化时重验。

2026-09-18 / EXP-188、EXP-208、EXP-299、EXP-303有限转向预检复验（`validated`仅source-cover native预检）：7只读/prepare调用证明4962可保留旧4961入边、正常续接2NEW，完整路径、仅扣两带预算及whole-path-native-rotation显式回传成立，见[本档预检](./gameplay-timeline.md#2026-09-18--宽带尾端两格转向通过原生预检尚未施工)。不是再次尝试原失败分拣器端点对，也不是允许忽略旧源旋转/货物或拆建旧带；施工后仍须唯一实体、材料与拓扑读回，再对真实尾端fresh核附件。新纸面证据同样应复用既有保护writer；继承的仅当前用户ACL不满足显式protected守卫，先核内容与授权、只收紧精确文件元数据，不改原证明内容。最近复验同日，0commit、持续供料与恢复另验。

2026-09-18 / EXP-124、EXP-259、EXP-303末端准入复验（`validated`限真实拒绝）：宽带20带完成保存后，首次实际4962→4743槽0预检返回BUILD_CONNECTION_INVALID；48精确槽候选、0 nativeChecks、0 facingPairs，停在no_finite_projection/TooSkew。这是端点朝向的预检查拒绝，不是OutOfReach，也不否认先前三条带路本身的原生预检。40调用/0commit，已建20带与primary67827487/R241/J90保持，见[本档负例](./gameplay-timeline.md#2026-09-18--宽带真实尾端附件拒绝入口修复未产生新施工)。整路布局必须前置核最难附件，先用最小有限片取得真实端点并验证，再延伸依赖线路；不能建完全部孤立带后才试连接。现有<11°原生方向界不放宽、原对不重放；新增两带source-cover只是纸面候选，仍须native全路预检和精确旧源保全。最近复验同日，持续宽带/紫糖未通过。

2026-09-18 / EXP-299、EXP-303入口与等待机制复验（`validated`限当前私有入口和离线观察器）：null/未定义自动参数使两次真实启动在Bridge前拒绝；按[IFX-160](./incident-fix-log.md#ifx-160--私有执行入口把空自动参数误判为额外实参)窄修后，相同shell/参数/函数导入smoke及276+83+20+21离线检查通过。入口或导入变化要测真实启动形态，不新增全流模拟；普通本地表示错误与游戏拒绝分开计量。既有有界同handle施工观察器已适配14/16带，仅在原action上观察真实预建筑/游戏tick与终态，不重commit、不单纯放大超时；本次在首个连接预检即停止，因此该适配尚无实机运行证据。最近复验同日，新现场与实际施工仍必须fresh检查。

2026-09-18 / EXP-303有限规划复验（`validated`限离线范围与停止边界）：缓存相同路径/场景计算后，两轮各60完整组合的Core段分别约6.960/11.091秒；较快计算仍全部未通过，不能代替路线质量或宣称总流程提速。辅助命令必须保留完整返回及PTY，丢失输出的候选保持未知，不盲重跑；候选前的语法错误不计游戏失败，也不重置同轮墙钟。三轮有证据设计仍失败时交主会话复查源点/走廊，不追加第四轮微调，见[本档处理器阻断](./gameplay-timeline.md#2026-09-18--处理器走廊三轮纸面方案未通过主会话接手改源或绕行分析)。私有库存上限不是产品上限，可明确列正常补料差额再比较有限替代，但不能放宽原生接口、碰撞或供电门。最近复验同日；目前零游戏调用，改源、补料及新路线均未获实机证明。

2026-09-18 / EXP-299同handle caller 截止复验（`validated`限本次已完成宽带首段）：已提交的20带在调用方180秒等待边界外仍由同一原 action 正常完成；随后的 reconciliation 与独立全场审计核销20个NEW带、唯一材料差量和保存后的4962实体/9656互返边/零预建筑，见[本档首段](./gameplay-timeline.md#2026-09-18--宽带首段20条带完成保存并独立审计)。caller 超时不等于 native 失败，既不重 prepare/commit 成功前缀，也不把约1.6秒的追加保存当作整段施工耗时。后续只能沿用既有有界同handle观察真实 prebuild/进展的规则，而非单纯放大超时；该观察器适配仍在进行、未经实机验证。最近复验同日；三线其余部分、附件接料、持续燃料与紫糖另验。

2026-09-18 / EXP-021、EXP-236、EXP-303补电完成后重新绑定送料现场（`validated`限24风保存及三带预检）：最后三风正常施工、扣料与独立完整审计通过，随后50个NEW点在最新4942实体场景上重新完整Core检查、原生吸附/跨度/路径及三次native prepare通过；实际1560000 J/t对1549300保守预算余10700，四处只读覆盖见证也重验，见[本档24风](./gameplay-timeline.md#2026-09-18--二十四台补电保存完成宽带三线重新通过完整预检)。不可仅因旧几何没改就沿用R165、旧玩家位置/库存或过期token。复用入口绑定当前完整审计/保存水位，245纯检查覆盖旧封存拒绝和身份/场景/路径/预算；纯函数与只读预览不冒充三线施工。未来真实带端连接仍在取得实体后预检，源最后接；不把发电机数量、瞬时容量或只读Foundry标为持续供给通过。最近复验同日；四聚变燃料依赖、紫糖两路输入/持续窗口及保存恢复均另验。

2026-09-18 / EXP-242、EXP-299、EXP-303返航续建实机复验（`validated`限三风有限工序）：BBBDE48C入口正常完成三build+save，第二/第三次prepare前原玩家记录均为working1、pendingBuild0/pendingRepair0，前项成功terminal、唯一实体与扣料先已验证；fresh原生准入接受后正常完工。最终readinessPending只说明保存后仍待回巢，不能替代逐次准入证据；root独立全场审计随后确认真正idle、旧配置/9618边/其他物品不变，见[本档21风](./gameplay-timeline.md#2026-09-18--补电达到二十一台返航中有限续建实机通过)。83+20纯检查复跑通过，有限工序执行101.456606秒；不能据此宣称相对不同距离批次或整条开发流程的因果提速。初始idle、稳定玩家、计数完整、零待建/维修、fresh token和未知结果停写均保留，不扩大为任意working状态可继续。最近复验同日；当前安装未改，持续燃料/满预算/紫糖另验。

2026-09-18 / EXP-021、EXP-236、EXP-246、EXP-303明确接近与部分站址准入（`validated`限一条Move/save和四个native预览）：延长2m落点的31.84m正常接近已成功保存，未重放两个不合格水边目标，见[本档接近闭合](./gameplay-timeline.md#2026-09-18--明确干地落点到达并保存先核销三个可建风机站址)。六风纸面最紧既有风距10.622650m、拟建11.040947m、连接≤16.4m、无新无供电consumer，仍不能代替完整原生建造碰撞：E−/c0r0/c1r0 native通过，c0r1却与4230重叠，后两站未预检。整组不获施工授权；可保留已满足独立供电/材料/保存条件的前三站构成有限批次，不必让不合格后缀阻塞全部进度。第三个成功站不代表第四站有效，纸面代理仅为筛选，不能调低阈值覆盖原生拒绝；后缀由Sol有界设计，成功前缀和失败原证据均保留。最近复验同日；新风施工、全24风预算和后续紫糖仍待。

2026-09-18 / EXP-246明确更远干地落点预览（`validated`仅本次prepare，Move仍待）：在R1原末点沿同一大圆方向延长2m，只评估一个新落点，实际chord31.836429m；fresh原生33点/66射线/零未知，弧长31.836838m，末端5段共4.974506m均为有效高出水面至少0.1m，终点约高水0.197040m。中心线对最近非带4642约3.593872m，1.5m正常容差和原地表门不变；带碰撞仍未证明。该结果允许对这一明确目标做有限正常尝试，不表示任意32m路线可走，不可重用此次token，也不将prepare标成Move成功。预览证明SHA-256 **8A0AE601E216D2BFB1ACF7F8FF2BF465A68ED299DF5544B4D488193FC133EA60**；最近复验同日，移后Walk/能量/库存及正常保存另验。

2026-09-18 / EXP-236、EXP-246、EXP-303分段落点必须满足地表条件（`validated`限本次预览拒绝与保存）：R1首腿正常完成，下一腿原生prepared/shoreRisk=not_detected仍不能盖过末端浅水采样(+0.119643m)，本地在commit前拒绝；原末点直达预览29.842m虽几何通过，干地后缀仅2.983883m，也零commit。不能把固定10m私有分段当成原生限制，机械切短反而可能把落点留在水边；可根据完整采样一次指定不同的更远落点，但须≤32m、末端至少4m连续高于水面0.1m、足够能量和fresh实际障碍复验，不能降低干地门或按聚合not_detected放行。入口新增显式`maximumLegMetres≤32`、缺省10，32个纯检查通过；未增加公共工具或自动寻路。负dry-suffix只是旧私有拒绝表达，不能作为负物理距离/原生错误记账。成功前缀已正常保存并独立整厂核验，详见[本档R1停点](./gameplay-timeline.md#2026-09-18--r1首段保存浅水落点与干地后缀不足均未提交)；新落点实机另验。最近复验同日。

2026-09-18 / EXP-236、EXP-303未来起点误差的有界几何复核（`validated`限离线算术，移动实机另验）：root在三轮Sol规划之后复核固定R1，不新增候选。对未来名义起点A、固定终点B和半径r=1.5m的停步误差，线段参数t处的起点扰动只有`(1-t)r`；以`min_t(|O-(A+t(B-A))|-(1-t)r)`再截到零求障碍中心距下界，比对整段统一减r更紧，但不降低小对象1.5m/其他非带3m运行门。4936场景中最紧4642下界1.718820m，原统一余量的2.985504<3不代表该线不可行；Terra独立数值极小化复核最紧三例与解析式一致，root纯函数23正反例通过且零游戏调用。测试中曾误用整数`Math.Max(0, double)`重载导致舍入，已改`0.0`，执行几何函数本就使用double；未据失败测试放行。首腿按真实起点，后腿仍必须逐次fresh实际原点/周边/地表、native prepare、同action terminal及Walk复读，不能用纸面误差界代替现场。此计算只包络既有中心线代理，不建模原生碰撞、球面实际路径或带碰撞，也不强迫更小到达容差。最近复验同日；执行入口仅新增纯规划函数，旧提交/终态/幂等逻辑未改，新方案与后续六风均待实机。

2026-09-18 / EXP-236、EXP-303多腿移动的实际起点复验（`validated`限一次正常Move、本地后缀拒绝及成功前缀保存）：C3首腿在67733976成功，实际停步距名义T1约0.973345m，仍在正常1.5m到达容差内；下一腿对2201/3166的中心线距却由名义1.707879m降至实际1.463294m，原1.5m代理门正确在出站前拦截。原164记录只有160个Bridge回复、一次native prepare/commit和一个成功终态；caller计数161包含了被本地拦截的调用，不能据此多算游戏预检。名义折线清场不保证从每个实际停步点仍清场；多腿方案须预留途中到达误差，并在每次真实落点重新核算，不能强迫0.5m容差、降低运行门或重放已成功首腿。下一方案对未来起点的不确定性可采用小对象3m/其他对象4.5m的保守纸面净距（原1.5/3m加1.5m），只是充分规划余量，不是新的原生门或碰撞保证。Sol第三轮R1的2.985504m未过此充分条件，不可称原生不可通行；三轮后交root设计，不能让Luna盲试或加第四轮相似搜索。原记录独立核销证明SHA-256 **674BC14409CD1CCAC02D4C21912C0E546EB0F2B3EDBAE478B97D1E0333A68B7D**；仅追加普通save到67746930/R218/J90，58只读阶段审计确认4936实体/9618边/零预建筑和全部旧状态，accepted2保留，见[本档成功前缀](./gameplay-timeline.md#2026-09-18--c3首段已保存实际停步复验拦截未提交后缀)。替代路线仍待，不因一次受控停止重建成功部分。最近复验同日。

2026-09-18 / EXP-242、EXP-299、EXP-303回航分层进入复用入口（`validated`限离线逻辑/原记录，新的实机施工待验）：私有风机入口不再在每次已验证成功的build之后强制等待所有无人机回巢。初始全idle门保留；后续只有前动作terminal成功、逐实体/材料/连接已核、玩家稳定且零pending build/repair、无人机计数完整一致时，才允许回航状态下继续原有限计划的fresh native prepare。保存后将`readinessPending`单列，原动作与保存证据先落盘，独立阶段审计仍要求真正idle，accepted不提前归零。没有延长60次/500ms等待、180秒action上限或增加并发写。17新增正反例加原66共83通过、备料20通过；原62份玩家记录离线重分类为2idle/60returning，默认idle门只准2，有限续建准入识别62，但不构成新的游戏执行。入口SHA-256 **BBBDE48C242ABE76E63469B212C28755EBF32F310062B8A5AF9F274566894149**，原记录复核证明SHA-256 **1A5BAA4E5E1D51E19F83E2CF745E68527B8D81C035185B353AACC4C259118DE8**。同源MCP/包内playbook明确不为每次普通build/save附加全idle门，扩展现有测试后两项指南回归通过，Release构建零警告/错误；安装态未替换，不声称新施工或部署通过。最近复验同日；动作、材料、目标或计数未知时仍拒绝，原生拒绝仍停。

2026-09-18 / EXP-303有限计划数据复核（`validated`限离线算术，不是native放置）：C3四短腿与六风站址使用本次4936场景和既有几何函数复算通过；manifest中E−→Q误填另一节点的距离，按明确两端重算为11.659897m，修订记录显式替代旧哈希、原件保留，未改候选或追加游戏尝试。连接距离应由实体/站址键对应的坐标计算，不手抄邻近边数字。剩余6风可在四短腿后合为7accepted含save，减少一个独立单风工序；这是计划优化，尚未证明执行耗时收益。native地表、实际落点及吸附后的全图条件仍逐步fresh核验，见[本档有限计划](./gameplay-timeline.md#2026-09-18--第十八台风机完成回航等待停止后仅保存成功前缀)。最近复验同日。

2026-09-18 / EXP-299、EXP-303施工终态与回航等待分离复验（`validated`限本次成功前缀核销、普通保存及阶段审计）：Q单风在67698936正常完成、唯一实体4936、2203库存7→6；私有执行器随后60次玩家复读仍为一架无人机回航，触发本地等待上限，第二台及save均未提交。该等待错误不是build失败、outcome_unknown或可重放许可。root随后fresh复核原action、实体与扣料一致、无人机已全idle，R214而primary仍67636618：`ownedSaveState=saved`不能代替保存tick覆盖最新accepted的证明。保留accepted1，仅执行新的普通save到67716265/R215/J90/accepted2，不重跑两风阶段或载入旧主档；56只读独立核4936实体/9618边/零预建筑、旧配置/库存/网络保持，见[本档成功前缀保存](./gameplay-timeline.md#2026-09-18--第十八台风机完成回航等待停止后仅保存成功前缀)。现行源码BuildUiIsIdle只检查人工预览状态，普通save也无all-drone-idle门；历史IFX-001已有返航中正常原生准入证据，不能把私有等待偏好当产品硬限制。此次未延长等待、修改执行器或完成后继施工；相关MCP说明测试2/2通过。原停止记录保留，后缀另取fresh计划；身份、材料或结果不明仍冻结，不能外推为任意working状态可继续。最近复验同日，关联IFX-001/EXP-242。

2026-09-18 / EXP-021、EXP-303风距余量实机预检复验（`validated`仅此两处native prepare）：正常短移后Q/E−实际范围76.426/78.125m，Sol以9只读、2prepare、零commit取得两处原生正例；实际吸附距旧风12.343732/10.622650m，批内11.659897m，单台正常预算2203×1、无新消费者/net4合并。root核原哈希与可执行标志通过，证明自选11m/79m余量不能替代原生10.5m与fresh buildArea硬门，详见[本档接近与预检](./gameplay-timeline.md#2026-09-18--南向短移正常保存与阶段审计通过)。不推广为所有10.5m以上必可放置，不削弱碰撞/科技/材料/整场检查；两台实际施工及后续持续供能仍待，执行必须fresh prepare，不复用这次token。最近复验同日。

2026-09-18 / EXP-299、EXP-303历史helper上下文复用复验（`validated`限本调用方与本次补验）：名义6.5m短Move及save一次成功，见[本档南向接近](./gameplay-timeline.md#2026-09-18--南向短移正常保存与阶段审计通过)。root审计调用方漏设当前Journal序号，旧helper默认J65使本地核验中断，原回复实际是健康durable90；此拒绝不代表游戏失败或需要回档。复用helper后须先明确绑定session、planet、revision、save tick与journal水位，不依赖其历史全局默认。此次补验先确认原进程已终止、精确54记录及原成功动作，再用显式J90在原审计追加续验说明、复核已取得快照并补5只读，最终4935实体/9618边/零预建筑及正常保存成立，59总读/零写；未重读整场或重放游戏动作。快照tick与最终状态tick分别记录，不伪称一个原子截面；结果未知、现场变化或证据不足不能沿用此补验结论。只证明这一短移、材料不变及保存，不证明其他带可通行、风机站址或持续供能。最近复验同日。

2026-09-18 / EXP-021、EXP-303纸面余量与原生门再区分（`observed`，新站址尚未native验证）：限定南侧五点计算中，E−实际paper snap距4687为10.623m、距历史稳定Walk点为79.528m，分别被本轮自选11m/79m筛选线排除；已有当前DLL证据却是风机最小10.5m，建造范围来自fresh玩家buildArea（此安装态80m），历史4931距玩家79.44m的正常施工也已证明79m不是产品门。修订先前“至少0.5m纸面余量”为规划偏好而非新的硬限制：可对原生范围内的明确候选继续有限只读预检，但必须按原生实际吸附位置重算全部既有/待建风距，并由fresh native检查材料/科技/地形/碰撞及范围；不能把略高于10.5m的纸面距离直接写成可施工。接近还须计入1.5m正常到达容差，历史Walk点并不保证实际停止位置让全部站址可达。本次不修改Plugin、原生阈值或成功设施，仅为Q/E−两明确点重新评估一条有限接近线；未发生移动或施工，若不合格仍按原失败边界停止。最近复验同日，关联[当前原生风距证据](./research/game-api-foundry.md)。

2026-09-18 / EXP-303工序耗时分解复核（`observed`限最近两批，不推算整体提速）：按精确raw-df0f696d/072f19db及raw-c799c15b/2758f3a4的recordedAtUtc核对，两风执行64.3043秒、阶段审计记录跨度24秒；三风执行71.0554秒、阶段审计记录跨度21秒（记录时间仅秒级）。三风184次调用中get_action_result97次、get_player_state66次，合计163/184≈88.6%；动作等待现为250ms轮询，施工无人机等待为500ms。调用占比不是耗时占比，节流不能宣称节省88.6%时间，未改轮询代码或动作语义。优先压缩有限批次选址/交接和重复解释未变机制的往返，既有脚本/源码哈希不变时复用已验结论，仍fresh核现场并保留原生预检、逐动作terminal、十写/阶段/恢复审计。完整审计本身只约二十余秒，不能凭总体等待长就删安全检查；审计到Git的墙钟间隔还含用户问答、分工调整与CI，不能全部归为施工开销。新职责分工收益仍待后续同窗量测，不新增通用工具或额外审批层。最近复验同日；完整工序用时必须另列规划、执行、核验、文档/Git和外部等待。

2026-09-18 / EXP-021、EXP-236、EXP-303三台原地补电阶段复验（`validated`限本批施工/保存/审计）：同一未改入口正常建4933/4934/4935并保存，库存10→7，root以56只读/零写核验4935实体、9618互返边、0预建筑、旧4932不变及net3三节点/三发电机增量，net4保持；详见[本档三台补电](./gameplay-timeline.md#2026-09-18--三台原地补电正常保存与阶段审计通过)。17/24及71.0554秒只分别代表累计目标计数和执行段，不能外推全流程效率；1525000 J/t条件容量仍比1549300预算少24300，余7风的10700最终余量仍依赖四聚变持续燃料。Terra做离线摘要核对不替代root整厂实机审计，普通发电机DTO的通用isWorking也不能单独证明持续发电。下一站址仍须完整纸面筛选及fresh native prepare，不重试旧占位点或改动已成功设施；最近复验同日。

2026-09-18 / EXP-303职责分层调整（`observed`限用户已批准的工作方式，效率收益待量测）：现行分工为主会话核心代码/安全/最终独立审核/Git，Sol Max工序设计及重设计，Terra Max既定规则计算/测试/证据比对与文档草稿，Luna Max有限游戏执行及批内检查；权威边界见[AGENTS现行分工](../AGENTS.md)。只传当批必要输入和规则，固定算术用已有脚本；不将同一历史复制给所有角色，不增加四级串行审批或执行者自签审计。默认不扩大Terra的离线/只读权限，所有游戏commit仍由Luna单流并共享accepted计数；异常和最终门结论仍由主会话独立核验。此次组织调整不等于已测得token/成本或速度改善，须用后续同窗实际记录比较。最近复验同日。

2026-09-18 / EXP-021、EXP-236、EXP-303原地补电实机复验（`validated`限两台施工/同网/保存）：按现有间距与预算规则，两个独立单风位置在当前建造范围边缘通过原生吸附预检，复核既有及批内最近风距至少11.3195m、无需移动。Luna复用同一有限入口建4931/4932并save67451871，风机12→10，完整4932对象/9618互返边及网成员独立核验通过；见[本档两台补电](./gameplay-timeline.md#2026-09-18--两台原地补电并保存重用已验证间距规则)。包内间距/动态容量专项MCP测试1/1通过；14/24计数和64.3043秒执行段不代表持续吞吐、完整功率或全流程效率。最近复验同日，下一站址仍须fresh prepare，不重放已建对象。

2026-09-18 / EXP-021、EXP-236、EXP-299风机选址复核（`validated`限原生拒绝与已知规则）：此前单风候选raw-b571c381被`WindTooClose`拒绝，3调用/零commit；完整4930场景证明其距4926/4928仅8.4348/9.1535m。当前DLL的风机最小间距10.5m及包内指南早已有记录，本次是外部规划遗漏，不是新增Plugin缺陷。后续纸面准入必须同时核既有风机、同批待建风机、并网距离、完整占位和范围；不能只查普通对象中心距或连接上限。采用实际原生吸附位置再核，保留至少0.5m纸面间距余量，最终仍逐步fresh native prepare；余量是本次规划选择，不是修改游戏规则。旧候选退役，不为不合格模块先移动；能在当前位置完成合法单风施工时无需坚持四风整图。最近复验同日；场景、吸附或DLL改变时重验。

2026-09-17 / EXP-236、EXP-246、EXP-299短步候选复验（`validated`限本次地表区分/短移保存；普通带碰撞类型仍未知）：中心接近普通带既不是必然阻挡的证明，也不是可穿越证明；当前DLL调研仍缺少对应prefab首碰撞体usage，不能把放置半径当成Walk物理碰撞范围。新5.68m绕行虽`shoreRisk=not_detected`，末端却低于水面约0.295m、没有有效干地后缀，86调用/零commit后退役整条依赖路线；另一个沿已观测干地的5m目标经fresh81调用预览，6点/12射线完整、unknown0、全干，末端高水约0.198m，且避开已知大设备中心。预览只支持一次普通Move有界尝试及成功后保存，不保证无碰撞；每步必须原action terminal和fresh Walk/低速/能量核验，失败不重复目标。14纯检查和3入口拒绝检查通过，随后本目标Move/save及完整4930实体阶段复核通过，见[短移实证](./gameplay-timeline.md#2026-09-17--末端湿地候选退役有限干地短移与保存通过)；不声称其他带、完整路线或后续风场已完成。最近复验同日；实际起点、地表、场景或Prefab版本变化时重验。

2026-09-17 / EXP-236、EXP-299、EXP-303线段近邻复验（`validated`限纯几何纠错/提交前拦截）：投影须使用`AB=target-origin`、`AP=entity-origin`和`Clamp(dot(AP,AB)/dot(AB,AB),0,1)`；把AP锚到target会将真实比例减1，端点最近距离正确也不能补救漏掉中段。地表33/66全干不等于route-clearance，完整场景反例3050/3047使本次Move在commit前被禁止。不得依赖未落盘、无测试的临时几何作为执行许可；复用现有辅助的纯函数，保留所有对象、明确中心近邻不等于原生碰撞范围。9项新增回归、原4930场景重算与[本档负例](./gameplay-timeline.md#2026-09-17--短移地表正例不等于路径畅通投影错误在提交前拦截)通过，关联IFX-159；不声称Move已发生或新路线安全。最近复验同日；起终点、场景、投影/坐标实现变化时重验，不能以干地采样覆盖几何矛盾。

2026-09-17 / EXP-236、EXP-302、EXP-303后继单风复验（`validated`限同入口跨工序复用）：已有执行/阶段审计机制不变时，只替换审阅后的现场数据并重新原生prepare，不为两件普通施工再造执行器/全流fixture；上一批经完整审计及绿CI后显式封存，不能因换计划自动归零。此次4929/4930分别独立接既有网3，覆盖对象22/14全部为目录证明不耗电的2001带，完整4930实体/9618边核验、正常save67117267通过；见[后继两风](./gameplay-timeline.md#2026-09-17--补电达到十二台复用入口不再改执行代码)。结构化计划仍须检查扁平对象集合，修正零写的数据表示不冒充原生负例。最近复验同日；场景/材料/网联/契约变化时重验，不把12风数量或59.503秒执行段外推为完整补电、持续供给或整体效率。

2026-09-17 / EXP-189、EXP-236、EXP-299、EXP-303单体替代复验（`validated`限本次两风施工/保存）：较大蓝图部分占位时，可在尚未施工的计划里明确选择未占用位置，使用已支持的普通单体原语重新完整prepare；不复用旧图token、假定部分许可或拆改已成功部分。两台4927/4928正常扣料并独立接网3，完整4928实体阶段核验保持原配置/边及消费者，见[两单风实证](./gameplay-timeline.md#2026-09-17--两台单风并网保存拒绝路径不作移动)。调用方必须核真实请求DTO，不能把响应/计划字段名用于请求；纯测试覆盖实际序列化字段，不只测试返回值守卫。本次错误prepare零commit后就地修正，再fresh执行；57+20检查及三成功terminal通过，关联IFX-158。末端湿地的Move预览未执行，不视作移动碰撞或游戏失败。最近复验同日；布局/现场/DTO/网联或材料变化时重验，尚未建的14风不计容量，不外推持续燃料/紫糖或任意单体场地安全。

2026-09-17 / EXP-189、EXP-236、EXP-303拥挤站址负例复验（`validated`限本次原生拒绝/零写）：四风蓝图的离线近似姿态和中心净距不是原生吸附后碰撞许可，须采用native返回的actual对象位置逐一核验。本次actual index0/1命中3348/3085，四次原回复证明owned/revision/save保持且没有施工；见[本档站址负例](./gameplay-timeline.md#2026-09-17--后继四风站址原生拒绝停止拥挤局部微调)。私有2m筛选阈值不冒充产品门；在有限局部无明显裕量候选时停止微调，改为有证据地正常接近稀疏区或使用现有较小模块。尚未验证新移动/场地，不外推全场无位置，不扩大为Plugin错误。最近复验同日；实际吸附、地形、已有实体、玩家建造范围或模块变化时重验，失败目标不重放。

2026-09-17 / EXP-021、EXP-236、EXP-302、EXP-303分组补电复验（`validated`限正常施工/逐对象扣料/同网/保存）：完整备料不要求所有站址同时成熟；已满足原生条件、整组材料、与既有及其他已批准布局不冲突且包含save预算的小组，可先完成并独立提交。此次两组4风经fresh inspect/prepare/commit和八实体读回，完整4926实体阶段审计证明旧产线保持，24风库存只扣8。只允许纯发电组保留既有`full_base_load_generation_deficit`的advisory，不因此放行新消费者或宣布满基础预算通过；尚未建的后续组不能计入容量。34+20项私有入口/共享守卫检查及[首批8风实机证据](./gameplay-timeline.md#2026-09-17--首批八台补电风机并网保存原产线保持)通过；132.169秒是执行段而非全流程耗时。最近复验同日；场景、计划分组、实际网联、材料、燃料依赖或后继负载变化时fresh重验，不重放成功前缀。

2026-09-17 / EXP-021、EXP-300、EXP-302补电预算复验（`validated`限目录/源仓/算术，施工未验）：旧完整场景网3有88风、9热电、4聚变；当前目录风5000、聚变250000 J/t。1512000低截面仍含热电，不作为稳定下界。此次有限预算不计九台热电，保留四台聚变正常燃料依赖，风加聚变为1440000；既有满基础1546600加宽带四附件1200及尚未定稿处理器最多五附件1500，共1549300。选择24风新增120000、纸面余10700，而非最少22风仅余700；这不是四聚变永久有料保证，后续仍fresh容量/归网及持续燃料验收。r5/r6/r7目录与三源仓fresh证明支持168铁/72磁/36铜→24风，正常三转移+递归手搓+save共五写，现有98带/9分拣器不挪用。保护计划SHA-256 **72749A562809C0C128F8A509626770C92B6C5AA528668BFE71544789CC36C0D4**，9只读/0prepare/commit来源证明 **BED98EA3B298AA406999B267500B5A29F30BC49F8EA3F2DC613EE4B65E41B0FF**；站址尚未native检查、六组模块施工另审，不据备料放行。最近复验同日；发电类型/燃料、风力、未来附件上限或现场变化时重算，已有成功部分不重做。

后继备料实机复验（EXP-021/302/303，`validated`限五写正常手搓/保存/全场保持）：三源即时守恒、递归净168/72/36、24风及原送料建材保持，save66847839/J90成立。复用既有入口与原记录核验，只做一次阶段全场读取，20/21项离线检查及4918实体/9618边通过；accepted5不提前归零。完整证据见[本档24风备料](./gameplay-timeline.md#2026-09-17--二十四台补电风机备料并保存成功产线保持)。执行124.654秒不是整个规划流程耗时；正常成本和保存正例不替代站址、同网联接或持续供电验收。最近复验同日，下一工序/部署/十写或燃料假设变化时复核。

2026-09-17 / EXP-259、EXP-300、EXP-302组合准入复验（`validated`限三条原生路径和本次零写功率停止）：自由端点先吸附后，还须调用当前Core跨度与完整端点规则；不能只用点数/占位过关，或把私有helper限额当产品限额。三条16/14/20点geodesic原生stage1均通过，最大纸面误差小于0.000094m；后续fresh容量1512000却低于满基础加四附件1547800，立即停止，不能等一个高容量点或用当刻满供代替完整预算。工序顺序优化为先核整批材料/满负荷预算，明确零写的本地参数错误只继续原有界只读后缀，不再重复授权/已通过预检；分别完成的输入线仍须协调共享资源且不等于整链持续供给。详见[本档三路及功率边界](./gameplay-timeline.md#2026-09-17--三段宽带原生路径通过满基础供电缺口阻止施工)，实际19调用/3prepare/0commit，纠正派生摘要21而保留原文件。最近复验同日；现场、原生吸附、供电/附件及DTO变化时重验。

同批量纲复核（EXP-300，`validated`限现有读取语义）：发电机buffer的`role=power-generation-current-tick`、`countUnit=joules_per_tick`先于物品名称决定count含义；183的1997是发电量，不能当氢库存，3058的0不证明燃料耗尽。当前generator读取分支不赋值`IsWorking`，默认false不构成停机证据。不得以这两项误读发起补燃料，也不能将瞬时发电从网络capacity相减便证明稳定剩余容量。原生Fuel/Wind容量计算、实际发电和燃料库存是不同证据；未观测库存就明确未知。原记录及源码边界同上，最近复验同日；读取实现或字段单位改变时复验，不新增公共观察字段。

后继指南修正：上述量纲、非权威false、整批满基础预算及不等容量高点的规则已写入包内/MCP同源playbook和三个既有工具描述。4条新增回归、全部145项MCP与13项Core量纲测试通过，Release构建零警告/错误；工具/资源数量、DTO和写入协议不变。[IFX-069](./incident-fix-log.md#ifx-069--临时只读摘要猜测dto字段并混合量纲)复验保持mitigated，安装版未热替换，文案不能代替真实补电或燃料观察。

2026-09-17 / EXP-259、EXP-303自由端点复验（`validated`限源码调用顺序及真实拒绝；具体吸附后路径仍待）：`native_geodesic`只选择原生线生成模式，不取消free start/end各自的`Snap(onTerrain:true)`。纸面点对旧1403距离0.472256m且Core清场，不能证明吸附后的路径也清场；fresh prepare实际拒绝point13/1403，0commit。失败未带完整plannedPath时不能倒填实际坐标，不以删端点/移近玩家绕过占位，原候选退役并保留旧线；见[本档首段负例](./gameplay-timeline.md#2026-09-17--紫糖首段带路占位拒绝原生端点吸附边界复核)。66项Core占位/路径模式回归通过，未修改公开原语。规则已写入包内/MCP同源playbook，新增3条资源检查后7条格线指南检查通过；[IFX-157](./incident-fix-log.md#ifx-157--geodesic纸面路径遗漏自由端点先吸附)区分源码指南与待部署/待改线实证。最近复验同日；后续纸面模型须先复核原生端点吸附再重算完整路线及接驳，仍以fresh原生返回准入，不外推旧纸面`coreClear`。

2026-09-17 / EXP-077、EXP-236、EXP-302、EXP-303单塔施工复验（`validated`限正常建塔/保存及几何供电证据）：新2201只使主网节点177→178，完整4918实体阶段核验保持原消费者、发电设备、第二电网、材料与Journal。动态发电容量不能作为静态网络身份；仍以fresh满供和完整实际负载预算准入，212项纯检查包含容量变化可接受/预算不足拒绝。历史玩家stateHash同样不作跨时刻身份，保留静态库存/位置及fresh执行hash。执行只查受影响邻域，独立完整阶段审计不删，避免单动作多次重复全厂扫描。实际新塔对拟建处理器尾部覆盖裕量4.569858m，尚不证明未来sorter已归网或产线持续供料；见[本档单塔保存](./gameplay-timeline.md#2026-09-17--紫糖末端电塔施工保存处理器尾部覆盖复验通过)。最近复验同日；消费者实际位置、网联/容量、材料或恢复变化时fresh重验，原成功塔不重建。

2026-09-17 / EXP-021、EXP-077、EXP-302、EXP-303单塔材料/预检复验（`validated`限本次正常r8/保存及固定点原生许可）：已有线圈应进入实际净材料预算，本次仅补2铁，正常制作1塔后保留全部送料建材。无塔时只做几何/功率规划；有塔后重新执行完整原生放置预检，不能把缺料或Foundry吸附点替代场地许可。此次唯一候选原生plannedPosition与供电代理一致，未来尾部覆盖余量约4.57m；这不等于塔已建/归网，下一仍fresh prepare及真实网成员/覆盖复读。113纯检查、正常三写save66389905和[4917实体阶段核验](./gameplay-timeline.md#2026-09-17--紫糖末端补塔备料保存固定落点原生预检通过)通过。最近复验同日；材料、真实落点、现场/网络或恢复变化即重验，不复用旧token或重做已成功备料。

2026-09-17 / EXP-021、EXP-077、EXP-185、EXP-302双送料备料与缺电复验（`validated`限五写备料/保存及本次覆盖负例）：97带/9分拣器方案在已有2带、零电路板时，115铁/5铜经r84×32和r85×9净增96带/9分拣器/1电路板，不能忽略递归配方的批次余料。96纯检查及[完整4917实体阶段实证](./gameplay-timeline.md#2026-09-17--紫糖双送料线备料保存末端供电缺口单独处置)通过。备料成功不等于场地通过：末端离10.5m覆盖还差约0.27m，不能四舍五入成已覆盖；回缩/侧移又破坏附件角度或跨度时停止同类候选，转为独立预检普通补塔，不改成功前缀。现有入口复用、正常保存与十写边界保持；最近复验同日，真实端点、吸附、供电/材料或恢复变化时重新核验。没有据纸面清场宣称原生施工或持续紫糖通过。

2026-09-17 / EXP-021、EXP-302启动验收复验（`validated`限r36三输入及首次自动宽带）：正常配置现有机器不扣配方所列原料，prepare的2纳米管/2晶格硅/1塑料→1宽带是配方语义；实际消耗必须另取原生统计和设备buffer。2255首个600tick窗已消费但宽带为0，随后两独立窗各产1，J90与save66249956共同证明启动，不能把首窗0当卡死，也不能把三窗全零判通过。窗口必须绑定同session/完整planet104四行/原生计数/非重叠边界；任一输入聚合消费为0、宽带产出为0时启动结果为false，仍可正常保存已成功配置，不重配或重放；未知身份/范围则停止。49纯检查及[三窗实机/完整阶段审计](./gameplay-timeline.md#2026-09-17--宽带三输入真实消费及首次自动产出保存)通过；最近复验同日，现场、配方、来源归因、统计窗口或恢复变化时重验。输出尚无长期去路，这份证据不证明持续生产、紫糖或库存配平。

2026-09-17 / EXP-124、EXP-236、EXP-299、EXP-302附件及表示边界复验（`validated`限八连接/到货/保存）：按实际七线首末端和原生exact_slots完成八只1124过滤sorter，源最后激活；实际位置和吸附功率代理分别核覆盖，不能要求二者机械相等。正常扣8材料、双端/满供/完整4917实体核验及save66133149成立，七条末带纯纳米管到货；2255未配置，不能称消费或持续生产。阶段审计再现IFX-115：JSON Int64即使值相同，也不能直接索引Int32 Hashtable键；统一实际取值键并先检查存在，缺项必须拒绝而非当空货路。旧79次真实读取保留，3项纯回归及后续完整只读核验通过，无游戏重放。详见[本档八附件](./gameplay-timeline.md#2026-09-17--八个纳米管附件保存末带到货及完整阶段审计通过)；最近复验同日，DTO表示、端点/过滤、现场功率或恢复改变时重验。私有辅助修正不冒充产品API新增或整链验收。

2026-09-17 / EXP-124、EXP-302有限带路复验（`validated`限七条独立2001空线及保存）：七次普通native_grid施工正常新增48带，实际ID分配不保证沿路单调；必须按原生plannedPath逐点唯一匹配、核完整内部双向边及两端自由，不能用ID排序生成后续连接。所有材料、原4861对象与新增拓扑经完整4909实体阶段审计闭合，save66069476覆盖八个terminal；152/70/21纯检查和两项审计入口拒绝通过。详见[本档七线施工](./gameplay-timeline.md#2026-09-17--七条纳米管空带保存正常成本与自由端复验通过)。复用共享执行/阶段核验，没有新增公共原语或逐动作全流框架；仍须以真实端点fresh预检八附件，空带不能冒充供料。最近复验同日；现场、端点、材料、原生吸附或恢复改变时重验，不重放已成功前缀。

2026-09-17 / EXP-077、EXP-236、EXP-302补塔实机复验（`validated`限三节点施工/保存/覆盖）：三个实际2201节点4859–4861经正常成本与无人机完成，网3节点只增3、原消费者/发电设备和网4保持；public详情networkId为null时不补造身份，以完工前后全网成员计数、唯一新增实体及fresh Foundry的网3组件联合核验。八个拟分拣源点与吸附代理分别对真实节点留出余量，最小0.690015m；完整真实需求1544200+2400通过当刻容量，不承诺长期发电或附件准入。[本档四写/完整4861实体核验](./gameplay-timeline.md#2026-09-17--三座纳米管补塔并网保存八个源点覆盖通过)和save65975266通过，不能用已补电替代七线/八附件的fresh预检。最近复验同日；实际消费者落点、网络/容量、配置或恢复后重新核验。

2026-09-17 / EXP-021、EXP-302、EXP-303整路备料复验（`validated`限七动作与保存）：两座新增电塔可复用原1线圈，通过正常r6递归补造后仍余1，不能把r8的顶层2线圈当成最终净消耗。65铁/5铜/2磁铁变为45带/8分拣器/2塔的净增量，三源即时扣减和全背包守恒通过；七个唯一terminal及save65946124落盘。复用同一stock入口和阶段审计，70/128/21检查与完整4858实体核验通过，未另建全流模拟。详见[本档七写](./gameplay-timeline.md#2026-09-17--纳米管整路备料保存七写完整阶段审计通过)；accepted7在推送绿CI及显式封存前保留，后续四写补塔独立计数，不重取成功材料，不据此证明场地/供流。最近复验同日，原库存、递归配方、工序或恢复变化时重算并fresh预检。

2026-09-17 / EXP-077、EXP-236、EXP-302岸边补电复验（`validated`限三处放置预检）：一塔需同时覆盖两个消费者的交集受限时，可以在尚未施工的有限计划中比较两塔分担，而非继续盲试同类岸点；必须重算整路材料、网络邻接和实际负载。当前保留T1，新增T2/T3分别通过原生正常放置，0commit。T3仅高水约0.005828m的移动采样不足以批准Walk目标，但原生建筑预检成功，进一步证明移动落地余量不能代替建筑地面规则。三transfer/三handcraft/save为七写，三塔/save另为四写，前者须完整审计并显式封存，不能以阶段切换自动归零后跨过十写。70/128纯检查通过；[本档三塔预检](./gameplay-timeline.md#2026-09-17--分开覆盖岸边缺电点三处电塔原生预检通过)不证明已建供电、整路原生许可或产出。最近复验同日；实际吸附、库存、网络、目标或原生条件变化时重新prepare。

2026-09-17 / EXP-077、EXP-236补塔预检复验（`validated`限实际预检）：电塔覆盖/连网几何合格仍不证明其整座碰撞体和地面可放置。首轮2201两候选分别撞3050、被原生NeedGround拒绝，0commit；两点退役并交回规划。第二轮T1原生成功，实际吸附点对请求/代理点覆盖余量0.690014/0.892062m；T2仍NeedGround，整体未通过，不重放成功预检或退役失败点。T1之后本地摘要误读不存在字段不改变原回执成功事实，应离线核完整DTO而非再试游戏。保留已保存线路和材料，下一只基于不同实际地面/碰撞证据做有限设计，不用移动地表剖面代替建筑放置门；见[首轮两拒绝](./gameplay-timeline.md#2026-09-17--纳米管两处补塔候选被真实放置检查拒绝)及[第二轮正负例](./gameplay-timeline.md#2026-09-17--首处纳米管补塔预检通过第二处地面缺口保留)。最近复验同日，候选、吸附、地形或旧设备变化时重验；这不是两塔已建、供电恢复或整条纳米管施工通过。

2026-09-17 / EXP-236、EXP-303功率代理坐标复验（`validated`限源码与真实只读返回）：Foundry machine-grid的`site.power`使用吸附后的`site.machines[].position`，不是原`site.origin`。本次请求slot点和评估点相差0.326130890m，负例只属于后者；正例同样不能跨未经核验的偏移移植到实际分拣器。应分别记录请求、吸附评估与正常prepare返回的实际位置，按真实电塔几何留出覆盖裕量，并另核实际消费者的满负载需求及完工网络/供电。代理机器缺料/碰撞不等于目标消费者场地失败，4500代理预算也不替代八只2011的2400。包内指南已加入该规则；6项指南、40项供电策略通过，新指南尚未冷部署。后继T2第三轮以23点剖面证实一条直达方向进入水侧，不能据此把整个二维覆盖交集判为水；四点纸面拒绝也不是整个邻域不可建，三轮后按规则交root。见[本档位置复核](./gameplay-timeline.md#2026-09-17--纳米管补电预览区分请求点与真实评估点)、[第三轮边界](./gameplay-timeline.md#2026-09-17--纳米管第二补塔三轮后交回主会话)及IFX-150后继；最近复验同日，节点、目标位置、实际吸附或DLL变化时重验，不据此开放无界候选或跳过原生预检。

2026-09-17 / EXP-259、EXP-302规划证据复验（`invalidated`限本次候选“全路无重叠”声明）：无输出或尚未取得成功终态的计算不是clear；局部两端角度合格也不能替代整路占位。root复用当前Core的有界空间索引策略，53点/4858实体明确拒绝point19/1931，未发生游戏prepare/commit；原候选撤回，见[IFX-156](./incident-fix-log.md#ifx-156--规划计算未完成被误报为全路清场)。后续纸面检查须保留明确结果、实际完整路径和场景身份，避免重新实现全场嵌套比较；仍由fresh原生预检最终准入。native_grid产品256点、旧私有20点切片和geodesic30m分别适用，不能混为同一限制或因此添加无必要的分拣中继。最近复验同日；后续候选/场景/路径模式变化时重新计算，不以本次拒绝否定历史已验证线路。

2026-09-17 / EXP-061、EXP-303过滤中继与功率复验（`validated`限本次七附件/保存/末带到货）：将既有晶格硅附件工序复用于塑料时，必须替换明确filter与目标slot，并区分旧2246侧取和新空尾端，不能机械要求旧源slot0空闲。七次fresh原生计划及完工双边连接证明1115正确过滤、2255 slot1保留且slot2新增；源最后激活，塑料穿过六条带到4850。实际预检容量从旧1584000降至1548000，按完整需求1544200仍有3800余量；此次真实正例支持动态完整预算，不能锁定旧容量或用4500代理代替七只2100。八动作/4858全场及正常保存通过，见[本档七附件](./gameplay-timeline.md#2026-09-17--七个塑料过滤附件保存原料到达末带)。45本片检查及同批21共享客户端通过；最近复验同日。recipe0末端积货不代表消费/稳态，接纳米管、启配方、负载或供给改变时继续复验。

2026-09-17 / EXP-259、EXP-303部分成功后缀复验（`validated`限塑料48格带及七实际附件预检）：补电并封存后，只建原计划未完成的4/12/3/5格，不重放已保存24格；正常成本、五终态及4851全场阶段核验通过。自动选槽不能靠请求强制指定，必须检查fresh回显；本次4850→2255选择slot2且既有4797/slot1保持，其余六对均belt虚拟slot−1。七实际落点供电及整批2100预算通过不等于附件已施工或持续供流。见[本档剩余带路](./gameplay-timeline.md#2026-09-17--塑料剩余二十四格带保存七个实际附件预检通过)。复用既有有限执行/阶段审计入口，20纯检查及21共享客户端测试通过；原token不复用，下一仍fresh prepare。最近复验同日，现场、槽位、库存或负载变化即重验；不外推为通用寻路或产线完成。

2026-09-17 / EXP-185、EXP-303补电规划及实机复验（`validated`限两塔正常施工/覆盖/保存）：补覆盖应先查背包，本次已有3塔，只用2塔、零补料。节点连接预检必须按fresh目录`IsPowerNode`包含所有类型，不能只查2201；root补核5个2203风机后，才闭合两候选全部邻接节点均为网3的证据。power-node详情的`powerNetworkId=null`不能当断网或擅自填写3，复用exact锚点Foundry组件及完工两网成员差量证明归属。原生两塔正常扣料、无人机及网3节点172→174实证通过，七源点exact覆盖、满基础1542100加未来七只实际2100的当刻预算通过；4500机器代理仍不当实际新增负载。正常保存65276142及4827全场十写审计通过，40项电力计算测试通过，见[本档两塔与十写](./gameplay-timeline.md#2026-09-17--两塔补齐塑料分拣覆盖并保存十写审计通过)。准入校验使用完整原回复而非字段裁剪后的规划摘要，辅助字段错误不计DSP失败、不重做成功动作。最近复验同日，实际落点、网联、后继负载或任意原生拒绝变化时重验；未建sorter仍须fresh实际端点/原生prepare，持续供料和保存恢复另验。

2026-09-17 / EXP-303、EXP-077真实分拣落点复验（`validated`限两段塑料空带及覆盖负例）：两条12格独立带原生建造均成功，第二条完工后的实际跨线分拣器也通过正常几何预检，但其精确`plannedPosition`供电证据为`uncoveredObjectIndices=[0]`、零覆盖组件；不能以相邻位置的代理覆盖或全网满供代替该点供电。停止后缀，保留已完成24格；没有提交分拣器、放宽检查或重放整批。独立全场4825实体/9436互返边与六个终态核验通过，原始stopped明确accepted6；Agent简报的accepted5错误，以回执为准。随后只交付save后缀，primary65169785和保存后七终态/全场复验通过，accepted7保持；详见[本档两段保存与负例](./gameplay-timeline.md#2026-09-17--两段塑料空带保存实际分拣落点缺电停止后缀)。40项供电计算测试通过，无公共代码/安装态变化、未实际重启。最近复验同日，补电、吸附/实际端点及后续负载变化时重验；既有指南和EXP-303的部分成功边界不变。

2026-09-17 / EXP-021、EXP-302有限补料复验（`validated`限本次两动作）：整路库存已齐大部时，只补明确新增桥的一只sorter，复用原1电路板并仅取1铁；不要重跑原备料工序。原生预算、即时来源扣料、整包净2011+1/1301−1及两个fresh终态通过，53纯检查通过。accepted从已有Move的1累计到3，留六带动作及save至10；[本档后缀](./gameplay-timeline.md#2026-09-17--只补额外跨线分拣器保留十写保存名额)尚未落主档、不构成施工/供流许可，普通核验不清零。最近复验同日；库存、路线、原生配方或后继动作数改变时重算。

2026-09-17 / EXP-259、EXP-236、EXP-302跨纬带根因和新正例复验（`validated`限当前局部free-ground）：下条保留当时“根因未证”截面，本条以随后源码及原生正例补齐。不能用固定赤道格点跨纬带估算整条路径；真实动态经度分段、吸附及去重复现26点和point13/3767冲突，新六段48点与native最大差小于0.000024m。原生`OutOfReach`与场地占位是独立门；先短移再fresh业务预检，不能放宽范围。`shoreRisk=not_detected`不保证样本高于水面，6m浅水候选未提交，3.5m全段高于水0.142m以上才唯一Move并复读Walk/速度0。94路径纯检查和7地表检查通过，公开指南吸收结论，安装态未更新；[IFX-155](./incident-fix-log.md#ifx-155--固定赤道格点模型误判跨纬带输送路线)与[本档六段预检](./gameplay-timeline.md#2026-09-17--修正跨纬带路线并完成短移和六段原生预检)不等于附件、施工或产出通过。最近复验同日；纬带、半径、路径模式、高度、现场或DLL变化时重新核对，禁止把这个窄模型扩成自主寻路。

2026-09-17 / EXP-259、EXP-302纸面格线边界复验（`validated`限首走廊占位负例）：预先计算的格点必须与原生实际生成路径一致；本次纸面最小中心距离0.539m，前两段合成的28NEW预检仍在point13/3767发生≤0.25m占位拒绝。失败未返回完整路径，不能把推测的具体Snap位置或纬带误差写成已验证根因；先查实际DLL和路由构造，不再按相同纸面坐标重投。32项Core占位检查通过，[负例](./gameplay-timeline.md#2026-09-17--塑料首走廊实际占位拒绝材料和旧线保留)仅一次prepare/0commit，原材料与成功前缀保持。提前完整预检有助避免生成不可续接的施工前缀，但合成路径的检查不是各分段、source-cover或附件的施工许可。最近复验同日；改端点、跨格区、原生路由或分段后重新验证。

2026-09-17 / EXP-061、EXP-185、EXP-302塑料工序复验（`validated`限既有规则纠错、备料和保存）：路线必须先按端点类型应用当前DLL和包内指南已有的距离/格距限制；不能把belt/device的5.5m借给belt/belt，后者5.031m四行桥纸面即退役，未作native尝试。Foundry会吸附代理位置，本次纸面中点与实际代理差0.126–0.629m，附近覆盖只能作为备料建议，实际sorter须用真正native落点重新核电，禁止忽略位置差后宣称精确覆盖。原2246是中段侧取，应保留slot0及所有旧边，不能误套此前自由末带的“slot0必须空”条件。复用stock入口以63铁/3铜完成51带/6分拣器，已有1电路板净保持；[正常保存和全4801实体阶段审计](./gameplay-timeline.md#2026-09-17--塑料送料材料备齐保存及五写阶段审计通过)通过，47纯检查及21共享客户端通过。此次只备料，材料充足不放行占位/供电/附件；新路线未施工。最近复验同日；端点类型、实际吸附、原生格距、材料和现场变化时重验。

2026-09-17 / EXP-061、EXP-185、EXP-302有限附件工序复验（`validated`限五连接/保存/末带到货）：五个真实exact_slots连接按下游→上游施工、源最后激活，五次build加save正常闭合；虚拟belt槽−1必须反查实际4–15槽，机器端2255精确slot1，端点只能有一条指向该新sorter的边，不能按对象ID剥掉任意重复边。完整功率按1540600+已建×300和剩余真实需求递减预算，最终1542100，不重复计已建负载，也不把代理4500算作实建。82纯检查、21共享客户端及[4801实体/9392互返边独立核验](./gameplay-timeline.md#2026-09-17--五个晶格硅过滤附件保存且原料到达末带)通过，所有旧配置保持。连接到recipe0设备可正常建成、原料可达末带，但空缓冲证明消费尚未开始；不能把到货称为持续1113/宽带产出。accepted6待推送绿CI后显式封存，最近复验同日；配置、上游、需求、功率或恢复后重新验证实际消费与库存趋势。

2026-09-17 / EXP-259、EXP-302多段有限施工复验（`validated`限33NEW正常施工/保存/阶段保全）：复用首线入口推进剩余7/9/3/4/10 NEW，各独立链分别维护真实尾端，并对整批新ID去重，不能把上一条链当成下一条的source。49纯检查、21共享客户端及[全4796实体/9372互返边阶段审计](./gameplay-timeline.md#2026-09-17--晶格硅剩余三十三格带保存及阶段全审通过)通过；652请求、六动作含保存用205.181秒，不含规划/交接和独立审计。主档64647459覆盖全部53带，但四链尚空载未连，不能据土建完成核销持续1113。下一按真实端点从下游接起、源最后激活，并给五个附件完整预算1500 J/t；功率只认现场满基础负载加未建实际需求，不把只读代理的4500计作实建、也不把历史容量当永久预约。accepted6在推送绿CI后显式封存，普通预览不清零；最近复验同日，端点、材料、覆盖、负载或原生计划变化时重验。

2026-09-17 / EXP-259、EXP-302空载续接与十写复验（`validated`限首线施工/保存/整厂保全）：fresh原生2/8/8/2段均正常完成，共20NEW、只扣20带；实际ID不单调，逐位置匹配及双向槽才能确定先后。source-cover的完工原生旋转证明仅覆盖source及其直接邻居，调用方不得把整条旧链rotation全部归一后忽略其他变化；只剥精确唯一的source slot0→首NEW slot1，保留所有旧边，不能按对方ID宽泛删边掩盖重复连接。绑定成功终态、空货物和完整原生计划后，其他旧成员的姿态仍须保持。40纯检查、21共享客户端及[十写全厂实机核验](./gameplay-timeline.md#2026-09-17--晶格硅首条二十格带保存及十写全审通过)通过；主档64564659覆盖4763实体/9312互返边，accepted10冻结等待推送/绿CI后封存。本次无输入，不是持续产出或重启复验；物料、拓扑、路径/原生旋转规则或现场变化时重验，最近复验同日。

2026-09-17 / EXP-302有限工序备料与开销复验（`validated`限本机守恒取料/手搓/保存）：同一已有stock入口完成65铁/3铜到54带/5分拣器/余1电路板，五动作含保存仅35.474秒；root13次fresh读核新增证据与关键不变量，不重复全厂扫描，accepted5不清零。见[本档备料闭环](./gameplay-timeline.md#2026-09-17--晶格硅整路材料备齐并正常保存)。此前单仓构建raw-e1a8d554为223次调用/101.359秒；本次lab建造及配置保存raw-1f477773与raw-e7a03ef3直接84次/约16秒，加异常独立核销4次与完整阶段审计64次为152次，调用数少约32%。这是不同任务的开销对照，不是受控基准或整体6倍提速：lab从备料到配置保存仍约84分钟，规划、交接及IFX-154核销间隙远大于实际执行。后续同时记录工序端到端与请求执行时间，不能只拿脚本秒数宣称业务提速；现场、库存、保存边界或执行器变化时重验，最近复验同日。

2026-09-17 / EXP-259、EXP-302路线语法复验（`validated`限源码/纸面分段和来源读取）：局部格点连通搜索的失败不是所有原生路径模式无解；有一串空格也不代表已形成可提交路线。当前source-cover必须至少2个NEW，`native_grid`先经度后纬度，可合并/重分L形减少动作，但每段仍须核真实完整plannedPath和正常成本，不能跳过占位或改掉source绑定。已保存纯料线的自由末端可作为新的显式分拣来源，保留既有缓冲而非拆旧线；必须fresh核货物、真实输入过滤链、功率及后续持续产出。此次53点重分9带动作/5sorter，38纯检查通过，尚无场地或施工许可；见[本档有限路线](./gameplay-timeline.md#2026-09-17--晶格硅送料三轮规划后收敛有限备料与带路)。最近复验同日，实际ID/旋转、纬区、路由或原生返回变化时重验。

2026-09-17 / EXP-302、EXP-303后缀保存复验（`validated`限4743配置/保存及完整阶段保全）：绑定独立核销证明后，只完成未执行的r55配置与save，accepted7→9，原build没有重放。root全量4743实体/9274互返边与九终态证明旧场保持、正常材料净0、Journal89 durable；primary64351298覆盖新建及配置。提前完整阶段审计可在推送绿CI后显式封存计数，但普通只读核验、配置成功或瞬时满供都不能替代持续生产/重启验收。见[阶段审计](./gameplay-timeline.md#2026-09-17--紫糖实验室配置保存与九动作完整审计通过)；最近复验同日，原料、连接、保存恢复或后续负载改变时再验。本条更新下条“后缀尚待”截面，不扩大IFX-154的修复范围。

2026-09-17 / EXP-302、EXP-303完工后缀复验（`validated`限本次成功前缀核销）：原生建造terminal与施工无人机全部idle不是同一瞬间。lab4743已完成并正常扣料，紧随其后的working1/pending0守卫停止不撤销已完成建造；稍后fresh为idle3。执行器的exit1及Agent叙述都不能替代原receipt、terminal和stopped计数。异常后先保留accepted、核唯一实体/材料，退役旧入口，仅交付已证明未执行的后缀，不能把“重新跑脚本”当恢复。11个纯守卫检查通过；配置/保存后缀和完整阶段审计仍待，主档64052125尚不覆盖新对象。见[本档核销](./gameplay-timeline.md#2026-09-17--紫糖实验室建成后核销成功前缀不重建)及IFX-154；最近复验同日，后续动作、caller、pending目标或保存边界变化时复验。

2026-09-17 / EXP-077、EXP-236内侧落点复验（`validated`限2901原生预检与当刻供电）：把真实地表剖面用于选定候选，再由完整原生预检决定footprint合法性，不能把剖面当二维地形图。已有带距离约3.6m的内侧lab位置确实通过正常碰撞/地面检查，反证用6.392m三维球上界作水平必然冲突。供电读取必须使用实际吸附坐标；代理自身缺料不证明场地，而独立lab native通过可与代理明确的网格功率证据分别核验。加8000后的7400仅是该截面余量；后继sorter/科研负载不得漏算或称已预约。见[内侧落点正例](./gameplay-timeline.md#2026-09-17--root内侧lab落点与实际供电预检通过)；最近复验同日，待正常施工后再验实际网络/库存，地形、工厂或发电变化即复验。

2026-09-17 / EXP-077选址粗筛边界修订（`validated`限源码和两点距离复算）：`placementRadius`是含高度/偏移的三维collider包围球上界，不是建筑水平精确footprint。上界球不交叠可供保守筛选，球交叠却不能反证原生collider必然碰撞；不能据此宣称整条拥挤走廊不可建。普通单件放置仍以既有fresh原生整座collider/地面预检为准，不跳过已有占位检查、降低Foundry保守规则或批准任意试点。三轮设计后移交root，见[选址交接](./gameplay-timeline.md#2026-09-17--lab选址三轮后转主会话重设计)；最近复验同日，碰撞模型、建筑姿态或DTO语义变化时重验，当前尚无新lab施工证据。

2026-09-17 / EXP-236、EXP-077地表线索复验（`validated`限第二native负例）：完整移动预览中的中心/沿线采样不能代表建筑离线的全部landPoints。第二候选(-66,-155)沿线接近水面、中心高于水面的线索仍被2901正常预检NeedGround拒绝；保留上一条“Move门不等于build门”的区分，同时明确任何原生拒绝优先、不能由样本深度反推通过。两处失败点退役、不继续沿岸换点盲试，见[第二地面负例](./gameplay-timeline.md#2026-09-17--第二lab候选整座地面预检仍失败)；最近复验同日，位置/姿态/地表或DLL变化时重验。

2026-09-17 / EXP-236选址解释修订（`validated`限证据分层，建筑合法性仍待native）：短跨越的末端4m/高水0.1m是Move接近与落地规则，不是建造门。第二lab候选的33点/66射线完整预览只有一个末端点满足该干地余量，因此不批准走到那里；但当前位置已在建造范围，不能据此替代`prepare_build`宣布场地非法。当前DLL逐landPoints检验整座建筑，移动路径样本既不能覆盖其NeedGround拒绝，也不能证明整座footprint通过。actual plannedPosition还要复查供电，纸面中心距/请求点覆盖均不是许可。撤销把该Move后缀不足称为第二次build失败的解释，保留首个真实NeedGround负例；包内指南与定向回归同步，见[移动与放置边界](./gameplay-timeline.md#2026-09-17--区分移动落地门与原生建筑放置门)。最近复验同日；动作域、地表、吸附位置或DLL变化时重验。

2026-09-17 / EXP-077、EXP-236选址复验（`validated`限2901的NeedGround负例）：空白中心、建筑间距和电塔覆盖均不证明地面合法。新lab候选(-50,-67)通过纸面粗筛，却被正常click-build的NeedGround拒绝；无功率/commit，保留已备好的成品，下一候选需以已有Walk及有界地表证据重新设计，不旋转重放同一区域。surface preview仍只作地表/岸边风险证据，不替代完整建筑footprint原生检查。具体调用错误与唯一原生负例见[lab地面拒绝](./gameplay-timeline.md#2026-09-17--紫lab首个落点被原生地面检查拒绝未施工)，最近复验同日；地形、建筑类型/姿态或现场变化时重验。

2026-09-17 / EXP-021、EXP-302备料复验（`validated`限一台2901）：以真实`itemId`核仓料，不能把1108石材当1005石矿。递归手搓有批量取整，已有1线圈、成品需4时仍需两批各产2，不能据“只缺3”线性削减原料。本片在fresh native顶层budget与最终完整背包核验下，仅消耗12铁/4磁/4铜/8石矿并新增1lab，原线圈1保持；静态纸面预算仍须正常prepare确认。详见[lab备料与保存](./gameplay-timeline.md#2026-09-17--独立紫糖实验室成品备齐并保存)；最近复验同日，库存/配方或来源变化时重算。纠正发生在写入前，不属于游戏失败或新增公共功能；成品库存也不授予放置、接线或持续生产许可。

2026-09-17 / EXP-302、EXP-303保存恢复复验（`validated`限当前PCB/微晶铜支路）：精确主档正常恢复后，完整4742实体/9274互返边和原J89保持，新连接继续产处理器；只能核销保存恢复，铜仓仍净降的限制不能被健康session或四个正窗口抵扣。旧session动作引用已封存终态，新session只fresh核本次恢复与现场，不假造旧缓存仍在。审计摘要里的`journal`数字不是原Journal DTO；遇到本地汇总字段错误，先保留原exit1，定向读取原回复并离线核销，不能重跑已经成功的游戏动作或整批只读采集。详见[恢复与全审](./gameplay-timeline.md#2026-09-17--pcb与微晶铜支路正常恢复完整场景保全)；最近复验同日，来源、现场、Journal覆盖或DLL变化时重验。完整阶段审计通过仍须推送绿CI和明确handoff才结束计数批次。

2026-09-17 / EXP-158、EXP-302补产窗口复验（`validated`限四个独立原生窗口与正常保存）：`get_overseer_production`每次是其自身尾随600tick统计，不是两次调用之间的累计量；多次采样的间隙须保留，不能用库存差填补为连续速率。唯一生产者/完整诊断可结合真实设备和连接做有界来源关联，但正产量与消费者增长仍不保证库存平衡。本次处理器仓223→231、铜四窗均产5，铜仓283→280，结论为真实补产且稳态未证。27的2012两格额定90/min排除了“普通分拣器30/min封顶”这一具体猜测；不把排除一个假说写成已证明背压根因，也不由炉60/min理论值承诺后继实际供给。详见[四窗与保存](./gameplay-timeline.md#2026-09-17--铜与处理器四个补产窗口及新保存点)。最近复验同日；新增消费、供电、来源/拓扑或恢复后重验。保存63991726后accepted1保持，九写归零仅依据先前完整审计、推送绿CI和明确handoff。

2026-09-17 / EXP-259、EXP-302与IFX-153后续复验（`validated`限本支路建造/接线/短窗及九写全审）：真实23带接上双1104过滤sorter后，848收到铜、微晶及处理器恢复生产，原849库存已为0，能与此前消耗旧微晶的启动区分。源仓不下降与铜/微晶计数增长共同支持补入，不把一次isWorking或全星球总速率当单机归因。最终满基础负载1532600、消费者463与满供实读，保存63905825覆盖新支路；4742实体/9274互返边/九终态和Journal原水位保持。IFX-153修复版本次所见容量均1584000，原1534948/代理false组合仅离线复验；以下“修复后实机待验”已由此窄正例更新，连续供电/燃料、长期供给和重启恢复仍待。完整证据见[接线与九写审计](./gameplay-timeline.md#2026-09-17--铜接通微晶与处理器复产九写全审及保存通过)；最近复验同日，供给/消费、容量/覆盖、现场或恢复变化时复验。

2026-09-17 / EXP-267、EXP-302功率预算修订（`invalidated`仅此前“1548000稳定容量下界”，`validated`限算术与原只读负例）：capacity从63867728的1548000变为63867738的1534948，旧值只能作为那个瞬时截面。不能由减去一台已知间歇机的额定值，就证明其余发电容量永久稳定。4500 J/t的Foundry假想设备仅用于既有覆盖/满基础负载取证；其完整预算失败不能等同真实600阶段失败，更不能改写为代理通过。真实计划每步比较fresh capacity与既有满基础负载加**全部尚未建造需求**，此片恒为1532600；缺额即停，不用低瞬时消耗、历史容量或等一个高点放行。数值分步、实际余量及代理原状态分别落盘；完成后再核全部新消费者与真实基载。IFX-153的9预算/17路径检查通过，原43read/0prepare/commit已核；修复后实机、连续供电和燃料仍待。最近复验同日；燃料、发电/消费配置、覆盖或时间截面变化时重验，此处覆盖历史同数值的稳定性推断。

2026-09-17 / EXP-259、EXP-302局部占位语义复验（`validated`限源码边界、`observed`限新纸面候选）：Plugin只将实际belt实体/预建筑交给Core的0.25m NEW中心守卫，不能把附近sorter、仓或电塔的中心距离误报成该Core拒绝；这些真实形状仍由native完整stage1检查。相同纬度的长新路可能覆盖一排旧带；有限改起点可以改变交叉相位，但纸面净距、长度和偏角均通过也不是原生许可。只计算明确局部对象和有限候选，三轮Sol未解决即交root；材料/电力预算中提前包含保存，不为直线失败拆改成功前缀。新铜候选及失败边界见[存档日记](./gameplay-timeline.md#2026-09-17--微晶铜源已定位三轮纸面冲突后收敛新候选)。最近复验同日；实际吸附、原生碰撞、实体端口/供电或DLL变化时重验。

2026-09-17 / EXP-158、EXP-302铜源需求边界复验（`observed`限两只读窗口）：848缺铜与上游“没有铜”不是同一件事。当前10炉有矿4、铜输出100且output-blocked，26存铜300，551持铜1，300是open path16的纯铜段并有2件；两个600tick窗铜0产、耗0/1。该证据允许规划新增真实消费连接，不能为了前置非零产量重建已完成铜矿/炉，也不能先声称源持续供给通过。连接后仍须证明原炉解除满输出、真实产铜/微晶增量、合理库存趋势及保存恢复；没有把静态库存冒充持续速率。证据为本次26回复保护记录，末读63686366/R39/primary63649858、0prepare/commit；SHA-256 **72F9E295C33CC1C722ABF5A771F563F3C02BD2D2694B46F8DDA5DFD339B32D2F**。最近复验同日；新需求接入、源/配置或窗口变化时复验。

2026-09-17 / EXP-259、EXP-302、EXP-303双端闭合复验（`validated`限本片接线、短窗供料、保存及全审）：带路按真实ID/朝向fresh检查后，两端可使用exact_slots，先接消费者再激活源头，保留旧旁路与输入。正常成本、双向新槽、26个新对象、全部4717实体/9222互返边与原Journal已核；主档覆盖本次施工。电路板自动送达且处理器短窗1/2件并不消除微晶上游缺铜：微晶0产/4耗和仓储72→68明确仍是库存启动。格线路径实际先经度后纬度，不能套球面插值；当前两条铜纸面L形路线已与旧实体/新PCB点冲突，离线否决，不送游戏。完整证据见[接线与保存日记](./gameplay-timeline.md#2026-09-17--电路板接通处理器并保存微晶铜缺口保持独立)。最近复验同日；源料、路由模式、拓扑、负载或恢复变化时重验，四写完整封存与后续批次计数须显式交接。

2026-09-17 / EXP-267、EXP-302调用方复验（`validated`限本次读流与双端预检）：私有入口导入纯函数并不带入它们执行时依赖的共享库存计数器；本次漏导入ActionClient在14次读取后、首个prepare之前失败，不能把异常文本或exit1推断为“0游戏调用”。以原保护记录纠正为14read/0prepare/0commit，补齐仅live分支依赖后，20调用的两个真实native exact_slots预检都通过。旧带路不重做，预检token不复用；下一仍fresh两端建造与save。最近复验同日；入口、导入顺序或DTO变化时重验，私有脚本修复不算产品代码功能或两端已接通。

2026-09-17 / EXP-259、EXP-302路径复验（`validated`限本条球面带的放置与施工）：三轮失败后，root从当前DLL的SnapTo/局部格距/球面插值规则重建局部格点，以5个真实点校准后只计算2条有限候选；纸面占位不足的候选直接丢弃。保留候选的24点与实际native路径最大差0.000148m，并由完整stage1及正常无人机施工证明，不能把双精度近似升级为原生裁决或全局自动寻路。旧仓不必作为中继，直接面向消费者空槽可缩短路线，但带路成功仍不代表两端sorter可放置、接电或实际供料；必须以建成后的真实ID/朝向重新prepare。原端点、材料和空带拓扑已独立核销，accepted1与未保存边界保留。证据见[本档24段施工](./gameplay-timeline.md#2026-09-17--电路板新球面路线24段建成旧线保持)；最近复验同日，DLL、纬区、原生吸附、端口或现场变化时重验，不重放三个已退役候选。

2026-09-17 / EXP-302、EXP-303第三路线复验（`validated`限零写拒绝与供料诊断）：改用球面直线仍不免除端点原生吸附和完整NEW占位；本次纸面候选在point0撞323，失败未提供完整吸附路径，不从请求坐标臆造实际native路径。三轮有证据的Sol设计失败后转root，不扩成Luna第四次盲试；已保存30带/2sorter和已封存accepted0保留。处理器复产必须同时核电路板与微晶上游：843无铜、848停产时，849历史微晶76只能支持有限消耗，不能用短暂处理器增量或零产零耗冒充持续供应。证据见[本档第三拒绝](./gameplay-timeline.md#2026-09-17--第三条电路板路线仍被占位拒绝交回主会话重设计)，最近复验同日；新路径、真实端口、库存/配方或源线变化时重验。私有预算键须沿用实际`construction-consumption`，静态保全不冻结含动态缓冲的配置哈希；相关本地修正在执行前通过纯函数检查，不是新产品能力或实机施工。

2026-09-17 / EXP-302、EXP-303第二几何拒绝与保存复验（`validated`限本阶段停止/保存/全审）：纸面独立路径中心线距旧带约1.5m，仍在`native_grid`实际NEW点1撞543；不能把未吸附直线净距外推为格线转折后的合法性，两个功率probe通过也不能放行碰撞。两候选失败后停止同类尝试，先正常保存已备物料，再核七个唯一动作、完整4691实体/9168边、净材料及Journal；提前完整阶段审计可在push绿CI后封存，不能部分核验便清零或重做备料。当前仅评估已支持的球面直线模式，未取得新路径许可或证明碰撞已解。证据见[本档保存与七写审计](./gameplay-timeline.md#2026-09-17--第二条格线路径拒绝已保存材料并完成七写阶段审计)，最近复验同日；路径模式、真实端口、库存/供电或重启变化时重验。

2026-09-17 / EXP-302、EXP-303备料复验（`validated`限四动作材料闭合）：独立PCB线路采用r84原生递归齿轮，而非拆成额外手搓调用；同一有限预算保留双sorter与save名额，accepted2→6后不归零。30普通带与2sorter已备齐，高速带单独保留，尚无场地/接线许可；正常保存仍待，durable Journal与成功terminal不替代世界落盘。优先直接执行已审阅文件并核调用/证据数：本轮一次stdin只读入口exit0但零调用/零证据，不代表任何实机检查；后续明确文件入口四动作成功，不重试旧意图。证据及时间边界见[本档备料](./gameplay-timeline.md#2026-09-17--电路板独立路线备料完成四次正常动作已核销)，最近复验同日；递归配方、库存、路线段数、暂停/退出或保存变化时重新核验。19.402秒只是执行段，不冒充整项工序耗时改善。

2026-09-17 / EXP-300、EXP-302供料预检复验（`validated`限当前零写拒绝及量纲纠正）：565→849唯一续带候选在NEW point2撞旧545，Core完整路径占位先于施工拒绝（不代表完整DSP stage1通过）；source-cover只豁免明确复用的源实体，不能豁免其他NEW点、改掉源绑定或重放同点。先复用已有PCB生产/输送线可减少源端sorter，但纸面28.178m不等于原生合法路线，565旁路573仍须保留。功率摘要的`energyCapacity`与设备buffer的`power-generation-current-tick`不同，不能相减就称排除该发电机；本例按已证间歇容量36000保守扣除，未来单sorter后余16300，而不是50081。错误规划派生值已否决，prepare负例不受影响；没有产品改动或消费/持续供电正例。证据见[本档电路板选址](./gameplay-timeline.md#2026-09-17--电路板复用直线被路径占位预检拒绝未施工)，最近复验同日；来源、带等级、路径、未来负载或发电证据变化时重算。不同等级库存不能混成同一带路的材料预算，额外带段和保存必须一同计入工序写上限。

2026-09-17 / EXP-300、EXP-302补电后缀复验（`validated`限本轮十台并网/保存）：先建七台后，新接网边界让剩余三台在无需Move的位置通过native；不能永远沿用施工前的可达电网或把未找到候选视为全局无解。九写在完整审计及推送绿CI后明确封存，后缀三台+save单独计0→2并保留累计；原七台、全4691实体/9168边、材料与Journal独立核销。前后全网容量含间歇热电，新增风电以唯一实体/同网节点/原生预算共同证明，不凭一次容量差宣称持续燃料或后续链满载已通过。文件名抄录错误被导入前拒绝，只纠正明确未执行的调用，不算native重试；精确复制已批准参数，不能猜测accepted。完整证据见[十台补电完成](./gameplay-timeline.md#2026-09-17--十台补电全部并网并保存转入处理器自动供料)，最近复验同日；后继负载、供料窗口、场景或DLL变化时重验，规划总耗时改善仍不由46.556秒执行段代替。

2026-09-17 / EXP-302有限工序落地复验（`validated`限本片施工/保存/全状态核对）：未取得最后三台合法布局时，保留已通过的3+4模块，限定两次build加save，正常新增7台/扣7并保存63153528；root核全部4688实体与9168互返边，九写保持冻结至阶段推送/绿CI封存。可以在阶段边界提前完整审计并结束批次，不用凑满十写才保存；这不允许部分审计后提前归零。两个原生候选的真实姿态与跨模块间距必须核对，执行仍全fresh，已成功部分不重做。执行106.838秒不包含布局/交接，不能冒充整个流程耗时；私有调用方的数组`.count`与元素同名字段冲突应以显式枚举和小型纯函数回归解决，不再创建全流模拟框架。完整证据见[七台并网与保存](./gameplay-timeline.md#2026-09-17--七台风机并网正常保存与完整阶段审计通过)。本轮复验EXP-018/021/300/303保持；剩余3台、未来用电、持续燃料/紫糖不在正例内，现场、程序集或下一有限工序改变时再核。

2026-09-17 / EXP-300、EXP-302场地复验（`validated`限两处零写负例）：补电首轮A实际native返回单对象`WindTooClose`，B则在完整native之前因1213占位拒绝；整图未通过时，局部`Ok`不授予子图或其他候选施工。纸面中心净距不能替代当前纬区原生翻译后逐对象/风机节点间距，未执行native时的名义条件不得当正例。两次同阶段拒绝后停止，C未调用，交Sol重新设计而非盲试第三处；已成功的10风机备料保留，accepted6不归零。普通蓝图纯新增风机的满基础负载功率字段仍为建议，不把旧网的部分缺口误写成native拒绝或要求首批就覆盖全部缺口；完整计划须有明确总补电预算，不增加或新覆盖消费者，最后并网及持续容量仍另验。证据只存[本档选址结论](./gameplay-timeline.md#2026-09-17--补电两处选址被拒施工前交回重设计)，最近复验同日；源选区、锚点、朝向、现场或DLL变化时重新检查。

### EXP-303 — Journal 与动作封存不等于世界已保存

- 日期/最近复验：2026-09-17；状态：`validated`，含指定来源元数据、当前DSP显式授权恢复及恢复后完整核销。适用：游戏进程已退出、已接受动作晚于最近正常保存的恢复决策。
- 结论：Journal durable89、成功 terminal 及独立 seal 能证明动作曾发生，不能证明这些建筑已进入可恢复主档。ticket minimum 也不是实际 header tick；必须定向核对，不能从 minimum 单独推断磁盘版本。当前 native v7 头部证实 primary61814274，晚于此的七写尚无落盘覆盖证据，其他备份未查，不能宣布已丢失所有备份或已经恢复。
- 流程：正常保存计入有限工序与 accepted 预算；第十写后仍严格冻结/审计。进程已消失时只保留旧动作的历史成功证据，不把新世界状态冒充旧 session fresh 审计，不离线刷新票据或盲重放已成功前缀。较新恢复来源或接受回档需明确授权。
...
- 本机闭环复验：`43b16c2`冷部署/64tools/1resource实际握手后，Luna只提交一次已确认62776058候选，正常保存62776089；root独立核完整4681实体/9168互返边、材料背包、原Journal与manifest保持，最后七写保留。旧十写以原终态加恢复状态核销，不能伪造旧session fresh审计；恢复单独计1并继续计数。一次库存833不等于持续1113，当前满电也不等于未来满载容量充足。详细证据只存[本档日记](./gameplay-timeline.md#2026-09-17--较新同身份-lastexit-恢复与七写核销通过)；原先“未部署/未加载”均保留为历史截面，当前边界由本条更新。异常拒绝仍离线，跨机/最终包另验。

### EXP-302 — 以有限业务工序复用执行与核验

- 日期/最近复验：2026-09-17；状态：`observed`。适用：已支持原语的本地规划、调用和阶段审计，不改变 Plugin/MCP 的准入。
- 证据：全部 995 个本地可达唯一提交纳入元数据/文件统计，关键阶段另核日记与原回执；9/11–9/16 的 474 个 main 提交有 462 个仅改 Markdown。缓冲备料到连接的支线跨约 6 小时 35 分日历时间，而末次分拣器真实调用流为 68.143 秒。此差异不能全归因于计算耗时或视为浪费，但结合重复 caller/fixture/auditor 缺陷，支持优先减少辅助实现与串行交接。
- 现行做法：按 AGENTS 的“执行流程收敛”复用已有入口，目标作为数据；单动作核材料/身份/受影响拓扑，十写与异常仍严格全审；纯函数测试与少量全流分层；日记保留事实，其他文档链接结论。完整原证据和历史记录不删除。
- 限制：这是已批准的流程调整，尚无调整后工序的耗时改善实证；不声称模型能力排名，不用提交数/测试数当版本进度，不减免 expiry、幂等、terminal、fresh native 检查或独立持续产量窗口。
- 复验触发：下一个完整业务工序、相同辅助代码故障再次发生、未知结果/恢复、最终验收。关联：EXP-299、IFX-106、IFX-135、IFX-147–151；早期共享入口为 `scripts/SpherewrightActionClient.ps1`，黑盒对照见 `docs/v0.3.2-sol-vs-luna-black-box.md`。

- 2026-09-17准入诊断复验：复用共享桥取得33份真实只读证据，root只核新增原回复与关键不变量，不重造全流fixture或重放全部历史；结论见[紫糖工序缺口](./gameplay-timeline.md#2026-09-17--紫糖链准入缺口定位供电余量与处理器电路板)。满基础预算不得重复计已存在的2255，也不能把氢热电偶发容量或单个18/min窗口当持续性。补电应覆盖已知后继lab和有界输送计划，而非每加一只sorter再重做同类容量判断；完整后继数量尚未明确时只列待定预算，不授施工。当前仍只是规划/诊断改善，尚无完整新工序耗时结论。

- 备料中的部分成功边界：723取铁70已完成，后续取磁只在prepare因整仓内容变化返回`STALE_STATE`，没有第二commit。先独立核销铁料和accepted1→2，再把同一调用入口参数化为明确未完成后缀，禁止重跑整批。Sol选择输入较少的562取磁、无连接的136取铜；前者只允许≤30秒观察一个≥120tick同hash窗口，随后仍fresh玩家/源仓并接受原生检查，不把安静窗口当作下个tick不变的保证；再次拒绝立即停，不无界等待或轮换第三来源。此时后缀尚未执行，不能先写成成功经验。关联EXP-021/299。

- 上述后缀随后实机通过：原料同步扣减、递归净成本和10风机产物逐项核销，主档正常保存62967243，accepted6保留，Journal89连续。复用了ActionClient与审计传输，14项纯函数/2入口/21共享测试，未新建全流模拟框架；root只核新增156文件及已封存前缀。执行55.684秒不包含两次执行之间约7分21秒的核销/设计/调用方调整，不能据此宣称整工序已经显著提速；此类交接时间仍须继续压缩。完整事实见[备料日记](./gameplay-timeline.md#2026-09-17--十台风机备料与正常保存完成)。没有修改产品准入，备料正例不授予场地施工或持续供给验收；EXP-018/021/303在本次材料和保存边界内复验保持。

- 后继规划再次暴露辅助开销：已被候选结果替代的全场嵌套PowerShell几何筛选仍消耗约740 CPU秒，另有针对巨大证据目录的旧标记全量搜索滞留。root核准实际命令和Agent归属后只终止这两个只读辅助进程，未停止DSP、未改游戏状态。两次引导分别错用session的`planetId`字段、把accepted6推作revision6，各只读一次session即停；原回执明确`localPlanetId=104 / revision7 / primary62967243`，候选尚未进入native，不能计作两次场地失败。已把区域/候选先裁剪、精确prefix读取、辅助进程生命周期和即时报告写入AGENTS；字段/版本只认真实DTO。此纠正不影响已验证风机库存和保存，不再为这两个本地错误造新全流测试框架。
...
2026-09-16 / EXP-299 单连接施工与预算时序（`validated`仅离线）：Sol E3AF95F5 的单次原生施工沿用已证明的 843/3→4680/11，但必须重新预检并核对原 expiry、现场 hash、全部预算和两端图。Foundry 的 ReservedDemandPerTick 是满基础负载保守预算与实际需求的较大值，不能等同某一瞬时 energyRequired，也不能用施工前后需求差认定一只设备的耗电。fresh 4500 代理允许真实失败或通过、原值照录；只有已知 300 J/t 的单只分拣器余量满足才继续，不冻结历史 2600。端点图自身 tick 必须与详情一致；提交前及结束的 session 上界必须覆盖玩家、预建筑、端点、场景和完整功率采样。

31 基础、root EAFC48F2 的 45 全流场景/16 禁写检查（PTY39499 exit0）、12 坏入口/解析和 21 公共客户端通过。测试仅替 transport/pause/persistence，实际调度和意图检查保留；准确证明明确拒绝、丢回执、回放、错实体/材料/端口及未完工均不触发第二次提交。8A409153 审计器 36 离线检查（PTY7948 exit0）和 Sol 静态复核通过，预备按原 record/intent 时刻及原等待分组核验完整流，不伪造 token、expiry 或墙钟；部署哈希和原秘密检查仍明确属于 caller 见证。实际施工尚未执行，正常最少136调用、上限800/600秒，0游戏/证据写入；真实末62105833/R143/accepted9不变，后继第10写后须严格审计，不能提前归零或保存。关联 IFX-147/151；最近复验同日，caller、DTO、原生程序集或现场变化时重验。

2026-09-16 / EXP-299单连接与实际功率边界本机复验（`validated`仅本次零写预检）：Luna8449/raw-12b99d0d/PTY64064 exit0得到原生843/3→4680/11、2011/filter1113正例；127调用/1prepare/0commit，末62105833/R143/primary61814274/J89/accepted9。rootF28/PTY10899 exit0核129原文件、完整结果/原expiry和manifest，独立封存050FAE74；不重放游戏或补造token。原拓扑4680/9164保持，仓已存在但仍未接通，不是持续生产或保存恢复证据。

本次实际满基础负载预算余2900 J/t：已知单只2011的300扣除后余2600；Foundry4500代理缺1600，必须保留其失败原值。只有实际原生sorter prepare通过、完整166节点身份/时序/几何及同一覆盖集合847/2031均已证明，才可把代理提供的网络证据用于这个300 J/t的窄判断；不能忽略真实放置blocker、伪称代理设备可建或替任意其他负载放行。现场变化后重读全部预算，不能沿用离线合成74700或本次2600。末token已丢弃、计数不归零，后继单写到10必须审核并严格审计。最近复验同日；安装、节点/负载、端点、人工操作或caller变化时重验。

2026-09-16 / EXP-299单连接预检闭环（`validated`仅离线）：Sol8449ED46的127调用只能prepare一次843→4680/filter1113，不能commit。后继现场首tick须不早于已封存建仓readTick；结束时再对比准备时玩家hash、两端endpoint hash/完整端口图，并将供电摘要、代理、前后166节点及快照全部限制在首末session tick内。只验证hash格式或只有起点下界不足以证明完整读回窗口；token无论结果都不复用。原生实际槽位必须是已选3/11，几何与300 J/t预算是额外拒绝条件而非放置许可。

35基础、root4BE22CF6完整33场景/16guard（PTY16152 exit0）、九入口/解析及21公共客户端通过；28功率检查见前一独立切片。测试必须锁定真实首次拒绝的调用计数和错误，而非任意异常；本次第60调用正确拒绝已有施工，fixture误期望“prebuild”，修为实际“pending construction”后完整重跑，未改生产守卫。F28F913D审计器26离线/静态通过，预备消费129原文件/127响应，只替原expiry时钟与脱敏token形状，完整结果及原manifest保持，封存前再次检查caller/self/power/store-parent/store-proof哈希。installedCohort、秘密存在及实际请求实参属于原caller见证，不从REDACTED或历史MCP元数据补证。真实末仍62017248/R143/accepted9；绿CI后仅Luna单次零写预检，不是施工、持续产出或恢复验收。最近复验同日；任何现场、caller/依赖或DTO变化须复验。

2026-09-16 / EXP-299单连接功率窄适配（`validated`仅离线）：C2A5BD32从已验证的单分拣器纯功率规则派生，仅更新R143、net3 consumer458、完整4680实体及本次scene证书；原166节点双侧时序/身份/位置、实际端口投影和完整覆盖集合检查保持。Foundry只读代理仍诚实保留4500 J/t原预算，未来实际施工仅需另外预留一只2011的300 J/t；代理本身不是施工计划，不产生功率预约。

独立root8398E07E的28检查/PTY61188 exit0使用已建仓原记录形状和明确合成的时间与预算，不修改raw：300余量正好通过、299必须拒绝，另核旧revision/消费者数、原生类型、缺失或移动节点、重复快照/时间边界、供电和代理几何等反例。adapter/fixture六坏入口在import前拒绝、两文件解析通过，0游戏/证据写入。此结论不替代完整caller全流或fresh现场预检；真实末62017248/R143/primary61814274/J89/accepted9保持，没有新物理失败计数。下一仍由Sol完成有界预检设计，经独立核验后才交Luna，后继第10个accepted仍须严格审计。最近复验同日；DLL、现场、预算对象或caller变化时重验。

2026-09-16 / EXP-299单仓与原施工分组本机复验（`validated`限一次正常施工）：Luna5529/raw-e1a8d554/PTY42373 exit0以1仓正常成本和无人机建成4680，真实30格/12空端口、空仓无连接，实体4680/旧互返边9164保持。末62017248/R143/primary61814274/J89/accepted9，原仓材料1→0、3带/1分拣器保留；无人机返航不等于可立即继续写，后继须fresh ready。root4F0D63C8/PTY13993 exit0核226原文件/223响应及整个结果，封存D63421DE，原manifest不变；不是重新调用游戏或重放已建对象。下一只可设计843→真实新仓的单连接零写预检，持续1113和保存恢复仍待。

原记录审计的Wait分组须以原时序为准，不能使用审计机器的新Stopwatch决定原流下一条是什么：本次首轮50响应/29.8886524秒后转prebuild/session，第二轮另27响应含复读终态。4F0D63C8保留原8轮/每轮60次上限和Action/Progress守卫，只替原墙钟break；提前结束须原首末响应≥29.5秒保守下界，否则拒绝，不能补造等待或结果。29离线/三入口/解析与静态复核通过；83CC未运行该原流，没有一次可虚构的旧审计失败。IFX-151在此窄原记录路径闭合，IFX-147正常施工复验；异常分支仍离线。最近复验同日，DLL、DTO、时序、现场或caller改变时重验。
...
26基础、6E015C81完整18场景/16guard（PTY79977 exit0）、双方六坏入口/解析及21客户端通过；两正例分别为请求点不变与明确合成的吸附偏移，不冒充实机。5118F2CB成功原记录审计器18离线/静态通过，但原流尚未发生；将只重验139文件/137响应，保留原expiry并使用原record时刻、脱敏token不补证秘密。最新真实边界61924904/R141/accepted8保持，未施工/保存或核销持续产出。关联IFX-150；最近复验同日，DLL、caller、候选或现场变化时重验。

2026-09-16 / EXP-299五写材料本机复验（`validated`限正常备料）：Luna1C9/raw-0512524a/PTY25348 exit0三次守恒取料及两次正常手搓，rootDDA/PTY77250 exit0核236原文件/224响应并封存2586CF0E；五唯一成功终态、完整材料净量、47页双场/13详情及durable J89保持。末61924904/R141/primary61814274/accepted8，净增1仓1分拣器、原3带不变，不重放或归零。原expiry只与原record/intent时间核对，秘密等值/唯一性仍属原caller见证；没有重造token、改原文件或声称保存/施工/持续生产。失败分支仍为离线证据，IFX-148此次原记录审计通过；最近复验同日，DTO/调用方/现场变化时重验。
...
同批规划复核：容量关闭同一快照61773742中101输出100+843存储1+4665携货1+4666携货0，已有1113下界102且未计带货，足以拒绝原100容量保守预算，不足以声称任意未来生产窗口物理不可行。不能用异tick详情相加、搬空历史库存或降低非零生产标准替代消费/容量方案。最近复验同日；caller/DTO/原生save、回执语义或现场改变时重验。

2026-09-16 / EXP-299单格容量与库存/产量边界实机复验（`validated`限本机正常配置）：Luna450F4CA7/raw-64c8fd0b/PTY53670 exit0，843于61773739正常SetBans30→29，唯一accepted1→2，原过滤、四边、2900高纯硅及同步有序buffer/inc保持。root36428248/PTY3702 exit0独立核146记录/143回复、唯一先落盘意图、双终态和全场4679/9164/13详情，封存0A498D36，原文件不变；28离线/坏入口/解析及Sol静态审阅通过，审计0游戏调用。最后61774204/R133/primary61386568/J89/accepted2，未保存或归零，IFX-146在此窄范围已复验。

仓1113从0到6只能证明正常到货，原101输出已有100积压、关闭为98，其他库存各有自身capture；不能用异tick加和捏造守恒、把存量搬运当新增产量，或把一个保守预算拒绝说成物理不可行。一格100的持续窗口准入仍未成立，后续先保存成功前缀，再设计真实下游需求或有充分fresh证据的有限容量。普通保存、持续非零native统计和保存恢复仍分别待验；新配置/生产链/DTO或现场变化时复核，最近复验同日。

2026-09-16 / EXP-299单格输出容量调用方边界（`validated`仅已列离线检查）：843已有第四条4666连接，不能套用旧三连接基线。只开首个1113预约格的正常SetBans30→29必须绑定fresh完整仓储hash，保持全部过滤、四边和同步有序非空库存；稍后原生到货与同步配置守恒分开。包装返回值在最终capture需显式解包，Journal无planetId且需保留原capture供闭窗验证；场景仅归一化获批bans字段，不能用旧配置整体覆盖待验值。

450F4CA7在提交前落盘唯一幂等意图，认证完成后才计实际请求发送；收据异常保存key/已知handle及accepted，不重放或虚构回滚。30基础、root4387AC2D的71检查及三坏入口/解析通过；36428248原记录审计器28离线、入口/解析和Sol静态审阅通过，未来原记录未读取。19个完整运行场景按未改caller分组覆盖：83AC88E1前14例完成断言，原PTY38124因第15例错误期望文本exit1；仅修测试期望/筛选的3EAD79F0将后五例重验，PTY98855 exit0/10检查。两种正例143调用/一模拟写，负例锁定具体首个拒绝点与实际调用/写/意图/身份计数，不把任意早期异常或旧进程exit1改称全组成功。

上述检查全部0游戏/证据写入；最后现场仍61707511/R131/primary61386568/J89/accepted1。本片推送绿CI后才可交Luna唯一fresh配置，原生配置、持续产出与保存恢复仍待。完整模拟时间线须一致，原生值类型不递归展开；全场先取完47页才校验，检测调用数不能按目标实体所在页猜测。最近复验同日，关联IFX-146；调用方、DTO、现场或库存规则改变时复验。
...
2026-09-16 / EXP-299与IFX-145 中断后只读闭合边界（`validated`仅离线）：成功action的后验脚本出错时，后继只读计划应绑定原终态、扣料和唯一实体证据，不能重新prepare/commit或重置accepted。182BAE5E只允许固定69读，并保留完整场、当前双向端口、配置、玩家、durable J89、科研、电网和零prebuild门；历史首次事件仍绑定原发生tick，不能错误要求它发生在新读取之后。时间适配接受的不同CLR形态应比较同一UTC瞬间，其余条目完整比较，不重新要求同一CLR类型。

144基础、root236、8完整场景16检查、三坏入口/解析和21公共客户端通过，0游戏/证据写入；每个负例同时断言调用数与具体拒绝原因。proof.gameCalls/gameWrites是整数0而非bool，首稿已在offline被拒绝并修正。最后现场61651777、accepted1保持，fresh实机和持续生产仍待；新caller、DTO或现场变化时复验，最近复验同日。Sol设计/规划，Luna待单次授权执行，root独立复核；离线脚本缺陷不伪造为游戏方案失败次数。
...
2026-09-16 / EXP-299十写后交接复验（`validated`限本批无游戏账本）：审计公开0aff551及CI35057952216成功后，C0F4637F/PTY68649 exit0只生成raw-042bd871/C29A0AF7单条handoff，绑定完整82审计文件并令accepted10→0；root再次核对manifest，0游戏调用/写入，不声称刷新61603347现场。31离线与三坏入口/解析通过，下一施工仍需fresh完整边界、材料与原生prepare。

调用方经验：JSON对象按完整键集和typed值比较，不把Hashtable键类型/遍历顺序当物料变化；get_session_state原生envelope.sessionId为null而result携带真实session，不能直接复用action envelope门，也不能放宽其他方法身份检查。当前源码及本次两个session/其余78回复一致。此修正只限私有账本，安装/协议/证据变化时复验；最近复验同日，关联IFX-143及存档日记。

2026-09-16 / EXP-299十写fresh实机复验（`validated`限本批完整审计）：root9F8BF372在raw-d508903a执行固定80只读，PTY2163 exit0；61602883–61603347核销10终态、全4678实体/9160互返边、14详情/166节点、两网/玩家/J88/科研，七组2883原文件manifest保持，证明999ABE20。玩家已ready，全部物料/inc只见十动作已解释差量；C/H成功前缀保持，H源仍未接。IFX-143的显式端口和物料门经本次实机读取通过，无游戏改动。

当前R129/primary61386568、accepted10仍冻结；80-read本身不能偷偷归零，须公开记录及明确交接。228安装文件fresh比对与64/1历史同批元数据区分；选中带空载不等于全线货物采样，保存、恢复、持续晶格硅都未由本审计核销。现场/程序集/脚本/下一批材料变化时fresh重验，最近复验同日。

2026-09-16 / EXP-299与IFX-143 十写审计判据复核（`validated`限当前私有审计器离线）：端口归一化只用于比较未变部分，目标槽必须另核验typed占用、对端ID及对端slot；物料单位和inc不能依赖PowerShell转换。reader9F8BF372加101的H输入/C输出及843输入端口精确检查、countUnit字符串items与inc整数0。contractCC138D15的66、reader173、完整80模拟读取的20检查、三坏入口/解析通过，0游戏/证据写入。完整模拟捕获并修正单行数组展开，未放松场地、原终态、材料或拓扑门。

843原有2900高纯硅合法保留，预约一个1113空格不代表清仓；562同样允许原有铜/磁铁库存，不能从目标产品推定仓内只有一种物品。本批十动作来自七组独立封存原记录，必须绑定原manifest/收据与终态幂等身份，再fresh核对；accepted10仅在完整审计、公开记录及明确计数交接后才能开启下一批。上述测试不是实机审计或持续产出，现场/脚本/封存/安装变化时重验，最近复验同日。

2026-09-16 / EXP-299与IFX-142 H炉端实机复验（`validated`限当前一写及原记录审计）：7FFE6284/raw-3788f356正常建成4678，4677/slot4→4678→101/slot1/filter1105，2→1sorter；已有C输出4665与全场旧配置保留。完整47页前后4677/9156→4678/9160，末61556324/R129/primary61386568/J88/accepted10。Luna PTY38811 exit0，root F72A72AB/PTY55203 exit0在223原文件/220回复上分段核销并以独立run封存62EB86F9，22离线/三入口/解析通过，审计0新游戏调用，原文件不变。不据脱敏占位符证明秘密相等，也不声称回放原墙钟或推断进程退出。

阶段边界保持：正常施工完成、无人机返航、稳定生产是三个不同条件。末returning-only可如实结束这一写，不能继续提交；第10写后必须完整fresh审计和明确交接，原记录核销不代替它。H供给端未接，首尾及新sorter空载，不把连线证明当产量证明；尚未保存。后续现场、脚本、安装或计数交接变化时复验，最近复验同日，关联本档日记及IFX-142。

2026-09-16 / EXP-299与IFX-142 已接线设备不能复用早期零连接门（`validated`限当前私有H炉端caller离线）：复用旧helper之前须核对它携带的阶段假设。La-AssertRecipe101原适用于无连接空炉，现在101已拥有C输出4665；不能删除这条成功边满足旧断言，也不能从比较中豁免所有边。7FFE6284以当前封存配置、完整端口图、recipe37/缓冲/满供为基线，只核销本次新边与目标槽。新H链未供料时保持首尾空带及新sorter空载，typed native builderValue只证有限值/单调，不引入未证明0..1范围。

证据与限制：22基础、root39纯检查（含真实已有C正例、丢C边与typed供电反例）、三坏入口/解析和21公共客户端通过；最终7FFE的A7F536F2完整22场景PTY29871 exit0，root独立核对fixture及逐场景失败原因/计数。正常135调用与长等待196调用均只模拟一写；无回执保留不确定性，有handle的错误回执及进度/读回失败保留原handle，不重放。测试0游戏/证据写入，不写成实机成功。测试本身的历史player/expiry类型/未验证pendingId误因与生产缺陷分开。当前accepted9不归零，后续一写到10即全审；现场、caller、helper或安装变化时复验，最近复验同日。

2026-09-16 / EXP-299与IFX-141 H端点实机复验（`validated`限本次零写预检及原记录核销）：0DE80FEB/raw-dd803f99的两个真实端点原生prepare均通过，分别计入两只未来普通sorter600J/t后余量74900J/t；开闭完整4677实体/9156边、C/H前缀、3带/2sorter、ready和J88保持。两个先后成功的预检不构成联合预约，也不能复用token施工。末61489408/R127/primary61386568/accepted9，零写，不归零；源设备842不是仓储。

原记录审计必须隔离“待验执行runId”和“新审计runId”：只在纯判据重放期间临时绑定原身份，finally恢复审计身份后才能写新证明。72C607AE同时在读取前后检查完整文件名/SHA和受保护ACL，逐项检查连续ordinal/原run、固定首尾/调用序列及冻结caller/power/发布CI来源；不往原执行prefix追加审计，不拿脱敏占位符证明真实秘密相等。root PTY94286 exit0封存63217811、原148文件不变；26离线/三入口/解析与21公共客户端通过，审计0游戏调用。该风险在草稿审阅时修正，无实际污染；caller、原记录、部署或现场变化时复验，最近复验同日。关联EXP-299/IFX-140/IFX-141，下一写至10后必须先完整审计。

2026-09-16 / EXP-299与IFX-140实际H端点预检后继（`validated`仅当前私有caller离线）：Reset只能清运行状态，不能删掉入口此前构造的不可变调用序列；native拒绝仍是一份已发请求，attempt/prepare应在调用前记录，nativeSent另计，dispatch必须finally关闭。若transport再复查依赖计数的同一guard，先递增会反过来拒绝合法prepare，因此只保留一次完整请求守卫和独立transport入口约束。0DE80FEB按原H11真ID固定两个只读候选，保全C19/双C/H11，不把原生拒绝当无调用或重试许可。

证据与限制：D4F0BA8D每候选独立600J/t预算与166节点括窗；15基础、root30纯检查、9实际Hs-Run模拟场景/10检查、root额外两失败计数检查及三入口/解析/21公共客户端通过。root PTY60883 exit0，首/次native拒绝准确68/83调用和1/2prepare，第二次只保留第一候选preview；0游戏/证据写入。fixture曾缺真实几何margin字段而先被正确拒绝，补齐后才算完整正例，不能以“抛错”代替到达目标反例。新caller只是preview，不代表两分拣器可施工；实际供电/端点、库存和完整场仍须fresh原生验证，两个预算不是联合预约。末真实R127/accepted9不归零；调用方、helper、现场或安装变化时复验，最近复验同日。

2026-09-16 / EXP-299与IFX-140 H11实机复验（`validated`限当前11带调用路径）：A540449C/raw-72c3210d一次正常build在61450410完成11新带，原10次进度页11/8/8/8/5/5/5/2/2/2与最终唯一正ID一致。多轮等待必须保持原action和原负ID集合，允许已完成对象消失、不允许重新出现；不能因单轮数量不降就重建或把离线mock当真实进度。两47页4666/9136→4677/9156、原C19/双C、玩家完整材料/inc、两网/J88和零prebuild通过，accepted8→9/R125→127不归零。末无人机非idle只保留本步成功，不准入下一写。

证据与限制：Luna PTY15066 exit0；root29BC9349对691原文件688回复分段复核，PTY45806 exit0/6原记录检查并封存06085FD0，19离线/三入口/解析与21公共客户端通过，审计0游戏调用。prepare与intent两处秘密字段均只检查已脱敏形状，不能由占位符独立证明原secret相等。实际头尾4668/4677仅为新一轮端点原生预检提供身份；米距、格距或成功建带都不代替分拣器放置批准。末61450736/R127/primary61386568/J88；H尚未供料或覆盖保存，持续产出/恢复及其他0.4门仍待。现场、原证据、caller、helper或安装变化时复验，最近复验同日。

2026-09-16 / EXP-299与IFX-140 H11有限后继复验（`validated`仅私有caller离线）：续建输入带不能直接复用原33带/4sorter及C槽空闲的整批预检。A540449C绑定已保存4666实体基线，分别要求H842/11与101/1空闲并保全已完成C配置，库存改为当前14带/2sorter；11NEW期望由fresh native plan及终态/完整旧场保全证明，不重放旧token或改动成功前缀。Gb helper实际返回refreshTerminalFirst，不能与S2的refreshSameHandleFirst混用；显式初始化纯函数依赖，避免旧REPL恰好有变量。

证据与限制：25基础、00D99219的14实际调用Lh-Run模拟场景/28检查通过，含11→5→0、原handle待决、回执不明及多种漂移；root37纯判据覆盖计划类型/预算/路径、请求边界、玩家和时限，入口/解析与75 Core回归通过，0游戏/证据写入。root审稿修正三种路径模式和每页snapshot的类型检查、负speed拒绝；旧snapshot反例曾先被过期tick拒绝，已改成同一fresh正例再单独变更字段，不能把误因拒绝算作字段覆盖。C7→A540反向字节SHA证明只变更该offline测试，不改执行体。原坐标格距重构不是原生sorter预检，真实H施工、接线、供料和产出仍待；当前R125/accepted8不归零。现场、原证据、调用方、DLL或恢复边界变化时复验，最近复验同日。
...
2026-09-16 / EXP-299与IFX-140第二C后缀实机复验（`validated`限本机接线）：07C8E0F1/raw-1d9c48d2先fresh验证原4665实际物理槽/双端和无人机ready，仅一次新动作在61351508完成4666；4648/实际slot4→4666→843/slot7/filter1113、2011仅3→2，首条与C19无重做。两次原handle成功terminal、两47页4665/9132→4666/9136和完整玩家/inc/J88/配置/166节点功率通过。最终returning不是此次失败：没有下一写的有界片可如实终止，但下一片仍须fresh就绪，不能继承旧ready或自动延长原失败caller。accepted6→7/R122→124保持累计，不因换执行器/审计重置。

证据与限制：Luna PTY10314 exit0；root DBEAACDF/PTY60778用197原文件194成功回复核销封存EF7DCB5C，6原记录检查、11基础+9独立纯检查及四入口/解析通过，0新增游戏调用。新功率保守1200J/t余74600J/t只属本次括窗，不沿用首条38900/旧preview74900，亦非未来预约。末61352042/R124/primary60758779/J88；该保存尚未覆盖C19和两连接，H输入、持续产量和保存恢复仍待。原过程exit与原记录成功分开来源，成功模板明确拒绝partial/stop，不补造终态。现场/调用方/安装变化或后续保存时复验，最近复验同日。
...
2026-09-16 / EXP-299与IFX-100格距/既有中段接点复验（`validated`限原坐标与源码重构）：root E0BC40EB/raw-93579719以已封存19带的2次经度/16次纬度格步推断segment200，明确不是直接runtime grid观察；复算旧首端4.210155格，米距/端向通过仍不足以满足one-belt/device≤3.499。既有4661/4648中段各有唯一必要条件合格对，约2.205格/0.587°；选接点前先核整条已建路径，不应因首尾不可达就另加中继或重建成功带。19带/18旧边、方向/货物/物理槽及fresh原生准入仍须保全；纯计算不核销施工、碰撞、供电或持续产出，也不伪造原错误实际分支。4纯检查/11原证据检查通过，无新游戏调用；包内指南吸收格距独立门、内部接点和禁止同对重试，2项嵌入资源MCP测试通过。现场/DLL/路径或端口变化时复验，最近复验同日。

同日`validated`限两处新接点native preview：Luna66B8D9DA/raw-b2812748的148回复/两prepare通过，真实PTY43577 exit0，61241849→61242765/R120/primary60758779/J88/accepted5，0写。101/slot4→4661及4648→843/slot7均是C输出接点；H输入仍待，不能因两个接点误称两条物料链。两个独立166节点功率括窗各按4sorter/1200J/t留下74900J/t，4500J/t代理余量71600J/t不可混用；实际覆盖最小保守余量分别0.062410m和4.208787m。开闭完整4664实体/9128边、19带/18内部边、14带/4sorter及其他配置保全；计划不构成联合预约，后续施工必须逐一fresh prepare。E56B92D3审原150文件并重放同一调用方，5检查/三入口/解析通过且零新增游戏调用；7模拟/57检查的业务体与最终入口由13检查证明等价，最终21基础/8旧边反例另验。仅关闭替代接点预检，不关闭施工、保存、H线或持续晶格硅门；包内指南尚未冷部署。最近复验同日。

同批私有依赖复验（EXP-299，`validated`限精确本机重现）：新指南的Release测试会正常重写可变bin程序集，导致冻结reader的旧SHA不再匹配；不是安装漂移或游戏隔离，尚未因此执行游戏。只设置SourceRevisionId的两次重建不能复得旧字节，因为SourceRoot.RevisionId仍将当前HEAD写入SourceLink/PDB。先证明Core/Contracts/构建props相对830e3c1无源码差异，再按已读SDK targets同时重现两个revision元数据，正常build0警告/0错误，精确复得Contracts909CB49C/Core3331B353原完整SHA并私有封存副本。未改reader哈希门或安装文件；以后冻结依赖使用可复核私有副本，新的发行/部署仍须从当前候选真实构建，不能把历史身份重现当作当前发布包。最近复验同日。

2026-09-16 / EXP-299与IFX-100接点必要条件复验（`validated`限本次原生负例/停止边界）：A3B4160C/D08C3E2C经21基础、7流程/46检查及Sol审核后，raw-65833656只发一次101→4664 prepare；原生exact槽nativeChecks1及后备4候选仍TooFar，bestFacing2.892°<11°不能覆盖失败。现有源码还独立限制one-belt/device原生格距≤3.499，米距≤5.5及端向只是必要条件；本错误没有公开实际CalcSegmentsAcross值，不能伪造是哪一个距离谓词的实测数值，更不能把TooFar当OutOfReach靠移动重试。一次prepare里的有限候选枚举不等于外部重放，失败后B/proxy/闭场均未执行。root C0B47A1F/raw-3913747c以原71文件/69回复重放并封存准确负例，5专用/三入口/解析通过，0新游戏调用；原Luna exit1保留。开场4664/9128、末session61193618/R120/primary60758779/J88、末观察61193634，accepted5不归零；叙述中的旧accepted4不得覆盖保护账本。第三轮虽有C19成功前缀，但完整连接未解，不能靠细分子步骤清零三轮设计计数，交root重设计并保留成功实体。计划/实际格距、现场、DTO或调用方变化时复验；最近复验同日，持续晶格硅仍待。

2026-09-16 / EXP-299功率代理与审计证明范围复验（`validated`限源码/离线）：同一Foundry代理的additionalBaseWorkPowerWatts为270000W，component.addedBaseDemandPerTick为4500J/t，不能混用；首稿4500W判据在进入游戏前由源码及旧原生DTO纠正并加入负例。两个零写候选各自按未来四sorter重新保留1200J/t，不把A预览当作B的预约；允许4500代理预算不够但1200需求仍够，不从已扣4500的headroom再次减1200。166节点括窗/实际sorter投影与proxy覆盖仍逐次核验，2303的placementRadius及其占位失败不构成sorter碰撞判据。7流程/46检查包含单位、原生类型、先A后B、A失败不试B、B失败不重放A、端点hash变化和最终安全导出；本次live在A prepare阶段因原生拒绝止步，功率代理仍未live执行。原记录完成标记只能证明记录前缀，不能证明后续stdout/进程exit0；原记录审计须显式分开进程退出证据。没有新增公共工具、DSP调用路径、写入放宽或版本完成声明；最近复验同日。

2026-09-16 / EXP-299与IFX-140原生完工和调用方截止分离（`validated`限C19实机及独立只读闭环）：raw-ff312c36在61142640已由正常无人机完成19带，原负ID集合不变而现场集合19→16→13→10→7→4→1递减，最终两次成功终态为19唯一正ID；builderValue只有很小正数也是真实进度，不能当百分比或提前认定卡死。原整批本地600秒在完整关闭场景之后到期，exit1/1132回复/accepted5保留；这不是原生超时、未提交或重放许可。root 9F63647F以原1135文件核清成功边界，再做独立有界78只读/240秒尾审；raw-5e972858末61155160/R120、全4664/9128、14带/4sorter、三设备/原J88/科研/两网/零prebuild通过，零新写，不延长原caller或清零计数。10专用离线/三入口/解析与Sol审阅通过；保护记录按文件哈希封存。JSON对象键顺序不是字段语义，跨PSCustomObject/Hashtable不能直接比较JSON文本；记录凭据已经脱敏，验证占位符形状不等于复原秘密绑定。以后为正常施工和独立尾审分别声明有限预算，墙钟截止只冻结精确前缀；动作仍pending时须带原进度状态和原生deadline交接，不能从零重置observer，本次已terminal不执行续观。未保存、供料、sorter或持续产出，未验证真实网络断线；后续现场/安装/调用方或DTO变化时复验，最近复验同日。

2026-09-16 / EXP-299与IFX-140建造回执/逐对象进度复验（`validated`限当前源码和离线调用方）：本批普通Build在返回成功回执前已创建预建筑，必须按waiting_for_game、正开始tick、完整19负ID及空完成tick/after hash建模；不能用假想queued正例证明真实路径。EF7873DC先保留accepted=true/合法UUID的原handle，再验证其余回执并决定是否清除不确定标记；string/int/bool需显式核验，动态比较会让布尔值或数字字符串冒充合法DTO。4EEF94F2真实执行31场景/66检查，通过原观察器19→10→0、终态19正ID及完整全场/库存/最终安全导出；另13基础、五入口、两解析、21公共客户端通过，Sol独立审查无阻塞，零新游戏调用。预建筑消失后的返回中无人机只能标成无新写授权，不能当成未完建造或下一次施工许可。C19限制一prepare/commit、同handle最多20轮/总600秒、单请求30秒；截止只冻结原句柄，不重放，不宣称真实网络故障已测。源码/安装/动作族或DTO变化时复验，最近复验同日；当前accepted4、末实读61045083/R118/primary60758779/J88保持，H线、sorter、供电预约和持续生产仍待。

2026-09-16 / EXP-299原生格线端向与IFX-066哈希域复验（`validated`限当前双路预检）：固定C/H改用native_grid后，raw-83494010实际获得19/11个NEW点及整路stage1通过，四端必要距离/四向偏差筛查通过；root raw-9b8ee532核原27文件并重放25响应及最终安全出口，零新增游戏调用。不能把计划相邻点推导的四向轴当成实际带旋转，也不能用整路放置通过宣称sorter/供电通过；只有带正常建成、逐ID和内部连接核销后才能用实际端点fresh准备附件。无源绑定的普通Build计划expectedStateHash仍是服务端组合绑定，不回显玩家hash；9F28D5EE按真实源码检查其规范格式，fresh玩家hash仍传入请求，未改服务端state-hash/幂等/原生检查。50纯判据、23调用方、4完整模拟、5原记录检查、56 Core与21action-client通过；合成正例特意使用不同hash域，已有IFX-066经验不能只应用于configure。原生preview零写、accepted4不归零，旧token丢弃；下一仅fresh建C19再验证真实端点，不因第三轮成功扩成整图施工。实际场地/端口/材料/安装或DTO变化时复验，最近复验同日。

2026-09-16 / EXP-299四材料原生正例与记录边界复验（`validated`限本次有限备料）：raw-e6cb51d6实际四commit成功，r84×11的真实终态为Fe−33/belt+33、耗880tick，r85×4为Fe−8/Cu−2/sorter+4、耗240tick，不能把阶段累计玩家净量Fe−3/Cu−1误写成最后配方扣料。原handle分别101/29次读取含末次复读，证明不能按顶层estimated ticks或旧五秒等待声称递归手搓失败；普通有界轮询本次有效，不放宽截止、不重试成功动作。root DAD29141/raw-d1210a54核原268文件/257回复、完整开场与四终态，8检查/三入口/解析通过，零新游戏调用。全局Journal只有session身份和capture tick，没有planetId；局部玩家/工厂/配方DTO才核planet，首版审计exit1及修正保留。跨Agent交接必须绑定当前切片commit，而不是旧阶段常量；本次旧HEAD误判发生在执行入口之前，没有raw或accepted变化。J88身份/持久水位/条目与其他库存保持，accepted4不归零，未保存/施工或证明吞吐。观察契约、配方、递归队列或交接切片变化时复验，最近复验同日。

2026-09-16 / EXP-299复验有限手搓的两层预算与执行出口（`validated`限当前原配方、历史终态和离线调用方）：r84/r85的prepare预算只列顶层gear/circuit，不把递归叶料列入同一扣料表；按运行时r5/r50/r84/r85展开，33带/4分拣器实际需41铁/2铜。DBFC7E70分别核顶层四字段预算与原生终态叶料净量，68离线及五坏入口通过；5266678C另绑定真实efb r84×10、9d6 r85×2历史记录，13检查/9场景通过真实四步路径和最终La-ReportJson安全投影，root独立exit0。prepare被拒或原action持续pending时，只保留成功前缀/handle，不重放整批；120×500ms轮询受30秒请求/240秒整批网络截止共同约束，截止不是失败动作的重试许可。原J88的historicalManualItemIds已经包含中间物/器材，不能因加工再次生成首产。夹具自身两次开发exit1与调用方早期整数键显示错误均保留，测试修正不计物理运输设计轮次。新材料四步尚未发生；fresh安装/owned/session/revision/完整物资、配方或Journal任何变化都必须停止并重新设计，不借历史证据放行。最近复验同日。

2026-09-16 / EXP-299复验既有斜带端向经验（`validated`限原端口与离线几何，非原生放置）：Sol第2轮HPS/晶格硅双外侧斜直线在root复核后撤回。5B07886D/raw-780b818a以e3fc三份详情及真实Core double函数核108直连对无合格者；两理想geodesic端的最小四向偏差分别40.773818°/40.673677°与14.630597°/14.580323°。设备outward射线上有带端且米距离合格，不能证明带自身四个虚拟方向与设备相对；这正是现有包内指南要求直末段、先关键接点再扩路线的原因。16离线/三入口/解析及原记录复核正常exit0，零新游戏调用；尚未观察native吸附、rot/collider或实际偏移，不将数学模型冒充原生拒绝。规划计算复用Core，手写角度须浮点clamp，不能因整数重载把余弦取整；特定端口对不冒充全局最短。第3轮仍交Sol设计，未建带没有正实体ID，普通sorter预检不能提前认证虚拟端点；需有限分段、新prepare与已建前缀去重。规则、端口、路线或现场变化时复验，最近复验同日；accepted0不再reset，原成功配置保留。

2026-09-16 / EXP-299先以真实端口否决不可能的运输计划（`validated`限已审现场/当前原生规则）：root/Sol对e3fc的101与843复核全部108自由native-slot对，最短10.723445807m仍超过当前非belt↔非belt7.5m硬界。74F86EE2经12离线/三入口/解析，raw-dd8ec26b对AE0BA95F DLL重新反编译核证、正常exit0；15 Core端口几何测试通过。公开端口与prepare均来自相同prefab世界变换，候选preview原样保留位置，两个非belt端点不能进入one-belt偏移后备；因此不必为已被必要条件否决的方案先手搓再发native prepare。反之，端口距离未超界仅是必要条件，仍须fresh原生科技/材料/角度/地形/占位/功率和逐对象读回，不能据距离放行。此反例没有游戏调用/写入，旧成功配置不撤销，accepted0不再reset；不能把设计否决称为实机原生错误。规则、安装、端口/占用或布局变化时复验，最近复验同日。

2026-09-16 / EXP-299十写账本与现场授权分离（`validated`限本次交接）：完整审计aadd02d推送/CI34999138941绿后，DA3B8490通过30项离线/三入口/解析，root raw-ee04cd2d严格核原97文件/96回复及native类型，保留文件哈希，正常exit0记录accepted10→0；没有新的游戏读取或写入。计数属于保护证据链，重复15次现场读取不能增强这次账本交接，也不能替代下一执行前的fresh完整场景、玩家/科研/J、材料和逐步native预检。handoff不得冒充更新后的game tick、取消成功动作或恢复旧token，下一开场失败不能再次reset；既有handoff存在时入口拒绝重做。最后真实观察仍60843327/R112/primary60758779/J88；现场/人工操作变化由下一执行的fresh门阻断，账本绑定、记录形状或进程交接变化时复验，最近复验同日。

2026-09-16 / EXP-299与IFX-121/134/139十写复验（`validated`限本机完整只读审计）：DBFF32E3通过46项离线、三坏入口/解析，raw-e3fc3280的96次实读正常exit0，60842734→60843327/R112/primary60758779/J88。十原/fresh终态、47页同tick4645实体/9092互返边、32详情/零prebuild、全玩家物料、科研与两网满供核清，228安装文件匹配6070cf8。审计只把已证843.storageConfiguration和101.recipeId替换进旧配置基线，不把整份当前DTO当基线；仓储动态存货可降但预约/连接须精确不变，来源异步库存不相减冒充转移即时守恒。元数据取原生第一页，报告沿既有安全投影，完整96读模拟覆盖最终出口；有界异步reader本次正常响应通过，不增加真实断线/超时验收声明。accepted10仍待本条push/绿CI后显式交接，不重做/重存/提前归零。101仍空载无边，本次不是持续产率、128带货物全覆盖或保存恢复证明。调用方/安装/现场或人工操作变化时fresh复验，最近复验同日。

2026-09-16 / EXP-299与IFX-121阶段A实证（`validated`限本机七动作/保存及原记录重核）：8F3477E1/raw-ac35cc9c的124回复全部success，七原/fresh终态依次核843 bans30、100高纯硅转移、唯一1113过滤、101r37、3铁/1铜及save60758779；末60758878/R112/J88/accepted10。真实配置revision按104/105/107/109/110/111/112演进，三次取料有原生双边即时守恒，全玩家净增100/3/1；843真实29格/2900、101空载无边。保护完成报告写成后stdout才因整数键失败，不能把exit1当原生未完成。root DA9D3B98/raw-0120da03重放原124回复exit0（0新游戏），10项专用离线及15场景/88检查确认安全导出；原先模拟只返回对象、未走最终格式化出口，是旧IFX-121经验遗漏的原因。当前58AC1C9E只保留offline，格式化复用既有安全投影，原8F3477E1保留不重写历史。accepted10须继续冻结至完整审计，不能因显示修好而自动归零或重放；没有两向sorter、持续晶格硅或CNT消费证明。报告形状/输出边界或现场变化时复验，最近复验同日。
...
2026-09-15 / EXP-299消费链调查与快照范围（`validated`限本机只读）：E241C894经36离线/四坏入口/解析，Luna raw-209b7962一次18读exit0；root原18响应独立审核exit0，均不写游戏。60526528/R102/primary60175189/accepted3，CNT满2000、HPS仓3000且上游输出另100；释放一个默认仓格会被自动补货抢回，因此共享输入/输出方案必须先按既有原语设计持久过滤/自动入仓限制的有界交接，而不是假定空格保持。101的空载/无边/满供是候选证据，不证明sorter距离或原生槽位可行。GameStateReader的assembler与lab为独立componentKind：50项assembler只能fresh证明r36/37缺失，不能证明Research r55缺失；旧全场证据和本轮分类证据必须分开。Journal未在该18读中重新观察，不能把上次88称作此轮fresh水位。范围限定的原数据复核通过不升级为整厂、产率或恢复通过；实际配置/放置/玩家可达范围变化时重新预检，最近复验同日。

2026-09-15 / EXP-299持续观察的动态开场条件（`validated`限本机负例）：E57ABD45由Luna唯一执行raw-a10a7ebc，29只读原生均成功，60466787→60466970/R102/primary60175189/J88/accepted3。实际3052 CNT1952/free48未满足提前声明100，完整预约功率余40100J/t满足；通用开场错误必须拆解为真实失败项，不能把容量不足写成电力不足。root原29响应/19详情/配置互返边/玩家/科研/J/assembler集合复核exit0，0新游戏调用，不改写原exit1。离线准备期间工厂继续运行，原695 CNT/1305余量不能沿用；库存增长不替代原生固定窗，871仅848石墨烯不代表无限上游。此类正常预条件变化交Sol重新安排真实消费/供给，不重试原观察、不降低门槛、不为取证临时清仓。IFX-139正常读取已获29次本机实测，超时取消仍仅离线假管道证明；本次0窗口，未验持续产率、收尾全厂或保存恢复。下一执行前fresh复验现场/容量/供给，最近复验同日。

2026-09-15 / EXP-299与IFX-139有界观察（`validated`限离线及原数据解释）：仅检查循环Stopwatch不能终止旧同步pipe.Read。04CE881B复用现有Core异步framing和同一CancellationToken，使握手/请求共享≤30秒网络截止及剩余240秒；57E11607的本地等待也受剩余时间限制。51 reader/74 caller/91 pure检查、四入口/解析、四项真实原记录正例均通过，零游戏调用；假管道测试曾因先同步写后读挂起而被终止，改有界并行I/O后正常exit0，VoidTaskResult也须显式丢弃以保留单DTO。原311的3052有695CNT/余1305、869原料不足但871有库存，不能把库存供应写成持续制造石墨烯；原目录同时有33/35两种Chemical配方，应按全部CNT配方ID及完整assembler集合证明实际唯一3050，不要求目录只有33。原3667的2012、872的null filter和870仓类型须保留，不能按邻近ID或普通新建sorter类型类推。新观察采用12×600tick原生计数、fresh前后两半货流及同仓库存趋势；未捕到短脉冲为inconclusive_sampling，不证明断供。全厂DTO不含beltCargo，额外带货物测试夹具明确为合成；异步详情不能相加当同tick守恒。最新现场仍60240896/R102/primary60175189/J88/accepted3，无新安装/保存/重启。读取器、helper形状、现场或会话变化时复验，最近复验同日。

2026-09-15 / EXP-299与IFX-134/138保存后核验（`validated`限现场及原记录）：root B460F5FC经43离线/四坏入口和解析检查，一次raw-311cbc26取得81真实只读回复；60239798→60240896/R102/primary60175189/J88/accepted3，场景60239826完整47页4645实体/9092边，24详情和三个原/fresh终态、全玩家settled/零prebuild/两网均通过，2011仅净减2且余0。原PTY86254 exit1发生在最终报告，不是游戏失败：Qs-Scene不返回snapshotId，不能照搬其他helper形状。412BE6DD经8项离线检查，用永久禁止transport的原响应游标重新执行相同81读/核验体，再从已证明的原生分页取真实元数据；raw-31e6aa43正常exit0，零新游戏调用，不伪造新观察或改写原调用方exit1。报告构造也要覆盖真实数据形状；资料齐全时只重建报告，不为显示错误重复扫描/保存/施工。此段只关闭两源保存后现场，尚未观察持续CNT、全部128带货物或重启恢复；后续供给窗口应使用有货阶段的严格纯度/时间界限，不能继续套用源关闭时的空带断言。最近复验同日，helper返回形状、现场/人工操作或恢复变化时复验。

2026-09-15 / EXP-299与IFX-138保存交接（`validated`限原动作与原记录核销）：Luna在cf5e89b绿CI后唯一执行09EDD665/raw-c678d01e，G4644/Ti4645及save60175189均真实成功，accepted0→3未归零；原exit1来自helper已更新s2主档、外层ss缓存仍旧的回调校验。root72D1F9FB以实际原保存/session复现，19项离线正负例和raw-7bdb662b原172响应核证均exit0，零补游戏调用。两源原生位置/两侧166节点几何、双端互返和2011净减2保持，末60175202/R102/primary60175189/J88。函数闭包/参数测试之后还须测真实跨helper状态交接；新保存边界由唯一已证terminal及实际session共同验证，采用前不从另一个滞后缓存取期望值。原调用方退役，不重放两源或save；原player/power均save前，原全场仅开场，仍待有限保存后只读核验和持续产出。新适配器、回调、保存或恢复路径变化时复验，最近复验同日。

2026-09-15 / EXP-299与IFX-137正式依赖复验（`validated`仅离线）：两源私有调用方09EDD665保留完整FunctionDefinitionAst声明，不再以Body脚本块代替函数。正式依赖的38个source、3个runtime及44个本地函数闭包显式核对；新pwsh只走正式functions-only导入，47项真实参数/调用正负检查避免完整旧offline预加载掩盖漏依赖。调用方219项、root独立64项导入及18项纯入口/解析均通过，零游戏调用；不要相加当独立用例总数。未来原记录审计器43项及最终状态83/原生准入42项只验证证据解释，不能提前证明施工；全场DTO可核机器/仓/分拣器buffers但不含beltCargo，不能声称全128条带货物已读，分拣器精确集合也不能按含风机的连续ID区间猜。当前仍59932861/R97/primary59894127/J88/accepted0，两源未接；下一仅新预检后两建一存、预算600/300 J/t、原终态与双端唯一读回，持续供料另验。新增导入、入口、现场或恢复后复验，最近复验同日。
...
2026-09-15 / EXP-299与IFX-136的包内指南修订（`validated`限离线资源）：830e3c1及CI34960271729成功后，agent-playbook明确post-terminal pending-readback顺序：绑定原action、相同fresh成功终态、唯一new ID及计划两端，只接纳该ID的预期互返边，保全全部旧边/其他配置，完整核实体、pose、过滤、实际供电与双端物理slot后才完成本地步骤；后验异常不得重放成功prefix。新增嵌入资源回归及既有资源发现/发电单位测试共3项通过，MCP测试项目Release build零警告错误。未改公共工具、动作或hash，也未冷部署；6070cf8运行安装与accepted6保持，最近实机仍59820632/R90/primary59372664/J88。后缀调用方修复及实际三建一存仍待，不能把指南通过冒充施工/持续供给。新调用方、端点变更、安装/DSP或十写边界触发复验，最近复验同日。

2026-09-15 / EXP-299 post-terminal核验顺序修订（`validated`限首链实机核销，调用方修复待验）：raw-af7af77c的第一只4640确已成功、两原终态一致且2011仅6→5，但调用方在把该已证ID登记进s2New前就调用严格旧配置endpoint reader，错误拒绝4610合法新边。成功prefix不能因后验异常重放。root02415F3F的18离线/解析/三坏入口通过，raw-fcabfed3一次11读正常exit0，同action/4640/双端slot4/旧边/过滤/供电/空带及全玩家/科研/J88/零prebuild核清，末59820632/R90/primary59372664/accepted6。修正必须仅对原/fresh同handle terminal加新详情已证的单一ID建立临时post-terminal核验集合，限该计划两端，仍核全部旧边与双向slot，再登记已建；不能泛化忽略新增边、提前增加built或放行未证ID。后续仅后三建一存，旧v3不得再live。当前电网容量按fresh值读取，不硬套旧1534000；原一次准入不能代替后续1500/1200/900 J/t剩余预算。新现场/回读阶段/人工变动/十写触发复验，最近复验同日，关联IFX-136。

同批离线审计教训（EXP-299，`observed`，待新六旧+四新审计复验）：174项候选检查不代表所有live路径依赖已覆盖；新增从真实Session DTO构造的保存边界正例发现peacefulMode/sandboxMode是状态字符串而非bool，实际全场正例又发现Ca-Edges及三项Sra纯helper未显式导入。Sol另用纯函数反例发现功率负值/储能计数和action scalar/array target身份绑定缺口。候选从未做游戏audit；旧5+5入口已在导入前停用并有拒绝测试，不用部分离线通过冒充十写验收。下一必须绑定真实六笔prefix，显式导入每项纯依赖、完整模式/功率/动作身份和逐阶段正负例，不能运行旧审计补数。没有因此修改公共API、游戏或已成功实体，最近复核同日。

2026-09-15 / EXP-299显式入口与完整授权参数复验（`validated`限本地调用入口）：Luna实际execute实参把40位风机审批SHA截成33位，v3/72E822DD在导入前正确exit1，未运行Bridge或接受动作。root对原29条纯guard AST复现完整参数通过/截短参数拒绝，两例exit0；后续使用data-only/3CF2E393整组承载获批参数，14个键按源码参数集合核对，不让模型重抄长标识、不截短匹配、不改变原准入。纯入口诊断不能用文本包含Import来判断执行导入（错误消息也含该词），参数数目也不得凭猜测写死；两次本地诊断拒绝保留，改用CommandAst与ParamBlock后通过。适用仅原四下游+正常保存的固定获批数据，非通用自动执行器；新head/CI/证明/范围须重审数据与哈希，任何accepted后都不得以换参数为由重放。零游戏调用/写入，accepted5、最后59785219/R88/primary59372664/J88保持，最近复验同日，关联IFX-106。

2026-09-15 / EXP-188/290/299及矿竭基线规则实机复验（`validated`限四点零写预检）：ac9a77f/绿CI后v3/72E822DD的raw-87ccd681完整155回复正常exit0；root193A37CD/raw-0807c7e7从原记录独立核验exit0、零新游戏读。两次47页4639/9068全场与四组166节点before/probe/after通过，每组86个合法实时发电buffer变化不再被错判；静态字段、两个hash与count对应关系、实际/代理原生球面几何继续严格。四点最小覆盖余量0.307769m，当前均built0，因此每点仍预留1800 J/t含未来600，不能按候选序号提前减预算；剩余余量4100，原4500探针余1400/无缺口。86沿独立已证空矿点DTO继续普通配置比较，无整机豁免。末59785219/R88/primary59372664/J88/accepted5、玩家/科研/两网/零prebuild保持；未施工或保存、不宣称持续供给，安装6070cf8未更新指南。下一每次真实施工仍须fresh预检，后续两源须再次核功率；新的节点、资源、人工变更、DSP/DTO或十写触发复验，最近复验同日，关联IFX-119/135。

2026-09-15 / EXP-085/157/185/299严格基线续接（`validated`限离线，fresh预览待验）：Sol v3/72E822DD绑定精确失败705、独立744九读、eb16deb及绿CI，仅用已核证的86当前DTO续接比较基线；保留旧记录和v2，后续仍逐字段核验，不能全局忽略矿点列表。root104项调用方、193A37CD的33项独立核验器及继承41项纯离线检查、解析/八坏入口均正常exit0，覆盖资源、身份、位置、连接、电网与未来再次漂移拒绝。独立核验器从原始九读和负响应核实矿竭，不依赖执行者自报；完整预览仍要求155份原回复、两次全厂、四个原生plan/probe及独立节点几何证据。零新游戏调用，accepted5/最后59755916/R88/primary59372664/J88保持，无部署或施工结论。适用仅86/197当前已证矿竭；新的矿点、现场、DLL/DTO或十写边界触发复验，最近复验同日，关联IFX-119/135。

2026-09-15 / EXP-085/157/185/299末点耗尽复验（`validated`限86/197，当前基线适配待验）：7717252/绿CI后的v2只读预览raw-705ad182在全场第一页停止；8原成功回复、零四点prepare/probe/commit。root9C577308的18项离线、解析/三坏入口通过，raw-74466c3f一次9读正常exit0：原59638796的86=[197]，59755870已空，旧197由独立资源inspect明确返回INVALID_ENTITY，其他原配置全部保持。末59755916/R88/primary59372664/J88/accepted5，全玩家/科研、两网成员满供/零prebuild不变，未重扫全厂；不能捏造精确耗尽tick或继续宣称该机有单矿产能。

这是对已知自然矿竭规则的精确复核，不是允许跳过整个86或所有矿机。后续只可绑定原失败、root证明与绿CI，将这一已证明的当前DTO作为比较基线，保留原证据，下一次仍完整核身份/位置/朝向/连接/电网/资源列表；任何其他变化单独停止核证。未授权修改旧产线、补矿或重放已完成对象；两源与四下游施工都仍未执行。附带复核IFX-119：ACL函数来自审计wrapper，读取error.code前必须先区分成功envelope；两次原始诊断包装器错误均只发生在本地读记录，纠正后才运行九读实机审计。最近复验同日，后续矿点/现场/原生版本或十写边界触发重验。
...
2026-09-15 / EXP-188/290/299 post-wind四下游调用方与独立核验准备（`validated`仅离线）：Sol冻结940375B5，只准4610→4622、4595→4603、4575→4602、4555→4594四个2011/Ti接点，再普通save；逐次fresh预检/原action终态/唯一实体及双边槽位、全背包和配置复读，accepted5→10后冻结、不归零、不接两源。预算按已真实建成数量扣除，分别保留六至三只普通sorter的1800/1500/1200/900 J/t，始终包含后续两源600 J/t；保留4500 J/t原始Foundry探针，不能伪改其不足标记或用当前低负荷代替全工作预算。root独立复跑41项调用方、21项动作客户端、40项Core电力测试、解析及六个导入前拒绝入口，均正常exit0，零游戏调用。

复验既有IFX-001/EXP-299：离线初轮因旧S2顶层导入污染作用域exit1，后改为对冻结D6D4EF25仅提取明确函数名单，不执行其任何顶层；独立旧版356项离线通过不冒充本次执行。随后一次离线因缺少节点快照元数据exit1，已明确初始化完整旧场景，并在每个fresh前/后73节点+93发电机见证之间切换元数据。root还在live前拦下预览误增built、新实体尚未进入读回许可和completion误记零写三处问题；修后生产分支回归分别验证预览序号不改变世界、原成功终态及fresh一致后只准一个新ID读回、四建一存合计五写/累计十写。所有首轮失败保留为调用方离线失败，不算游戏失败或Sol现场三次方案计数。

root独立F55CBFE2复用冻结54B56A01的原生/Core投影、保守覆盖集合和前后节点夹读算法，仅重绑一台已建风机后的4639–4643对象、166节点、162+4电网及73/93分类范围。25项离线、解析/三拒绝入口和functions模式不导入Bridge通过；四个历史下游pose在实际post-wind结构上复算，夹读时刻与目录revision是明确合成夹具，不能当fresh现场或新token。漏算/重复风机、旧161节点、旧目录、未知类型、覆盖差异、节点移动/配置变化及不完整/错序快照均拒绝；最近复验同日。实际四下游preview/施工、实时两源余量和第十写保存仍待，accepted5未变，不扩展公共观察或规划体系；安装态、进程、场景、目录或配置变化均须重新核对适用性。

2026-09-15 / EXP-007/188/242/290/299单风机施工及返航误判核销（`validated`限本切片）：Luna raw-a60c9099的唯一普通风机在59638794成功，实体4639、只扣2203×1；7011F776随后把working1/pending0当失败，真实exit1保留，不重建。root DE52E5E8/raw-90b8c224独立核184原成功响应及8fresh，正常exit0：原前后完整场景59637813/59638796为4638→4639实体、9068互返物流边不变，五实际几何附件保留，新风机确属net3且两次输出543/265 J/t。fresh59654915/R88/primary59372664/J88/accepted5，玩家Walk/低速/能量充足且已idle，背包只风机−1，六sorter/三塔/线圈1保留；网3为162节点/448消费者/91发电机，当前capacity1534000、94947满供，网4成员不变、零prebuild。原脚本没有结尾J/power/prebuild；本次明确补读而非伪造原结尾，全场证据仍保留原tick。

这再次复验既有IFX-001，而非新Plugin缺陷：completed与drone readiness必须分开；只有原terminal、唯一实体/扣料/拓扑与零pending等证据齐全，才能把working记录为未空闲，下一写仍需fresh hash/native prepare。原execute已退役，C6C72953保留64离线（含8项原DTO/变异就绪检查）；独立审计11项、21客户端、2既有MCP指南回归、两解析/五拒绝入口通过，零额外游戏写。新的有界返航分支未live运行；不声称已修完所有私有脚本。最近复验同日，不保存/清零/接源；正发电和低载满供不是持续燃料或S2满负载验收，下一只推进四下游后保存至第十写，再审计后接两源。

2026-09-15 / EXP-188/290/299单风机施工前复核（`validated`限离线）：Sol给出仅一次普通2203建造、accepted4→5不保存/移动/清零的计划，root实现7011F776并通过56专用、36/89继承、21客户端、Parser0与三坏入口。用全部165原节点而非两座近邻核候选：其原生投影会连接1499/1510/2439/3960/3961；1510→3961、1499→1510、2439→180→1498→1510均在双方较大连接半径内，最窄边仍有0.7865m余量。网4四节点534/535/536/919距候选最近也169.353m；这些只是原场景几何证据，执行前后还须完整场景及实际网归属复核。实际发电读取buffers的power-generation-current-tick、joules_per_tick/unitsPerItem0，不存在可臆用的同名实体标量；0输出不等于未建成或准许重建。最近复验同日，实际施工尚待；一次接受后只能沿原handle核销，原生负prebuild最终必须解析为唯一正实体。普通yaw0朝向按当前Maths.SphericalRotation的up/forward轴独立核验并接受q/-q等价，不把角色和材料检查当成完整旋转证据。

2026-09-15 / EXP-188/290/299单风机原生候选复验（`validated`限只读放置）：4F89CF77通过36专用、89继承、21客户端与解析/四坏入口；Luna raw-6028f67a唯一18调用/一次prepare、正常exit0，root raw-c9f07b4d仅读全部原响应独立通过。固定请求只产生一个原生候选，实际Snap点与请求差0.203358m，材料预算/唯一path/范围按返回位置核对，不能把合理原生吸附误当位置损坏。placementRadius只是保守外包络/离线选点证据，实际碰撞由已存在的完整collider检查和DSP原生建造条件裁决；不要求离线先证明必然通过才准做无副作用prepare。最近复验同日，59611967/R86/primary59372664/J88/accepted4；四明确实体仅提供局部连接几何与net3消费见证，纯Tesla的null字段不是断网，真正新风机的networkId/发电仍需施工后核销。没有新发电、全厂/科研重扫或持续供给结论；任何实际施工须新token且不得重放材料/S1。关联IFX-133的位姿解释边界，但本次不关闭S2实际覆盖与接源验收。

2026-09-15 / EXP-299四动作备料真实核销（`validated`限本切片）：Luna raw-bd31d77b正常exit0；root raw-d0e283b8核106原成功响应及12fresh、四原/fresh终态、完整玩家净量/来源配置/原J88和两网/零prebuild，正常exit0；89专用与21客户端回归通过。三transfer即时源减7铁/2铜/4磁铁与玩家增加闭合，r7递归真实净耗这些叶料并留下风机1/线圈1；整批净增与手搓净耗分开记录。后续源仓自然补货不能抹掉同action即时守恒，handcraft导致revision84→86但仍只有一次accepted，不用revision差数写入。最近复验同日，59587601/R86/primary59372664/J88/accepted4保留；没有新建风机或save/reset，不外推容量增量、连续供料或全厂/科研重验，下一仍须独立位置及原生条件。

2026-09-15 / EXP-188/290/299有限备料与预算边界复验（`validated`限实读材料、原生配方和调用方离线）：raw-750fffe5的10个成功只读响应至59562452确认1511/562在交互范围内且足够供一台风机；raw-e92926da只修复摘要集合名解释，不重读或改变原exit1。目录使用buildings而非items，缺失不得当空；中间整数键预算在JSON证据中显式转为字符串键/物品行，离线输出错误不能触发游戏重试。01AE4CFF的89专用检查覆盖原生r5/r6/r7、真实目录/缺失字段、数量/来源/零inc、四动作请求/终态守恒、离线无传输与禁止成功前缀重放；另21客户端、解析/4坏入口通过。最近复验同日，关联IFX-134。

本批仅1511取7铁、562取2铜/4磁铁与r7一次，accepted0→4后不清零、不附带保存。手搓净叶料−7/−2/−4与整批净+1风机/+1线圈分别核销；动态来源后续补货不替代原动作的即时扣量证据。风机尚未建成，按已观察1529000容量/1528100预约加5000、减六sorter1800所余4100 J/t只是限定低截面的预算；不能信用未来发电、忽略其他燃料退出或省掉fresh满负载预检。一个离线候选因净空不足被明确淘汰，不算实机失败，更不获得施工授权；源最后与累计10写审计边界保留，既有S1/燃料链不改。

2026-09-15 / EXP-188/290/299有限燃料诊断实测（`validated`限20原只读及局部见证）：Sol raw-1d829c14与root raw-9e515434正常exit0，59532008–59532034/R81/primary59372664/accepted0；F4658678的88专用及继承回归通过。网3capacity在没有Agent写入时回到1565000，3060行2507 J/t正输出；容量恢复不等于已修好持续供料，更不是未来六只1800 J/t的fresh满负载许可。3965/H58/G0、4182持H1、4089有H2、抽样G段/分拣器空、3065仓/3069空均按各自tick保留，不能跨对象相加或从26tick宣布持续死锁/恢复。最近复验同日；开场玩家/J88与末尾session/power/空prebuild的不同观察范围必须随摘要保留，未读取的末尾玩家/J/科研不填已验。下一仍需有限方案和完整未来预算，禁止因这一高容量截面重放旧预检或使用旧token。

2026-09-15 / EXP-188/290/299发电归因及有限诊断边界（`validated`仅源码、原DTO及调用方离线）：3060在两份完整快照中由3657 J/t变0，是九火电非零集合减少的唯一成员，且总capacity少36000与其额定值相吻合；这不是直接读取单机capacityCurrentTick或fuelCount。PowerSystem原生输出受共同发电比例及剩余需求的顺序分配限制，不同tick summary比例不能硬套设备行；curFuelId只是燃料身份，不是库存量。Sol第3轮固定12对象/20只读，root F4658678通过88专用/继承和解析/4入口拒绝；3065用真实2101仓语义，list缺少beltCargo仍为unknown，合成空带夹具只证明离线校验。最近复验同日；该局部诊断不能声明全路径覆盖、同tick守恒、持续产量、末尾玩家/J/科研或新建电力许可，实际采样仍待。角色分工的辅助代码交接不算一次失败，也不放宽三轮设计/两次相似实操的停止边界。

2026-09-15 / EXP-188/290/299满负载守卫负例复验（`validated`限当前原生快照与零写拒绝）：Sol唯一raw-d4e9080e在第六source原生放置/实际覆盖通过后，被真实剩余功率拦下；root72CC8C88/raw-11891258从64原响应独立复现，12专用及继承/解析/3入口通过。probe59491869网3容量1529000、已预约1528100、无export；六只尚未建的1800需量缺900 J/t，而原4500探针缺3600，两者不得混淆。轻载94300/94300满供和此前1565000容量不能替代当前满基础预算；容量变化具体原因尚未诊断，不能猜是太阳、风或燃料。最近复验同日；完整开场4638/9068与前侧73/92节点只证明各自原tick，后侧与结尾未读就保持缺失，不能因geometry已过而跳过整体验证。accepted0、零重放/补读/保存，后续必须先解决供电余量并重新绑定有限方案；不拆改成功部分、不盲等碰到高容量截面，更不能把一次余量正值当持续发电证明。

2026-09-15 / EXP-188/290/299 S2节点夹读复验（`validated`仅私有调用方离线，IFX-133为fixed_offline_live_pending）：Sol D6D4EF25/root356及独立几何38、预检199、七动作139、继承回归、四解析/13拒绝入口通过，现有40 Core供电回归保持。GameSessionTracker的revision不是全世界人工改动哈希，不能把同session/revision当成节点位置未变；完整开场场景之外，每个候选使用已有componentKind过滤分别在原生probe前后读取73纯节点与92发电节点，逐身份/位置/朝向/配置核对165全集，再核真实点和代理点的保守覆盖集合等价。四快照分别记录tick，不伪称同tick，root从原响应独立核验夹读顺序和几何，不能只信执行器摘要。最近复验同日；预览只补原五计划之后尚未调用的index5，旧计划保留原tick且不充当新写权限，实际六build各须fresh；意图和收据保留几何与夹读证明。零新游戏调用，Plugin/MCP未变，真实预检/施工和持续供料仍待；节点/目录/原生语义变化必须重新拒绝或验证，不能只增容差或重做成功部分。

2026-09-15 / EXP-188/290/299覆盖投影复核（`validated`仅当前源码及原DTO复算，IFX-133仍open）：不能将Foundry请求origin等同于Snap后的machine.position。普通sorter以真实entity.pos/plugAlt投影；仅当完整实际节点集合在真实点与探针点均远离原生边界、覆盖集合完全相同，且原生probe唯一归属已知网时，才可转用该网的预算归因，不是放宽位置容差。root独立504FB195/raw-ee11bfc3以22正反例和40 Core通过复算d723五候选，第五的165节点全集中两点都由875/3753覆盖，最小保守余量1.883685/1.701861m。目录缺项只对三座2103/2104严格station按当前原生分支证明无powerNode；其他未知项拒绝。最近复验同日；旧S1全场与d723探针不是同tick，须fresh全场/节点再批准有界施工，当前未新调用游戏、未改Plugin/MCP/accepted计数，不把源码或代理几何当持续供给。原位、投影位、各证据tick和原失败必须保留；DSP、目录、现场或读出规则变化后重验。

2026-09-15 / EXP-188/290/299 S2供电投影反例（`observed`，IFX-133）：Sol raw-d723c7d0的871→4505 native prepare已成功，但Foundry制造台probe在相同请求origin上返回不同machine.position，偏移0.257892m；root raw-877f4502仅原记录独立复现，零commit。前四belt源同点并不证明warehouse物理槽也必同点，代理设备所在网3、原4500预算余32400也不能直接证明实际sorter位置覆盖。最近复验同日；保留原位/投影位和实际距离，后继必须按已确认原生覆盖语义证明真实目标，不能盲加容差、篡改probe或重放成功部分。当前只定位调用方未证实的几何假设，源码解释/有界修订和实机正例仍待Sol第2轮，不算原生建设失败或持续产线通过。

2026-09-15 / EXP-018/037/290/299 S2有限执行器复验（`validated`仅离线及已有源码契约）：E1E8FA16经Sol及root独立342项检查、解析/7入口拒绝、真实S1生产binder和6070cf8源码契约核对。四断口先接、两源最后，已完成S1不重做；accepted7保留且没有自动reset。41C624F5预检核验按真实[4,5]索引数组而非模糊布尔值绑定源步骤，185/139及继承通过。最近复验同日；新一次独立脚本的只读引用缓存应从空开始，复用已经装载的离线fixture再次装载时，重复身份守卫拒绝不代表游戏或原生建造失败。尚未进行S2现场预检、建造或持续供料验收，不能把计划计数当已完成，亦不能把调用方等待结束等同于取消已接受游戏动作。

2026-09-15 / EXP-018/037/290/299 S2调用方核验复验（`validated`仅离线边界）：独立4D2A5D47预检复放与56B5632B七动作审计通过185/139项专用、继承回归、两解析/六错误入口拒绝和132项Core测试；真实raw-923a560c/raw-c64528f4的newSorters是数量9，身份来自原九build唯一targetObjectIds，不能用数组夹具替代真实proof形状。货物缺失count必须检查属性实际存在，不能让PowerShell标量Count=1漏过验证。原生FoundryPowerPlanner的headroom/deficit是余量正部/缺口正部；保留4500探针原值而只预约尚未建的六只1800 J/t，不双算S1已建负载。最近复验同日；接源后从“空带”改为“明确纯目标物品/正确单位/inc”，但保持原配置和全部互返边，逐段快照不加总为产率。此处无S2游戏调用或供应验收，七动作后保留accepted7，来源持续性、消费和保存恢复仍须实测。

2026-09-15 / EXP-018/037/290/299 S1计数边界实测复验（`validated`限当前只读交接）：97cae7a/CI34924508618成功后，raw-c64528f4/59382698从真实raw-923a560c审计核同档R81/primary59372664/J88、三十见证、完整玩家科研、动态满供两网与零prebuild，正常exit0、accepted10→0，零新增游戏写。最近复验同日；该raw及其精确auditCommit/CI才是S2新边界，旧shared/Ti reset或预计R81不能替代，不再执行本次reset。原raw-923a560c各自tick还观察到共仓G=0、Ti332/CNT332以及来源G2597和Ti单段2，说明有限种料不等于持续生产；局部/重叠/不同tick库存不能拼成同步总量或产率。下一S2必须fresh真实槽位、全部材料与剩余1800 J/t预算，接源后验证合法动态货物而非继续要求空带，真实持续供应和消费另验。

2026-09-15 / EXP-018/037/188/200/242/290/299 S1施工及十写复验（`validated`限九普通sorter/保存）：Luna raw-0a1a19df与root raw-923a560c/59375717证明4630–4638九个唯一实体、逐次2011总15→6、原native计划/各次剩余满负载预算和原/fresh十终态；47页4638/9068、149详情、128空带、全旧配置/玩家科研/J88/两网/零prebuild保持。设备slot10/3各实际持有目标sorter且双向互返，原生belt实际槽由现场读回而非硬编码；九消费者计入网3，正常保存59372664/R81覆盖。最近复验同日；这里只核销S1，尚未接源、实际恢复或持续供料，不能以连接数/短库存增长宣布产线完成。

root实际审计exit0，原Luna外层未保留数字退出码则继续标未知，不从PTY不存在推断；原游戏十终态和save可独立确认，不据外壳元数据缺失重放。后继36D16E5B计数辅助47专用、220/247/50/107/45/50继承offline、三入口/解析、真实raw-923a560c proof正例通过；须其结论push/绿CI及fresh三十见证后才关闭accepted10，重复归零或旧shared/Ti凭据不能替代本次边界。当前尚未reset，后续六连接须另fresh完整材料/实际功率/端口，现有成功九只不重做。

2026-09-15 / EXP-018/188/200/290/299 S1预检实测复验（`validated`仅零写native计划）：bdc6f9a/CI34923211651成功后，Sol raw-d3f1e602由root raw-327abd01独立原记录核证，九计划/九实际pose probe、完整47页4629/9032、17详情及11组session/player均通过。全部15只满基础预约4500 J/t后余35100，而不是使用旧179100余量或当前轻载需求；后续仍逐次fresh，不保证未来容量。3052两个独立候选分别slot10/slot3，不能因此跳过第一只完工后的后继端口检查，也不要求新prepare必须选择旧preview槽。原/审计均正常exit0，无commit/补读重跑，accepted0/R62/primary59071065/J88、末读59362628保持。314执行器、247预检核验器、220十写审计离线及132 Core此前通过；这里只关闭S1现场预检门，实际施工/扣料/新实体/持续供料及保存恢复另验，最近复验同日。

2026-09-15 / EXP-018/037/188/200/242/290/299 S1执行边界复验（`validated`仅离线/原有语义）：Sol冻结4F869126、root审计A2769EB1，独立314/220专用及247/50/107/45/50继承offline、两解析/八入口拒绝、132现有Core通过。新S1夹具不是Unity正例。belt prepare的virtual=-1不等于完工实际槽必须4–15，生产核验以非负实际槽、before未占及两向精确互返，slot2正例与占用/非互返负例齐备；设备仍要求fresh原生physical slot且实际持有本sorter。两份3052 preview独立，不预约联合端口，第一连接成功后第二须fresh选剩余空槽。

满负载预算须分清既有与剩余：每次原只读probe仍完整保留4500 J/t保守模型，已有i只完成时其300×i已计入原生existingReserved，执行新增预约为300×(15−i)，不能再次全扣4500；缺口、几何不明和成员异常仍拒绝，不把瞬时满供当预算。preview与execute均有完整47页现场核证；施工同handle进度、完成跨读竞态和JSON安全pending、save真实+1及纯返航/完工分层保留。最近复验同日；现场预检、实际九建加save、十写核验及持续供料另待，任何计划/程序集/现场变化须fresh核证，成功部分不可因新方案重放。

2026-09-15 / EXP-188/200/290/299预检核证复验（`validated`仅调用方离线和安装态）：root CC2A1AD7通过247专用加50/107/45/50继承offline、3入口/解析，40现有Foundry供电测试通过。原记录审计明确核只读probe的实际机器位置和请求origin均对应native sorter位置，按原生完整保留负载而非瞬时需求核15只4500 J/t；设备端口依据当前观测，3052两份独立预览不是联合占槽。只观察17端点缓冲，其他设备配置看完整scene，不把未读取的生产buffer当已保全的动态快照。未来实际preview及施工仍待，不能将合成S1夹具作为Unity正例。raw-11db6026另证当前6070cf8的228文件哈希、64 tools/1 resource/71120字符指南、精确鉴权和正常exit0；无游戏写、部署或DSP重启，非最终包验收。最近复验同日；原安装不因新文档提交而自动变成新批次，源码/安装/现场变化须重新核证。

2026-09-15 / EXP-290/018/299计数交接实测复验（`validated`限当前只读边界）：2ea146b/CI34920128723成功后，raw-5327ba1d/59316547以原共仓root审计raw-a014a51b核同档R62/primary59071065/J88、全玩家科研、五对象完整配置/预约、零prebuild及两网满供，真实exit0，accepted10→0。主网capacity1565000、required=served=82404且成员精确不变，证明动态容量守卫修正可在完整fresh交接中通过；不覆盖原失败记录，不借瞬时满供宣称15sorter全负载或持续产量。50+107+45+50离线和既有指南回归已通过，最近复验同日；本proof及其确切approval/CI是S1的新计数边界，旧Ti reset不能替代，不授权重放原动作或跳过下一次全量预检。

2026-09-15 / EXP-290/018/299复验（`validated`限私有守卫修复与原记录反例）：dc7dda3/CI34919056704成功后，计数核验raw-8d6ffb7d在59305094因旧静态capacity等值exit1；两网满供且成员保持，主网1601000→1565000、required=served=328881。IFX-098的动态容量语义此前已写入当前研究和包内指南，新的计数辅助却沿用旧Ta-Power；只修当前辅助，不批量改写历史验收脚本。50专用/107+45+50继承offline、3入口/解析及1项既有指南回归通过；raw-ef7f2277原DTO复放同时证明旧拒绝与新守卫通过。最近复验同日；修正保留容量差、严格成员/实际满供/非负有限值/完整摘要核验，不把轻载满供或旧高容量当未来满负载许可。accepted10和主档59071065/R62/J88未变，无reset、重启或新游戏写，fresh交接及S1满预算仍待。

2026-09-15 / EXP-018/037/219/220/221/299共仓十写复验（`validated`限配置/有限启动/保存）：原raw-06ca84c9十步由root raw-a014a51b/59073969独立核销，五配置同步有序库存/三连接保持、四正常500转移双边守恒，最终5石墨烯/5钛/20纳米管/bans0；全4629实体/9032互返边/140详情、128空带和完整玩家科研/J88/两网/零prebuild保持。save59071065/R62真实覆盖，未实际恢复；28+107+45+50 offline及真实audit proof通过，最近复验同日。仓464/486/20及3050工作只证明有限种料后正常产出，不是持续补料或成品消费，跨tick各组件缓冲不得相加假装同步守恒。Luna预期场景计数不替代root实际全场，PTY失踪也不证明退出码为0；以原终态、保存和完整现场证据为准。下一须待本结论push/绿CI及唯一计数核验后再接有限下游，源最后；真实输送、长期需求、保存恢复另验。

2026-09-15 / EXP-007/219/220/221/299共仓预检复验（`validated`仅原始零写计划）：raw-70987988三计划由root raw-5e2a5a5b独立原记录复放，五session/四player/十二详情及完整科研/J88/两网/零prebuild保持，末读59061447、R47/primary59024068/accepted0。来源2916石墨烯/3000钛、距离54.664/79.366m在实测80m范围；未重跑现场调用。root初审发现PowerShell数组自身Count遮蔽成员count，必须用逐元素求和或显式Measure-Object -Property count，不把30格误称30件；真实30格2916/3000正例和格数不变但物品不足负例已加入107项独立审计，另28计数/45+50继承离线与入口/解析通过。关联IFX-132，最近复验同日；配置状态变化后的后续prepare仍待实际前置完成，计划不预留库存，不把预检或有限种料外推为持续供给。

2026-09-15 / EXP-219/220/221/299共仓预约离线复验（`validated`仅调用方和已有原生语义）：Sol冻结2E30773D把3052设为5石墨烯/5钛/20纳米管的有限九动作加save计划；root独立137执行器、104+45+50只读审计离线、7+3导入前拒绝/两解析与47仓储Core通过。SetBans只约束自动插入范围，不停止原有机器取料，也不阻止玩家正常AddItemStacked；配置完整格与库存守恒以同tick原生回执证明，后来inspect允许正常消耗。每次改空格过滤前须仍有五个对应正库存格，失去条件即停，不暗中补料。真实有序滤格与计划/回执逐项匹配，5/5/20只是容量分类，既不代替原生格身份也不保证长期库存。源各取500且余至少500；原J88已含seq74纳米管首产。适用于当前owned现场/安装，现场预检和执行仍待；状态、配置、安装或原料变化须fresh复验，最近复验同日。容量依据是EXP-219/220/221、IFX-042及game-api-m0的仓储UI研究；此前近期条目把EXP-294写作容量依据属于误引，EXP-294仍只管临时燃料功率/能量。没有新的公共工具或直接缓冲写入，不以有限二十格成品容量核销持续消费。

2026-09-15 / EXP-018/037/299钛十写交接复验（`validated`限当前只读计数边界）：ab88bb4/CI34901031995成功后，raw-26a16106在59032726核原Ti审计raw-e7b9f009及执行raw-9268e07d的同档R47/primary59024068/J88、完整玩家科研/两网/零prebuild，保护记录accepted10→0。32+45+50离线、3入口/解析及实际Ti审计proof测试通过，零新增save/游戏写。最近复验同日；后继须用Ti实际root audit/reset/green approval，不把G阶段凭据、Luna摘要或预期实体数当新基线。共仓预约、真实源接入和持续产量均另验；基线变化或重复reset必须拒绝。

2026-09-15 / EXP-007/018/037/185/242/299钛空载输送与保存复验（`validated`限当前九次正常施工及十写核销）：raw-9268e07d真实新增75带，两次4609/4624源cover各保留父尾、仅扣NEW；source/邻接姿态仍由当前原生完整拓扑证明，其他配置不豁免。root raw-e7b9f009/59026341核全部十原/fresh终态、4629实体/9032互返边/89详情、完整旧配置/新带空货/玩家科研/J88/满供及零prebuild。正常save59024068使46→47，原执行器最终summary真实通过，关闭IFX-126在该后继中的live门。最近复验同日；没有原料接入或持续供应声明，新线路保存恢复仍另验。32+45+50计数/审计offline、3入口/解析与真实audit proof正例通过，归零仍须审计push/绿CI后的唯一只读核证。

本轮T0/T1分别经16轮/478.3695734秒和12轮/337.7410744秒正常完工；这进一步证实固定少量现实轮数不能替代原生施工/真实进展判断。原same-handle有界轮数、墙钟、原生deadline、同ID进度与未知即停全部保留，没有重提成功动作或改游戏速度。长等待的现场经验不用于保证其他距离/无人机/供电条件，变化须重新核验；最终settled首读成立，不补造精确返航时刻。

2026-09-15 / EXP-188/195/200/229/299钛九路径现场预检复验（`validated`限零写预检）：原raw-d87b8d31九份完整原生自由计划20/20/8/7/13/2/2/3/2全部通过，root raw-2c67cf7f仅复放原记录，核十组session/player、14详情、J88、两网及空prebuild，末读58996201/58996202保持R28/primary58959632/accepted0。科研边界仅包含原Journal与玩家科研点数，不虚称增加了完整科技树采样。77列点只代表分开预览，T4/T7须使用未来实际父尾fresh source-cover，整批才是75 NEW；预检不预约联合场地、不授权旧token施工。最近复验同日，原位80m范围和75带/15sorter/3塔符合当前设计；正式施工仍必须fresh全路径/材料/哈希核验，现场或安装态改变即停。仍未接来源、配置共仓或证明持续产量。

2026-09-15 / EXP-007/018/037/185/242/299钛后继执行器复验（`validated`限当前源码与offline）：冻结8FB213D0将已核G十写三段6+4来源、raw-f2c294bb、raw-a9656b39及独立后继green approval绑定，九路径75 NEW+save才组成下一个十写。root独立185、45+50、5入口拒绝/解析及21客户端通过；原raw-0f9ffec2保存前session与raw-2cc7f598保存后session直接调用生产helper，证实save27→28且terminal后改绑primary58959632，拒绝旧primary、额外revision、外来session与隔离。build+2/save+1各依当前6070cf8真实调度路径，不把递增数做成全局规则。最近复验同日；只读预览、实际父带cover与施工另验，私有脚本或安装批次/会话/现场变化须重核。每轮≤30秒、有限轮数/墙钟/原生期限与同ID进展、pending不重放及最终施工成功/返航就绪分层均保留；不扩展Plugin/MCP或宣称持续供料。

2026-09-15 / EXP-018/037/299十写完成后计数门复验（`validated`限本次只读边界）：e8f50bb/CI34895190217成功后，raw-a9656b39在58973117复验raw-f2c294bb的同档R28/primary58959632/J88、完整玩家/科研、两网/零prebuild，才以保护记录accepted10→0；未新save或游戏写。32+50离线/入口门已通过，后续钛独立审计45+50离线通过但不代表钛施工。最近复验同日；后继不能把旧G的2+8分段用于现在的6+4，也不能把缺少原执行器summary当作可重做。基线/计数证明均绑定实际本次root审计、已推送提交及绿CI；变化须重新核验。

2026-09-15 / EXP-007/185/242/299有界进展观察实机正例（`validated`限当前三次施工）：root只复放raw-0f9ffec2，G5的原待施工集合11→8→4→1，5轮/123.6840115现实秒后正常成功，真实耗1972游戏tick；G6两轮/43.1655787秒、G7一轮/27.5339709秒后同样成功。三个完工后仍处于纯返航状态，没有等待全idle或重放；这关闭了下方“新等待后缀live待验”的正常施工分支，但不抵扣IFX-126保存断言错误或未触发故障分支。实际进度总和会随着已完对象退出而下降，不能用聚合总和的回落误判倒退，必须按相同对象与剩余集合判读。最近复验同日，五张实际非空prebuild页全部同档/有限进度；本次复放零新游戏调用/写入。
...
2026-09-15 / EXP-128/200/229/298/299成功前缀与后缀实测分离（`validated`限静态核验和原预检复读）：IFX-105后的4501必须保留；raw-6c506b90八次原生prepare均成功，root独立核58818297/R11/primary58361888/accepted1、九次玩家/session、原J88/两网/零prebuild，没有重复预检。后缀明确只有八段带及save，53 NEW带；两个cover使用后续真实父实体，旧整批入口继续退役。最终审计必须合并原塔raw与九个新action，不能把后缀当新计数周期或漏算已接受的成功动作。

执行器及独立审计均须实际调用受测试的纯节点核验helper：保留真实null字段，拒绝缺失字段、伪造网号、错误身份/位置/连接；完整电网摘要仍独立验证网3节点+1及其他成员/容量保持。Sol在root审计中找到同类残留断言并于游戏写入前修正，不是新的实机失败。后缀51/继承89/独立40离线、8错误入口、解析及1 MCP回归通过；指纹ae8d9d9d的有限后缀才可在审批push/CI后执行。最近复验同日；实际续建、正常保存、消费者受电/物流及持续产量仍待，不能把正确nullable语义变为全局配置豁免。现有包内规则已经覆盖，不新增公共工具或字段。

2026-09-15 / EXP-128/188/208/298/299纯电力节点语义再验（`validated`限原成功前缀核销）：Luna raw-f300c4a7正常建4501/2201，58803728原terminal成功、材料4→3，却因私有守卫要求纯power-node的powerNetworkId=3而停止。IFX-105和包内playbook已明确该字段只为consumer/generator填写，null不能当断网，也不能被擅自填成3。root raw-5bcb5033/58809206核原/fresh收据、4501实体/8802互返边/旧配置保持、完整玩家/科研/J88与零prebuild；两网满供且仅网3节点+1，其他数量/容量保持，没有原生失败、隔离或第二次建塔。

执行器必须把实际纯节点DTO的null正例纳入后置控制流测试，不能仅测prepare中的建材/位置预算；本次89离线没有覆盖该失败分支，保留真实exit1。原全批入口两种模式已退役并验证零调用拒绝，1项现有MCP指南回归通过；只续原未提交后缀，accepted1/R11不得归零，新塔仍未保存。后续消费者须以自身真实网号/供电比和货物流量验收；本次节点增量不外推一般合网、持续供给或完整Foundry。最近复验同日，后缀修正及实机另验。

2026-09-15 / EXP-195/200/229/299施工前复验（`validated`仅零写预检及离线调用方）：Sol raw-a5ed5325九个原生prepare通过，root独立复读58794302/R9/primary58361888/J88/accepted0。G1/G7当前独立空场路径分别列7/6点；后续必须绑定真实父带尾端、fresh endpoint/player hash，正常source-cover才分别新增6/5，整段53带加1塔。未建父带的预检不预留场地、不给后续cover可提交承诺；施工后不得重扣共享点或据空带声称供料。执行只限9建+1存，source/warehouse/sorter与共仓配置在后续另批批准，保存不等于必须重启。

本次独立审计在游戏写入前阻止三种调用方宽放/形状错误：精确矿机86的197基线不能变成整机配置免检；两个潜在旋转目标的归因必须有当前228文件安装指纹、whole_path_native_rotation_v1和同一原生成功terminal/fresh证明，实际旋转随后严格保持，不是任意角度豁免；`,@(ordered)`再被`@(函数调用)`包裹会产生双层数组，改为纯排序helper正常逐项输出并由调用方一次收集。89执行器/22独立审计离线、8入口拒绝及95带Core通过；关联IFX-107而不新建重复问题号。最近复验同日，后续任一矿点、现场、端点、安装批次或玩家变化仍fresh核验；本条不证明施工、持续产量或完整Foundry门通过。

2026-09-15 / EXP-007/195/229/299备料及跨重启十写复验（`validated`限本批闭环）：Luna raw-2a9bbc90五笔正常转移/直接手搓全部原终态成功，最终128带/15普通/4塔；root raw-38d2a5ae/58720936核四笔退出前fresh与六笔当前进程fresh收据、全4500实体/8802互返边/108详情及两源、全部玩家科研/原J88/两网/零prebuild。计数不能因中途正常恢复或这五笔是新进程内动作而只记5；accepted10在审计push/CI前继续冻结，primary58361888尚未包含新材料。30审计+52继承离线、4入口、3退役及21客户端通过，最近复验同日。

原首轮审计仅因86覆盖[197,201]→[197]停止；完整配置扫描raw-2d003e80无其他差异，raw-dac6f16e及最终审计分别证明201不存在、197同Stone/group15/唯一矿机且余970→935，其他身份/位置/旋转/连接/供电保持。仅核销这一独立证明的自然矿竭，不改原快照、不忽略其他矿机/剩余节点变化，也不推定精确耗尽tick或保持旧两点产能。末尾JSON显示再次遗漏整数键安全投影（IFX-121），但原始读取和最终审计记录已先耐久保存；只读原记录并修正显示/嵌套数量离线往返，不重放执行或重采全厂。下一方案必须使用此新基线和fresh hash；矿脉、玩家、端点、配置或现场变化继续触发复验，物料在手仍不等于已建输送/持续供给。

2026-09-15 / EXP-185/221/227/229/294/299双输入路线与整套备料复验（`validated`仅候选/预算）：raw-29026528/raw-c74b985e的17条原生自由路径由raw-c39608e7联合核为132列点、128唯一NEW（53石墨烯/75钛），四处未来源复用不重复计料，也不据此提前批准source cover。原钛候选撞3801及电塔NeedGround负例保留；raw-1ff8d5f9第二原生塔候选完整site和普通建造准备通过，单个覆盖缺口距塔8.864m，全15普通sorter计4500 J/t后当前满基础负载余179100 J/t。电塔只连网，不凭预检增加发电或宣布真实受电；实际新端点、原生时序及持续供料仍要逐步核验。3052是纳米管输入/输出共仓，须先解决原料/成品容量与后续消费，不能把空仓或备齐128条带当供需闭环。

本次raw-e8ed2b0b两源fresh正常取料预检和当前目录证明106铁/15电路板足够r5×31、r84×31、r85×15，源仓分别保留2894/3585。仅五笔有界执行获批准，accepted5到10后必须冻结；52离线/5导入前拒绝/解析及101 Core通过，旧准备结果不复用。当前零游戏写、原物资/J88/primary58361888保持，普通小提交不触发重启。适用于此owned现场及同批安装；材料、场地、配方、供电、端点或会话变化均fresh复验，最近复验同日。关联EXP-185的理论/实际供给边界及EXP-294共仓容量语义，不新增公共动作或蓝图类型。

2026-09-15 / EXP-196/208/299自然批量准入边界复验（`validated`，IFX-124）：19棒是r41批量2的首次原生拒绝数量，不是必须凑满20才有阻塞。raw-4e957fa8预声明最多七组/100墙钟秒/6000游戏tick的只读事件观察，保留20→20后自然命中19→19；当前安装MCP在58556835–58557434的ready600tick窗产0，前后3403停机/满电/完整输入，明确返回count/threshold19和confirmed output_blocked。root raw-aa09799b独立核17原响应/两组样本，63 Core+2 MCP及4入口拒绝、228同批文件/64工具/1资源/71120指南/纯stdout/exit0通过。先前固定探针raw-c35afb54的20棒exit1不重命名为成功；条件未命中时允许有界等待自然事件，不改库存、不重启，也不把事件采样选择用于挑选有利产率窗口。accepted5/primary58361888/J88保持；只核销诊断，满仓900棒或短窗0/0不证明持续供应。适用当前DSP及已验证输出公式；配方、原生准入公式、安装程序集变化后重新核对。最近复验同日。

2026-09-14 / EXP-001/002/007/185/293第二供源恢复复验（`validated`限此次恢复）：raw-60b7db38正常退出/Steam单次启动，首个主线程只读REQUEST_TIMEOUT后同一进程ready，未重复启动/消费票据；raw-6025e3c8实测新进程228同批文件/64工具/1资源，Luna resume-8a733578正常重存58361888。root raw-affa59de核4500实体/8802互返边/108详情、原J88/物资科研/双网，恢复首读位差36.74微米、后续0；19审计/20恢复/21生命周期、3入口/4退役、8 Core/21客户端通过。raw-20b7a442的r31/r58工作及871库存1540→1562不等于独立支线流量或持续供给：九段带的瞬时空货既不证明停产，也不证明输送量。accepted5不归零，完整恢复审计与后续现场分开；普通小提交不要求重启，合并至完整产线/明确验收节点，不能省略Roadmap实际恢复门。适用于同档当前安装；后续真实消费、配方/拓扑或部署变化须复验，最近复验同日。

2026-09-14 / EXP-001/002/007/229同批运行态重启准备复验（`validated`限保存/预检）：raw-d7526839保存58361856，root raw-a0251682/58365620独立保全108配置/物资科研/J88/满供；raw-6f96d8bc实测安装态64/1/71120字符/纯stdout/exit0，raw-7b28f7a4/58377726逐一核四原/fresh终态、228安装哈希及当前DSP/BepInEx引用。功能源码仍与6070cf8同批，无必要重建/重装；旧1852-test构建记录与本轮19保存/21生命周期/4入口/8 Core/21客户端回归分开，最近复验同日。执行入口只对这一明确重启可用，不复用历史PID/session/票据，不把元数据握手、正常save或restart ticket当真正恢复。accepted4不归零；下一经Steam单次启动、fresh同档恢复后须核全部实体/新连接及保存序列化边界，不据其关闭持续燃料/IFX-104。

2026-09-14 / EXP-185/221/227/297第二供源收益与速率边界（`validated`限独立三分钟）：raw-36f7caed石墨烯16/16/16、重氢30/25/25、燃料4/2/2每分钟，root raw-87735abd/314原始核对通过；34离线/4入口/2退役和135 Governor/Foundry Core通过，最近复验同日。石墨烯达到15±10%，燃料首窗超出2±10%，不挑窗、不重跑替换；accepted3/primary58238457/J88不变，无连续覆盖或实际恢复证明。gap为game ticks，原生P/C窗口与selectedBufferItemDelta自带的观察起止必须分开；首个缓冲差跨越上次实验，不得按本次一分钟归因。全星球石墨59–60生产、62–66消耗与有限仓增长不能推出持久平衡。碳纳米管3050的3052是双向共仓，自动接入前须按EXP-294/IFX-042确认输入/输出容量和长期成品需求，现有空闲3051不自动成为输出仓。适用于当前同档/安装/实际拓扑，后续供料配置、真实消费和恢复时复验；IFX-104仍open，禁止新增通用观察字段掩盖已可读出的边界。

2026-09-14 / EXP-007/185/221/229第二供源保存与再测边界（`validated`限正常保存）：raw-f70e3c77保存58238457，root raw-bac8ead6/58263168独立核同一原/fresh终态、108配置/完整物资科研/J88/两网满供/零prebuild，19保存/2退役/8 Core/21客户端通过，最近复验同日。accepted3/revision114保持，旧57954957主档已被正常更新而非回滚，恢复能力续签不等于实际恢复。增加供源是明确实验干预，可另声明15石墨烯与2燃料棒/min各三个原生分钟，但原单源10/min负例必须保留；新观察43详情、54离线检查、每目标独立窗口及显式间隙，零写入且不授予稳态或新Governor锁定。额外石墨出料不自动证明氢分配/重氢达标，有限产物仓不等于持续需求；后续真实消费、供给窗口和恢复均触发复验，IFX-104仍open。

2026-09-14 / EXP-007/185/221/227第二路石墨实际接通（`validated`限施工/物流正例）：raw-d2981bfc新4499/4500分别3格/2格、1109过滤、主网满供，3966南槽1实际持有4500且旧4189输出保留。root raw-63f93a51/58218591全4500对象/8802互返边/108详情、原/fresh终态、完整玩家/J88/零prebuild通过；30/31私有检查、2退役、98 Core/21客户端通过，最近复验同日。旧满石墨3966已工作、输出0及870收到13是有界物流证据，既不等于连续22.5石墨/min，也不证明共产品氢已送往重氢；原十写后新accepted2保持，primary57954957不覆盖此次支路，必须先普通保存再独立观察。19保存/4入口离线不冒充保存成功或实际恢复。GitHub TLS查询超时不代表红CI或游戏失败，只能重读同一run；不将它作为重放已接受动作、重开进程或更换存档的理由。

2026-09-14 / EXP-221/227/299两实际补源端点复验（`validated`限原生预检）：raw-fa9ae189/58177018在原十写绿CI与显式计数核销后，对4490→4422、3966槽1→4498获得各一次exact_slots/零offset/过滤1109原生正例，两个实际pose全600 J/t后余111600且旧4498对象不变。31预检/3入口/3原native核对、30执行器/4入口及98 Core通过，最近复验同日。前次约3.774m跨越旧带的候选已获得原生放置证据，但仍须正常建造后验证实际span、双端物理slot和货物，不能从几何或token推出持续流量；源最后接入，不借此增加其他写入/保存，已建67+9带和原五连接保留。accepted0是raw-90b1b42b对已完成十写的唯一账本核销，不是重启或忽略历史accepted。

2026-09-14 / EXP-007/221/227/229/299九带分阶段施工复验（`validated`限材料/施工/十写核销）：Luna raw-38afd1b5以正常转移、r85和无人机完成1电路板/2sorter/9NEW带，root raw-03c814f8/58156525独立核十个原/fresh终态、4498对象/8794互返边/106详情与原科研/J88/满供/0prebuild。新九带实际ID反序4498→4490，必须按原生计划坐标唯一匹配，不能按ID升序当货物流向；两端保持自由、零货，旧67带/五过滤连接不变。全窗物料2001−11/2011−3/1101−2/1301−1，未解释正增量为零；private49/46/30、入口4/3、退役3、原native数组4及10 Core/21客户端通过，最近复验同日。PowerShell单元素响应须先显式数组化再合并；失败的root只读审计不授权重跑Luna已成功前缀。accepted10在审计提交/推送/绿CI前冻结，普通保存尚只覆盖57954957，不能把材料在手/九带完成当双端接通、持续供料或恢复正例。下一使用真实4490/4498端点重新预检，旧预检与执行入口不复用；固定三窗口负例的观察入口也已退役，不选择更有利窗口覆盖原结论。

2026-09-14 / EXP-185/227/229第二路补源有限规划（`validated`仅原生九带预检，接线仍候选）：raw-ed3f20ab/58104674全4489对象证明旧布局未改；3966明确空闲南槽1可作为补源候选，原输出4189及旧石墨支路不拆。离线完整占位先否决长直线穿氢带、十带末端覆盖4137，九NEW末端提前一格，一次full_path_stage1原生prepare通过；不能将旧障碍豁免或缺料当场地通过。两次候选pose电网探针仅证明现有覆盖/满基础负载预算，600 J/t新增需求后余111600，不计未建探针为发电/实际消费。两只未来sorter仍须真实首尾ID、fresh端点hash、原生槽位/碰撞/时序、正常扣料及双向读回，约2.674/3.774m和额定三格30/min不替代它。两台r58合计30石墨/min是理论上限，不是可分配30/min，既有消费者及实际窗口仍要核验。30离线/3入口/原始native九点/65 Core/21客户端通过，最近复验同日；适用于当前安装态、当前现场与9带/2sorter预算，场景或库存变化即复验。accepted7只允许下一有限三步到10后先审计，禁止接着施工sorter或清零计数；IFX-104与持续燃料门未关闭。

2026-09-14 / EXP-185/227/297单源容量预算纠正（`validated`限当前供给负例）：raw-d9d0496c三组原生分钟均石墨烯10<15、D2为15<20，燃料0/2/2不能选择后两窗称达标；root raw-3f3ff256的252原始核对与102 Governor Core通过，最近复验同日。3110仅3122←3084供料，当前r58/2308满速只能15石墨/min，经3→2配方最多10石墨烯/min；额定30以上的分拣器/畅通67带不补足22.5实际供给需求。此前输送/供电预检正例保留，但若据其推定整个15/min供给计划完整，该推定无效；后续预算必须同时绑定实际有向上游、逐源配方速度、既有消费者和目标需求，不能把全星球47–48石墨/min当单支路可分配供应。3965/3966满20石墨而另一支路已工作，应重新分配真实支路，不重建已成功线路、加发电机或搬空库存。至少新增7.5石墨/min并不保证氢/D2送达，真实下游消费/多窗口及恢复另验，IFX-104仍open。观察器不因findings空或P=C授予平衡；私有审计比较语义字段和明确顺序的物品数组，不拿Hashtable序列化键顺序作状态哈希，原始DTO及游戏hash不修改。

2026-09-14 / EXP-007/221/229/299已供料线路保存复验（`validated`限保存与有限产出）：raw-fb1b3119普通保存57954957，root raw-34212d1b/57960159核同一原/fresh终态、91配置/完整物资科研/J88/满供/零prebuild。19保存/6入口/2退役和8 Core/21客户端通过；accepted7/revision104保持，未重启或回放。871从原空仓至前次独立读回的24石墨烯只是有限自然产出，单次库存差与相邻带货量都不能冒充产率。下一固定石墨烯15与燃料棒2/min，各三个3600tick原生独立窗口；两个目标的采样边界分别保存，跨目标不假定严格同时，任何缺口显式保留，库存差不等于P−C或计数器。54观察器/4入口离线通过，不锁新扩产实验、不把finite871终端当长期需求；持续消费及实际保存恢复仍待，最近复验同日。

2026-09-14 / EXP-027/068/240/299五只实际过滤连接复验（`validated`限施工与端点）：Luna raw-ec84ec43完成4485–4489，源最后激活；root raw-dcf1a77b/57931236独立核五原/fresh同终态、4489实体/8778互返边/91详情、普通sorter净−5与其余物资/旧配置/J88/满供/零prebuild。实际4483接870槽2且端口持有本次4485，四个跨带接点也按虚拟slot的真实占位双向反查，原酸入口和旧石墨出料不覆盖；五只实际时序为一格/两格，不把预检三格额定预算当实测速率。55执行器/100端点/14实际源离线、2退役入口与155 Core/21客户端通过，最近复验同日。成功执行入口退役，accepted6/revision103保持；下一保存及独立供料窗口分别复验。有限仓可暂时接收产物但不构成持续终端需求，IFX-104不能凭线路接通、单点货物或机器满供而关闭，禁止重复整段施工。

2026-09-14 / EXP-027/227/240实际五接点重设计正例（`validated`仅原生预检）：在两NEW已独立核销后，raw-0c716add/57874207用真实4483→870空槽2通过原生准入，另四个真实跨带接点也全部通过；未重复旧4481→870。全五个实际pose的原网覆盖及1500 J/t预算余112200 J/t、原仓预约/纯石墨源/旧边、全场与玩家/J88保持。14实际源绑定/4入口/100继承端点离线通过，最近复验同日；2011当前nativeStt200000按Core三跨度上界得120tick/30件每分，仅为覆盖22.5需求的额定预算。下一逐个fresh prepare并实际施工、确认真实格跨度/消费者满供和持续物流，源接点最后开启；未建/无吞吐证据，不把五个token跨现场变更复用，不核销IFX-104。

2026-09-14 / EXP-208/227/229/299两节真实源续接复验（`validated`限施工）：Luna raw-d457b09c普通完成4481源cover/NEW4483、4484；root raw-1336e001/57872518逐原/fresh终态核4484实体/8758互返边/84详情、旧65带/原生只允许的源与直接邻居朝向保全、46→44及原玩家/J88/满供/零prebuild，未重放原失败sorter。38执行器/18源保全/100端点离线、5入口/2退役与13 Core/21客户端通过，最近复验同日。真实4483是下一候选输入源，不能猜NEW编号或把未建时中心角当实际插值准入；原4484自由尾端保留，无拆旧、注入、供料或保存。accepted1/revision93/primary57766756保持，下一实际五接点、持续消费与保存恢复分别验证，IFX-104及版本门不据空链核销。

2026-09-14 / EXP-208/219/227/229有限空闲口续接预检（`observed`，未施工）：root raw-6ede6362/57848972完整核4482现状并以真实末带4481做两NEW的full_path_stage1/non-removing cover预检，正常材料2带、五sorter保留，旧65带不拆除；新直线中段对870空槽2的中心角约9.089°仅用于挑选候选，不能冒充真实带插值接点已经通过。五只未来consumer全需求预留后余112200 J/t，但真实端点施工、受电和吞吐另验。此批一次性离线算式曾重现IFX-113整数重载假零角，未据此执行；已丢弃该角度结果，使用明确double边界和0–180°/零向量回归。后续38执行器/18源保全/100端点离线、5入口和13 Core/21客户端通过后，仅批准这两节原生施工，再以真正生成的实体ID/hash重新预检仓口；accepted0不变，不重试4481→870或放宽产品角度，最近复验同日。

2026-09-14 / EXP-027/227/240实际端点拒绝复验（`validated`限4481→870负例）：raw-77e408b0从真实末带接预约仓时，当前保守直槽/单带插值子集返回36尝试/4投影/0facing、33.413°≥11°；nativeChecks和candidateChecks均0，不把它描述为实际施工碰撞或完整UI所有路径不可行。原五候选预览退役，后三个跨越及源接点未尝试，Luna零新增授权；raw-58940bf4/57813690保全六条现成末带、仓/电塔和玩家/原J88，北向槽0/1占用、槽2空闲是分别观察到的事实。100私有/4入口、155 Core/21客户端通过，最近复验同日；关联IFX-041/047，中心距离/形状预检仍不能替代真实槽位和插值角度。下一仅由root依据空闲口重新设计有界连接，保留成功前缀，新的带/端点分别fresh预检；accepted0与已保存主档保持，不重试原目标、不新增观察字段或放宽原生规则。

2026-09-14 / EXP-007/221/229/299正常保存与十写复验（`validated`限本窗口）：raw-efcd70d7普通保存至57766756；root raw-882be80e/57774289核一个明确失败Move、八个成功施工和一个save的十个唯一原/fresh终态，41 NEW/净−41与累计65空带分开计数，4482对象/8754互返边/78详情及全物资/科研/原J88/满供/零prebuild保持。19保存/29十写离线、2退役入口、8 Core Journal/21客户端通过，最近复验同日；成功保存入口永久退役，计数仍10，审计push/绿色CI后才明确关闭。保留历史卡停和微小位置变化的独立核销，不重放成功或失败目标；续签恢复能力并不等于实际恢复，空路不抵扣石墨到货/持续消费。下一五个实际过滤端点和消费者功率、正常供料、保存恢复分别触发复验，不新增原语或放宽原生准入。

2026-09-14 / EXP-061/229/230/299五段真实后缀复验（`validated`限东侧23 NEW）：raw-6c3321c5按两个独立起段和三个实际source-cover完成3+8+6+3+3，不把旧无source形状参考当连接授权。root raw-926c611d/57753635核五原/fresh终态、4482实体/8754互返边/78详情、实际17/6带两分支及西侧保全，净扣23/原J88/双网/零prebuild保持。30执行器/52东段离线、5入口/2退役、134 Core/21客户端通过，最近复验同日；实体分配序号再度不等于输送顺序，只以原生计划点和实际互返边排序。所有施工入口退役，accepted9不重置，65新带未保存/接源；第十写只准正常保存后完整审计，五个真实过滤接点及到货/持续消费另验。

2026-09-14 / EXP-007/229/293/299未提交后缀复验（`validated`限2+5 NEW）：root已独立核销首11条和当前稳定现场后，raw-fc88fa89两次真实源cover均正常terminal，不放宽位置门或复用旧hash；两段实际源身份/完整旧链、原生允许的源及邻带旋转、NEW扣料分别绑定每阶段原始读回。root raw-5e2c16f4/57736742核4459实体/8712边/47详情和38带空链，玩家净−7/原J88/双网/零prebuild保持；56离线/5入口/2退役、13 Core/21客户端通过，最近复验同日。前段11和当前7条执行入口都永久退役；accepted4、primary57628301保持，累计42新带尚未保存或供料。后续东段/实际过滤接点、十写与保存恢复须再验，不据空载链核销持续燃料或0.4。

2026-09-14 / EXP-007/103/229/293/299已完成源cover与后置停止复验（`validated`限本次11带）：raw-31889a7f原1f79e0a3在57718854成功并逐对象核销，但玩家0.906mm微差超过私有0.1mm停止门；不能据此改写为原生建造失败或重放整个三段执行器。root raw-514044a2/57723090核原/fresh同终态、4452实体/8698互返边/40详情、31带旧源完整保全、净扣11/全物资/原J88/满供/零prebuild，两次fresh位置差0。保留历史微差及未知来源，依EXP-293用当前稳定现场和fresh hash继续未提交后缀，不扩大保存序列化容差、不改产品哈希或原生门禁。45施工/12核销离线、5入口与13 Core/21客户端通过，最近复验同日；IFX-115长短整数键遗漏在此前零写预览中复现并纠正，29实际ID回归纳入。accepted2不重置，原三段入口退役；35新石墨带未保存/供料，后续施工、十写、供料和恢复触发复验。

2026-09-14 / EXP-007/037/229/242移动不是业务终点（`validated`限本次失败和续接预检）：raw-f8149414的27.3m全干采样不保证穿过建筑，原c29155bf在57687007按position_stalled/180tick/余6.625m停止，结构化doNotRetrySameTarget与5m、4m×4建议齐备；root raw-00689284复核健康Walk/约800MJ、仓3051/化工3050邻近，但不猜具体碰撞体。原目标和未执行后缀退役，不重试、不把失败改为成功。raw-3ce15c1b/57695685证明实际停点已能fresh预检4441源cover/NEW11，因此继续业务而非执着到达某坐标；无额外移动、重建或新原语。25接近/13失败边界/18源保全离线通过，最近复验同日；源保全测试夹具的连接属性须保持DTO顺序，不能误把字典序变化报成原生拓扑差异。accepted1/revision74不重置，预检非施工/到货，下一真实续接与保存恢复再验，原先“非水风险不等于无障碍”的指南保持有效。

2026-09-14 / EXP-007/229/242/299长施工与十写复验（`validated`限原两个action）：raw-cd8e6967的20条带在调用方观察窗到期时未terminal，raw-61bce541证明余5正常预建筑/三无人机工作/健康能量，raw-d7a6c50d补取同一a85b7f5c于57668664成功，历8613tick。不能把调用方异常改写为游戏失败、unknown或新提交许可；补取终态/首次完工详情的独立raw必须明确绑定原handle，不能冒充原执行器已有证据。raw-d9d30394/57676082核十个唯一原/fresh终态、4441实体/8676互返边/63详情及全物资、科研、J88、供电，35施工/13续读/37十写离线通过，最近复验同日。accepted10在审计push/绿色CI前不归零；24新带未入主档且无货，后续宜先评估正常短移到工区减少三无人机长距离往返，但不得以此移动未完工订单或绕过Move的现场/终态检查。与此前48带和20带长施工结论一致，无新产品故障或原生超时放宽。

2026-09-14 / EXP-185/229/230/296石墨绕线预检（`validated`仅当前十段参考）：raw-cc271f44/57648794将65个未来NEW拆为十段full_path_stage1参考，并明确六处共享源点必须待实际建成后带真实identity/hash再做non-removing cover；五只未来sorter亦未获得端点施工证明。当前111带/5普通sorter够整路，五个原网供电探针剔除未建4500后统一余112200 J/t；材料必须取fresh背包，buildKind按Contracts及历史原生读回核belt，不猜枚举。64离线检查通过，二十处旧配置/真实纯石墨/原J88/满供保持，最近复验同日，accepted8未归零。此类只读形状预检不能替代实际源、槽位、施工、到货或持续消费；阶段施工和十写后重新核验，不加通用工具、不把IFX-104背压标成解除。

2026-09-14 / EXP-221/229酸支线保存复验（`validated`限自然到货和正常保存）：raw-d16bb2d8的save57628301由root raw-775a5b89/57629467核原始/fresh终态、零物料变化、二十二配置/原玩家科研/J88/双网满供/零prebuild；仓870在没有再手工入料时98→100、真实带货纯酸，故证明有限自然到货。不能把相邻带货物相加为总库存，也不能因保留酸格满而称持续消费；石墨仍未接入。26保存/到货离线、21客户端及10入口负例通过，最近复验同日；accepted8/revision69不重置，所有已完成save入口退役，主档覆盖预约/三新连接。续签protected resume不等于已重启，后续石墨接入、产量和恢复分别复验。

2026-09-14 / EXP-027/068/240酸过滤端点实机复验（`validated`限三连接）：raw-b4f66d82正常建成4415–4417，root raw-19639938/57619809核三原/fresh终态、原始完工姿态/配置、三只普通仅净−3、4417实体/8632互返边和十九旧/三新详情；两个设备的真实槽10/1持有本次sorter，带端虚拟槽按实际反查，全部新consumer在主网3且满供。此前32a60383预检在容量变更后由826b4efc重新prepare，旧token未复用，原合金供酸边未覆盖。47执行器/52继承端点、105 Core及4入口负例通过，最近复验同日；accepted7/revision68保留，执行入口退役，未保存。新酸线路与870预约保持不能代替消费速率，下一到货/保存恢复及石墨供料时再验，IFX-104背压结论未关闭。

2026-09-14 / EXP-221混仓预留实机复验（`validated`限870当前两原料）：raw-76fbc361四原动作和root raw-ea27902b/57601908证明正常100酸种料、原生入机2/仓98及五酸格/二十五石墨格/bans0；两个配置终态的同步库存、before/after配置、两端点及取证时间完整，实际位置按投影而非公开buffers索引决定。完整4414实体/8620边/19详情及玩家、原J88、满供/零prebuild保持，57执行器/4审计离线通过，最近复验同日；accepted4/revision62保留，已完成执行入口退役。既有源863的其他酸线路未改动，不将有限100种料当长期消耗，也不宣称已自动补酸或阻塞解除。关联IFX-042和包内仓储同步边界指南，实际供料/原料变化、保存恢复时再验。

2026-09-14 / EXP-007/221/240混仓预留与实际酸端点复验（`observed`限本次预检）：raw-32a60383三个真实酸过滤sorter端点均原生通过、旧858覆盖且满基础负载余113700 J/t，仍须逐动作fresh预检/施工及真实受电；路径通过不代替端点检查。raw-be43b40a仅预检863→玩家100酸，未伪造第二笔入870前的玩家库存；固定后缀是实际种料稳定后lock-occupied，再将其余空/匹配格预约为石墨，投影须保留1–5真实酸格、总30格/bans0。Luna一次性只读断言把pick/insert写反，raw-8dc2fbdd零prepare/commit即停；实际pick870/insert869，与原双向边一致。57私有离线检查覆盖反向端点拒绝及原生同步库存/配置/时间证据，44 Core仓储测试通过；不是Plugin错误、未修改运行时或新开工具。原十写由abddcff绿色CI后显式关闭，当前accepted0；实际种料、过滤、接通或重启后再验，有限100酸不能计为持续消费。关联IFX-001/042及包内mixed-input capacity指南，最近复验同日。

2026-09-14 / EXP-007/185/229/239/296十写保存复验（`validated`限本窗口）：raw-ba79a354普通save至57566838，root raw-3fc66e5b/57569983核十个唯一原始/fresh终态（1转移/5手搓/3建造/1保存）、11个唯一NEW带、完整4414对象/8620互返边/36详情和全物资净值，J88身份/历史/durable及科研/两网/零prebuild保持。46审计离线、继承67备料/19保存、9入口拒绝通过，最近复验同日；保存入口退役，主档覆盖本次元件与空载带。accepted10只能在此审计推送且CI通过后明确关闭，不因单次保存/只读归零。续签protected resume不是实际恢复，空带不是持续酸或燃料消费，下一过滤连接/供料/重启再验。

2026-09-14 / EXP-221混合输入预留边界复核（`observed`限870当前空仓）：raw-56287ef8证明870的30格过滤全0且buffers空，原来只有873/874两条输出，869两原料为0；这时尚未出现实际堵塞。供酸前应先用正常有限实物分配与已有原生仓储操作形成两种原料的真实保留空间，不能让酸先填满全部格后再补石墨。原生filter-empty-or-matching覆盖空格/匹配格，lock-occupied只锁真正已占格；必须根据fresh投影核对，而非假定可直接指定任意格或把同为0的过滤/库存混同。不扩大工具或动作域，不把预防方案标成已执行；实际过滤/货流或原料集合变化后复验，关联包内mixed-input capacity指南与IFX-042共仓死锁记录。

2026-09-14 / EXP-195/208/229/230有限实际源cover复验（`validated`限4406的五NEW）：raw-f42c6e59的7a7bfc8f于57557144完成，root raw-19cf4ee4/57560791复核新5带、原6带、整场4414对象/8620互返边及物料116→111。完整native_grid/stage1、non_removing_belt_cover与whole_path_native_rotation_v1缺一不可；仅源4406和直接邻居4404允许成功终态已证明的原生朝向，fresh值还须与该原始完成读回一致，其他旧配置/朝向不放宽。27离线/4入口、13 Core旋转和2 MCP指南通过，最近复验同日。玩家、原J88、两网与零未完目标保持，已完成入口退役；accepted9不得先归零。十一空带不是过滤连接或持续供酸，下一保存/十写、三实际端点及真实货流时复验，原持续燃料/Foundry/准备门不核销。

2026-09-14 / EXP-007/229/242/299未提交后缀续接复验（`validated`限三条南带）：原第一段成功后，root raw-6abc940f按当前revision/真实场地重新prepare；Luna raw-236002e8只接受一个南段动作，57546561终态通过。root raw-dbd3022c/57549321核两段原action、6个空带/自由端、4409对象/8610互返边及所有旧配置，材料累计仅减6。7后缀隔离、27就绪/2原终态及4入口负例通过，最近复验同日。北段没有重建，南段使用新计划而非重跑双段；成功之后两个执行入口都退役。与IFX-001关联，公开MCP/指南已有该语义，不改Plugin门禁；accepted8不归零、primary未更新，源cover/分拣器/供料或十写再次核验，不把空带成功当成酸或燃料持续性。

2026-09-14 / EXP-007/229/299施工完成与无人机返回再次复验（`validated`限当前三带）：raw-12810dcd首段action1f36cac9在57537202已成功，原raw保留terminal；私有As-Ready却先调用含idle检查的Assert-StartupStationaryBuild，使working=3在轮询前抛错。这是调用方顺序缺陷，不是建造失败/未知，也不需要重启。root raw-60f4fd02/57540732独立确认三个NEW、净扣3、4406实体/8606互返边及全部旧配置/玩家/J88/供电不变。等待改为先保全物料、位置、Walk/能量/队列和零未完目标，再有限观察返航无人机；六新正反例、共27检查加2原终态、5入口拒绝通过，最近复验同日。包内指南已区分两种事实，本次无需新增工具/字段；任何调用方必须真正执行该顺序，而非只写等待循环。原双段入口退役，成功首段不可重放；accepted7不重置，南段、中段和实际供酸均尚未通过。后续连接、十写或重启触发复验。

2026-09-14 / 窄供电复核补充：power-node详情的powerNetworkId为null不是断网证明。raw-6d63cb8e的首个原生供电探针已完成，调用方误读不存在的site.objects只影响展示；保留原响应，raw-cbc80e1f仅补剩余两点，三处均covered/主网3且完整原生负载一致。858→路径的几何距离最大8.828m<10.5m；几何覆盖、原生组件归属与建成后真实sorter供电须分别核验。不新增观察字段、不重放施工，三个探针均只读/executable=false，未建4500 J/t不计入实际负载或新增容量。

2026-09-14 / EXP-229/230/296短路分段预检复验（`validated`限三段原生形状）：raw-caadfbd6完整4403对象/15详情复读后三段native_grid均full_path_stage1通过；由两处既有网格校准得到的候选只用于指定位置，不是新增原生放置算法。3+6+3点中，中段未来的首点是首段真实末端，完整成本为11 NEW带/3普通sorter；无source的六点参考不能冒充源cover已经通过。首段建成后须实际源ID/endpoint hash、完整旧路径保存、原生旋转证明和NEW5预算另fresh prepare，端点分拣器亦须实际槽位验证。61离线及87 Core路径/占位/source测试通过，最近复验同日。当前原生电网预算预留900后余113700 J/t，明确剔除占位探针4500；既有满电/额定余量不是持续燃料供应，原successful资产不得重建，accepted6不重置。

2026-09-14 / EXP-185/296/299/301直接配方预算实机复验（`validated`限固定元件包）：d9467d5/green CI后按root raw-98c07a3f核销旧十写才归零。raw-ca0c73e6一取料/五正常手搓及root raw-839826ae证明4真实磁铁、现有140铁/2铜/9电路板按当前r85/5/84/6/8生成120带/8sorter/4塔，最终保留4铁/1电路板，无递归猜料或额外产物。67离线/4错误入口/2退役入口及21客户端通过，最近复验同日；六原handle、全玩家/来源/J88/满供/零prebuild保持，accepted6不重置。主档57449785仍未包含后续备料，整图地形/物流/电力及持续吞吐另验；不能把目录数量闭合叫成施工/生产或保存恢复通过。

2026-09-14 / EXP-185/239/296/299十写复核（`validated`限本窗）：raw-048092a3/57498260逐一核十个原始/fresh成功终态，四个原buildId/8风机扣料，以及4403实体/8602互返边/56详情、完整玩家/科研/J88/两网/零prebuild。26手搓/61审计离线、4错误入口/2退役入口和21客户端检查通过，最近复验同日；本窗八风机和一sorter已保存，后续两次取料与recipe50×4尚未保存，不混淆主档与内存。计数10只能在审计推送且CI成功后关闭；没有新动作授权、重复建造或物品增加。首个只读preview曾因null action展示字段出错，在零commit边界修正后fresh预检通过；不能把本地展示异常当成游戏失败，也不得借此重放已接受action。

2026-09-14 / EXP-296/301预算更正（`validated`限原生配方分解）：raw-e36b08f5目录复核发现前段把电塔所需磁铁当成铁块可手搓，实际recipe2输入是1001铁矿，不是1101铁块。120带/8普通分拣器/4电塔的直接备料为144铁块+6铜块+4磁铁，或另用4铁矿正常制作这4磁铁；已取148铁块/6铜块的守恒正例不变，但尚缺4磁铁，余4铁块保留，不得称整包齐备。当前电路板recipe50×4仅需8铁块+4铜，独立满足；后续电塔必须先取真实磁铁或铁矿，禁止递归猜料。最近复验同日，配方ID、输入和产出倍率同时检查；该纠正不新增动作或公共能力。

2026-09-14 / EXP-301 — 备料计划必须同时核对物品身份与数量（`validated`限本次调用方）：raw-221841da把铜写成1102，保留库存检查在第二prepare前拦截；该数值实际为磁铁，不能把拒绝解释成缺铜。纠正为当前运行时铜1104、铁1101后，raw-dee8cd2a和root raw-0a4a54e7证明两次148铁/6铜正常转移的即时源扣减与玩家增加。私有有限请求守卫新增错误物品ID负例，36离线/4入口/21客户端检查通过，最近复验同日；公开Plugin/MCP未改。计划应从目录/原生配方取ID，并把ID、source、方向、数量、预留量和fresh hash一起绑定，不能仅给整数ID附一个口头名称。关联IFX-131、EXP-296/299；换配方、来源或版本须重验。备料成功不证明线路可放置或供给可持续，已成功的两动作入口退役，accepted9不清零。

2026-09-14 / EXP-196/208/297副产物背压再次复验（`validated`限现有负载）：raw-bd24943c三次46对象原始读取及155项独立核对证明3083/3965在油/氢齐备、满电、石墨20时被原生批次门限制；第二窗四裂解都confirmed output_blocked，现有递归诊断已追到重氢上游，不需新增字段。13只读隔离和41项容量/燃料Core通过，最近复验同日。六热电正发电与16新风机不等于石墨消费达到裂解需求，必须用实际负载/物料消耗，不能再叠发电机、反复腾库存或把氢毛P=C当可分配供应。其他窗仍有产出，不称全程停机；3074为空，勿套用旧D2占满共仓原因。IFX-104保持open；下一核真实下游需求、共享上游和完整预算，供给窗口及保存恢复另验，accepted7不重置。

2026-09-14 / EXP-030/185/239成功施工与供给验收分开保存（`validated`限正常保存）：raw-d2f1e2ba普通save57449785及root raw-733daa69/57450384证明16新风机、最终石墨入口、原玩家/科研/J88、两网和零prebuild保持，accepted7不清零。19保存离线、六个退役入口拒绝和21客户端检查通过，最近复验同日。供给负窗口不意味着回滚或重建已成功设施；先保存确证成果，再按fresh上游证据重新设计。续签protected resume不是重启实测，保存也不能把2/0/0燃料产量改判为持续供应通过；新的保存边界使旧primary绑定的执行脚本失效，已完成入口永久停止preview/execute，只可沿原handle审计。

2026-09-14 / EXP-007/185/239/300游戏窗口、循环物料与负验收（`validated`限三段观察）：raw-673c3ff5的600秒现实观察在约16.5tick/s时仅得到两完整游戏分钟；确认原helper终止后，raw-725d959d只补未测第三段并复用原两段原始证据，明确不满足原现实上限、不构成连续覆盖。155项独立证据核对与22观察离线通过。棒产量2/0/0、重氢均10/min不足2棒/min所需20，不能因库存747→750或氢毛P=C就宣布配平；库存读数跨间隙，裂解的循环氢投入/输出与其他消费者也不能全部分配给碰撞机。三次石墨和四聚变发电抽样非零、两网满供及114600 J/t满载余量只核销供电截面，不掩盖上游供给缺口。最近复验同日，accepted6/J88保持；下一从明确源设备/端口/原生配方复验真正可分配供应，不增加机器掩盖缺料，也不重做已成功补电/入口。

2026-09-14 / EXP-185/239/300最终入口的完整预算与真实槽复验（`validated`限施工）：旧瞬时负载拒绝流程不重跑。补齐并保存16台风机后，raw-139277b5再次原生预算/槽预检，Luna raw-697ca67a正常建成4403，仅2012−1。root raw-689cde70/57416614核全4403/8602、4223→3058的真实互返槽与过滤1109、全部旧配置/玩家/原J88/零prebuild；建成后600已纳入1522400完整基础负载，不能又作pending扣一次。当次容量1637000、余114600 J/t只证明这个功率截面，不保证未来燃料或持续产率。17新离线、四无效入口/两退役入口及21客户端检查通过；最近复验同日，accepted6、primary57403040，入口尚未保存。持续供给应采样不少于原生生产周期的完整窗口并核库存、上游和抽样健康，不能用短窗零产出、P=C或一次到货代替结论。

2026-09-14 / EXP-030/185/239/300完整补电保存与满载复验（`validated`限本截面）：raw-24146026普通保存57403040，raw-1e8f1327/57406347沿原handle核全部16风机、玩家/科研/原J88/两网及零prebuild，accepted5/revision31，续签恢复不等于已重启。raw-5a97b18e保存前的只读原生Foundry占位探针提供1601000容量、1521800完整现有负载、零外送；排除未建探针4500，并预留最终分拣器600，余78600 J/t。不得用瞬时低负载、未来石墨燃烧或重复计入已建风机来扩大预算；入口建成后其600已在完整负载中，不能再次扣除。19保存离线、四退役入口及21客户端检查通过，最近复验同日；后续入口仍需自己的fresh预算/场地/槽位和正常成本证明，持续供给、恢复及版本验收仍保留。

2026-09-14 / EXP-007/185/195/300分层返航与全额补电实测（`validated`限本机有限施工）：646db18的调用方分层在第六批raw-2b5fe26e真实触发：两对象已完成、正常2→0扣料后返回awaiting_drone_return/passed=false，不误报施工失败、不开放新写入。root沿原action/build独立审计raw-945f70dd/57398367确认idle及全4402/8598/54详情、全玩家/原J88/两网/零prebuild，最终4401/4402没有重建。40离线、十二退役入口与21客户端检查通过；最近复验同日，accepted4不清零。六批共16新风机/80000 J/t额定增量已成立；仅前八台被现有主档覆盖，正常保存、完整负载复核和持续燃料必须另证，不能把数量、额定功率或返航就绪当作产线完成。

2026-09-14 / EXP-007/185/195调用方返航分层实现（`validated`仅离线）：原30秒窗口不延长；原action和精确build已证明全部completed后，仅在Walk/能量/空队列保持、待施工与待维修均零而无人机仍working时，调用方返回awaiting_drone_return与passed=false，保留原handle/逐对象/扣料证据，明确未做完整后审计且下一次commit前必须fresh settled。随后只能只读审计同一结果，不能以此状态重新施工；其他玩家变化、缺少计数、负数或真实pending仍拒绝。两批参数各40项零游戏调用检查通过，覆盖ready/returning及七类异常；这不是Plugin新增字段或延长产品超时，新的实机执行尚待。最近复验同日，已完成五批的preview/execute均退役，最后一对须重新现场预检。

2026-09-14 / EXP-007/185/195重复返航观察到期的停止边界（`validated`限两次原handle核销）：第四、第五批construction action均已成功且全部对象正常扣料，但调用方各30秒返航观察到期；后一批raw-5309f139在57373401完成4399/4400，随后原handle证明idle，root raw-3650c5bd/57377840进一步核全4400/8598/52详情、完整逐对象及玩家/原J88。runner退出码不替代action结果，也不赋予重放许可。连续两次同阶段后先停止下批，root应把“对象完成”和“下一写入前玩家就绪”分成明确调用方状态；保留原action/build、以只读续核返航，任何未完成施工、维修或玩家条件变化仍停止，不延长产品超时或放宽准入。31离线、十退役入口及21客户端检查通过；最近复验同日，accepted3不清零，累计14/16且主档仍仅覆盖前八台。新的分层调用方流程尚待实现和实测，不据本条声称已经修复或全套补电完成。

2026-09-14 / EXP-007/185/195施工终态与无人机返航分层复验（`validated`限本次两台）：raw-2f128cf3的第四批蓝图在57364376已完成两对象和6→4扣料；调用方随后等待无人机返航到期，不能把runner exit1升级为action失败、unknown或重新提交许可。root raw-5fe8b742沿原action/build核相同成功终态及全部无人机idle，再由raw-e043ca40/57366895核全4398/8598、50详情、全玩家/原J88/两网/零prebuild；不延长产品超时、不重放任何模块。31离线/六退役入口拒绝通过，最近复验同日；accepted2不清零，累计12/16且本批尚未保存。后续须先区分具体哪层等待到期，并核原有证据后只继续未执行后缀，不能用调用方异常覆盖已经成立的逐对象终态。

2026-09-14 / EXP-135/189/195/300稀疏对角选区解决已证明的内部间距阻塞（`validated`限本批两台）：不修改失败蓝图的内部数据，而从当前owned中明确4391/4393重新原生导出两对象模块，raw-3acca9ef完整native正例、raw-272f8e98预检、Luna raw-182ca0d0正常两台施工及root raw-d4ea3091/57356386全4396/8598复读闭合。两段8→6扣料、独立buildId/4395–4396、旧八台及全部玩家/J88保持；31×3离线检查覆盖明确批次边界，旧两批执行已退役。原失败四台不重试，后续源选区和位置仍须native验证，不能从本例推出所有稀疏形状必可放置。最近复验同日，accepted1是完整十写审计push/绿CI并显式关闭后的新窗口，不是重启清零；累计10/16、尚未保存本批，持续供能及其他版本门仍待。

2026-09-14 / EXP-135/189/300跨纬带复制须检查内部实体间距（`validated`限本次原生负例）：raw-2da27715/57334171的四台补电原生预检均WindTooClose，返回姿态两对间距10.4783/10.2115m，小于当前DLL原生10.5m。外部场景净距、库存、科技与完整功率正例并不保证模块内部无冲突；以旧site经纬刚性平移预测只能筛选候选，不代表新纬带网格的实际翻译结果。紧凑四台排列在原区域已正常建设八台，不能据此前成功推广到任意纬度，也不应回改成功对象。零施工后退役该候选，下一仅从明确owned选区导出更稀疏模块，仍以native返回姿态及最终条件为准。最近复验同日，原16台总预算和持续燃料门保持；纬区、方向或源选区变化时重验内部与外部净距，无需新增原语、改蓝图数据或放宽游戏规则。

2026-09-14 / EXP-007/030/185/239跨恢复十写分段核销（`validated`限本窗口）：raw-43fbf35a保存57310664，raw-4d04fcf0/57319323核十原始终态、旧六项独立历史复读及当前四项fresh终态、4394/8598/46配置、八对象逐项扣料、玩家/原J88/科研/两网/零prebuild。受保护恢复切换了世界时间段：五项旧备料未保存，历史成功与当前库存必须分别保留，不能跨段相加或因恢复清零accepted。Blueprint终态的targetObjectIds也不作为完整对象清单：本次聚合字段各列三项，精确buildId的durable/fresh逐对象进度各证明四项，验收以完整后者和全场唯一实体为准，不据聚合摘要补建所谓缺失对象。19/60离线、6入口拒绝及一项输出序列化回归通过；stdout显示失败前已持久化完整审计证明，故只复读精确证明并修正显示，不重做save或审计。最近复验同日，accepted10仍待push/绿CI核销；恢复实测与完整16台/持续供给未通过，身份、Journal、对象进度或场景变化后重验。

2026-09-14 / EXP-135/185/195/300第二组有限补电复验（`validated`限两组八台施工）：已完成模块不能被下一组覆盖或整图重放。第二组提交前精确读取第一组buildId的durable/fresh对象与原扣料证据，再对不同批准位置重新inspect/prepare；Luna raw-872d226f只新增四台4391–4394、库存12→8。root raw-a817e2d0/57300681复核完整4394/8598、原42配置及4新详情、全玩家/原J88/两网/零prebuild，两个buildId和八个实体各自唯一。两批各31离线、6入口拒绝与21客户端通过，最近复验同日；accepted9不清零，下一普通保存为第十写并先冻结审计。四台额定20000 J/t只代表本批容量增量，累计8/16不等于完整补电或持续燃料门；保存恢复、其余八台和真实持续供给仍另验。场景、材料、原build进度或供电变化时重新核验，不复用预览凭据。

2026-09-14 / EXP-135/196/208/300单区补电施工复验（`validated`限首组四台）：先按完整场景及实际原生姿态筛除已知碰撞，再进行完整native检查；raw-0a76ccd2/3246786b证明选定四台均可施工并接入主网3，无需另外建设桥接塔。Luna raw-668fa318唯一有限提交与root raw-9e8ce722/57278661全4390/8598复核证明四对象正常无人机施工、16→12的四段独立扣料、唯一新实体4387–4390、原物流/玩家/J88保持，accepted8不归零。每台当前输出的joules_per_tick缓冲为正；额定新增20000 J/t与实际分担负荷必须区分，不依据通用isWorking或机器数量宣布持续供能。31离线/4入口拒绝/21客户端通过，最近复验同日。第二组选址仅raw-a36440c7原生预检通过、未提交；原16台完整目标、保存恢复及持续燃料仍待，现场变化后fresh预检，不重放整个已完成模块或旧备料步骤。

2026-09-14 / EXP-001/002/030/239暂停后恢复边界复验（`validated`限同一精确primary）：暂停不保证游戏仍运行，也不保证退出前所有内存进度已普通保存。先确认实际进程、有效票据和同批安装哈希；Steam也已停止时，既有进程守卫拒绝后只核验注册程序并经Steam启动一次，等待真实DSP/Bridge，不重复启动或修改配置。raw-c2518612/50d1fd1d证明228文件及真实64/1元数据；Luna raw-44e81bc0唯一恢复到primary57243648并重存57243680，root raw-205bc36e/57252641完整4386/8598、38详情、全玩家/科研/原J88/两网/零prebuild通过。主档内16风机保持；主档之后五次已成功但未保存的小额材料动作不在恢复库存中，必须区分其历史终态与当前世界状态，不能沿用一塔/一线圈或直接重放原脚本。21审计/4入口拒绝/21客户端及95相关Core回归通过，accepted6→7不清零。最近复验同日；不据此声明未保存进度可恢复，也不计新增供能或持续产量。进程、票据、Journal或现场变化时重验；不枚举其他档、改票据或把自动重存误称为人工操作已全部保留。

2026-09-14 / EXP-135/208/300小型单区模块与占位复验（`validated`限原生只读数据/拒绝）：源对象数量更少不保证单纬区，raw-266d2f48仍跨区；明确选择2973/2974/2976/2977后raw-ca6c36a1才是单160区四对象原生代码。必须使用现场返回姿态，而不是把cursorOffset或传入anchor擅自解释为几何中心；raw-49e7f045碰2976、raw-55087f64碰4330/2922，虽均范围内且主网3全负荷预算通过，最终native检查仍未执行。既有中心距离约1.25–1.75m不足以排除风机/带OBB碰撞；后续先按完整现场和保守净距筛除已知冲突，再由原生条件裁定地面/碰撞/风机间距。两次零写后停止第三个同类预览，accepted6及成功备料保持，不拆旧对象、不缩减16台预算。最近复验同日；单区、预算或无中心重叠都不能代替完整原生检查与真实发电。

2026-09-14 / EXP-135/196/208/239/300地面负例和两次上限复验（`validated`限当前拒绝与停止边界）：齐料后的raw-226143f1、raw-d63c1de5两个不同2201接网点均返回BUILD_LOCATION_INVALID/NeedGround，没有commit；连接距离和零负荷预算通过不能外推原生地面通过，范围外蓝图转换Ok也不能补足地形证据。raw-37aa094c/57381011复核两份拒绝、全玩家/J88/两网及零prebuild，accepted6不归零；停止第三个相似候选，改由root重设计有限发电布局，材料保留，原80000 J/t候选增量和完整供电门不缩水。临时只读核验应复用原Journal身份、完整entries及durable字段比较，不能要求含采样tick的整份DTO逐字不变；本次在零写断言停止后复用既有校验通过，没有改动真实日记或游戏。62离线复跑通过，最近复验同日；今后候选必须逐步证明地面、碰撞、接网和全负荷，不把NeedGround擅自解释为可填海或必须新增地形原语。

2026-09-14 / EXP-030/135/196/208/300风电接网与新小额备料复验（`validated`限本次五动作材料闭环）：raw-ce535d4f的新址没有占位且转换Ok，但14范围阻挡、16台孤立，不能把其80000 J/t当作主网余量；采用当前目录零负荷2201接网，不能用有充电负荷的2202等价替换。raw-9438be21确认723当前可达和正常r6/r8预算，背包2001/2002为带而非电塔，按数字相似误读的计划在零写材料守卫处停止。raw-313e0682三fresh预检、Luna raw-606515a3五成功终态、root raw-b5165161/57357737独立证明从723取2铁/2磁铁/1铜，最终新增一塔并余一线圈，其他玩家/配置/J88/科技/电网保持。先玩家、后动态源仓、立即prepare，单步骤仅一次；这是新小额任务，不重放此前112铁失败预览，也不放宽内容hash。62离线/三入口拒绝/21 ActionClient通过，最近复验同日。accepted6不归零；材料到手不是已建塔、接网或整图原生检查通过，场地/源库存/姿态/功率改变后仍需fresh预检和真实产量验收。

2026-09-14 / EXP-001/030/196/239/300冷部署、恢复与转换复验（`validated`限当前安装和原生转换）：178b506/CI34796258288绿后明确关闭十写，raw-c6734685正常关旧进程、保留程序集备份、同批6070cf8的228文件安装并单次Steam启动；raw-159ace38核新Plugin认证、64/1/71120字符和纯stdout。Luna唯一protected resume，root raw-11294d8e/57254850核完整4386/8598、38详情、全玩家/原J88/两网/完整科技，主档57243648，accepted1保持。私有审计曾把operation标签误作actionKind，在零新游戏调用处停止；依据原始终态及OwnedWorldResumeCoordinator的resume-owned-game常量修正、补正负例后15离线检查通过，不重放恢复。raw-b5ef2d8b的同代码/同锚点16对象全部从NotAlignTropicAnchor变Ok，但修正姿态仍有12范围/2占位；raw-f9b37280单分区零位移且准确拒绝原实体占位。最近复验同日；Ok仅是转换条件，nativeCheckPerformed=false仍不得prepare施工。后续先依据返回姿态核空地，再按业务范围判断必要移动；不能继续使用修复前位置、删除占位对象或把转换通过等同发电。

2026-09-14 / EXP-001/030/196/239十写保存边界复验（`validated`限当前保存与完整审计）：Luna raw-d748a0c8普通save57243616，root raw-dc13df76核同一终态；raw-fe968861/57250221独立核十个唯一原始/fresh动作、全4386实体/8598互返有向边、38详情及全玩家净2011−1/2203+16。手搓最后一项只采用原action的晚到终态，不补提交；成功Move后用真正新位置作保存前后见证，整窗另外核明确位置变化，不能复用移动前的“静止”基准。原J88、自然完成科技、两网仅增一消费者与零prebuild保持。12保存/46审计专用及各125/137继承离线、六入口拒绝通过；最近复验同日。accepted10不因save归零，需结论push/绿CI后显式关闭。暂存6070cf8的1852测试、228文件及64/1元数据不等于新Plugin已经装载；安装后须另核同批哈希、新进程和同档恢复。背包风机、可用票据、离线修复均不抵扣实际发电或持续生产；部署/会话/现场变化后重新核验。

2026-09-14 / EXP-300 — 跨纬线蓝图必须复用完整原生游标吸附（`validated`限当前DLL转换，完整施工另验）：IFX-130的原生UI证据显示RecalculateCursorPos之后需要SnapTropic，再生成分区/预览。漏掉此步时，原位置网格吸附和游标跨区重算可造成比例/锚点错误；raw-5d3ef722与raw-081e2e27是两次零写负例，不代表真实施工尝试。适配器按同一顺序调用原生方法，只修改工具的一点数组；既有8区/每区高≤256使纬度扫描≤2049次，后续SnapLongitude固定≤160轮/每轮≤8区，不新增目标规划循环。审批必须使用返回的逐对象姿态，既有64m跨度、玩家范围、碰撞、科技、材料、电力和哈希全部保持。1852测试、完整Release零警告错误、编译调用顺序及源码64/1验证与后续raw-b5ef2d8b/f9b37280的独立本机证据分开；前者不运行Unity，后者证明同锚点跨区对齐修正及单区零偏差/正确占位拒绝。最近复验同日，已冷部署并同档恢复，accepted1/J88/primary57243648；修正后的范围、占位、完整原生检查和真实施工仍需通过，不把转换Ok计为新增容量。游戏DLL、吸附规则、源分区、现场或姿态变化时重验，不重放成功前缀或清除原生拒绝。

2026-09-14 / EXP-135/196/237/253移动失败上限与蓝图纬线边界复验（`validated`限两个未提交落点及一个选址负例）：raw-56d74339/57198266的完整27点采样末端仍低于水面0.810m，第二次同类干地余量失败后退役后缀入口，未提交或重放已成功首段。root转为业务本身的只读整图检查，raw-5d3ef722/57201312显示16/16材料、科技及主网3全节点预算可满足，但源蓝图跨纬线分区，任意切平面新锚点并不保留原生分区比例，16对象均BlueprintWrongTropicRatio，另10处占位；原生建设检查尚未执行。17专用/108继承离线、入口拒绝及21 ActionClient通过，最近复验同日。必须同时满足原生翻译/地形/碰撞/整图材料/完整功率，不以某一维通过抹掉其他blocker；后续先研究源分区和正常原生平移，再核新址，不加寻路、放宽条件或重做备料。accepted9及原位置/库存/J88保持，未保存或建设；预算增量80000 J/t不计实发，供给、现场、源代码或原生规则变化后重新验证。

2026-09-14 / EXP-009/237/246/253有限跨越与已成功前缀复验（`validated`限本次接近）：raw-ad8fea4a首段约18.3m、中间浅水深约0.309m、末端连续干地约4.812m；遵守既有完整采样/零unknown/高度/能量及1.5m容差约束后，57177477正常Move成功。root raw-01426c6c/57179649确认同一终态、Walk0/800MJ、全库存/16风机/J88保持。聚合not_detected不是全程干地或路径无碰撞保证；第二段虽有完整采样，末端仍湿，调用方在commit前拒绝。现有suffix计算出现负一个采样间隔时只解释为无合格干地余量，不据此放行或改原生规则。28专用/80继承离线、入口拒绝及21 ActionClient通过；最近复验同日。双段入口退役、accepted9保持，下一只重新设计未提交后缀，成功前缀绝不重放；没有新工具、自动寻路或位置写入。现场/采样/玩家状态变化时重新预检；本次不证明新风机选址、发电、保存恢复或完整供能。

2026-09-14 / EXP-030/196有界备料与晚到终态复验（`validated`限本批16风机）：Luna raw-aa40808f六步正常取料/手搓，最后r7×16在调用方观察到期时仍active；root raw-b75e2248沿同一action核57161332成功，raw-f50ab3dc/57164112独立核六唯一终态、即时材料守恒、全玩家净增16风机、38配置/原J88/两网/零prebuild。等待到期不等于原生失败或补交许可；执行入口退役，仅接受精确原最后action的后来终态证据，缺失、错ID、非最后对象或pending均拒绝。74执行/5晚到终态专用与158继承离线、21 ActionClient通过。首次事件缺席可能是历史排除，不能从成品倒造手搓首次；本次J88保持。最近复验同日，accepted8不重置。背包成品不计装机容量；新址、原生碰撞、接网、逐对象扣料、保存恢复及持续燃料仍需独立证明，游戏/玩家/日记/原action变化时重新核销，绝不重放整批。

2026-09-14 / EXP-135/196/208有限补电须继承完整预算与真实选区（`validated`限读回、导出/重解析、整批材料预检）：raw-adccca35/57120949复核53对象和四聚变输出，满基础1521800加待建入口600仍比容量1485000高37400 J/t，当前198426满供不能替代峰值。16风机的80000原生风能只是候选增量；最低已观察1449000加增量仅余6600，不能保证未来燃料不降或新址没有额外负荷。raw-ec94ba9a导出/重解析16明确风机、相同hash和零边界；相邻实体编号不保证空间相邻，2981/2982因真实距离超界排除。723首次材料预检因动态库存STALE_STATE零写拒绝，不能重放成功导出或削弱hash；raw-512d420f/57138976通过先玩家后源仓的fresh次序，在1511/562取得三项明确材料正例，预算112铁/48磁铁/24铜与r5×16/r6×24/r7×16闭合。私有缓存读回的Int64/Int32键问题在零写边界停止，增加规范化及存在性负例后才继续。20/34/14专用加26基础离线与21 ActionClient通过；最近复验同日，accepted2不变。新址原生碰撞、额外覆盖负荷、完整材料及建成后实际功率尚未通过；库存/现场/配方/负荷变化时重新计算，不把只读导出或材料正例冒充蓝图施工或持续供给。

2026-09-14 / EXP-001/030/239普通保存须覆盖自然科研最终状态复验（`validated`限本次保存）：Luna raw-56fd4b83在57109484唯一正常save成功，root raw-47195a7a/57110388核相同终态、零物品差、精确主档tick及新保护恢复能力，36选定对象/玩家/原J88/两网/零prebuild保持。1133/1312最终完成现在都被主档覆盖；2104/2904仍未选，不能因已有后续计划补造解锁。12专用加14继承离线及21 ActionClient通过，accepted2不重置。最近复验同日；本次是选定配置与科研的保存证明，不是新的全厂审计、持续供给或实际重启验收。科研自然完成、主档tick或Journal变化后重核覆盖；仅有ticket可用不能代替退出/恢复实测。私有离线摘录另暴露“某次详情集合不含所需目标”和“臆测DTO字段”两项调用错误，均在零游戏调用时拒绝；后续先核精确记录覆盖及契约字段，不能把缺失摘录解释为实体消失或原生失败。

2026-09-14 / EXP-196/208启动观察与持续供给分层复验（`validated`限六窗原生计数和边界发电，`observed`供给趋势）：raw-664f87fe/57094482的六个独立600tick窗全部保留，包括2/0/2/2/0/2棒的零窗和49/52/54/55/47/51tick间隙。三新聚变站各窗前后均有正J/t输出，证明实际启动，不能把功率缓冲当件数；正常960tick配方周期跨越600tick统计边界，不因短窗0或瞬时空输出便判定停产。源仓848→826仍有启动填充/在途变化，带货可能重复投影，不能相加或用有间隙产量补成守恒；重氢仓580→579必须连同上游输出26→2和下游输入3→18解释，不能据单仓平稳宣称配平。主网末窗容量1485000 J/t不替代满基础1521800预算，最终石墨入口另计600，缺口37400仍有效；仅安装但未供料的发电设备不记为可用容量。22只读观察离线、解析/入口拒绝及21 ActionClient检查通过，原36配置/玩家/J88/两网保持、accepted1不重置。最近复验同日；正常保存、恢复、持续补充、最终石墨入口与全功率分别另验，功率、燃料分配、负荷或现场变化后必须fresh重算，不把本次启动观察当作全0.4通过。

2026-09-14 / EXP-061/135/185/242/299最终源接通后的证据边界（`validated`限施工/守恒/拓扑，`observed`初始供料）：raw-523ed402在147带/七被动分拣器/三站全部可达后取得3955槽9原生正例；Luna raw-53143480正常建4386，root raw-d6c9d081/57084045核原始/fresh终态、全4386/8598、玩家仅扣1普通、源旧槽6/8完整、实际目标带槽4互返与device持槽、原J88/两网/零prebuild。源接通后cargo可以从空变为1802，不能沿用被动施工阶段“必须空带/空sorter”的观察断言；仍须完整只含指定燃料、配置不变和正常物流，绝不修改原生hash/幂等/终态。35专用加42继承离线及21 ActionClient通过。最近复验同日；raw-91795758仅见A站发电、B/C尚在输送，原生600tick产出2棒但设备输出缓冲为0，既不能因瞬时0缓冲否认生产，也不能把900启动库存、可达拓扑或一站发电升级为三站持续供应。满基础预留1521800/当次容量913000 J/t仍有缺口，未供料站不计容量；accepted1不归零、primary56628333未更新，后续窗口/功率/保存恢复各自验收。

2026-09-14 / EXP-061/135/185/188/242/299四项后缀与跨调用十写核销复验（`validated`限施工、材料和拓扑）：raw-ad16163a只重验未提交的四处，Luna raw-774287d3正常完成4382至4385；先前4381成功后返航观察到期不影响其保留，后缀执行器追踪返航但不重放已成功动作。root raw-bd7981a7/57057166核本窗口十个原始/fresh终态、4385实体/8594互返边、原4376配置只加批准边、两新带七新分拣器、全玩家净铁−3/带+1/普通−6/高速−1/齿轮0、原J88/两网/零prebuild；必须将首接点原收据与后四项收据分别引用，不能把恢复后的执行伪造成同次无间断批次。22后缀和58审计专用检查各加350继承离线及21 ActionClient通过。最近复验同日；accepted10仍冻结至本结论push/绿CI及显式保护记录关闭。147带及七被动接点完成不等于供料，源3955仍断开，primary56628333未更新；只允许fresh验证最后源入口，不重建旧对象。源库存900燃料仍是有限存量，随后须分别证明持续补充、实际发电、满基础负载预算和保存恢复，不能用已接线或瞬时满供替代。

2026-09-14 / EXP-135/188/299施工终态与无人机返航观察分开核销（`validated`限本次首接点）：raw-60cc28b7的4381建设在57031293已有原始成功终态，后续私有客户端的返航等待到期仅使PTY退出，后四项没有提交。raw-3fe22bfe读同一终态并确认三机idle；raw-923e7e48/57035998独立核全4381/8578、物理槽4互返、玩家仅扣1普通、原J88/两网/零prebuild。终态先持久化，调用方异常不重写游戏结论；原执行器最后session读数早于终态时，独立settled读数必须标明来源，不能补造原收据。66专用加284继承离线检查通过，混合历史预检中的失败响应须先按实际字段筛出正例，缺字段在离线阶段已拦下，未产生游戏写。最近复验同日；accepted6不归零，只允许fresh批准未提交的四项后缀，观察时间用尽时仍跟原handle/返航状态，不延长原生动作、重放首项或声称整批完成。源未开、primary56628333未更新，持续供给另验。

2026-09-14 / EXP-061/185/242有限中继须以实际持槽和受电闭环核销（`validated`限本次两接点施工）：raw-b87629b3在实际4377/4378建成后对不同于旧失败的两接点取得原生正例，取料覆盖10.126/6.289m均有声明余量；Luna raw-c6532b3f正常建4379/4380，root raw-8829a840/56978017核两原始/fresh终态、全4380/8574、逐端点物理槽4互返、旧邻居/完整玩家/J88/两网/零prebuild。普通span3和高速span2的原生时序分别600000/200000，不能只按item等级推定实际吞吐。34预检及34执行器专用和继承离线检查通过。最近复验同日；旧4355→4356的TooSkew仍有效，新中继只是经有限正常施工解决当前几何阻塞，不是修改原生规则或通用自动寻路。accepted5/primary56628333保留，源仍断开；后续只能增加明确未建边，真实燃料、库存趋势、完整功率与保存恢复仍需分别验收。

2026-09-14 / EXP-061/185/242中继几何从预测推进到原生土建（`validated`仅两带建设）：raw-4d0a00a0以齐料、全4376现场和完整native_grid预检确认两条NEW；Luna raw-12d0103c正常建4377→4378，root raw-867c3553/56890747核原始/fresh终态、全4378/8566、两条实际位置/旋转/四向端点、21原配置、玩家仅−2带/J88/两网/零prebuild。28预检/19施工专用及继承离线、解析/入口拒绝通过。旧4355→4356的16.896°负例仍有效，不被新带建设改判；预测朝向现在可由实际端点替代，但两个新分拣接点仍须各自fresh原生预检和正常施工证明，不能跳步宣称角度阻塞已解除。最近复验同日；accepted3/primary56628333保留，未供料/保存。后续接线只增加批准的新边，复核真实受电、材料和逐对象反向持槽，不修改或重做旧成功路径。

2026-09-14 / EXP-135/188/299两段正常手搓按中间态与净差分别核销（`validated`限本次中继备料）：c094b21/绿CI后的raw-7f18c513显式关闭原十写；Luna raw-011505d1只做r5×1、r84×1，root raw-43aeb4c2/56831984核两项原始/fresh终态、21配置/全玩家/原J88/两网/零prebuild。r5的齿轮+1和r84的齿轮−1必须分别保留，整批净差为铁−3、带+3、齿轮0，不能把第二步的齿轮消耗误写成整批净消耗。26专用加143继承离线及解析/入口拒绝通过，accepted2不因手搓或新调用归零。最近复验同日；primary56628333未覆盖本次备料，4条背包带只是齐料证据，后续仍需完整native场地预检、实际新带姿态与双边接线证明，不能把离线中继推断升级为施工或持续供料通过。

2026-09-14 / EXP-061/185/188跨带拆成有界中继且按原生配方补净缺口（`observed`中继几何，`validated`仅备料及十写审计）：raw-4fefa0f7的两新带候选把原16.896°失败改为预测1.173°/5.366°，同时满足长度/跨度；既有坐标复现不等于未来CargoTraffic朝向、碰撞或分拣器施工已通过。原145带与所有站杆保留，用已有一只备用2012，不另做普通分拣器。raw-b6a7a2f0的fresh r5/r84证明制3带实际需要共3铁，不能凭印象把r84的2铁写成1铁；取料前把齿轮中间态和整段库存算清。raw-434fedfd正常取料，root raw-76df9171/56771215核十个原始/fresh终态、全4376/8564/21见证/玩家/J88/两网/零prebuild；27材料/40审计及继承离线通过。accepted10保持冻结，主档仍56628333；最近复验同日。齐料后必须重做完整原生放置及实际端点预检，3300J/t仅是八个燃料分拣器加最后石墨入口的运行余量，不是全厂满负载预算或持续燃料产率。

2026-09-14 / EXP-001/030/239同批环境与自然科研保存边界复验（`validated`限本机当前安装和读取）：raw-d8736d32复核228安装文件、64 tools/1 resource/70562字指南/当前Plugin认证/纯stdout；raw-213ee643复核游戏及BepInEx两原生引用，15几何Core测试通过，没有部署或重启。raw-76df9171确认1133/1312自然完成且J88不新增伪造事件，但primary56628333仍早于1312解锁，正常保存研究启动不能替代保存最终完成。最近复验同日；下一保存恢复须核最终科技和新增物料，不能仅凭旧票据可用宣称已保护最新进度。离线中继原型的数值键JSON输出异常在零游戏调用阶段改为既有安全序列化后复跑，不影响原生动作或记为游戏失败。

2026-09-14 / EXP-061/185/242独立补验未尝试后缀（`validated`限三个接点预检）：raw-b7172853/56709587对4368→4373、4375→4229槽3、4370→4230槽1首次取得exact_slots正例，真实取料点覆盖9.553/2.832/4.189m全部满足声明余量；完整4376/8564/原玩家/J88/两网保持。23专用加93继承离线及解析/入口拒绝通过，零commit。前段失败后，不应把未调用的后缀标成失败或通过；可以不重试前段而独立消除后缀未知，随后只设计真实阻塞。最近复验同日；施工前仍用新的玩家/端点hash和计划，逐对象核物理持槽、受电、材料及终态，不能复用这次预检凭据或将正例外推为持续生产。

2026-09-14 / EXP-135/239科研自然完成须与保存水位分开（`validated`限1312本次解锁复读）：raw-199ef65d/56672790、raw-4a80b48d/56678809均确认1312已解锁、288000/288000、unlockTick56633621，队列为空且2104/2904仍未解锁。原durable J88保持，零新游戏写；首次选择与完成是不同证据，不补造完成时的首次选择。现primary56628333早于解锁5288 tick，已成功保存研究启动不等于保存最终完成。最近复验同日；下一普通保存/恢复后须再核科技，不能拿解锁配方代替紫糖实际产出、把currentLevel4当成已完成或把最低曲速前置当完整科技准备。

2026-09-14 / EXP-001/135/239补电后的保存边界复验（`validated`限正常保存及后读）：raw-1c44f462保存56628333，root raw-df37eb26/56633776核原始/fresh终态、20配置/全玩家/两网/J88/零prebuild和重新签发protected resume，15专用加50继承离线及解析/三入口拒绝通过。accepted9/revision261保留；本次保护完整145带、4376电塔和科研选择，不冒充实际重启、接线或持续燃料。最近复验同日；下一写后若达十笔必须先完整审计，不因保存自动重置。

2026-09-14 / EXP-061/185/030跨带角度与长度必须同时满足（`validated`限当前六实体/九组合的排除）：raw-b63f587a用已观察四向端点和当前Core公式重现原失败4355→4356的16.896°，并逐对证明角度合格的4353→4356等组合都超过5m。当前同批原生BuildTool_Inserter对belt/belt长度0.4–5m、跨度≤3.2；device/device才是0.9–7.5m、跨度≤3.799。角度通过不是完整原生正例，不据此选择必然超距的下一候选；本次零prepare/commit。15个Core端点几何测试用既有portable SDK通过，PATH runtime-only未能启动测试的尝试不计通过，也不修改系统SDK配置。最近复验同日；下一有限中继需重新核材料、全部NEW占位、源/目标实际端点、正常供电和完整native预检，不能用几何筛选扩大白名单、改成功对象或把旧TooSkew直接变成已修复。

2026-09-14 / EXP-061/185/242分开核销覆盖修复与下一端点角度（`validated`限本塔施工及已观察预检）：raw-a3404ed5正常建成4376，root raw-13c1e2cc/56597573核完整4376实体/8564互返边、唯一原始/fresh终态、玩家仅扣1塔、主网仅增1节点/0消费者/J88/零prebuild。raw-252e4916的4349→4227重新prepare证明真实取料点距新塔2.516m，覆盖阻塞已解决，不能继续引用旧中点或旧缺电结论。下一4355→4356却因TooSkew在16个端点候选的角度前置阶段拒绝，nativeChecks=0；带已建成、跨度看似足够和供电可达都不能代替端点朝向或完整原生连接检查，备用belt/device规则也不能外推给belt/belt。该失败是零commit预检，不是写隔离；后三处仍未知。19施工加31继承、6只读加93继承离线与解析/入口拒绝通过。最近复验同日；下一先保护已完成部分，由root检查实际源/目标位置及旋转、旧路径与候选端口；保留所有成功带/站/塔，不扩大动作原语或重复失败接点。accepted8、primary56326770不自动归零或更新，持续供给和完整功率另验。

2026-09-14 / EXP-135/188/299复用背包余料的最小补电备料（`validated`限取料/手搓）：fresh玩家仍有1202×1，原生r8只需1101×2，不应忽略余料而另做磁线圈或扩大组件批次。raw-e61d9c61只预检两铁转移，Luna raw-34cd262f的正常转移/手搓由root raw-7a813e00/56558638核原始/fresh终态、源仓3000→2998的即时守恒、玩家完整差量/原位置、13旧配置/源仓/两网/J88/零prebuild。27专用加31继承离线、解析/三入口拒绝及21 action-client通过；accepted7/revision258，未施工或保存。科研可以自然推进，累计写计数不因辅助材料阶段重新开始；fresh revision以原动作与当前状态核销，不能硬编码“每次写只+1”。最近复验同日；取料点候选只是独立离线几何，必须在实际拿到电塔后重新验证完整原生放置、未来分拣通道及受电/合网，不能用背包有塔替代覆盖已修复。

2026-09-14 / EXP-135/239科研自然完成与后续选择复验（`validated`限本次完成/选择/正增长）：raw-3646b271的1133为unlocked、180000/180000、unlockTick56478014；不能仅以旧队列或投入接近上限判断完成。确认空队列、1312全部原生前置满足和fresh selection hash后，Luna raw-afec4b00正常选择1312，56489621成功且durable J88。root raw-db0d1a20/56493549核原始/fresh终态、唯一队列及36→6874/288000正常增长、其他科技保持、完整玩家/13配置/两网/零prebuild和原日记前缀。17专用及14继承离线复跑通过，accepted5不归零、主档仍56326770。本条只将1133从进行中提升为已解锁，不将1312选择提升为已解锁、紫糖产出或驱动4完成；科技自然完成不伪造新的“首次选择”事件。最近复验同日；研究结束、缺料、供给/身份变化及保存恢复后重新读回，科研与施工可以并行但各自证据和保存水位必须分开。

2026-09-14 / EXP-061/185分拣器实际取料点而非中点的覆盖负例（`validated`限当前预检；草案feed-A覆盖推断失效）：raw-34dd14bd/56459027的4349→4227槽1原生prepare成功，plannedPosition确为sourcePosition，距4227为4.795m；原草案estimatedCenter距站3.458m不代表原生受电位置，不能用4.5m站覆盖放行。原生源码既有BuildStepPlan.Inserter保留源位置，PowerSystem.NewConsumerComponent采用entity.pos再按plugAlt投影，和已记录的取料点供电规则一致；未增加游戏API。raw-7d4e510c复核全147现有节点无满足声明余量的候选。两项更早端口预检通过不补足第三项供电，后四项保持未知，七项均未commit；accepted4/J87/所有玩家物品和已有4355→4375建设保持。29专用加64继承离线、解析/三入口拒绝通过。最近复验同日；本条替代候选JSON中feed-A中点覆盖的推断，不废弃已完成实体、原生连接正例或其他尚未实测的候选；先修正有限供电方案，施工后仍须读取真实powerNetworkId/serveRatio与物料流，不能把本负例说成已建空转或整个电网断电。

2026-09-14 / EXP-061/242/299独立尾段与完整前缀保全复验（`validated`限施工/材料/拓扑）：raw-13f27cda在全4355现场对照及全部NEW带点占位检查后，两条原生15/5带路径通过；Luna raw-0d62df7d正常完成，root raw-494d1440/56428247核两个原始/fresh终态、4375全实体/8564互返边、全玩家只扣20带、原J87/两网/零prebuild。原125带完全保留，B段实际顺序4373/4371/4372/4374/4375再次证明不能按分配ID大小猜拓扑。22专用及42继承离线、解析/四入口拒绝通过；accepted4不归零，主档仍56326770，新增尾段尚未保存。raw-253ca098/ae43fa1d另证明原同批228文件、原生引用及实际MCP64/1/70562指南未漂移，不能称作新部署或重启。最近复验同日；七处分拣连接必须使用实际建成端口和原生计划的真实取料位置重新验证供电，草案中心/空带/仓中有限900燃料均不代替持续供给、全基础负载预算或完整0.4验收。供料、现场/科研、保存或重启变化后再验。

2026-09-14 / EXP-001/135/239科研期间普通保存复验（`validated`限当前保存及后续读取）：raw-ca58b5ac正常保存56326770，root raw-23669c89/56330481核原始/fresh终态、20配置/玩家/两网/原durable J87/零prebuild及更新primary/protected resume可用。正常科研可继续推进，不以“等待研究完成”推迟保护已经完成的建设；保存不是新研究选择、恢复验收或产量窗口。14离线、解析/三入口拒绝通过，accepted2/revision249不归零。raw-851038a4/56333559仍见1133在队列、35470/180000且未解锁，其余三项未排；该正增长与机甲空科研库存的既有证据一致，不据此声称长期供糖充足。最近复验同日；重启、科研完成/缺料、供给或身份变化后重新核验，三座空载新站仍不计持续发电能力。

2026-09-14 / EXP-135/239正常科技选择与研究进展复验（`validated`限1133本次选择和正增长）：已审计十写在cdc75ac/绿CI后由raw-aa3bc75a显式10→0，raw-52ccbbfe只通过fresh selection hash选择1133，56298014原生终态成功，durable J87，蓝800/红800/黄200是完整研究预算而非选择时一次扣料。root raw-0ebc6c43/56305389复读同一action、队列1133和6813→7249/180000正常研究；其他科技、玩家、20配置、两网/零prebuild/J87保持。必须分开“选择成功”“研究正增长”“最终解锁”和“完整远征准备”，最初16点与后续正增长均不能抵扣未完成科技。机甲无矩阵库存/科研点，正常投入来自既有研究设施；本项不冒充长期供糖速率。19离线及复用42项、解析/入口拒绝通过，accepted1/primary56035121尚未保存。最近复验同日；研究缺料、完成、供给变动或保存恢复后再验，不直接改科技、不把当前预算当全部远征升级需求。

2026-09-14 / EXP-242/299分段续建与跨调用十写核销复验（`validated`限本次材料/配置/拓扑）：raw-9a5d0882只从已独立证明的4309继续36/5/5NEW，三个原动作均成功，未重放旧79条；累计125新带、背包21带。root raw-6eabf5cb/56276316合并save、三设施及两次调用内六路径的十个唯一acceptance，9原始完工加1原handle后续核销分别匹配fresh，不因调用方分段或等待到期而重置计数/补造收据。全部4355实体/8528互返边、20见证/全玩家/J86/两网/零prebuild通过；accepted10仍须本结论push及绿CI后显式关闭。19后缀、43审计及复用42离线检查、解析/入口拒绝通过。分页类型在实体而非页外层，私有初版错误读取已在零游戏调用离线检查中拦住并修正；不是新增产品字段需求。最近复验同日，后续供料、源路径有货、保存或重启后再验；这些空载带不证明持续燃料、满基础负载功率或最终0.4通过。

2026-09-14 / EXP-242/299长施工等待与有限后缀复验（`validated`限本次原动作核销）：raw-8da64964前三段各8/23/48条NEW，第三段仅调用方8次观察到期；raw-f548f677仍见9个待建/3机工作/约758MJ，raw-c4caf190读同一6429b866成功终态56171146。root raw-a91c5175/56205252核三原handle、4309全实体/8436互返边、材料仅79带、原玩家/J86/两网/零prebuild；accepted7保留。不得将观察异常改判原生失败、重放已接受部分或补造原执行器的终态记录；只从明确未提交后缀继续，并承接累计写计数。23审计加复用42施工离线检查、解析及三入口拒绝通过。第三段复用旧西段时，保留原生whole_path_native_rotation_v1成功证明，并复读23对象身份/位置/旧边/空货物；不是无条件忽略rotation。最近复验同日；下一动作、断线、源货物或现场变化后重验。未新增产品故障，不代替持续供料、完整供电或保存恢复证明。

2026-09-14 / EXP-061/135/299零负载施工后缀复验（`validated`仅建设与拓扑）：raw-d2aa7d98三处fresh原生正例后，raw-a5bd594d依次正常建成4228/4229/4230，root raw-aeca0ddb/56077086核三项原始/fresh终态、4230实体/8282互返边、材料净扣1杆2站、原玩家/J86/两网/零prebuild。32离线及解析/三入口拒绝通过；不新增consumer，不信用未供料发电，不重建旧4226/4227。执行授权必须配对“精确脚本路径＋该文件哈希＋当前有限步骤”，不能把旧首段脚本哈希套给后缀；本次误套在任何API调用前停止，root核新文件后执行，没有代码漂移或原生失败。accepted4仍承接前一次save，未自动清计数；primary56035121尚未覆盖后缀。最近复验同日，供料/接线/保存变化后再验，不外推为完整功率或持续供给通过。

2026-09-14 / EXP-135出发科技证据复验（`validated`仅当前解锁状态及研究预算）：raw-01b0d306/56067282证明1704已解锁但2904/2104未解锁、队列空；读取器按当前LDB合并直接及隐式前置，原生研究路径两者都检查。1133/1312/2104/2904四个零投入缺项递归去重后研究需求为蓝4100/红4100/黄2700/紫500，不含建设/燃料/翘曲器或其他升级。必须区分“配方可造”“机甲曲速前置解锁”“补给持续生产”“实际航行验收”，也不能把未解锁记录的currentLevel=4当作已升4级；以unlocked及原生前置为准。此次零写，不记为首次研究或产物事件。最近复验同日；科技、原料、队列及现场变化后重新读取，不把该最低预算升级为完整远征清单。

2026-09-14 / EXP-001/135/299首段设施保存复验（`validated`仅正常保存）：十写审计964208e推送/CI34770681700绿后，保护记录raw-0e5d4e6d显式10→0；raw-54920eb7的唯一正常save终态56035121、revision228/accepted1及新protected-resume可用由root raw-34a0c9f2独立复读。九配置、全玩家、原durable J86、两网成员/满供和零prebuild保持，16离线/解析/三个入口拒绝通过。沿用“审计通过不自动清计数、保存不代替重启、空载站不计容量”的原结论；未产生新的接口故障。最近复验同日；下一写或恢复仍须fresh身份与现场证据。

2026-09-14 / EXP-061/135/185有限零负载施工前置（`validated`限首杆/首站建设与网络读回）：root raw-d0309ffe于55962712复核完整4225配置、两网满供、原玩家/J86与accepted8/revision223，再对固定pole-link和forest-A做fresh native预检，两项姿态/材料预算均通过。当前目录证明两者均为非charger、非consumer、work/idle为0的power node；2975到新杆8.899m、杆到新站15.879m，在已知22.5m连接距离内。可先按正常成本/无人机完成这两个不新增负载的基础设施对象，并逐对象核主网仅增加节点/发电机、没有新消费者或意外合网；不把空载站记为新增发电容量。25离线及解析/入口拒绝通过，执行后accepted10必须冻结并完整审计。燃料分拣器、最终石墨入口、完整满基础负载余量和整组持续供给仍未批准或通过；不是为了绕过Foundry整图预算而提前宣称建厂完成，后续组合计划仍须满足其原有门禁。最近复验同日，现场/库存/网络变化即失效。

同日实机补充：Luna raw-c9ae3a21正常建成4226/4227，55972016/55972941终态成功且主网节点+2/发电机+1/消费者不变；两件材料正确扣除，站无物流连接、发电0。root raw-ba8f8e2c于55986042核十项原始/fresh成功终态、4227实体/8282互返有向边、九详情、原玩家/J86/两网满供/零prebuild，27审计离线检查通过（另复用25施工及64备料检查）。该正例把上述预检提升为正常施工与网络读回证据，未提升持续供料/完整功率/保存恢复结论；accepted10/revision227/primary55240705仍须待push/绿CI后显式关闭，不能因审计成功自动重置或重放成功前缀。

2026-09-14 / EXP-135/188/299必要组件有限备料与施工门分离（`validated`限本批材料守恒）：Luna raw-64e58934只从1511/562取42铁/4铜，再按当前r5×9、r84×9、r50×4、r85×7正常手搓，六项55921064–55922706均原始terminal成功。每步重新绑定玩家/源仓状态与原生配方预算，不借递归手搓扩大材料，不将完整电力尚未证明误标成已施工。root raw-bb7824e3在55936185独立核六项原始/fresh终态、背包净增27带/7普通分拣器/1板、原3站/2杆/2快、原J86、两网满供及完整4225配置/零prebuild；accepted8不归零，主档仍55240705。

64执行器与25审计离线检查及解析/三入口拒绝通过；显式模式在任何导入前快照和拒绝，无模式不能进入游戏调用。成功备料只是后续已选燃料线路的材料前缀，不能外推为source-cover/端口/地形/持续发电/完整Foundry通过，也不能重跑已经接受的六步。复验触发为下一阶段现场、背包、session、负载或供料变化；最近复验2026-09-14，相关全材料、日志身份和禁止重放经验保持，未产生新的产品接口故障。

2026-09-14 / EXP-061/135/185/299分项原生预检与动态供电预算复核（`validated`限当前证据，联合施工仍`observed`）：单独合法的未来对象可能互相占位。raw-666d6c28/731c5f57的带/杆都通过原生预检，但联合几何检查发现未建杆距未来C-spine约0.624m；raw-a187f5c4在施工前修正杆位和向站体偏近的A-prefix，raw-b2c865a3使用已有3955真实sorterEndpoints重对齐入口。八段149点中的四个共享点只能在实际source-cover证明后按145 NEW计料；不将独立路径的首点在接线时再建一次。七个未来分拣器仍需真实实体和原生端口证据，不把几何跨度当作已通过原生角度/碰撞。

raw-9876966c的fresh主网容量663000，不支持沿用旧843000以及三站后71200余量。当前同网61风/9火电/1聚变且无太阳能；与既有game-api-foundry的EnergyCap_Fuel证据一致，燃料容量是动态值，不能既把轻载required当满负载，又把某次较高capacity当长期保证。仅计风力和正常供料后的四台聚变为1305000 J/t，尚低于旧记录1519100满基础负载加2700新负载，完整供电结论必须保持未通过；这不是native建造规则的新增门禁，也不授权扩大白名单或改成功设施。

当前材料源和配方足以独立完成42铁/4铜→27带/7普通分拣器（余1板），不必把必要组件制作同后续负载接通混为一个不可分割动作；但材料阶段只能声明备料完成，不证明三站施工、持续燃料或Foundry整体验收。9项只读预算及64项有限执行器离线/解析/三入口拒绝通过；55866085玩家/原J86/healthy、accepted2/revision213/primary55240705保持，无游戏提交。复验触发：供料、发电状态、负载、端口、现场或session变化；最近复验2026-09-14。历史高容量保留为当时读数，固定余量推断被本段取代。

2026-09-13 / EXP-061/135/188/299资源辅助选址与原生足迹正例（`validated`限三个固定落点，运输方案仍`observed`）：只读资源节点可用来提出有地面依据的候选，但树/石头的位置不是整片地面或碰撞证明。vegetation还包含Detail；raw-37bcd690的128页上限只产生截断观察，不能推断未出现的区域无资源。使用现有resourceType=Tree后，raw-89379499的26页/2534棵树为完整单快照，11离线及后验通过。相对方向必须投影到当前球面north/east切平面，不能用global x/z冒充本地四方位；Luna的离线摘要已据此纠正。

root以新森林区域三个明确树锚点14463/14502/14761形成独立固定方案，raw-39034844先fresh核全4225配置/零prebuild，再逐点读取原生资源身份并prepare2211；三个落点均原生通过、每项真实预算1，吸附后距玩家48.784/75.961/75.836m且保守足迹间距通过。21离线/解析/三入口拒绝和55734088的玩家/原durable J86/healthy边界复读通过，accepted2/revision213/primary55240705不变，无Move/施工/扣料。旧两套失败布局仍退休，未省略位置触发隐式搜索。

适用范围仅为当前场地的无副作用预检，不等于保留位置、联合施工许可、原生清树结果、整组供料或持续发电；旧90带/三塔预算不能移植到新地块。下一先完成新全路线/材料/电网与实际端口方案，施工仍由Luna fresh prepare并逐终态核销。复验触发：现场/树/材料/位置/朝向或session改变，以及正式施工前；最近复验同日。没有新增产品字段、工具、白名单或首次工程事故。

2026-09-13 / EXP-061/135/246/253一维干地不是建筑足迹证据（`validated`限地面/占位观察边界）：停止两个首站方案后，raw-8d1602dd只读南向29.746m地面，31样本/62射线/零unknown，末端18点为16.856187m连续干地；14离线/解析/三入口拒绝通过。这是既有Move预览中的观测数据，不是Move、建筑或整片区域批准。若不准备提交Move，不得把预览产生的短时凭据当作后续通用许可。

原4225实体快照在选定四个干地点旁仍有约1.6m距离的旧带，22点横向±12m又分别靠近2975风机/1241分拣器；这两个固定横向点只作几何筛选，未作native建造尝试。55628240原玩家/J86/revision213/primary55240705和accepted2保持；下一需证明二维足迹、原有设施和完整供料同时成立，不把干地线或中心净空外推为可建。旧两次失败布局已在导入前硬拒绝重新preview，需独立新设计才恢复候选；最近复验同日，无新增产品字段、原语或部署。

2026-09-13 / EXP-061/135/185/299全对象净空仍不是原生地面证明（`validated`限第二首站负例）：raw-454f2dfe对一个明确转向的新首站位置做native检查，虽已用全4225对象排除旧带条带，仍返回BUILD_LOCATION_INVALID/NeedGround。38+397离线/解析/三入口拒绝证明边界和物料算术，不证明地面或联合布置。与先前3594碰撞构成首站同阶段连续两次失败，必须停止更多首站坐标尝试，由root先补地面证据；零commit，原成功设施和accepted2不变。

raw-38c59b3c在55614794核完整玩家/位置、原durable J86、healthy owned/revision213/primary55240705保持。源码TryPrepareCoreBuild的显式位置只形成一个native候选；省略preferredPosition会形成最多12朝向×3距离×7横移的252候选列表，因此本轮不把省略坐标当作绕开停止线的捷径。复验触发为现场、地面、朝向、库存或选址实现变化；最近复验同日，不新增原语、放宽NeedGround或宣称全厂/施工通过。

2026-09-13 / EXP-061/135/185/299整组施工占位复验（`validated`限原生负例）：raw-f15ff7b9的首个聚变草案虽在玩家施工范围内，原生精确build-collider仍与既有带3594相交，返回BUILD_LOCATION_INVALID；候选筛选不能只看非belt建筑中心，已有带和分拣器不能因低矮而当作空地。34+397离线/解析/三入口拒绝只证明有界调用和预算，不替代原生占位。该首项后停止，后续八项未预检、零commit，原成功带路没有改动。

raw-f183dccb在55573966独立核玩家完整物品/位置、durable J86、3594配置及healthy owned/revision213/primary55240705，accepted2保持；不作全厂或供电产能声明。下一只重设计未建站点、朝向与供料路径，先把全部既有对象纳入几何筛选再fresh native检查；单项准备不是整组联合预约，更不是材料已齐或连接已建。最近复验同日；现场、朝向、材料或拓扑变化即复验，无新增产品故障或动作类型。

2026-09-13 / EXP-057/135/246/253/299施工范围与未建布局优先复核（`validated`限此次拒绝，替代布局尚为`observed`）：原三站草案有一处距实际玩家82.8m；root raw-bd7fa779尝试约5.899m完整地表预检，55513470的7点/14射线/零unknown仍不能支持落地，因为末点在水下0.251720m。零合格尾点在私有显示公式中成为−0.983102m，语义应读作没有有效干地而非负长度；不得拿aggregate not_detected或几何净空推翻原始地水命中。守卫在commit前拒绝，原两次成功Move和accepted2均保留。

raw-379078e8在55523129复核原始/fresh旧成功终态、12配置、全玩家/原J86/两网和零预建筑，800MJ/healthy/revision213/primary55240705；14+397候选离线、解析/四入口拒绝、10+387只读审计与精确负例核验通过。优先围绕可执行的业务范围设计尚未施工的整体布局，再决定是否需要Move；此原则不授权移动或删除成功设施、绕过材料/碰撞/供电预检，也不证明新布局已可建。下一停止该旧落点，不作同类重放；位置、场景和原生布局变化后fresh复验，最近复验同日，没有新增产品故障或全厂审计声明。

2026-09-13 / EXP-057/135/246/253/299近岸替代目标实机复验（`validated`仅限这一短移）：旧第二目标末端干地不足后，没有重复提交或重放成功首段；root raw-1f2d59b8绑定首段实际落点，选择已观测的近侧干地再做完整native预检。新的18.914m路线20样本/40射线/零unknown，中段最多约0.0269m浅水，连续干地末段9.9548m、末端地水余量0.197481m；allDry=false并不单独否决符合现有短跨越规则的有限尝试。Luna raw-780a0017在55436667正常成功，稳定Walk/速度0且无背包变化，未降低4m末段、1.5m普通容差、径向或能量规则。

root raw-6eece54c于55466049核两次原始/fresh成功action、12配置/全玩家/原J86/两网与零预建筑，accepted2/revision213不归零，主档仍55240705；10+377预检、14+387执行器、10+387审计离线及各自解析/三入口拒绝通过。此正例不证明后续路线无碰撞，也不替代完整施工方案或新全厂审计；附近材料库存及配方只作观察，不能当作已批准取料/制作。位置、实体布局、地表采样或session变化时仍须fresh复验；最近复验同日，原失败目标的负例保留，不新增产品故障或通用动作能力。

2026-09-13 / EXP-057/135/246/253/299接近成功前缀和第二段浅水负例（`validated`限精确两步边界）：d7c10bc/CI34761477133成功后，root raw-da859f3b的24.973m首段全干地预检通过；Luna raw-316d6b85首Move在55379289正常成功，实际落点(-112.186974,-96.27235,-135.02066)稳定Walk，accepted1/revision211。下一段必须按这个实际位置重新采样，不能复用名义两点间几何或过去走过的终点。

该fresh第二段33样本/66射线/零unknown，虽然native订单prepared且shoreRisk均not_detected，末端最后两点地面高于水面仅0.132080/0.064924m，只有一个样本满足0.1m余量，连续干地长度为0而不是4m；调用方在commit前拒绝，PTY exit1。保留唯一已成功首段，不把脚本非零退出泛化成首Move失败或重跑整个脚本；这个负例不授权降低地水门或放宽半径/到达容差。root raw-1fe4116e于55401198核原始/fresh成功action、13配置、全玩家/J86/两网/零预建筑，无unknown或隔离，accepted1不归零。14+351预检、19+365执行器、12+365前缀审计离线和解析/入口拒绝通过；安装态228文件/64工具/1资源/70562字符指南另复验通过。最近复验同日，下一只从已观察干地重新设计一个有界目标并重验完整地表，不把观察窗口、旧token或成功前缀当作后续授权；没有新产品故障或全厂审计声明。

2026-09-13 / EXP-057/061/135/185/299聚变施工前复验（`validated`限计数关闭与只读边界）：a988c67/CI34760450642成功后，raw-a96fca51于55306350以原十写审计、同一玩家/原J86/55240705主档和保护记录读回明确accepted10→0，revision209。raw-5f5c8471于55318565复核全4225配置/17详情及原背包，3955为900棒、3403输出20/满进度、3404输出10/满进度；当次两网满供但main3容量仍843000 J/t。三站成品不等于已建容量，有限库存不等于自动持续供给；选址和全供料预算仍未原生通过。20+324关闭、7+344只读入口离线及各自解析/三拒绝通过。

当前实际起点不能直接复用历史落点连线：raw-b578bd08复算两条未提交首段，分别距877中心1.023195m、距1214中心1.041275m，均在prepare前按现有全段3m调用方边界淘汰；这是几何排除，不是新原生失败、已观察碰撞体或实机卡住。不得将历史单次成功和端点可达当成新起点的路线授权；先用完整近邻布局重新设计有限绕行，再做完整native地表预检。两项精确反例复算通过，没有第三次同类直线尝试或游戏写入。最近复验同日；新路线、实际落点、配置或session变化都需fresh核验。原IFX-129浮点修复继续有效，没有新增产品缺陷或动作原语。

2026-09-13 / EXP-007/057/135/185/299三聚变有限成品与覆盖保存（`validated`限本次加工/十写审计）：Luna raw-bc1771d8按唯一十动作计划执行，返程Move于55227985成功，实际落点(-91.60643,-101.863457,-146.011414)满足三个业务仓真实可达。18酸/54石墨经既有r31线产36石墨烯，窗口55228483–55235353为6870tick；36石墨烯/12钛经r33线产24纳米管，窗口55236127–55239243为3116tick，均在事前有限上界内且最终输入/在途/设备进度归零。同期r103×30正常产30超磁环，r113×3正常得到三件2211，于55240705覆盖保存；不可把r103误写成石墨烯配方，也不把有限批次平均值当稳定自动供给。

root raw-c39cff49于55253680独立核十个原始/fresh终态、全4225实体/8282互返边/27详情与设备实际持槽、两条化工线最终全空、完整玩家净物料/inc和原durable J86/两网，exit0。合金36/涡轮60/磁铁90/酸18/处理器12/石墨84/钛12全部正常消耗，中间物无残余，119带/2高速保持，仅新增三件聚变站。16+238预检、45+254执行器和25+299审计离线及解析/三入口拒绝通过，实机没有额外Move、重放、未知终态或隔离。accepted10/revision209先冻结，本结论推送且CI成功后才以保护记录关闭计数。已有受保护恢复入口不等于本批已实际重启；没有新电站施工/加燃料，不能计新增容量或持续供给，最终热电入口仍未建。最近复验同日；下一三站选址、全线路/材料/供电预算和每次native预检仍独立完成，不复用已执行脚本或此前旧起点路径。

2026-09-13 / EXP-057/135/185/299三聚变有限加工预检（`observed`，以下为执行前截面，结果见上）：8e40485/CI34758586917通过后，raw-1a2efd21明确关闭上一十写，当前accepted0/revision196/primary55002268/J86。root raw-83d18e6f于55179348复核全4225配置/17详情、玩家原料和原日记/两网；从实际离仓位置到既有化工操作点的28.38218m原生预检含30样本/60射线/零unknown且全部干地，旧仓762/761全段中心距离3.56258/3.64942m，终点最近中心8.93918m，三个业务仓名义距离62.207–73.117m。中心净空不等于碰撞体保证，执行仍须一次fresh完整场景、实际位置与native预检；不批准额外Move或同目标失败重试。

只批准最多十个新accepted步骤：返程、向870投18酸并等原生首料拉取稳定、再投54石墨、r103×30、从871取36石墨烯、向3052投36石墨烯并等稳定、再投12钛、取24纳米管、r113×3、普通保存。r31/r33实际180/240tick与现有分拣器120/80tick周期得到保守串行有限批次上界16800/9240tick（含600tick余量），不是持续产率声明。两线实际net3满供且全空，不能直接沿用旧单站脚本的net1或12/8产物数；所有材料来自已保存背包，正常消耗后仅新增三件2211，不施工、不喂燃料、不计容量。J72/81及J32/74已分别记录环/聚变手搓与石墨烯/纳米管自动首次，不新增或伪造首次历史。16+238预检离线及解析/三入口拒绝通过；执行器必须逐步新双哈希/计划、原始终态和守恒，任何失败只保留成功前缀，十写后冻结审计。最近复验同日；位置、材料、原生配方、配置/电网、日记或session变化均重新验证。

2026-09-13 / EXP-007/057/061/135/185/246/253/299有限离仓及十写审计（`validated`限本次短移与审计）：71299cb/CI34757304775成功后，Luna raw-813d8156仅提交一次约4m的分离侧移，原始终态55066739成功，实际落点(-66.5139,-109.418472,-153.915833)、Walk/速度0、约799.85MJ；石墨84/钛12及其他材料保持，accepted9→10/revision196。这证明该精确短候选可执行，不证明旧返程可走、通用起点豁免或无碰撞自动寻路；没有返程、加工、施工或新保存。

root raw-f0bbd87f于55088826独立核本批两保存/六Move/两取料的十个原始及fresh终态，完整4225配置/8282互返有向边/28详情、设备实际持槽、全玩家净材料/inc、原durable J86、两网及零预建筑通过。12+173预检、13+185执行器、26+198审计离线及各自解析/三入口拒绝通过；审计初次离线缺省stage.final触发StrictMode，按IFX-108先ContainsKey再读取修正，未发生游戏调用。accepted10先冻结，只能在本结论commit/push且CI成功后以受保护记录明确归零；55002268主档已保护取料但尚未覆盖新位置，最终热电入口/三聚变站仍未建，不核销持续燃料或版本门。最近复验同日；返程须从新实际位置fresh核全段、终点容差、地水采样、能量和原业务可达性，不重放旧脚本或已完成步骤。

2026-09-13 / EXP-057/061/135/246/253与IFX-129的有限离仓候选（`observed`，以下为Move前截面，执行结果见上）：更正浮点投影后，root只选择约4m局部切向侧移，明确远离当前762近邻，不复用穿过762/761的旧返程。完整4225配置及40个含带/分拣器的近邻详情复核后，raw-a2b300f6于55047967完成一次native预检：实际弧长3.99566m、5样本/10射线/零unknown、全部干地，末点高于水面0.196283m。起点距762为2.76973m且分离点积2.86555为正，其他非带中心全段至少5.22933m，终点最近中心5.24905m，保留1.5m到达容差余量；这些只是明确短尝试的几何/地表证据，不是碰撞体或Walk保证。

只有这一精确起点/762身份/严格分离的单次短移候选可进入fresh prepare，普通路线仍保留3m检查；全局阈值、状态哈希和原生Move不变，不允许把近邻起点当作旧返程的豁免。12+173预检、13+185执行器离线及各自解析/三入口拒绝通过，包含朝仓内移、另一方向终点余量不足、旧返程及新增中途障碍的拒绝。accepted9/revision194/primary55002268/J86保持，Luna只可在本次新预检/批准后尝试一次；第十写终态后冻结整批审计，不追加返程或保存。最近复验同日；任何当前位置、对象配置、采样或能量变化均重新验证。

2026-09-13 / EXP-001/007/185/299取料与返程解耦保存（`validated`限普通保存）：返程尚未通过不应阻止保护已经核销的材料；Luna raw-49858c48在当前稳定Walk位置仅做normal save55002268，root raw-55728be4于55009994核原始/fresh保存终态、七配置、全玩家/84石墨/12钛、原J86/两网/零预建筑，accepted9/revision194。16+147保存、10+163只读审计离线及解析/三入口拒绝通过。正常保存不要求回到生产设施旁，也不证明返程完成、实际恢复、发电容量或持续供给；将位置与物料一起按owned identity保护，不移动或改动既有世界来保存。最近复验同日；下次accepted写为第十项必须整批审计，计数不因提前审计/保存归零；恢复或现场变化后重新读回。

2026-09-13 / EXP-007/057/135/299与IFX-129几何解释纠正（`validated`限数值修正，旧“只在起点最近”结论`invalidated`）：私有PowerShell的Clamp(value,0,1)在本机把动态Double投影绑定到Int32重载，0.094840445被舍入为0。必须显式使用Double参数并用真正的内部最近点反例测试；返回段正确距762为0.721942m/fraction0.094840445，距761为1.749804m，原先2.76973m只代表起点距离。该返程不能通过“单近邻起点豁免”直接执行，也不能依赖同函数先前给出的候选排序；中心距离本身仍不是碰撞体或可走保证。首段的历史“receding”解释同时撤销，正确内部投影0.164042、中心净空3.435762m，其实际成功终态不撤销。

27+95几何/25+122审计离线、三文件解析和已完成脚本execute退役拒绝通过；负例应保持原表达式类型，直接给首参数加Double会改变重载选择，不能再要求它复现旧错误。root raw-70788080于54978643重核七原始/fresh终态、4225配置/8282互返边/18详情及玩家/J86/两网，84石墨/12钛、accepted8/revision193/primary54777500保持，零新游戏写。保留旧raw错误几何字段及03ad6fe中的初步解释，显式禁止将其继续当规划依据；后续绕行要用独立可复算几何和fresh原生地水证据，尚无恢复实机通过。公开PowerShell脚本未发现该调用，Plugin/MCP和原生规则未变。最近复验同日；任何重载、参数类型、目标、起点或障碍配置变化时复验。

2026-09-13 / EXP-007/057/135/185/246/253/299取料成功前缀与实际落点复验（`validated`限五次Move和两次取料；返程`observed`未提交）：Luna raw-ca2863a8五个fresh短移正常终态54926718–54929354后，716/768两源即时分别扣84石墨/12钛，玩家相应增加，J86保持；root raw-da99d4a3于54947167独立核七原始/fresh终态、4225配置/8282互返边/18详情和完整玩家/日记/两网，accepted8/revision193/primary54777500。43+118执行器与25+118审计离线及解析/入口通过，不把导入offline的零调用摘要当整个execute零调用。

IFX-129明确区分实际落点与名义目标：候选距仓762约3.43m，正常1.5m到达容差后的实际起点距仓2.76973m，返程中心线的最近点位于起点fraction0，因而固定3m守卫在原生prepare之前拒绝。近邻中心距离只是几何证据，不等于真实碰撞体，更不是原生Move失败。后续必须绑定实际位置，并预留到达容差；对已处于近邻内的起点另行设计一次有界背离，不能用降低全局阈值、重试整路或改动成功建筑来通过。此处仅定位调用方守卫边界，尚未批准或执行背离；两笔取料已完成，返程/保存未做、十写未满，成功前缀及计数保留。复验触发：位置、方向、原生地水采样、实体几何或库存变化；最近复验同日。

2026-09-13 / EXP-007/128/246/253/299首段浅水拒绝复验（`validated`限预检负例）：raw-5f5bbd4f的21.12m预检虽原生可提交、46射线/23样本完整并标not_detected，终点仍低于水面0.33647m；聚合标签不能覆盖样本，也不能代替末端至少4m/高于水面0.1m的已有规则。原目标未commit；root raw-3e12faef于54858190核玩家/日记/边界保持，accepted1/revision181/primary54777500/J86。只将原采样中已观察干燥的上游点用于下一候选，仍须完整fresh预检，不能因几何绕开建筑就假定目标是陆地。同期IFX-115复现的JSON实体ID键类型错误在49次只读后被捕获，原生prepare0；规范键读取后已能继续完整现场核验，23+95离线含真实Int64/Int32正例及非整数/非数值/缺项/越界拒绝。最近复验同日；位置、路点、采样或现场变化均需重验，不改动已成功工厂或增加新建筑掩盖导航问题。

2026-09-13 / EXP-001/007/185/299备料保存边界复验（`validated`限普通保存）：本批十写完成并推送0d6aab1/CI34754076325成功后明确归零，再由Luna raw-6b8595e7执行唯一save；root raw-eccb4662于54786549核其原始/fresh终态、25关键配置、完整玩家/日记/两网/零预建筑，primary54777500/revision181/J86、accepted1保持。18+77保存、13+95审计离线及入口/解析通过；负例必须修改真实DTO已存在的属性，不能让夹具构造失败冒充生产守卫拒绝。新备料与新首次微晶事件的保存证据已经闭合，但restartResumeAvailable只证明受保护入口可用，不证明本批实际恢复或持续供给。下一取料需fresh位置/来源/地表预检，旧成功路点不能替代当前现场，计数不因普通保存再次归零。最近复验同日；下次恢复、玩家位置/背包变化或动作边界变化时重验。

2026-09-13 / EXP-007/137/185/188/299有限加工与真实日记水位复验（`validated`限十二处理器/十写）：Luna raw-71749ac3以r50/r53/r51三次原生手搓消耗24铁/36铜/48高纯硅、净得12处理器；root raw-4645abdf于54752107核七转移加三手搓全十终态、4225实体配置/8282互返边/31相关详情、原玩家及J86/两网。revision174→180反映真实排队/完成更新，不能假设三动作只加3。微晶首次事件在54737086真实增加J85→86，旧前缀完整、无pending/error。审计离线初版把已验证新事件后的水位仍固定85而拒绝；修正仅在原首次事件检查通过后逐条推进预期水位，不放宽前缀/身份/种类/时间或任意新增条目，32+121审计及44+77加工离线、入口/解析通过。此批没有移动/施工/保存；主档54556473未覆盖新材料，不能按旧恢复票据假定备料已保存。推送且CI成功后才结束十写冻结，下一普通保存再继续有限取料；材料加工不抵扣持续自动供给或三站实际发电。最近复验同日；新动作、存档恢复或日记变化时重验。

2026-09-13 / EXP-007/137/185/188/267/299有限近处备料复验（`validated`限七次转移）：Luna raw-728c8f5c七个独立accepted均terminal成功，root raw-8eadb6c6于54711295核原始/fresh终态、同tick双边守恒、七来源配置、完整玩家及J85/两网，accepted7/revision174不归零。整批预先核库存、78m内来源、空格/堆叠，仅用玩家真实已有数量抵扣；每个动作仍需新的现场哈希和原生计划，整批草案不是资源预约。执行器45+32、审计35+77离线及入口/解析通过。原生目录按目标输出查得微晶r53，再以输入/批量/耗时/解锁状态确认，不把未经验证的r52猜测执行成量子芯片。七笔备料并未取到范围外的84石墨/12钛，也不是三聚变落点、持续供料或新增容量证明；手搓材料加工与场地/发电验收分别核销。最近复验同日；玩家移动、库存/来源或配方变化时重验。

2026-09-13 / EXP-018/137/185/188/266/267原生满负荷及补电候选复验（`validated`限诊断和离线比较）：raw-4920e739的现有Foundry只读单机占位探针独立捕获main3原生reserved1519100/capacity843000/export0；探针4500与待接入口600分开，后者峰值缺口676700 J/t。这证明除私有公式重复预留外，还有真实全基础负荷缺口；普通瞬时满供不能替代该门，探针的占位/非可执行标记不能冒充施工预检通过。raw-46efee60原生燃料热值和配方用于比较136风机/19热电/3聚变，三聚变需36合金/30环/24纳米管/12处理器且满载额外4.5棒/min；19热电需456石墨/min。新容量在正常建成、供燃料和读回前不当作已有容量，储棒与额定值不等于持续供给；增加总发电容量可能降低热电负载比例，不能当作石墨消耗已经改善。17+62离线、解析/入口拒绝通过，尚未分配材料/批准布局。最近复验同日；现场、燃料/物流或充电上限变化时重验。

EXP-267集合投影再复验：临时离线库存摘要再将数组Count当逐项count，并写了未经原始字段确认的tick；修订为逐条items单位求和，分别测空/单/多条和排除joules_per_tick，built分页与另一次空prebuild分页分开验证snapshotId/tick。原始54565170快照的900合金1300、3955燃料864未变，没有按错误数量取料或改游戏。旧原生raw与失败/错误摘要保留，不能把修正显示当库存突然增长；下一材料分配仍必须fresh库存、距离与原生prepare。

2026-09-13 / EXP-001/007/018/188/299施工后保存及十写边界复验（`validated`限本批保存/审计）：入口预算未批准不阻止正常保存已完成支路；Luna raw-47db6696唯一save推进primary54556473/revision167，accepted9→10。root raw-264d37e7于54569947核kit6+paths3+save1原始/fresh终态、全4225配置/8282互返边/499详情/244纯带、实际设备槽、全玩家/J85和两网；无新入口、unknown或重放。审计必须按真实第十动作改为save，不能套用原“最后分拣器”草案而虚构新增实体/扣料；已核批次外位置差不放宽本次静止约束。17+45保存、23+122审计离线及解析/入口拒绝通过，推送/green CI之前不归零。该结论不含实际恢复、完整负荷容量或持续供给；复验触发是下一现场变化、恢复或十写边界。最近复验同日。

2026-09-13 / EXP-018/128/137/185/188/299保守预算拒绝与调用计数复验（`validated`限负例核销；新预算未批准）：Luna raw-b25e847a实际四次只读后，当前342446+范围预留455400+新增600+充电100000=898446 J/t超过容量843000而exit1，原生prepare/commit均0，电网仍满供。不能把导入模块的offline gameCalls0套到外层execute，也不把预算拒绝叫作原生施工失败。root raw-5a308510于54494790核当前位置/全玩家119带2高速/J85/两网和三个原端点保持，accepted9不归零；原执行入口退役、20+45离线及退役守卫通过。最近复验同日。

raw-6de71da9的完整4225配置快照及离线分项显示：1752对象范围内当前87521/满载456000，范围外当前69264，全充电节点峰值80000 J/t。旧保守公式会再次预留已经运行的范围内负载；修订时须用同一快照分项、保留全范围满载和新增600，并单独处理充电及范围外波动，不能从另一tick电网总量减去当前详情。普通build_catalog不含三座物流站；其原生powerDemandPerTick仍可读，不能因目录缺项默认零或虚构峰值，net3两站的100000/500000最大充电能力必须与当前各1000区分。最初诊断因此exit1的原始证据保留，只补做已存数据分析，没有再次执行游戏动作。该分项不批准新预算、不证明全厂满负载或持续供给；下一现场/负荷变化须重新核验，不用偶然低负载窗口绕过尚未解决的预算边界。

2026-09-13 / EXP-007/018/188/219/299真实父带续接及待完成入口复验（`validated`限31带和首次原生预检）：Luna raw-a68dcfd7完成5+23+3个NEW，第二、三段绑定前段真实尾带fresh cover，不把早期全NEW形状预览当实际父带。三次接受均terminal成功，23带施工多次观察未终态时继续同一action；root raw-89be801f于54414864核三收据、材料150→119、4225配置/8282互返边/122详情及31新带纯石墨。旧源4173朝向采用成功原生保全读回，原槽/供料保持，accepted9未归零；33+12执行器、15+45审计及入口/解析通过。最近复验同日。零重放不只是处理失败动作，也包括等待中的正常无人机施工；当前带尾到货不等于消费者或持续燃料完成。

raw-4d29d9bd在原生调用前发现私有检查错误：电塔3080的powerNetworkId为null，与当前consumer/generator专属填写路径一致。复用包内已有“null不是断连证据”规则，不将缺字段当0、凭空判网或新增通用字段。raw-26cc2cd4随后首次原生预检4223→3058槽−1→3/过滤1109/一高速通过，既有塔几何覆盖6.749m；完工后仍必须验证真实受电和持槽。698238 J/t限域启动投影不等于全厂满负载通过，24石墨/min额定消费也不等于当前实际或足够消纳30/min副产物。复验触发为原生现场、库存、端点、供电或运行负荷变化；下一次施工使用新token，当前预检、primary53983928与安装态64/1复验均不能冒充入口建成、保存恢复或最终0.4包。

2026-09-13 / EXP-007/018/039/185/299有限备料与批次外位置变化复验（`validated`限六终态及当前现场；IFX-128）：Luna raw-ebbe57df三转移/三手搓得到两高速，全部terminal成功/exit0；root raw-b34a2c0c于54366418独立核原始/fresh六终态、完整4194配置/8220互返边/12详情、全玩家物料/inc、两网和durable J85，accepted6/revision160，背包150带/2高速，未新建或保存。最近复验同日。此为有限启动材料，不是以手搓冒充持续生产；同源CI34748981272明确success后才执行，原startup_failure不因补跑成功改写。

详情采样是子集，不是完整身份目录：3080不在原445详情中但在4194实体快照中，调用方须先检查精确基线存在，再用完整快照比较通用配置；可选详情须与其一致，不能用null、空配置或跳过对象替代。28+60离线覆盖真实缺详情正例及缺对象/null/错误身份/详情冲突负例，原raw-9c0b5a50的exit1保留。审计期间另发现批次外位置变化7.2943m，必须先停止旧位置假设；经两次独立观察稳定Walk/800MJ、原六动作无Move、物料和完整工厂不变后，以当前位置fresh read/hash继续。来源未观察到，不声称人工移动或放宽单次施工位置守卫。raw-afc487cc的汇总字段错误只从已落盘DTO重算，不能把观察/汇总失败解释成动作失败、重放整批或丢掉accepted计数。下次现场/会话变化或施工前重新核验；本次非十写不归零，也不证明持续燃料或保存恢复。

2026-09-13 / EXP-007/018/039/185/299失败计数与可达材料替代复验（`validated`限本批十写）：Luna raw-f7f6500d只原地从1511取两铁，54222829终态成功/exit0。root raw-503688f7于54233347核十个不同accepted的原始/fresh终态（九成功及一次position_stalled）、全4194实体/8220互返边/445详情、213条纯料带、设备实槽、两网和完整玩家/J85；正常停止的失败Move仍计一写，其itemDeltas为空且零重放。revision151/primary53983928保持，本窗口净量仅高速−6/铁+2，19+75审计离线与解析/3入口拒绝通过。最近复验同日；推送/CI前不归零，不把审计通过等同供给或新材料已保存。

raw-168d58e7进一步证明无需继续脱困移动：当前铁、铜、电机源距离35.24/73.26/67.40m，额外两铁/一铜/一电机三份原生预检都通过。按当前r50产两电路、r85产两普通、r88产两高速的精确直接材料预算，可绕开仍在范围外的电路仓；原方案“之后必须为电路另找路点”由此取消。只用于有限施工备料，不是用长期手搓替代自动生产；三次转移/三次手搓及最终分拣器尚未执行。已有实物和新位置应重算整个小配方链的可达成本，不能把先前起点不成立的材料替代永久排除，也不能反复撞一个已不再必要的业务路点。

2026-09-13 / EXP-039/053/061/066/299历史Walk目标不保证全程通行复验（`validated`限当前停滞与零重放）：raw-7a84768c的27.44m正常Move在完整干地预览后仍于54192378返回position_stalled；180tick不足0.75m且余距6.9747m，结构化禁止同目标重试，未触发unknown或quarantine。root raw-8d1f447a于54198618核同一原始/fresh终态、全玩家零物品差、Walk/速度0/800MJ、durable J85/两网和healthy revision150。该失败计入accepted9，不能因脚本exit1、后续业务未执行或仍满能量而丢掉接受计数；primary53983928未变。最近复验同日。

先重算原业务实际范围：停下后铁与电机分别35.24/67.40m，电路仍81.80m，实际范围80m，不能把一个源可达外推到整套材料。现只批准不再移动、fresh预检取两铁到十写审计，随后为电路另选不同有界接近点；旧目标及原执行入口不再运行。后缀11+64离线、两脚本解析与4入口拒绝通过，实机取料尚待。表面not_detected、历史落点和附近对象身份复读都不是碰撞无障碍保证；使用已在范围内的业务动作，不为达到建筑/路点中心重复移动，不改watchdog或能量规则。

2026-09-13 / EXP-007/188/219/299独立支路条件预算复验（`validated`限三段只读预检）：Luna raw-e88c5aab只完成原raw-fb060281之后未做的两段形状，exit0；root核三份真实原生计划及12端点前后详情、原玩家/J85/两网，末读54129830、accepted8/revision149/primary53983928不变。51项私有离线及21项action-client检查通过；IFX-127的原exit1与成功首段同时保留，不因后缀成功重写原始结果。最近复验同日。

源cover的5NEW与两个全NEW空地预览24/4不能直接相加作为施工扣料：后两段共享未来父带，条件预算为5+23+3=31个唯一NEW。空地预检不证明未来实际父带的cover、旋转及货物保持；必须父带真实存在后fresh预检和逐段核销。3058/3080复用不代表额定消费必然兑现，最后分拣器仍缺料/待真实槽位检查；热电即使达到额定24石墨/min，也不足独自消纳新单元理论30/min，仍须用真实需求、物流和独立产量窗口证明最终配平，不能以已通过形状预检或消费者数量替代。

2026-09-13 / EXP-007/188/219/299源复用路径计数复验（`validated`限原生首段与offline修正；IFX-127）：raw-fb060281的4173续接已原生通过，公开plannedPath恰为5NEW，旧源另在plannedBeltPath绑定，不在路径或材料里再次计数。调用方原32合成检查把同一错误假设写进fixture，不能替代真实协议样本；应复用已有Assert-FDPath，并测试原5点响应与插入旧源的负例。34+12原断言/继承与5项后缀、解析/3入口拒绝通过，旧完整入口退役，只准补两段未做预检，不重放首段或把原exit1改写为成功。最近复验同日。

root raw-0427d825于54105240核六相关详情/完整玩家/J85/两网/零预建筑及healthy revision149，accepted8仍保持。原生输入正确被私有后置检查拒绝不是原生施工失败；源码/协议已有的NEW语义不能凭合成测试重新解释。独立石墨去路候选保留旧3375/3377和环供料，31NEW/一高速只是有限几何材料草案；最后原生槽、当前缺料、负载决定的热电消费和完整净供给均须另验，不把闲置热电接料等同全量副产物配平。

2026-09-13 / EXP-007/128/137/196/208/299燃料分钟窗口与瓶颈交替复验（`validated`限三窗诊断）：raw-4f8de11c于54035917–54046800取得三个独立3600tick窗，棒2/2/0、重氢15/15/15件每分钟；单仓730→733不能替代稳定非零产率。末次氢仓与中转仓为空，碰撞机输入氢1，棒机合金2/环2但重氢17，未满足配方20件；环已有石墨且输出10，先前环缺石墨的处置不能直接重放。新三炼油机四组详情均输出满、满供停止，下一只重设计新增石墨去路，不继续拆旧成功消费端。最近复验同日；root核91详情/四次供电、原玩家/J85与healthy revision149，accepted8保持。

19+23离线、解析/入口守卫及一次exit0观察通过。Governor同源系列可保留更早的库存观察起点：本次首窗selectedBufferItemDelta不是本次分钟库存差，须读其明示起止tick；本次另存真实前后详情，不混成同tick守恒。50/36/48间隙不补齐，7.5棒/min仍只是历史额定诊断参考，非准备目标。新增热电消费也取决于实际负荷，不能按额定发电直接保证固定石墨消耗，或因其当前可发电就核销全量副产物平衡、持续燃料及恢复门。

2026-09-13 / EXP-007/037/185/299最后接料后的覆盖保存复验（`validated`限正常保存）：raw-e22511e5唯一save11b37f9f成功覆盖53983928；root raw-8468989e于53989329核原始/fresh终态、八个既有源口/机器/出口、原完整玩家、durable J85、两网与零预建筑，最后源4194配置保持。14+12离线、解析/入口守卫及本次实机保存通过，accepted8/revision149继续累计。最近复验同日。

分阶段施工时，“空载单元已保存”不能替代最后接料后的覆盖保存；现在只关闭最后源口未覆盖的缺口。可用的protected resume票据不等于实际恢复通过，八详情定点审计不等于全厂审计，保存也不证明副产物平衡。下一采用三个独立3600tick原生窗口诊断燃料供需，保留未测间隙、在制/在途与前后库存边界，不把历史额定参考7.5棒/min当作已声明或达到的准备目标；持续供给及重启恢复仍须分别实证。

2026-09-13 / EXP-007/128/137/196/208/299石墨共享出口与采样边界复验（`validated`限三窗观察）：raw-7a9b5ad4在53944591–53946731内取得三个不重叠600tick窗，六次详情显示新3964/3965/3966输出40油/20石墨/20石墨且满电停止，4191持石墨1/进度0；汇流前3374和接入点3375均3件，后3376最后降为2。这支持新口受回压的当前诊断，不证明全带永久静止、合流永不公平或新机逐tick零产。3059–3063旧火电都有真实石墨发电，存在消费端不等于新模块已分到释放空间。最近复验同日；root核132详情/原玩家/J85/两网及healthy revision148，accepted7保持。

全行星三窗棒产/耗均0/0，单仓3955却699→700，最后后置详情3403从在制6817500回到0；窗口之外完成/在途到货不能归入已关闭的原生窗口。石墨产/耗4/3、5/6、4/5及重氢0/0、5/20、5/0全部保留，不用库存增长或全局其他生产者替新单元证明持续产出。原生1800tick测量与138/189/152tick间隙分别记录，后置详情也不当同tick快照。IFX-118固定后缀实际exit0，16+25离线覆盖得到本次有限实机正例；普通保存另有14+12离线/入口检查但尚未执行，保存恢复、持续燃料及全厂准备仍待。

2026-09-13 / EXP-007/014/299只读失败的原始记录核销复验（`validated`限证据边界，IFX-118再现）：raw-a83dc72c的28个live只读响应存在，最后一次53902763/3375成功；之后OrderedDictionary整数位置索引越界，再被catch未初始化rows错误遮盖。控制台只显示继承offline测试不证明零live调用；必须检查受保护原始响应和具体执行点。root raw-1ff768fb于53920301复验原玩家/J85/两网及healthy revision148，确为零写、零生产窗口、accepted7不变，不能将读取失败解释成原油入口失败或重放源口。最近复验同日。

后缀仅补缺失的三个独立600tick窗口，用普通Hashtable的统一Int32键或对象数组、既有完整session/owned/健康边界与Rfo-Window，生成字段只取实际DTO；原始错误直接保留，已取得的每个响应先持久化。16+25离线、解析/入口守卫通过，后缀实机仍待；不声称所有历史临时收集器已整改，也不把后缀重读当前动态状态称为重新执行游戏动作。固定库存只聚合同一3955仓格，石墨带不得相邻加总，采样缺口与实际供需仍分别记录。

2026-09-13 / EXP-007/014/037/185/299原油源正常接入复验（`validated`限施工与身份/货物审计）：raw-331f6e9c完成唯一源4194，53818970原生成功且2012精确1→0；root raw-a48220b5于53864691核4194实体/8220互返边/433详情、全部213物料带及原玩家/J85/两网。最后源口实际两格、真实belt槽双向持有，新旧配置保全；680904 J/t投料前有限启动锁定值仍成立，不重复加算启动后的已启用负载。最近复验同日；尚未覆盖保存/恢复，accepted7不归零。

施工正确不代表副产物平衡：三台已真实投料，但本次快照显示油输出40、两路石墨输出各20且三机不工作，氢的75带段为空。即使出口拓扑和额定带宽正确，共享去路的实际消耗与合流公平性仍须独立窗口证明；当前不称持续净氢45/min或燃料就绪。私有审计遗漏既有EXP-014规则，空带统计在离线阶段访问不存在Sum而失败、零游戏调用；仅修为初始化零并逐项合计同一带段，新增空/非空、错料、总数不符、缺项及负值正反例，39+91离线与解析/入口守卫、实际审计通过。不能加总相邻段货物，不能把调用方失败解释成源口施工失败或据此重放。

2026-09-13 / EXP-007/037/185/299原油源有限启动预算复验（`validated`限接源前预检）：raw-7cd4906e于53758982以4193对象的已审计配置和189 fresh详情，对拟加2803→3986做显式下游可达分析，含1719对象/158消费者；无隐式物流塔跨界，仍不是所有其他独立产线的因果上界。可达设备原生目录/当次值保守合计455400 J/t，不再仅按三机/十五分拣器及一对撞机的257000预算；再加原实耗160568、拟建高速600与充电余量100000，共716568，当前843000容量余126432。最近复验同日。

14已建高速按实际跨度容纳各声明流量，源口原生预检接受、整份物料仍需一高速；理论净氢45/石墨30/油0每分钟来自当前两配方与机器速度，47件油井缓冲和正资源节点不是供给速率证明。42+25只读预算及六个配方计算正负例、24+67单源执行器离线通过，包含投料前后功率门的差异：先锁定启动总需求上界，投料后将实际需求/容量与该锁定值比较，不能把同一已启用负荷再次加到上涨的实耗。源分拣器可已正常持有原油，不能套用此前空载出口的“有货即失败”断言；仍严格核物品、双向真实槽和原库存。实际投料、持续供给、公平分流及保存恢复均另验，不归零accepted6，不把有限启动预算替代完整Foundry基础负载门。

2026-09-13 / EXP-007/037/185/299炼油空载出口与跨线桥接复验（`validated`限五固定连接）：raw-ef997366正常建成4189–4193，G两段桥接及既有G出口、H出口和空原油入机边均按fresh精确过滤/实槽核验；原油源2803未改变。root raw-519df7c3于53722809独立核五build加一save、4193实体/8216互返边/432详情和实际机器端持槽，2012精确6→1，原九高速/213带及完整玩家/J85/两网保持，普通保存53711696覆盖全部14高速。三件实际两格、两件一格，均满供但无货；最近复验同日。

55+95执行器、22+150审计离线及解析/入口拒绝通过。输出先接、唯一原料源最后留空，使这六笔仍能严格证明新增线路空载与旧货物线路未被反向取料；它不保证接源后的公平分流或持续产量，也不替代完整基础负载预算。accepted6继续累计，不因独立阶段提交归零。下一必须把新物料可达的旧设备也纳入启动功率评估，并核原生输送跨度/额定能力、实际原油供给与原库存；既有257000 J/t余量不能直接泛化成整个可达下游的上界。保存恢复与持续供给另验，不更改完整0.4门。

2026-09-13 / EXP-007/037/185/299炼油内部九连接及覆盖保存复验（`validated`限空载施工）：raw-ac0c1cb0按已选九对端点逐个fresh原生预检、正常扣一高速分拣器与无人机施工，唯一新对象4180–4188，最后普通保存53664451。root raw-7e61f595于53680665核九build加一save、4188实体/8196互返边/427详情及完整玩家/J85/两网；2012精确15→6，原槽/旧边无覆盖，机器端和belt虚拟端的实际槽均反查持有对应新实体。三件实际两格、六件一格，2012原生progressRequired为200000/100000；三格只是既有离线可接受范围，不冒充本批实测。最近复验同日。

83+12执行器、30+95独立审计离线及解析/入口拒绝通过，所有十笔known successful且保存覆盖三台配方；没有以调用方超时换键重试。213带、三机及九分拣器仍无货，当前满供和257000 J/t启动余量不抵扣满基础负载或持续制氢/燃料门，也未实际重启。只有本结论push/CI通过后关闭十写窗口；后续五个空载输出侧连接可以另批fresh施工并保存，55+95离线检查明确排除原油源2803，原油最后接入前仍须独立核全链供需、运输及真实功率，不能用15份早期独立预检当联合预留。

2026-09-13 / EXP-007/037/185/299配方后缀与跨执行器十写核销复验（`validated`限三机配置，IFX-126）：raw-7519f0dc只执行3965/3966的r58，实际两步分别+2revision，首台3964/r16的53550757原成功终态保持。root raw-48839c6c在53614437把六次带施工、一保存、原脚本的一次配置和修正后缀的两次配置共同计为十个唯一接受，核4179实体/8160互返边/418详情、零配置扣料及原玩家/J85/两网。脚本分段、后置断言失败或更换执行器均不能丢失先前accepted计数、重做已成功前缀或把原exit1改称成功；最近复验同日。

12+54后缀、17+54审计离线及解析/入口拒绝通过，IFX-126私有修正已有实机正例。三台仍空载、213带无货，primary53515875不含此次配方；下一内部连接可按既有原生预检逐个fresh施工并覆盖保存，但不能提前接原油、按理论净氢45/min宣称实际产量，或把增量启动功率余量代替全厂基础负载/持续燃料预算。十写账本push/CI通过前不归零，后续15个分拣器的实际槽、货物、供电及输送和恢复分别复验。

2026-09-13 / EXP-007/037/299动作完成与调用方版本计数分层复验（`validated`限首台配置核销；后缀`observed`，IFX-126）：3964/r16的原action在53550757已成功，私有后置+1假设却令脚本exit1；root raw-77fb39be证明完整玩家零变化、三机16/0/0、27空槽、durable J85/两网及实际revision112。revision不是已接受动作计数；当前配置由Complete和调度分别递增，必须读当前真实值并核对具体路径，不能把save的+1推广给其他动作，或因调用方断言失败重放原生成功动作。最近复验同日。

原完整入口已永久退役，两台明确未提交后缀的12专项+54继承离线、解析/3入口拒绝通过，包含真实110→112和0/1/3增量拒绝；不宣称原42检查覆盖了遗漏的计数条件。accepted8保留，后缀实机及十写核销另验。raw-17cf8ff2十五对过滤器/真实槽及电塔覆盖逐对原生通过，只是预检；未来每个新分拣器仍可能改变占位或设备空槽，按fresh状态核对后续计划，原料入口最后接通，不能把空机配置、逐对预览或正常保存等同持续供给。

2026-09-13 / EXP-007/037/076/185/299已完成前缀保全、尾段施工及覆盖保存复验（`validated`限46带和七写阶段）：raw-b39f11d1按固定H-E/H-F/G-A/G-B/G-C/G-D构建46个NEW格，先前167格仅作真实父路径/邻域证据，未按旧全NEW预览重建共享格。196→150连续扣料，六个原action均正常terminal，普通save53515875覆盖全部213带；37+53执行器离线及解析/入口拒绝通过。最近复验同日。

root raw-d47f1c36核七个原始/fresh终态、4179实体/8160互返边/418详情和213空带，原68格氢路径的4124朝向变化必须匹配正常whole-path保全读回，其余配置/旧边、玩家/J85/两网保持。41+90审计离线及解析/3入口拒绝通过；七写不能因阶段提交或保存归零，下一三笔配置后执行十写审计。raw-a284a190的r16/r58预检只证明当前空设备可选配方，预算中的input/output不是配置动作扣料；真正配置仍须fresh完整设备哈希、原生终态、零玩家物料变化和未投料读回，不能以保存能力代替实际恢复或以空带施工代替持续产量。

2026-09-13 / EXP-007/037/076/185/299长路径分段观察、原生源保全及十写复验（`validated`限本次八路径147带）：raw-3ef0a69e在既有20带上仅续建明确八个后缀，八笔正常action全部有独立终态及逐格实际身份，343→196扣料连续；观察按30秒有界分段、最多十段，只等待同一action，不因一次观察届满或返回对象ID乱序而重放。对象归属按唯一实际位置恢复路径顺序，不能按ID数值排序推断拓扑。首次保护保存53394663和最终覆盖保存53451154共同计入十写，计数只在独立审计、账本推送及CI成功后关闭。最近复验同日。

root raw-b6be7846核4133实体/8072互返边/341详情及十个原始/fresh终态；原3986对象只允许3967的新增出口和由whole_path_native_rotation_v1证明、成功后复读相同的源路径朝向，不能泛化为忽略rotation。147新增及原20带均观测为空，三机仍未配置/接料，原完整玩家/J85/两网保持。41+12执行器、52+53独立审计离线及解析/3入口拒绝通过；安装raw-6f1c6135的228哈希与64/1/70562握手保持同批且正常退出。此结论不证明持续制氢、实际恢复或全厂满基载通过；新增原料/输出、配方、余46带、分拣器、重启和最终验收时重新复核，禁止据已有全NEW几何引用重用旧token或重建完成前缀。

2026-09-13 / EXP-007/037/185/299多对象正常施工的观察届满与终态续读复验（`validated`限同一action、20带及十写核销）：raw-df73dfb6在53343044仅接受一次crude-A，60秒调用方等待结束时游戏动作仍非terminal，执行器退出而没有撤销或重试。root raw-5dbb466e读到同一action在53350181自然成功（7137tick、3只正常施工无人机），raw-c4e7b26e补齐20真实对象和原背包/J/供电。观察超时不是action_failed、outcome_unknown或“施工未发生”；后续有界执行器应继续分段等待同一action，不把客户端等待窗口当作整个施工期限，也不能因脚本退出创建新幂等键重做。最近复验同日；适用于已有正常长施工，不放宽卡路/断能看门狗或接受不明结果。

raw-462b492c在53363086完成严格十写复核：4塔、保存、3机、保存和20带，全部有唯一接受/终态，连续净扣2201×4/2308×3/2001×20，3986实体/7782互返边/183详情、实际旧槽/纯料与J85保持；新20带只有19条内部邻接的双向表达，两个自由端不容额外边。23+12施工、17+53审计离线及解析/入口拒绝通过。当前primary53310976不含新增20带；下一保存后，旧尾3967只能走fresh non-removing cover，不能重用独立全NEW预览令牌或重放整段。十写结论push/CI前保持冻结；无持续原油供给、产率或完整Foundry声明。

2026-09-13 / EXP-053/062/076/185/299空载设备施工与功率证据分层复验（`validated`限三机成本/身份/空槽/保存）：Luna raw-1a4c02aa正常建成3964–3966并保存53310976；三份fresh原生计划绑定原位置/yaw0/90/90，2308连续3→2→1→0，三个recipe0空机均有9个实际空闲端口及主网3满供，既有供电节点/发电机不变、消费者恰增3。root raw-ade0d55d在53317089核九笔原始/fresh终态、全3966实体/7744互返边、35详情、原玩家/J85/两网，accepted9保留；25+12施工、16+37独立审计离线及解析/入口拒绝、21项action-client检查通过。最近复验同日，关联既有bootstrap与增量供电经验。

raw-a39142e8的Foundry单机只读评估明确区分原生放置Ok、覆盖通过与full_base_load_generation_deficit：三个独立评估共享同一1462100J/t既有基础负载及843000J/t实际容量，不能相加三份缺口。三机和15高速新增57000J/t后整组满基载仍缺676100J/t；普通空机bootstrap保留257000J/t实际增量启动余量，不把目前供电比1当作全厂满载或持续燃料证明，不放宽完整Foundry组合计划门禁。复验触发为接料/配方/新增消费者、燃料下降、恢复或最终验收；后续sorter仍须绑定真实端口及fresh原生检查，九写后下一accepted必须做十写审计。本轮一条只读结果格式化访问不存在siteEvaluation后停止，先前读回已落盘、零写入，改为已观察到的site字段读取，未重放游戏动作。

2026-09-13 / EXP-053/076/185/299供电未完成后缀与覆盖边界复验（`validated`限八塔施工/保存）：上一十写结论ff9a64c及CI成功后，raw-30e23bfd仅施工原四个剩余落点3960–3963并保存53223312，不重放前四塔；本批电塔6→2、accepted5/revision70/J85。每个新节点接入已建主网锚，实际网3仅nodeCount增加，未合并网络或改变消费/发电成员。13+37离线、解析/入口拒绝通过；私有只读几何命令曾因for直接接管道在解析阶段终止、零导入/Bridge，改为先收集循环输出再排序后才完成复核。

root raw-b1d0a828在53233502独立核五笔原始/fresh终态、原完整背包/J、3963实体/7744互返边和16详情，八塔真实位置给三机预定中心留6.240/5.031/5.031m至10.5m覆盖的余量。最近复验同日。该几何不是未建机器/分拣器的真实powerNetworkId或产能证明，后续仍逐对象fresh预检、实际持槽/连接/供电及持续物流复读；保存成功也不等于实际恢复通过。五写计数保留，下一五写后再次完整审计，已建八塔不重做。

2026-09-13 / EXP-053/076/185/299供电前缀、实体池复用和十写复验（`validated`限普通施工/保存/保全）：raw-a61a3d4c四个2201分别正常建成3402、3067、3958、3959，保存53180451。身份判断基于原完整3955实体集合及精确新落点，不要求新ID大于历史最大值；3402/3067在施工前已不存在，可以原生复用，不得据数字相同宣称覆盖或据数字较新免除唯一读回。四塔仅令主网节点+4，不新增消费/发电成员，背包逐次10→9→8→7→6。

root raw-8249de26于53196791独立核十笔唯一接受及原始/fresh终态，包含已知失败Move而非只数成功；全3959实体/7744互返边/148详情、真实设备持槽、旧纯料通路、完整玩家/J85/两网和零预建筑保持，accepted10/revision61/primary53180451。25+12执行器及17+40审计离线、解析/入口拒绝通过；最近复验同日。启动预算57000J/t新增满工作负载加200000J/t碰撞机余量只用于本次有限支持单元，不能替代Foundry整厂满基载或持续燃料证明；仍无生产/恢复结论。计数待账本推送/CI成功才关闭，已建四塔不重做。

2026-09-13 / EXP-053/076/185原生吸附路径与既有出口保全复验（`validated`限预检）：固定原油新带头改到旧2803出口东侧，避开既有竖带；只修未施工草案，旧线路保持。raw-f3a7f8b7的第八电塔和15段全NEW带形状全部prepare通过；root核full_path_stage1、sourceBindingMode=none、逐段NEW计数/扣料回显及吸附后213唯一点，重合仅限原声明的父路径尾/子路径头。这既不授权覆盖旧出料带，也不提前提供不存在实体的端点身份。

13+24离线、解析/3入口拒绝通过；root raw-4e9ab32d在53142389复核六个旧对象配置、原玩家/J85、两网和零预建筑，accepted5/revision52/primary52877974保持。最近复验同日。整图几何预算不等于可执行整图：后续parent须先正常施工再fresh source-cover，设备须真实槽位/双向连接预检，整组供电和持续供料/副产物消费仍须分别证明；不能沿用这些独立全NEW形状token重放已建共享点。

2026-09-13 / EXP-018/246/253/299有界绕行与干地后缀复验（`validated`限本次正常Move）：raw-7d6dc7fd的南、西段成功，第三段只被调用方全干地策略拦在commit之前，没有第三次接受。root复核AGENTS原有解释：中间浅水和末端浅水必须区分；本候选32点全部有效、全长30.141m、中间深度至0.261m、实际采样间距累计末端18.473m连续干地≥0.1m，可在充足能量、1.5m容差和原生watchdog下作一次有限尝试。仅修改私有明确后缀执行器，不放宽Plugin规则、不依据shoreRisk标签或旧token决定放行。

25+12离线、解析/3入口拒绝通过；raw-a0d7d2f2的唯一北向Move b267b900于53106960成功，root raw-af1fbc83在53117800核南/西/北原始及fresh终态、原全背包/J85、两网和真实Walk0/800MJ。accepted5/revision52、primary52877974保持，原失败目标与成功前缀均未重放，无施工/保存。最近复验同日；此证据不证明通用无碰撞寻路，仍每段fresh、失败停止，也不替代接下来材料/场地/供电/连接与持续产率的业务预检。

2026-09-13 / EXP-018/246/253/299有界脱困及浅水解释复验（`validated`限单次恢复）：raw-b36c053c的首个南向约4m正交Move7323b0e0于53020079成功，root raw-6b3d1e79于53027452核原始/fresh终态、稳定Walk/速度0、原背包/J85和两网，accepted2/revision46，未保存或重做原失败目标。12+24执行器离线/解析/入口拒绝通过。允许短移成功不等于整个厂区路径可达；仍由root确定后续有限目标，每段重新prepare并核终态。

raw-6b3d1e79的另一个向东候选shoreRisk=not_detected，但实际groundBelowWaterMetres后段为正0.095–0.302m，缺少有效干地后缀，未提交；与先前明确ground/water命中不矛盾，风险分类不是Walk保证。替代南向首段raw-85eac6c3有11点/9.296m全段≥0.1m干地余量，仅支持下一次明确候选；其后西/北转向必须从实际位置分别复读，不根据投影先签未来token。最近复验同日，机器、供电/输送和持续燃料的未验门保持。

2026-09-13 / EXP-018/053/246/253/299原生距离、地表与障碍边界复验（`validated`限明确负例）：raw-ca6d8d26的三炼油机和七电塔原生通过，第八因OutOfReach停止；两未建裂解机yaw90及217唯一格/15分拣器/8电塔只是有界整体候选，未建parent、真实槽和联合占位仍另验。root raw-f56c82e8独立核验，没有把前七正例外推到第八或完整布局。最初内联预检对预期负例throw未捕获而停止，零游戏调用；文件化预检12+12离线明确捕获拒绝后才进行真实prepare，没有重跑游戏动作。

raw-70e2cf1f的28.413m地表30点全部命中干地，仍不能证明没有建筑碰撞；Move664c9d1a在52997350由position_stalled/180tick结构化终止、剩25.559m、禁同目标重试。root raw-af76b543/f06a98fd核原始/fresh终态、完整玩家/J85与healthy revision44，失败计accepted1且无隔离/物料变化；附近仓/化工厂分别2.282/3.523m，不能仅据距离宣称精确碰撞体。首个向南约4m正交候选使其距离增至5.818/6.404m，原生及5点干地通过，但风机3048仍距目标3.198m，保持单次有界尝试和终态复读，不宣称无碰撞保证。最近复验同日；原失败目标不再提交，连续同阶段两次失败仍停交root。

2026-09-13 / EXP-007/018/053/076/185完整备料及十写复验（`validated`限原生材料/保存/保全）：raw-8d7dd0ae完成四转移、四手搓及普通保存52877974；加本批首笔保存共十写。root raw-93086b98于52921227独立核十个原始/fresh成功终态、唯一接受、精确源仓同tick变化、连续物品收据、完整背包及原durable J85。净铁−33/2011−1/2012+14/2201+10，现铁与2011均0、2012共15、2201共10、2001共363、2308共3；原33余铁已消耗，gross材料或历史余额不能当最终库存。

全3955实体/7744互返连接/144详情、真实端点持槽、既有纯料输送、两网采样满供及零预建筑保持；本批不含施工、移动或恢复。33+56执行器与16+24独立审计离线、解析/3入口拒绝通过，最近复验同日。计数冻结到结论推送/CI通过后再关闭；后续新写仍fresh prepare。已备齐输送/供电材料不证明精确路线、端口、功率覆盖、副产物去路或持续产率，完整三级Foundry和其他0.4门继续保留。

2026-09-13 / EXP-007/018/053备料保存及业务距离复验（`validated`限保存/取料预检）：raw-1e10176d的唯一save0dc69fee覆盖52851248，root raw-d4944b2b在52856859核原始/fresh终态、全玩家、原durable J85和两网，accepted1/revision30，未实际恢复。四个fresh取料prepare分别为26电路13、727电机7、562磁铁10及铜5；实际距离74.289/59.823/66.095m仍在当前业务范围且均原生通过，不为精确走到仓中心新增Move。

5+12保存离线及解析/入口拒绝通过，最近复验同日。预检后的库存/状态哈希只属于当次读取，每次后续取料仍重新读取和prepare；先前四个token不能跨写复用。剩余既定手搓只生成10线圈、13普通及14高速分拣器、10电塔，原铁33和一件2011按预算消耗；三炼油机及360带不能重新制作。此为准备材料而非生产线完成，场地/端口/上游/副产物/实际功率与持续产出仍各自待验。

2026-09-13 / EXP-007/018/053/076/185本批十写及360带备料复验（`validated`限材料/全厂保全）：raw-e33bfad9的取393铁→制120齿轮→制360带在52827720完成，齿轮是已消耗中间品，实际净带+360/铁+33。root raw-e2342f1d于52836363独立核十笔唯一接受、原始与fresh已知成功终态、逐物品连续收据和全背包；相对批次开始净钢−30/石材−30/玻璃−54/线圈−72/电路板−18/炼油机+3/高速分拣器+1/铁+33/带+360均有原生证据。

3955全实体/7744互返边、144详情/设备持槽、77段合金及燃料/氢/石墨纯料路径、原J85和两网采样满供保持，唯一变更仍是已批准3402回收；没有新预建筑、瞬建或新供给验收。24+32执行器与12+12审计离线、解析/入口拒绝通过，最近复验同日。accepted10/revision29在本条推送和CI通过前不归零；旧主档52683739不覆盖新备料，下一普通保存后再推进剩余既定材料。备用物品数量不是精确布带成本、功率覆盖、模块产量或最终版本完成证据。

2026-09-13 / EXP-007/018/053/185输送备料预算复验（`validated`限配方/预算和取铁预检）：raw-212d1562/5a8c84cb确认gear配方5、belt84、pole8、basic-sorter85、fast-sorter88、coil6的原生输入输出。新增360带/14高速分拣器/10电塔的保守上限需393铁、13电路板、7电机、10磁铁、5铜，并消耗已有一件2011；已持三炼油机不重造，已有2012一件留余。三机及14工作分拣器额定56400 J/t，另预留完整碰撞机200000；当前主网余量604146只是当次启动预算，不替代完整Foundry基础负载预检或输送带/供电拓扑验证。

raw-fe83893b补读九热电及相关入口：183实际烧氢，3059–3063实际烧石墨；3069–3072虽仍过滤氢却全部Picking/空手，因此不得只按保留的filter认定它们当次正在耗氢，也不因此继续拆除。净制氢扩产必须保留石墨需求及输出背压检查，材料上限不等于整条线路已可执行。24+32离线、解析/3入口守卫通过，最近复验同日；首个有限后缀仅1511取393铁、r5×120、r84×120，accepted7→10冻结后审计。全部终态和原生扣料逐笔核对，任何失败不重放成功前缀；完整场地、持续供给和0.4发行门保持待验。

2026-09-13 / EXP-007/018/035/185分流后配对窗口复验（`validated`限所读窗口）：raw-5b46f646六个不重叠600tick窗累计石墨48/40、氢127/125、重氢10/40、磁场环2/2、燃料棒2/0；root raw-8bae582d在52735858核288原始详情、11fresh详情、原玩家/J85/供电，accepted7/revision24/primary52683739保持。棒仓样本288→290而重氢仓275→269，库存边界跨未测间隙，不能把这两个差值当严格同窗产消平衡，也不能以棒增产核销净供氢缺口。

全部12个环详情采样输入石墨零且不工作，并不推翻原生窗口累计两次产环；固定两段带占用、分拣器Picking或空缓冲也不能单点证明线路完全断开/偏移错误。汇总可空的belt专用字段时先按对象分支，不能对非belt的null直接取子字段；本次首次摘要在新读取前失败，修正后复核，未重放观察或游戏动作。7+25离线检查通过，最近复验同日；保持间隙143/142/128/142/104/135、无持续供应声明。下一依完整净供给与串联容量预算推进已备三炼油机，不据单点空料继续拆第二个热电入口；持续燃料、完整三级Foundry及最终包门均保留。

2026-09-13 / EXP-007/018/035/185石墨分流纠正保存复验（`validated`限普通保存）：raw-ea0bb03e的唯一save e63bc2f1持久化52683739；root raw-9d46b90e于52688005核原始/fresh终态、17详情和原完整玩家/J85、两网成员与采样满供，accepted7/revision24且新protected resume能力可用。没有实际重启恢复、额外供料或施工，不把保存通过自动扩张为整个分流方案效果通过。

4+12离线、解析/入口拒绝通过，最近复验同日。下一把原生产耗和精确取料位置分开核：带段有货不等于分拣器当前能取到，也不允许因某短窗无产量立即再拆热电。六独立600tick窗口及间隙必须保留，结果决定后续root方案；原三炼油机、实际持续燃料、完整三级Foundry和最终双包门不因保存而减少。

2026-09-13 / EXP-007/018/053/076/185/196分流回收实机复验（`validated`限回收与保全）：raw-ad3584ed/c46bf815在52652817仅回收2012/3402并返还一件；root raw-02b5f87f于52664793核原始/fresh唯一终态、3955实体/7744互返边/22详情、全玩家只增2012一件、原durable J85、两网只少一个消费者、零预建筑。accepted6/revision23/primary52494816；原3058/3377/3436及其余连接保留，不把正常回收当整条燃料线重建，也不把该次accepted写错计成零。

20+12私有离线/入口解析及21项Core分拣器回收测试通过，最近复验同日。只读审计的整数键字典必须用既有安全投影或明确键值比较，不能由序列化错误推断游戏动作失败、重跑原执行器。52664737虽见3428带段有一件石墨，随后环输入仍零；段级有货不等于取料点对齐、送达或持续产量。当前仅关闭此一次受控调整，普通保存、原生流量/目标生产和后续供给预算另验；不以局部断料纠正核销完整0.4或在无新证据下连续拆入口。

2026-09-13 / EXP-128/137/185/196生产优先的有限分流预检（`observed`，尚未回收）：六窗已证明环缺石墨后，root明确重新批准只回收3402，而不是让Luna盲改已经成功的热电改燃料阶段。raw-e180e9ca确认3377槽4↔3402↔3058槽3，3436仍为既有环入口；raw-492eeefb的原生回收prepare通过。此方案保留电站、主带、五个其他热电入口及所有成功燃料/氢路径，仅停止最后一处自动石墨取料，让剩料继续沿原带流向生产；不承诺释放的流量必然足够。

正常回收的核验必须包括2012返还、可能随原生回收的0或1件石墨、唯一目标消失和两邻端只去掉本条边；内部余燃料仍可继续发电，不能把当刻发电当永久容量。预留该热电全部36000 J/t损失、碰撞机200000、三炼油机48000及输送12000；52629018实际余量658558，扣后362558，fresh不满足则停止。20+12私有离线/入口解析与21项Core回收测试通过，最近复验同日；仍须实机执行、原库存守恒、输送/产量和正常保存证明，本方案不核销完整制氢/三级Foundry或全厂基础功率预算。

2026-09-13 / EXP-128/137/196/212燃料瓶颈交替复验（`validated`限六独立短窗和直接缺料）：raw-568bfa64的六600tick窗口共3600实测tick，间隙明确不计；root raw-16552d97核六原生窗口/216详情、原玩家和J85、52610402 healthy revision21，accepted5/primary52494816不变。石墨产/耗41/44、氢110/110、重氢15/0、环0/0、棒0/1是选定窗口计数，不是连续一分钟稳态。3403十二样本重氢40/合金2/环0，3404十二样本涡轮4/磁铁6/石墨0，均停机；本批不支持继续以重氢缺料解释直接停机。

上游库存回升或全厂P=C可能来自下游因另一物料停产；必须与精确目标设备输入、实际产耗和分流通路一起判读。已有裂解工作、石墨离开机器和热电有实际石墨发电，不保证位于热电下游的磁场环得到料；不能仅按拓扑可达或名义发电容量认定副产物分配完成。7+25离线/解析/入口拒绝通过，最近复验同日。后续root重设计须预算生产优先的石墨供给和剩余长期消耗，保留已通过线路；还没有新旁路施工或持续燃料通过，现场需求、分流、供电或新增三炼油机投产后再验。

2026-09-13 / EXP-007/053/076/185/188/299历史站位回程与整组预览复验（`validated`限原Move和单机/联合净空）：raw-a3231617的666aa105在52519504正常成功，包装却使用错误revision20后置常量；root raw-30bef617/0d8238da核原终态、fresh revision21、原背包/J85及Walk0，accepted5，未重放。失败Move只推进一版的旧经验不能机械套到成功Move；离线入口/计划10+12测试不证明未测试的终态版本常量。后处理失败和游戏失败必须分开，不因包装异常回档、移动重试或漏计接受。

三台2308已有后，root raw-74c1cc3d/0d8238da证明一r16/两r58各自原生Ok，三台整组库存足够；实际吸附后相互32.8345/57.1404/28.9041m，超过两半径加0.5m的23.8439m保守净空。单机预览必须额外组合检查库存总数/相互占位，不能重复花同一预制件或拼成不存在的整图token。52536304三候选仍未受电、没有完整物流/副产物方案，executable=false；本结论不证明可施工全图或持续产量。最近复验同日，后续建成对象/供电/物料/玩家位置变化必须fresh重检，原已成功线路不重做。

2026-09-13 / EXP-007/018/035/069/185/299炼油预制件保存复验（`validated`限正常备料/保存）：上一十写审计b745195及CI34728612797通过后，新批raw-be2daeb8只做三手搓与普通save，终态52492313/52493871/52494722/52494816。root raw-aa8cf67a于52500506核四个原始/fresh唯一终态、每步直接原料守恒、16详情、玩家/两网/零预建筑和原durable J85；accepted4/revision19/primary52494816。原始完整背包只增三台2308，棱镜和电浆是已消耗的中间产物，不能累加成库存或供应。

13专用+32继承离线与解析/3错误入口通过，最近复验同日。脚本导入的offline摘要可能包含被测试旧方案的acceptedBefore；实际计数必须看新执行意图、唯一commit和终态，而非把零游戏调用的自测输出算进写窗口。普通保存签发受保护恢复能力不等于已经恢复，预制件到包不等于设备建成/原生完整场地通过。后续必须带真实库存重新检查此前NotEnoughItem候选及整组物流/电力/副产物预算，不复用旧准备结果或提前宣告制氢完成。

2026-09-13 / EXP-007/018/035/076/185/299炼油备料十写复验（`validated`限材料、终态和原现场保持）：raw-d827f08a五转移/两手搓在52445674–52452054完成；root raw-f8ca46cc于52473585核本批十个唯一接受及原始/fresh终态、全3956实体/7748互返边/139详情、真实持槽、77纯合金带/燃料氢路、两网和原durable J85。原生恢复action没有完成tick，排序使用其新session的真实主档/观察水位；唯一position_stalled仍计写次数，不为全成功列表漏掉失败或伪造恢复时间。

相对恢复后背包净增30钢/30石材/18电路板/54玻璃/72线圈；石矿108、磁铁72、铜36的取入已经被两手搓消耗，口头的“取料合计”不能当剩余库存。逐动作即时before/after与完整玩家差量共同证明，零预建筑、healthy/无checkpoint保持。执行20+12、审计12+12离线及解析/入口拒绝通过，最近复验同日；accepted10/revision12在本条push/CI完成前冻结、不归零。primary52171232尚未覆盖备料；下一只继续明确未执行的棱镜、电浆、炼油预制件后缀，整图运输/供电/实际产量仍另验。

2026-09-13 / EXP-007/018/035/076/299取料后缀与递归备料复验（`validated`限转移，余料仍为计划）：raw-131259b1唯一钢转移64f2fe97在52424676同tick源792减30、玩家增30；root raw-b2bb9bdc于52429043独立核原始/fresh终态、全玩家、源配置和原J85，accepted3/revision3/primary52171232。17专用+12继承离线及解析/入口拒绝通过；未重放失败Move，未多取钢、未施工/保存。临近仓的正常运输仍会改变延后库存，守恒采用动作同tick双边证据而非两个延后快照强行相减，最近复验同日。

raw-0c12400f复验现有95/26/562的材料，原运行时配方6/11/12/15/57证明三炼油机额外需30钢、30石材、108石矿、18电路板、72磁铁和36铜；玩家原有材料另保留。玻璃输入是1005石矿，不得误把1108石材计作可转换输入；每步新prepare还须核精确配方批量、直接物料、终态与真实Journal增量，缺少旧首次事件不意味着可补造新事件。现有三来源均可达，避免为取料继续Move；整份物流/电力成本尚未包含，三预制件不是完整施工计划或持续制氢证据。配方、材料、位置、来源或session变化再验。

2026-09-13 / EXP-007/053/061/076/179移动失败后只继续已可达业务复验（`validated`限当前失败核销和fresh预检）：历史Walk点经当前附近实体复读后，Luna raw-17f04ed5的唯一Move在52379467返回position_stalled/181tick/余4.3522m，已前进51.2673m但不是成功到达。Wait封装抛错使旧脚本后续钢转移没有执行；必须读回精确原action和现场，不能重跑整个脚本或把异常当未提交。失败计accepted2，旧目标禁止重试。

root raw-8ea1855b/02226ac8独立核终态、原背包与J85、healthy边界；52408376玩家Walk/0且约798.57MJ，源仓792距58.5207m，原业务30钢fresh prepare通过。此时不执行脱困探测、不强求抵达旧目标；下一只允许新token的业务后缀，仍须终态与双边物料证据。12继承离线守卫通过，最近复验同日。历史Walk点不保证当前路线无碰撞，原生可准备也不等于到达；尚无取料/新炼油施工或持续供给正例。现场位置、拓扑、库存或session改变后再验。

2026-09-13 / EXP-128/137/196/212/246/253/299新增制氢预检与取料路线边界（`observed`，未施工/移动）：b54ea37/CI34726513364成功。raw-54f61f78复读两原油节点仍存在，原生理论合计157.4364/min而当次600tick实际24/min；不是两口井已耗尽或新采油井的必要性证明。raw-c3f557a1对明确北侧单炼油机候选返回原生NotEnoughItem、缺一2308，且未受电；机器草案要求原油30/min、产氢15和油30、960kW，不证明地形/完整原生放置或整段运输通过。后续一个等离子精炼加两个裂解单元的配方预算为三炼油机、原油30/min、净氢45/min、石墨30/min、设备2.88MW；这是待完整选址/物流/副产物消纳的设计单元，不是运行速率或就绪目标。

为取得正常建造材料，root仅预检钢仓792的两条短接近候选。raw-244ec8a1/037bb6c5虽原生可准备且末点高于水面，沿途均跨水，末端连续至少高水0.1m的长度仅2.9969m/1.9854m，未满足1.5m普通容差对应的至少4m落地余量；均未commit。raw-b847e1e0于52354661核原位置/全部物资/J85及accepted1/revision1/primary52171232不变，停止第三次相似直接跨越。下一由root重设计分段绕岸或复验历史Walk路径；不缩容差/放松末端阈值、不把not_detected浅水当陆地，也不因缺钢新增重复钢产线。最近复验同日，后续路线/材料/供电变化再验。

2026-09-13 / EXP-027/128/137/196/208/299供氢分配与分钟库存缺口复验（`validated`限独立原生窗口及消费者普查）：c164913/CI34725706387成功后，Luna raw-2ab65c4b仅执行三对3600tick只读观察，root raw-2336afd9于52297036独立核六份原生窗、63详情、三供电采样及原玩家/J85，accepted1/revision1/primary52171232保持。红糖三窗6/7/6件/min；同窗氢产107/107/105、耗108/108/106，石墨产43/43/41、耗47/47/45。制棒三窗均2件/min，但重氢产均15、耗0/20/20，制棒台重氢缓冲15→10→5；不能把两轮依赖旧重氢的产棒当持续供给。14专用+12继承离线、解析/3入口拒绝通过，最近复验同日。

红糖窗口52279651–52283250、52283395–52286994、52287145–52290744；燃料窗口52279663–52283262、52283401–52287000、52287157–52290756。每目标分别测10800tick，组内间隙及目标间6–12tick偏移不拼成连续/同tick守恒。raw-7d106923/2336afd9还核全部九座热电：六座现烧石墨；旧183仍实际烧氢，134/2516标记氢但采样发电为0，不能把零发电当不存在或按满载计实际消耗。配方消费者普查不足以覆盖燃料消耗；净制氢预算还须包括这些原生发电支路。当前源码明确基本分拣器filter=0投影为null，结合有效组件/目标可解释为未过滤；不能在纯料拓扑分析时静默删除该边，也不能跨DTO把所有null都视为零。下一只设计净增产、旧支路分流和完整运力，不重做已保存回送；后续配置/负荷/来源或恢复变化重验。

2026-09-13 / EXP-027/128/137/196/208/299恢复后缓冲活动与真实缺料复验（`validated`限三份独立窗口和诊断）：raw-c187eea3/697246f1核三个600tick native窗、138详情和原玩家/J85/三供电采样，accepted1/revision1/primary52171232不变。三窗氢总产耗都43件，但3073两次confirmed缺氢7/10、9/10；氢仓始终599–600与棒仓151→153不能反证设备缺料，也不是配平。后两窗涡轮814和磁场环3404满输出10/10，不能仅凭上游停机扩产；先结合下游需求和物流解释。12继承+13窗口离线/解析/3模式守卫通过，最近复验同日。

统计窗口为52202228–52202827、52202967–52203566、52203713–52204312，135/139/146tick间隙不计入1800tick覆盖；原生件数与每分钟速率严格乘6转换，棒0/2/0只是一轮完成时间分布。仓库存照与统计窗错时，不强行拼成整区间守恒或十分钟稳态。下一以净制氢（扣裂解回流）、原红糖需求、各段运力及长期燃料需求共同预算，不以缓冲新增/库存净增取代持续供应；后续实际线路/负荷/配方变化再验。

2026-09-13 / EXP-001/002/018/027/069/128/196/208/212/217/299新缓冲与氢回送恢复复验（`validated`限同档生命周期和结构保全）：raw-08b07cfb证明正常退出、Steam仅启动一次，不直启EXE、不重装；raw-78aba174/8f2dca63在前后两进程实际核228同源安装文件及MCP64/1/70562、正常退出和纯stdout。唯一resume-1e3d6440从52171200恢复并自动重存52171232，root raw-d89f1f23独立核全3956实体/7748边/135详情、原材料/Journal、真实设备持槽及满供采样，accepted1/revision1/J85保持。13生命周期/11恢复/14审计离线和解析/入口拒绝通过，最近复验同日。

恢复后3058/3059仍用1109实际发电，3079用1802实际发电；旧ID3068明确仍是新的储氢回送而非历史热电入口。上述发电是单次J/t样本，不是连续产率或燃料数量；恢复前供给窗口不能直接充当恢复后证据。下一保留完整净制氢、内部回流、串联运力和长期需求预算，另测原生生产窗口；不重做已恢复的成功连接，也不因恢复后新revision而清零accepted1。原生DLL/安装批次、后续施工/负荷或再次恢复变化触发复验。

2026-09-13 / EXP-018/027/069/128/196/208/212/217/299储备与燃料分流十写审计复验（`validated`限正常覆盖保存和完整批次审计）：raw-4855343a唯一save52171200覆盖最新氢回送；root raw-74169b8b于52175276核十个原始/fresh终态、十次唯一接受及即时材料链，3956实体/7748互返边/135详情、原玩家/J85/两网、真实设备槽和关键纯料物流通过。净普通分拣器+1/氢+2来自先手搓后两次原生回收，连同其余四类材料净扣减全部守恒；不是物品注入或未知增量。15保存/12审计离线、解析/3入口守卫通过，最近复验同日。

主档52171200/revision20已覆盖本批，但可用恢复能力不等于已经恢复；先保留accepted10，落账推送且CI通过后才清计数。ID3068回收后用于不同端点的新分拣器，十写证据必须保留生命周期顺序，不能以同ID覆盖历史收据。完整静态保全、满供采样、普通保存都不证明持续产率或供需配平；同档恢复、净制氢及串联输送/长期燃料需求仍待。恢复或后续施工、库存/负载变化继续触发复验。

2026-09-13 / EXP-196/208/212/217/299有限氢回送施工复验（`validated`限唯一正常施工及材料/端点保全）：raw-b49f46d0一次fresh施工终态52159079，原生复用空闲ID3068，但新对象为3064槽8→3338的2011/filter1120；root raw-66c8fbc6独立核24详情、真实持槽、互返边和全部原连接，玩家只消耗2011一件、保留两氢，原J85及两网保持。实际stt600000证明三格普通分拣器理想上限30/min，不能沿用一格90/min或静态距离估算值。16执行器/8审计离线、解析/3模式拒绝通过，最近复验同日。

对象ID可原生重用，历史回收收据与当前新对象必须按动作时间、当前场景缺席、唯一新姿态/端点和材料共同区分，不能单看ID宣称旧设施复活或重复施工。accepted9/revision19/primary52129437尚未保存新回送，不把结构通过当到货或持续净供给；第十写正常保存后先完整十写审计，计数不因保存归零。完整净制氢、串联瓶颈、燃料长期目标及恢复仍另验；保存/恢复或线路/负荷变化触发复验。

2026-09-13 / EXP-196/208/212/217/299氢储备有限回送复核（`observed`限原生预检和预算）：raw-cc322c3e证明3064slot8→3338的2011/filter1120原生可放置，3338在旧取料3337下游，完整旧路再经3347/3074/3075到3073。保留源入口3617/slot7、H/D2预约和其他路线；用现有回收材料一件，不加塔/带或手工注氢。raw-d8411b84和当前目录证明原3616供电半径10.5m、候选源端距9.9731m，实际powerNetwork/供电比仍须完工读回；当刻容量余量不是全厂连续或基础负载余量。

新增返回只让有限储备可用，不增加配方净产量或自动消除后续瓶颈。普通2011全功率单件1/2/3格为90/45/30 min⁻¹，预检尚未给真实stt时保守按30预算，完工核原生200000/400000/600000再定值；不抬升3347的90或3075的60上界，也不以600库存承诺长期流量。16私有离线/解析/3模式守卫及26Core通过，最近复验同日，真实施工/到货/保存/恢复另验；原生可复用空闲实体ID，完工唯一性以当前完整场景中不存在加精确姿态/端点证明，不要求ID大于历史最高值。

2026-09-13 / EXP-027/128/137/196/208/299自然燃料切换与后续瓶颈复验（`validated`限双热电转石墨、独立短窗和缺料诊断）：raw-1fb15c4a/ba65ca93证明3058/3059的13次样本均fuel1109且正发电，13次供电采样满供；没有清空内部氢、直接改燃料字段或补料。首个切换样本之后才开始三个独立600tick窗，合计1800tick并排除81/81/69tick间隙。12继承+13专用离线/解析/3入口守卫通过，最近复验同日；accepted8/revision17/primary52129437/J85不变。

切换成功不等于上游已配平：中窗3073的confirmed material_shortage给出氢7/每轮需10，另一位置3064氢600保持；两裂解厂实际工作只能证明共同产物阻塞已有改善，不能把gross氢102/126/132 min⁻¹当净外供，也不能把棒12/0/0短窗或仓130→131当持续供应。下一H路线必须同时核库存位置、内部回流、全路径最小容量、其他长期消费者和所需净新增供给；本次不撤销此前当前四机净氢上限60/min或原串联60件/min边界。后续线路/负荷/配方/安装或保存恢复变化再验证，不因短期好转下调验收目标。

2026-09-13 / EXP-027/069/128/196/299热电回收覆盖保存复验（`validated`限正常保存）：raw-53e07517唯一save52129437，root raw-60a3d056独立核原始/fresh终态、18详情、原玩家/J85/两网及可用protected resume；accepted8/revision17，不是实际恢复或十写审计。12继承+3专用离线/解析/3入口拒绝通过，最近复验同日。raw-7a7c6629证明回收氢入口后电站仍可自然燃烧内部余氢；应等待真实fuel item改变和非零发电，不直接清料、更改字段、恢复旧入口或因暂未切换重做成功回收。棒仓130是库存，不替代连续产量；保存/恢复与持续供给仍分别核验。

2026-09-13 / EXP-027/196/208/299热电分流调整实机复验（`validated`限两次回收与保全）：raw-f3dfe0d8的3067/3068逐个fresh原生回收均唯一terminal，各回2011一件及当刻氢一件；root raw-2bb5c6d1核3955实体/7744互返边、18详情/真实设备槽、玩家净差量、原J85和两网只少两个消费者。不能将预检手持量当作固定执行量，也不能丢掉普通回收带回的货物。23执行器/5审计离线及21Core通过，最近复验同日，accepted7/revision16/primary51984902未保存新变更。已有石墨入口和源货保留不等于电站已经改燃料；要等原生耗尽内部氢后观测actual fuel及发电，再测持续氢/石墨供需，不能直接改字段或清缓冲制造切换。下一普通保存、自然燃料切换及恢复仍分别核验。

2026-09-13 / EXP-007/128/299只读缺失后缀实机复验（`validated`限调用方修正和三份独立原生窗口）：raw-fc641760唯一运行exit0，复用首窗后仅补52116100–52116699和52116718–52117317；root raw-de4ed7ac核16详情、原生整数计数/每分钟换算、玩家和J85，accepted5/revision12不变。三份600tick只覆盖1800tick，19118/18tick间隙不得拼接。局部129空、2802原油48–50及707精炼油39–40并存，不能从实际产量低于理论就断言全部油井耗尽；后续先核真实去路和分配，不默认新增油井。16离线再次通过，最近复验同日；原始两次caller exit1保留，IFX-125状态仅私有后缀修复，不外推持续燃料、全厂平衡或游戏API变更。

2026-09-13 / EXP-007/128/299只读后处理失败与原生证据边界复验（`validated`限原始响应核销，修正后缀live待验）：raw-86983ad0的未括号函数调用把`-and`当参数，raw-1c877cea的固定数组`.Add()`在首个原生生产窗口成功后失败；不能将caller exit1解释为没有有效窗口，也不能让Luna盲试第三次。root raw-a7d908b4核销首窗52096382–52096981及owned/healthy/accepted5/revision12/primary51984902不变。修正调用方复用Roundtrip边界、显式可增长集合、每窗先写保护证据，再添加摘要，只补明确缺失的两个600tick窗口；16离线检查含真实窗口、错误宽度/重叠/会话/不完整覆盖/错误产率和三次追加，解析及3入口守卫通过。零游戏写入，不修改Plugin/公开接口或重放此前动作。两矿机当前原生理论原油158.9412/min与实际18/min必须分别保留，单机输出49是背压线索，不是耗尽证明；缺少finding或氢P=C仍非持续平衡。最近复验同日；后续现场/配置/供料/会话变化重新验证。关联IFX-125。

2026-09-13 / EXP-027/128/196/208/299热电共同产物分配重设计（`observed`，仅预检通过）：raw-6c4ea42a/3854011b证明3058/3059当前烧氢，但各自的既有石墨输入3308/3402已接通/满供，源3298/3377当前纯石墨非零。root在净制氢blocker落盘后选择仅原生回收两个旧2011氢入口3067/3068，保留电站、石墨连接、氢仓和燃料回路，让游戏自然烧完剩余氢再选择石墨；不能直接写燃料或把预检当已经切换。两份native prepare已过，21 Core与23私有离线/解析/3模式守卫已过，真实回收/燃料转换/长期副产物消耗/保存恢复仍待；最近复验同日。

正常回收应以执行瞬间的native cargo证据和玩家精确净增为准：基本分拣器可能在预检后搬完一件，不能强行要求此前读到的1氢仍在手上，也不能无约束接受额外物品。这里只允许2011各+1、原生实际氢0或1且inc保全，逐次fresh端点/玩家哈希、唯一终态和目标消失；移除第一条后必须核共享3064的新槽态再做第二条。观察容量扣除两台热电额定值仍须覆盖当前负荷和额外碰撞机200000 J/t，但这不是全厂额定余量或连续供电证明；新动作后再观察actual fuel item、发电和氢/石墨产耗，不凭更高热电数量或低负荷瞬时值宣称消纳闭环。

2026-09-13 / EXP-128/137/196/208/299循环配方与输送共同预算复验（`validated`限当前无增产原生上界，实际持续供应未通过）：raw-857ff89e/3d242d83证明r16两机各15氢/min，r58两机各gross45但自身消耗30，故当前四机净增上限是60而不是gross120；仍须扣其他消费者并证明原油/精炼油/副产物去路。对7.5棒名义参考所需150氢，至少缺90净氢/min，不能把循环产物的gross计为外供，也不能用满仓搬运替代新增供给。raw-b2e5daaf确认关键2012的stt：三格300000→完整往返60ticks→60件/min，两格200000→40ticks→90件/min；3343与3075的串联60氢上限最多支持30重氢/3棒，即使3405另外能搬60重氢也不能取较大值作为全链能力。旧重氢库存使此前6棒/min短窗成立，既不推翻理论瓶颈，也不证明长期可持续。

最近复验同日；26既有FoundryTransportPlanner Release测试通过，raw-3d242d83只读历史证据/零游戏调用。raw-b2e5daaf/857ff89e两次读3083石墨输出20且停机，静态路线仍通向六台热电和环线；没有逐段实时证据前不能称断线，混合热电燃料和副产物背压须随新增负荷复核。整仓600氢并不在碰撞机输入口，不能把它当额外生产率；新增回送只能是有限调配，完整修复仍须生产、输送及真实长期消耗同时满足。私有计算器的语言数值字面量错误只在离线纠正，不重放已接受动作；后续供料/拓扑/配方/负荷/保存恢复改变时重验，accepted5不归零。

2026-09-13 / EXP-007/027/069/128/196/299燃料回路覆盖保存复验（`validated`限正常保存与相关对象保持）：raw-ad5dea62唯一save终态51984902；root raw-ca36fadf于51989724核原始/fresh同一终态、18关键对象相对原完整工厂基线的配置/连接、玩家库存/位置、原durable J85、两网成员及零预建筑。accepted5/revision12，没有因保存清零计数。12私有离线检查通过；正常保存签发protected resume能力不等于已经加载验证，新线路的恢复仍待。保存可以先保护真实施工进度，但不会消除此前三个独立窗口证明的重氢净消耗，持续供应仍未通过；后续恢复或供需/拓扑变化必须fresh复读。最近复验同日，旧主档51680646未覆盖新线路的记录自此只作历史截面。

2026-09-13 / EXP-128/137/196/208/299燃料实际供需复验（`validated`限三个独立分钟窗，持续燃料未通过）：raw-547a8e31/aaf44067保留三段3600ticks而非600ticks，合计180秒，不是180分钟；段间36/30ticks不能补齐。实际产棒6/6/6每分钟、重氢产10/10/5及耗60/60/60，棒仓76→82→88而重氢仓348→297→242。无finding/无blocker或P=C字样不能覆盖库存下降和上游缺口；相对当前实际消耗的净缺口50/50/55，与相对单机7.5棒名义参考的65/65/70必须分栏解释，诊断参考不冒充已锁定远征目标。最近复验同日，11私有只读离线/解析通过，零游戏写、accepted4/revision11/J85保持。

raw-abbc68ff实物确认关键供氢/重氢口3075/3347/3405/3076已是Mk.II；新增储备负载后要复核实际分拣器等级、跨格时序和连通源，不能凭“普通分拣器瓶颈”就安排不适用的升级，更不能因缺口扩大当前白名单。3064氢600与3073缺氢同时出现，表明库存位置和可用供给必须分别核算；本次只确定需重验供氢路线/产率及重氢输送，不从一处满仓断言已具备足额持续供给，也不把增建碰撞机当默认解法。后续供料、配置、升级、拓扑或保存恢复变化重新测量；既有回送线路保持，主档尚未覆盖的边界单列。

2026-09-13 / EXP-018/027/128/196/208/212/217/299燃料回送施工复验（`validated`限两条原生连接及瞬时到货）：raw-a5c5687f/74463da5证明先3955槽6→3956→3409、后3411→3957→3955槽8；第一条成功后重新取现场端点hash及prepare，第二条并未依赖旧预览预约。两2011/filter1802均正常扣料/无人机完成，真实仓槽与带4..15物理槽双向核对；旧直通/3415/旧仓和3957实体/7752边保持，两网仅增加两个已满供消费者。最近复验同日，27离线/解析/3入口检查通过。读51906082仓45棒、聚变59746 J/t；将既有带/设备缓冲转入新仓是正常货物流动，不等于45件全部新产，也不是实际生产速率。accepted4/revision11、主档51680646未覆盖；后续三个独立原生分钟窗口需同时保留供需与库存趋势，不能以到货一次、P=C或额定产能核销持续燃料。连接/槽位/过滤/电力/材料或保存恢复变化须fresh复验。

2026-09-13 / EXP-018/080/128/196/208/212/217/299储备空仓及原生端点复验（`validated`限一次施工和独立预检）：r85×2的公开预算为直接配方铁块2/电路板2；本次原生递归使用已有铁块1加铁矿1，终态和全玩家净变化必须核铁矿−1/铁块−1/电路板−2/2011+2，而非机械套直接预算。raw-600a768e/f0dab947证明正常制作与原J85保持；缺少旧manual first可能是跟踪起点的历史排除，不补造首次。随后raw-199669d5/3e03c45b证明空仓3955正常扣料建成、3955实体/7744边、空默认30格、原设备/两网保持。最近复验同日。

几何估算已取得实际原生证据：3955槽6→3409、3411→3955槽8均exact_slots、过滤1802、供电点距离4.765m/4.491m；之前2.78m/4.5°估算不再代替这些真实槽位。仍须先出后入、逐步fresh prepare，不能将两次独立预览当联合预约或忽略第一条建成后的新碰撞条件；原燃料直通与3415保持，入仓口施工前也不把空仓900棒容量当库存。23/16私有离线检查及解析/入口拒绝通过；accepted2/revision7、主档51680646未覆盖，连接、实际积累、长期上游供需与保存恢复另验。位置/槽位/邻接/材料/供电或安装变化均触发重验，不改已成功旧仓和线路。

2026-09-13 / EXP-007/018/048/080/208/299备料及十写复验（`validated`限本窗口材料和既有状态保持）：Luna raw-1cf99153正常取26电路板2、1511铁块1，terminal51785594/51785728的源端即时差量与玩家严格守恒；之后源仓正常自动回填不推翻该同tick证据。root raw-f4cde4ed于51797840核全部十个原始终态、当前session三个fresh终态、3954实体/7744互返边/114详情、真实持槽、77纯合金带/11受电分拣器、玩家净+2电路板/+1铁块及原durable J85；和平/非沙盒/1×、healthy、零预建筑、两网满供，实际聚变37292 J/t不是库存。17执行器/12审计离线检查通过，最近复验同日。accepted10冻结至本结论推送且CI成功，不由revision3倒推计数；后续才重新计数。主档51680646未覆盖本次取料；既有建成/保存/恢复不能被这次材料阶段重复计功，持续产棒仍未证明。下次写入、部署或物料/拓扑变化重新核对应边界。

2026-09-13 / EXP-196/208/212/217/299燃料储备方案复核（`observed`限拓扑规划，尚未施工）：已有燃料带上游单向截流进仓，可能使下游聚变拿不到后续新棒；当前有电和缓冲尚满不能证明这种方案长期安全。root撤回单分拣器草案，改为评估3411→新独立仓→3409的正常双分拣器回送缓冲，保留原燃料带和3415直供；不改3064/3065/3074的成功配置。新仓2101原生空地预检通过（raw-1d622a14），同位置两个端点仅有约2.78m/4.5°切平面估算，必须在建成后以实际槽位/原生预检重新证明，不称整条回路已通过。原生目录每个2011满载300 J/t，新仓0，两个分拣器增量上界600 J/t；当前电网余量不是全厂满负载预算或持续燃料证明。先仅正常领取26的电路板2件及1511的铁块1件，配合现有铁矿1件供后续递归手搓两个分拣器；达到accepted10立即冻结审计，不连带授权施工。最近复验同日，落点/槽位/供电/物料或消费路径变化必须重验。

2026-09-13 / EXP-001/002/048/137/196/208/299同档冷部署与长周期实机复验（`validated`限本次恢复及20棒诊断，19棒精确边界仍待）：6bfc533同源1846-test/228文件冷部署，实际安装MCP64/1/70562指南/纯stdout/exit0，CI34713850292成功。正常保存51680614→protected resume51680646，root raw-5039de88/77848b77前后独立核3954/7744/114、77带/11受电分拣器、原J85和玩家物料；位置差15微米明确保留，accepted8不归零。实际原生发电47021 J/t不是燃料库存，恢复也不等于持续供给。最近复验同日；程序集/DSP变化或下次部署须重验。

IFX-123的600tick小于960tick漏报已在新安装关闭：raw-a42fbaa9窗51683855–51684454中停机/满电/20棒/产0，返回confirmed output_blocked及阈值19；raw-c1bf268f离线独立核原12个MCP响应，玩家/端点hash和revision保持。原严格19棒探针exit1是现场数量不符，不能把读取到的20棒正例包装成19棒验收或脚本exit0。IFX-124的19棒判定已有离线正例及实际安装阈值读回，但精确19场景只可等待正常消耗，不清仓/手供/注入制造。缺少finding仍不证明配平，0/0和有限缓冲满载也不替代长期燃料供需。63 Core+2 MCP回归通过，完整1846同批测试成立。

私有执行经验补证：恢复前预审发现调用方猜测install proof有journal/installedHashesMatched，实际为pluginFilesMatched、mcpFilesMatched、allInstalledHashesMatch；J/revision必须由该proof引用的独立pre-close审计核对。按真实schema修正并补25项离线守卫后才交Luna单次执行，无错误游戏调用。MCP get_status的structuredContent.status与其他业务result同样不得混读；本地证据读取错误只重读原raw，不重放游戏。保留已有反例和已完成对象，不将辅助脚本错误扩大为游戏恢复失败。

2026-09-13 / EXP-196/208/299原生出料阈值离线修正（`validated`限当前DLL与离线，IFX-124）：不能把每批q件的整批缓冲上界当成首次原生拒绝数量。单/多产物DLL分支均支持Smelt的101−q、Assemble的9q+1及其他配方19q+1；既有计算器按这些阈值判断，实验室/采矿门不变，异常q>100熔炼不猜正阈值。原output_buffer_capacity键保持兼容，MCP/指南解释它不是物理仓容；18棒不误报、19/20棒能识别，缺少停止/速率/电力证据仍不能越过整周期守卫。63相关测试先16红后全绿，新增20 Core+1 MCP后Debug/Release1846、完整Release/locked restore和真实源码64/1/70562指南通过。最近复验同日；当前实机仍133d96f/accepted6/主档51590594/J85，新修复待另一同批冷部署与原场景复验，不宣称持续燃料或版本完成。

2026-09-13 / EXP-001/002/048/137/196/208/299冷部署及燃料保存恢复复验（`validated`限本次恢复；持续供给与新诊断正例未验）：133d96f的1825-test完整Release同源228文件经过staged与installed实际MCP64/1/69804/exit0验证，raw-38ddae6d保留备份、正常关闭、Steam单次启动。51590563→protected resume51590594，root raw-aa0ca894/9a0ad9d9前后独立核全3954实体/7744互返边/114详情、玩家物料、源3954实际持槽与纯料、原J85和两网结构；3079恢复后实际发电31817 J/t。accepted6保留、非十写，位置差6.1微米明确记录。最近复验同日；不把source握手、staged包、已安装或普通保存混为实际恢复，更不称最终包/持续燃料完成。

EXP-196诊断边界修订（`validated`限新实机反例，IFX-124）：raw-3512b944的600tick窗51599099–51599698产0，3403前后均停机/满电/19棒，却无finding；raw-a1af1f0c于51605434复核。IFX-123原20棒限定不能覆盖部分腾位后的原生批量准入拒绝；当前DLL的produced>9×productCounts使批量2在19即阻塞。不能重复等20或拿整批理论容量冒充精确准入阈值；下一只修既有计算器与测试，不增工具/字段或写原生状态。原测试失败是只读验收前提不符，不是恢复失败，不重启/重建。私有保存守卫另在导入前快照模式、复核原玩家/完整Journal；动态发电capacity不列为固定结构，成员数和实际供电仍严格检查。原IFX-081的字符串替换尾锚问题又被解析器在任何调用前截获，改用字面回调，无游戏影响；不把调用方脚本错误归给DSP。

2026-09-13 / EXP-196/208/299长周期诊断复验（`validated`限原生反例与离线修复，IFX-123）：r41/MkI的960tick周期超过固定600tick原生诊断窗，旧classifier在停机、满20棒、输入齐备/满电时仍永远返回null。重复相同只读窗口不能解决一个恒不成立的门。当前DLL和3个先失败的回归确认根因；仅对停止、known-zero、供电已知充足、满输出的非采矿设备开放已有output_blocked分类，其他整周期规则不变。19 Core+1 MCP新增后1825 Debug/Release及完整Release通过。MCP描述/包内指南同步，但当前安装仍旧批，live正例待正常冷部署与同档恢复；accepted4/主档51439686/J85不因代码测试改变，持续燃料与完整0.4门不缩减。最近复验同日；不从“没有finding”推导配平，不把短窗0产或部分腾位直接等同于这一窄满输出证据。

2026-09-13 / EXP-037/048/137/142/185/196/208/299自动燃料启动复验（`validated`限真实施工、首产及当时发电；持续供给仍未验）：Luna raw-79c82d79仅正常建成3954，900槽10→3892、2011/filter1107；51381916终态成功，玩家仅扣一件普通分拣器。root raw-3599fb8b于51429537独立核完整3954实体/7744互返边、106详情，77带纯合金、11分拣器受电，原玩家与旧配置保持，accepted3不归零。J85在51385784由factory-production-register记录首次自动1802，原84条历史不变且85全部durable；没有采煤、手搓启动棒、改仓容或人工供棒，证明库存支持的自动启动实际成立。最近复验同日；后续正常保存必须覆盖J85，不再用旧J84固定守卫误拒绝已证明的新增历史。

首个独立3600tick窗51384607–51388206实际产棒8/min，聚变3079随后实际发电且两网采样满供；generator buffer的joules_per_tick是功率率值，不是燃料数量。相同首窗重氢产35/耗80每分钟，不能从缓冲或一次成功推出持续配平，也不能把当前供电比1当全厂满负载预算。14项只读审计离线、解析及三入口拒绝通过；这次不是十写审计、保存恢复、持续燃料或版本完成。后续独立窗口须分别检查产量、库存趋势与输出背压，不直接把启动瞬态缺口当永久矿源缺口。

保存补证：Luna raw-d5dbd9b0正常保存51439686，root raw-86cca4bd于51444252复核同一原始/fresh终态、23关键对象、玩家和完整durable J85；accepted4/revision61，计数未重置。21项保存守卫离线、解析及三入口拒绝通过；没有重启。raw-67b9099e另一3600tick窗已为产棒0，3403输出20、三种输入齐备；重氢仍有390库存而3073仅6氢输入。这些是新的输出/上游诊断边界，不否定已经发生的自动首产，也不构成持续吞吐验收；下一写入先追踪当前真实瓶颈而非重做已成功合金线路。

2026-09-13 / EXP-137/142/185/188/208/299启动前置修订（`validated`限库存/配方与单源预检；供电和持续生产仅待验）：raw-ae9f0e81于51340499核900储存1300合金，3403已有40重氢/2磁场环，只缺合金即可满足r41首两批其他物料；旧“先采6煤、手搓6棒、临时禁格”的方案是一个备选，不应固化为已有自动产线的必经前置。本次用原生900槽10→3892、2011/filter1107单源预检和正常成本重新收敛方案，原玩家备料不回滚、不重建77带/十分拣器。最近复验同日。

即时net3为485001 capacity/86100 required J/t，核心五机/七快速/十一普通全工作234500 J/t小计后尚余164401；小计不包含所有上游和旁支，不能冒充全厂满负载通过或预支聚变容量。仅允许条件性的首批真实生产验证，后续实际供电、材料来源和持续窗口仍须核销，供电不足时停止新写。36项私有执行器离线及解析/三入口拒绝通过，原J84只允许真实后续自动1802首次追加，离线合成事件只测试守卫、不写Journal。没有实机供料或产棒正例，本记录不提高版本完成度；完整上游预算与满负载门保持打开。

2026-09-13 / EXP-061/128/179/246/253/299干地分段正例及东侧负例（`validated`限本次短移与事前拒绝）：不同约3m目标保留全部有效干地样本、原0.1m余量和1.5m容差，raw-433b7ba8的唯一Move在51280467成功；root raw-e36c325d于51288467独立核原始/fresh终态、2.126m真实位移、Walk/零速、玩家/原J84及关键对象保持，accepted2/revision58不归零。这里没有跨水，不把“跨水末端至少4m干地”误当所有纯干地短移的最小长度。随后东侧4m候选raw-726d33b8的末两点已低于水面0.122/0.220m，零commit即停，不执行关联北行或继续同类候选。最近复验同日；地表采样仍不证明建筑净空，短移成功也不能沿新直线穿回仓库；需要重新核对业务前置，不能把一次采矿绕行变成无限寻路。

2026-09-13 / EXP-061/128/179/246/253/299短移负例复验（`validated`限事前拒绝及无写核销）：raw-e0b446de的约4m脱离候选原生prepare可提交，但五个完整地表样本的末端仅高于水面0.052m，未达既定0.1m余量；前三至第四点均至少0.153m，不能以聚合not_detected或多数干地点代替末端条件。零commit/零actionId；负dry suffix仅为本地未找到合格末端样本的诊断，不是负路径或水下证据。root raw-0cce8e58/raw-d08f95a8分别于51224522/51251319核原位置/玩家、healthy、accepted1/revision56、保存51163978与原durable J84保持。最近复验同日；旧候选停止，重新选点仍需fresh采样/原生预检及真实终态，不改容差或地表门槛，也不将此负例冒充原生Move失败或通用不可通行结论。

2026-09-13 / EXP-061/066/076/077/128/179/299采集接近复验（`validated`仅当前DLL语义与原指南；新移动待验）：`PlayerAction_Mine.GameTick`只在原Mine订单targetReached后采集，Spherewright的1.2m前置接近点及80m范围预检不提供碰撞保证；harvest未复用Move停滞看门狗，不能将它当远程静止取矿。当前玩家靠近3050化工厂/3051仓与带，故只批准基于fresh现场的一次约4m切向脱离，要求全部地表采样为干地、原生Move终态及真实Walk/低速/能量复读；不是连续自动寻路或49m采集许可。17离线及解析/三入口拒绝通过，尚未执行。原生调用、超时和工具面不改；资料见research对应节，最近复验同日。后续从实际落点重新规划，不能转身再次穿过同一建筑或忽略未观测植被。

2026-09-13 / EXP-007/048/069/080/182/208/299保存边界复验（`validated`仅正常保存及独立复读）：完整十写结论ab49bcd及CI成功后，Luna raw-4f9aeefd新窗口只正常save到51163978；root raw-1135a3bc于51178211核唯一原始/fresh终态、3953实体/7740互返边/92详情、全玩家和原durable J84。20执行器/15审计离线及解析/入口拒绝通过，最近复验同日。77带与十个合金分拣器仍空载，6涡轮/9磁铁正常保存，无额外物品差量；accepted1/revision56保持。签发protected resume不是已执行重启，空载施工落盘也不是持续燃料；后续现场、材料、安装或恢复变化时仍须fresh核验，不重建已保存对象。

2026-09-13 / EXP-007/018/080/185/208/237/299完整十写复验（`validated`仅八空载施工加两取料）：Luna raw-31e5862f在51107248/51107329正常从827取6涡轮、723取9磁铁，同tick源端3000→2994/400→391与玩家精确增量吻合；后续自动回填不用于推翻原始双边差量。root raw-1de89fbf于51133478复核十个原始/fresh终态、3953实体/7740互返边/1032详情、真实端口/供电/玩家及完整durable J84，132离线及解析/四入口拒绝通过。最近复验同日。

审计比较JSON数据时，数组施工顺序必须保留，普通对象键顺序不是业务身份；本次本地检查对两条取料intent改为精确source/item/count及字段数核验，换键序仍应通过、数量或身份变化必须拒绝。这只是离线检查修正，未放宽原生动作或重放物品转移。accepted10/revision55在结论推送及CI通过前保留；77带仍空载、3074未改、主档50325905未覆盖本批。下一采集仍须审查附近仓/化工厂/带障碍，能源、原料库存和临时仓容方案变化后fresh验证；完整启动、持续燃料和0.4准备门不因本审计关闭。

2026-09-13 / EXP-007/018/080/137/235/299启动备料复验（`validated`仅四项当时预检，执行与持续供给待验）：raw-38934573于51053796证明两次精确取料、529六煤输出预算和3074全30格自动入货限制可正常prepare；accepted8/revision53/J84/主档50325905不变。仓库禁格不删除现存物品，也不等于冻结全部出货；未来取重氢/普通存入/恢复0禁格的顺序必须单独核销，不能把空格暂时出现当容量已锁。煤在80m交互范围内不保证移动路径可通，预计采集tick不包含无障碍保证。最近复验同日。

已用56项离线检查限制下一执行器只有827取6涡轮及723取9磁铁，fresh prepare/唯一commit后逐项terminal，accepted10即停；三种非法入口在导入前拒绝，offline零导入/零游戏调用。预检脚本29项通过，原采煤响应追加完整预算核销但不重发或复用计划。玩家未取料或获得燃料，六棒仅是有限引导候选；持续产出、发电增益、仓容恢复、保存恢复及完整0.4门不得由这些正例替代。库存、现场、session/revision或安装态变化即重新读取，不让正常自动回填掩盖transfer的同tick双边证据。

2026-09-13 / EXP-142/157/184/188/215/219/224/299启动前复验（`validated`仅读数边界，启动方案待验）：root raw-0d9ae706于50999555用现有Foundry的明确占位负例取得真实满基础负载；net3原有1462100 J/t对实际449166 J/t，缺1012934 J/t。假想新机4500 J/t与真实原有负载分列，现有满供不冒充满负载余量。全入边闭包2083对象/198已知消费者目录及当前功率497500 J/t仍不是完整因果上界，916目录缺失要另列原生100000 J/t充电上限，不能当零。最近复验2026-09-13。

有限燃料引导可以用正常取料/采集/手搓，但须在施工前明确材料、容量、允许启动的负荷与燃料时间预算；6棒/14400tick的额定最大耗用推算不是已产棒或持续供应。普通仓库存无1802/1205并不证明其他缓冲也无；3079发电buffer的0 J/t尤其不能解释成燃料库存0。Foundry原生网络容量按当时实测保留，不能额外加尚未证明燃料可用的250000 J/t额定聚变功率。3074已满，取重氢可能立即使上游恢复，且空出的格可能自动回填，需另证预留与正常手动存入规则，不把一次取出当成已锁定空位。

同次只读调用方纠正了列表无beltCargo与详情observed的混用：保留原失败证据，改为77条精确inspect，null/unknown/非空均拒绝；28离线及解析/入口拒绝通过。末读50999613、3953实体、accepted8/revision53、原玩家/J84/主档50325905不变，零游戏写。仅关闭启动预算的定位阶段；源激活、实际发电/持续产棒、保存恢复和完整0.4门未核销，材料/负荷/容量/现场或安装态变化后重验。

2026-09-12 / EXP-007/018/027/048/128/188/195/208/219/299合金其余八桥接复验（`validated`仅空载施工与保全）：Luna raw-7a423f73完成3946–3953；root raw-f4830915于50926031核八原始/fresh终态、全3953实体/7740互返边/1032详情、实际互返槽位/普通STT及供电、原玩家和完整durable J84。77带及十个合金分拣器全空，玩家净−8普通、余3带/1普通，原纯料路径及两网保持；110审计离线、解析和三项入口拒绝通过，最近复验同日。

摘要不能代替原始证据：本次转发把首端3893误写3903并混入上一批CI/脚本摘要；受保护intent绑定eece174/CI34702391501、八fresh计划/终态和独立实体读回均一致，实际未接错，不据转录错误重放或改建。此复验沿用既有唯一回执核销原则，不新增游戏原语。accepted8不因阶段核验归零，主档50325905未覆盖施工，源900仍关闭；激活前必须fresh计算完整同时启动负荷及可用供给，不用十一分拣器额定3300 J/t替代全链，也不预支尚未受燃料驱动的聚变容量。源连接、库存/负荷、安装批次或恢复边界变化时重验，持续产棒和完整0.4门仍独立。

2026-09-12 / EXP-007/018/027/048/128/188/195/208/219/269/299合金连接十写复验（`validated`限本窗口施工保持）：Luna raw-83c2e407正常建成3944/3945；root raw-6755ef9a于50856240独立核八带路加两分拣器的十个当前终态、全3945实体/7708互返边/1024详情、实际设备持槽、旧纯料链与两网。完整玩家净−42带/−2普通、余3带/9普通，全77带及新sorter空载，原J84完整耐久保持；105审计离线与入口/解析通过，最近复验同日。accepted10直到推送/CI成功才关闭；96离线支持下一有界八件空载桥接，不授予900源激活或保存/持续燃料结论。

同一写窗口内对象可能先新建、再合法接线：配置比较保留初始身份/姿态等不变量，最终连接由所有批准动作逐边构造并精确核对，不能拿中间空带拓扑要求最终状态完全相同。本次离线样本拒绝了错误的3905比较，并以正例及四个额外边负例验证修正，未丢弃边检查。IFX-081也再次由AST在游戏调用前捕获，跨语言替换一律使用返回字面文本的回调，不能因为模板曾通过而省略生成文件的解析/哈希复核。源激活/负荷与其他完整0.4验收仍独立。

2026-09-12 / EXP-128/188/195/208/299合金十一连接预检复验（`validated`限实际原生预检）：root raw-f18a7d15于50801202完成十一2011/filter1107正例，先燃料台槽4，再九段桥接，最后混仓槽10；actual exact_slots/零offset和逐件预算一普通均成立。取货点到实际目录电塔4.4559–10.1308m且留至少0.2m，46详情/玩家/J84/两网保持。92预检与42双对象执行器离线及入口/解析通过，最近复验同日。

整套材料齐备及十一分别成功的预检不是联合预约，也不是已施工、已受电或已供料；执行仍逐步fresh，设备实际持槽与belt虚拟槽对应的实际空闲槽均须完工反查。只批准先两件空载连接到accepted10，不为一次阶段审计归零；其余九件和源激活另批。3300 J/t是十一普通的名义负荷，不代表启动燃料台/上游后的整体余量，也不能提前计入未产棒的聚变电源。持续供应、保存恢复及完整0.4门不外推。

2026-09-12 / EXP-007/018/027/048/128/188/195/208/219/299合金完整空载带路复验（`validated`限八施工及旧线保持）：Luna raw-6f1aa988正常完成F2/K两源续接和C/D/E/G/H/I六独立段，共42NEW；root raw-0232a8a4于50783562独立核八原始/fresh终态、全3943实体/7700互返边和1022相关详情、全部77合金带空载、原持槽/纯料链/供电/完整玩家/J84。76审计离线、85执行器离线及入口/解析通过，最近复验同日。

真实二十条/四条父路径按24上限完整读取。原生whole-path保全不是忽略整条旧带朝向：仅3886/3890改变，并逐一等于原成功施工读回；直接邻居3885/3889和其余旧朝向保持。审计详情取原917与实际施工观察的完整并集1022，没有为沿用旧上限丢弃目标。完整玩家净−42带、余3带/11普通；accepted8不因阶段审计归零。未保存/通料或建十一分拣器，源900仍无出口；源激活前仍要fresh端点、实际消费者受电和启动负荷，持续燃料与其他完整0.4门另验。

2026-09-12 / EXP-128/188/195/208/219/299合金实际源带复验（`validated`限两个原生预检）：raw-3c55524a于50700801以真实3886/3890和fresh源端点/玩家哈希分别准备F2/K，两条NEW、full_path_stage1、whole_path_native_rotation_v1均通过，路径与既有空地参考扣掉首点后一致。42详情、二十条/四条实际父带的空载与旧拓扑、完整玩家/J84/两网保持，23离线及入口/解析通过，最近复验同日。

空地参考与真实source cover分开留证：计划中的原生旋转保全标记仍须真实施工后核对，不等于源/邻居已经成功保全。后续读取上限须覆盖实际F1二十条父带；旧酸路模板的八节点上限不适用，本批有界完整源证据不能被截短。剩余两续接四带与六独立段三十八带共42，现45带足够，每项仍重新原生预检；两计划不是联合预约，尚无新增accepted、施工、供料或保存，十一过滤连接/持续燃料与完整0.4门不提前核销。

2026-09-12 / EXP-007/018/027/048/128/188/195/208/219/299合金空载施工十写复验（`validated`限四段施工保持）：Luna raw-af256e78按F1/J/A/B完成20/4/2/9条普通带；root raw-98bee00e于50678005独立复读十个当前进程终态、全3901实体/7628互返边/917详情、所有实际旧持槽/关键纯料链、两网和完整玩家/J84。四段精确有序路径均空载、自由端真实未接；父带末端3886/3890由位置及双向拓扑确定，不能按原生分配ID排序猜方向。74审计离线与入口/解析通过，最近复验同日。

本十写保留原恢复一次、备料五次、施工四次，不按revision17推算动作数。相对恢复后净+40带/+11普通，实际45带/11普通；原900仓无输出，仍未供料或保存。离线审计发现旧模板900详情上限不适用于本次邻域：由730历史详情、本批新增对象及已观察邻域去重，实得917且零未知ID；据此固定为精确917项并完整逐项读回，未缩减审计集合。推送/CI成功后才关闭accepted10窗口；下一真实source cover、保存恢复和持续供给另验，不把空载建成抵扣完整Foundry或远征准备门。

2026-09-12 / EXP-128/188/195/208/219/299合金输送齐料场地复验（`validated`限固定形状）：raw-498f17cc于50618484完成十独立路径及两空地参考的十二次full_path_stage1预检，实际点数条件性合计77；143预检离线和65有限执行器离线、入口/解析通过。首尾全3866实体/7566边、15详情、原玩家/J84及两网保持，accepted6/revision9/主档50325905不变，最近复验同日。

未来子段的空地首点与父段原生尾点重合，只证明同一位置和完整条件性材料预算，不证明已有source cover的方向、货物/原邻边保全或真正分拣器槽位。先正常施工F1/J两个父段，再做真实接头预检；同批另建A/B，四条独立空载路径共35带、零新消费者，累计十写即完整审计。原生独立正例不是联合预约，施工仍逐段fresh prepare；不存在的父ID或预览令牌都不得用于commit。源供料保持最后，280118 J/t操作余量不取代完整启动/全厂预算；后续十写、真实cover、连接和恢复分别复验。

2026-09-12 / EXP-007/018/048/188/208/219/299合金路线备料复验（`validated`限正常备料及原厂保持）：Luna raw-78bbf05b五个唯一accepted动作按86铁/11电路、齿轮25批/普通带25批/普通分拣器11批完成；root raw-1cec12b5于50570453独立核原始和fresh终态、完整3866实体/7566互返边/76详情、真实设备持槽/受电、原玩家与J84完整耐久历史。相对备料前背包只有+75带/+11普通，最终80/11、叶子原料为零，未改变既有线路或产生预建筑。46审计离线及两入口拒绝/解析通过，最近复验同日。

accepted1→6与实际revision9分别保留，revision不假定每动作只加一；审计按真实顺序、终态和物品差量核销，不以revision差值替代accepted计数。备料成功不自动扩展为施工授权：齐料后仍须完整fresh原生路径检查，两个未来父带的source cover只能在父实体存在后证明；下一计数从6继续，第十写冻结。主档50325905未更新，观察tick不当作保存，900仍无出口，持续燃料/完整三级Foundry及准备门不核销；后续施工、恢复和十写边界复验。

2026-09-12 / EXP-007/128/188/208/219/299合金路线预算复验（`validated`限离线完整预算及两取料native正例）：raw-22e4048a的固定12段/最多78带/11普通无需移动或新电塔，source900.slot10与target3403.slot4及各跨带方向、取料点覆盖留≥0.2m余量；F2下移一格避免依赖3478覆盖边缘，未放宽原有覆盖余量。raw-2bf20155于50515727证明86铁/11电路两取料预检，按实际三个配方25/25/11批计算可制成75新带/11普通，加原5带得到80/11，尚未执行。51有限执行器离线、21相关Release回归及入口/解析通过，最近复验同日。

完整候选物资预算、独立端帽native正例和未来父带cover是不同证据层；未来cover须父实体建成后重新prepare，不能从空地预算扣掉一个对象就宣布已验证。3300 J/t新增输送与236300 J/t六设备启动集合不代表完整因果链或全厂，349820 J/t只是该次观察余量，源激活前须复核，未产燃料的发电机不能提前抵扣。Luna仅执行已批准两取料/三手搓、accepted1→最多6；所有其他旧背包内容、原J84和成功厂区保持，不复用预览令牌，不在备料后自动施工。

2026-09-12 / EXP-007/128/188/195/208/219合金输送两端复验（`validated`限两独立native场地）：raw-ad3eeba4完整3866占位/35详情后，raw-2d6439df的A两NEW、K空地三NEW均通过full_path_stage1，既有槽位、玩家、J84和实际两网保持，末读50477651。32采集/20预检离线及入口/解析通过，最近复验同日。端到端约105m不意味着需要移动；玩家位于中间时两端仅59.9113/45.8407m，应计算每个实际施工点相对玩家的距离，不凭两端间距额外下Move。

未来父带未存在时的独立空地三NEW预览，只证明该空地形状，不证明复用父带的两NEW预算、朝向/货物保全或分拣器可接。900是混仓，合金出口必须显式1107过滤，原两个输入不得修改；3403已有40重氢/2磁场环不等于已经产棒。材料源2700铁/3600电路也不是整图预算或齐料预检。accepted1/主档50325905保持；后续备料、真实连接、启动负荷、持续供需及保存恢复各自核验，不以预检正例替代0.4完整验收。

2026-09-12 / EXP-027/048/188/208/219恢复后固定短窗复验（`validated`限恢复加工）：raw-75c765c7在新session中的六个独立600tick窗保留酸4/4/4/4/4/4、合金4/0/0/4/0/4，原两正窗/合计≥8/输出净增判据通过，测量合计24/12、900合金588→599、863酸426→424。14/17/61/40/35/42tick缺口及三个合金零窗全部保留，不能重取有利样本，也不能把错时仓库净增11与统计12强行当同区间守恒；产物在机器/分拣器/带/仓之间正常流动，统计和实物读数各有时点边界。

37观察离线通过，首尾3866实体/7566边及唯一生产者、每窗40详情/两网满供、原玩家/J84保持，末读50395224。最近复验同日，accepted1/revision1/主档50325905不变，只有3600已测ticks，不声称十分钟连续稳态或新保存。油输入不足截面和酸库存下降仍限制长期供给，下一接900→3403真实燃料负荷后须重验上游产消、库存趋势和实际电力；本次恢复加工正例不替代持续燃料、全基础负载或完整三级Foundry验收。

2026-09-12 / EXP-001/002/007/027/048/188/195/208/219/299酸/合金同档恢复复验（`validated`限持久性）：raw-15433bc8正常关闭/Steam单启、raw-875b71a7实际228安装哈希及64/1/68175指南握手通过；resume-19563f3c从50325874恢复重存50325905。root raw-1bf781d3于50343166核全3866实体/7566互返边/730详情，75水带/七水分拣器与21酸带/六酸分拣器、真实持槽/过滤/货物流和两网满供保持，全玩家材料不变、首读位置差0m、原J84身份及全部耐久历史不变。旧十写已由9acba67和绿色CI核销，新accepted1/revision1保留，不把新session或审计当计数归零。

24恢复审计离线通过，最近复验同日。跨进程只查询新resume动作；旧十动作采用正常关闭前已fresh核验并封存的终态，不在新进程猜旧action结果。指南比不可变安装来源，不能把源码指南更新称作已部署；未更换运行程序集或最终包。恢复后重新开始固定六窗观察，不能复用旧session产出、隐去零窗或拼接缺口。实际运行满供不核销全厂满基础负载；酸库存下降、油输入不足与未接900→3403仍限制持续供给结论，后续接入燃料需求、保存/重启或十写边界再复验。

2026-09-12 / EXP-007/027/048/188/208/219/299普通保存与十写复验（`validated`限已建状态及覆盖保存）：Luna raw-8e78c9d5正常保存50325874，root raw-44b6318b于50350333独立核十原始/fresh终态、3866实体/7566互返边/730详情、各实际设备持槽及既有关键材料路径/新增21纯酸带/六普通的真实供电。三次两带、六次单普通及一次save按原accepted计数核销；全玩家净−6带/−6普通，原J84/两网/零预建筑保持。审计覆盖源带合法旋转的既有独立证明，不因正常货流改变静态配置或强行回滚。

75审计离线、21客户端库存检查、13恢复/Journal Release回归及入口/解析通过，最近复验同日；accepted10须本结论推送且CI成功后才关闭。主档50325874覆盖施工，不能把稍后50350333观察tick当保存tick；原生运行期间的后续产物并非都在该主档。12生命周期离线不是重启/恢复实测，接下来仍核当前安装哈希、正常关闭/Steam单次启动、绑定该主档/J84的protected resume及产出；酸净下降/长期供需与完整0.4门保持未验。

2026-09-12 / EXP-027/048/188/208/219固定短窗复验（`validated`限恢复加工）：raw-ca07ce72六个预声明、互不重叠600tick实机窗，合金产4/4/0/4/4/0、酸4/4/4/4/4/0；保留全部零窗和10/45/41/39/44/13tick缺口，按原≥两正窗且实测合计≥8及输出净增规则通过。900合金316→332，863酸477→465；720tick批次的合金存在零窗，不能据此单窗判停产，也不能用正窗挑样或库存增长宣称长期稳定。源仓下降、油输入不足仍限制后续供需结论；实际产出与可分配持续供给必须分开。

证据范围：37观察离线、16相关Release回归及入口/解析；首尾3866实体/7566边、每窗40详情/21纯酸带/六满供普通及两网/原J84，末读50300135。观察零写、accepted9/revision33/主档49805037保持；20保存器离线和三入口/解析只支持下一唯一正常保存，不冒充保存/恢复实测。最近复验同日；重新启动、原料/功率/输出边变化及接通燃料需求后必须复验，不把六个有缺口的原生统计窗当十分钟连续实验或完整区间守恒。

2026-09-12 / EXP-007/027/048/188/208/219/299单源激活复验（`validated`限连接保全/初步供料）：raw-8f7cb40c原生源3866终态50217017，root raw-2b49285f于50236079独立核3866实体/7566互返边/257详情、真实863槽7持有新分拣器、六普通过滤及实际满供、21带纯酸、完整玩家只扣一普通、原J84保持；1491输入酸3、900合金68，源仓595。这是恢复既有J45合金产线，不伪造新首次，不把源库存下降或一次68件快照当作可持续供给。

30执行/27审计离线及入口/解析通过；最近复验同日，accepted9保留，主档49805037未覆盖施工。原生Overseer接口只提供600tick固定窗口，720tick合金批次可使个别窗为零；因此事前固定六个互不重叠窗口、酸/合金各至少两非零窗且所测合计各≥8并要求输出仓净增，所有零窗和缺口保留，不凑连续十分钟或把不完整窗口总数当完整区间守恒。该短窗恢复证据仍不能核销稳定燃料/全基础负载/完整Foundry；正常保存恢复和后续持续供需各自复验。

2026-09-12 / EXP-007/027/048/188/208/219/299五下游空载酸连接复验（`validated`限正常施工及旧厂保全）：raw-f25ef346建成五个2011/filter1116，root raw-5640dbf9于50188066核3865实体/7562互返边/241详情、真实双端实槽、1491槽0持有3861、普通grade/STT和实际net3满供。原21酸带及新五件均空载、旧端点/选定纯水/J84保持，两网只增五消费者，玩家仅净−5普通。74审计离线及两入口/解析通过；accepted8保留，主档49805037仍未保存本批。本次241详情有明确集合边界，不冒充十写全审计。

最近复验同日。后续只用剩余一普通接863槽7→3840，供酸源最后激活；旧成功配置不拆改，所有prepare/terminal/双端/材料仍fresh。先查Journal而非假设“恢复产出=首次”：1107的真实自动首产已为J45/tick11172619，酸已有J28，燃料棒目前只有J82手搓。900是历史混仓、仍有空格，未来1107出口必须显式过滤；正常工艺进入仓不是物品注入，也不是持续下游需求已建立。源激活、真实生产、保存恢复和满基础负载/Foundry/准备目标分别提供证据。

2026-09-12 / EXP-007/188/208/219/299空载酸连接预检复验（`validated`限六个实际native正例）：raw-35e8bd2e于50130899核六个exact_slots/零offset/过滤1116/每个一普通预算，优先验证的3860→1491槽0虽靠近塔3仍正常通过。几何距离只能帮助选择有界候选，不凭距离猜测原生碰撞；各取货点在实际10.5m覆盖内也不能代替施工后的powerNetworkId/serveRatio证明。26相关详情、完整玩家/J84和两网保持；没有联合槽位预留、施工或供酸。52预检/69执行离线及71 Core/MCP回归通过，最近复验同日。

当前普通grade1基础STT200000，必须按该目录乘实际span核验，不能套用2012的100000基础值；五个空载下游成功且复核后才另行接863源。五对象执行器显式绑定原六正例的前五个、当前owned session/revision、精确预算/端口和fresh token，任何失败即停，只累加accepted而不因预检或状态汇报归零。7800 J/t是包含既有合金炉的操作启动预算，不是全厂满基础负载或持续燃料证明；源激活、保存/恢复或十写边界须再次复核。IFX-112的残留LASTEXITCODE再次误报52项离线调用失败，修正外层为PowerShell终止异常判断后离线独立完成；实机六次预检仅执行一遍，没有据包装器错误重放动作。

2026-09-12 / EXP-007/188/195/208/219/231/299实际源带完工复验（`validated`限六NEW正常施工与精确旧厂保全）：Luna raw-37909530的B1/D/E三动作各正常消耗两条带并成功终态，root raw-aef6ae12于50100697核全3860实体/7542互返边、188相关详情、21酸带空载/自由端、完整玩家净−六带、原J84和两网满供。实际只有源3844旋转改变；允许变化集合只含源3844与直接邻居3843，且必须匹配Plugin按原生路径推导并核验entity/collider后的原成功读回，其他旧旋转一律精确。不能把“原生允许转向”扩展为任意旧路径旋转豁免，也不能把合法源转向当作必须重建旧线。

41项审计离线正反、两入口拒绝及解析通过；最近复验同日。新窗口accepted3/revision21保留，本次为三施工阶段复核，不是新的十写审计。余五带/六普通、主档49805037未覆盖新施工、863无出口；后续六个过滤分拣器仍须真实端点fresh原生预检、双端持槽和实际受电证明，供酸出口最后激活。21条空带不是持续合金/燃料或完整Foundry门通过，后续连接、保存恢复或十写边界再次复验。

2026-09-12 / EXP-007/188/195/208/219/299实际尾端续接复验（`validated`限native prepare）：raw-502d1786于50032105证明3844→B1两NEW及D/E各两NEW，三预检均通过，原七条B带仍空载且未改变。23预检/38执行离线与13旋转Core通过，下一仍须fresh原生预检/真实扣料/终态，不能把预检的whole_path_native_rotation_v1声明当成已施工的保全证据。源带执行沿用原生entity/collider及路径推导比较；root完工审计只允许该声明覆盖的源/直接邻居旋转变化，其余旧路径需保持精确，后续需再次读回验证。最近复验同日，前一十写已由3f38ef6及绿色CI核销；当前只读记录不重置计数，新的写窗口在下一批准执行器中明确记录。仍未接六分拣器，非持续酸/合金/燃料或完整Foundry验收。

2026-09-12 / EXP-007/188/195/208/219/299酸输送首段十写复验（`validated`限15正常空带/旧厂完整性）：raw-7f0c5e13完成A/B/C的3/7/5 NEW，root raw-8558f485于50010451核3854实体/7534互返边/685详情、完整玩家与J84、旧料路纯度/真实持槽和两网满供。新对象必须按精确位置/有向边归序，A和B实际ID均不单调；不能以排序后的ID代替物理路径。十写跨重启时，旧save采用关闭前原进程已经fresh验证的终态；当前九动作仍逐个fresh核对，不把旧ID拿到新进程猜结果，也不把内置重存多计一次。

51执行/71审计离线、84带契约/Core及21动作客户端检查通过；accepted10/revision15，较恢复点净−6带/+6普通，余11带/六普通。最近复验同日；本审计推送且CI绿才允许下个计数窗口，主档49805037尚未覆盖新对象。真正自由端、完整旧水路、已通过蓝图/Governor均保持，但空载带不是供酸、持续燃料或三级Foundry证据。未提交B1/D/E/分拣器，所有后续仍按实际端点fresh预检，不重做这15带。

2026-09-12 / EXP-007/188/219/299有限酸输送备料闭环（`validated`限正常器材与守恒）：raw-5954abe8的15铁/6电路取料和三项直接配方均正常terminal；root raw-1f9466a9于49957962核全3839对象/7510互返边、52相关详情/实际持槽、完整玩家净增九带/六普通、原J84及两网。46项离线正反及入口/解析检查通过，accepted2→7不归零、revision实际9，不能假设一动作只推进一次revision。备成26带/六普通不等于整个现场可施工、硫酸已送达或持续燃料成立；下一原生齐料fresh预检仍不可省略，三个独立带段后即达十写冻结。完整详细读回仅在对应审计边界复用，不把本次52对象详情声称为667详情全审计。最近复验同日，主档仍49805037，其他Foundry/准备/最终包门不变。

2026-09-12 / EXP-007/188/208/219/299酸到合金有限候选复验（`validated`限五独立带预检与两取料预检）：raw-e6a43f91的A/B/C/D/E原生全路径NEW3/7/5/2/2通过；真实槽位几何纠正后的raw-6e1f9ec2保留B1未来父带cover及六分拣器未证明边界，不把几何对齐、目录速度或电塔覆盖算成实际输送。22带/六普通上限、7800 J/t操作启动增量与完整Foundry满负载预算分开，既有水/油/石/钢/钛线不改。

raw-9c969bd8的15铁/6电路板取料预检通过，固定三普通配方可把17带备成26带/六普通；39带路、51备料离线和41相关Core通过。原成功保护记录之后的嵌套整数键显示再次触发IFX-121，只修安全显示/加往返并复读原记录，不重放游戏调用。集合本身Count不等于各记录count，必须逐项投影检查原生路径预算；早期离线误判无游戏调用。首次目录漏planetId见IFX-122，15模拟守卫与实际修正读取通过，不能把调用方STALE_STATE当世界变了。

最近复验同日。accepted2/revision1/J84和主档49805037保持；下一仅本记录推送且CI通过后的两取料/三手搓，最多到accepted7，不建造/供料/保存。未来父带、实际端点、原生全预检与真实消费者受电都必须在对应施工前后复读；这不是硫酸→合金已通或持续燃料/完整三级Foundry完成。

2026-09-12 / EXP-001/002/007/128/188/195/219/299活跃水路同档恢复复验（`validated`限持久性/短窗供料）：raw-5b003aae正常关闭/Steam单启、raw-1e22e15a实际228安装哈希与64/1/68175 MCP认证通过；唯一resume-f6753dc7从49805005正常恢复并重存49805037。root raw-e5347554于49816764核全3839实体/7510互返边/667详情、75水带/七普通分拣器/旧纯料路线及真实持槽/受电保持、首读位置差0m、完整玩家和原J84身份/历史/84durable，accepted2不因新session或审计归零。

源码指南已前进时，恢复脚本先证明运行代码与b806cf1无差异，仅允许精确0886f07中已审阅的指南段及六测试差异；所有安装文件仍逐一比原清单，MCP资源比不可变安装来源的指南，不比更新后的工作区，也不免除实际握手或声称已冷部署。raw-38cf063b恢复后三个独立600tick酸窗口各4=24/min，863仓428→432→435→438；15/26/20tick缺口不拼接，末读49821256。12生命周期/17恢复审计/15窗口离线、15入口与解析检查及13恢复/Journal Core通过。

最近复验同日。主动水路的货物可以正常移动，后置条件从旧空载阶段改为observed纯水或空，但精确配置/双端/原支线/物资与真实供电不放宽。863无出口和上游供给/库存趋势仍需验证，短窗恢复产出不等于持续燃料、P=C配平、全厂满基础负载或完整三级Foundry；后续仅接未完成下游，下一连接/十写/部署再验。

2026-09-12 / EXP-001/002/007/188/195/219/299通水现场保存复验（`validated`限普通保存及同进程核销）：Luna raw-fdfe89d0唯一save49805005正常终态；root raw-5a69b352于49813613核原始/fresh终态一致，全3839实体/7510互返边/667详情、观察到的纯水货物和七普通分拣器真实双端/供电、完整玩家与原J84全部耐久身份/历史、两网保持。12离线和4入口解析通过；新窗口accepted1/revision20不因独立审计归零，保存不耗建材、不重放已完成施工。

最近复验同日。新恢复能力只证明可进入后续protected resume，不证明已经跨进程恢复；后续必须使用此次主档49805005的fresh票据，不能沿用49350923旧门槛。安装态仍为原同批，源指南较新必须分开陈述。863尚无出口，不能用短期酸库存上升或正常保存核销持续硫酸/合金/燃料、满基础负载、完整三级Foundry与最终包门；下次正常重启后复验身份、全部配置/物资和实际货流/产出。

2026-09-12 / EXP-188/195/219/299实际供水和酸产出复验（`validated`限三个独立600tick窗口）：raw-0b45319f的三个原生窗口49764596–49765195/49765221–49765820/49765880–49766479各产4酸=24/min，whole local factory唯一r24为861，物理输出863由123升至128/131/134，末读49766565。3838/3839实际携水且861水缓冲6→8→8，纯水物流/真实受电/配置及原J84保持；酸buffer及输出仓的实物变化独立支持统计，不把库存差强行当同tick native计数。

窗口前/间缺口31/25/59tick不计入1800已观察ticks，不拼接十分钟或长稳态。862石矿库存下降，861另有油5不足下一批6的截面；后两统计窗水/石消耗0不等于没生产或物料凭空增加，缺口、投料/完成时序及物理缓冲必须分别保留，不能据短窗宣布P=C配平。没有手动供料/新动作，863无下游仍是酸→合金的硬缺口。15观察离线、4入口解析及13保存离线/4入口通过。最近复验同日，2c178e2/精确CI成功后旧十写窗口关闭；只批准接下来一次正常保存，再核恢复和持续供给，完整0.4门不变。
2026-09-12 / EXP-007/128/188/195/219/299入口施工与十写复验（`validated`限当前普通动作/材料/拓扑）：raw-5b06cec5的3839于49743160正常完成，source3829→3772终于接入纯水，source/target实际空槽及2011/filter1000/真实供电均读回。root raw-f63622f3于49755342核一protected resume和九build共十原始/fresh终态，全3839实体/7510互返边/667详情；全部窗口仅三带/一塔/七普通消耗，新对象11个且无重用，所有既有水/石/油/钛钢及其他已观察纯流、旧配置与两网保持，原J84完整身份/历史/84durable不变。

62项离线与4入口解析通过。保护审计成功落盘后，末尾materialNet的Int32键被ConvertTo-Json拒绝，见IFX-121；独立读取原落盘记录，修显示为字符串键并加往返测试，内部整数计数不变。不能把stdout格式错误等同游戏或已完成审计失败，亦不能隐去首次exit1。最近复验同日；accepted10/revision19仍冻结，只有本结论推送及CI通过后关闭计数窗口。主档49350923尚未更新，接线与纯水可观察不代表持续制酸/燃料、全厂满负载或完整Foundry门，后续独立窗口及保存恢复仍待。

2026-09-12 / EXP-188/208/219/299单入口启机边界复验（`validated`限fresh现场/预算）：raw-aa7f38db于49715752证明最后3829→3772原生2011水过滤预检和105相关对象、完整玩家/J84/两网保持；剩一普通足够，旧133覆盖真实取料点，仍保留14100 J/t当前操作余量而不核销全厂满基础负载。离线夹具原先遗漏864，先只读raw-64fee877补齐861/863/864实际状态再纳入27项正反测试，不从缺字典项推断API字段缺失。四入口解析通过，零commit。

启机将是本窗口accepted9→10，须独立十写审计后才能增加任何动作；只提交这一个新入口，不移动/保存/追加设备。后置cargo从“下游必须空”转换为“全部仍可观察且只能是水”，配置/双端/供电/全部物资守恒不放宽。末端861已具12油/16石矿，864与空863的输出保持，不能将这些库存或native预检当水已到达/持续硫酸。最近复验同日，首次通水、十写、保存恢复和持续窗口再验；原三级Foundry、燃料及最终包门保持。

2026-09-12 / EXP-007/188/195/219/299五后缀与共享槽实机复验（`validated`限本次空载施工）：0886f07/CI通过后，Luna raw-0354b536只建3834–3838；root raw-20fd063a于49692790将原两成功和本五成功逐个关联原始/fresh终态，核全3838实体/7506互返边/209详情、原J84全部历史和背包净成本。3832/3833保持不重建，3802/3827既作来源又作目的仍逐次fresh核新空槽，861真实slot1持有3838。六普通STT均按同物品目录核验，实际network3/ratio1，不用电塔null网号、缓存目标或理想周期代替受电/拓扑。

55后缀与71组合审计离线、4入口解析、21 action-client通过；当前源码128相关Release测试仍适用。accepted9/revision17、17带/一普通/零塔保留；原18水源带纯水、72下游带空载、旧水支线不变。最近复验同日；最后水源入口必须单独fresh预检并成为第十写后冻结审计，不能从空载接线推断通水、酸或燃料持续性。primary仍49350923，未保存/重启，完整0.4门保持。

2026-09-12 / EXP-007/188/219/299普通分拣器计时与成功前缀复验（`validated`限当前目录/原生构建/双端核销）：raw-acc60e67的3832/3833已分别正常终态，但调用方误用2012计时枚举停止，见IFX-120。inserter的progressRequired为原生STT，不是耗时；必须结合本物品inserterSttRaw/grade和1–3跨度。真实2011/base200000/span3的600000对应理想满供单件周期120tick，既不是卡住也不是实际吞吐证明。两份真实样本、合法跨度和19负例共24项离线通过；源码包内指南和六个MCP指南回归已吸收此经验，原安装态尚未更新。

root raw-74ff7821于49657031独立核两原始/fresh终态、全3833实体/7486互返边/202详情、17带/六普通/零塔和原J84全部耐久历史。3778/3799原空slot4持有新3833，所有旧边/配置、原水支线、两网及72空载带保持；新塔纯节点null网号仍正常，新消费者明确network3/ratio1。27私有审计离线、7解析/入口及26Core/102MCP回归通过。accepted4/revision7不归零，旧脚本revision5不作为当前身份水位。七步执行器已退役，下一只续未提交五只；不得为修断言重建成功对象。最近复验同日，后续接水、十写、保存恢复及目录/游戏版本变化时复验；未通水/保存，满基础负载、持续燃料及完整0.4验收门不缩减。

2026-09-12 / EXP-188/208/219/299普通维护与全厂峰值证据分离复验（`validated`限native预检和固定执行边界）：raw-87462a02证明最后3829→3772水过滤连接及新塔native正例。单塔只读场地完整检查无并网/新覆盖旧消费者，仍真实保留970199 J/t全基础负载缺口。FoundryPowerContracts明确full-base为advisory、不是原生建造条件或token；普通有限维护仍fresh原生放置、正常成本/无人机/逐对象终态，不可据此放行完整Foundry计划或核销满负载门。

将施工和启机分开：本次只批准一塔加六个无进水下游分拣器，真实首连接暂不提交；明确1800 J/t新消费者额定值、14100 J/t当前操作余量、无新旧消费者误激活和实际双端持槽/供电核验。共享源/目的对象不能沿用此前独立预览hash或视为槽位预约。21预检/65执行器离线、八入口解析、144相关Core及21 action-client通过，零游戏写。最近复验同日；每一实际施工及首次供水/十写/保存恢复后复验，accepted2和完整0.4目标保持，不能把预检和操作余量标为运行产率或全厂峰值通过。

2026-09-12 / EXP-128/188/195/219/299水源三NEW实际续接（`validated`限该正常施工及即时核销）：raw-398d7cf3于49496648只建2298→3831→3830→3829；root raw-6da1c2ee于49510555独立核全3831实体/7482互返边/161详情及三带正常成本。旧源15→18，旧2299支线保持，72下游带仍空载隔离；仅2298的原生朝向变化，按Plugin成功终态的path/collider证明和精确完成读数接受，不把所有源路径旋转都豁免。实际ID必须按批准点/拓扑排序，不按数值大小排列。

41离线正反、三入口拒绝/解析、146相关自动回归及21 action-client通过；原J84完整耐久、玩家剩17带/七普通/一塔、两网成员/当前满供保持，accepted2不归零。IFX-112调用方再次误用LASTEXITCODE，但首次41离线输出有效、无live调用；修正检查后只做独立只读审计，不能据包装器错误重放已成功施工。最近复验同日；原支线/材料/端点/实际受电在后续连接、十写及保存恢复时复验。本段未保存或接入酸链，不核销持续供水、完整启动/满基础负载或燃料门。

2026-09-12 / EXP-128/188/195/219/299已建源带三NEW后缀预检（`validated`限当前源/齐料/native计划）：raw-6c434036于49450966核原15条纯水路径、2298自由物理出口和92详情；原A三NEW原生完整预检再次通过，whole_path_native_rotation_v1、正常三带成本及原2299支线保持。22请求/预算/原生保护/电网/离线禁写正反、4入口解析、146相关自动回归和21 action-client通过；只批准推送/CI后一次fresh正常续接，不把旧计划当权限。

这一步保留独立72带和所有七个未建分拣器，只扩展原水路的自由末端，不构成对新酸链的供料；不要求新三带空载，但必须observed纯水、无额外连接、旧支线不变和完整材料守恒。完成后逐对象核销原生旋转与真实尾端，再补尚未存在的首分拣器预检；不能从位置草案推断未来实体ID。最近复验同日；实际source cover/货物/端点和下一写入时复验，accepted1保持，满基础负载/启动预算及持续燃料未核销。

2026-09-12 / EXP-188/200/208/219/299真实水端点与纯节点语义复验（`validated`限六原生预检）：raw-10674513在首个prepare前暴露私有守卫再次把882纯节点的null网号当错误；已存在的IFX-105和包内playbook本应先落实。修正只允许实际2201/power-node几何，不填造网络归属、绕过消费者受电或满基础负载检查；真实null及非法身份正反纳入28检查，3入口/解析通过，无游戏写入。

raw-528f59ad于49406476取得六个实际2011/filter1000/exact_slots正例，末端861真实空slot1，13详情/完整玩家/原J84和两网保持；144相关Core及21 action-client通过。计划取料点落在五处既有塔和一处未建塔候选的几何覆盖内，不证明未来新消费者实际入网。共享3802和3827两次出现，必须建成前一只后fresh核后一只，不能把独立预览当槽位预约；实际源A首连接仍待，不外推完整可执行供水或持续燃料。最近复验同日；施工、实际持槽/受电、源端与供电预算变化时复验，accepted1保持。

2026-09-12 / EXP-001/002/007/128/188/195/219/299空载水带恢复复验（`validated`限同档持久性与受控恢复）：旧十写审计1fde25f及CI成功后，Luna raw-38024110正常关闭并Steam单次启动；228安装/native引用一致，raw-636661fd实测同批MCP64/1/68175和精确新Plugin，未重装或换档。唯一resume-456a7b3b把正常主档49350892恢复并重存49350923，root raw-693d5d4f于49369060核全3828实体/7476互返边/626详情及新旧原始终态，72条新带/六个空载内部连通分量、旧供料/实际持槽/受电和完整物资保持；首读位置差0m，原J84身份/完整历史/持久水位及两网保持。

跨重启核销使用退出前原终态，不能向新进程重查旧action或把旧token带入恢复后施工；新增accepted只有一次resume，自动重存不是第二次Agent写入，accepted1与revision1仍不因审计归零。12生命周期/17恢复离线、9解析/入口、13恢复/Journal Core及21 action-client通过；本次只读恢复证据不宣称通水、持续酸/合金/燃料或全厂满基础负载通过。最近复验同日；下一实际端点/启动负载、货流或十写时复验，未来接线失败不得重建已保存的72条带。

2026-09-12 / EXP-007/128/188/195/219/299空载施工、保存及十写复验（`validated`限本阶段材料/拓扑/普通保存）：Luna raw-78b1d8d4九个独立正常建造终态完成72NEW水带，第十个动作save49350892成功；root raw-0e01f8b3于49378475逐原始/fresh核十唯一终态、全3828实体/7476互返边及626详情。路径方向按原生坐标和实际连接确定，不按实体ID排序或把共享父带重复记为NEW；三处本阶段新父带cover均实际完成，72条带形成六个空载内部连通分量，新增132有向边，没有旧世界外边。旧3756实体、七组原料货物/真实持槽、玩家净扣72带和原J84全部身份/历史/84durable保持，两网未新增电力成员。

128项原始/合成离线核销、186项相关Core/Contracts/MCP及21 action-client通过；accepted10与revision34分开记账，审计落盘推送且精确CI成功前不关闭窗口。原source A、七分拣器和新塔都未施工，新带逐条observed为空，故本次不激活新生产负载；正常保存不是恢复或通水证明，更不能据瞬时满供核销全厂满基础负载。最近复验同日；下一同档恢复、实际端点/供电接入、货物变化或十写时复验，成功72带不能因新的激活计划失败而整段重放。持续酸/合金/燃料与完整三级Foundry及其他版本门仍待。

2026-09-12 / EXP-188/195/219/299空载施工与自然资源变化复验（`validated`限前置证据）：raw-ef663b55复读全3756实体/玩家/J84/两网。root批准九段72NEW普通带加一次保存的有限空载阶段，明确不建水源A、七分拣器或电塔；新增电力组件为0，不把瞬时满供当满基础负载通过。完工须逐原生路径、唯一ID/材料、无外边/零货物及原世界保持核销，任一拒绝停止、不回放整阶段；85项私有离线与186项相关自动回归、21 action-client通过，实际施工另验。

86石矿机自然少202时不能全局跳过资源检查。raw-f86c7abe结合原202的INVALID_ENTITY、197/201同组15/单矿机且正余量，才将这个精确当前基线三点改两点；位置、朝向、配方、电网和连接保持，原三点理论速度不能继续沿用，精确耗尽tick未知。资源产物来自实际yields而非猜字段/矿种，失败响应null result不当成功节点，原错误独立保留，关联IFX-119。最近复验同日；激活负载、矿竭、节点/槽变化和十写/保存恢复时复验，不以空带或两节点存量证明持续酸/燃料产量。

2026-09-12 / EXP-007/021/103/208/299只读收集器与燃料启动复验（`validated`限记录边界）：raw-94517132在3079原生读取成功后因私有OrderedDictionary整数位置索引失败，不能将本地收集错误称为游戏拒绝或重放已接受动作。root以普通Hashtable统一Int32键、唯一ID/32详情上限和导入前显式模式修正；六项离线正反及入口/解析通过，raw-7b2716b7只补固定目标/一跳后缀，共22详情，末读49221324、accepted0/revision15/J84/primary48796250。短函数名也可能命中既有别名，使用明确的命名函数，不重造临时调用层。关联IFX-118；最近复验同日，新增收集器或ID来源类型变化时复核。

同次证据分别解释库存、功率及未完成门：3074逐格重氢20需汇总为580，不能拿单格值当全仓；3079的0 joules_per_tick只表示无当tick发电，不是燃料件数。3403仍缺合金，1491仍缺酸，已有环输出10及旧供给不重做。49220688→49221298两次电网满供且成员一致，仅证明相隔610tick的两个瞬时点，不能抵扣全厂满基础负载负例或宣称连续供应；55项电力/燃料目录Core回归通过。当前无施工或有限投料许可，下一root先明确启动负载方案，实际接水、激活负载、燃料库存/功率变化及保存恢复时复验，不把故障定位当作产线完成。

2026-09-12 / EXP-188/200/219/240/299完整场地与满负载预算分离复验（`validated`限本次原生预览及负例）：raw-1dfe2b0a、raw-1a1fba04十段native场地正例只证明当前空地/真实2298源；未来C1/C/H cover必须先有实际父带，再用fresh源hash。十段原始NEW计数中三处共享点只计一次，实际候选75带、库存92，63原始/合成检查验证共同端点/预算/新点互撞，不替代未来分拣器槽和真实输送。nullable sourceObjectId=null表示无源，沿用已有Assert-FDPath的显式数值归一化；不能把它误报成原生失败或要求把不存在实体写成另一种身份，关联IFX-116。

原生单塔场地通过仍不等于供电方案通过：raw-fe032b71当前Foundry全厂满基础负载容量449056、预留1455200、既有缺口1006144 J/t。headroomAfterPlanPerTick是非负余额，缺电时为0；不得从0再减新2100负载而把总缺口误写成2100。正确有符号预算重新按容量+新发电−已有预留−出口−新基础负载−新增覆盖旧负载−本次额外负载计算，raw-cf910384得−1008244 J/t；原私有headroomAfterSevenSorters=-2100明确弃用，关联IFX-117。全厂满负载门未放宽，下一需先明确既有发电燃料/启动链功率，不拿瞬时consumerRatio=1或未来发电理论值当现有余量。最近复验同日；实际启动、施工、功率/燃料变化和十写触发复核，当前零游戏写入。

2026-09-12 / EXP-128/208/219/299直接手搓预算与十写窗口复验（`validated`限本套件正常材料/动作/旧现场保持）：raw-ec70a470五配方按30齿轮→90带→七普通→两线圈→一塔执行，99铁/7电路/2磁铁/1铜正常消耗，保留原2带并余一件正常批量线圈。原生收据和完整背包同时核销，不能只看最终92带而遗漏中间材料。root raw-97e8d291于49086887核十个原始/fresh唯一终态、3756实体/7344互返边/463详情、七组纯料路径、原J84和两网；53原始/合成与四入口/解析通过。

本次五手搓的accepted逐项6–10，而revision依次7/9/11/13/15；revision是现场版本，不等于accepted动作计数，不硬填“每次恰好+1”。此前一次未接受的STALE_STATE单列，原resume计一次。十写审计后仍须落盘/push/精确CI才关闭窗口；本阶段未保存/建造，不证明供水、硫酸或燃料。最近复验同日，后续完整native放置、实际端点、十写或保存恢复触发复验，原成功线路不得重做。

2026-09-12 / EXP-128/208/219/299明确拒绝后的单步后缀闭环（`validated`限本次取料守恒与非重放）：原三取料成功、第四commit明确STALE_STATE且未接受后，只把未执行的723取1铜交回Luna。新raw-0ccc85ac/9d5ca2e7于49053470成功、100→99；root raw-0350fe11于49057594同时保留旧拒绝和新原始收据，核四个fresh终态、3756实体/7344互返边/52详情、完整背包净量/原J84/两网。最终accepted5含原resume，不把五次commit尝试误算成五次取料，也不把两个raw前缀伪造成一批原始成功。

30组合证据正反、24原部分审计回归与五入口/解析通过；后续五手搓73离线检查不作live声明。本次早期工作目录错误发生于脚本启动前，不代表原生拒绝或消耗；执行始终指定仓库根，不能因本地命令exit1重放已接受动作。最近复验同日；下一实际手搓、十写审计、实体施工或重启触发复核，原料齐备不证明场地、供水余量或持续燃料供应。

2026-09-12 / EXP-128/208/219/299有界材料阶段的明确拒绝复验（`validated`限接受/拒绝及成功前缀核销）：raw-d21056bf三次普通取料取得99铁/7电路/2磁铁，第四次1铜commit明确STALE_STATE且无actionId，不是outcome_unknown。accepted由1到4；不能把提交尝试次数4当新增accepted4，也不能从本地进程exit1推断此前三步回滚。root raw-edce0a9f逐原始/fresh终态、3756实体/7344互返边/52详情、全背包count/inc/原J84/两网核销成功前缀，铜仍0。原生错误只说明背包或整仓状态已变，未定位具体物品，不把目标铜数量的单点稳定当成整仓hash稳定。

后续只允许独立的未接受单铜步骤，复读当前session，使用当前背包/仓哈希及新的prepare/token；原三步不进入后缀执行器。raw-09c68235 fresh单铜预检通过，24审计正反/三入口、16后缀正反/三入口、21 action-client通过。预检不是预约；第二次同阶段拒绝须停交root，不开放第三次盲试或放松state hash。最近复验同日，实际单铜接受/拒绝、五次手搓及十写触发复验；尚未构成施工、供水或保存证明。

2026-09-12 / EXP-128/188/200/208/219/240/299供水分支及有限材料复验（`validated`限源身份、两段原生场地及四取料预检）：raw-2922c1a9全3756对象/45详情证明当前水源752→753→2300→2284–2298，保留末级2299向761供水。尾带空闲物理输出和纯水货物不证明有可分配余量；另一候选2246同时有水/塑料/油过滤，不可因其中一个水过滤器就把整路当纯水。共享862石矿已从恢复时186增至697，当前空格不等于长期水格，明确采用独立861入口，不改原仓或已成功供料线。

root raw-b0fe7fbf/raw-b528b6f1的北侧接收/东侧接近各两NEW带通过完整原生场地，但尚无实体，分拣器实际槽/朝向仍未知。离线发现旧带1230/1231、塔2411及853/854占位后只调整未执行候选，不以中心粗筛替代native碰撞。raw-175c3c89十段/七连接、≤91带/七普通/一塔为有限上限，正常五配方预算明确到中间齿轮、磁线圈及一件批量余料；与实际未来native NEW数量分别核对，超预算不能暗中加步。

raw-b9a9490b四取料原生预检、24详情/全玩家/原J84/两网保持。59私有正反、三入口/解析、21 action-client与15 Core几何通过。只准四次fresh prepare/commit，原accepted1计数继承至5，不夹带手搓、施工或保存。预检不联合预约，材料不足时必须补齐后再做整条路径完整预检；七个预测端点须等待唯一真实NEW对象再逐个核验。最近复验同日；材料取回、任何场地拒绝、新连接、供水窗口或十写触发复验，持续硫酸/燃料和三级Foundry门保持未完成。

2026-09-12 / EXP-001/002/007/128/188/208/219/299石矿油路恢复复验（`validated`限同档持久性和调用方核销）：十写审计e3ba15c及CI成功后，raw-f0513cd0正常关闭并仅经Steam启动一次；228安装/native引用匹配，raw-f66aa6ca实测安装MCP64/1/68175和新Plugin认证，无运行时代码变化所以不重装。Luna唯一resume-2ec06216正常恢复/重存48796250/J84，root raw-a712991d于48821669核3756实体/7344互返边/438详情、三石矿/四油连接及37/33带、旧钛钢环和所有实际持槽/受电；原玩家count/inc、首读位置、Journal完整身份/历史及两网保持。

调用方规则补充：跨任务机械复用先解析，再核对字段业务身份；JS替换文本中的美元特殊序列须用回调返回字面量，不能让原文尾部被展开或把steel指标命名成stone。JSON数字读成Int64后不能原样索引Int32键的Hashtable；先检查合法数值范围、转为同一键类型，不能以缺少cargo字段推断对象不可观察。IFX-114/115保留首见与修正；四替换回归、12生命周期/18恢复正反和7入口/解析通过，13恢复Journal Core只覆盖产品策略，不冒充私有脚本回归。首次raw-43b8efb1已读438详情但核销未完，不算通过；修正后独立全读通过且无额外游戏写。

accepted1/revision1，861石矿16/油12/水0明确了下一供水缺口；到货缓冲和恢复保持不是持续硫酸/燃料证明。后续水路不得仅凭原共享仓现在有空格就假定长期可供，须明确独立输入或有界库存分配并重验原配置；当前未批准改原仓。最近复验同日；供水/酸出口、持续窗口、十写或部署变化触发复验，完整三级Foundry和其余版本门不缩减。

2026-09-12 / EXP-128/188/200/208/219/299石矿三连接到货/保存/十写复验（`validated`限真实端点、运输到达和当前保存完整性）：Luna raw-431eaac0三只普通filter1005分拣器3754/3755/3756正常终态，原95slot9和862slot4实际持有新对象，3751共享源/目的使用不同原本空闲的真实带槽，原1573及866保全。861于48796187收到6石矿后正常save48796219；root raw-801d5a56于48805799核十个原始/fresh终态、3756实体/7344互返边/438详情及全玩家净量，石矿input16。整窗由三手搓、三带塔施工、三分拣器及一保存构成，不能只核最后四写；手搓中间品和施工材料必须累计后与窗口初始全背包count/inc独立对账。

69审计原始/合成正反、三入口/解析、21 action-client及25端点Core通过。原J84身份/全部历史/84durable、两网满供、25既有供料分拣器及化工原入口/新消费者受电、明确六类带货物纯度保持，无重复扣料或额外拓扑变化。第十写审计推送及CI成功前不归零；保存后的恢复和持续耗料另验。石矿仓可以积压，6→16的化工缓冲只证明送达，不能推出硫酸/合金/燃料生产或源供给稳定。最近复验同日；同档恢复、水与酸出口接入、持续窗口和下个十写触发复验。

2026-09-12 / EXP-128/188/200/208/219/299石矿实际跨线端点复验（`validated`限三原生预检及有限执行守卫）：raw-367bd84e于48730863以实际3750/3751和当前95/862空槽取得三个普通filter1005/exact_slots正例；跨旧1573不需要改其配置或覆盖旧实体，原866可作为已证明的末级石矿入口。取料点到原98/新3753为7.868/5.595/2.747m，不能用目标端有电代替取料端覆盖，塔存在也不能代替新消费者完工后的真实受电。18详情、完整玩家/原J84/两网保持；accepted6/revision56/primary48083246未变。

52执行器原始/合成正反、三入口/解析及25端点Core通过。三独立预览不构成联合预约；只批准逐笔fresh连接、唯一NEW实体和原本空闲端点互返核销，后续共享3751必须以上一步新增边更新基准。仅在861实际石矿输入为正后保存；有限观察未到货则停止未执行后缀，不重放已经建好的三只。一次到货只证明物流到达，不等于持续生产/配平/恢复；第十写后先全审计并推送CI再恢复计数。最近复验同日，实际连接/货流、十写和保存恢复触发复验，水及酸出口和完整燃料门保留。

2026-09-12 / EXP-128/142/188/208/219/299石矿跨旧线施工复验（`validated`限正常施工和完整性）：Luna raw-0a399aaf正常建成3737→3749→3750、独立3751→3752及3753塔，三原始终态证明4带/一塔材料消费和五NEW，无瞬建、覆盖1573或重放原33带。root raw-57e57f18于48717203核当前六终态、3753实体/7332互返边/435详情，原406与施工121等精确取并集；源cover增加两条物理带边、独立两带增加一条边，合计六条有向货运边，电塔不计货运边。实际仅3737朝向变化，不能把原生source cover一概判错，也不能忽略其他旧旋转/拓扑。

新增塔已进原主网，实际只增加一节点，消费者/发电机及另一网成员保持；但尚未施工的两只新分拣器不能据塔存在就算受电。37石矿带由真实动作ID目标取并集并复核为空，不从数值ID范围推断方向/成员；原料线路纯度、25供料分拣器与设备持槽、完整玩家count/inc、原J84完整身份/历史/84durable和两网满供保持。29原始正反/三入口/解析通过，accepted6不归零；primary48083246未覆盖此施工。下一三个真实端点按原98/新3753的实际取料覆盖逐一fresh预检，供石矿与持续酸/燃料另验。最近复验同日；三端点、实际受电/货流、十写及保存恢复触发复验。

2026-09-12 / EXP-007/128/142/188/208/219/299齐料后的石矿跨线场地复验（`validated`限三手搓及真实齐料预检）：已推送十审计与成功CI之后，Luna raw-d373f694正常执行齿轮/普通带/普通分拣器各一批，4铁/1电路净耗和3带/1普通净增分别由原始终态与完整玩家证明，最终6带/三普通/一塔，J84保持。root raw-6440cc70于48665243核三原始/fresh终态、16详情、全玩家/原Journal身份及历史/两网，再用齐料库存对两短带和电塔重做三项完整原生预检。18原始证据正反/三入口通过，不拿旧预览token或缺料结果替代当前预检。

下一源3737的实际上游已有33带，必须按64的明确上限读取整条旧链，不能继承旧19/21段时的32限制。29有限执行器离线/三入口/解析与70带源、原生阶段及连接Core通过；只允许两条NEW源续接、两条NEW独立接力和固定电塔，旧1573/原过滤配置不动。源及直接前驱朝向以正常完工证据核验，其余旧链不放宽；三真实过滤端点和实际消费者受电须后续独立读回。accepted3不因本次审计归零，primary48083246未覆盖石矿施工；齐料/场地通过不是施工或持续投产。最近复验同日；各施工终态、端点/供电接入、十写和保存恢复触发复验。

2026-09-12 / EXP-007/128/142/185/188/208/219/299石矿补料与十写完整性复验（`validated`限实际材料与十写核销）：Luna raw-3a25edfe两取料终态48615811/48615953分别证明723扣4铁、26扣1电路，accepted到10后冻结。root raw-8e579e6f于48626313核之前五备料、三批33NEW带和本次两取料的十个唯一原始/fresh终态及预算；将三批施工的独立逐对象/原生朝向证明与当前完整3748实体/7326互返边/406详情对照，不向新进程查询旧action、不省略先前备料或因为只读复验归零。完整玩家总净变化为铁+4、电路+1、普通分拣器+2，33新带已按三施工全部扣除，原带/塔/其他库存与inc保持。

33石矿带仍空载，旧油/钛/铁锭/磁铁/涡轮纯度、25供料分拣器受电和设备持槽、原J84完整身份/全部历史/84durable以及两网成员/满供保持。72原始正反、三审计入口/解析和21 action-client通过；无源的独立首段在私有严格模式fixture中显式写source=0，不访问不存在的键或放松实际源检查。十审计落盘/推送/CI后才重启计数；新窗仅三次显式普通手搓，当前三配方及完整4铁/1电路预算经27离线/三入口验证，逐批齐直接材料，不把原生可递归制造与私有不递归核验器混用。primary48083246未覆盖石矿施工，审计/补料不是保存或持续供应。最近复验同日；后续手搓、齐料场地/真实端点施工、十写及保存恢复触发复验。

2026-09-12 / EXP-007/128/142/188/208/219/299石矿跨线替代方案复验（`validated`限几何排除、原生场地/取料预检及执行守卫）：raw-7eb72884以当前Core端点函数同时检查源角、目的角和相反方向误差，六条现成尾部带至861三个空槽均不能直连；不是重试旧3737→861。离线直延候选与旧1573重叠后，raw-4d8240ff改用保留旧线的2NEW源续接、2NEW独立接力及过滤分拣器跨接，再送原仓862空slot4，复用866/filter1005送入861，原865/867保持。不能为通过角度检查拆改成功线路，也不能把离线中心排除当原生场地检查。

原858至新取料点11.20m已超10.5m，新方案预算包括一现有背包电塔，同时覆盖两个取料点并在原网连接范围内。电塔DTO的空powerNetworkId不是无供电证明；按当前catalog半径作几何规划，完工后仍以真实消费者网号/供电比核验。完整石矿分支需4带/三普通/一塔，加原化工/四油分拣器共14100J/t保守峰值；只差一带/一普通，叶材料4铁/1电路可正常得到三带/一普通，不能绕过实际齐料后的fresh预检。

raw-d8cb13eb于48551693通过两短带与电塔三处原生场地、两取料预算预检；16详情、全玩家、原J84身份/历史/durable及两网保持。独立预检不构成联合预留，未来实际两分拣器仍未预检；33已建带未改、accepted8/revision42/primary48083246保持。44取料离线正反、三入口/解析及25端点Core通过；JSON键序不是语义身份，取料预算按有序步骤及source/item/count逐字段比较，整数键记录沿用证据安全序列化。只准两次取料到十写冻结，先全审计再批准手搓/施工，不借补料重置计数。最近复验同日，齐料后的完整路径/端点、十写、真实供料或保存恢复均触发复验；持续石矿/酸/燃料尚未通过。

2026-09-12 / EXP-128/188/208/219石矿设备端负例复验（`validated`限原生前拒绝与零写保全）：raw-27001970在入口95→3733正例后，实际3737→861遭12候选/零nativeChecks/no_finite_projection拒绝。传送带整段可放并不能证明其尾端与设备空口朝向适配；不能把TooSkew误报成已检查地形/碰撞，也不因缺少分拣器就补带或改旧设备。root raw-5a260a23于48473366核14详情/全玩家/原J84/两网/零预建筑保持，accepted8/revision42/primary48083246没有变化。

三预览入口、一只读审计入口及25原生端点几何/拒绝诊断Core通过；具体修复候选尚未确定。保留全部33已建带，先落盘明确blocker再由root读取真实端口与带姿态重设计；Luna不重放同一端点，不把上一入口已解决的负例当作本次末端已解决。最近复验同日；新候选须独立fresh预检，同一阶段第二次同类拒绝后停止类似尝试，不第三次盲试。石矿到货/持续酸/燃料与保存恢复另验。

2026-09-12 / EXP-128/188/208/219/299石矿顺序源续接实机复验（`validated`限2+12NEW施工及保全）：Luna raw-baa24ed5先从3717建3736→3735，再把实际3735作为第二个fresh源，正常建12NEW至3737；原生全路径/源绑定/NEW预算均成立，未独立重建13条、猜号或重扣源材料。root raw-2e4847ae于48459207核三原始/fresh施工终态、3748实体/7326互返边/406详情。第二个源是在本批新建的，审计必须按有向路径逐步加入预期图，不能假设两源都在旧基准内；朝向范围由每步当时的源与直接前驱推导为3717/3716/3735/3736，只允许原生完工证明的变化，其余旧对象精确保全。22原始正反/三入口通过。

全33石矿带仍空载，玩家只少14普通带、余3带/两普通/一塔，25旧供料分拣器和设备槽位、明确物料纯度、原J84及两网保持。accepted8/revision42不重置，primary48083246尚未覆盖施工；源续接通过不等于分拣器、供石矿或持续供酸/燃料通过。下一两端须绑定真实3733/3737与当前95/861槽位fresh预检；到十写再核全部十终态，不能把本次三个施工终态核销冒充十写全审计。最近复验同日，两个连接、十写与保存恢复触发复验。

2026-09-12 / EXP-128/188/208/219/299石矿入口及有条件源续接复验（`validated`限真实入口/走廊预检和执行守卫）：raw-509aa112于48358833将空仓口9接现成第二带3733，2011/filter1005/exact_slots原生通过，取料点距98塔7.868/10.5m；保留第一条3734空上游段和原859，无需增加预算。它核销上一条的替代候选待验项，不把旧95→3734负例改为成功。raw-77fceed7于48366381证明完整末段13点几何可放，但首点未来才成为源；实际执行必须先完成两NEW转角，再以唯一真实尾端fresh预检12NEW，不能复用独立13点token、猜源ID或重复扣源材料。

两步执行器复用已验证FD-Path、完整stage1与whole_path_native_rotation_v1；逐步核原19/21空带、仅源及直接前驱的原生证明朝向变化、NEW点位/互返边和材料，第二步失败保留第一步。34离线/三入口/解析和70相关Core通过，合成第二源fixture只测守卫、不充当live证据。PowerShell模式快照必须是不同名字而非仅大小写不同，原J84身份/历史边界须首写前复核；本次都在执行前落实。accepted6/revision38/primary48083246未变，两实际续接和两个分拣器尚未施工，861入口与持续供矿/酸/燃料另验。最近复验同日，真实施工、十写或保存恢复触发复验；关联IFX-106/113。

2026-09-12 / EXP-128/188/208/219/299石矿仓口重设计（`validated`限当前几何负例/零写核销，后续候选为`observed`）：raw-6a699f7f中3717两NEW转角通过，但95→3734的40个exact-slot候选无DSP检查，后备微调无有限投影；不能按最后TooSkew文本把它解释成真正DSP碰撞，更不能移动玩家或重复相同端点。中间对齐的slot10实际由859占用，空侧口9/11偏角33.4°；规划必须把占用与端口朝向一起算，不能只看仓中心与带中心是否对齐。raw-f3dab6f3于48339863核零commit、11详情/全玩家/原J84保持，19带不拆改。

原始angle字段因整数Min/Max重载错误被raw-b5e71e92明确作废；改为double比较的夹取、7已知角度/2退化拒绝/实际候选共10离线后，空slot9到现成第二带3733为9.67/9.66°，仅是一个需fresh原生预检的不同候选。只改变尚未施工入口的目标，保留第一条空上游带、旧859和33NEW/两普通总预算；同类第二次拒绝后停止，不第三次盲试。accepted6/revision38/primary48083246，14后续带和两个分拣器均未建，持续供矿/酸/燃料与保存恢复不获证明。最近复验同日，真实候选预检/施工、十写或恢复触发复验，关联IFX-113。

2026-09-12 / EXP-128/188/208/219/299石矿首段施工复验（`validated`限19NEW及原厂保全）：Luna raw-52d3cc21的c057245b于48308221完成19条正常无人机带；root raw-1240a547于48320465核唯一原始/fresh终态、3734实体/7298互返边/373详情和全玩家/原J84/两网。原281详情、备料16和施工/邻居97须在live前精确取并集373；17离线/三入口通过，不复用上一阶段的详情数量上限。实际路径坐标顺序3734→…→3718→3716→3717，ID不单调，后续source必须取已验证有向尾3717，不能排序或猜号。19带只扣19，原配置/连接、25供料消费者受电、关键持槽及五类旧货物保持；accepted6/revision38、余17带/两普通/一塔，primary48083246尚未覆盖。只有空载施工正例，后续14带/两连接、真实石矿供应和保存恢复另验；最近复验同日，后续连接、十写与恢复触发复验。

2026-09-12 / EXP-128/188/208/219/299石矿首段复验（`validated`限19NEW完整预检及有限执行守卫）：raw-0126068c于48267165以齐料库存取得首段19NEW完整stage1正例，位置、无source绑定和19带预算一致；16详情/全玩家/原J84/两网保持。两个三点样本只用于提前识别窄处，不能替代这次齐料完整预检；同理首段通过不证明未来14NEW源续接或两个真实端点连接。29离线正反、三执行入口/一预览入口和32占位Core通过；共用ContainsKey缺省守卫及FD-Path单数组返回约定，原生NEW实体必须逐点唯一对应、双向内部边和两端自由读回。只允许一次19NEW正常施工、accepted5→6，不移动/补塔/改旧线路/投料/保存，不清零既有五写；后续源续接绑定真实新尾端，不能先捏造ID或复用旧token。最近复验同日，实际施工/端点/供矿、十写和保存恢复触发复验。

2026-09-12 / EXP-128/188/208/219/299石矿备料五写复验（`validated`限正常器材生产和守恒）：Luna raw-e48bb002仅两原生取料/三手搓，root raw-c644da93于48247755逐一核五原始/fresh终态、精确预算及两仓即时扣料35铁/2电路，净增33带/两普通，完整玩家count/inc及原三带/一塔保留。16来源/油路详情、861持槽、四油分拣器满供、原J84身份/历史/durable及两网成员保持；31原始正反/三入口通过，不把两次生产中仓库快照当作扣料证明。accepted5/revision36/primary48083246，不重置计数、不冒充全厂十审计、施工或实际石矿供应。

同批IFX-112：PowerShell脚本的成功不能从残留的LASTEXITCODE推断；该值只代表最近一次原生命令。离线脚本已经返回31项通过，外层误判后尚未进入live审计；直接修正调用方执行状态判断并单独执行未开始的只读后缀，不重跑五个游戏动作。最近复验同日；下一完整路径预检、施工、累计十写和保存恢复触发来源/材料/配置复验。

2026-09-12 / EXP-128/188/208/219/299石矿来源与有限备料复验（`validated`限来源、两短段及材料预检）：旧859从128搬石矿入95，但原生filter0对应DTO null；不能用当前持货1005推出filter1005。初次raw-1eda3782在全只读20记录后被私有错误断言终止，未进入prepare/commit；修正为保全真实无过滤配置、精确双端互返、矿点、可用石矿和观察到的纯货物，未来混合仓的新出口另要求1005过滤，详IFX-111。离线使用同一生产检查函数及真实来源正例，含过滤被改、源/目标错配、矿点/库存缺失、缺互返、unknown/串料和错误持货单位等负例，合计37检查及三入口通过。

raw-aa19c3c5两个三NEW短样本只证明当前矿机旁及建筑间隙可放；不得冒充19+2+12完整线路、转角或真实连接通过。raw-4260b975于48217508原生验证两取料35铁/2电路及三配方预算，候选33NEW/两普通加原三带余量，原98/858塔几何覆盖和13800J/t峰值具备；两网、16详情、完整玩家/原J84保持。五步备料的正常动作与后续齐料全路径须分开验，source cover只绑定届时真实尾端；候选覆盖不是实际分拣器受电证明。预览不清零十写计数，本阶段和既有十审计都推送/CI后才授权Luna最多五写；石矿到货、水/酸出口、持续燃料与恢复均待。最近复验同日，实际备料、施工、供料窗口或恢复触发复验。

2026-09-12 / EXP-128/188/200/208/219/299油到货及十写复验（`validated`限33带/四连接/到货保存）：raw-fc6e5b6f四普通2011/filter1114终态成功，861slot6实际3715，48083214收到6油后正常save48083246；root raw-e4bed1b0于48104570核十原始/fresh终态、3715实体/7262边/281详情、完整玩家/原J84/两网，油input12。前一轮raw-ad25568f的私有断言误把1417磁铁1102列入铁锭1101；施工前后原始cargo均1102×3，修正五组明确物料映射，不将中文简称“铁线”当物品ID。原始前/后货物正例和五种串料/缺项负例进入同一生产检查函数，40离线/三入口通过，仅重新只读核验；详见IFX-110。33新油带、46钛带与旧钢铁/磁铁/涡轮纯度、25分拣器受电、所有关键持槽保持，只有四新consumer/16边和四普通材料消耗。accepted10直到审计commit/push/CI才重置；此原生save覆盖当前油路，但没有重启或持续耗料证明。油缓冲6→12只是到货，水/石矿与酸出口未完成，不能外推持续酸/合金/燃料或完整Foundry；下次供料/恢复、十写或窗口验收触发复验，最近复验同日。

2026-09-12 / EXP-128/188/200/208/219油路四连接复验（`validated`限真实端点预检和固定执行守卫）：raw-a4b9936a四个2011/filter1114计划均exact_slots，三桥实际带中心匹配，末端861原空slot6；实际取料位置分别落在已有882/132/2411/858覆盖内并有0.2m以上余量，风机原生供电半径7.7m不能套用塔的10.5m。计划指定节点仅为几何依据，纯节点网号null不等于断电；完工仍须真实消费者powerNetworkId/serveRatio和双端持槽证明。63执行器离线/两种入口各三拒绝、111相关Core通过，14详情/玩家/J84与两网保持，零游戏写。四次独立预览不是联合预约，每次fresh prepare且只续当前未执行一步；不再增加带/塔。当前accepted5，只允许四连接后实际861油到货再正常save至十写审计，一次到货不是持续供给；供料、水石输入、保存恢复及十写触发复验，最近复验同日。

2026-09-12 / EXP-128/188/200/208/219源带续接施工复验（`validated`限当前15NEW及旧厂保全）：raw-cc04221f从实际3679/3696续13/2带，两原生终态48006162/48007516成功；root raw-84cfb36a于48015495核五累计终态、3711实体/7246边/277详情和完整物资/J84。原254详情与施工147详情并集预先精确核定为277；不能凭估计增加审计范围或漏读依赖。只有源及直接旧邻居3679/3680/3695/3696进入已安装原生path/collider朝向证明，实际源3679/3696改变，其余旧朝向/配置/连接仍须精确匹配，不把整段旧路径的任意朝向差异豁免。15NEW净扣15普通带、源零扣料/不拆除，原18带不重做；12审计正反/两入口通过。accepted5/revision19保持，primary47744754未覆盖施工，33条新带仍空载，四过滤连接另行fresh原生预检与实际受电/到货核验。只证明有界正常施工，不外推自动供油或完整三级Foundry；连接、十写、供料窗口及保存恢复触发复验，最近复验同日。

2026-09-12 / EXP-128/188/200/208/219硫酸源带续接复验（`validated`限真实源预检与有界执行守卫）：raw-d000d8f1对3679/3696实际自由尾端取得13/2NEW完整stage1与whole_path_native_rotation_v1正例，源不扣料/不拆除，原整路径朝向必须由原生终态证明；不能在调用方把任何朝向变化无条件当正常。短角先限定最多三NEW及三个候选格，实际返回两NEW后固定这两个点，未来执行不再自行取剩余额度。18详情、全玩家/J84及两网保持，23私有/三入口、70相关Core通过；accepted3/revision15不重置。两个独立预检不预约材料/端点，每次执行重新prepare；只批准15NEW两动作，原18带不重做，四过滤连接/持续供给仍待。施工、原生结果不明、接线或保存恢复触发复验，最近复验同日。

2026-09-12 / EXP-128/188/200/208/219/299硫酸三段施工复验（`validated`限18NEW施工及完整性）：raw-cd99a444三个原生无人机动作完成2/7/9带，修正缺省键请求守卫实机通过，单次接收的有向ID数组与坐标逐一对应。root raw-f83ae0bf核三原始/fresh终态、全3696实体/7216互返边/254详情、旧钛钢环及真实持槽、全玩家count/inc与原J84/两网；只扣18普通带，无串料或新增外部连接。原123与施工148详情并集必须事先从原始证据计算，本次为254；先前凭估计220的只读上限导致停止，不能靠漏掉对象来通过。22审计离线/两入口、39执行守卫及21 action-client通过。accepted3/revision15保留，primary47744754未覆盖新施工；下一两源cover和四实际连接分别fresh验证，不能将三段空带当供油、供酸或完整Foundry。后续连接/产量、十写或保存恢复触发复验，最近复验同日。

2026-09-12 / EXP-200/299与IFX-108油路调用方回归（`validated`限零写核销及离线修正）：root新三段脚本重复直接读取可缺省sourceObjectId，Luna raw-78832bd1在首个原生prepare前停止，不能归因于DSP放置或缺料。root raw-0315d9f0核零prepare/commit、revision9、完整玩家与J84、零预建筑，accepted0。修正必须复用已有ContainsKey守卫，离线测试同时经过生产使用的真实缺省请求形状，不能只测返回plan；39正反/三入口通过，实际施工仍待。console没有raw前缀不等于没有保护记录，先按唯一任务intent定位并核销，禁止以输出缺失为由重放。此复验只覆盖新调用方，不宣称所有私有历史脚本已整改；最近复验同日，下一实际三段施工和原生端点预检触发复验。

2026-09-12 / EXP-128/188/208/219/299硫酸油路独立段复验（`validated`限三次齐料原生路径正例）：raw-0d967c48用实际36带/四普通确认原选三段2/7/9NEW，全路径stage1、预算、首尾和所有点一致；17详情/完整玩家/原J84与两网保持。23私有路径/材料/结果形状正反、四入口及32带占位Core通过。材料齐备后成功的独立空地路径仍不能证明未来source cover、分拣器角度/取料覆盖或联合施工；每段施工前重新prepare，完工按坐标唯一匹配扁平ID及首尾自由端，成功部分不重做。先前十写审计b59f3b3与CI成功才准重置accepted；当前只批准18NEW三动作，不包含移动、投料、保存或其余油/水/石矿/酸输出。真实施工、后续端口/供料或十写触发复验，最近复验同日。

2026-09-12 / EXP-128/142/188/208/219/299硫酸油路备料十写复验（`validated`限器材守恒与当前完整性）：raw-c04a2646两取料/三手搓仅将40铁和4电路变为36普通带/四普通分拣器；每笔原生预算/终态和仓库即时扣料可独立核销，不用生产中的两次仓储快照代替动作内守恒。root raw-75ad7580于47873971核四个退出前旧终态、当前恢复及五个新动作，全3678实体/7186互返边/123具名关联详情、原钛/钢/环连接和设备持槽、完整玩家count/inc、原J84身份/全条目/durable水位及两网保持；27私有正反/三入口与21 action-client通过，accepted10冻结至commit/push/CI成功。

备料没有施工/投料/保存，不以器材库存或几何预算宣称油路原生预检通过；下一必须使用当前齐料状态重新检验完整路径和实际端点，原成功线路保持不动。恢复action没有completed tick时保留空值，由实际恢复/正常重存边界提供证据，不能补造时间或向新进程查询旧action。Journal缺某次首次条目不是历史从未制作的证据，本次保持84而非补记。当前revision9/primary47744754；后续路径/端点预检、施工、十写或保存恢复触发复验，最近复验同日。

2026-09-12 / EXP-007/128/142/188/208/219/299硫酸上游与有限油路预算复验（`validated`限原始观察、源货物及取料预算）：raw-2af134ef三窗由root raw-8769fe6d核82详情和实际计数，钢P2/2/1与缓冲54/56/58分开，1114精炼油P2/2/0/C1/1/2与1116硫酸0分开；51/37tick间隙排除，满油仓600不是持续可分配余量。raw-f7fac65f的27详情确认既有纯油3170、纯水2298、纯石矿87/88与空酸输入，窗口零P不能单独反推矿源枯竭或线路损坏。候选不得穿过旧带中心；保留成功线路，分段接力只是方案，须齐料后原生全路径/实际端点/取料供电复验，不能把0.25m中心排除规则当原生碰撞检测。

当前油支路预算上限34NEW/四普通/零新塔，末尾拐角原生路径未知且最多三NEW；现有882/132/2411/858覆盖候选取料点，整台化工加四普通按13200J/t留余。原生40铁/4电路取料预检和三当前手搓配方可得36带/四普通，23私有离线/三入口、39Core/2MCP/21 action-client通过。只准五备料动作到十写审计，不能顺带施工或把此油切片当三输入及酸输出整链通过。末尾console整数键JSON失败发生在成功证明已保护落盘之后，复用ConvertTo-EvidenceSafeValue并加嵌套配方键序列化回归，不重放操作；这是旧EXP-007的调用方遗漏，不是新游戏失败。accepted5/J84保持，下一备料/原生路径/端点或十写触发复验，最近复验同日。

2026-09-12 / EXP-001/002/128/142/188/208/219新钢线路恢复复验（`validated`限当前保存/安装/连接与Journal持久性）：raw-2090c3b3在47744723/J84及四终态核销后正常关闭、Steam仅启动一次，228安装/native哈希匹配；启动中一次主线程读取超时，同一存活进程后续菜单ready成功，不重复启动或把观察超时当恢复失败。raw-e914fb66实测安装MCP64/1/68175及准确Plugin认证；resume-8d7cd2c2唯一恢复正常重存47744754，root raw-98b760e4核全3678/7186/94、新钢炉/四短带/三连接、原钛/环持槽、全玩家count/inc及原J84完整身份/历史，两网满供且首读位置差仅0.0000037m。12离线/四入口、13恢复/Journal Core和21 action-client通过，accepted5不重置。

恢复终态使用当前新进程原始/fresh证据，旧四动作只用关闭前核销结果，不向新进程查询或重做旧action；后续必须新session/hash/prepare。钛和钢各8、硫酸0明确区分已恢复的输入与仍缺的原料，不能把恢复成功、缓存或设备数当持续合金/燃料产率。下一规划现有硫酸三输入和输出全链，保留成功段；供料施工、十写、持续窗口或再次恢复触发复验，最近复验同日。

2026-09-12 / EXP-128/142/188/208/219/221/299近端钢接线与保存复验（`validated`限实际到货、正常保存及完整性）：raw-d2badf36三个普通exact_slots连接依次成功，3671原生input2/output3虽共享角点坐标仍分别持有3677/3678，合金1491的11持有钢入口3678、旧7仍持有钛入口3670。带虚拟槽按实际双边读回，不能仅从预检、坐标或分拣器自身目标推定设备端未被覆盖。47744695合金原生input1103×8后才普通保存47744723；root raw-0ddbf703于47766235核四个原始/fresh终态、全3678实体/7186互返边/94详情、全玩家count/inc、原J84身份/历史/持久水位和两网满供，旧钛/环及钢暂停保持。52私有离线、129端点Core、21 action-client通过；accepted4/revision25不重置。

该证据不依赖Journal一定出现钢首次事件；本次仍84，不能从缺条目反推未生产或补造历史。原生输入确认实际交付，保存确认覆盖，但单次8件库存和restartResumeAvailable不能替代持续消费窗口或真正重启恢复；尚缺硫酸导致的合金停产不能被“已有钛和钢”掩盖。下一只验证新钢路线正常恢复，再规划硫酸自动输入，不重复已成功炉/带/连接或改旧供料；实际恢复、持续窗口、十写及材料/端口变化触发复验，最近复验同日。

2026-09-12 / EXP-188/208/219/221/299近端钢三端点复验（`validated`限实际原生预检与执行守卫）：raw-3016f41a证明2315→3675、3674→3671及3671→1491的三个普通exact_slots/铁铁钢过滤正例。炉输入2与输出3虽共享角点坐标却是不同原生槽和朝向，必须分别按真实attachment核验；不能因坐标相同就误判同槽，也不能据两个预检跳过施工后的碰撞/持槽检查。合金11空闲，钛7及旧1/2/3/4保持，所有取料点在既有塔覆盖内。52私有端点/Journal/到货/保存正反、三入口及129端点Core通过。仅在十写审计与审批push/CI后执行三连接；用原生input1103确认实际到货再普通保存，失败保留成功前缀。若出现1103首次，必须保全原J84完整身份/前缀，并证明接通后、正count、真实生产寄存器来源及durable水位；没有条目不是未生产过的证明。accepted10/revision18/primary47461589仍保留，尚未建连接或证明持续钢/合金/燃料。实际施工、供料窗口、十写或保存恢复触发复验，最近复验同日。

2026-09-12 / EXP-128/142/188/208/299近端钢施工十写复验（`validated`限正常施工与完整性）：raw-cc505e1b完成3671正常建炉/设r63/四NEW路径，root raw-def96eff于47696224核十个唯一原始/fresh终态、全3675实体/7174互返边/91详情，旧3670配置、46钛带/18供料分拣器及持槽、全玩家count/inc和原J84保持。仅扣炉1/带4，配置无物料转移，网3只增一炉消费者且两网满供；13私有/两入口、46带Core与21 action-client通过。四带原生完工ID顺序为3675/3673/3672/3674，必须以plannedPath逐点唯一匹配，不排序ID或重复包装FD-Path返回数组；本次实际成功复验IFX-107约束。accepted10冻结到本审计commit/push/CI成功，revision18/primary47461589保持；新炉空载、四带首尾自由且未保存，不是已供钢。下一仅用真实新端点fresh预检三个连接，不把两个独立放置正例当连接证明。接线/持续消费、保存恢复或十写触发复验，最近复验同日。

2026-09-12 / EXP-128/142/188/208/299近端钢齐料预检复验（`validated`限原生放置与当前预算）：raw-5dd85ff6的原选炉位/四NEW短带均在实际材料齐备后取得原生正例，11关联详情和原J84/玩家保持；一炉/三普通满速6900J/t与既有塔覆盖核定。13私有位姿/成本/路径/原生钢配方和完整配置哈希检查、两入口拒绝及46带占位/拒绝Core通过。炉位预检与带预检不构成联合占位预约，先建炉后必须再fresh检查四带，未来分拣器不能借空地正例跳过真实端点/槽/取料供电核验；仅批准一炉、r63、四带三动作到十写审计。旧炉暂停不改、旧钛线不动，不注入铁或把空载建设当持续供钢。accepted7及primary47461589保持；施工、真实接线/消费供给、十写或保存恢复触发复验，最近复验同日。

2026-09-12 / EXP-128/188/208/219/299近端钢完整器材复验（`validated`限七动作备料与守恒）：raw-08bced97续四取料和三配方后，root raw-966f0c3d于47639093核七原始/fresh终态、动作内双边扣料及完整预算、86定向详情、旧供料真实持槽/两网、全部玩家count/inc与原J84身份/全历史。只用先前7铁和新7电路/2石材/2磁铁/1铜得到1炉/3普通分拣器，原四带/一塔/六高速带/三合金/20重氢保留；25离线/两入口及21 action-client通过。手搓沿用真实首次事件的有界前缀验证，不能从Journal缺某物品条目推断从未手搓或补造历史；本次实际J84没有增加。accepted7/revision12保持，未普通保存或重复全厂审计；启动材料齐备仍不证明炉位、真实端口或持续产量，下一仅fresh整案预检，不改旧钢/钛线、不复用旧token。后续原生放置/连接、十写、保存恢复触发复验，最近复验同日。

2026-09-12 / EXP-128/188/208/219/299近端钢备料十写复验（`validated`限7铁守恒与当前完整性）：raw-707b347a唯一转移47574246由源723扣7、玩家加7，accepted10即冻结；root raw-7e1ee99a于47599887核八旧终态、当前恢复及该转移，全3670实体/7168互返边/81详情，46纯钛或空带、七新及十一旧分拣器满供、仓/合金/环真实持槽、完整玩家count/inc、原J84身份/全部条目/持久水位及两网保持。791/793历史钢暂停仍保留，不用新备料授权修改成功线路。5取料计划离线/两入口与21 action-client通过，原始/fresh转移收据一致；旧进程收据采用退出前已核销证据，恢复收据无完成tick时从session/save另证，不能编造。仅审计commit/push/CI成功后清零accepted，revision2及primary47461589保持；7铁未保存，受保护审计不是存档。后续只续剩余四项转移和三种配方，不重取铁或继承旧token；完整启动套件也不证明新炉、三个连接或持续供应。下一备料、原生场地/端点预检、施工、十写或恢复触发复验，最近复验同日。

2026-09-12 / EXP-129/137/188/208/219近端钢启动方案复验（`validated`限诊断、有限预算和四带预检）：raw-cce7d795三独立窗/78详情由root raw-03d28773核销，793仍为有意暂停、861无输入、1491仅钛8；满油/铁仓与钢2267不等于持续供给。新炉候选加四短带可避免约52.55m旧钢仓长线及899混合输入，但只有四带和五取料取得原生正例，不能把新炉/三个未来端点视为已通过。完整预算7铁/7电路/2石材/2磁铁/1铜，经r6/r56/r85得到一炉/三普通分拣器；复用四带、现有塔覆盖，6900J/t为原生满速峰值，20钢/60铁每分钟是容量而非所需0.5钢/min的实测供给。24套件离线/两入口、12观察离线/两入口、7统计Core/2窗口MCP通过。实际材料到位后必须fresh检查整案，原炉/钛线不改，当前accepted9保持；未来设备放置、连接或共享铁需求变化触发复验，最近复验同日。

2026-09-12 / EXP-001/002/128/188/208/219钛线恢复复验（`validated`限当前同档完整性）：原47461557/J84主档正常关闭、Steam启动和唯一protected resume后重存47461589，228安装文件及原生引用哈希一致，实际64tools/1resource通过；没有新部署或游戏加速。root raw-e226de64核3670实体/7168互返边/78详情、46条纯钛或空带、七新/十一旧满供分拣器及原持槽、完整玩家/J84历史和两网。恢复审计的目标集合必须覆盖所有随后引用对象：原76详情不含3403/3416，首轮只读停止后显式补入，并以同一完整工厂快照核身份/配置，不把缺失值当零或跳过检查（IFX-109）。12重启离线、两重启入口/七恢复入口负例及13恢复/Journal Core通过；这些Core测试不代替78详情实机复读。旧进程八终态在退出前封存，新进程只查本次恢复action，accepted9不重置；当前钛8而钢/硫酸0，恢复通过不证明持续合金/燃料。下一钢/硫酸供给、十写或部署触发复验，最近复验同日。

2026-09-12 / EXP-128/142/188/208/219七连接钛输入复验（`validated`限真实供料/覆盖保存）：raw-6a696ffb七个钛过滤分拣器按普通2/高速5正常完成，1491于47461537真实input1106=8，save47461557；root raw-326ee54d核八终态、3670实体/7168互返边/76详情、完整玩家/J84/两网、46带纯钛或空载，新七和旧十一分拣器实际满供，仓10/合金7实际持槽正确，原成功线路保持。两处斜拉阻塞经有限直线接力解决，没有改原生角度、搬迁原42带或重放材料前缀；原独立空段3659仍可空载，不能要求不在实际供料路径上的每一段都有货。62私有正反/两入口、129端点Core、13恢复/Journal Core、21action-client通过。先前预览现已补齐真实连接/到货证据；库存8不是持续合金产量，普通save不是重启恢复，accepted8/revision52不重置。下一同档恢复以及钢/硫酸供料变化触发复验，最近复验同日。

2026-09-12 / EXP-142/188/208七处真实钛连接复验（`validated`限原生预检）：raw-8e70afd0在两段接力实际建成后，为七个明确实体对取得exact_slots正例，包含此前只靠几何推导的四个新接力连接；普通/高速各按本类型原生预算核定，仓10槽、合金炉7槽、当前真实取料点和现有风机/电塔覆盖通过。原768→3621及3652→3659失败对未重试，不放宽11°或扩大白名单。17相关详情、完整玩家/原J84和revision37保持，129端点Core/21action-client通过；accepted10不由预览重置。多个独立预检不是联合预约，先施工的分拣器可能改变槽位/碰撞，后续必须fresh prepare并比对已批准姿态和实际双向持槽；七正例不等于已通料、持续产率或保存恢复。下一逐连接施工/供电/真实到货触发复验，最近复验同日。

2026-09-12 / EXP-128/142/188/200/208/299两段接力后缀十写复验（`validated`限46带完整性及私有请求守卫）：raw-97257492仅两笔2NEW终态47409732/47411712，缺省可选端点键通过修正守卫并进入原生预检/正常施工；root raw-2e9e0d13于47417757核十终态、3663实体/7140互返边/69详情、46空带、完整玩家/J84/两网，旧42带及三材料前缀无重放。17后缀/两入口、14请求形状、80带Core/4Contracts、21action-client通过，IFX-108本有限后缀`fixed`，不外推全部旧脚本。汇报首笔tick误抄由原始对应action纠正，转述不能覆盖机器证据；不重做已核销动作。accepted10仅在审计push/CI后清零，revision37/primary47296284保留；新4带未保存，六条空链不是实际供钛。下一七端点实际预检、连接/供料、十写或保存恢复触发复验，最近复验同日。

2026-09-12 / EXP-200/299可选请求键与前缀保全复验（`validated`限三笔材料前缀及离线修正）：raw-c755aee5两转移/r85成功后，首个独立带请求不含sourceObjectId；root附加守卫在StrictMode下点读该键而停止，尚未调用Bridge prepare/commit。raw-21acbeb5核八累计终态、65详情、原J84/完整玩家/两网，accepted8/revision33及42成功带保持，只有2普通分拣器净增。可选hashtable键必须先ContainsKey再索引读取；缺省或明确整数0才表示无复用，正/负实体ID、null、字符串或数组均拒绝。14项使用真实FD-Path请求形状的零游戏测试复现旧错误并核正负例；原26计划测试没有经过请求守卫，不能覆盖此缺陷。80带Core/4Contracts及21action-client通过，不把调用方错误包装成原生TooSkew、隔离或结果未知。记录IFX-108后仅批准未执行的两小段，旧五步脚本不得重跑，已备材料不重取/重搓；下一实际后缀/十写触发复验，最近复验同日。

2026-09-12 / EXP-142/188/208有限直线接力重设计复验（`validated`限当前施工前预算、两带段与实际末端预检）：raw-cd960863的3658→1491/slot7通过exact_slots，仓侧与末端两个2NEW段原生完整放置通过；无需修改42成功带或旧交叉线。以4带、5高速及2普通分拣器形成明确候选通路，r85×2的2铁/2电路叶材料与两源取料预检齐全；按当前原生2012=600/2011=300 J/t累计峰值3600 J/t核现有余量，不给普通分拣器套高速耗电。19相关详情、完整玩家和原J84保持，accepted5/revision29/primary47296284；26私有正反/三入口、129端点Core及21action-client通过。预算完整不代表尚不存在的四处连接已通过原生端点检查；先有限备料/建设，核对真实新ID后再逐连接prepare，不忽略几何/科技/货物/电网或已成功前缀。下一实际建设、十写与分拣器端点触发复验，最近复验同日。

2026-09-12 / EXP-188/208/219/299未接通走廊保存复验（`validated`限42带普通保存和定向完整性）：raw-75b3eb7e唯一save47296284后，root raw-a3485649于47301054核四施工及保存原始/fresh终态、63相关配置、完整玩家count/inc、原J84身份/全部条目/持久水位及两网成员/满供，零预建筑，42带仍为空载。18保存离线/两入口拒绝、13恢复与Journal Core及21action-client通过。保存不会核销未施工连接或连续供给，也不等于已实机恢复；定向审计不能重置十写计数，accepted5/revision29保持。保存推进revision及主档水位后，旧预览/执行草案必须整体重新绑定并fresh预检，不能只替换token或沿用revision28。先保护成功前缀，再修订未完成入口/末端；不重放整条路线。下一连接方案、十写或实际恢复触发复验，最近复验同日。

2026-09-12 / EXP-142/188/208直线跨带与fallback解释复验（`validated`限两原生exact-slot正例；斜跨为`observed`拒绝）：raw-5852fc09证明3630→3623、3622→3624的1106过滤原生连接和现有风机覆盖；3652→3659对角线TooSkew，16尝试/nativeChecks0，fallback仅支持单belt加设备而报告unsupported_endpoint_family。不能据最后fallback一句否定已证明的belt-belt路径，也不能以其他直跨成功推定对角线合法；不扩大类型白名单或放宽11°。仓侧2NEW引导段原生放置通过，但未来两实际连接尚未验证。raw-faf8595b核35详情、全玩家/J84/两网与零预建筑，accepted4/revision28/primary46880194保持；129相关Core/21action-client通过，零游戏写。停止类似候选并先commit/push正负边界，后续仅完整核定明确直线接力及额外材料，不改42成功带；新方案/端口/供电变化触发复验，最近复验同日。

2026-09-12 / EXP-142/188/200/208真实钛入口朝向复验（`observed`，原生选择规则拒绝）：raw-4b00d570的768→3621为TooSkew，最佳24.065°不满足触带<11°，facingPairs0/nativeChecks0；公开原语保持拒绝，没有commit。raw-b082bf48核35详情、完整物资/inc、原J84和两网，accepted4/revision28/primary46880194不变。独立带段full_path_stage1、空槽/覆盖几何与库存充足均不能证明将来分拣器朝向可用；原42带施工结论有效，但不是完整供料。附近998为纯钛而767是钛晶石设备，2580–2584是有机晶体，必须按实际组件/货物/拓扑选源，不把设备编号当角色。初次预览库存helper缺失在prepare前零写终止，调用方改读实际storage/items元数据后才得到朝向拒绝；依赖缺失不能冒充原生失败，也不应靠导入含副作用的历史整脚本补齐。129端点/过滤Core与21action-client通过。先commit/push阻塞，再重设计明确入口，保留42成功带、不混料；新方案及真实连接触发复验，最近复验同日。

2026-09-12 / EXP-128/142/188/200/299钛路线后缀恢复复验（`validated`限当前42带和私有返回形状修复）：dee5beb/CI通过后Luna raw-02cde2f4只做3638续6NEW及独立2NEW，单次赋值/平坦ID检查正常完成；root raw-75633d61于47230618核四累计终态、3659实体/7136互返边/63详情，42带形成8/2/30/2空链，源路径保全、旧厂和完整玩家/J84/两网保持。原34带未重建，末8带只扣8；24后缀/三入口、13形状、136带Core、21action-client通过。IFX-107本次有限后缀实机通过，不外推所有旧脚本已整改；accepted4/revision28保留，不从脚本重启重新计数。五连接尚未施工，空带链不是供料、持续产率或保存恢复；primary46880194保持。下一真实端点/现有电网覆盖、施工、十写或保存恢复触发复验，最近复验同日。

2026-09-12 / EXP-200/299调用方数组后处理故障复验（`validated`限原始成功前缀及离线形状纠正）：Luna raw-334453d8的4/18NEW两笔成功后，root脚本重复包装FD-Path返回的单对象ID数组，下一尾端Object[]→Int32转换失败；没有第三prepare/commit。root raw-6f115e97核两终态、全3651实体/7122互返边/55详情、原J84/玩家/两网，34条当前新路线及3638唯一尾端证明成功前缀不可重放。13项纯表达式正反复现故障并验证单次赋值/扁平正整数ID约束，136带Core与21action-client通过；原41项静态路径测试未覆盖结果组装。调用方应先单次接收函数结果、检查平坦/正整数/唯一/精确数量，再存入外层路径列表；不能靠递归强制展平未知对象掩盖错误，不能重跑前缀验证修复。accepted2/revision24和primary46880194保持，新施工未保存；先commit/push停止边界，后缀仅6+2NEW另行fresh核定、执行和审计。关联IFX-107；最近复验同日，实机后缀尚未验证。

2026-09-12 / EXP-128/142/188/208/299钛线路首三段十写复验（`validated`限12NEW正常施工及当前完整性）：raw-73f19cb3仅路径0/2/3的4/2/6条普通带成功；root raw-e853be6a于47126161核七备料与三施工原始/fresh终态、全3629实体/7078互返边/168详情，旧3617对象配置/位置/朝向保持，新链逐点唯一、空载、首尾自由，旧磁铁/氢链及环持槽/供电不变。仅扣12普通带，完整其余物资/inc、原J84身份/全部条目/持久水位和两网保持。136带Core、36固定路径/三入口、21action-client通过；accepted10冻结至审计commit/push/CI通过才重置，不改变revision20或primary46880194。独立空段成功不能证明共享角续接、未来实际分拣器端点/供电、钛到货或持续产率；下一剩30NEW必须绑定实际唯一尾端fresh完整预检并保留原生源路径/货物/朝向证明，不复用旧token或重复12成功带。审计落盘不等于新带已保存；后续施工、第十写及保存恢复触发复验，最近复验同日。

2026-09-12 / EXP-142/188/208既有风机供电重设计（`validated`限当前原生目录、七段预检和几何范围）：raw-2de627c2确认2203的isPowerNode及7.7m原生覆盖，2201为10.5m；供电候选不能只筛电塔，也不能给风机套电塔半径。raw-d3c8f0fc把第二跨线取料点移入130覆盖，七段预检通过、三共享角去重后42NEW/五高速/零新塔；两个风机覆盖内取料点距3.369/5.156m，其余三点均在既有电塔半径内，当前主网有3000 J/t增量余量。未来实际分拣器的真实取料点、端口和供电仍独立复验，不能仅以几何宣布已运行。纯电塔基本详情的networkId为null是现有读取范围，源码CapturePower仅写消费者/发电机；调用方在该断言处零写停止后修正解释，不把null当断网或新增字段，最终仍须消费者真实网号/供电比验证。40供电Core、36固定路径正反/三入口、8几何和21action-client通过；accepted7/J84、库存及primary46880194保持。此前两个塔位拒绝记录保留，重设计没有第三次塔位试探或修改旧厂；下一仅12空带到十写审计，其余30带/五连接与持续供料另验。最近复验同日。

2026-09-12 / EXP-142/188/208钛线供电候选停止（`observed`，原生拒绝与调用方规划边界）：raw-f60f77e6的七空地带段各自通过full_path_stage1，44点扣三共享角仅是41NEW候选预算；首塔NeedGround，附近风机130的唯一修订候选raw-cea6211f为PowerTooClose。已有风机合法不证明旁边另一电塔合法；球面距离/满电余量也不能替代地形、原生最小间距和实际取料覆盖。两次同阶段拒绝后停止并交root，不让Luna扫描第三点，不修改成功旧厂。raw-44b4fd23复验玩家全库存/inc、原J84身份/条目及revision14/accepted7保持，8离线有界几何/21action-client检查通过，零游戏写入。后续先复核已有节点（包括具有原生覆盖的发电设备）的实际配置，重新设计明确跨线与供电安排，不据此新增观察字段、扩建筑白名单或擅自放宽原生条件；新方案/后续施工触发复验，最近复验同日。

2026-09-12 / EXP-128/188/208/299钛输送器材复验（`validated`限七项正常取料/加工与守恒）：root原生两源预检raw-f101c81a后，Luna raw-40f0a7ae完成4电路/2电机及五固定配方；root raw-f39e63cb核七原始/fresh终态、全部原生预算、57相关对象及十一既有涡轮/磁铁/氢分拣器实际受电，玩家仅按叶材料得到50普通带/5高速/1塔，原6高速带/3合金/20D2完整保留。14预算正反/2入口拒绝/21action-client通过；54铁成功前缀未重放，J84原身份与全条目、两网保持，accepted7/revision14不重置。备齐器材不代表路径可施工、分拣器可跨线、源仓可持续供应或合金已恢复；必须对最终路径、科技、真实端口、取料覆盖及整条预算再fresh核定，独立预览不是联合预约。器材未普通保存，定向57详情不是完整工厂审计；下一施工、第十写或恢复触发复验，最近复验同日。

2026-09-12 / EXP-128/142/188/208/219/299钛备料跨重启十写复验（`validated`限54铁守恒和当前完整性）：raw-6f83c165唯一转移46951035证明源723扣54、玩家加54；root raw-d0fc8fdc于46960203核八旧终态、当前恢复及该转移共十个唯一动作，全3617实体/7060互返边/147详情、67纯磁带、四满供分拣器、新氢入口、环四真实持槽、完整玩家物资/inc和原J84身份/全部条目/持久水位及两网保持。5计划正反/21action-client零游戏调用通过。旧进程收据使用退出前已核销证据，不向新进程重放；恢复结果的completedAtGameTick为空，不据此编造终态tick，精确主档46880194由恢复保存及当前session另证。审计commit/push/CI成功后才清零accepted，revision2不变；54铁未保存，不把受保护审计文件当游戏存档。后续只续两项剩余转移/五配方，不能重取铁或继承旧token；整套备料不等于实际钛供给或持续燃料，下一备料结束、施工、十写或恢复触发复验，最近复验同日。

2026-09-12 / EXP-129/137/188/208/219钛合金上游复验（`validated`限当前停产原因/有限备料）：raw-8b3f370a/root-bb3018ff的3窗/48详情确认硫酸和合金输入空、钢炉暂停，精炼油实际P/C为8/5、8/5、4/5；其他七项所选本地产消为0而多个源仓满载，不能从零产量推断远端断供或矿耗尽。793硅石过滤有历史152e668e主动避免抢铁证据，现有steel2267和iron3000不授权自动清过滤；须把目标需求、原生瞬时满速和上游余量区分。三窗间28/29tick排除，库存和携货是现场证据而非持续产率。raw-845ad4cc只证明54铁/4电路/2电机三项有限取料及原生五配方预算，可把现有2带/1高速/1线圈变为50带/5高速/1塔，原6高速带/3合金/20D2保留；这是下一轮原生现场检查所需的有限启动材料，不是假定最终线路成本已知。13预算正反、2入口、27统计Core及21action-client通过，零游戏写/accepted9/J84保持。备料后仍须完整fresh路径/端点/科技/电力核定，超套件上限先重新规划，不补丁式放宽检查或越界建造；下一取料/十写/现场预检触发复验，最近复验同日。

2026-09-12 / EXP-001/002/128/142/188/208/219新增磁铁与氢通道恢复复验（`validated`限当前同批安装/线路及Journal持久性）：raw-5d418712在save46880162/J84和八终态核销后正常关闭/Steam启动一次，228安装/native哈希保持；启动中session只读超时后，同一新进程菜单ready复读成功，启动等待不当作恢复失败，不重复启动或消费票据。raw-cce2fe17实测安装MCP64/1/68175及准确Plugin认证；唯一384f94e7恢复并正常重存46880194，root raw-5f1c0764核全3617/7060/147、67纯磁铁带、四供磁分拣器/新氢入口双端与实际供电、环四真实持槽、全部玩家count/inc及原J84身份和条目，首读位置差0m、两网满供。13恢复/日记Core、21action-client、12私有边界正反和4入口拒绝通过；accepted9保留，旧八action仅采用退出前原始/fresh终态，不向新进程重放。恢复后必须使用新session/hash/prepare，不能复用旧token。环成品10/燃料台重氢40和环2只证明当前缓冲，合金0仍阻塞，不把恢复成功当连续燃料或最终工件验收；下一供料/十写/恢复触发复验，最近复验同日。

2026-09-12 / EXP-208/219/221/297/299石墨到环与首次产出复验（`validated`限到货加工和Journal新事件）：raw-78639cfd第二独立600tick窗原生1205产1、3404输出6→7，J84于46857791真实追加首次流水线1205；root raw-2e777101核原始48详情和J83完整前缀/身份/84durable。中途没有游戏写入，因此之前缺料观察已被后续原生生产反证，不继续修一条已实际到货的线路。输入buffer采样0可能因为一周期刚被消耗，单段空带及输出sorter瞬时idle不能当断路；必须合看原生产计数、设备工作/成品与完整连接。六热电本次确有两氢/四石墨发电，但石墨P=C没有证明满足环的目标需求或长期配平，fuel1802仍0。私有初步摘要沿用了旧J83，本次以原始84更正，不重跑、不补造；后续保存须绑定已验证的84完整历史，19边界/计划/Journal正反与2入口拒绝零游戏调用通过，仅一次正常save。两6tick间隙不拼接、未读当前电网摘要，保存恢复及持续产率另验；下一供料/保存/十写/恢复触发复验，最近复验同日。

同阶段保存复验（`validated`限原生save及定向持久性）：raw-982df517唯一保存46880162覆盖新氢入口/J84，root raw-2047a156于46882027核79配置/两网/完整玩家count和inc/Journal身份及全部条目，原始与fresh终态完全一致，零预建筑；accepted8不重置。19保存守卫/2入口拒绝、21action-client及27相关Core通过。save前绑定的是已核验真实84，不回退到前一观察的83，也不允许任意追加未知事件；接受收据立即记数，即使随后调用方失败仍不能重放。此处未关闭/恢复，不从restartResumeAvailable推断恢复已通过；只有定向79对象审计，不当作完整工厂或十写重置依据。后续正常恢复、供料变化或十写再验，最近复验同日。

2026-09-12 / EXP-208/219/221/297氢旁路后三窗复验（`validated`限原生短窗及目标设备证据）：raw-c3dd0589的三个独立600tick窗经root raw-59227273核原始计数/速率/单位、48份详情、session及Journal身份/完整前缀。石墨产30/min、耗12/12/18；3058/3059各窗均以氢发出正J/t，两裂解厂工作，不能再沿用入口前“本地石墨零产”作为当前事实。3064氢逐格求和为87/96/104；第三样本20×5+4=104，纠正初步摘要124的加法错误，不改原始证据、不重跑实机。氢库存仍增长，且本星球总耗氢包含裂解原料，不可全部归因于火电，也不可据短期发电宣布长期消纳通过。3428一段空载/3404石墨0只定位下游缺料，不否定已测本地产出；3403合金/环仍0，四项下游产消为0。窗口间10/9tick排除，实际1800tick不是连续长窗；两台热电的通用isWorking=false不覆盖真实generation-current-tick证据。本批未重读其余四台热电或电网摘要，不外推全网状态；零游戏写入、accepted7/revision48/primary46598914/J83保留。持续去路、真实下游供料及正常保存恢复另验；后续供料变化、十写或恢复触发复验，最近复验同日。

2026-09-12 / EXP-188/208/219/299短氢入口施工复验（`validated`限正常建设与真实连接/供电）：raw-22c06cb0的3616电塔与3617氢入口均terminal成功，root raw-9c156872于46843112核77相关详情、正常各1扣料、3337真实belt slot4及3064实际slot7持有3617，旧slot3/9和成功链均保持。网3仅增加1节点/1消费者，两网满供；入口确在搬运氢，原J83/物资/inc保持。24方案/Journal与21action-client通过。accepted7/revision48保留，未覆盖保存；未重复全厂审计，不将两新对象或仓内短暂入货当持续去路。后续须独立产消/库存窗证明火电真实耗氢及裂解/石墨恢复，generation-current-tick仍为J/t而非燃料件数。下一窗口、十写、保存或恢复触发复验，最近复验同日。

2026-09-12 / EXP-188/208/219/299短氢入口与补电复验（`validated`仅原生预检及有限执行守卫）：raw-a5ba1292证明齐料后3337→3064空闲slot7可用；raw-273f7148首电塔落点被3059体积占位拒绝，不能凭中心距离宣称无碰撞。明确一次改位后raw-e46385d0通过，吸附点取货覆盖8.653m/既有电塔连接4.501m，仍需先正常建塔并核电网，再fresh预检同一入口，两个独立预检不是共同预约。24正反检查覆盖预算/槽/偏移/过滤/位姿/供电及Journal完整前缀；首次产环若真实出现，只允许连接后原生production-register的1205流水线首次及完整durable语义，不把正常新事件误当历史损坏；3入口拒绝/21action-client通过。accepted5/revision44/J83未变化，下一固定1塔+1入口，不拆旧物、不清仓、不重放。实际发电/耗氢须量测，按已验证PowerSystem分担语义，风电名义富余既不证明火电零消耗，也不保证火电满载去料。下一施工/十写/恢复触发复验，最近复验同日。

2026-09-12 / EXP-128/188/208/299氢旁路完整备料复验（`validated`限五动作与定向审计）：十写审计push/CI通过后，raw-1fb4a68e只执行剩余铜1及r6/r85/r88/r8四手搓，不重放先前四转移。root raw-1b4044ac于46779690核五原始/fresh终态、各prepare原生预算/动作净变动、47相关详情/旧物理持槽、全部物资/inc、原J83及两网；最终2高速/1塔/余1线圈，原2带/20D2保持。9预算正反/21action-client通过。accepted5/revision44保留，未重复全厂审计、未建新物、未保存本批；完整材料是下一次完整原生现场预检的前提，不代表摆放、供电或持续消费已经通过。下一施工、十写或恢复触发复验，最近复验同日。

2026-09-12 / EXP-128/188/208/299氢旁路备料十写复验（`validated`限四转移与审计）：raw-454875fb四终态只取4铁/2电路/1电机/2磁铁，root raw-3b97d7a2于46728232核十个原始/fresh终态、全3615/7056互返边/103详情、既有供料与设备实际持槽、全部物资/inc、原J83及两网；零新实体、预建筑或隔离。四材料用各自动作内原生扣料证明，不要求流动源仓后续快照仍等于扣料瞬间；原20D2/2带保持。9余料预算正反/3入口/21action-client通过。accepted10仅在本审计commit/push/CI成功后清零，revision35保留；剩余1铜不得重复之前四转移，四手搓只生成2高速/1塔/余1线圈，不含新供料线路或产率证明。下一新原生预检、施工、十写或恢复触发复验，最近复验同日。

2026-09-12 / EXP-208/219/221/297燃料启动依赖复验（`validated`限停产诊断，修复未验）：raw-ef91f4db三个独立600tick窗证明目标七物料本地产消为0，满供的环/燃料/对撞机/裂解厂仍停；石墨0不能归因为入口未接，现有3428→3436→3404已连接但上游无货。root raw-4134ceb4/5280cb29证明重氢仓580/对撞机输出99/裂解氢59、58，3064–3066只向六火电输出氢而无自动入口；不能把这些空仓当持续氢源，powerServeRatio=1也不能当正发电。选择先评估已有3337纯氢带向3064的短入口，保留旧双燃料连接、配置及成功线路，禁止反复清库或把新仓缓存当持续消耗。原生现场/实际供电尚未通过，不将附近煤组32的候选扩成额外施工。raw-4ce68dea完整小套件五来源预检、10预算正反/2入口/21action-client通过；只批准723取4铁、26取2电路、727取1电机、723取2磁铁，accepted6→10立即冻结全审计，余1铜及r6/r85/r88/r8四手搓另批。2高速/1塔/余1线圈只是明确材料上限，不是施工许可；后续实际耗氢、石墨到环、钛合金供给、独立窗口和保存恢复仍须证明。新入口或任何供应变化触发复验，最近复验同日。

2026-09-12 / EXP-188/208/299磁铁到货与覆盖保存复验（`validated`限本次正常连接、实际物流和save）：raw-256de31d四个2012/filter1102正常终态形成3612–3615，46598885设备3404原生input1102=6，之后才save46598914；root raw-1605913e于46604146独立核六累计终态、3615实体/7056互返边/94详情、67纯磁铁带、新旧分拣器受电、真实双边持槽和原J83/完整物资/两网，全部通过。设备slot10持有3615，原3/7/11不变，不能仅凭新sorter自述端点推断未覆盖设备槽。36端点/到货正反、21action-client和5恢复源选择Core复跑通过；执行前2入口拒绝通过，离线检查零游戏调用。accepted6/revision31不清零；普通save只证明当前已建线路被覆盖，不等于重启复验或持续燃料。石墨与合金仍缺，不能由磁铁输入非零推断整条配方能运行；下一施工、十写、恢复或上游供给变化触发复验，最近复验同日。

2026-09-12 / 四磁铁过滤连接与到货后保存边界（`observed`，关联EXP-188/208/299）：8692459及CI34623971451成功后，accepted1/revision22保持。只按raw-dddd6d4c的四实际exact_slots方案正常建2012/filter1102，最后3404空slot10且原3/7/11保留；每步fresh token、真实双边持槽、实际网3满供及背包守恒。36端点/到货正反与2入口拒绝检查零游戏调用通过；四连接后最多30次有限只读采样等待3404原生input1102>0，零输入、output、错误单位/身份/配方或inc不能算到货。实际到货后才一次普通save，到accepted6停；若未观察到，只保留已建连接交root，不重放、不加手动投料或其他动作。raw-7401f692当前仍缺石墨/合金，磁铁到货不外推产环或持续产棒，最近复验同日。

2026-09-12 / EXP-142/188/208磁铁全通道/真实端点复验（`validated`限结构/成本与原生只读连接预检）：raw-091e448f唯一正常尾段与root raw-9108a65d证明3611实体/7040互返边/83详情、67空带、33旧源带和全部旧厂/物资/J83/两网保持；仅3596的正常转角朝向由安装版whole-path原生计算证明，不能将任意朝向变化纳入豁免。raw-dddd6d4c四实际1102过滤连接全部exact_slots，3404空slot10且原3/7/11保留，取料真实位置均有既有塔覆盖；9计划/2入口、21action-client及486相关Core通过。保持accepted1/revision22，不把预检、67空带、几何覆盖或当前峰值余量当已建分拣器/真实供给；后续新token和实际双边槽/电网/磁铁到货、普通保存仍须分别证明。primary46321489尚未覆盖本批，最近复验同日。

2026-09-12 / 磁铁末15带执行边界（`observed`，关联EXP-188/299）：7b56b6d及CI34623085655成功后允许已审十写归零，revision20不变。raw-a7711594以真实3596和完整剩余17带/4高速fresh预检15NEW，确认non-removing cover与whole_path_native_rotation_v1；9计划/2入口拒绝检查零游戏调用通过。只批准这一正常续接到accepted1，保全原33带源链和电塔3479等既有对象，不接分拣器、不改配方、不移动、不投料或保存；实际新尾端出生后再预检四连接。预检成功不能替代逐点唯一实物和后续实际供料，最近复验同日。

2026-09-12 / EXP-128/142/188/208磁铁首52带十写复验（`validated`限正常结构/成本和当前完整性）：raw-5b5f66c8四终态完成7/12/17/16NEW，第四段只复用真实3580共享角；root raw-16bdf378核六备料加四施工共十原始/fresh收据、全3596实体/7010互返边/181详情，三条空链7/12/33、全部旧配置和物资/inc/J83/两网保持。99涡轮带和六实际受电分拣器未改变。26计划正反/2入口拒绝、21action-client与486相关Core通过；push/CI之后才清零accepted，revision20/primary46321489保持，52新对象未保存。新带ID流向不必单调，仍按原生plannedPath逐点唯一匹配与互返边核验；不能用空链、normal cover或库存充足代替后续实际过滤连接/持续供给。后续3596续15NEW仍须fresh整个33带源路径保全、完整NEW占位/材料与无人机终态，新增连接/十写/恢复时复验，最近复验同日。

2026-09-12 / 磁铁通道首52带执行边界（`observed`，关联EXP-188/208/299）：dd7c505已推送且CI34621962214通过；raw-21cf691f在完整69带/4高速、accepted6/revision12下重新验证原五段独立full_path_stage1，26固定路径/预算正反及2入口拒绝检查通过。只批准前三段7/12/17NEW及第四段16NEW，共四次build到accepted10；第四段必须绑定第三段实际唯一尾端，使用non-removing cover/whole_path_native_rotation_v1，不重建或重复扣共享角。末段15NEW和全部四过滤连接留待本次完整十写审计/commit/push/CI之后；旧磁路、硅/油交叉、涡轮路与电塔不改，不移动、投料、配置或保存。独立空地预检不等于联合预约，每步包含刚建对象重新prepare，任何拒绝或无法唯一读回即保留成功前缀交root，不重跑整段。最近复验同日。

2026-09-12 / EXP-128/188/299磁铁有限套件复验（`validated`限正常备料/手搓与守恒）：raw-f35fd725两次剩余转移和四次正常加工完成，root raw-8b947d06独立核六唯一原始/fresh终态、原生双源扣料、完整玩家物品/inc净变、40相关配置、原J83身份和两网。49铁已在玩家，只取4电路/2电机；既有24带加45新带，四基础分拣器全部正常升级配方加工为4高速，原20D2保持。这是手搓建材，不是已有建筑原地升级或磁铁产线产物证明。10私有预算/2入口拒绝、21action-client和486相关Core通过；accepted6不清零，材料未保存。定向审计不替代下一第十写的完整工厂/动作审计；材料齐全也不继承旧token或独立预览的场地通过状态，后续原路线及真实端点重新prepare。最近复验同日。

2026-09-12 / EXP-128/142/188/208/299跨重启十写复验（`validated`限49铁守恒及当前完整性）：raw-905abb39的唯一正常转移以原生终态证明723扣49、玩家加49；root raw-7a8e63a1核八个旧进程退出前收据、当前恢复和本次转移共十个唯一终态，全3544实体/6912互返边/122详情、99纯1204带、六满供分拣器、3404真实持槽/input4、全部物资/inc、原J83及两网保持。21action-client、5计划正反及2入口拒绝检查通过。恢复不清零写计数，也不向新进程查询或重放旧action；退出前原始证据与当前会话fresh终态分开保留。仅完整审计commit/push/CI成功后清零accepted，revision2与primary46321489保持；49铁尚未保存，不能把审计持久化当游戏已保存。后续仅取剩余4电路板/2电机、正常加工完整套件，实际磁铁端点/供电/输送和持续生产另验；新增施工、十写或恢复触发复验，最近复验同日。

2026-09-12 / EXP-128/142/188/208磁铁分支固定规划复验（`validated`限独立原生路径/整段材料预算，执行仍待）：raw-630ac312在恢复后的同一世界为1278→3404固定五段完成full_path_stage1预检，7/12/17/17/16点仅两共享转角，67NEW，跨旧1918硅线与3243油线需四个1102过滤连接；已有电塔候选覆盖，无新增塔预算。raw-b0eb504c按当前r5/84/85/88验证49铁/4电路板/2电机及三真实可达仓储预检，原24带可补到69带/4高速，9材料/1共享角/4入口检查和110相关Core通过。独立空地预检不是联合预约；未来共享角须等唯一实物后non-removing cover，每条连接须fresh真实槽/货物/取料供电证明，不能把库存充足、几何距离或满电余量等同施工/持续供给。旧1217仓6000磁铁不替代上游持续生产窗口；保留已成功磁路与涡轮线路，不挪旧设备。计数仍9，下一49铁转移后先完整十写审计再继续。最近复验同日。

2026-09-11 / EXP-001/002/128/142/188/208涡轮通路保存恢复复验（`validated`限当前同批安装与线路持久性）：d87a0d8/CI34616013802成功后，普通save46321458覆盖此前独立观察到的涡轮到货；root核八终态及228文件/native匹配，正常关闭并Steam单次启动，raw-18ade5a7验证安装stdio64/1和同批指南。resume-092bfb5d只恢复票据精确主档并重存46321489；root raw-74bad650核3544/6912/121、全部99纯1204带、六满供分拣器与设备真实持槽，input4、全物资/inc、原J83和两网保持。13恢复Core/21action-client及8保存边界/6入口拒绝检查通过，累计9不清零。跨进程只保留退出前已证明的旧终态，再对新恢复action fresh复读，不能向新进程重放旧动作。恢复后仍须fresh现场/hash准备后续动作；本次不是新代码冷部署或最终包验收，更不证明持续燃料产率，磁铁/石墨/合金供给及独立窗口另验。最近复验同日。

2026-09-11 / EXP-128/142/188/208涡轮到设备和保存复验（`validated`限真实输送/守恒/普通保存）：raw-f6f013ac四正常过滤连接与save46240070完成，root raw-3158d12c于46259548核七原始/fresh终态、3544/6912互返边/121详情、99纯1204带、六满供分拣器、四旧交叉带及设备旧3/11槽、全玩家/J83/两网。3404真实涡轮输入4、slot7持3544；只扣四高速，余24带/0高速/0塔。110端点Core/13恢复日记Core/21action-client/2入口/28计划检查通过，accepted7不清零。到货是后续fresh观察，不能倒填为保存时同一buffer数量，也不能当持续产率；环缺磁铁/石墨、制棒缺合金/环的边界仍在。protected resume可用不是恢复已验，下一同档恢复后再复读结构/货物，随后新自动输入/独立窗口另验，最近复验同日。

2026-09-11 / 四涡轮连接与覆盖保存执行边界（`observed`，关联EXP-188/208/299）：bdc2a43推送且CI34613852325成功，accepted2/revision40保持。仅依raw-2201f3a5四个真实exact_slots/filter1204方案，依次接3448→3487、3481→3489、3499→3500、3540→3404 slot7；每步重新prepare并核实际双向槽、原端点保留、材料和主网满供，四高速全部正常施工后才普通save。2入口拒绝/28计划正反检查零游戏调用通过；最多新增五accepted到7，阶段结束不清零。不得追加带/塔/移动/手投料/配置，任何拒绝或不明保留成功前缀并停止、不重跑整段。此保存仅覆盖当前世界进度，实际到货和持续产量另由独立复读证明；最近复验同日。

2026-09-11 / EXP-188/208末41带及真实四端口复验（`validated`限结构/成本与只读连接预检）：raw-b23e4b45两终态完成30+11NEW，root raw-58fccc61于46215337核3540实体/6896互返边/58详情、连续空41带链、全玩家/J83及两网；3529共享转角保留，全部旧对象不变。110相关Core/21action-client/2入口/11计划检查通过。raw-2201f3a5四真实端点全部exact_slots/filter1204，终点3404真实空slot7，旧3/11保持；实际取料点供电5.551/6.050/5.551/5.220m均合格，当前原生2012四只峰值2400能量/tick有余量，不外推全厂未来负载。accepted2不清零，primary45973184未保存新增104对象；四预检还不是实际连接/到货或持续燃料，后续仅四连接和正常save，逐对象新槽、材料及供电仍须fresh复验，最近复验同日。

2026-09-11 / 涡轮末段41带执行边界（`observed`，关联EXP-188/299）：raw-f3f393ef在当前3499对象、65带/4高速下fresh原生检查30点横段及12点转弯段，两独立空地段均通过。实际执行只扣30+11NEW，第二段必须绑定第一段真实唯一尾端，以non-removing cover和whole_path_native_rotation_v1保留整条30带，不重复建造共享转角。2入口拒绝/11计划正反检查通过；十写审计796f727已推送，必须等CI通过再清零并仅执行两次build，到accepted2即停核真实端点，不接分拣器/补塔/投料/移动/保存。原生独立预览不是联合占位预约，第二步仍须含刚建30带的完整fresh预检；最近复验同日。

2026-09-11 / EXP-128/188/208后两跨段与十写复验（`validated`限20带及完整性）：raw-d35b92f8两正常终态建8+12NEW；root raw-950e9d6b于46172584核十原始/fresh收据、全3499/6816互返边/72详情、完整玩家inc/J83及两网。新段首尾自由且空载，旧首跨纯1204和三塔保持，只有正常20带扣料。沿流向ID不保证单调，应按plannedPath逐点唯一对应和互返边核验，不能按ID排序解释路线。110带/端点Core、55供电Core、21action-client、2入口/10计划检查通过，push/CI后十写归零、revision36；primary45973184未覆盖新增63对象。剩41带/4高速仍待完整fresh施工，不能用空段或一次到货替代最终消费者/连续燃料；新增端点、十写或恢复再次复验，最近复验同日。

2026-09-11 / 后两空跨段到十写的执行边界（`observed`，关联EXP-188/208/299）：raw-6deb712e在三塔实建后的accepted8/revision32重新原生预检既定8+12带，全路径/NEW预算通过，85带/4高速足够剩余61带/4高速；2入口拒绝和10计划正反检查零游戏调用通过，110相关Core/21action-client保持通过。只允许两次正常空段施工到accepted10，随后完整审计/commit/push/CI才清零，不追加连接或保存；已有38带、两高速及三塔均保留。原生空段通过不能核销未来分拣器端点和持续输送，实际新实体出生后另fresh预检；两阶段都没有源cover或目的合流，禁止擅自绑定旧交叉带。最近复验同日。
...
2026-09-11 / EXP-188/208首处跨线实际输送复验（`validated`限本段物理连接、成本及到货）：raw-f42dbc6c正常建3475/3476，root raw-e39e2755于46059349核五原始/fresh终态、全3476/6780边/48详情、真实设备持槽、完整玩家/J83及两网；新38带仅见1204，跨线后3474及末3448分别有1/2涡轮，旧2408保持且无1204。实际货物与逐对象连接一起证明跨线，不从中心距离、prepare或空带推断；相邻段库存不能相加当吞吐。110相关Core/21action-client/2入口/14计划检查通过，accepted5不清零，primary45973184未覆盖本次40个新对象。未证明最终3404输入或持续燃料，后续只续61带/3塔/4高速，逐个真实端点和实际供电再次预检；施工、十写或保存恢复触发复验，最近复验同日。

2026-09-11 / 首处跨线的两连接执行边界（`observed`，关联EXP-188/208/299）：c65338d已推送、CI34608258287成功，accepted3/revision22保持。仅使用raw-19228232的827→3437与3441→3474实际exact_slots计划，各扣1个2012、过滤1204，逐对象核实warehouse slot10/真实belt槽和网3满供。两入口拒绝/14计划正反检查零游戏调用通过；任一原生拒绝或证据不明就停止，不替换端点、不追加塔或剩余带。连接会正常开始输料，但没有手动投料、保存或将短暂输送当持续燃料验收；实际跨线和到货由独立复读确认，最近复验同日。

2026-09-11 / EXP-188/208涡轮首段复验（`validated`限38带结构/成本及真实端点prepare）：raw-1e07cf90三正常施工，root raw-c1b063d3核三原始/fresh终态、全3474/6772边/46详情、两新空链与所有旧配置、全玩家/J83/两网，只有38带成本。raw-19228232对真实827 slot10→3437及3441→3474得到exact_slots/filter1204，取料点距750/2411为7.036/9.518m；旧2408不参与新链、不被合流或改动。110相关Core/21action-client/2入口/12路径检查通过。accepted3不清零，primary45973184尚未覆盖新带；分拣器未施工，原生跨线prepare不是实际跨线或持续供应证据。下一仅两明确连接，实际失败即核销并停止，不继续猜后续跨线。最近复验同日。

2026-09-11 / EXP-188/208首处涡轮跨线施工前复验（`observed`）：1651512已推送且CI34607158774成功、accepted归零后，raw-38bb67fd在123带/6高速/3塔全预算齐备时重新预检全部七空地段/三塔位，位置与原计划一致。首批仅4+7+27共38NEW、三个accepted；第二段未来从第一段真实尾部non-removing cover续接，2408及所有旧带保留。2入口/12路径正反检查零游戏调用通过，实际端点/过滤接线仍待，不先建完整99带再猜跨线。私有摘要的预算字典用字符串键才能ConvertTo-Json，首次只读打印失败未产生游戏写入；修正后fresh复验通过，不能据打印失败重跑任何已接受施工。最近复验同日，关联EXP-299。

2026-09-11 / EXP-128/142/188/208器材十写复验（`validated`限预算、守恒和正常保存）：raw-984c8598三个取料及三次正常加工补2高速/1塔，save45973184；root raw-a922051c核十原始/fresh终态、全3436/6700边/49详情、完整玩家inc/J83身份/两网。123带/6高速/3塔/20D2已保存，2入口/7预算/21action-client通过，本条push后十写归零、revision16。固定99带通道的四个交叉旧带原样保留，后续先建首个跨线所需端点并fresh核原生过滤连接，再推进其他部分；空地分段预检与整图未来端点是不同证据，不把前者当后者。没有实际新通道或连续燃料结论，新增原生施工、端口改变或恢复时再验，最近复验同日。

2026-09-11 / EXP-128/188/208纯涡轮通道预算（`observed`，未施工）：raw-4d50698c/raw-d384f1e1对明确827→3404通道的七个独立空地带段做完整原生检查，通过99件NEW预算；四处旧带2408/1280/1920/3242保留，用未来带端之间的过滤分拣器跨接而非合流。raw-9b967bf5的三个明确塔位分别距未来取料点5.551/5.551/5.220m、距现有锚6.395/15.649/15.868m，native放置通过；实际分拣器取料坐标、跨度/供电和joint现场尚须建带后fresh证明。当前目录raw-c3f90067给2012峰值600能量/tick，六只新增峰值合计216kW；电塔原生覆盖10.5m/连接22.5m。这是输送新增负载，不保证全厂同时满负荷或连续燃料。现有123带/4高速/2塔仅缺2高速/1塔，raw-62d61df6从723/26/727取4铁/2电路/1电机均在80m内，消耗已有1线圈，r85×2/r88×1/r8×1后材料闭合；2入口/7预算检查零游戏调用通过。仅授权Luna六次备料动作再一次正常保存至accepted10，先严格审计，不施工、不改旧带，不把单项有库存的prepare误当整图材料足够；关联EXP-299，最近复验同日。

2026-09-11 / EXP-128/188/299涡轮备料复验（`validated`仅限有限器材及守恒）：raw-9b10b978正常取120铁、r5×40/r84×40，root raw-c46534fe核三原始/fresh终态、同tick源仓扣120、完整玩家count/inc净增120带、12项相关配置/J83身份/两网。5私有预算、2入口拒绝和21action-client通过，最终123带/4高速/2塔/20D2，accepted3保留且材料未保存。无施工时定向审计明确标注覆盖范围，不把它计作十写全厂审计或清零。既有827持续产线保留，所有后续分段/端口仍须材料齐后的fresh原生预检；本批不证明路线、连续供料或燃料门，最近复验同日。

2026-09-11 / EXP-128/188涡轮输送备料边界（`observed`，未施工）：f0da84d十写恢复审计推送/CI成功后，保留既有827涡轮产线，仅准备独立纯涡轮输送；raw-c16ea6cd的723取120铁预检通过，距离60.6303m，r5×40/r84×40完整预算只产120普通带。现有3带/4高速/2塔/20D2保留，最多3新accepted，无Move、投料、配置、施工或保存。2入口拒绝/5配方预算检查零游戏调用通过；这只是有限器材上限123带，不表示完整线路地形/碰撞/实际端点/供电或持续供应已过，必须材料齐后fresh全路预检，不因缺料预检推断空地。关联EXP-299，最近复验同日。

2026-09-11 / EXP-001/002/128/142/188/208恢复十写复验（`validated`限本机同档结构/材料/日记连续性）：228安装/native哈希、raw-ad6e3c31的实际64/1/68175指南/纯stdout通过，save44252587正常关闭后唯一5b4dbc50恢复并重存44252618。root raw-12ef656f核9旧fresh终态+1恢复、全3436/6700边/102详情/12持槽、全玩家及J83原身份、两网；0.01934mm恢复位移，不向新进程重放旧action。34相关Core/21action-client/4私有入口拒绝/102-ID映射检查通过，本条push后十写归零、新revision1。私有JSON ID应统一为本地索引的int类型，不能用Int64查Int32哈希键后把null误报成实体缺失；这是调用方类型修正，不放松身份/拓扑比较。环输入仍0、D2仓580，恢复不证明连续燃料；新增供料或独立窗口后再验，关联EXP-277，最近复验同日。

2026-09-11 / EXP-188/208/298重设计连接及保存复验（`validated`限结构/成本/有电/正常保存）：raw-b7b7d33b唯一3436使用3428的native_single_belt_segment/inputOffset4→3404 slot11，实际坐标与预检一致、网3满供，原尾3427/19带/全部旧连接保持，无需额外电塔。save44252587覆盖本批；root raw-fe793b6f核9终态、全3436/6700边/102详情/12持槽、完整物资/J83和两网。110相关Core/21action-client/2入口/9私有检查通过。不能把已验证的原生单带微调一概当exact_slots，也不能忽略模式、offset、实际坐标和双边持槽；计划改变必须重新审查。环输入仍0，连接成功不等于真实供料/连续燃料；accepted9不重置，protected resume及后续实际输入另验，最近复验同日。

同日root供电重设计（施工前`observed`截面，后已由上述施工/恢复核销）：4a056f2已推送后，只读raw-f7a12268明确选择现成倒数第二带3428→3404；原生native_single_belt_segment给inputOffset4/destination slot11，真实取料点距3080为10.0191312m，在10.5m内。这复用已有单带微调能力，不是扩大工具/白名单或更改覆盖门；旧19带与尾3427保留，没有新增电塔。后续仅fresh相同计划的1过滤分拣器、完整双端/供电读回和一次save，actual input与持续燃料另验。关联EXP-188/208/298，最近复验同日。

2026-09-11 / EXP-188/208/298供电与源带复验（`validated`限19带与提交前拒绝）：raw-6e5f24c6正常续10+9NEW，root raw-815ccc35于44235197核7终态、3435实体/6696边/101详情/11持槽、原65→84完整源链、全部物资/J83/两网。原生分拣器计划成功不证明供电，覆盖要按plan的实际取货点，不按设备中心或两端中点估算；3427→3404取货点距3080为10.5784055m，虽仅超10.5m约0.0784m仍须拒绝，不容差放宽。未提交分拣器/保存，不把调用方拒绝称native失败或unknown。下一取料点需root fresh预检，已有19带不得重做；accepted7/revision47、primary44157098未覆盖，持续供料/恢复另验。110相关Core/21action-client/2入口/12私有检查通过，最近复验同日。

2026-09-11 / 普通出棒输送与覆盖保存复验（`validated`限结构、成本和保存）：raw-9964d0ec正常建3413→3406的8带及3414/3415/3416三过滤分拣器，3403→带→3079和3404→3403均双向/物理持槽确认；旧氘入口、混仓预约、3081和成功线路保留。root raw-d2a2ddb0于44172522核五终态、3416实体/6658边/20详情/11持槽、完整玩家inc/J83/两网，save44157098覆盖此前全部配方和材料。110相关Core/21action-client/2入口/9计划检查通过，accepted5/revision43不清零，未恢复或连续产棒。手工构造expected连接应按原生slot顺序排列后做完整比较，不能把append顺序误当拓扑变化，也不能忽略新增/丢失边；本次零写审计修正后通过，关联EXP-070/142/188/208，最近复验同日。

石墨续接的下一步边界（`observed`，尚未施工）：raw-97848782以3378为明确源，10+9NEW原生检查通过，第二段水平对齐3404而非沿用旧对角末端，完整预算19带/1高速，实际新端点另验。现有连通上游实际65个2001，私有64遍历上限不能充当完整证明；仅私有helper增加显式1…128读取界限、默认仍64，本计划96覆盖第一次65/第二次75并要求确实到源头，0/129在调用前拒绝。Plugin的完整路径/材料/货物/朝向证明不改变，没有新工具或自动候选循环；新路径增加后重新核上游完整性，最近复验同日。

## EXP-299 — 私有离线模式必须与游戏执行入口隔离

- 日期/状态：2026-09-11，`validated`仅限本次入口修复、零调用离线检查和既有动作核销。
- 适用范围：复用PowerShell私有实机helper；dot-source在调用者作用域绑定param，通用ValidateOfflineOnly等名字可能覆盖调用者开关。
- 结论：模式在所有导入之前快照并校验互斥，使用任务专属名字；默认拒绝执行，真正游戏写入必须显式execute。offline/preview授权不能因导入变成commit；无模式、冲突模式及离线分支须有零游戏调用检查。不得导入整个旧执行入口来获取未声明的副作用。
- 证据：raw-cf346391的五次意外正常动作来自root，不标为Luna执行；中断脚本后root raw-cb0fd919核累计8终态、3405实体/6632边/17详情、库存/inc/J83/满供电及无未知结果。私有修复4入口/9预算检查、21action-client通过；没有重放。已做部分保留，Luna仅续明确未执行的r85/r88后缀，accepted8不清零。
- 限制/复验：本次私有修复不证明所有旧脚本安全，也不改变公开API；每次新增导入/模式、自动化入口或中断恢复时重验。关联IFX-106，最近复验同日。

2026-09-15补充：依赖复用还须保留完整函数声明及参数，正式调用闭包必须在无旧offline预加载的新进程核对，并实际执行正负分支。函数可发现不等于函数体已执行，污染环境中的测试通过不等于生产依赖齐备。当前窄修复与验证见IFX-137；不据此放宽任何游戏原语、幂等或逐对象核验。

同日后缀/十写复验（`validated`）：02dcd76推送及CI通过后，Luna raw-93d63746只执行r85/r88两正常加工，原五步未重做；root raw-fb3d1e72于44129317核全十原始/fresh终态、3405实体/6632边/17详情、完整库存/inc/J83/两网，30普通带/8高速已备齐。2入口/5预算私有检查及21action-client通过；本审计push后accepted10归零、revision34，主档44006653未覆盖新材料。下一施工仍须fresh原生预检，已有8NEW出棒带的独立预览不能替代施工后真实分拣器端点证明，持续燃料门不核销。

2026-09-11 / EXP-070/142/188/208两配方及氘入口复验（`validated`限配置和实物流）：raw-cda23c6a仅将3403/3404设为r41/r103，再建3405，3074 slot9→3403 slot5且1121过滤；两配置实际零扣料、建造仅扣1高速。root raw-1356ca2c在44069452核三原始/fresh终态、全3405/6632边/14详情/6持槽、完整玩家inc/J83/两网，制造台自然收到40D2、玩家原20D2保持。8私有/21action-client通过，accepted3/revision23不清零，primary44006653未覆盖。配置预检AddRecipeBudget仍展示化学计量，不要求空预算或可选target回显；已有规则在本次私有预检被误用，改正后才commit。3403→3079被native TooFar拒绝，不重试相同端点，也不因其他入口通过就施工未验证出路。后续有限带路、环/合金实际供料及连续制棒另验；已有仓格和成功线路不拆改，最近复验同日。

2026-09-11 / EXP-128/188/208十写及原生两机施工复验（`validated`限两空机/保存）：独立候选间隔6.0504m且均受现有3080的native10.5m半径覆盖，第二台仍在第一台实际完成后重新完整prepare，不能把独立预览当联合占位。raw-5a9a8722正常建3403/3404并save44006653；root raw-72cdce66核十原始/fresh终态、3404实体/6628边/22详情/5持槽、全count/inc/J83/两网，只增主网两个消费者且保留20D2，无重造成功链或额外投料。1626 Core/21action-client/6私有检查通过；push后accepted10→0、新revision17，配置/实际端口、消费/自动供料和保存恢复仍另验。当前机器原生0配方、无货、无物流边，不据此称为运行产线；施工/供电/外部连接或恢复变化后复验，最近复验同日。

2026-09-11 / EXP-128/188/208有限燃料器材复验（`validated`限取料/手搓材料守恒）：先用现成723/26的fresh完整预检确认24铁/8电路，再显式r5×16、r45×2，避免把自动递归配方假设写入预算。raw-1eec2672四终态、root raw-2665ad83全3402/6628边/12详情、玩家count/inc/原20D2/J83和两网保持；1626 Core/21action-client/5私有预算检查通过，最终净2个2303。设备分别供r103/r41，不重造已有电机/涡轮链；材料就绪不是选址/端点/自动原料通过。accepted7/revision12不重置，尚未覆盖保存；新址、资源/配方、施工或恢复变化后复验，最近复验同日。

2026-09-11 / EXP-219/220/221容量预约实机复验（`validated`限本次配置/守恒）：全30格只有D2时先lock-occupied，原生取20D2后空格仍受D2预约保护，H不能在两动作间抢占；fresh filter-empty-or-matching只把实际空出的第0格改为H，其他29格不动。raw-e928a01e三终态、root raw-5d1514be全3402实体/6628边/18详情/4持槽和count/inc/J83核销通过；29相关Core/8私有计划/21action-client通过。公开buffer顺序不被当原生格索引，最终索引来自native计划/同步回执；玩家保留20D2，无销毁/反复清库。后续H20确实入仓、D2达580，但输出满再次限制对撞机；容量预约不是需求，须真实燃料消费者才能验证连续供应。accepted3/revision6/primary43874376未覆盖配置，保存恢复另验。混合动作审计应先按actionKind判断是否有配置回执，并核DTO的isWorking字段；只读展示失败不重复成功动作。关联IFX-042/104，最近复验同日；库存类型、输入/输出连接、消费者或恢复变化后复验。

2026-09-11 / EXP-001/002/128/142/208/298部署与十写复验（`validated`限本机冷部署/恢复/指南）：同源b806cf1的228文件逐个哈希匹配，完整Release零警告错误、1799测试；raw-54965a4b验证安装态64工具/1资源/68175字符指南与实际Plugin认证、纯stdout和exit0。普通save43874344、正常退出及唯一protected resume后save43874376，root raw-b400151e核十终态、3402实体/6628边/55详情/17持槽、玩家物资/inc、原J83身份/durable及两网，0.102mm恢复位置差，无重复施工或投料。push后计数10→0，新revision1，当前585D2满格阻塞原样保留，不冒充持续供给或最终包/异机验收。调用方代码生成必须保留PowerShell字面文本：JavaScript replacement字符串会解释`$'`，此次静态解析捕获重复插入正文、零游戏调用；改用replacement回调并通过解析/只读预检后才执行一次。后续同批部署、恢复或生成执行器时复验；关联IFX-105/042/104，最近复验同日。

2026-09-11 / EXP-219/220/221/297复验（`validated`限容量死锁诊断）：六热电接通后raw-4f5e8f39第一组六窗油/H/D2/石墨生产0，root raw-b0a429c3核3074的585D2实际占30格、3347持H/3345纯H尾包、对撞机缺H和裂解H58/59积压、G输出0；不是石墨再次堵塞。输入和产物共仓也须在开放前预约，最后堆的剩余数量不等于异物可用格；已有指南未落实是本次规划遗漏，不新增接口补救。29项仓储Core含585/30格新回归通过，只证明原策略不覆盖非空格及fresh空格预约/守恒，不代替正常转移实测。保持accepted8/revision31/primary43777922/J83、成功新线不重做；下一有限容量预约加真实D2消费者，禁止反复清库或用G的P=C=0宣称配平。原始失败窗保留，3058六次6117J/t、其他五机0，测量G消耗4并非全部不发电；JSON展示整数键错误已转为itemId数组，9项私有观察/实际序列化检查通过，未重跑。关联IFX-042/104，最近复验同日；预约、真实消费与保存恢复后再验。

## EXP-298 — 电塔合网与燃料消耗必须分别按实际证据解释

- 日期/状态：2026-09-11，`validated`限本机原生现场语义、离线指南回归及b806cf1同批冷部署后的MCP读取。
- 适用范围/结论：当前reader的纯power-node网络字段可为null，不能据此判未供电；正常补塔可能合并网络并改变编号/数量。先核真实consumer/generator成员、覆盖、节点守恒和完整拓扑，再更新调用方预期；未知变化仍停下核销，不因私有断言失败重放已成功动作。热电按实际负载分摊发电，机组翻倍不等于耗料翻倍；生产/消耗、库存、其他消费者和事先声明的需求须分别验证。
- 证据/产品化：raw-2c31c4e2证明3350建成、1+3→3且原边保持；raw-06a588ea/2aa36715证明两台正发电仍可堵住裂解。现有MCP power-summary描述与嵌入/包内playbook增加精简规则，新增回归后115项MCP通过，无新工具/字段/写入能力。
- 限制/复验触发：不提供一般自动合网、路径或耗料保证；新增/拆除电力设施、供电或需求变化、更新reader/DSP、冷部署及独立产量窗口时复验。b806cf1安装态raw-54965a4b已验证指南/说明，raw-b400151e证明同档恢复；持续副产物去路IFX-104和共享容量IFX-042未关闭，不能由指南或恢复成功替代产量。关联IFX-105/EXP-188/297，最近复验同日。

2026-09-11 / EXP-188/208/297有限施工复验（`validated`限正常建造/保存）：e9f3006十写审计push后归零，Luna按预检三段12/14/22NEW源续接与四条1109过滤高速至3063/3062/3060/3058原空slot3完成，最后保存43777922；不动原slot0氢入口或原两石墨入口。root raw-c379a3d5核八终态、3402实体/6628互返边/80详情/12持槽、全物资/J83/两网，取货点3357/3394/3386/3377实际受3082/3350/3080覆盖，主网3只增四消费者。48带/4高速守恒后余6/1，原生源路径朝向检查及即时after与fresh相符。121相关Core、21action-client、12私有路径/覆盖检查通过。复用执行器要显式绑定当前两网/主网3，不能套历史网数；accepted8不因阶段审计归零。六台接通及名义33.42MW仍不能证明长期石墨消耗，实际燃烧受真实负载分配，独立窗口、无再投料与恢复另验；最近复验同日。

2026-09-11 / EXP-128/142/188/208十写复验（`validated`限已知失败核销/单塔和原生合网）：raw-07044ee4的2531ffdf已成功建3350，客户端在错误要求纯power-node网络字段后停止，不是原生失败；现行`CapturePower`只为consumer/generator写该字段，旧3080/3082也为null。root raw-2c31c4e2核十原始/fresh终态、3350对象/6516边/29详情/8真实持槽、全物资和J83；其中Move仍按一次已知失败计数，不伪装到达，也不丢弃其足够施工的位移。新塔实际将1+3并成3，124/338/71成员计数与逐对象/站内网络映射守恒，旧网4不变，两网满供；不是“只添一个网1节点”。因此私有后续执行器须显式绑定此次新网3和两网基线，不沿用历史网号/网数；不是放松供电或拓扑检查。125项相关Core、21项action-client和原14项私有连接检查通过。push后十写归零，primary43665411未覆盖新塔；实际新分拣器供电、连续石墨消耗及保存恢复另验，最近复验同日。

2026-09-11 / EXP-128/179/297现场接近复验：唯一Move79e01038于43703396被180tick position_stalled明确终止，余21.636m，未达到历史目标，不标成功、不重放。root raw-b61896d2复读实际已前进约33m、Walk/0/800MJ/healthy，无unknown；当前位置已够到业务目标，raw-3d28b895三段完整native预检与一塔落点通过，NEW预算12+14+22=48带、4过滤高速和1现有背包塔，原54/5/3足量。不为“完成Move”继续撞目标或把失败位移作废。下一仅Luna新prepare建该一塔到accepted10后冻结全审计；实际路径互相占位、新端点/供电和持续石墨消耗仍待，不能用分别预检替代完工证明。125项移动/表面/供电/源带策略Core通过，最近复验同日。

2026-09-11 / EXP-246/253重验：材料足够后的完整带预检明确OutOfReach，不绕过距离门；20m/31m接近候选分别终点水下/末端连续干地2.9968m，按原4m门零commit停止两次类似尝试。root改用raw-cb7160c2的真实历史Walk落点，raw-07619372复读当前近邻最近实体6.444m、800MJ及55.127m正常Move预检。长preview明确unavailable，不能改成干地/路线净空证据；只允许新prepare、1.5m容差、单次普通订单及真实终态/Walk/低速/物资/能量核对，不自动追加恢复或重放。10项watchdog相关Core通过；accepted8、save43665411/J83未变，实走仍待，最近复验同日。

2026-09-11 / EXP-188/208/297修复器材复验（`validated`限备料和保存）：raw-83816f4f三取料/四正常加工/save43665411共八终态；root raw-3c3a4d9c在43674875复核全3349对象/6516边/20详情、全部物资inc/J83、三网成员和满供，净+54带/+4高速，无其他结构/配置改动。9项私有预算正反检查、21项action-client通过；accepted8/revision13仍保留，不以阶段审计清零。第一段12NEW预检后第二段原生OutOfReach，须先明确普通接近再fresh完整预检；材料足够、第一段通过、其他几何推算均不能批准整路。持续副产物消耗、实际新端点和供电仍待，最近复验同日。

2026-09-11 / EXP-297 root重设计准备边界（`observed`，未施工）：374f7be已推送且CI通过后，取消“用正发电代替完整耗料”的假设。优先把现有石墨线从北侧绕到四台闲置热电的空外侧口，保留全部旧氢入口、仓和成功石墨/油/氢线；不改旧设备、不清库存、不增加发电机或无限缓存。直接3307→3065的只读短接被原生TooFar拒绝（raw-4e076b67），不重试相同端点。预计上限54带/4新增高速/至多3现有背包电塔，完整路径/落点/未来实际端口尚未native通过，不能据材料准备开工。raw-9bd25633三源58铁/4电路/2电机均正常transfer预检通过；仅批准Luna三转移、四正常手搓、一次save共8写，得到54带/4新增高速，既有1高速/3塔保留。9项私有配方预算正反检查与21项action-client通过，后续仍按fresh完整场地/动力和连续消耗核验，最近复验同日。

2026-09-11 / EXP-297持续副产物负例（`validated`限本次诊断，修复仍待）：raw-06a588ea第一组六个独立600tick窗43579718–43583472全部南厂未工作、北厂工作；D2仍300→305不能掩盖未达双厂条件。实际测量3600tick、间隙155，采样粗氢56不是净供氢或整个区间守恒。root raw-2aa36715复读3083油2/氢4、石墨20/20、满周期且满供电；既有Overseer raw-3327d6fc同样定位满输出。两台热电有石墨正发电不等于消耗覆盖双厂30石墨/min；原生按负载比例分配，新增机组也不能按当前单机燃烧量简单倍乘。旧三个双出料仓没有自动入口，不称持续氢源。16项燃料/输出堵塞Core通过，无产品代码或游戏写入。保持accepted0/primary43515698/J83；先记录push，再由root设计明确长期需求、完整输送和实际能量预算，Luna停止原方案重复观察/补催化料。先前有限启动/保存恢复证据保留，持续供给门不核销；关联IFX-104，最近复验同日，消耗链变化、独立窗口和恢复后再验。

2026-09-11 / EXP-277调用方类型纠正（`validated`原始类型与离线守卫，重跑观察待验）：raw-730cff4e首窗只读发现163是2101/storage、600精炼油、tankFluidCount=null；165才是2106/tank且明确0。root私有观察脚本错误把两者都要求tank字段，零写即停，不是游戏缺少字段、产线停产或隔离。修为先核object/item/component类型，163仅合计storage/items，165仅使用有效tank字段并在正数时核物料；9项私有窗口/错类型/缺失值检查、17项现有tank Core及21项action-client通过。原尝试没有合格sample/group，不补造通过；下一重新声明独立未来窗口，不修改Plugin/MCP、供给阈值或已有产线。最近复验同日，累计accepted仍0。

2026-09-11 / EXP-001/002/103/188/208双裂解恢复十写复验（`validated`限同档线路恢复）：raw-57397084正常退出/228同批及native哈希/Steam单次启动，raw-a74b863b完成安装MCP64/1/66807认证；唯一61d1dba8恢复43515667并重存43515698。root raw-7176bb8d在43524476复核退出前九个fresh终态、新resume、全3349/6516边/42详情/9持槽、玩家与J83原身份。两r58与r40、空启动仓、155D2和石墨实际发电保持，首读位置差0m，三网满供；28Core/21action-client通过。本条push后十写归零，不向新进程重放旧action、不把保存后旧会话的209D2要求成主档内容、不拼接跨重启产量窗口。尚未验证连续制棒/长期供需，下一只读稳定窗口与真实原料方案另验，最近复验同日。

2026-09-11 / EXP-056/070/208双裂解启动复验（`validated`限配置、有限运行与正常保存）：raw-715b92dd的3083 r58于43512576成功，随后五次间隔≥600tick的双厂工作/油非零/石墨发电/启动仓H0/满供电样本通过，D2从110至120，正常save43515667。没有补投H、移动或新建。root raw-bee2271c于43537237核九原始/fresh终态、全3349/6516边/42详情/9持槽、完整玩家/J83/三网；78项Core/21项action-client通过。accepted9/revision171保留，阶段提交不清零；下一protected resume为第十写，再作恢复/全厂审计。当前D2=209属于保存后自然生产，不能声称已包含在此前主档；恢复前后窗口不拼接，持续供给/实际燃料制造另验，最近复验同日。

2026-09-11 / 南厂有界启动计划（历史`observed`，上方实机仅核有限范围）：继承accepted7，只允许3083 fresh r58配置及观察通过后普通save，到accepted9；不补H、不加建筑、不移动，原北厂/端点不改。按raw-43d1b319的实际发电与332750J/t估算门重新fresh，仍保留原生实际欠供的否决和非全厂理论满载限制。≤7200tick内至少三次间隔≥600tick的双厂工作、石墨发电、D2超过初值并继续增长才算本次有限启动通过；完整样本保留，不把短观察称作持续燃料供应。4项私有配方/容量正反检查通过，后续保存恢复和长期窗口另验，最近复验同日。

2026-09-11 / EXP-056/070/208北裂解启动复验（`validated`有限启动，非持续供给门）：raw-e30d0817仅3084配置r58、784→玩家→3348一次40H，并正常save43472766；三正样本43471417/43472091/43472733均北厂工作、启动仓H0、石墨实际发电、D2到15/20/25、供电比1。root raw-46dc0ab0于43483607核本四写及前序三写、全3349对象/6516边/42详情/9物理槽、全玩家/J83/三网；accepted7/revision168保留，未重启。fresh D2=50已超过原10+40H全部转化最多20的上限，证明存在新制氢贡献，但仍不证明长期供需配平。

同批预算修订：实际采样需求曾达308150J/t，高于开机前284150估算，故撤销其“保守全峰值”含义；当时真实可用357001且供电比1，没有读到欠供。新油需求会同时唤醒旧上游，不能只加新机器额定功率。raw-fa9758d3复读到原油采集/原炼油工作及183氢热电、3059/3061石墨热电实际发电，不能把容量增长全归因于新石墨支路。第二厂raw-43d1b319已按含两原炼油厂额外31200J/t的332750估算重做三次只读，实际393001/余60251；此仍非全厂理论满载预算，必须fresh复核实际负载，窗口欠供不得丢弃。最近复验同日，无补投H/第二配方/持续产率或恢复结论。

2026-09-11 / 北裂解厂有限启动计划（历史`observed`，功率估算已由上方实机修订，非持续供给验收）：raw-795df85f三次递增tick只读均为主网62350J/t实际需求、285002可用容量，原生r58预检通过。仅3084设r58，3083保持r0；初始估算加入对撞机work-idle198000、单裂解厂15600、16条可能活跃高速各450和1000余量，得到284150J/t，不提前记未受料热电。该数遗漏上游恢复负载，不再作为保守上限或产量基线。只批准784→玩家→3348一次40H催化料，再以≤7200tick、三次相距≥600tick的真实北厂工作/石墨发电/D2送达样本验证有限启动并save；观察不足/缺电则停，不重复投料。6项私有配方/预算正反检查通过不等于全厂功率模型已验证，第二厂需另核实际燃料容量，最近复验同日。

2026-09-11 / EXP-070/142/208启动入口保存复验（`validated`限施工/保存）：raw-3a67189d建3348/3349并save43427078；actual source10→belt5且原belt4保留，过滤1120、网1满供。root raw-a5b20324于43436373核三终态/3349对象/6516边/42详情/9物理槽、全物资/J83/三网，78项Core/21项action-client/8项私有检查通过。accepted3保留，完整氢输送已保存但未恢复、配置或启动。raw-848f434d当前目录给出裂解厂work/idle16000/400、对撞机200000/2000、热电gen/fuel36000/45000J每tick；启动须按实际可用容量和新增峰值预算分步，不能先把未收到石墨的热电算作有电。最近复验同日，后续真实燃料/负载/稳定产出与恢复另验。

2026-09-11 / 裂解启动入口有限计划（`observed`待施工）：只施工raw-f9cae402的单一空2101落点，native新仓西侧9/10/11之一经fresh确认为空后，以1120过滤2012接3095的实际空belt槽，保留已有3096输出槽；实际取货点须受1510供电。root批准两个build加普通save共三写，8项私有计划正反检查通过；尚不配置或投40H，失败即保留明确已成前缀并交root，不另换点/重放。最近复验同日，仓位预检不能替代出生后端点检查。

2026-09-11 / EXP-070/145/188/208氢回送复验（`validated`限完整施工）：raw-ccf90f2a四加工/六施工共十终态，36NEW/2高速正常扣料并建成；3074实际新入口slot1、原3/6/7及3085回流保留，两高速1120过滤且借原塔满供。root raw-c49af246于43408100核3347对象/6512互返边/73详情/8物理持槽、全玩家/J83/三网；124项Core/21项action-client/19项私有检查通过。push后accepted归零、revision158；原primary43311976未覆盖新线路，不能声称保存/恢复或持续供氢。启动仓单一预检raw-f9cae402通过，但未来新附件未通过；40H只能作为正常仓→带启动催化料，不能直接填设备、反复投料或称为持续原料。最近复验同日，配置/投料/负载/保存恢复分别另验。

2026-09-11 / EXP-208石墨去路预算补充（`validated`原生算法，非持续燃烧验收）：同哈希DLL重新只读反编译确认，普通发电器按网内实际负载/可用容量比例分摊，再按分配能量正常扣燃料，不是“风电够用就优先风电、热电必停”。因此57风机的17.1MW不能单独判定两台石墨热电不消耗，也不能用两热电满负载额定值承诺石墨全吸收；保留需实际燃料与产量/库存窗口的边界。具体公式证据见Foundry研究文档，未新增字段/写入或改变已成线路，最近复验同日；负载、发电/燃料种类或程序集变化需复核。

2026-09-11 / 氢回送有限执行边界（`observed`待施工）：root只批准r5×3/r84×3/r85×2/r88×1四次正常加工，再建10/4/19/3共36NEW带和两条1120过滤高速；3085旧回流持槽与整条旧带原生朝向证明保留，跨油借3057供电，最终3074仅接受native选择且fresh仍空的北侧0/1/2之一，保留已占3/6/7，不强制覆盖槽。完整原料预算与19项私有正反检查通过；四加工加六施工到accepted10即停审计，不加投料/配方/移动/保存，不把新附件候选当实际已连通。最近复验同日，实际端点、供电和物资守恒仍逐动作核验。

2026-09-11 / EXP-018/185/208氢线路备料十写复验（`validated`限守恒取料/审计）：raw-6ead0ada的11铁/2电路/1电机三笔transfer均有原生源减量、完整玩家物品/inc增量和成功终态；取料不能算加工或已保存。root raw-ccdcb8c1于43344223合并此前七动作，核十终态/3309对象/6434互返边/54详情/4热电持槽、全物资/J83/三网，accepted10须待commit/push后归零。primary43311976只覆盖此前石墨接线，不能跨用来证明后续原料已落盘。现有27带加正常r5/r84加工9带才满足36NEW完整氢路线预算；实际新端点、真实输送和连续燃料仍待验证，最近复验同日。

2026-09-11 / EXP-070/145/188/208石墨热电接线复验（`validated`仅施工与普通保存）：raw-693f0870六个正常build形成24带/3高速，3297以1109过滤跨油，3308/3309各用3059/3061原生slot3且旧slot0氢入口保持，三者实际网1满供；d893b441保存43311976。root raw-2f6821c1在43320544核七终态/3309全对象/6434互返边/51详情/4热电物理持槽、全玩家/J83/三网，78项Core/21项action-client/14项私有断言通过。accepted7不能清零，revision135；没有投料、配方或连续燃烧证明。两种燃料入口物理并存不代表热电会同时或优先消费其中一种，必须以真实负载及库存趋势核销后续去路。

同批有界氢线路：raw-b5a15fb6的10/5/19/4点分别native通过，但未来两个共享源点不能重复扣料，整路NEW=36；现有27不足9，分段prepare成功不证明整图预算齐备。跨油3.667m、预期真实取货覆盖9.946m/6.591m仅是待实际新附件复验的几何。raw-c3da657d三源只读足量/可达，后续按完整配方补9带/2高速的11铁/2电路/1电机预算执行守恒取料；先到第十写审计，再加工/施工，不用当前单段可放替代全材料/顺序占位检查。最近复验同日，后续端点、真实货物、负载与恢复仍须复验。

2026-09-11 / EXP-145/188石墨跨油线重设计（`observed`只读候选）：raw-9af77e9d的7NEW南行源带保持，raw-9d0f5b6a通过油线西侧7NEW自由带及向北11点路径的完整原生检查；北段执行时仅source-cover已有首点、扣10NEW，整段预算24带/3高速。跨油空隙3.683m，以1109过滤分拣器显式跨过3277，不能铺带覆盖旧油路。后续两出口仅用3059/3061已观察空闲slot3，保留原slot0氢入口，分别按3057/3082真实取货位置供电预检；未出生端点的分拣器还没有native通过。root只批准六次有界build再普通save，任一步拒绝即停、成功前缀不重放；14项私有精确路径/预算/过滤/槽/供电正反断言通过。热电额定容量不代表燃料已消耗，本次仍不授权投料或宣称持续石墨去路，最近复验同日。

2026-09-11 / EXP-070/142/145/188/208完整油路实机复验（`validated`限本路线和保存）：raw-acb0aa06从accepted0完成2塔、113带、3过滤跨接和正常save43253457；root raw-954c217d在43259130核十终态/3282对象/6376互返边/52详情/27原设备持槽、全玩家/J83与三网，124项相关Core/21项action-client/13项私有计划检查通过。末桥3282的目的3111原slot4已有3124，原生选择新的真实slot5，两个方向读回且原slot4不变；由此再次否定“belt始终用slot4”的调用方假设。3111有2精炼油且末桥实际持油1，仅证明到达；不将相邻段货物加总为吞吐，也不把recipe0空厂视为生产。主网只增加2节点/3消费者，原既成配置/端口保持，未重启。push后计数清零，revision122；执行者单条旧计数文字不覆盖原始十收据。

同批EXP-145/188负例：raw-9af77e9d的石墨源3100南向续带只读通过，但下一西行段在planned point1与新油带3277重合而拒绝；零accepted，无对象改变。离线中心线草案不是现场原生通过，必须把最新已成带路纳入整路交叉检查；不得删除source绑定、拆改成功油路或重放相同候选。下一改为有供电、过滤和双端核验的显式跨接，未获施工或连续石墨消耗结论。最近复验同日，后续新端点/配置/供电/生产与保存恢复再次复验。

2026-09-11 / EXP-142/145/188/208受电绕行前缀实机复验（`validated`仅本次有限施工）：raw-a83c54ce的八次独立fresh build正常建成32条带及三条1114过滤高速跨接，实际取货位置均在142/143/133供电内、网1满供；不能以源/目标中点代替分拣器真实受电位置。源cover的旧路径朝向由Plugin当前whole_path_native_rotation_v1即时核验，不将合法原生旋转误认损坏，也不跳过该证明。root raw-f35b2d3a于43177172核十终态、全3164对象/6142互返边/113详情/27槽、完整物资/J83及三网，32带/3高速成本与新增35对象一致，旧前缀保留。当前源码fresh locked restore/build后1797项测试、21项action-client及12项私有计划断言通过；离线源码与仍为cd83b28的运行程序集分开陈述。push后计数清零，revision103、primary43040721未覆盖本次施工；下一两塔和西段必须fresh顺序预检。实际3111已有3124占belt slot4，末端新入桥必须按原生可用真实槽核验并保留原持槽，禁止沿用此前空带的slot4假设。未核销持续输油、配方生产或保存恢复；施工/供电/端点变化及十写/恢复触发复验，最近复验同日。

2026-09-11 / EXP-188/208/253供电落点重设计（`observed`限只读原生方案，不是施工验收）：首跨口两次NeedGround已单独记录后，root改为南绕1042、跨1210、再北跨旧横带，取货点分别借143/133现有10.5m覆盖；raw-24f2ba25四段完整原生带预检通过，跨接仍须针对实际新实体fresh。西端原拟一塔同时覆盖1264/1904，raw-d5982056首候选NeedGround，未继续盲选。raw-ef96f1b的既有prepare_move只读33点/66射线证明该点附近约0.3m浅凹；聚合shoreRisk=not_detected不等于建筑NeedGround必然通过，也不能当整段干地。沿实际样点的干地余量重新分为两塔，raw-af2b9b01分别通过原生放置，吸附后至各自取货点8.878m/9.445m；第二点使用30.470m完整已有地表预览，未提交Move或新增观察工具。整路候选预算145NEW带/6高速/2塔，现有196/11/5足量；两塔互距约8.058m且首塔距既有913约6.907m，只是接网几何，最终以建成后的真实消费者网号和满供核销。复验触发：每次分段绑定、互相占位、实际端口、供电及保存恢复；最近复验同日。不改旧设备，不扩大白名单，不在Plugin内自动选路。

2026-09-11 / EXP-074/188/208/284外侧油路与地面限制复验（`validated`限native正负例）：raw-f4bd7a57六段和raw-39a4cb1f第一西段的完整原生带预检通过；保留两处未来共用源带后预算146NEW/五个2012，但原生附件、顺序占位及实际供电还未通过。1042前取货点距143约11.725m，大于10.5m覆盖；第一塔位与向已有143地面靠近的不同第二塔位（预计取货覆盖8.148m）仍在raw-39a4cb1f/raw-239ad229分别NeedGround，零accepted。不得把旧电塔附近或可放带当作地面可放塔，也不从node的powerNetworkId=0单点推断断网；几何连通只作候选，实际消费者读回才证明供电。两同类拒绝后停相似尝试，记录后由root改变路线/供电结构，不再盲试第三塔位；第二、三跨口共享塔未试。只读helper的通用参数曾被dot-source覆盖并在边界处停止，已换独有参数后才开始第二native预检，复验EXP-284而非放宽边界。accepted2/revision87及primary43040721保持；最近复验同日，所有成功旧设备保留。

2026-09-11 / EXP-007/018/070/074油路器材保存复验（`validated`限加工/保存）：raw-7f684755以r8×4守恒加工四塔，普通保存43040721；root raw-61e9a01e于43047024核两原始/fresh成功终态、全玩家inc、31关键设施原配置/边、J83原前缀及三网不变。库存5塔/196带/11高速/1线圈，accepted2/revision87继续累计，不提前清零；8项日记Core、21项action-client和12项私有预算断言通过。没有施工或新投料，protected resume可用不等于恢复已验；最近复验同日，供电落点/后续施工仍须fresh原生检查。

2026-09-11 / EXP-007/018/074/080外侧供油备料边界（`validated`仅有限器材与守恒，路线仍为候选）：固定“先西后北”的离线候选避开中央密集区，横段约108.28m、纵段52.83m，仍有1042/1264/1904三处旧带交叉；加入口水线跨越及末端接入，不能把几何草案称作可执行整路。另一方案两新PLS仅r93就需80钢/80钛/80处理器/40粒子容器，未把旧塔快照或目录缺少基本产线角色误当新站已可施工。root选择现有普通带/分拣器，备料上限新增180带/10高速/4塔；运行时raw-3d6dcc38确认r5/r6/r8/r84/r85/r88及可达来源足量。raw-0b597998已完成五次转移与五次正常手搓，叶料198铁/4磁铁/2铜/10电路/5电机；root raw-28d2efb4于42997607核十原始/fresh终态、五源即时守恒、五配方全背包差量、3129实体/6072互返边/89详情/27物理槽及J83/三网不变。现有196带/11高速，剩8铁/5线圈；4塔加工延后，primary42933302未覆盖器材。8项日记Core、21项action-client及12项私有离线配方/守恒断言通过，本条push后十写清零、revision84。无新施工或供料，缺料修复后仍须逐段完整fresh原生预检；最近复验同日。

2026-09-11 / EXP-007/018/037/048/070/074/080油源前缀实测与十写复验（`validated`限正常施工/有限到货/保存完整性）：3NEW首带3126由784真实空闲slot2经2012/1114过滤跨过旧带接出，原生span2、实际belt slot4，未建先前可放仓候选，旧路和旧配置完整保留。raw-9fe00a6e有两施工和save42933302原始收据；root raw-d0728a81核十原始/fresh终态、3129实体/6072互返边/75详情/27物理持槽、背包2001净−25及2012净−5/J83/三网满供，仅30新实体。122项带/分拣器/日记Core、21项action-client及4项私有离线检查通过；commit/push后十写清零。逐带beltCargo均只见油，邻段不可加总、出口积油不代表持续吞吐。完整外部路线仍未批准：离线固定长直线路径与现存多条带重合，尚未提交原生施工，不能省略端点绑定或覆盖旧带；下一先比较有限跨线与已有物流能力成本。最近复验同日；延伸、真实持续消费者和保存恢复另验。

2026-09-11 / EXP-070/080/253油源出口设计复核（`observed`，施工前边界）：原生可放中转仓不意味着必须建仓。以784真实空闲北向slot2和现存带占位设计3NEW东向短带，raw-6e955dbd完整native_grid预检通过；另11NEW北段也通过，但中间跨线、实际附件和完整至新厂路线仍未证明。root只批准先3带→针对唯一真实带首端fresh预检的1114过滤2012→正常save，最多3个accepted使当前7达到10，随后先审计；任一拒绝保留成功前缀。不建先前候选仓、不改旧端口/旧路、不因预检成功跳过完整材料和碰撞检查，也不把有限出口缓存说成持续供给。最近复验同日；4项私有离线路径/预算检查只证明调用方边界，实际槽、跨度、材料、供电及十写完整性另行核验。

2026-09-11 / EXP-007/018/037/070/074/080裂解厂产品端口复验（`validated`仅空载结构完整性）：四条2012初始过滤施工以3122/3123输出1109、3124/3125输入1114，石墨原生span1、油span2；不能从中心距离或旧氢分拣器推定跨度。root raw-ae615052核原始/fresh四终态、3125实体/6064互返边/70详情、22旧设备持槽及4新设备槽/4实际belt槽、玩家仅2012−4/J83及三网满供。73项分拣器Core和8项私有计划断言通过，accepted7/revision64保留；primary42766876未覆盖本批，无配方/投料或连续供给结论。另复核外部油源落点：raw-ec326f32被752的完整原生碰撞体拒绝，不能用中心净距放行；不同落点raw-3aa551a1仅prepare通过，不代表端口连线或完整路线通过。最近复验同日；后续每个端点须fresh实际槽证据、整路线预算和实际消费者，施工/十写/恢复再次复验。

2026-09-11 / EXP-007/080/103/188/214/246/253裂解备料十写复验（`validated`限有界脱困、转移守恒及完整性）：两次不同短目标3fea3f39/e35ee159均经完整采样和terminal到Walk；792在首次侧移后80.059m仍被原生拒绝，下一仅补新的短目标，不放宽80m或重放失败Move。铁20完成后磁铁commit的STALE明确未接受，保留成功前缀；fresh换铜源562并只补五笔，完整六种玩家净增由各动作的同tick收据核销，723正常回填不抵消原成功。root raw-10326e73核十唯一终态含一已知失败、3081实体/5972边/57详情/18槽、全玩家/J83/三网；1103真实完成72000/72000，revision14、primary42301785未保存本批。53项科研/移动Core及21项action-client通过；本条push后计数清零，石矿72/石材28与加工尚待。最近复验同日；来源/距离/库存变化仍需新完整prepare，不假定另换来源必定消除并发变化，后续施工、十写及保存恢复重验。

2026-09-11 / EXP-007/103/246/253再次复验（`validated`限正常stall终态和禁止原目标重试）：raw-d6ca0615的5579b75e在181tick位置无进展后终止，剩5.531m；此前24点地表预览与5.924m干地末段并未承诺无建筑碰撞。root raw-d8f96b6f核完整原始/fresh失败收据、全部玩家/J83和16个近邻，Walk0/800MJ、owned/healthy/revision4，仓884距1.972m，具体碰撞对象仍仅推断。accepted2包括这次失败，八笔后续取料为0；先测业务距离（792仍81.496m）再设计一次新侧移，不重放整段或旧目标。37项移动Core与21项action-client通过；最近复验同日，下一短移仍须完整采样、有限候选、terminal与fresh业务预检，不把普通只读脚本错误或STALE当已接受动作重做依据。

## EXP-297 — 连续制氢必须核清精炼油出口、裂解净产与当前能源

- 同日双通道施工复验：raw-6c52d3ef正常建成22NEW石墨南行/油北行空带，第二路fresh原生预检包含第一路实体；root raw-ebb25368核两终态、22唯一位置/40互返新边/四自由端、21旧近邻及氢线路、全玩家/J83和三网。41项带Core、21项action-client、4项私有双路线检查通过，accepted3/revision56保留，primary42766876未保存新带。四个产品过滤连接只读通过原定厂slot3输出1109/slot5输入1114，之后仍需新预检/逐实体读回；空线不证明油持续来源、石墨持续消耗或实际制氢。最近复验同日，关联EXP-068/188/296，施工/供料/十写和恢复再验。
- 同日结构保存复验EXP-188/214：bb71e5c7正常保存42766876，root raw-fbb791ec核原始/fresh终态、44设施、完整玩家/J83/三网与protected restart；accepted1/revision52保留，未重启、未供料。8项日记Core与21项action-client通过。raw-fae3cd74/ffabf70f的两条厂西11NEW空带仅分别原生预检通过，内石墨南行/外精炼油北行，22带只是本地通道预算，仍须顺序fresh相互占位和未来过滤端点实测；不能据此承诺完整供油/副产物去路或持续产率。最近复验同日，接线、供料、十写及恢复后再验。
- 同日四氢连接/十写复验：raw-a85c8f0d的3096–3099均为2012/1120、native span1/progress100000、网1满供；设备slot2输出在slot0输入上游，belt端实际占slot4。root raw-e721b22f于42754293核10/10原始/fresh终态、全3099实体/6008互返边/54详情、18旧持槽/4新厂槽/4实际带槽、全部物资inc/J83和三网；旧3081配置/连接保全。73项过滤/端点Core、21项action-client、8项私有计划断言通过；本条push后accepted10清零、revision51，primary42600208尚未覆盖施工。限空载拓扑验证，recipe0/无投料，不能核销实际循环供料和持续净产。调用方私有审计还须将JSON物品/实体数值键显式统一为int再查类型敏感hashtable；两次只读汇总类型错误均无游戏写入，修正后完整重读而未重放任何build。最近复验同日，正常保存/恢复、配置或供料后复验，关联EXP-068/080/188/296。
- 同日回供通道阶段：b367fcc3以11NEW普通带正常建成3095…3085北→南空链；root raw-3ed592cd于42713687核原始/fresh终态、精确路径/11唯一实体/20互返边/两自由端、22旧近邻、全玩家/J83/三网，accepted6/revision43保留。41项带策略Core、21项action-client和6项私有路径断言通过。四个1120过滤端口只读预检通过，各厂slot2出氢在本厂slot0回供点上游，拟保留循环投入后供下游；其实际缓冲优先级、启动、净流量仍待实测，不把只读预览或空带称为持续制氢。建成、供料、旁路和窗口变化触发再验，完整带闭环/目标合流仍未开放；primary42600208尚未覆盖本批。最近复验同日，关联EXP-068/080/188/296。
- 同日完成科研和预算复验：1103真实unlockTick42459498、72000/72000，root raw-10326e73独立复读；仅解除r58科技前置，不代表已经配置或制氢。raw-b8eac620的只读Foundry预算显示主网当前285002J/tick、已有满基础负载预约1363000J/tick；ratio=1的空闲现场不能替代全部负载预算。该位置本就被3074占用，只用于诊断，不是可施工候选，不弱化原生碰撞或满基础负载门；后续模块需另行证明供电与现场。
- 同日有限选科正例：7e04fa31通过原生队列选择1103，400蓝/400红仅为未来正常消耗，J83真实记录首次。root raw-27855ec9核原始/fresh终态、全玩家、原Journal前缀和7燃料设施，11166/72000说明研究已推进但未完成；当前285002J/tick满供不等于32.1MW持续可用。31项科研/燃料Core及21项action-client通过；accepted1/revision3、primary42301785保持，未保存本次选科。此证据只把“选科及前置校验”提升为validated，供氢去路仍为observed/待施工。
- 日期/状态：2026-09-11，`observed`限本档当前供氢阻塞；最近复验同日。
- 结论：氢不足不必然是原油不足。先读每种产物的真实出口与下游；多产物配方任一满输出都会限制其他产物。裂解的循环氢必须按投入2、产出3的净增1计算，不能把总产3全分配给外部。有限启动燃料耗尽后，重算当前容量，不能沿用聚变点火时的32.1MW。
- 证据：root raw-963617bd与Luna零写raw-903ea188（27详情）均见141/707的r16原油输入4、油输出40、氢输出0且未工作。163确有600精炼油，165的独立tankFluidCount确为0；带的selected-segment cargo不作整条路径总量，buffers为空不证明罐/带为空。290只读到炉端，尚不能证明下游连接完整，未作修改。root raw-1a10361e验证1103前置1102/1121已解锁，正常科研预检为400蓝/400红矩阵，空队列，无commit；本次实际主网capacity285143J/tick，不能继续按535000预算。
- 决策边界：选科时r58仍锁定，先仅选择正常1103科研，让原生研究与后续有限施工准备并行；随后真正完成见本条新增证据，选择本身不等于完成。2棒、10D2及现有油/氢库存均不是连续供给；燃料2/min与奇异物质3/min的只读物料草案合需D2 50/min、氢100/min，未包含其他消费者，也不是可执行布局或稳定速率证明。
- 限制/触发：需进一步验证原油持续源、循环氢优先回供、石墨长期需求、真实输送和峰值供电；不改已完成线路，不扩大建筑/蓝图白名单，不用搬库存代替去路。科研解锁、配方/物流/供电变化、十写与保存恢复后复验。关联EXP-188/208/291及存档日记001。

2026-09-11 / EXP-001/002/188/214/291聚变恢复十写复验（`validated`限同一受保护主档）：raw-99a9a8a4正常关闭已保存DSP、228安装/native哈希复核和单次Steam启动；raw-b8b5641f证明实际安装64工具/1资源/66807指南及认证，非新包或异机验收。97e657b0唯一恢复42301753并正常重存42301785；root raw-a52260ba核九退出前完整终态及新resume、3081实体/5972边/26详情/18持槽、全部物资和J82原跟踪身份，32.1MW与1802正发电恢复，未重建/重投、D2仍10。45Core恢复/电力及21action-client通过；十写计数在本条push后清零。跨进程旧action不向新session重放；使用退出前留存的fresh原收据，当前世界另作全量核对。保存前后窗口不拼接，仍只是两棒有限启动，不抵扣持续燃料/24MW峰值或三级Foundry验收。最近复验同日，下一供料/负载、十写及恢复再验。

2026-09-11 / EXP-188/291/296有限聚变启动复验（`validated`限两棒点火和普通保存）：raw-334bd429在原生仓储transfer的双边即时守恒之后，使用既有1802过滤送料，不直接填发电机buffer。42301594/42301654/42301715实读燃料1802及正发电，capacity从285000升至535000J/tick；三读间隔60/61tick，均在预声明1200tick内，不能拿这196tick以内的启动证据冒充长期供应。普通保存42301753，root raw-8a4ac4b4核九原始/fresh终态、全3081实体/5972边/26详情/18槽/完整物资/J82，三网满供。50项分拣过滤/端点、23项燃料目录/日记Core和21项action-client通过。accepted9/revision62保留不归零；聚变站虽然开始发电，燃料仍是两根手搓库存，自动制棒、供给配平、峰值24MW负载和恢复仍须独立证据。最近复验同日，供料、负载、任何采样缺口或恢复触发重验，不能因燃料耗尽再投同一批。

2026-09-11 / EXP-068/188/292聚变送料实机复验：3081经正常2012成本/无人机接成3074slot3→3079slot2，初始1802过滤、STT300000、实机网1满供；root raw-3a120632独立核终态、7详情、双物理持槽及全玩家/J82，旧3075/3076和仓6/7槽不变。补电前的边缘覆盖没有被当作可用供电，补塔后实际只增加一个网1消费者；21项action-client通过，accepted7/revision60保留。当前两棒仍在玩家，发电buffer为0，此正例只覆盖空载施工/过滤/供电，持续送料及保存恢复另验。最近复验同日，投料、端点/过滤变化和恢复触发再验。

2026-09-11 / EXP-188/292补电正例：raw-af3d7a78以一段全干地Move及一座普通2201解决既有原生范围与未来送料源端覆盖；root raw-a6865d1f核两终态/7详情/全玩家/J82，新增3080只使网1节点115→116，发电机/消费者/其他网保持。送料源端至实际新塔8.839917m，同一3→2空槽及1802过滤fresh native prepare通过，未借用中点或改旧实体。40项Core电力测试通过；accepted6/revision58不清零，未供棒/保存，建成后的分拣器真实电网仍须复读。这是有界补电施工而非持续发电/燃料验证；最近复验同日，送料建成/负载变化或保存恢复再验。

2026-09-11 / EXP-188/292供电边界复验（`validated`限新站空载施工）：raw-f5d20f5c的3079由正常2211成本/无人机完成；root raw-0fe8488c核全3079实体/5968边/24详情/16原持槽、全玩家/J82，主网仅+1节点/+1发电机，未供燃料capacity仍17.1MW。目录额定15MW不代表当前实际可用15MW，发电站并网也不保证附近分拣器有电。实际3074slot3→3079slot2过滤1802的native预检通过，但分拣器消费者源点距3056为10.549929m，大于目录cover10.5m，聚变站cover4.5m同样不足；不能用两设备中心或分拣器中点代替消费者源端。下一先有界接近并复验新增电塔（首候选OutOfReach、零accepted），保留已成站与所有旧槽。21项action-client通过；accepted4/revision54不清零，primary42167220未覆盖施工，持续燃料仍待。最近复验同日；实际供电范围、过滤、端点、新负载、保存恢复或十写触发再验。

## EXP-296 — 有限批次收齐预算包括实际送料跨度与末批输出

- 同日三件空载施工复验EXP-188/292：raw-ff9b1bcd三个独立正常build完成3082塔及3083/3084炼油厂；root raw-57e1fb21于42662056核三原始/fresh终态、全3084实体/5972互返边、39详情/18旧持槽/18新空槽、全部玩家inc及J83。几何预检已由两厂实际网1满供补证，主网仅+1节点/+2消费者、三网均满供；成本恰为1塔/2厂，recipe0、未供料。39项Core供电与21项action-client通过。accepted5/revision41不清零，primary42600208尚未覆盖施工；空载满供仍不替代完整物流、全厂满负载预约或连续产出。最近复验同日；接线、配方、供料、新负载及保存恢复后重新核验。
- 同日器材保存与预检：raw-2521f304额外塔正常加工后保存42600208；root raw-674bf80a核两终态/全玩家/J83/26设施/三网，accepted2不清零、revision35。真实10高速/0普通以结构化物品ID为准，更正执行者文字误写，不据此重做。raw-e48cf94f/9a194e31/30cfd10d只证明燃料区两空厂和中继塔分别原生可放；未来顺序施工还须fresh检查相互占位。塔距两厂约8m且距既有节点小于22.5m只是覆盖/连接几何，须以建成后实际电网读回核销。该空载小切片不得外推为完整管线材料、全厂满负载功率或持续供氢已通过，关联EXP-188/208/292；最近复验同日。
- 同日裂解首批器材：raw-0b49fd54两笔95取料与八笔正常加工得2炼油厂/10高速分拣器/2小仓，r6与r11/r88的每批输出2必须纳入整批守恒，不能用请求count当最终物品数。root raw-44442664在42580371核十终态、全3081实体/5972边/42详情/18槽、全玩家inc/J83和三网；8项日记Core、21项action-client、16项私有离线配方/预算检查通过。EXP-295继续成立：本批没有新增可见首次事件，不从缺席伪造历史；本条push后计数清零，revision32/primary42301785尚未覆盖加工。器材不等于完整布局材料，额外塔及实际物流预算仍须独立确认；raw-8bbcce6b北侧候选被NeedGround拒绝，旧带附近也不能假定有可建地面，第二草案未尝试，不绕过地形或扩展白名单。最近复验同日，施工/地形/物流或十写与保存恢复触发复验。
- 同日启动燃料备料：r41单批实际输出2根而非1根，raw-36fe3db3三终态证明正常Move、20D2取料及1合金/20D2/1环→2棒，J82仅真实首搓1802。root raw-5b1ada54核23旧设备/全玩家/原Journal前缀/三网；accepted3/revision52保留、primary42167220未覆盖本批。首个2211落点native预检已通过，但真实供料端口仍须建成后逐对象复读；不将仓距或可建状态当供料和发电证据，有限两棒不等于持续燃料。
- 同日石墨烯/纳米管复验：用既有两支路分别按span3/120tick与span2/80tick预算，预声明6000tick/12件与5000tick/8件；raw-93e4141c实际2516/1172tick前收齐并完全转入聚变站物品，期间利用普通加工时间完成另两项有限手搓。root raw-9b686358在保存42167220后核十终态、3078实体/5968边/34详情/16槽/全部背包/J81；8项Core连续性、21项action-client、19项私有日记断言通过，accepted10在记录push后清零。EXP-295继续成立：只允许实际观察的处理器/聚变站J80/81，不从缺席倒推首次。2211仍在背包、主网仍17.1MW，不能把手搓当建成或有限批次当持续燃料。
- 同日十写审计复核EXP-007/103/188/214/246/253：既有库存取出16合金及29石墨，三段明确Move各有fresh完整采样及Walk终态，未重试失败目标。raw-f01f3cd4核十唯一原始/fresh终态、3078实体/5968边/43详情/16物理槽/全部玩家/J79/三网，27项地表策略通过；accepted10在该记录push后清零，revision33/primary42093684保留，取料未保存。反向接近点仅原生只读通过，下一腿必须重做fresh预检；仍不把原料齐备或有限生产当作持续产线。
- 同日正例复验：原合金四条2011均实际span3，按32酸送料、末批加工及末端输出预声明9000tick收齐16件。raw-d7638f2c四笔transfer即时守恒，raw-cdcce753于42088714（投酸后5118tick）看到16合金且输入/在途输出/进度归零；只证明有限批次在截止前完成，不声称连续满供或精确产出tick。正常保存42093684，raw-159b091c核五终态、3078实体/5968边/53详情、完整物资/J79/三网，21项action-client通过；accepted5保留。旧硫酸6000tick失败不改判，下一仍需石墨及持续燃料链。
- 日期/状态：2026-09-11，`validated`限当前原生普通分拣器与有限硫酸反例；最近复验同日。
- 结论：配方加工时间不是“最后一件进入输出仓”的截止时间。预先核实际输入/输出跨度、供电、现有buffer与正常循环；理想容量只是预算依据，仍要实际终态/物流读回。超出声明窗口仍保留失败，后续库存完成另记，不能追改旧窗或重投原料。
- 证据：raw-b49a2663从72石投料42005827起观察，5947tick仓34、6013tick仍34，6000tick/总38目标失败；后续独立raw-f4b7f3e在42020311读到38及全部原料/其他酸buffer归零，证明旧2+新36完成，但不是原窗口通过，也未确定最后生产tick。raw-881f9a25确认2011原生STT200000、866为span2/400000、864为span3/600000；既有FoundryTransportPlanner按满电每tick10000与来回过程给80/120tick理想单件循环，不能用span1速度替代。26项相关Core与21项action-client通过。
- 本轮复核EXP-007/103/188/214/246/253：两个新目标逐段fresh预检，第二腿5.835m干地后Walk；四油水/三Move/两石/一钛共10终态、3078实体/5968边/46详情/全背包/J79与三网完整，源95取72石而非向862提取。只有16钛进入合金输入链，钢16仍在玩家；本条push后十写清零，primary41855925尚未保存新进度。
- 限制/触发：这不是连续供给、配平或提高吞吐的证明；不把跨多个tick的buffer求和当同tick守恒，动作守恒使用原生即时收据，完整批次用静止收尾核销。改变跨度、过滤、负载、配方、供料或目标窗口后重新计算与实测；不为有限启动引入新升级类型或改造已成功产线。关联存档日记001、EXP-127/188/214/294。

2026-09-11 / EXP-007/103/246/253取石接近复验（`validated`限安全拒绝与成功前缀）：油水供料中的STALE发生在第四笔prepare，前三终态及油总54/玩家水36由raw-5c596c4e证明，后续仅提交剩余水，未重取/重投油。raw-8e5bce68先Move确已terminal completed且Walk；报错行来自下一Move的surfacePreview，不能因多步helper整体exit1就重放前一个目标。第二目标与此前root西候选分别水下0.228/0.248m，两者未commit，达到相似岸线拒绝边界后先记录再重设计；有建筑可见、邻近风机或数学连线均不替代当前干地证据。raw-f2b4a3a3核五唯一成功收据、完整玩家/油水/旧2酸/30D2/J79/三网，accepted5保留，27项地表策略通过。下一仅用具体已读样本和障碍几何重选短目标，仍须每段fresh surface/终态/Walk，不延长watchdog或取消4m干地门。最近复验2026-09-11。

2026-09-11 / EXP-007/103/188/214启动备料十写复验（`validated`限六笔正常取料及完整性）：root先逐源确认库存/80m范围和完整native预检，Luna raw-3d6bed28连续执行六笔fresh prepare/commit/terminal；102原17钢仅取16、其他源同tick扣料与玩家加料逐项一致，后续自然补货不替代瞬时守恒。root raw-8e5723e1核两build/save/resume/六transfer共10唯一动作、3078实体/5968边/35详情、全背包净20钛/16钢/22涡轮/33磁铁/8电路/8微晶及原30D2/J79/三网。跨进程动作使用原始终态与退出前保留的fresh对照，未把运行时结果淘汰当作重放依据。21项action-client通过；本条push后十写清零，新材料尚未覆盖保存41855925。有限一座聚变站/两棒的材料叶项不等于完整燃料链，缺硫酸/石墨烯和正常加工仍须后续证据；新供料/负载、十写或恢复触发再验。最近复验2026-09-11。

2026-09-11 / EXP-001/002/103/208/294并网保存恢复复验（`validated`限当前两塔及有限首批）：raw-0b85beda复核全部228安装文件/native引用、正常关闭并一次Steam启动；raw-6ffb042d验证实际安装64/1/66807指南与认证/stdout。唯一恢复5259ac39读取ticket-bound primary41855894并重存41855925，root raw-aa333dbc核3078实体/5968互返边/29详情、16个设备实际持槽、全玩家及Journal身份/J79，首个恢复位置仅0.026249mm差。原两座被动电塔及三网成员、主网285000J/tick/full service保持，仓30D2/r40进度0无重复施工或投料；证明耗尽燃料的既有热电节点仍可在本次保存恢复后维持接电，不把其额定发电算入实有容量。accepted4保留，45项相关Core/21项action-client通过。限制：空载满供及有限30D2不代表持续燃料，也不为24MW双机负载授予预算；新负载/供料、十写及下次恢复时复验。最近复验2026-09-11。

2026-09-11 / EXP-208/294被动并网后的有限恢复（`validated`限本次正常施工、全网满供和保存）：第二塔3078于41841253建成，使既有缺电支路加入已加强主网；三网全部满供，主网114节点/66发电机/313消费者与285000J/tick实际可复读。未补任何氢/燃料，仓25→30在新1800tick恢复窗内最多703tick发生；区别于此前9000tick失败，不能追改旧窗或当连续产率。raw-177d0cef/raw-f31745c5核全3078/5968边/33详情、只允许原网5→1映射、完整背包只扣本次电塔、实际设备持槽、J79与覆盖保存41855894；原带1968和首塔3077保持。优先复用既有导电节点消除了对会耗尽启动燃料的依赖，剩余过程原料仍需真实持续输入；当前17.1MW不支持24MW双对撞机满载声明，后续聚变及燃料全链另验。accepted3不归零，保存恢复待实测；最近复验2026-09-11。

2026-09-11 / EXP-208热电节点互联实机复验（`validated`限本次被动并网）：20b42e1c在41816166正常建成2201/3077，root raw-c88864f3核全3077/5968边、原动作与完整玩家净−1、旧1/2设备允许的原生网号合并，其余配置拓扑保持。网1实际汇合102节点/57发电机/300消费者、270000J/tick满供，其余3网未变；证明耗尽燃料的热电183仍可作为导电锚，避免按风机集合误算需要跨水三塔。没有猜合并后保留哪个网号，也没有把纯node摘要null当未通电。最后支路改选旧带西侧，raw-7927f4a5原生占位/地面/材料及两端距离通过，但尚未commit；当前观察可合并容量285000/需求256250J/tick不是24MW或持续燃料/未来全部机器满载预算。accepted1保留、J79/25D2不变，保存恢复和最后接网另验。最近复验2026-09-11。

2026-09-11 / EXP-208接网重设计预检（`observed`）：完整节点目录而非仅风机配对显示，既有热电183是net1的原生power node，没燃料只影响发电，不消除接电作用；它与net2风机2715相距38.588m。raw-e697dede对一座2201在原生吸附点(-35.9962158,-104.067429,-167.195328)完整预检通过，距两锚18.82584/21.54195m、玩家79.41855m，材料/科技/地面/碰撞通过。复用现有OnNodeAdded及22.5m半径，无新增充电器负载，候选从三座中继缩为一座；尚未施工，不能先认定电网已合并。仅node设备的通用entity摘要可能没有powerNetworkId，不能将null当作未接网，实际合并必须用两端具有原生网号的锚、完整网成员与容量复读证明。另一个接net5候选被既有带1968碰撞拒绝，未获施工授权，不改旧带、不绕过碰撞；下一按固定新候选fresh预检。最近复验2026-09-11。

2026-09-11 / EXP-066/208/246/253十写复验（`validated`限动作完整性与地表拒绝）：raw-ebc346b0首Move成功，但下一段终点实际在水下，caller在commit前拒绝；raw-e33c7ba0两个不同南向短目标以4.7846m/17.1410m干燥末段、零unknown和1.5m容差完成Walk终态。没有把地表采样当避障保证或重放已完成第一步。两个首塔数学候选raw-0ddda3b4/raw-48b66628仍由原生NeedGround拒绝，说明合法连接距离不证明可施工地面；达到两次同错后冻结同类尝试，未扣料。save41775077后root raw-5e748074核10唯一终态、3076/5968/31详情、全背包4塔/J79及五网；本条commit/push后清零。供电blocker依旧，不能宣称并网/30D2/持续能源通过。当前原生2202连接45.5m但充电work80000/idle1500J/tick，若作为替代也必须另计负载和材料、完整新址预检，不因接线更远自动采用。67项相关Core回归通过；最近复验2026-09-11。

## EXP-295 — 旧档首次排除集不能由日记条目缺席反推

- 日期/状态：2026-09-11，`validated`限当前attached-existing-save手搓与调用方修复；最近复验同日。
- 结论：Journal没有某物品的首次事件，不代表该物品从未手搓。旧档附着时的历史排除集与后续事件共同去重；调用方不得以“本次必须新增sequence”作为成功动作后置条件，也不得因此重做已成功动作。
- 证据：当前档跟踪始于4428079且历史覆盖不完整；raw-e02d2e11对精确当前Journal的只读核验确认1202/2201都在historicalManualItemIds。r6×2实际产生4线圈，r8×4产生4电塔；root raw-467d35d3核五个原始/fresh终态、完整背包净2201+4及J79无变化。源仓自动补货造成一次未接受的STALE_STATE，成功的前两次转移被保留，原生同tick收据而非较晚源仓净变化证明取料守恒。
- 修正：废止私有多步骤helper中的J80/J81假设，只执行明确未提交的最后手搓；未改变Plugin、Journal、状态哈希或准入。现有GameplayFirstOccurrenceDetectorTests的历史已知但无事件再附着用例及其余测试5项通过；FuelPowerCatalogPolicyTests15项和action-client21项也通过。
- 限制/复验触发：不能因本例假定任意缺席事件都属历史；必须区分当前Journal身份、trackingMode/历史边界及原终态。新档、其他物品、Journal变化、持久化失败或未知动作另验。关联IFX-103、EXP-007/103/188；四塔尚未施工、材料尚未覆盖保存，accepted6不因阶段审计清零。

2026-09-11 / EXP-001/002/188/292/293恢复复验（`validated`）：raw-57cc544e核228同批文件和native引用后正常关闭一次、仅Steam启动；raw-d4a8e929验真实安装MCP64/1/66807与纯stdout。唯一resume1802d6b0载ticket-bound primary41619631、重存41619662，root raw-3517827d核全部3076/5968/27详情、八分拣器16个真实端点、25D2及在制进度566301/J79/全背包保持，首读位置误差0.000008268m。正常回收再原生复用3074 ID没有引发重复施工、重复投料或历史分叉，IFX-101本次窄闭环含保存恢复已通过。恢复不会补燃料或把原生低于10%的停产消除；accepted1保留，不将恢复成功当作生产目标达成。最近复验2026-09-11。

2026-09-11 / EXP-007/103配置动作复验（`validated`）：raw-eb5f2b27唯一配置2506919d于41584093成功，root raw-3130d6e1核原始/fresh终态、r40、空buffer及H60/J78/healthy。配置Complete与启动尾各递增revision，实际12→14；accepted仅7→8。私有caller硬套+1后的异常不等于游戏失败，不能重跑两步脚本。改用仅剩余投料的一步fresh计划，raw-8cd9b986唯一607f4604于41599156完成60H转入3074，目标0→60/玩家−60且inc保持，actual revision15/accepted9。复用已存在的“revision不是动作计数”经验，不修改Plugin语义、不补造重氢产量；后续批次/保存恢复另验，最近复验2026-09-11。

2026-09-11 / EXP-188/227/292双连接复验（`validated`）：原生空仓重建后，3075用3074槽6→3073槽0，3076 fresh改选3073槽1→3074槽7，两个2012均300000STT/三格/满供。raw-2d68c1bf独立核全3076实体/5968互返边/27详情、正常两件扣料及四个端口持有本次sorter；没有槽覆盖或第一路重做，原TooClose布局的接线纠正实际通过。保存恢复、首批重氢及长期供给仍待，accepted7不归零。配置r40的prepare预算列10H input/5D2 output，仅描述配方吞吐，配置动作本身仍应无玩家扣料；不能误要求空itemBudget或把配方预算视为已投料。raw-01aec35b只读配置/60H转移预检通过，后续仍fresh提交。最近复验2026-09-11。

## EXP-294 — 临时燃料的功率与能量预算会随等待失效

- 负载窗口复验（2026-09-11）：raw-7161185e共131次有界样本，前5批25D2实际入仓；第6批前后容量由231000降15000J/tick，需求202050，比例7.4239%。9000tick目标未完成；raw-c2555cf8与保存/十写复读证明progress固定566301/1500000。已留存原生AssemblerComponent.InternalUpdate在power<0.1时直接返回，不能把剩余风电当作慢速最终必成，更不能用isWorking=true解释为实际推进。临时燃料补给距投料已等待约71400tick，空载消耗使最初432MJ预算不再成立；单次提交前容量仍足够，也不能证明余能量足够。保留25D2和在制第6批，未重投，正常保存41619631；下一从已产出的20D2可用量与实际合金/环/聚变材料重新预算，未授权盲目重复氢补料。`validated`限这次失败原因与守恒，连续燃料仍未通过；最近复验2026-09-11。

- 补料实机复验：raw-16409047三笔各20氢在41527687–41527877正常终态，root raw-f48838ae核同tick双边守恒、19对象/全玩家inc/J78/网成员，并实读capacity恢复231000、全满供。仅此有限补料闭环`validated`，不等于持续能源；accepted5/primary41312641未保存，60氢过程原料保留。接线/负载/实际产出仍另验。
- 日期/状态：2026-09-11，`validated`限动态功率下降、零写拒绝、三笔守恒补料与上述25D2/第6批失电证据；双向接线已另验，30D2目标和持续供给未通过。
- 证据：raw-e93e5409观察net5 capacity231000J/tick、三燃料仓为空；稍后raw-a0adcd48在接线prepare前停止，root raw-1581c825于41508912复读capacity15012、required=served3750、原电网成员/玩家120氢/3高速分拣器/revision5保持。额定设备没有消失，但可用容量已不能启动12MW负载；当tick发电buffer不是燃料件数或剩余热量。
- 决策：不沿用最初180氢的完整剩余能量假设，不因正常动态容量变化重启或判拓扑损坏。先用现有120氢中的60正常补至原三燃料仓，余60只计首批30重氢；新增燃料名义432MJ电能对六批r40主动180MJ，另列辅助负载与等待时长，并在负载写入前fresh复验满供和实际容量。
- 限制：有燃料或满供的一次采样不证明持续能源；该首批量不核销原60重氢备料、两聚变/六棒或持续燃料门。仅正常transfer/施工/配置，不直接读取私有游戏内存、填燃料或更改速度。预检与已接受动作分开计数，先前两私有脚本拒绝均零accepted。
- 复验触发/最近复验：2026-09-11；等待、投料、负载增加、恢复或DSP版本变化时再验。关联EXP-016/188/290/291。

2026-09-11 / EXP-016调用方材料边界复核（`validated`限离线与零写读回）：双分拣器私有脚本把“本批施工2件”误当“背包恰有2件”，raw-694bd77f在prepare前拒绝实际3件的合法备料；无commit/accepted。root raw-a9877f4b核revision5、实际2012×3/H120/2101×0，修正为初始3→每步扣1→保留1备用，并以原始快照验证正确边界通过、错误完成计数拒绝。全玩家/inc和真实原生预算仍严格核对，不能把多一件已有备件视为新生成物资，也不能用大于预算检查忽略未知增量。原始3件来源随旧审计保留；最近复验2026-09-11。

2026-09-11 / EXP-292替代施工复验（`validated`限一仓和独立预检）：9d1e302a于41457611正常消耗2101×1，在已证明消失的旧3074之外的新位置由原生复用3074 ID；root raw-5ae16312核全部3073幸存者、新3074总数/5960互返边、全玩家inc/120氢/J78/五网。新真实端口的双向native prepare通过，输入6→0源端覆盖9.53485m；两个独立预览使用相同物理槽，第二路不能直接沿用旧计划，必须第一路完成后fresh重新选空槽。没有接线commit、配置、投料或覆盖保存；accepted2保持。私有证据集合可能含合法失败响应的null，筛选成功实体前先检查null/字段，调用者错误不等于游戏动作失败；重试前仍核原始收据与revision，不能重放已接受动作。最近复验2026-09-11，关联EXP-188/227/292与IFX-101。

2026-09-10 / EXP-292替代落点设计（`observed`）：raw-5dd3ca5b只做一处空仓原生放置预检，2101×1预算通过；按吸附后的新位置、原仓完整native ports与球面朝向变换复算连接。预计相邻两路端距约4.25m、格跨度约3.39、朝向偏差约2.85°；原生双设备范围是0.9–7.5m、跨度≤3.799及组合下界1.451，不能只检查米距离。电力消费者位于分拣器源端，不能用中点覆盖替代。该200m星球的格跨度目前为离线估计；新仓建成后必须用实际端点再走完整native prepare，并在第一条连接完成后重新选择第二条空槽。不能把独立预览当联合槽位预约，或把仓库放置通过写成接线/生产通过；最近复验2026-09-10，场地/端口/供电或DLL变化时重算。

2026-09-10 / EXP-292实际回收复验validated：唯一3cf3429f于41383839正常移除空3074、返还一个2101；root raw-f886dc79核完整3073幸存者/5960互返边/24详情/全玩家inc与120氢/J78/五网保持。结合原生同帧全实体/预建筑/连接保留证明，此项不再只是prepare预算；没有自动重建或保存，accepted1保留、primary仍41312641。只核销当前窄空仓恢复能力，替代落点和保存恢复仍须fresh独立证据。最近复验2026-09-10。

2026-09-10 / EXP-002/007/103/214/250/292十写复验validated：raw-f7fb5768完整核十唯一收据、3074/5960边/25详情、全玩家/J78/五网；实际恢复首读位置差0m，随后7.594354m另列为未归因变化，不能混为恢复误差。当前稳定状态与fresh hash是后续动作边界，raw-739a5027已实读旧hash被STALE_STATE拒绝。本条提交推送后十写归零；既有restore≤1mm、同owned/Journal/库存/拓扑/unknown隔离不变。先前等待“任意后续位移必须明确归因”的调用方附加门撤销，见EXP-293/IFX-102；不是得到用户确认或允许继续未决动作。最近复验2026-09-10。

## EXP-293 — 恢复时点与后续操作必须分开核验

- 日期/状态：2026-09-10，`validated`限本次调用方恢复审计与fresh预检。
- 根因：完整工厂审计结束后的玩家位置被拿来代表“刚恢复时位置”，把之后发生的位移误纳入1mm保存序列化门，额外等待用户确认，与既有混合操作/fresh-read流程冲突。
- 采用：首先以实际恢复后的原始带tick玩家快照核保存恢复，再以完整fresh世界、物资、Journal与稳定玩家作为下一动作现场。原动作必须有确定终态；不能以这个规则忽略outcome_unknown、quarantine、错误owned identity或不明物资/拓扑差量。未证明的位移来源保留未知，不伪造人工操作或原生漂移结论。
- 证据：实际恢复首读41312664与保存位置相同；后续7.594354m单列，raw-f7fb5768全量十写审计通过；raw-739a5027证明旧玩家hash遭STALE_STATE拒绝，raw-591eeb75的fresh空仓prepare通过且无写入。
- 限制：恢复首读缺失、跨时点差异含无法核销物资/实体/配置或仍在运动时，不以本例放行；所有旧plan废弃，新prepare/commit仍绑定当前hash并重新检查。完整窗口输出还须复查十唯一actionId，不能复用被后续电网循环覆盖的变量；raw-6e9b8fd1输出已由raw-f7fb5768替代。
- 最近复验/触发：2026-09-10；任何恢复、玩家介入、未决动作或审计helper变化时再验。

2026-09-10 / EXP-292原生预检复验validated仅限prepare：已安装cd83b28的raw-591eeb75对3064连接仓给INVALID_REQUEST，对3074默认空仓给prepared/commitAllowedNow及2101×1回收预算。全程gameWrites0、玩家hash/两仓配置与连接/J78/revision保持；不能把可执行计划或预算当实际拆除/到账。后续需fresh prepare再独立commit/终态/全场核验；accepted10位置归因门不因这次只读正例撤销。最近复验2026-09-10。

2026-09-10 / EXP-001/002/007/103/214/250与292冷部署复验：cd83b28全228文件/已安装MCP64/1/66807通过，protected resume由41312610重存41312641，首读玩家位置与保存一致。后续十写审计因7.594354m非Agent位移停止，raw-8cec9cc5再次稳定Walk0且库存/J78/原五网/healthy不变。恢复首读与之后位置变化必须分开，不能把米级差塞进0.001m序列化容差或重放resume。位移来源待用户确认；accepted10不归零、回收live仍待。最近复验2026-09-10，确认后再以fresh位置/状态哈希执行原业务预检，不沿用旧位置假设。

2026-09-10 / EXP-002/007/103/214/250部署前复验validated：raw-4981cae2核save41312610及完整3074/5960互返边/25详情、玩家120氢/原五网/J78；accepted9保留，提前审计不重置十写窗口。cd83b28的1797项测试、完整Release和staged MCP64/1/66807只证明候选工件，当前Plugin仍e4e12ff；EXP-292回收live仍待。正常关闭后才替换，新进程恢复将计为第十写并先完成跨进程证据审计。最近复验2026-09-10。

2026-09-10 / EXP-227精确槽诊断实机复验validated：e4e12ff恢复后raw-972373b1对原3074→3073仅一次fresh prepare，108对中实际nativeChecks2，原生lastNativeRejection为TooClose，后续TooSkew另列且不再遮蔽。玩家hash/端点/配置/Journal/revision保持，零commit；IFX-100限于错误可诊断性关闭。角度约6.7°和约1.74m端距都不足以单独授予准入，设备双端还受原生最小格跨度约束；不能用米距离大于0.9m就判“足够远”。空仓位置由root重设计，Luna不盲试、不自动拆迁；最近复验2026-09-10。
...
2026-09-10 / EXP-016/103/188/214/291有限启动复验validated：raw-376a4ec6三次各60氢即时双边守恒后，net5真实容量231000J/tick、六机均非零输出，再正常建成空载3073；root raw-800ac9ee核全3073/5960互返边/玩家120氢/J78。只有实际有燃料容量才计入后续满负载预算；发电buffer的joules_per_tick不是燃料件数，三仓各余36也不能将其差额直接当已燃烧量。四写保留accepted5，尚无工艺原料、持续燃料或新保存证据。调用方PowerShell脚本成功后不得用残留的LASTEXITCODE误报失败或重放；核精确终态和已持久化审计结果。最近复验2026-09-10，新增工艺负载/十写/保存恢复时再验。

2026-09-10 / EXP-002/007/103/214/250/291恢复复验validated：已核完的正常save41054080之后，工作恢复时游戏/descriptor均不存在；仅核228安装哈希后一次Steam启动。Steam先正常自更新，descriptor早期超时不是启动失败或重发许可。raw-035c9118实装MCP64/1与新Plugin认证通过；唯一protected resume e26b433e重存41054111，root raw-ecd69931核全3072/5960边/30详情、六送料真实持槽/跨度/供电、全玩家/300氢/J78及新票据。沿用≤1mm的原生保存位置微差边界，本次实测0.024mm；不修改hash、旧token不续用、不声称能恢复上次保存后的自然生产时间。accepted1保留，空载恢复不是有料持续生产；最近复验2026-09-10，投料/负载/十写及下次恢复再验。

2026-09-10 / EXP-016/103/188/214/291氢送料与保存十写复验validated：raw-9721e3c3六个普通2011唯一施工并save41054080，root raw-9a72106c核三仓/六分拣/save及全3072实体/5960互返边/30详情/全玩家/五网/J78。六条真实span1、filter1120、双方设备真实持槽、net5满供；这是空载正常施工/保存正例，不是有料吞吐或恢复实测。基础额定90/min来自当前STT与真实跨度，只用于比较单机满载18氢/min，不能代替现场送料。下一先以已留180氢守恒启动三对机组，确认实际capacity后才把额定供电用于对撞机负载；余120氢不动。最近复验2026-09-10，本条提交后十写归零，投料/新负载/下一十写与恢复再验。

2026-09-10 / EXP-016/103/188/214/291燃料空仓与冷启动复验：raw-6ab4f17d三仓正常施工由root raw-6e535eb9核原始/fresh终态、全玩家/J78/五网与原八电节点，此部分validated。同raw六路空载1120过滤预检为observed：实际仓槽9/3各接相向火电槽0，不能用旧模型估计槽号替代本次原生结果；各对端点独立。候选在火电自身4.5m覆盖内，满工作14700J/tick≤既有风电15000，保留无燃料时的启动供电，仍须建成后核实际powerServeRatio及span。零投氢，不能把有限180氢或额定功率当持续供给；accepted3不重置，最近复验2026-09-10，接线/投料/十写与保存恢复再验。

2026-09-10 / EXP-007/103/188/194/291北侧施工十写复验validated：raw-59535c17只完成3056之后七个未提交对象，root raw-f86da5d7核两Move/八build、3063实体/5936互返边/21详情/全玩家/五网/J78；原生材料与无人机施工、成功前缀不重放、0.1mm调用方后置门均通过。每对机组建成后未来仓/对撞机仍fresh原生预检，证明本次候选的逐步相容，不外推整图原子成功。net5只有计划八节点/六机组增量，空火电仍不计容量，实际仅900kW；燃料/分拣和持续运行待验，primary仍40815604。本条提交后十写归零，最近复验2026-09-10，接线/投料/十写与保存恢复再验。

2026-09-10 / EXP-007/103/188施工后置复验validated：raw-e4b29aec首塔3056正常完成、单份材料守恒，但私有脚本把玩家position的JSON字面相等作为静止条件而误停。root raw-feb1b9b6证明只有0.000018663m坐标差、Walk0/800MJ、原始/fresh成功终态一致、完整其他玩家字段/J78及仅net5一节点增量。下一施工的私有后置检查使用≤0.0001m有限容差并保留全部非位置守恒与settled门；这不改变产品state hash、原生prepare或设备位置标准，也不声称已确定微差的物理来源。超过界限仍停，不能把此正例外推为允许未知位移/订单。accepted3不归零，不重放3056；最近复验2026-09-10，后续节点/十写与保存恢复再验。

2026-09-10 / EXP-188/194/291北侧方案复核observed：当前原生地面规则对每个模型的landPoint作射线及水面高度比较，roughRadius或另一模型的Ok不替代它。raw-bb2dd194三个有界只读方位用于区分南侧浅水与北侧陆地后，raw-3bfab2f9全12个固定设备单独native预检通过；raw-7eea3735核预计只接net5、冷启动满基础882/900kW与未来有料12.99/13.86MW预算。未建/无料的额定火电不计当前capacity；实际连接、物流/功率和相互碰撞仍须每步fresh核验，不称整图原子批准。完整预算保留180氢燃料/120氢加工，尚非持续供给；accepted2不重置，最近复验2026-09-10，施工/供料及十写后再验。

2026-09-10 / EXP-188/194整图地面门复验validated：raw-7b53665b与raw-be1392ae的两个固定阵列各有不同火电候选被NeedGround拒绝，两次后停止，零施工；同组电塔/单机Ok只保存为后续设计输入，不授权分拆施工绕过整图检查。较大的对撞机曾通过放置，也不能据此推断周围任意小设备有足够地面。accepted2/revision45/primary40815604保持；下一先核原生地面判据及既有有效点，再给新的有限整图，不引入地形写入、任意布局或新动作。最近复验2026-09-10。

同期只读边界：raw-bba57930三个火电/四条送料链/11对象与完整旧工厂一致，热电设备槽可观察；2011实体本身没有可枚举建造槽的unavailable不否定它已有的双边连接。power-generation-current-tick中的0 J/tick只表示本tick发电，不能当作燃料件数或永久容量；配平继续使用原生额定档案与实际供料分别证明。末端干地规则经raw-cb7160c2实际稳定Walk再次通过，仍不推广为任意路线保证。

同日EXP-246/253施工区接近预检：raw-90b32b02的28.30m候选有中间浅水且末端连续干地仅2.9274m，调用方在零commit时拒绝。沿同一来向向陆地内延伸1.8m后，raw-d0929783完整32点/64射线/零unknown、30.0913m弧长、末端4.8534m连续干地，继续保持1.5m容差与4m门。只读预检通过不等于已Walk、无碰撞或施工通过；每腿实际执行后重新核销，失败不重放。accepted0/主档40815604保持，最近复验2026-09-10。

2026-09-10 / EXP-061/246/253与十写保存复验validated：退出原夹缝后，两条明确侧向腿及两条施工接近腿全部正常成功；没有重放失败目标、拆除旧障碍或用重启假装位置改变。raw-1132ec98核3取氢/1已知失败/5成功Move/1save40815604、全3055实体/5936互返边/13详情/全玩家/五网/J78，300氢与自然完成1416已被主档覆盖；本条提交后accepted10→0。恢复方法只证明本次候选链，仍非全局避障或任意干地可行保证。随后raw-37d09e73实距73.50m的原对撞机候选完整native prepare成功，再次说明先前OutOfReach不等于场地通过或失败；仍须整套供电/输送与提交前fresh预检。最近复验2026-09-10，后续施工/十写/保存恢复再验。

同日短退出实测：raw-5f59aae2的2.4m倒向目标由唯一Move 2cff1fa6在40784395成功，实际位移1.6871m，root raw-a5bec264核原始/fresh终态、全玩家/300氢/J78和当前近邻，accepted5/revision32。该小退出有效，但并不等于已离开整个工厂；下一只按已观察1491/899/900之间的间距分两条明确短腿绕行，每腿新预检和终态，首个失败或实际落点不在预审走廊即停。保持EXP-061/246/253，不能从一个成功点推断任意后续路径安全。

2026-09-10 / EXP-061/246/253卡位复验validated（失败检测，不是脱困）：raw-64deba4b的完整干地6.84m Move仍在旧设备夹缝中被181tick看门狗以position_stalled明确停止；root raw-dc79c023核四次accepted含该已知失败、此前300氢的逐笔守恒和未变设备/J78。无unknown/重放，accepted4不归零；正常保存仍40699158，不能说新取料已保存。

下一有界恢复设计为observed/待实测：旧返程raw-81e71128给出进入夹缝的大体来向，但没有连续轨迹，不能声称倒向必然已走通过。raw-32983b1d的4m倒向候选虽干地，却接近仓900中心，故不提交该点；把单次目标缩为同方向2.4m、保持1.5m原生容差，先只尝试退出首个夹缝，再fresh读回决定后续。距离是现场候选而非修改通用门槛；不延长watchdog、不把采样当碰撞保证、不改成功建筑。若这第二次同阶段Move仍失败，按现行两次上限冻结、落盘结论并由主会话重新设计，禁止第三次盲试。最近复验2026-09-10，关联EXP-057/061/081及存档日记001。

2026-09-10 / EXP-007/048/062/103/188/291辅助套件十写复验validated：raw-52669ee5的5取料/4普通craft/1save40699158由root raw-546045f5完整核销，3055实体/5936边/全玩家/五网/J78保持；只新增指定4仓/6基本分拣/2高速分拣/2塔。原生递归须先核完整配方及叶材料，recipe85的4批电路、recipe8的1批线圈已计入精确净扣料，直接itemBudget不冒充叶材料账。未新增首次时保留真实空增量。提交后十写归零，下一场地/取料/供电再验。

同批候选边界复验：raw-cb977df3已吸附的2310候选被原生OutOfReach拒绝，且实际无电力覆盖；这不是碰撞/地形/供电全通过。先安全接近再fresh完整预检，不复用旧token或从未报告碰撞推断场地合法。最近复验2026-09-10，实际移动/施工/供料与恢复继续验证。

2026-09-10 / EXP-007/016/048/062/103/289/291启动备料十写复验：raw-8f780157四次取料、三项显式普通制造及正常保存40658193完成六火电库存；root raw-9ffedbda核十唯一终态、全3055/5936互返边/全玩家/五网/J78和零预建筑，原两对撞机未动。本条提交后计数归零。J无新增事件是原始结果，不能因历史条目缺失强求“首次”或伪造事件；跨tick补货不抵消transfer同tick源端守恒收据。实机验证只覆盖本批制造/保存，不含供电运行。

同批重新采用EXP-016冷启动边界，原孤立无电分拣器死锁仍应预防，尚无新启动正例。raw-510f90b9利用已占3050的参照点取得net5原生满工作保留774kW/风电900kW；占位拒绝与电力预算分别陈述，不能把预算读数当场地通过。六火电目录额定12.96MW仅作未来有料预算，施工后必须核实际容量、连接、既有及新增满载和持续燃料。最近复验2026-09-10，后续施工/供料、十写和恢复重新验证。

2026-09-10 / EXP-007/048/062/103/289十写复验validated：raw-a844bf95的1416正常选择和save40522477成功，root raw-ae720457核十原始/fresh终态、全3055/5936边/21详情/全玩家与五网、完整durable78。1134自然解锁40513325，1416仍10498/180000；观察/选择/完成三时点分开，不把科研预算当即时背包扣料或把未解锁科技当可建。保留2对撞机但不计供电收益，有限纳米管支路已耗空且旧配置无变化。本条落盘后十写归零；最近复验2026-09-10，冷部署/恢复及后续施工重验。

## EXP-292 — 空仓布局纠正必须走可核销的原生回收

- 日期/状态：2026-09-10，`validated`限当前DLL研究、离线实现、冷部署、prepare正反例和单个空仓的正常回收；后续替代落点及覆盖保存恢复待验。
- 适用范围：完全空、默认、无叠层/配送器/其他功能或标注、无任何实体/预建筑/缓存引用的单个2101；不是所有仓库或一般删除。
- 结论：单体放置通过不等于业务接线通过。TooClose含原生格跨度条件；已成但未使用的错误布局应明确记账、正常回收后另行fresh施工，不放宽间距、直接改位置或掩盖规划错误。回收与重建是两次独立动作，不能假设ID不变或自动回滚。
- 证据：IFX-101复用DoDismantleObject；30格原生内容、双向/缓存引用与同层self指针独立证明，8192每池上限；回收核一个2101/count/inc及所有其他entity/prebuild/连接。49Core/1MCP新增后1797测试、完整DSP Release零警告错误和源码MCP64/1/66807指南通过。
- 实机预检：cd83b28全228文件冷部署后，raw-591eeb75拒绝已连接3064、允许默认空3074；无commit，材料预算不是已回收证明。
- 实际回收：3cf3429f于41383839仅回收3074，root raw-f886dc79独立核3073幸存实体/5960边、2101+1与全玩家inc/Journal保持；尚未重建/保存。
- 限制：只验证本次默认空仓。普通有货仓、过滤预约、叠层、未知引用或瞬拆设置均拒绝；正常UI回收可能触发通知，任何异常副作用只隔离核销、不重放。
- 复验触发：本次冷部署、首个回收/替代落点、保存恢复、DLL或拆除协议变化。
- 关联/最近复验：EXP-007/195/227/248，IFX-100/101；2026-09-10。

## EXP-291 — 燃料机组额定档案与实际供给分开

- 当前范围提升`validated`（当前DLL/只读档案与保存恢复）：安装MCP raw-43fdc14f观察火电36000/45000、聚变250000/250000 J/tick及各自mask1/2，氘棒600MJ；因此聚变基础额定15MW/满载1.5棒每分钟，火电效率基值0.8。冷部署与raw-de7e4c9a全场/玩家/J78恢复核销通过，1736离线测试及CI均绿。不证明实际燃料库存、首批重氢、持续供给或机组已建；不能拿15MW目录值提前抵扣Foundry实际capacity。最近复验2026-09-10，后续供料/负载/恢复再验。

- 同日离线修复复验：nullable三标量档案、旧DTO/非法范围/类型和MCP可发现性均有回归，Foundry不把该额定值加作未建/未供料机组可用容量，assessment hash不变。完整Release及1736测试通过；此处`validated`仅限离线策略/契约，不替代当前游戏额定数据及燃料/持续运行验收，安装仍efe12c2。最近复验2026-09-10；冷部署后再更新实机状态。

- 日期/状态：2026-09-10，`observed`；范围为当前DLL的普通火电/聚变及既定两对撞机支撑方案。
- 结论/证据：raw-ce34b3e9只有风力预测和五网当前capacity，不能据此给尚未安装的聚变机组赋额定值。当前PrefabDesc→NewGeneratorComponent的三标量与EnergyCap_Fuel/GenEnergyByFuel证明发电、满负载燃料能量和实际可用容量是不同证据；燃料档案尚未公开，IFX-099为直接预算blocker。
- 限制/下一步：只补充该有界只读档案；目录值非当前发电、库存或持续供给证明，不自动计入Foundry计划可用容量，未燃料化的新电站不能当免费供电。施工后仍需实际燃料/上游、供电和持续产出验证。最近复验2026-09-10，代码、冷部署、燃料变化、十写/恢复和DSP变化时重验；关联EXP-290、IFX-099。

2026-09-10 / EXP-007/048/062/103科研后置复验validated：1134已正常选择，root raw-d292feb1核八唯一原始/fresh终态与玩家/J77，不重放raw-f2a67799成功动作。revision是会话状态版本，不是accepted计数；当前普通research的Complete和启动尾都递增，单动作83→85有实现与实机双证，不能硬套+1。Journal首次选择用观察tick（本次40487227），不强等action终态40487225；核原始事件、身份/前缀、durability与真实观察区间。私有脚本错误不应被说成游戏失败或隔离；下一写须fresh actual revision/hash。accepted8不归零，最近复验2026-09-10；科技完成、后续聚变选择和十写重验。

2026-09-10 / EXP-007/048/062/103/289对撞机设备批次复验validated：raw-a90fb398的recipe39×2正常扣料、完整产物和save40442007经root raw-db80e552七终态/全玩家/J76/相关设备独立核销；无库存注入、重复制造或新增施工。首次事件40441345与整批终态40441945分别保留。两台背包设备仅关闭有限制造门，不能据此宣布24MW供电或持续氘/奇异物质成立；1134与1416仍需正常科研，发电机数量与燃料预算必须使用当前原生证据而非记忆中的额定值。accepted7不归零；最近复验2026-09-10，下一科研/供电、十写及恢复重验。

2026-09-10 / EXP-007/048/062/103/137/289框架启动批次复验validated：raw-e14e6030两指定静态来源正常取料、recipe80×40完整扣料/产物及save40421084成功；root raw-67b5afc9核五原始/fresh终态、全玩家及新J75/旧前缀，其他900原库存保持。recipe.timeSpend×批次是配方基础时长，不是含机甲制造速度后的实际耗时；以终态40421045核销整批，不把首次40411685当完成。跨自动收料的碎格经验沿用EXP-137，实际容量与数量分别核验。40框架加剩余原料只证明设备制作材料齐备，不证明设备供电或运行；accepted5保留，下一两设备/保存及十写重验。

2026-09-10 / EXP-007/048/062/103/214/289纳米管整批保存复验validated：raw-0d889a0d证明normal save40380434之前已停产、160全在3052，保存后两读仍160且所有原料/携货0；精确主档票据绑定J74。有限投料不能冒充持续供应，未重复首次；accepted1保留。补正本批私有容量草案的“conservative”命名：raw-ce34b3e9真实160产物占4个分散格而理论ceil仅2格，按stackSize取整是理想最少格数，不是自动入仓碎格情况下的保守上限。后续须保留实际格占用、原生容量预检和全批完成读回，不能用该理想数单独核销容量；本例没有满仓或丢料。最近复验2026-09-10；后续制作/供电和恢复重新核验。

2026-09-10 / EXP-007/048/103/214/289/290投料与十写复验validated：raw-07ed5a69两次正常转移有同tick仓增量80钛/240石墨烯，root raw-933a8d6c核十成功终态、全3055实体/5936边/玩家/五网及J73完整前缀。唯一新J74是真实1124流水线首次，不是强求或补造；64件实际入仓且厂在产，仍等待全部160完成和原料/在途清零。先缺第二原料时让第一原料原生缓冲收料稳定，再fresh追加的有限策略本例通过，不外推为无竞争或无限续料能力。跨tick分段数量只作观察，最终整批守恒须停产/输入与携货清零后复读；未来恢复或下一生产段重新验证。最近复验2026-09-10；此条落盘后十写归零，未声称已保存或持续供应。

2026-09-10 / EXP-007/103/188/214共享仓接线复验validated：raw-68d20bf5四个唯一成功动作完成recipe33和三2011，raw-98f686e8独立核八终态、全部互返/不同槽、三个实际span2、玩家净扣3sorter和durable73。两原料与输出均独立过滤，3051空置保留，没有拆改成功部分；45/45/45每分钟只是当前原生单件跨度容量，实际供料/产出尚未发生。共享仓追加第二原料前需等第一原料的设备缓冲收料稳定，再fresh完整hash准备；稳定观察不是免竞争许可。审计脚本的任务revision参数也必须避免dot-source旧默认同名覆盖；这次只读误断言经独立session72复读及参数隔离消除，不能据调用方错误重放游戏动作。最近复验2026-09-10；accepted8保留，供料/十写和保存恢复重验。

2026-09-10 / EXP-188/214有限直连改案复验observed：3051→3050被原生prepare拒绝（raw-3fda8e96，无commit），返回文本只保留最后槽对，不能将其TooSkew泛化为全部枚举槽对的共同根因。不重放失败对；保留成功厂/仓，另一侧3052进出两个独立槽对在raw-2fc6b3e6预检通过。有限启动批次可提议同仓分别按两原料/产物过滤，但必须逐件fresh核空闲槽、过滤/互返、原生跨度及容量和全物料守恒；不能据两个预检宣称第三槽或实际供给已通过。本次三个sorter尚未施工，3051保留空置、不把它冒充正在服务的输入仓；最近复验2026-09-10，后续施工/供料/十写重验。

2026-09-10 / EXP-007/103/188/290工坊本体复验validated：raw-d46e02b8只续第三风机和一厂双仓四件，root raw-d34e02ea核四原始/fresh终态、完整3052实体/5924互返边/14详情、全部玩家及durable73。原两风机和旧四网成员保持，新网实读900kW/一消费者；原生端点在厂/双仓建成后均可观察，不猜槽位或把之前单件预览当最终物流许可。新厂配方0/零输入输出，双仓空，3基础分拣器及启动材料仍在玩家；下一配置/连接/供料须fresh预检并实际核验过滤、双端、满负载和跨格容量。最近复验2026-09-10；accepted4保留、尚未保存、没有纳米管或持续供应结论，后续现场/端点变化及十写重新审计。

### EXP-290 — 动态可用发电容量不是固定拓扑指纹

- 日期/状态：2026-09-10，`validated`，限当前DLL和本次独立风网审计；关联IFX-098、EXP-188。
- 证据/结论：raw-7c093c8d的network1容量221000→185000 J/tick但节点/消费者/发电机数量保持且满供，raw-0e7f9775核三旧火电配置。当前原生EnergyCap_Fuel与PowerSystem.GameTick证明capacity是当tick可用量；必须分别核拓扑、当前供需和满负载预算，不把旧容量等值当结构安全条件，也不能忽略实际缺电。raw-e79fe1ca独立核十终态、3048实体/5924边/17详情、玩家/J73与唯一新增600kW/0消费者网，未放宽动作hash。
- 限制/复验触发：燃料、日照、设备/连网变化或DSP升级时重验；原功率buffer不是燃料库存。只证明本次新网未合并旧网，不证明旧厂全负载或持续燃料。公开指南仅源码更新，实机执行器修正已验；最近复验2026-09-10。

2026-09-10 / EXP-188计划互相约束补充：单件空场预检未包含将来新建邻居。3047/3048完成后，原第三风机raw-9ce368b6被WindTooClose拒绝，零新写；当前DLL明确wind节点平方距离须≥110.25。主会话只将尚未提交候选重新选到距3047为13.3088m、距3048为18.2153m处，raw-e437ac18完整native预检通过，未移动/重建成功两件；10.5m是当前版本原生下限，须留吸附余量并复核连网/旧场地。第三风机仍未施工；最近复验2026-09-10，布局或版本变化重验。

2026-09-10 / EXP-007/021/103/188/289备料闭环复验validated：稳定观察后仍有一次未接受prepare（raw-1551d84c），因此不能把稳定hash推成无竞争保证；raw-724799b2逐请求双端复核与raw-75891b94两源预检通过，raw-0aee4665仅完成剩余六动作，root raw-d37857be核八唯一终态、全工厂/材料/J73，无重放、未解释物料或改hash。3风机/2仓齐备不等于已供电；raw-9b7e93e3六新址的逐件原生正例仅证明当时单件场地。900kW独立网与774kW满负载预算必须再核实际新网、已有未供电负载、端口及物流，不能挪用旧网瞬时空闲或把新增数量当实际吞吐。最近复验2026-09-10；accepted8保留，下一仅两风机到十写审计，后续化工厂/仓/分拣器/投料仍待。

2026-09-10 / EXP-007/103仓储回补与未接受恢复复验validated：raw-53118e9b及raw-77eeb1e5各成功取一项后，第二项prepare均STALE_STATE、无commit；主会话停止同类连续尝试。root raw-5fee865d核两唯一终态、全玩家仅铁+29/磁铁+8及J73；723铁2671→2672→2700、26磁铁1792→1793→1800是原生自动回补，不是额外玩家转移。CanonicalStateHash.Factory绑定全部buffers，下一目标物品数量未变不能证明整仓hash仍有效；保留错误只给双端合并原因的证据限制，不声称知道拒绝瞬间哪个字段改变。后续先对完整仓状态做有界稳定观察，再fresh完整prepare/commit；本次26/95三读覆盖274/278tick且hash一致，仅是候选前提，不是准入保证。无需暂停或改造成功产线，更不能绕过内容hash、复用token或重跑成功前缀。最近复验2026-09-10；来源、自动入出料或玩家状态改变须重验，accepted2不归零，剩余六动作尚未执行。

2026-09-10 / EXP-007/018/062/103/287/289后缀与十写复验validated：raw-8f06def0仅完成两次正常制作、260石墨烯取回和save40032002，J73正确不增；raw-ade29279完整10原始/fresh终态、全3046实体/5924边/20详情/玩家/四网核销后才归零。合金三种投入全部耗尽、80产物全在唯一900，闭合有限批次；手中化工厂/分拣器仍不是投产。首个新厂原生预检raw-4bbefacd明确拒绝旧带1432碰撞，未动工也未消耗；沿用EXP-188/214/285，设备高度包围半径与中心间距不是水平放置许可，保留同位带体身份和成功旧线，下一候选需完整现场和材料/物流规划。最近复验2026-09-10；安装仍efe12c2，aaaa7e8指南仅源码/CI通过，无扩大动作域。

### EXP-289 — 历史种子没有首次时间戳，缺失条目不等于从未产出

- 日期/状态：2026-09-10，`validated`，限当前旧档挂接语义、这次六动作核销与对应离线回归；不外推无证据库存增量。
- 适用范围：`attached_existing_save`且历史覆盖不完整的Journal、有限手搓或生产后置验证。
- 证据：raw-5829a796中recipe57×8于39980594正常以16石矿制成8玻璃；raw-c25898e5独立复读六成功终态和全玩家/工厂，J73完整且持久化健康。精确当前Journal的1110/2011历史种子存在而无伪造首次，2309已有J5。
- 结论：Entries中没有物品不能推出“下次必有+1”。保留旧条目完整前缀并验证持久水位/健康，以原动作终态和材料账证明制作；历史种子抑制重复事件是正确行为。只继续未提交后缀，不重放已做玻璃，也不写入缺失历史来满足调用方断言。
- 限制/复验触发：历史模式、种子、记录边界或存档变化须重验；普通Agent不依赖私有文件访问，使用公开历史覆盖语义且不自行推定首次。真实新增事件仍须有实际证据、序列及durability，不允许忽略错误/pending。
- 关联：IFX-097、EXP-007/048/103、日记001。最近复验2026-09-10；公开MCP/包内指南此时仅源码离线更新，安装仍efe12c2。

2026-09-10 / EXP-007/062/076/246/287后缀与十写复验validated：在失败Move后只fresh提交已经可达的80钛业务和normal save39951562，raw-e672c12d两终态不重放先前八动作；raw-132b1a1d独立核9成功+1已知失败、全3046/5924边/21详情/玩家/J73之后才归零。旧石墨烯支路的390石墨+130酸此时全部耗尽，260产品全在唯一输出仓，闭合有限批次；合金仓25仍只能叫在产、不是80完成。既有首次记录不重记，失败前玩家快照不冒充失败后位置；继续区分已保存现场、有限制造和持续供给。最近复验2026-09-10；新配方/连接/科研目标、保存恢复和下一十写时重验，未放宽任何原生检查。

2026-09-10 / EXP-007/061/076/246/253返程停滞复验validated：raw-81e71128的历史Walk去程和160钛取料都成功，反向路线仍在181tick/余36.7547m产生原生position_stalled；成功去程或历史端点不是当前反向无碰撞保证。失败后的旧玩家快照只描述起点，必须重新读取真实位置：root raw-b93eb7e3实读到仓仅6.235m、Walk0/778.914MJ，原80钛业务prepare已经完整通过。因此不需要为“回到原站位”再提交Move，不重放失败目标，也不把已接受七个成功前缀重做。八终态/全玩家/12配置/J73已核；余投钛/保存未执行、accepted8保留。最近复验2026-09-10；此为已有指南的再次实证，不外推任意长路线/过时坐标，也不因客户端最后一条旧位置报告改变现场结论。

2026-09-10 / EXP-007/018/062/076/287/288启动制作与十写复验validated：raw-ba5000ab在末批工艺和传输全停止后证明438油/584石矿/292水→292酸的完整有限物料账；正常recipe103×50另有三项输入−100/−150/−50和环+50完整原始终态。J72首个环39856158早于整批终态39862038，必须分开记录首次事件与整批交付，不伪造自动环产线。raw-593ed117只取290酸/投130给已有石墨烯支路，863余2、160携带给合金；既有J32石墨烯首次不重记。raw-de44d234核10唯一终态/3046实体/5924边/22详情/全玩家/科研点/J69→72及正常save39875323后归零；石墨烯输出46只证明批次已启动，不等同260完成或持续供应。最近复验2026-09-10；取料/配方/过滤/科研目标变化仍须fresh预检，后续不重放成功前缀，有限启动材料不能抵扣Foundry/持续燃料与翘曲器门。

2026-09-10 / EXP-007/076/103有限批次守恒复验validated：末批260油追加前，raw-d6955f58在生产已停且输出全部到仓时核出29批116酸及4油/352石矿/176水余料，再按实际非空仓格13+新增整栈13≤30通过fresh预检。raw-4da731f9唯一转移、root raw-098601ed原始/fresh终态与全玩家油−260/J70/七件配置核销；后续跨tick读到仓285、全支路288不作原子292完成证据，等待末批和输出链真正清空。有限73批不外推为持续原料供给；不重复供料、不补造历史酸首次。最近复验2026-09-10；accepted5/revision8未归零，原primary39730547不冒充本段已保存，目标配方/库存容量/过滤变化时重新核验。

2026-09-10 / EXP-007/076/103/288分批酸启动复验validated：原生30格输入仓的166油/284水占用与584石矿完整native transfer预算先验，七件既有酸支路配置/过滤/供电保持；raw-663ffe37投料与1502正常入队，raw-faff4803核两终态、玩家仅石矿−584和J70，实际酸仓48。1502奖励表已观察为空，且玩家蓝/红及隐藏研究点均0，科研正常增长不改变持有库存；不根据旧目录unlocked字段认定当前科技状态。原J28已经是酸首次，不补造新首次。174油/232石矿/116水的首29批最多116酸，原总预算438油/584石矿/292水对应73批292酸；剩余260油仍随身，容量未fresh通过前不能追加。瞬时working=false不是持续stall，有限产出也不是持续供应。最近复验2026-09-10；accepted4/revision7/J70保留，后续供料/配方或科研目标变化时复验。

2026-09-10 / EXP-007/018/246/253接近复验validated：两次末端仍浅水的短预检拒绝后，停止同类猜点；只复验历史实际Walk落点和当前完整3046场地，按原有长跨越条款单次80.184m Move。raw-fbe996b6成功后真正Walk，再直接使用可达的石矿transfer，未再撞建筑中心；raw-b6ac5e0c核两终态/玩家仅石矿+584/J69/八配置。源95原生同tick1100→516后自然回补582，不用隔时差替代即时守恒；到达791.385MJ后正常充回800MJ，无能量写入。长预览unavailable不因此变成无碰撞/干地保证，未来仍须当前障碍和能源/终态验证，不外推为任意长路线。最近复验2026-09-10；accepted2保留、原primary39730547未覆盖本段。

2026-09-10 / EXP-001/007/008/288诊断保全复验：原8动作均在重启前保留唯一原始/fresh终态，后续科技奖励具体元数据仍待。只执行一次正常save保存当前未核销现场39730515，而非回滚到旧primary39445023；root raw-566a121e证明全3046/5924边、14相关配置/有限库存、玩家/J69和新精确主档票据。保存本身不核销太阳能板1，不开放普通后续写入或归零accepted9；该保全例外只服务已实现的直接阻塞诊断冷部署。恢复后仍须具体奖励表、旧unlockTick、前后完整库存/容量及原始动作边界共同证明。staged1714同源228文件及真实MCP64/1新指南已验，但尚未安装/live。最近复验2026-09-10；重启或任何额外物料/拓扑变化时重验。

2026-09-10 / EXP-007/018/063/103/137暂存与科研复验validated：raw-83fbef9a明确转出400硅/77蓝至无自动出边900，raw-e1a1cd95核精确1002钻石/400硅/77蓝、原设置和三终态/J69；零玩家蓝使当前1501正常研究不与机甲隐藏点消费混淆，背包实际17格。暂存不是物料消耗，后续40硅仍须守恒取回。矩阵类正常转移的调用方审计须允许唯一动作所证明的显式3600点/件净量，不应一概要求总点数0变化；未声明变化与inc变化仍拒绝，三离线正负例通过。未改产品写入路径、hash或未知结果政策。最近复验2026-09-10；存储出边/容量、研究供给或动作物料变化时重验，accepted6不归零，主档39445023未包含本段。

2026-09-10 / EXP-103/137/246/253及显式未接受恢复复验validated：石墨50仍占2格、研究结束后蓝77/黄40退回，使实际背包25格而非按剩余数量推算的22格；油水调用方改为读取真实slotCount并预留科研退回空间，最大37/40。raw-2d306592仅前两转移accepted，后续STALE_STATE明确未接受；root证明油41分布于仓29/机12后，仅fresh提交剩余水，raw-bf5ce8d2成功，原前缀不重做。raw-6bc67377核3终态、全背包/科研点、既有酸支路、源水回补600、供电/J68；有限油水不是硫酸产出。两次原生移动预览末端实际低于水面0.640/0.056m，后一shoreRisk=not_detected不推翻实际射线高度、不能当干地；零Move写入后停止相似候选并改查历史Walk。最近复验2026-09-10；原生堆叠/科研、场地或库存用途变化时重验，未改变hash/准入/游戏状态来迁就调用方。

2026-09-10 / EXP-007/018/076/287十写及分批容量复验validated：raw-1f7469f5经正常转移将390石墨放入既有870，raw-ff93fb3c按仓/分拣器/869总和核验，不把自动转运误作损失；无硫酸时不宣称recipe31已生产。完整10终态/3046实体/5924边/28详情/J68/玩家及供电审计通过，落盘后归零；主档39445023早于39495861投料，明确尚未保存该动作。运行catalog的油/水/硫酸stackSize均20，石矿100；73批硫酸原料438油/584石矿/292水合计43格，大于单仓30格，不能只核总件数或把背包40格当仓容量。应固定分批、每步fresh预算并证明原生产库存保留；不以清空黄糖上游或临时投料冒充持续供应。最近复验2026-09-10；容量/过滤/出边/现场或配方变化时重验。

2026-09-10 / EXP-007/018/076/246/253/287启动批次复验validated：raw-a7b8428b只有暂存金刚石accepted，后续取板明确STALE_STATE/result=null；root先核销成功前缀，再仅fresh重建未接受取料。原已知Walk锚点之间的较长直线仍可被建筑阻断，raw-0cb93cf5的181tick stall不撤销此前两短Move，也不允许相同失败目标重试；失败后的玩家位置须真正fresh读取，不能沿用动作前缓存。raw-630228ed另一6.493m短接近成功，业务prepare可达便停止移动。raw-b3f9e357使用既有853/849/854实际消耗32电路和32微晶、产出并回收16处理器，只有有限输入，不算持续供应；旧J29首次记录保持。raw-16ac68b8核9终态/3046实体/5924边/22详情/全部玩家物料与J68，保存39445023/rev27/healthy，accepted仍9。临时仓900仅有入边、1002金刚石仍可唯一读回且留19格给原产物；这不是任意混仓或改造旧线的通用许可。最近复验2026-09-10；恢复、源/过滤/配方/出边或库存用途变化时重验。

2026-09-10 / EXP-246/253的末端干地门复验validated：raw-fb11e553/e3349135为两个不同的30m级接近点，native prepare均通过，但完整采样的末端低于水面1.686m/0.765m；外部执行器正确在零commit时拒绝。不能把prepare成功、全程有射线命中或沿建筑方向移动当作终点可Walk，也不能把调用方预检拒绝记成已执行Move失败。两次后停止同类合成目标，回到明确历史Walk/现场复验设计；不借此放宽门槛或增加自动选路。855/856过滤编号须从实际字段读出，857对单输出recipe51的无过滤状态保持，不为匹配脚本臆测改动成功部分。最近复验2026-09-10，accepted0/rev15/J68/主档39295217，尚无新动作。

2026-09-10 / EXP-007/018/246/253/287十写复验validated：raw-1d949425的两台对撞机显式配方展开只有8项启动原料、零舍入余量，不包含24MW之外的基础设施预算或持续供给证明。raw-e277afdc从2992/723/827/792分别取石墨233/磁铁53/涡轮100/钢80，保留原线库存；两次Move使用新的prepare，短水段末端实测干地5.807m≥4m且成功后Walk0，不能外推为无碰撞寻路。流动库存的守恒以同tick action借记和全玩家差量核验，不要求隔时仓数恰差转移量。raw-da85a007完整核销10个原始/fresh终态、3046实体/5924互返边/16额外详情/0prebuild、全玩家材料科研点、四网和J68，正常保存39295217/rev15/healthy；落盘后accepted10→0。剩余钛/硫酸/处理器、建材科技及自动供给须fresh验证，未把库存筹集标为制造或生产线完成。最近复验2026-09-10；重启、来源/配方/科技/电网或现场变化时重验。

### EXP-288 — 原生科技奖励与动作后库存增量要分开核销

- 日期/状态：2026-09-10，`validated`；仅当前DLL字段语义及本次1501奖励的完整本机核销，非通用增量豁免。
- 适用范围：当前DSP0.10.34.28529，owned世界正常科技完成与跨tick玩家物料审计，不是任意增量豁免。
- 证据：两次油转移结束39613767，最后原始玩家39613814无2205；1501解锁39617713后raw-b9f2f612观察太阳能板1。DLL的UnlockTech按AddItems/AddItemCounts发奖，GainTechAwards走普通TryAddItemToPackage；现有progression未公开具体奖励表。
- 结论：先保留成功动作与事件时间边界，停止后续写入，不重放动作；只有奖励元数据、解锁事件、真实前后物料和容量吻合才能解释增量。奖励既不是手搓/流水线产出，也不应回填Journal首次生产；只读奖励表本身不是投递收据。
- 限制/复验触发：目录缺失、未知项/容量溢出、多个可能来源或版本/Mod/现场变化时不能推断；当前仍待只读补丁与同档复验，不放宽hash或outcome_unknown。
- 关联：IFX-096、EXP-007/063/103/239、存档日记001。最近复验：2026-09-10。

- 2026-09-10离线补丁复验：现有progression公开`completionItemRewards`，Core对两数组有界整表检查/深复制，未知返回null而不是空/部分表。21新增回归涵盖兼容、顺序/重复、上限、非法/未知、旧hash不变/真实解锁仍失效和MCP指南发现；完整Release1714通过。只读元数据不是投递凭证，原玩家全量核销、accepted8边界与后续冻结仍保持；本条具体1501奖励结论未因单元测试提升为live validated。

- 后续实机核销：efe12c2冷部署/同档恢复39730547后，raw-22ff6292实际MCP给1501→2205×1；raw-7cc37975独立核原39613814无板/9空格、39617713唯一解锁、仅板+1且inc0、其他玩家及油账/科研点、J69无伪造首次生产，全3046实体/5924边/14配置和10唯一终态保持。恢复位置仅原生序列化偏差0.000017132m（预先界定1mm），未写位置。最新结论覆盖上面“仍待”历史截面；accepted10在本次落盘后归零，继续原有限材料主线，不重放补油。没有放宽hash、unknown隔离或奖励容量/多来源条件。

### EXP-287 — 终端配方已解锁不证明建材、持续上游和功率已就绪

- 日期/最近复验：2026-09-10；状态 `validated`，仅限本次运行目录、九来源限定调查及两次正常选科证据。
- 结论：分别检查终端配方、设备建造配方的每项原料前置、真实输入来源/库存/工作状态和满基础负载电力。只读物料图不能提供施工权；旧设备存在、曾经生产或其输出仓有库存均不证明当前持续供给。限定两跳未见某物品不等于全厂不存在。
- 证据：raw-39d26974在39197700完成58唯一详情；涡轮827=3000/电机727=3000/钛块531=2985，但粒子容器883、石墨烯869、处理器853、钛合金1491及硫酸861缺输入，899仅651硅石。recipe39已解锁，其建材仍缺1132/1711/1521；两台2310按原生workEnergyPerTick=200000各需12MW，不能靠当前空闲机器的瞬时余量承诺全负载。
- 执行复验：root原生无副作用预检通过后，Luna1132→1711两项正常选科，raw-0f6c3561；root raw-d08c4450核销两唯一终态/零同步物料差、12配置/全玩家科研预约点、原J66及新J67/68。1132自然hash11020/135000，仍未解锁；accepted3/revision6/primary39144575，不是产线完成或已保存本次科研。
- 限制：普通enqueue没有优先级调整的专用researchQueueReadback，不能伪造该字段；本次用既有原始/当前终态、同tick物料差及相邻progression证明选择。三级翘曲器3/min仍仅物料草案，未通过原生场地、完整运输/功率或持续产出。
- 复验触发：解锁下一前置、改变原料来源/目标速率/布局、电网变化、重启或将草案转入prepare；关联EXP-239/267、存档日记001。37项相关自动测试通过，没有新增工具、白名单或游戏旁路。

2026-09-10 / EXP-286及原Governor声明复验validated：raw-efc1b193从15个已记录原生窗独立重算union37128tick，58–66/min满足原62±10%，同时保留历史resetReason为unobserved_game_tick_gap；后续成功不抹去原55失败或补回缺口。实际双仓持续入货、源/拓扑/材料与采样供电/告警证据分别复核，不能用P=C、机器翻倍或旧基线重定价替代。raw-f4092988/93df9d6a随后正常保存39144575并核销，accepted1/J66。通过的样本作为这次已完成实验的受保护外部证据保留；运行内采样仍non-durable，下一重启若再观察必须从0开始，不把历史报告写回当前计数。最近复验2026-09-10。

2026-09-10 / EXP-192/195/231复制运行门复验validated：raw-f1f7c8ed三独立原生窗与五次双仓增长，联合原矿持续自动出煤、明确过滤/互返线路、五附件供电及19配置fresh保全，证明复制模块并非只靠预装原料或一次入仓。两仓按各自tick净入约15.177/14.812/min，不能把整颗星球计数当成逐炉产量。模块持续生产与原31→62 Governor严格误差是两个验收项；第三窗55/min越界的失败仍保留。无需因通过局部门而重做已成功施工、修改目标或重锁声明。最近复验2026-09-10，accepted0。

### EXP-286 — 最近一次重置原因不等于当前窗口失败

- 日期/最近复验：2026-09-10；状态 `validated`，范围为Governor现有DTO解释和有界外部采样。
- 结论：`throughputValidation.resetReason`保留历史原因；用当前实际速率、锁定误差、窗口边界、当前健康/归因和连续覆盖共同判定，不能将相同原因每次出现计为新失败。重复失败必须对应真正失败的独立窗口。
- 证据：raw-dbb13bca的60/57/55/59/60仅55实际越界，第五窗连续3600仍保留历史原因。私有脚本误停后修正，raw-9afbdce3按真实缺口重置再观测。新增Core状态序列与MCP嵌入指南测试，87项相关测试通过。
- 限制/复验触发：不改变原31→62、±10%、36000tick声明，不让调用方补录历史信用；完整十分钟仍在观察。公开指南尚未冷部署，下一同批构建/部署及DTO语义变更须复验。关联IFX-095与日记001。

2026-09-10 / EXP-001/007/008/192/195/231十写与重启复验validated：raw-d966baa8按保存前独立核销的九个原始终态和恢复后的唯一新终态串联10写，不要求跨进程缓存重新提供旧action，也绝不重提旧动作；完整3046实体/141详情/互返边、物料、科研点及J66逐项保持。所有五外部附件恢复后仍有电、两仓有真实产物。原闭图外接提示继续准确保留，不能误用旧completed buildId；原Governor声明恢复不等于历史采样恢复，continuous0且samples非durable。accepted10归零的依据是完整审计和本次落盘，不是简单重启。最近复验2026-09-10。

2026-09-10 / EXP-001/007/192保存范围复验validated：接线后先确认两输出仓真实收到石墨，再普通保存至39067604；raw-d55d9f80与root raw-cf36fb5c相同成功终态、24实体配置、零材料变化及J66一致。保存只证明本次世界进度落盘和最新healthy resume capability，不代替重启后的连接/材料/产量复验；accepted9不归零，下一恢复是第10写，恢复前后不复用旧token或旧闭图续建计划。最近复验2026-09-10。

2026-09-10 / EXP-195/231全接线施工范围validated：raw-93cf8365只续剩余A输入/高速煤源，真实span3/stt600000/300000与批准预算一致；raw-288a9b84全3046/475/141/5924边及8动作/完整物料/J66核销。重设计必须按真实跨度与source供电执行，三前缀和补电均未重做。首个3600tick窗口60石墨/min、目标链finding空只算初步流量；原31→62声明不动，源配置变化会把连续观察置0，不能累计此前停工或未采样时间。下一保存恢复后以完整窗口证明持续供给/两模块实际输出，accepted8不归零。最近复验2026-09-10。

2026-09-10 / EXP-195补电窄live通过：raw-755acc54以普通单塔build造3044，raw-4d39fa3b独立证明3043真实进network2、全原连接保持、唯一2201借记和四网结构仅1节点/1消费者增量；单塔只读power预测与实机一致。当前两未提交sorter的source与新实体锚点一致，既有Foundry电力探针的实际吸附位置必须与该点相符；设备接收slot可能off-grid，吸附到附近网格的探针不算同点证据。剩余900J/tick预算及source覆盖已复验，建后stt/电网/真实供料仍待。最近复验2026-09-10，accepted6不归零。

2026-09-10 / EXP-195补电方案预检：raw-ccd5fba6显式选择owned塔2985导出单对象数据，仅以现有inspect_blueprint的原生power assessment复验备用2201落点，不创建蓝图施工计划。该点与普通prepare的真实吸附位置一致，只连接network2，不合网，唯一新覆盖为300J/tick的已建分拣器消费者；原保留峰值75000、容量85000，补电后余量9700J/tick，尚未施工。说明文字仍为数据、只读结果不提供写token；下一单塔普通prepare/commit与3043真实有电复读不可省略。最近复验2026-09-10。

2026-09-10 / EXP-195/231供电前提反证：原B入口改为3027后，native放置和双端通过不代表新source端位置有电。raw-bc9b7623/20531804证明3043跨度2/连接正确却network0，原炉2989仍network2。前批“全接点覆盖”的隐含前提invalidated；已成功三对象/借记保留，accepted5，后两项停止。补电必须在现有目录真实cover/connect半径、完整现场和原生预检下证明无意外合网/新增负载，再逐对象读回；不放松完工供电断言。备用塔只是预检候选，最近复验2026-09-10。

2026-09-10 / EXP-192闭图与运行图边界：BlueprintConnectionsMatch会检查所有原生槽，故原6对象施工completed后添加外部输入/输出会给construction_topology_changed_or_unproven。raw-7e24b65e/20531804的原对象字段和借记完全不变；独立全图比较逐边证明差异只来自本批已授权3041–3043。不能强求运行产线仍是原封闭施工图、删除该提示或复用旧buildId续建；运行态核验应保留原内部边、全部实体身份、明确每次普通附件及其双向边，而非忽略所有外部差异。只记录该已解释边界，不削弱Plugin的施工/恢复安全检查。

2026-09-10 / EXP-195/231接入预检复验：raw-0bc17c0f在实际持有4基础+2高速、正常返程且原煤路拓扑保持后，对四个基础接口和112→2995高速入口全部fresh通过。此前基础入口的正例没有外推为高速正例；材料短缺不当成场地通过。现有4基础+1高速的额定工作功率与原6基础预算相同，四网满供，仍须施工后逐件核对实际跨度、电网、原连接和货物过滤，再测两模块与上游持续流量。当前只读范围validated，施工/公平配分/倍产待验；accepted2不归零，最近复验2026-09-10。

2026-09-10 / EXP-018/195/231/246/253十写复核：raw-48610d41完整核销两仓、一save、三段带、两次Move、一次正常转移和一次recipe88的10个唯一accepted；两个Move跨不同执行raw，失败的第二prepare不计写。新深入落点Move为成功终态且Walk0，末端干地准则获得当前局部live正例；不能外推通用寻路。源727持续生产，因此物料守恒采用action同tick3000→2999与全玩家前后差量，而非隔时仓库存。全3040实体/135详情/5904互返边、完整材料科研点/J66及四网通过，accepted归零；当前4基础+2高速是实际库存，不是供料/吞吐验收。最近复验2026-09-10。

2026-09-10 / EXP-246/253的落点容差复验：raw-54ed6abd首Move成功后，第二段原落点的fresh末端干地仅3.93998m，调用方在commit之前正确拒绝；accepted7保留，不能把下段prepare异常记成上段terminal失败或重放成功前缀。root独立核销后沿相同接近方向将落点深入干地1.5m，raw-b5131292得30.063m/32点/零unknown/5.8187m干地，仍用1.5m到达容差和≥4m干地门。该只读候选为observed，真实Move/稳定Walk待验；安全阈值未放宽，地表采样仍不是碰撞路径证明。37项移动回归通过，最近复验2026-09-10。

2026-09-10 / EXP-231/195有限重设计复验：原B输入候选invalidated后，raw-5bd81ed8用真实直线3027而非拐角3025接B(slot0)原生通过，A和112→2995基础连接也通过；这不是高速版本或持续流量通过。南侧新带碰旧2721、北延碰旧662两次占位负例后停建，不能第三次盲试。当前改用已有grade2/2012的候选；原生catalog确认三跨度满电单件上限60/min，实际recipe88需电动机而非记忆中的磁线圈。以现档真实配方/库存决定正常备料，不能为额定速度绕过材料/碰撞，也不能把支路容量等于公平分配。原成功蓝图/47带/仓都保留；最近复验2026-09-10，accepted6。

2026-09-10 / 私有只读几何分析纠错：raw-fc1d1509因PowerShell选择Math.Min/Max整数重载把方向余弦取整、产生伪0°，在任何新commit前弃用；raw-8ad7fcdf改用显式double并通过45°对角反例后，3025偏差44.86°、3027为4.75°，与后者原生正预检相符。只读几何排序始终不是原生放置授权；不将弃用分析写成实机成功，也未改Plugin阈值。后续脚本保留该回归检查。

2026-09-10 / EXP-231外部分拣器假设反证：raw-8ffbdeab中3025→2989原生TooSkew/no_admissible_seed，未进入native候选放置检查；3025是后续source-cover改变几何后的实际拐角，而非最初预想自由尾。原“此尾可供B炉、span3”的拟议条件降为invalidated，不能把完整带路成功当设备接入也成功。依据当前sorterEndpoints筛选下一明确端点，且更换端点会改变真实stt/span与支路配额，必须重算后再写；禁止为追求原计划跳过面向检查、重试相同pair或重建成功带。两输出正预检仍仅是预检，不是已施工。最近复验2026-09-10，accepted6、零新增写。

2026-09-10 / EXP-231有限source-cover实机复验：raw-46d293f2在原批准32点自由带后，仅以实际尾3025/3026和新的endpoint hash追加2/13 NEW；旧尾的原生朝向变化由既有whole_path_native_rotation_v1证明，材料只扣47新对象、不扣保留源。raw-2ac15b62核销全3040/5904互返边及旧2993配置/全材料/J66，当前范围validated。原生无人机完工ID不是路径序号（3027在3026之前、3029在3028之前）；必须用规划位置唯一匹配，并以连接反证错误排序。空带施工完毕不等于供料；后续六端点分别fresh，accepted6不归零。最近复验2026-09-10。

2026-09-10 / EXP-195输出边界阶段复验：原六对象蓝图施工闭合后，raw-3c0b0fc9用两个独立fresh prepare/commit造出原批准空仓2992/2993，再正常保存38758959；raw-342661a0独立核销三终态、全2993/469配置/双向边和完整玩家材料/科研点/J66，仅2101少2。该范围validated；空仓存在或save成功都不证明模块持续产出，后续煤路与输出sorter仍各自原生预检，不能重放已建仓。accepted3继续累计，最近复验2026-09-10。

2026-09-10 / EXP-192/195取消与重启续建实机施工范围validated：raw-2b1a372e在原buildId/原site上只提交四个not_submitted，保留2986/2987和原借记，6/6终态完成；逐对象无人机施工、配方/过滤/四对内边/供电及正常材料证明齐备。raw-5fa74868全2991/467/85、全物料/科研点/J66与10个唯一accepted核销通过，历史unknown原样保留且不能按零写重试。该证据关闭有限施工、取消、保存重启和只续未完成部分，不替代输入输出接通及持续产出。十写落盘后归零，最近复验2026-09-10。

2026-09-10 / EXP-195审计对自然矿竭的边界复验：106在持续运行中覆盖点6→5，唯一减少318，fresh原节点为INVALID_ENTITY且其余五点均正余量/同煤组/唯一矿机；位置、朝向、配方、供电与旧边完全保持。仅对这个有独立证据的变化更新当前基线，不能全局忽略resourceNodeIds或编造耗尽tick。原六点矿速假设降为superseded，下一供料和Governor窗口依据fresh五点资源、真实出煤及全链速率；不因设备数量未变就假定供给不变。raw-de68faac/raw-5fa74868，最近复验2026-09-10。

2026-09-10 / EXP-195/271/282恢复边界复验：隔离来源仍不授予Governor历史声明恢复证明；先保全实体/材料并正常健康保存38707623，再按新healthy票据exact-primary重启，raw-df900fa6正常attach原hash/31→62/锁38461509，连续观察保持0。蓝图两项完成及四项未提交、全厂/玩家/J66均保全，不需编辑归档、替换buildId或重锁基线。此闭环在当前9514d84本机validated；未观察到的采样间隙仍不得拼接，接料后的连续2×实验尚待。最近复验2026-09-10，accepted9保留。

2026-09-10 / EXP-195显式停止实机补证：原unknown带已由独立原生证明核销后，raw-532abc38的新prepare/cancel只改变有限计划停止状态；2个已建对象、4个未提交对象及所有材料收据完全保留，背包/科研点/J66零差异。此正例不授权清除真正未证明的对象：若pending/submitting/unknown仍无唯一世界证据，cancel也不能把它们变成可重做。accepted7，普通保存/健康声明恢复及剩余施工仍待，最近复验2026-09-10。

2026-09-10 / EXP-195/231实机复验：9514d84从隔离状态正常关闭，经fresh固定LastExit及J66门恢复到38674189；raw-67acc3ab用当前原生路径/碰撞体证明原idx2带2987，保留唯一实体与101→100原借记。全2987结构/464非带配置/81详情、互返边、材料科研点及四网通过，局部完成识别升级为validated。对象得到证明不自动把原unknown计划改成运行；须原取消入口停止未提交部分，再用新token继续明确剩余项。隔离票据不是Governor健康计划重启凭据，不能放宽该门或重锁扩产后基线；下一普通健康保存/计划恢复仍须证明原31→62、锁38461509及连续观察归零。最近复验2026-09-10，accepted6，关联IFX-094/IFX-093；剩余四对象及持续产出尚待。

2026-09-10 / EXP-231复用到蓝图完成态的离线修复：当前DLL再次确认正常CargoTraffic完工覆盖entity/collider朝向；BlueprintEntityMatches改用已存在的精确只读路径/碰撞体证明，预建筑球面朝向检查不变。新回归证明捕获四元数的微小未解释变化仍拒绝，以及pending结果重新证明后仍保持原unknown停止，须显式停止未提交工作与fresh新授权，不自动重做。50相关/1691全套Release与当前Plugin构建通过；原动作实机解析/同档恢复待验，accepted5/quarantine保留。最近复验2026-09-10，关联IFX-094。

2026-09-10 / EXP-192/195/231反例：原蓝图续建首个belt在正常扣料及预建筑后触发完成匹配失败，raw-0a468c9a保留accepted5/idx2 pending，raw-9d3434be现场有精确位置的空带2987；不能因异常重试整图或忽略扣料。预建筑球面朝向与正常CargoTraffic完工渲染朝向不是同一证明；普通source-cover已验证的只读原生路径/碰撞体公式应在蓝图完工同样核对，而非简单忽略rotation。IFX-094尚待修复/唯一性及隔离恢复实测，原锁保持31→62。最近复验2026-09-10。

2026-09-10 / EXP-195/282与IFX-093实机复验通过：e336604同源冷部署后，原primary38583441受保护恢复为38583472；root raw-ed745c72证明原部分蓝图所有对象/借记记录、完整2986实体/464配置/80详情及材料/J66连续。旧v2声明未经文件编辑原样attach，原hash、31→62、锁38461509保持，连续观察归零；已完成炉带来的源配置变化只重置观察，不更换基线。该范围升级为实机validated，但五对象续建/接料/持续产出与2×十分钟仍不能据此核销。最近复验2026-09-10，accepted4保留。

2026-09-10 / EXP-282与IFX-093离线修复：私有声明v3按decimal数值规范哈希，v1/v2只窄兼容整数JSON补.0导致的旧表示，实际磁盘归档不重写。正式锁定前必须用Plugin实际序列化器往返验证，不以另一序列化器的测试代表持久化。25新增/153相关/1689全套Release通过；raw-70fe3036已只读用新Core验证原31→62声明，原文件SHA保持、原锁/目标不变、连续观察0。旧安装态尚未替换，后续施工仍等cold/live原声明复验；最近复验2026-09-10，accepted2不变。

2026-09-10 / EXP-282持久化反例与EXP-195恢复复验：d8c0cf9冷部署及protected resume38525680后，原部分蓝图的对象、材料和全厂拓扑均保持；root raw-b815e51a完成2986/464/80细核，但原Governor锁无法重新attach。raw-a4324cc7证明是decimal scale参与旧完整性哈希，Plugin JSON把31写为31.0；归档数值/身份未坏，不能用重新声明或编辑原文件解决。相关“持久声明可跨重启”结论暂仅保留离线范围，实机受IFX-093阻塞；修复须覆盖实际Plugin JSON往返及旧记录的数值等价格式，保持原hash、2×目标和锁定时刻。最近复验2026-09-10，accepted2保留，下一游戏施工冻结。

2026-09-10 / EXP-069/195恢复覆盖前提复验：原有限计划取消后，raw-d3634910正常保存38525649同时覆盖游戏中已建炉与此前持久Governor声明；root raw-85a651cb核销原终态、全物料/科研点/J66及同buildId状态。raw-9e4a5282再次核对同批新旧228文件和正常保存/进程前提；仅是可重启前置，不等于计划已经跨session恢复。后续不能读取旧save或重新生成整图来绕过持久进度；最近复验2026-09-10，accepted1保留。

2026-09-10 / EXP-195取消与十写实机复验：raw-9f1668ed使原部分计划从paused到cancelled，已完成炉及一物品持久借记保持，五项未提交，没有拆除/退款；root raw-8ee2dee1十唯一终态、2986实体/464配置/80详情/全厂双向边和全部物料/科研点/J66核销通过。只有本条落盘后accepted10归零；保存覆盖与新session原buildId续建仍另验，cancelled不等于删除计划或已回滚世界。零预算取消不能复用要求非空数组的私有建造预算断言；该预览层问题已在零写状态纠正，运行时原生取消未受影响。最近复验2026-09-10。

2026-09-10 / EXP-192/195有限执行首个实机正例：raw-9e18bc1a完整六对象预算通过后只批准maximumObjectsToSubmit=1，正常造出idx0炉2986即terminal paused，保留五项not_submitted及独立buildId；root raw-a894342d验证一物品借记、持久前后库存hash、真实配置和供电。暂停不是失败、全部完成或取消；后续须针对同buildId单独取消/保存/fresh续建，不能重新提交整图。该原生子对象另推进revision，因此调用方从实际会话读回94，不按普通build固定两次递增猜测。最近复验2026-09-10，取消/重启/持续产出仍待。

2026-09-10 / EXP-188/194补电正例：已锁Governor后，raw-a09cc15c仅按五个固定原生候选逐座施工；root raw-4f867e33证明四风机/一塔唯一新实体、全材料守恒和旧2980配置/全厂双向边不变，network2实际容量85000J/tick、其他三网结构保持，无新覆盖消费者漏计。建后同一六对象整图的原生放置和满工作功率/覆盖全部通过，关闭该模块基础供电阶段；未来外部sorter仍须逐个fresh端点和实际供电核验，不能把这五座施工当复制模块已经产出。最近复验2026-09-10，accepted8保留，正常保存恢复仍待。

2026-09-10 / EXP-282预执行持久锁定实机正例：raw-dc840149固定三窗31/34/28，精确均值31，先重新定价62再立即用新服务端提案锁定38461509；root raw-84bd3a05证明同revision81、无扩产写、无源变化、声明durable且2×/0.1/36000不可变。10800基线覆盖与276间隙不拼成验证窗；锁定后的源变化/取消重启仍须从零重新积累连续合格观察，不回填旧样本。下一施工绑定原声明，不重选旧基线或第四窗。最近复验2026-09-10；本机19c9069，实际两倍吞吐仍未通过。

2026-09-10 / EXP-284有限脚本参数复验：dot-source的被加载脚本参数会留在调用作用域；通用LoadFunctionsOnly名称可能覆盖外层同名开关，造成只加载函数后提前返回。施工前离线发现并给本阶段所有preview/offline/load开关加专属名称，五个原生姿态/预算及篡改负例重新实际输出通过；没有把无输出视为游戏已执行或重复提交。私有执行脚本仍不进入包或公共仓库；最近复验2026-09-10。

2026-09-10 / EXP-063/239/282研究负荷与声明精度复验：3503正常选择由raw-28cf42a2及root raw-0c0760d0独立核销，J66、全库存加科研点保持，未改变矿物能力或工厂。声明目标须从三个原生完整分钟整数产量以decimal计算均值再乘2，不能对ConvertFrom-Json近似double的100/3再计算并把略偏目标宣称为严格2×；原34/31/35与六个非法/不稳/超预算算术反例通过。随后的有限实验明确预热7200tick、三个未来窗、即时重新报价/持久锁定，不用过期提案或第四窗选优；本条发布时尚未取得该新实验结果。最近复验2026-09-10。

2026-09-10 / EXP-063/069/135业务范围与保存复验：普通到达容差后的实际位置仍可能让最远一个业务prepare返回OutOfReach。raw-50c2dd54以已完整观测的3m全陆地短目标正常移动一次后，原A仓位置fresh原生预检通过；不把“接近成功”当全部业务均在范围，也不继续走向建筑中心。随后正常保存38413066，root raw-b1b948d3独立核销两个终态及全物料/科研点/J65连续，accepted2不归零；此项仅证明范围和正常保存，不冒充建造或重启恢复。最近复验2026-09-10。

2026-09-10 / EXP-007/021/239十写复验：raw-a207f8c9独立核销运输3501/3502选择、保存、四次Move、两次取料和一次正常手搓的十个唯一终态，全部成功，2980实体/458配置/74详情/四网、J65和全物料/科研点保持，净2011+2，accepted10落盘后归零。只允许精确解释106的309矿点消失这一差异：原七点证据保留，309明确不存在、另六点正余量且相同矿机，其他配置全核对；不能普遍忽略resourceNodeIds变化或再用旧源能力报价。318剩约1036，后续需继续fresh观察；未证明精确采空时刻。最近复验2026-09-10。

2026-09-10 / EXP-007汇总边界复验：批次末尾报错不代表零写。raw-6de24513第一段已成功但外部摘要误报accepted3，被原始终态和fresh revision66更正为accepted4；raw-082734e9最终JSON序列化报错也不撤销六个已成功动作。有限执行器应即时输出每个accepted/actionId，交接须枚举全部原始commit/terminal再核计数，不从最后一条错误或旧摘要推断结果。新的后缀绑定精确已成功action和位置，只执行未完成步骤；没有借用旧token或重放。最近复验2026-09-10。

2026-09-10 / EXP-188/194/239模块外部预算复验：旧煤112直接续接因1694占位拒绝；2723五NEW虽原生通过，其上游2012/span3只有60煤/min且有旧炉负荷，不能把“可延伸”当“有足够可分配供给”。独立32+2+13NEW北侧煤路分段native通过，两个未来续接必须绑定建成带尾，不把自由预览直接作为省略源的施工计划。raw-44f3a091核对整批2炉/8sorter/49带/4风机/1塔/2仓和满工作余量8800J/tick；设备中心距离不单独裁定带碰撞，新sorter端点及跨网归属必须在实体存在后fresh确认。两span3输入理论新增30石墨/min，仅在新基线均值25–37.5时满足2×±10%的设计预算，不据此宣布Governor通过。明确燃料储备仓也不代表永久消耗；最近复验2026-09-10。

2026-09-10 / EXP-282续采核销：raw-be9d6767三个未来独立3600tick窗34/31/35，root raw-36e7ef10独立核对原始提案/源配置/3502负荷和会话保持；原生baseline恰好含这三窗、均值100/3、最大偏差7%，ready且无告警。10800tick观察和150tick间隙分开，不追加第四窗或计为连续36000tick。未锁180占位目标；扩产前的2×定价/持久锁须紧接有效基线，同revision/源且不超过3600tick，准备期间过期的候选不能复用。此处只验证采样结果和调用方有限续采，不是扩产或Governor通过；最近复验2026-09-10。

2026-09-10 / 同一源的新量测不等于新绑定：raw-bbc1d980初始3600tick窗返回count1，而调用方错误要求0并停止，尚未采计划内三个未来窗。GovernorMeasurementSeries按真实源绑定/周期与窗口时间管理最多三条记录，不按脚本启动清空。只修外部有限实验的断言：保留原始入口证据并排除该窗，仅继续事前声明的三个未来不重叠窗，最终核对native baseline.start/end恰好等于三个新窗的边界。不能强制重置真实历史、把入口窗补为稳定基线或用额外窗择优；沿用EXP-282，最近复验2026-09-10。本条仅调用方诊断，续采结果未验收。

2026-09-10 / 原生后续等级与需求复验：完整progression只有unlocked=false/未满级且前置已完成的条目才是后续选科候选；已经完成的3501及历史1143不能重复作为“可选研究”。3502经raw-35744aec正常选择、root raw-c11d9f91独立终态/全物料/科研点/J65核销，accepted3保持；没有自动选择矿物升级来改变本次扩产的上游能力。新负荷重新按事前固定三段60秒量测，不将选科、瞬时满输出或旧历史窗当新稳定基线。沿用EXP-063/239/282，最近复验2026-09-10。

2026-09-10 / 场地失败后的有证据重设计：raw-2bd55f24按已证明炉位对齐后，同一六对象数据全部原生Ok，关闭此前整图地面blocker；原两个失败布局仍保留。外围电塔两候选分别被2037/558占位拒绝后，停止挤入旧线，改用raw-fe7d1264的既有Foundry功率快照证明第一风机位置已有network2覆盖，再沿明确节点设计补电。不能将tower详情的powerNetworkId=null解释为无电/无连接，也不能只从几何猜旧网身份；功率helper与普通施工地形条件独立解释。四新风机/覆盖塔/两仓只有候选正例，实际相互占位、满工作负载与连接仍须fresh核验，不自动施工；最近复验2026-09-10，沿用EXP-188/194/285。

2026-09-10 / 保存与研究选择边界复验：3501运输机舱扩容仅走正常选择，raw-25cc1d00唯一终态38057182，Journal64记录后续首次选择；普通save raw-3c3a0b09成功持久化38065858。root raw-eb890670/a51b19dc及独立终态复读核对装备、非矩阵物料和背包加MechaLab科研点保持，J64 durable/healthy，accepted2不归零。沿用EXP-063/069；正常选科与保存不等于研究完成或供给/扩产验收，先前已结束研究负荷的未锁基线不复用。当前位置已在业务范围，不为保存或远处建筑中心额外移动；最近复验2026-09-10。

2026-09-10 / 整图位置不能由局部Ok批准：raw-c6a8cb56第二完整布局只有一组三对象通过，另一炉NeedGround，零提交；两布局重复地面拒绝后停止，只保留通过组位置作为重新设计的输入，绝不把它直接算已施工/已批准，也不拆分施工绕过整图地形门。沿用EXP-188/194，最近复验2026-09-10；后续需重新原生检查全部对象、满工作负载供电和外部接线。

2026-09-10 / 十写与业务范围复验：raw-eec52802两段真实Move成功，root raw-42ca8c8a在38039677完整核销九成功加一失败的十个唯一原始/fresh终态、2980实体/458非带配置/74详情/四网及全玩家物料净零、科研点/J63/healthy，accepted10经账本落盘后归零。正确的地表余量4.9481m来自浮点实际间距；不使用PowerShell混合整数重载生成的四舍五入摘要。全部原生物料和场景保持，主档仍37810618，后续不重做已经成功的接近路线。

2026-09-10 / 超范围占位Ok的反例闭合：进入施工范围后，raw-f7aca9e4的同一六对象蓝图真正nativeCheckPerformed=true，两炉NeedGround使相邻sorter连带失败；此前nativeCheckPerformed=false时的Ok不是地面或碰撞正例。材料/科技满足与地形满足分别核验，不能用前者覆盖后者；没有提交/扣料，原预备基础设施布局暂不执行。沿用EXP-188/194，最近复验2026-09-10。1704在38019605正常完成导致科研队列变空，Governor须在新的真实需求下另建并锁定执行前基线，不回填旧研究状态。

2026-09-10 / 陆地岛不能外推为更深陆地：raw-abff4dcd先正常走到干地边缘，但下一段采到的末端再次在水下0.2603m；此前约2m干地段只是局部岛。未提交后两段，root原终态复读通过，accepted8/revision55。南侧不同通道raw-5b9647b1末端仅3.9564m有效干地，也不向上舍入；未来候选必须保持完整实际采样，不能用延长坐标或中间陆地点推断新终点。此限制适用于当前不均匀岸线；沿用短跨越规则，最近复验2026-09-10。

2026-09-10 / 短跨越末端余量复验：raw-5d0284ff和raw-32069fe3均完整观测/零unknown且aggregate not_detected，但实际有中间水段；按真实arcLength/(sampleCount−1)计算末端连续干地分别0.9981m、3.9641m，未达4m，未commit。后者不能按“每点约1m”计成4m。两次同类只读候选后停止，改为先接近岸边的干地点再完整跨越到更深陆地；只分段测量和原生Move，不在中间水段停下、放宽阈值或改地形。accepted7保持；沿用EXP-246/253与现行AGENTS短跨越规则，最近复验2026-09-10。

### EXP-285 — 脱困观察不能排除带体/分拣器或合并共位对象

- 后续接近正例：raw-1ccda9d8的三段固定折线经129/753间隙和752南侧到达历史工作点，37975341/37975620/37975996全部正常；root raw-d6e2143f复核原终态、全物料/科研点/J63及healthy，accepted7/revision53。经过单层带体的正常Move可以成功，本反例不构成“凡有带都禁止通行”的规则；观察所有对象用于设计有限候选，仍不能仅凭中心距离认定碰撞或畅通。当前候选蓝图材料/科技满足，但六对象仍超范围、原生放置检查未执行；业务prepare未通过才继续有限接近。最近复验2026-09-10，冷部署新指南仍待。
- 后续局部实机正例：raw-7b4e38d9经一次4m退出及两段明确北侧waypoint全部成功；root raw-6a3b2fbf核销三终态/物料/J63，accepted4/revision47。没有立即切回原带端，保持旧线不动。随后北侧两浅水终点只读拒绝，未把部分水下但aggregate not_detected当陆地；新址接近仍未完成。44fb176完整Release零警告错误、1664项及实际源码64/1/59713字符资源通过，安装仍19c9069，冷部署待验。
- 状态：`validated`；日期/最近复验：2026-09-10；范围是当前只读近邻反例与指南离线回归，不是通路实机修复。
- 证据：raw-a1611d58的8m侧向接近在37846909明确停滞，后两段未提交；root raw-0ea89035证明原终态/物料/J63保持。此前过滤掉带和sorter后，最近大建筑中心约4m；raw-5155c670对8m内全部72对象复读，2243及2247/2251/2252距玩家仅0.4413m。共位不代表同一对象，近邻遗漏已经被反证，实际哪个碰撞体挡住仍未直接观测。
- 修正：包内MCP资源与移动工具说明要求包含带/分拣器、保留不同ID、按当前目录识别类型；roughRadius/placementRadius不是水平步行轮廓，中心距离不保证通行。短退成功后要离开整个观测障碍簇，不能立即重新穿越同一阻挡线；不拆成功产线。
- 限制：自然资源只读分页未完成且续读过期，不能宣布附近无树石。107项MCP回归含2新用例通过，安装仍19c9069，新指南冷部署和重新设计通路实测待验。关联EXP-061、IFX-092；现场/安装/类型变化后复验，accepted1仍累计。

2026-09-10 / 有界恢复与十写计数复验：root改走4m正南侧移，raw-d0f3fea4在28tick内成功，未重放任一失败目标。raw-249121f4逐一核对5craft+2失败Move+2成功Move+1save全部唯一终态、2980实体/458非belt配置/74详情及物料/科研点/J63/四网；800MJ/Walk0/healthy、revision40/save37810618，账本落盘后才将accepted10归零。两次失败有结构化position_stalled而不是未知结果，不能从计数中扣除。该正例只证明本次4m退出，后续通道仍须逐段fresh预检和停稳复读；没有路径无碰撞的普遍保证。最近复验2026-09-10，适用当前安装19c9069与本档。

2026-09-10 / 终态移动失败后的正常保存复验：两次接近Move均已明确失败、非outcome_unknown且write health healthy时，可正常保存已经完成的装备与当前停止位置，无须重开档或回退。raw-140aae69普通保存37810618、root raw-27d4885d独立核对原终态/物料/科研点/J63连续及新resume票据通过，accepted9不归零。该结论仅适用于当前已核销终态；不放行隔离状态保存、失败Move重放或第三条同类盲试，也不证明保存恢复施工门已经通过。最近复验2026-09-10，沿用本档owned/save和有界恢复边界。
...
2026-09-10 / EXP-028/239/284装备阶段复验：五个固定正常配方7×4/8×2/86×2/85×4/84×20的原配方、五个篡改输入拒绝和整批算术离线通过；Luna raw-8ff2de82实机终态由root raw-1024e388独立复读，净消耗铁80/石材8/板4/线圈14/齿轮24，装备增量准确，科研点/燃料/位置/J63无变化。accepted5不归零，revision33/save37427745，没有施工或新保存。备料与完整场地/功率/端口预检分开，不以原生正常手搓的成功替代蓝图施工验收。

2026-09-10 / EXP-007/021/028/239/283十写复验：root raw-443b6e3a于37691506逐一fresh核销3次裸模块手搓、1次1704选科、4次外部材料转移和2次组件手搓；完整2980结构/458配置/74详情与四网无改变，矩阵按背包加原生缓存点数守恒，其余净额逐项对应唯一终态，无不明正增量。J63/63、800MJ/Walk、0prebuild/healthy、revision23/save37427745；此条落盘后accepted10→0。下一设备手搓仍不是场地批准：4风机/2塔/2成品仓/4附加sorter及60额外带只是有界预算；仓储仅为明确燃料储备，不称无限消耗或配平，全部扩产施工仍在fresh基线锁定之后。

2026-09-10 / EXP-021/239/283复验：1704在1143/1703均自然完成、队列空时正常选择；raw-d3057427唯一终态37659605与root raw-f1d439f4 fresh一致，J63、全库存加科研点守恒、healthy/0prebuild。研究需求由空队列变为真实消耗，不能继承旧需求下未锁且过期的三窗候选；当前accepted4/revision15/save37427745。只核销普通科研选择和点数保全，不把4322/360000的推进写成科技完成、基线锁定或扩产通过。

### EXP-284 — 有限执行脚本必须隔离导入参数并以实际动作回执计数

- 日期/最近复验：2026-09-10；状态`validated`，仅私有PowerShell执行器，不是Plugin动作缺陷。
- 根因：dot-source helper传入`ValidateOfflineOnly`后，同名变量留在调用方作用域；脚本无参启动也误进自己的离线分支。退出码0和PASS不代表游戏行动发生。
- 修复/验证：调用方改用唯一任务前缀开关；验证原三个配方、三篡改输入负例、完整库存预算，以及helper导入后false仍false。原失败没有Bridge调用/接受；修正后raw-eebcf746三个唯一正常手搓终态由root raw-a3cdc7c0fresh核销，accepted0→3而不是把离线执行算一次。共享helper也可覆盖session等普通变量，必须先导入再赋本任务绑定。
- 限制/复验：不改模型配置、游戏API或公共工具，不把私有脚本修正包装为产品新功能。导入helper、参数/分支或入口改变后复验；接受过的脚本不能因为摘要失败而重跑。关联IFX-091及日记001。

2026-09-10 / EXP-239科研预约复验：1143正常完成37594454后，MechaLab原277200蓝/144000黄点返为背包77蓝/40黄，点数完全守恒。raw-eebcf746与root raw-a3cdc7c0证明裸模块材料按三配方准确消耗，不应拿“所有背包物品差额必须只等于手搓delta”误报；必须对6001–6006按背包×3600加缓存pointCount逐项核对，并验证whole/remainder公式，其他物料仍严格逐项delta、inc和位置/燃料不变。队列空意味着实际需求变化，不能继续使用此前已过期且未锁的基线候选，也不能清仓制造短暂生产。accepted3/revision13/save37427745/J62，无施工或Governor通过。

2026-09-09 / EXP-002/007/021/028/239/283十写复验：root raw-5a3bad9b于37583050核销两个正常保存、两个protected resume、一次正常选科、一次已知Walk移动和四次取料。全2980结构/458配置/74详情、四网及关键双向边保持，完整玩家净增铁10/铜2/石材4/电路板10与原科研点/燃料仓/J62无异常；accepted10在本条落盘后归零，revision7、主档37427745，未施工/新保存。裸模块必需材料与外部供料/功率/输出材料分开记账，不用部分备料冒充整套施工方案。原始完整审计结果已先落盘，最终控制台因整数hashtable键序列化报错，只通过现有安全序列化helper修输出并读取原件，没有重做动作或全厂读取。

2026-09-09 / 场地预览边界复验：raw-520a480e的六对象超80m且缺料，nativeCheckPerformed=false，单对象初始Ok不代表原生碰撞/地形检查已运行；材料与距离改善后必须fresh全图检查，不能靠反复查看相同缺料候选取得批准。全部planned consumers未覆盖；raw-5e65e0e进一步证明旧network2正常满负荷预约61800/65000J每tick，仅3200余量，远低于两炉及内部sorter的12600新增需求。仅为本次有限模块的规划证据，不授权扩容/施工或宣称煤理论231/min就是当前富余。新的accepted写入已使旧未锁基线候选失效；后续事前固定一次测量并在同一有限流程内重报价/锁定，不能一边长时间汇报一边把旧候选当fresh。

2026-09-09 / 蓝图数据/证据边界复验：raw-f685119b与root raw-c2082943证明2719/2739/2737、2720/2733/2727六对象往返保留两recipe17、两个完整span2/filter1109分拣器及两带；四外部边明确剥离，旧5对象的场地或施工结论不可继承。六对象材料不包括后续输入/输出/补电，原炉额定产量也不能保证复制后的实际增量。该raw只有一份玩家快照，守恒比较引用完整恢复raw-ed0e83fe，不能在摘要中编造前后两份读取；调用方投影/比较脚本失败只修离线解析并重核原件，不重复执行游戏或导出动作。零写、accepted5、无场地/施工或2×通过。关联EXP-283及日记001；后续改变选择、蓝图hash或外部接线时重新核验。

2026-09-09 / EXP-283安装与恢复复验：19c9069的1662项/完整Release/CI34368150212成功，raw-8ee8762a冷部署228文件哈希全匹配，raw-831f6820实际MCP64/1/58878指南及新Plugin认证通过。save37427705→protected resume37427745，root raw-ed0e83fe完整2980实体/58详情、四网/全部物料及科研点/J62保持；位置44.283微米浮点差在既定恢复容差内，不当作需要Move的偏移。accepted5不归零；正常研究持续不授权手改科研点或统计。此处仅关闭同批安装/同档恢复，长窗读数与稳定基线另做固定三窗实验，不能把部署当产量证明。

### EXP-283 — 低速批次产线的计数分辨率与事前固定测量周期

- 日期/最近复验：2026-09-09；状态`validated`，限DLL/离线语义、原失败实验、19c9069同批冷部署/恢复和首次固定60秒基线；未锁定扩产声明，连续十分钟目标仍待live。
- 新实机：raw-47e30a7f、root raw-d174e334独立复核三个3600tick原生窗口实际43/39/42min⁻¹，覆盖10800tick/间隙78tick，均值124/3、最大偏差7/124<10%，baseline ready且真实科研需求保持。仅一次绑定加三窗，不事后挑选样本，不拼间隙为连续覆盖，也不将原600tick失败改判。accepted5/revision1/save37427745保持、零游戏写。未锁定草案180不代表2×目标；报价/锁定必须fresh核对同一保留候选，以整数计数和decimal计算准确两倍，避免反序列化成double后的舍入差。改变采样周期、源或候选后不得套用旧平均值。
- 证据：raw-07f625de由root raw-dbd16e4e复核为三独立600tick窗口36/42/36min⁻¹，均值38、偏差10.5263%，覆盖1800tick/73间隙。报告里的36/39/38只是累计均值，必须从Supply实际速率区分；科研1143正常推进不代表产量已稳定。禁止第四次盲采或把三次均值当独立样本。
- 测量限制：600tick内单件是6/min，低速整数批次可跨越10%容差；这本身不证明现场没有物流/需求波动。当前DLL的level1每6tick聚合6个自动生产/消耗样本、600格覆盖3600tick，末端为最新完成的6tick边界；手搓只影响lifetime的total[6]/[13]，不能用其差值假称自动产线计数。
- 最小修复：仅Governor显式选择600或3600tick，默认与已有声明仍600；测量周期绑定系列、提案和v2持久声明，v1原哈希/缺省语义兼容，基线前固定，锁后不可更改。两个原生600格环逐值非负/总计一致、游标/数组/identity均fail-closed；末端按6tick对齐，绑定前历史不计，库存区间仍采用真实capture tick。三个独立稳定窗口、10%误差、准确2×及至少36000tick持续覆盖不放宽；长窗不保证真实产量稳定，不把短窗故障/目标链健康证据隐藏。
- 离线：新增50 Core/2 Contracts/5 MCP，1662项及完整Release零警告错误通过；合成每95tick一件的恒周期线在600tick产生36/36/42、原判unstable不改，新3600tick独立窗38/38/38可ready。覆盖对齐/不满窗/不支持周期、环损坏/负值/lifetime隔离、源/周期切换、锁后不可改/恢复归零、低/高/零产/缺口/健康失败、v1兼容与旧Plugin回显拒绝。真实源码MCP64工具/1资源/58878字符指南一致、额外stdout0，无游戏写。
- 限制/复验：无新增通用读取工具、无游戏写入/加速、无新类型或布局；已有失败数据保持。适配必须验证对齐、陈旧/畸形/负值、手搓隔离、源/session/周期切换、恢复声明和缺口；本机稳定基线不抵扣真实复制、取消、原buildId重启续建或2×连续36000tick。关联日记001和game-api-overseer原生窗口证据。

2026-09-09 / 1143正常需求恢复动作闭合：raw-8255aecc唯一选科37294454，root raw-6d1bb947独立终态/全物料与科研点/J62审计通过，科研已实际上传5324/300000；仅选科技、不加速/解锁/清仓/改工厂。与“完成科技”“整链恢复非零”“锁定扩产基线”严格分开。下一测量必须记录新的科研需求状态、固定次数/窗口/误差，不能将原空队列失败改判或无限挑窗；accepted3保留，普通保存及扩产验收仍待。

2026-09-09 / 复验EXP-196/281的真实需求前置：后续raw-5a7977f3的一次铁/铜/电路板备选均0产，源21缺输入而1500/10/36满输出；原17详情配置与保存后基线相同，历史成功供应不保证当前非零。raw-3a3daaba证明1142完成后队列空；只允许先正常选择已满足前置的1143，raw-e882e0cf预检为蓝/红/黄各1000未来研究消耗。不清仓、不扩容、不改成功线路，也不把取走物资造成的短暂产出当可持续基线。需求状态实际改变后才重做事前声明的测量；原失败保持，选科本身不是Governor锁定或扩产验收。当前仍accepted2，具体选科终态另记。

2026-09-09 / 复验EXP-196/282的候选边界：石墨固定七轮raw-74a6a89f及root raw-69ae9077，7×600tick共4200被测/150间隙，生产36/30/42/42/30/42/42min⁻¹；当前13对象配置和七轮供电保持、三炉输出各100，仓114在2733–2736，三个下游消费不同步。任何连续三窗都不满足既定10%，因此停止本候选重复试窗，不改报价修复为“稳定通过”，不靠理论90或把一个复制炉等同全部星球计数。2737/2727为带而非额外输出仓；必须按真实componentKind解释货物。蓝图数据正例可以保留，但进入Governor前另选可证明持续输入/输出和非零稳定产量的既有模块；这不是重新施工旧铁/铜/磁/钛/油供给的授权。零游戏写，accepted2保留；候选需重新设计而非第三次盲采。

### EXP-282 — 候选目标报价不应抹掉同一现场的真实基线

- 日期/最近复验：2026-09-09；状态`validated`，源码/离线回归及82002fc同批冷部署后的只读重报价本机正例；其他状态失效分支未新增实机声明。
- 根因：Governor适配器把`scale.PlanHash`放进被动观测绑定。只读草案从90改为基线30的两倍60，会改变预算及PlanHash，继而将三个ready源窗口清为0；没有任何游戏写、源变化或采样缺口。外部Agent因此难以先量基线再准确报价2×。
- 修复：既有`GovernorSourceBinding`增加有界观测绑定，只含原selection-series、实际静态sourceHash及1–64个去重观察item ID；同一输入观察范围内仅目标速率/计划机器数变化不清历史。源/recipe/端口/模式/网络/session及观察item范围变化照常失效，库存范围变化不能借新items伪造正增量。完整Foundry/提案hash仍变化，必须fresh检查新提案，再于任何扩产写入前锁定；已锁定目标/误差/scale不可改。
- 验证：新增13 Core/2 MCP回归；真实Core编译的90→60预算变化复现旧绑定ready→warming_up/0，新绑定保留3窗/30。范围/非法输入边界、原42/36/36仍unstable、旧声明不可重定目标/写后不可锁/缺口清零等Governor173项及相关MCP4项通过。完整locked restore、Release0警告/错误、51 Contracts+1454 Core+100 MCP共1605项通过。
- 实机：raw-baac2d6e/000e6cf9安装4+224文件哈希一致、实际MCP64/1/58216指南通过；普通save37195912→protected resume37195943，root raw-4a6186d9核对2980实体/58详情、全部配置/物料/科研点/J61/四网，accepted2保持。raw-2b02c50c中新session先0窗，再取得37204112–37204711/30min⁻¹；草案60→90保持这1个真实窗口及sourceHash，完整提案/预算hash正常改变，仍warming_up且没有声明或游戏写。
- 限制/复验：不修改原生API、600tick窗口、稳定阈值、2×/10%/36000tick或动作协议，64工具/1资源不变；它不会把raw-8b336d9f失败基线改判通过，也不增加实际产能。本机只读正例不等于三个稳定窗口、源变更/锁后改目标的实机负例或Governor验收；后两类约束仍由离线测试证明，没有为验证而改动成功产线。关联IFX-089及日记001；观察范围、源哈希、声明持久化或预算编译变化时复验。

2026-09-09 蓝图/Governor窄复验：raw-8b336d9f和root raw-c56e5ec5证明108/115/113/116/114的五对象原生导出/重解析签名及配置一致、闭合内部sorter边、五外部连接未自动复制、空仓配置未携货。预算缺2011/2012/2101/2302各一，只有传送带足够，尚无场地或施工授权。归因包含原三石墨炉；三独立600tick速率42/36/36均值38但偏差10.5263%>10%，必须保留unstable，不把恰好同tick的P=C、满缓冲或无诊断警报当稳态，也不重复试窗挑有利样本。草案60不是2×，没有锁基线或游戏写。此证据只关闭数据往返子步骤，EXP-281供给结论不外推为扩产通过。

### EXP-281 — 原生批次库存脉动与区间精度必须在观察前固定

- 日期/最近复验：2026-09-09；状态`validated`，限当前恢复后黄糖供给实验，不是一般配平阈值或Governor通过。
- 方法：先以非负事件的最大不相交窗下界/最小覆盖窗上界验证计数；密采提高可证精度，不相加重叠窗。三个独立组各≥9600tick，目标7.5/min、误差10%不变；组内无漏采，组间间隙分列。原43窗20–24的不确定界仍失败，不能回填成20。
- 库存：整次实验入口固定原矿3000/Ti2999；仅允许当前recipe65耗2原矿、recipe26耗3钛的单批脉动，每组必须自然恢复至原入口，禁止逐组降低基准或累计容差。有机/晶石/油/塑料仍按各组首末两次均值不下降、氢不增长；必须同时证明真正生产、消费和远端采矿，不能用满仓抵扣。
- 证据：私有1032个区间算法、6000个密采设计、6个批次规则离线用例；raw-0ace7ca7一次504原窗/27库存，root raw-dc8ca21d独立复核546原始详情，三组黄糖精确20/20/20件（7.460367/7.5/7.489078每分钟），氢全0，油0/0/+1、塑料+12.5/+12.5/+12。有机20、晶石3510保持，Ti2999–3000且每组补回；其余选定上游均实际非零。原始JSON精确整数计数/区间是权威，反序列化decimal/double速率的舍入差只以1e-9比较序列化一致性，不放宽验收门。
- 限制：组间13tick缺口不计入连续观察；黄糖仓均值各组−10.5，研究消费高于产出，不能称科研配平。输出满提示保留；没有改Plugin计数、工具、速度、旧raw或成功建筑。改变配方批量、库位范围、采样节奏、目标需求、供电或物料流向时须重新预声明并复验。关联EXP-196/239/278/280、IFX-088、日记001。

2026-09-09 十写及保存复验（EXP-002/007/021/028/239/278/280/281）：正常save37073510后root raw-b494cdad于37081393核销十个不同accepted终态；其中旧session八个使用原始终态及既有独立审计，当前resume/save另fresh复读，不能跨session重放历史action。完整2980实体/458非带配置/55详情、四网、关键双向链路、玩家库存及科研点数、J61保持，0prebuild/healthy/无checkpoint，只有已证明的五台风机−5。14风机及旧成功路线不重做；日志落盘后accepted10→0/revision2。原生1142解锁tick37045963/240000点，Journal仍记录首次选科而非伪造解锁条目。恢复后的实际供给门关闭，不替代蓝图取消/续建、预锁Governor2×连续十分钟或最终包门。

### EXP-280 — 滚动生产窗不是整段库存守恒账，保守下界不足不证明实际断供

- 区间方法的有限负例：私有纯计算helper以最大不相交窗权重给下界、最小覆盖权重给上界，1032个合成边界/事件子集/非法输入检查通过；它只给界，不保证精确值。raw-2bd661ae的43窗/9652tick/零覆盖缺口仍得黄糖20–24件，上界8.9515/min不足以认证≤8.25，不能反称实际超产。钛仓两点均值−0.5也不能外推持续亏供：原始八次为3000/3000/3000/3000/2999/3000/2999/3000。第一组按原声明失败且停止，accepted9不动，未来方法需重新预声明并先离线证明采样精度，不重复相同窗口。
- 调用方字段修正：当前FactoryBuffer的原生公开role是input/output，不是assembler-output。该次organic摘要0来自错误过滤，原8份详情仍明确20；必须重核既存raw，不把过滤不到当真实零库存。其他同批库存按itemId汇总不受影响。此规则不允许事后放宽原失败判据或用满仓代替实际生产；阶段已停止，没有额外游戏写入。

- 后续冷部署：79be0ae的1590 Release测试和完整构建通过；raw-3657fad3正常关闭/同批228文件安装/Steam单次启动，raw-6986c17f实际64工具/1资源/57691字符新指南、认证新Plugin和stdout通过。resume-e679a887正常主档恢复36892270→36892301，root raw-11233ef0完整2980配置、58详情、14风机/旧拓扑、四网、玩家及J61证明保持。accepted9/revision1不归零；新指南已安装可读，下面“安装仍62d7d4c”仅保留前一源码截面。EXP-002/007/021/028/239/278同时复验，不将普通保存恢复当成蓝图续建或供给门通过。

- 日期/最近复验：2026-09-09；状态`validated`，限离线数学反例和MCP指南，不是本档供给验收转绿。
- 根因：原24窗调用方将14400已采tick的P/C与14900tick首尾库存区间混用。按该次相对采样时刻构造每480tick生产/消费一件的守恒流，生产相位79、消费239，完整区间各31件，但有500tick间隙的原24窗统计29产/31耗。原生600tick速率各自正确；重叠窗相加又会重复计量。
- 修正：既有production工具说明和包内/MCP resource指南明确非可加性、独立库存区间、保守流入下界低于目标只能判证据不足。禁止用零P=C、满仓或理论产能证明配平；必须预声明新的观察方法，保留旧失败，不放宽Governor执行前基线/目标/误差/连续tick规则。NativeProductionRateCalculator及Plugin字段、状态哈希、动作、工具数量均不变。
- 验证：NativeProductionRateCalculatorTests新增两个合成回归，包含守恒31/31→采样29/31和重叠9件→错误相加12件；该类7项及MCP指南/液罐相关5项通过。此前raw-0e564632仍是原联合门失败；其中760输出20、767输出10、259原矿仓3000在初始加24样本均保持，530仅出现输出满，不据低于45的保守下界宣称矿路损坏。
- 限制/复验触发：指南源码已变更、安装仍62d7d4c；后续完整本机供给/保存恢复和Governor仍待。任何采样间隔、窗口聚合、库存覆盖或吞吐判据变化时复验。关联EXP-196/278、IFX-088、存档日记001。

2026-09-09 补电有限阶段复验（EXP-021/028/237/239/278/279）：raw-d89bbcb2仅建余五台2976–2980并save36768323；root raw-c54e392d在36774733核对全部14建造/两保存终态、2980唯一实体/458配置/44详情/原2966拓扑和完整材料科研/J60。风机合计正常−14/余0，原先成功九台未重放；主网77节点/40发电/254消费者、其余三网不变，未产生独立孤网或合并旧网。accepted6/revision37保持，不因完成六写阶段审计归零。已保存不等同恢复和持续功率通过，新的八窗另行事前声明，原氢消费者继续保留。当前185000可靠风电与221000瞬时总容量分列；以后负载或燃料变化时仍须重验，不把火电可用值固化。

2026-09-09 十写复验（EXP-021/028/239/278/279）：raw-9ebf71be revised14原生预检通过，raw-dd50d69a首九台正常施工2967–2975及save36748275，root raw-1382da95完整2975/453配置、39详情及十个唯一终态通过。旧2966全保持、风机净−9/余5、全科研点数/J60/healthy/无prebuild；四电网未合并，仅主网节点和发电各+9、消费者不变。原吸附后首连接16.3961m（原未吸附目标16.925m不能用于判断），后续仍逐台fresh场地/库存预检；不能把独立正例直接当全图可施工。落盘后accepted10→0/revision26，下一限定余五台和保存。实际主网需求已到173729，先前151466峰值及22%名义余量只属原调查，不当作新稳态保证。持续功率/氢/黄糖仍待；保存不等同已重启复验。

2026-09-09 十写复验（EXP-002/007/021/028/237/239/278）：raw-2763e967正常取料/14风机手搓，root raw-44443203/22b191b8完整2966/444配置/36详情与十个不同终态通过，恢复前后各五次分开核算；本段仅磁铁−42/风机+14，旧两建筑成本各−1有独立旧session证据。J60/healthy/无prebuild，四网瞬时满电不关闭已知功率波动。账本落盘后accepted10→0、revision7/save36513849；没有保存新材料或施工。复用教训：dot-source会污染同名参数，应使用任务专属开关；实体objectKind在entities层，不在分页wrapper上；原生燃料仓有4格但正项列表为空是合法耗尽，按已观察容器及逐项累计处理0，不通过空管道访问Sum。原完整快照/原动作不重做，补缺失只读后完成审计。初版选址违反两次同类失败停止规则的修正与后续边界见EXP-279/IFX-087。

### EXP-279 — 候选错误中的对象ID不是新失败类别

- 状态：`validated`（只读调用方停止规则）；日期/最近复验：2026-09-09。raw-c4439851首两次BUILD_LOCATION_INVALID同为exact collider overlap，仅对象1261/1902不同；按错误全文分组使相同阶段继续到14个预检，虽然零游戏写，仍违反用户的两次停止上限。
- 修正/验证：移除错误分类中的动态对象ID，并另计连续未获批准的候选数，任一到2即停。原两条不同ID回包归一类别、两失败后不得消费第三条成功fixture的离线检查通过。旧计划不再执行，原带不修改，额外得到的11个孤立正例不抵扣完整并网/材料/场地审查。
- 后续：主会话先记录最小占位blocker，再重排有限布局；每个位置仍fresh原生预检、现有旧带/已成功部分保持、正常成本与逐对象核验不变。任何有界候选循环、原生错误分类或接手计数时复验；不新增产品工具或自动布局器。关联IFX-087、存档日记001。

### EXP-278 — 完整供给摘要必须报告主网缺口，火电数量与离散计数不能补足缺失证据

- 原阈值补电后复验：raw-0e564632/root0646cb34的24×600覆盖36817769–36832668，统计14400/未计500tick。主网等已采设备无欠电，H98/101、罐0/0且24个明确0，油196/186/均值+5，塑料79/56/均值+14，黄糖31/48和10/11/10组量原子门通过。联合仍因有机28<30、钛晶石29<30且29/31、原矿保守42.400852<45失败；259仓不降、530输入+1、远端矿181，只有530输出满诊断。不能通过复测本身改写失败，也不能把下界小于阈值误当实际流量小于阈值。停止相同重采/新建，下一先核对整个库存区间、未采计数和周期相位；保留所有原数值/原始raw，后续实验须重新事前声明而非回填。accepted7/revision39/J61、save36768323，零写。

- 负载衔接已执行：raw-1809147d单次1142入队，root raw-c12530d0在36810850证明J61/原始与fresh终态/全玩家科研点数、1142实际4815/240000；2903已自然于36803484解锁。accepted7/revision39/save36768323，未制造对撞机。完成14风机及八窗局部复验后，只批准一次原数值不变的24窗联合复验；新的私有审计把所有已采主网欠电列为失败，而非仅附带报告。旧481详情离线重审为原三失败加主网欠电，不补造历史通过。统计间隙仍分列，原保守流入/离散P-C边界问题不因一次新观测自行降级。

- 下一负载衔接：raw-95c70550读到2903已正常研究167189/180000；raw-b26bc5e1/b743dc7f确认当前可正常研究1142微型粒子对撞机、前置1124满足、蓝红黄各800及fresh准入通过。为持续供给复验保留真实科研需求，同时服务0.4翘曲器材料前置，下一仅追加此一科技（prioritizeQueued=false），不制造对撞机、不更改现产线或飞行。已有2903允许自然完成而不是撤销、重新优先或直接解锁；accepted6起，最多新增1写。具体Journal种类以契约technology_first_selected为准，不凭缩写猜字段。该预检不是研究已提交或已完成。

- 补电后局部实证：raw-1b46064f预声明8×600，root raw-20165c46重核36774030–36779054/4800采样tick及225未计间隙，四网/96详情均无欠电，主网负载124331–165608、容量221000；165八个明确0，氢32产/36耗，原油67产/60耗，黄糖两组各5。支持 sampled 主网修复及原持续氢去路保持，不抵扣完整供给/保存恢复/Governor门。精炼油64/65、水10/11为原始离散差，黄糖10/28明确当前科研耗速高于本线产速；不得将仓内库存下降隐去或误称最终配平。accepted6/revision37/save36768323/J60，零写，不复用原24窗充数。

- 有负载复验：raw-f6813d6b唯一正常选择2903，root raw-d6171e77证明J60/全玩家加科研点数守恒及正常hash6182。八次预声明功率读数中主网需求125339–151466、容量151000，第一比率0.9969234，三火电中仅183实际发电；165持续明确空。保持旧85.2%欠电负例，不将零发电字段当燃料数量。下一材料计划上限14风机、可靠风电185000（比本次观测峰值约22%余量），旧氢去路不改；仅备料三写到accepted10先审计，候选地形/完整并网和实际氢收支仍另验，不把发电台数当供给闭合。accepted7/revision3/save36513849/J60。

- 状态：`validated`；日期/最近复验：2026-09-09；范围：当前母星供给诊断及私有验收解释，不修改产品统计或旧验收阈值。
- 原始复核：raw-0e167778最后两组详情有8个network1欠电读数，最低0.9088501541；旧私有摘要仅列2/4，已补全所有已采设备的欠电列表。新恢复四窗最低0.8521588966不是重启后才引入的故障。原24窗依然失败，不将新增报告或明确空罐语义回填为历史通过。
- 当前功率：raw-bd9da5e5/56bc0295为23风机+3氢火电，无太阳能；两火电当时0发电、另一台正发电，动态容量151000而非固定三台额定和。curFuelId/发电能量不能证明燃料件数，需把真实负荷、可持续燃料和可靠容量分开预算，不据单次满电或设备台数排除不足。
- 计数边界：原24×600共14400统计tick，首尾跨度14970另有570未采tick；完整库存区间更长。P/C离散差与跨区间库存下界不能被当作完整连续流量。259始末均3000、530输出25个样本都100，故保守下界41.949未达45不能单独推出矿源损坏；钛晶石30/31也不独立证明长期亏供。保留负例，先修已证实电力问题，再明确声明符合真实需求/库存边界的后续验证；不改旧阈值、补造缺口或盲目再试。
- 下一有限执行：2103已自然完成、队列为空。允许唯一正常选科2903恢复真实0.4研究需求，再只读确定有负载下的缺口；不先建风机/换燃料或重做成功部分。root预检已通过，accepted6保持，未commit。更改功率/负载、保存恢复、下一供给声明时复验。关联IFX-085、EXP-266/276/277及日记001。

2026-09-09 冷部署/恢复复验（EXP-002/007/021/028/068/070/237/239/277）：62d7d4c的1586回归及228同批安装哈希、实际MCP64工具/1资源/56883字符指南通过；正常save36513818→protected resume重存36513849后，raw-07114077完整2966/444配置/44详情及全玩家加科研预约点数、J59保持。accepted5→6，不因恢复/提前审计归零。私有最终console遇整数键JSON限制时，已落盘审计仍有效；以既有ConvertTo-EvidenceSafeValue显式规范化输出，不把展示失败当游戏失败重放。四个原始恢复窗由root raw-9c42fa2d复核，165明确0/0/0/0，新井资源下降且黄糖P4/C6，但主网19个设备样本欠电、最低0.8521588966；恢复瞬时四网满电不能替代运行窗口供电稳定。先诊断现有发电/燃料/真实科研负载，不重建已成功矿源、清有限库存或盲目再跑完整门。原24窗及Governor阈值保持；下一写前必须先解决本次直接阻塞并fresh核验。复验触发：负载/燃料/电网变化、下次独立窗口、保存恢复或十写。

### EXP-277 — 储液罐明确空值必须来自有效原生组件，不能由缺失缓冲猜出

- 2026-09-11调用端复验：先核实体类型再解释专属字段。普通仓163的tankFluidCount=null是“不适用”，不是已知罐库存未知；其600精炼油来自storage/items。私有裂解观察把两者混用已零写停止并修正，详见顶部同日记录；165空罐语义不变，未新增公开字段。
- 状态：`validated`；日期/最近复验：2026-09-09；范围：当前DLL及0.4只读适配，离线全分支、62d7d4c冷部署/同档恢复后的明确空罐本机分支；非空新字段live仍待。
- 原因：科研恢复后的24窗氢P105/C165，但165仍24次空buffers；既有CaptureTank也会为无效组件返回空列表，不能证明罐未净填充。该字段缺口直接阻断既定供给验收，不是非阻塞性扩面。
- 修正/证据：当前DLL原生TankComponent正常清空可清fluidId，SetEmpty还清身份。沿用池/cursor边界，再验证id/entityId和非负一致数量；新增现有entity DTO可空tankFluidCount，0仅代表这一个有效罐为空，null/missing仍未知。正缓冲保留原语义，不重复加总、不覆盖整个堆叠罐组、不涉及写API或改变动作/hash。
- 离线验证：新增26项（17 Core/6 Contracts/3 MCP），全1586测试、完整Release零警告错误；覆盖空罐/有液/错误身份和数量/旧DTO/序列化/现有三种哈希不变/包内资源及描述。原安装与原始旧窗口不补造该字段，仍不计通过。详见IFX-084及research。
- 本机复验：raw-5a52edf0/07114077证明正常保存恢复后165有效且tankFluidCount=0、buffers为空；raw-cfb698d6四个独立恢复窗再次为0，root raw-9c42fa2d核对，没有为历史旧DTO补字段。当前只有一个罐且为空，因此正数新字段仍只有离线证据；不能为凑验收搬氢入罐。
- 复验触发：自然出现非空罐、正常保存恢复或当前版本变化；原黄糖31、钛晶石净−1、原矿保守下界41.949的完整联合失败保持，后两项须独立解释，不用观察修复掩盖供给。最新恢复检查另发现主网欠电，accepted6保持。

### EXP-276 — 满仓零需求不是缺料，空储液缓冲也不等于已证明零库存

- 科研恢复复验：raw-217545e7只正常选科2103，root raw-825ea9f3核销唯一成功终态/J59及全部材料。蓝77/黄40正常从背包预约到mechaResearchItemBuffer，合计点数未变，不能把背包下降当额外扣料；实际hash自然到8176/108000且84/774/767/760恢复working。accepted4/revision15/save36373351，不是升级完成或联合供给通过。私有后置空数组错误只读核销且未重放（IFX-083），EXP-239在当前边界继续适用。

- 状态：`validated`；日期/最近复验：2026-09-09；范围：当前owned母星的只读供给诊断及调用方验收，不新增产品观察字段或动作。
- 证据：原24窗raw-06a792f4、独立核对raw-3bfb8df6均证明黄糖0/0；778满6000、774输出10和上游积压，科研队列为空。原双出口及两lab已有研究物料，不是输送边缺失；新井节点540在三个独立时点减少26、707有输入，只证明当前供给连接在工作。精炼油库存均值+27发生在黄糖零需求下，不外推满负荷通过。
- 决策：先恢复0.4确需的正常科研消耗，再观察现有模块。不得为满仓反复修矿源、搬到另一个有限仓或把P=C=0称作配平；原连续窗口门失败不撤销，也不改变阈值。
- 调用方修正：当前CaptureTank在空液体和部分无效组件条件下都可能省略buffers，因此无tank-fluid记录须保留unknown，不能赋0或计氢罐合格。私有核对器增加unknown失败原因；正数量/错误role/空DTO离线对照及同一原始实机窗口重审通过，氢P99>C86亦独立失败。详见IFX-082；没有重采样改判或Plugin改动。
- 复验触发：科研重新消费、任何新供给窗口、储液观测变化或保存恢复；关联EXP-068/070/272、存档日记001。accepted3/revision13/save36373351/J58保持，提前只读审计不归零。

2026-09-09 入口施工保存复验（EXP-007/021/028/068/070/185/237）：新2965塔及2966桥均有当前session原始/fresh唯一成功，root raw-f27514a4在正常save36373351之后完整复验2966工厂/444非带/40详情。2935与2750的虚拟−1槽真实为4，2966两端持有正确对象、过滤1007且主网满电；旧交叉带1827保持，不把单点空带或瞬时炼油缓冲当持续量。全玩家仅1塔/1sorter成本，J58/healthy保持，accepted3不因提前审计归零。接下来必须按原24独立窗口检验油、塑料、氢、钛和黄糖，再独立验证该保存的protected resume；新井/带/桥数量不能代替真实端到端供给。复验触发：连续生产窗口、保存恢复或下个十写。私有模板的执行前AST教训见IFX-081，不影响Plugin/MCP动作面。

2026-09-09 跨恢复十写复验（EXP-007/021/028/185/237/272）：正常save36339576后，root raw-e1db7585独立核销旧session5次原始终态、新session恢复及4次动作的原始/fresh终态、完整2964工厂/442非带配置/186详情和全玩家/J58。旧段−71带/−1塔/−1sorter与恢复保存基线完全一致的库存回退分别保留；当前只计新段−71带，不把两个session的成本或生产时间拼接。恢复action的completedAtGameTick未提供，以成功终态及实际加载/重存36231245证明，不补造tick。无prebuild/unknown/隔离、不明增量，四网当前满电，余41带/1塔/1sorter。账本落盘后accepted10→0，revision8；新保存不是protected resume或持续黄糖/Governor证明。复验触发：首桥施工、下次恢复/十写或库存/拓扑变化。

2026-09-09 最近复验（EXP-007/021/028/185/237/269）：新session raw-6d8d1c06以单一原生action补齐48带，root raw-0092f951的167详情/全玩家证据证明唯一源2916旋转变化、其余119旧配置及双向端点保持；末2935/邻2936再次从本次完整路径确定，不能因ID恰与旧session相同就沿用旧token。accepted9保留，下一步save及分段十写审计；自由末端原油不是持续供给。准备后续私有脚本时，JavaScript字符串替换中的`$'`把PowerShell正则的尾锚误解为替换模板；AST在执行前拒绝，改用字面量回调后通过，零游戏调用。脚本生成后的AST/哈希复核继续必需，不因原模板曾通过而省略。复验触发：后续保存/桥接、模板替换或十写；0.4原验收门不变。

2026-09-09 恢复后补建复验（EXP-007/021/028/185/237/272）：在完整证明23对象未被主档保存之后，raw-df6de757用新session/fresh计划正常补建；root raw-55093254逐点核销119详情、旧96配置及全玩家仅23带成本/J58/healthy。新末2916与原session末2915不同，即使许多ID被重新分配也不能跨恢复视为同一身份；后续必须从本次原始target IDs和路径唯一匹配末端。只允许已批准源2893的原生旋转变化，未重建保留的井网/南干线。accepted7不提前归零，主档36231245尚不含新23带；下一十写按恢复边界分段核算，旧已回档成本不能加到当前库存。复验触发：后段接线、保存/恢复或十写；局部原油到货不替代持续黄糖及Governor验收。

2026-09-09 恢复复验（EXP-007/021/028/237/272）：resume-2dafe7aa只载入健康票据的exact primary36231214并重存36231245。root raw-3414ca1c全2893实体及普通配置、23详情、全玩家count/inc、四网/J58证明保存基线完整恢复；后续73对象确实不在当前世界，库存精确回到112带/1塔/1sorter，不是不明正增量。允许继续的依据是该完整恢复证据和新的现场预检，而非重放旧成功action；旧ID也不得跨session当作同一实体身份。原五次accepted历史保留、恢复另计一次至6，不因恢复/提前审计归零；下次十写审计需分开保存前后两个session的物料边界，不能把已被恢复的旧消耗再次加到当前背包。复验触发：缺失部分补建、下一次保存/恢复、十写或实体/物料不符；原连续供给/Governor门不变。

2026-09-09 最近复验（EXP-002/007/021/028/068/070/185/237）：原始raw-9e7fee52/a1559e11和root raw-2e062bad证明正常电塔2965及首桥2966已在暂停前完成，虚拟−1槽再次实际解析为两端slot4，材料各−1、旧配置和J58保持。用户暂停后无新写入；继续时DSP已关闭，正常保存水位仍只证明36231214。因此明确分开“原动作终态成功”和“恢复后的世界仍保留施工”：旧成功收据不授权重放，未保存部分也不能直接标记已丢失，必须在exact-primary保护恢复后逐对象核验。安装同批228哈希及恢复票据副本一致；Steam只启动一次，首次主线程读取超时后同进程fresh读到菜单ready，无第二次启动或其他存档访问。accepted5保留，恢复会重新计数但不提前归零。复验触发：实际恢复、实体/材料差量、Journal水位或下一十写；连续生产和Governor门不抵扣。

2026-09-09 最近复验（EXP-007/021/028/185/237/275）：raw-b87d381d单一action完成最后48NEW，root raw-ee0f8550按原始完整48个target IDs逐点核销167对象和119旧配置；实际末端2935/末邻2936，不能依赖最大ID或手工摘要选择后续端点。源2915原生精确旋转变化与其余旧边/配置保持分别核验，全玩家仅48带成本/J58/pre0/四网/healthy通过。18066游戏tick正常施工不延长游戏超时、不重放；原定216条新带齐备仍不等于端到端供给，首桥与供电尚缺。accepted3/revision93、主档36231214未包含本窗新带，连续黄糖/保存恢复/蓝图及Governor门保留。复验触发：实际桥接双端、功率、物料和持续窗口，或十写/保存恢复。

2026-09-09 最近复验（EXP-007/021/028/185/237/275）：raw-aea60d53以一个有界原生action完成23NEW西转弯带；root raw-4b8efdaa独立逐点/双边核销119对象、96旧配置、全玩家仅23带成本、J58及healthy。实际拓扑是2893→2916→…→2915，末邻2913；与此前84带再次证明实体ID不是路径序号，后续source-cover必须绑定实际末端。只按成功原生完成证明核销源2893朝向变化，其他配置/旧边保持。末端两件原油只证明局部输送，不计炼油供给或持续黄糖通过；accepted1保留，主档36231214尚未包含本段。复验触发：下一段实际端点、路径/预算变化、十写或保存恢复；正常批量施工仍逐对象核验，不延长游戏超时或重放成功部分。

2026-09-09 十写复验（EXP-007/021/028/185/237/274/275）：新原油源与南干线正常save36231214后，root raw-9808905c对十个不同accepted的原始/fresh成功终态、2893完整工厂/442非带配置/110详情、全玩家成本、四网、J58和owned/healthy完成审计。七个建造action逐对象对应4风机/1井/91带，没有以聚合计数掩盖对象或物料；两次Move保持正常订单/落地，已有原生端口出料目标及尾带旋转变化均按精确批准后态核销。全窗净91带/4风机/1井，燃料仅正常减少43.327909MJ，pre0/3idle/无unknown/隔离。账本落盘后accepted10→0、revision87，主档包含本阶段施工；尚无本阶段protected resume或端到端持续黄糖证明。复验触发：下次十写、恢复/安装变化或剩余线路接通；原连续窗口和Governor基线/2×门不缩减。

2026-09-09 最近复验（EXP-007/021/028/185/237/275）：raw-eea19923以一条有界正常action完成84NEW南干带，source2809不重建/重复扣料；root raw-3a4dd9ee逐点核销96对象、原生精确旋转证明及旧边/全玩家仅84带成本/J58。首实体2852、末2893，中段ID往返，不得按数值排序推断拓扑。22314实际游戏tick的施工使用同action持续观察，未把调用方观察预算当作重放或修改游戏超时的理由；现有油井出口原油已到末端，但只是未接入炼油的局部到货。actual2893→23NEW的预检通过仍非施工；accepted9保留，下一步save及十写审计，主档36081834尚不含本次新源/干线。复验触发：实际尾部/路径/材料/电网变化、十写或保存恢复；有限批量仍逐对象审计，不扩展白名单或自主循环。

### EXP-275 — 资源设备接上原生出口后，出料目标也是批准的后态

- 状态：`validated`；日期/最近复验：2026-09-09；范围：当前油井native_device_port施工的调用方验收，不扩展任何写原语。
- 结论/证据：raw-a5bc5a78完成七个NEW带，root raw-eb49aad9复读2802:0→2803:1及完整2803→2809，末带已有1007。GameStateReader从miner.insertTarget读取insertTargetObjectId，因此原空值→本次首带2803是正常连接的伴随状态，不能机械要求所有旧字段都不变。
- 守卫/验证：只接受原目标空、原边空、新目标精确等于批准首带且双向slot存在；余下身份/位置/朝向/资源/电力/配置和全玩家仅7带成本不变。一正四负原始回包对照拒绝错误目标/缺边/错slot/矿点变更，ActionClient21及源带策略60通过。保留原成功action，修私有验收逻辑后只读核销，没有第二次施工。
- 限制/复验触发：native_device_port、设备组件、连接或游戏版本变化时重新确认字段语义；不能类推为任意sorter目标或其他配置均可忽略。此次仅井口输送，新保存/恢复和持续黄糖供给另验。关联IFX-080、EXP-007/028/237、存档日记001。

2026-09-09 十写复验（EXP-007/021/028/068/070/185/237）：raw-c52a6456正常save36081834之后，root raw-b986deb0独立核销十个原始唯一accepted/fresh成功终态、同tick转移/施工材料守恒、全2797对象/437非带配置/70详情和两只新过滤sorter双向槽。全窗仅新增54带及两sorter，先手搓一只故玩家净2001−54/2011−1；所有其他库存/inc/原配置边、J58及owned/healthy保持，主网消费者仅+2，三网当前满电。私有审计可选terminalPrefix以ContainsKey显式检测后重跑通过；41带超出调用方观察预算的终态来自同action后续收据，不补造原始终态或重放。现于账本落盘后accepted10→0，revision68；保存不是protected resume或持续输送证据，新井/剩余路线与原黄糖验收门不变。复验触发为下一十写、部署/恢复或拓扑/库存不符。

2026-09-09 最近复验（EXP-007/028/068/070/185/237）：第二跨线桥原始raw-963850f6及root raw-8769a19a证明2797按正常无人机施工，以1007过滤连接2791:4→2747:4，network1/ratio1；prepare的虚拟slot−1没有被当成实际槽，完工两端真实slot4均反向持有本次sorter。68个既有对象剔除唯一新边后配置与旧边全保持，材料仅2011−1，J58/全玩家/healthy核销，accepted9不因提前检查归零。该桥跨过旧2522却不修改其旧边，补充显式有界跨线正例；仅证明施工及瞬时供电，空载不能证明持续原油输送。主档35281231尚不含新施工，下一步先正常保存再十写审计；原始未知结果不重放、source-cover与destination合流边界不变。复验触发：两端/过滤/带路或电网变化、保存恢复及下一十写边界。

### EXP-274 — 油井瞬时需求不能反证原生工作功耗，备电必须区分工作与待机

- 状态：`validated`（当前目录语义、原生四风机建网及井口产油；非全线持续供给）；日期/最近复验：2026-09-09。
- 实机补充：raw-b99a2823按四风机后单井的五个有限fresh步骤建成2798–2802；root raw-1b7d2c87独立证明新network3为4节点/4发电机/1消费者、20000J/tick/ratio1，井工作预算14000，原三个电网及2797旧对象配置保持。材料仅4风机/1井，540矿量自然70733→70715、井口50原油，J58/healthy；不能把有限井内库存当持续外供。accepted6/revision80、主档仍36081834，尚未保存恢复。raw-ee9c0b58实际source-bound七格出口前两点与旧free-path不同，重新完整原生预检并绑定native_device_port后才作为施工依据，预算仍7；复验了EXP-028的端口推算不替代实测边界。
- 范围/结论：DSP0.10.34.28529的原油萃取站2307目录工作14000、待机400J/tick，普通分拣器2011为300/150，当前母星风机2203容量5000J/tick。实体powerDemandPerTick读取当前consumer.requiredEnergy，是瞬时请求；400样本不能否定工作14000，也不能拿满供电比证明峰值容量充裕。
- 证据：raw-cfdad8e0原生get_build_catalog及当前GameStateReader目录字段映射为prefabDesc.workEnergyPerTick/idleEnergyPerTick；实体读回映射为consumer.requiredEnergy。旧EXP-010/017保留的400/550瞬时读数不是测量错误，错误在将其外推为工作上限，现由本条替代该解释。
- 预算/限制：三风机15000覆盖单井满工作只余1000，再加一普通sorter只余700；本次四风机20000的草案余6000/5700。实际还要验证原生落位、覆盖、连网和全部消费者，不能只数风机或宣称新井已投产；目录工作值也不等于每tick必然消耗该值。
- 离线复核：当前源码重编译后Core1422项通过，其中电力相关51项独立复跑通过；此处仅修正证据解释，没有Plugin实现或安装批次变化。
- 复验触发：原型/游戏版本、矿物升级、电网/消费者或发电环境变化；关联EXP-010/017/025/237、game-api-foundry及存档日记001。
...
- 原油线路接近十写复验（2026-09-09）：raw-d1285fa8于35794538完整核销九个成功Move及一个已知position_stalled/180tick终态，原始/fresh收据一致且无重放。2741实体/435配置/13相关详情、三网当前满电/pre0、全部玩家物品/count/inc/科研/J58保持，正常燃料热量−54.917845MJ；最后Walk0/400MJ，revision51、主档35281231未新保存。ActionClient21及MCP岸线指南4项通过；accepted10仅在本条落盘后归零。岸线干燥前段和当前复验的历史Walk终点分别完成一次新接近，不能据此改判早先失败、取消完整供给门或宣称新井已施工。
- 完整油源备料复验：raw-c5efde54于35292110核销两Move/四transfer/三handcraft/一save的十唯一原始和fresh成功终态，2741实体/435配置/14详情、三网满电/pre0/J58保持；普通save35281231/revision32和新protected resume成立。全玩家净磁铁−28/铜−14/电路−6、带+252/风机+4/油井+1，原石/石材/钢/铁按转移加递归消耗相抵，其他物品/inc及科研缓冲保持；正常燃料热量−38.555998MJ，最后Move位置保持Walk0/400MJ。已备1油井/4风机/257带/2sorter/1电塔，占37/40格；新井未建、没有重启或完整供给证明。ActionClient21项与库存说明2项回归通过，accepted10只在本条落盘后归零。
- 原油备料十写复验：raw-a63a057f于35131420完成两科研/四Move/两transfer/两save的十唯一原始与fresh终态核销，完整2741实体/435配置/14详情和三网满电/pre0保持；35122406正常保存、新protected resume、J58/58及旧J57完整前缀成立。全玩家净1301+6/1104+14，蓝77/黄40由科研缓冲自然取回背包，按EXP-239合计原生点数而不是只比背包；全部其他物品/inc/手持/手搓和最后实际落点保持，正常燃料热量−28.160894MJ。53项科研/地表/看门狗回归通过，accepted10仅在本条落盘后归零。新原油源尚未建，原供给失败不撤销；无冷部署或重启，不能冒充这些验收。
- 恢复十写复验：save34784513、正常关闭与唯一protected resume重存34784544后，raw-d7b8a717在34803168核对十原始终态、全2741实体/435配置/16详情双端、材料净2012−2/2011+2、玩家加科研缓冲、燃料/能源、J57/57和满电/pre0/healthy。跨恢复坐标差0.000044118m须记录，不以十进制JSON字符串完全一致误判损坏；恢复后的新位置仍逐读保持，Plugin哈希/准入不放宽。原远端升级保留保存前精确证明，母星只读不声称已fresh核对远端实体。228安装文件哈希及raw-1f36eb44实际64工具/1资源/53671字符握手通过；accepted10仅在账本落盘后归零。已知原油亏供保持，局部恢复不抵扣完整供给/Governor。
- 移动窗口复验：raw-64fff6a9九个约30m Move均先有完整31–32点地表采样，再以唯一成功终态/Walk/低速/400MJ核销；结合此前落地save，raw-34c5e99b在34596687完成全179实体/28配置/13详情/十fresh终态与全玩家/J56/电力/健康审计。已知建筑净空不替代原生地表或动态碰撞检查。首次只读调用端的dot-source参数默认值覆盖了调用者revision，保存到任务专用变量后重验成功；没有游戏状态错误或动作重放。地表/停滞41项回归通过，accepted10仅在本条落盘后归零；剩余两Move和升级仍待。
- 远端复验：单次检查点飞行成功后，raw-dcfd1b2e在34506427将102完整179实体/全部28非带普通配置逐一与既有远端全量基线比较，再fresh核销13深详情/双向边、十原始及fresh终态、全玩家材料差量与正常燃料消耗、单网/J56/pre0/healthy。没有重复逐设备请求普通配置，也未省略全量；accepted10归零仅在账本更新后。源星104的备料/加注/保存阶段无工厂写入，不把不可远端详情读取的源星旧快照写成此刻fresh检查。
- 状态：`validated`；日期/最近复验：2026-09-09；范围：当前安装批次、本档2741实体的十写审计；没有新增工具或放宽审计范围。
- 依据：`GameStateReader.CaptureFactoryEntities`和详情入口都调用`TryCaptureFactoryEntity`，包含连接、配方、过滤、矿点、仓储/物流配置与电网；详情额外提供`SorterEndpoints/BeltCargo`。原始2739实体分页中的433个非带配置逐一与同期详情一致；配方、过滤、位置、边、仓限制和物流输出选择六个突变反例均被拒绝。
- 实机：raw-e3c94ba3在34334222取得同snapshot/tick的28页、2741唯一实体和全部435非带配置，逐项比较已声明的两新附件及两过滤变更，其余保持；raw-c201dca2在同session/revision2冻结边界继续，fresh核对九个相关详情及双向连接、完整玩家净差、三网、J56/56、零prebuild与healthy，于34362361完成。没有把跨调用动态库存当成同tick，也没有重放十个已接受动作。
- 调用端修正：空的合法物品匹配应为0，但缺inventory/count/inc、负数、非整数或错误字段必须拒绝；四正例、八负例通过。首次只读审计在空匹配的`Sum`处退出，复用已落盘且身份/完整性复验通过的分页；第二次全部检查及证明已落盘，只在显示整数键字典时失败，直接读取原证明核销并补显示回归，不重做游戏写入。
- 边界/复验触发：每个十写窗口仍取完整工厂快照并检查全部普通配置，不改成抽样；端口候选、带货物、动态生产和执行前hash仍须按相关动作fresh读取。session/revision、预期拓扑、捕获实现或DLL变化时重新验证，不拿旧分页充当新施工预检。
- 本轮核销：十个唯一accepted均有原始成功终态，净消耗2012×2、正常仓到背包1109×400，其他玩家物品/手持/科研buffer/手搓不变；燃料未加注，Walk0/400MJ。2268恢复bans30但此前12133>6000的排残时限失败保留；无未知结果、隔离或新checkpoint。账本更新后accepted10归零，主档仍34182305，石墨转移尚未另存。关联EXP-007/255/269/271及存档日记001。

### EXP-271 — 跨重启续建必须保留执行前声明，而不是继承连续观察信用

- 状态：`validated`（声明恢复语义仅源码/离线）；日期/最近复验：2026-09-09；范围：0.4蓝图取消/重启续建与Governor两倍验收的组合边界；修复已冷部署，尚未实机锁定/恢复声明。
- 部署复验：c32a5e0同批228文件和1556测试通过，raw-1fc207d4实际安装MCP认证新Plugin、64 tools/1 resource/53671字符指南一致。正常save34182274→关闭→protected resume重存34182305，raw-d3a5b0af/705122df证明既有塑料过滤、闭仓和局部生产保持，J56/56连续；这只是安装和普通同档恢复，不是Governor声明持久性正例。不得把空声明存储下的成功恢复冒充已经验证原基线恢复。
...
- 证据：raw-f23c705f的四个唯一终态至33549946成功，全部玩家快照直至33550757保持原位；raw-1e2865a8在33572099读到后来位置改变，revision仍164。完整工厂和玩家物品保持，不据此推断操作者或移动原因。
- 处理：保留动作内精确位置/成本检查；在后续审计显式区分该时间边界，绑定raw-d07d103a于33576965的新稳定Walk位置，重新核验完整2737结构/31配置、全部玩家物品、三网/J56及原始/fresh终态。raw-c532eadd于33595350通过；不删除旧失败证据、不重放原动作、不复用旧token，accepted8保持。
- 限制/复验触发：这不是任意变化豁免。无法证明原动作结果、物品出现不明差量、原实体/连接变化、ownership改变或机甲继续移动时，仍须停止并分析。当前计划只能使用最新player及双端hash重新prepare；未知原因不写成已验证人工操作。
- 关联：IFX-074、EXP-269、存档日记001；新位置、任一写入或恢复时重新核验。
- 新位置续作复验：raw-62d78eaa两个A出口附件分别在33608712/33610170完成，动作内位置保持raw-d07d103a显式新锚点；raw-779781e2于33642112完成十动作/2739结构/433配置及全玩家净差审计。前八动作仍逐快照核对旧锚点，后两动作与审计核对新锚点，原实体/连接/物品、J56及healthy均保持；没有把未知位置变化原因升级为已知，也没有放宽任意后续位置变化的守卫。

### EXP-269 — 供给修复后以实际需求和库存重新归因，保留原失败门槛

- 原油科技方案的明确边界：raw-a752744d/raw-4c236c00两唯一正常动作选择并优先3601，原生排序即时证明3403研究进度/库存保全；raw-c4e64d24单次有限只读观察到34885251自然解锁。原油理论42.1968→46.3848/min、炼油理论仍60，升级后仍低于已观察用油48.25；不能假设科技名称给出的收益、把理论值当实产，或因此盲跑第三次原整链窗口。root raw-297e1665在34901299复核原始/fresh终态和J58，16项科研策略回归通过。accepted2/revision5/主档34784544，新科技未另存；保留正常升级，下一步只为原油准备新源及完整成本/供电/输送方案。未开发540仍仅候选，不给施工或长期供给信用。
- 恢复后局部正例：raw-136112bd四个事前声明600tick窗均在34784544恢复点之后，root raw-a5102819复算102源矿37、104耗矿40/钛块20/黄糖5，各窗非零；证明持续路径没有因恢复消失，不能称远端实体fresh复读或完整油库存门通过。同期油P30/C32，已知亏供保持；accepted0/revision1/J57，未写入或拼接跨恢复窗口。
- 真实科研需求下的再归因：raw-0feda37c的24独立窗由root raw-b17c3e85保持原阈值核对；黄糖30/三组10、11、9，塑料77/78且库存均值+0.125、钛到货下界52.8041/min、源矿240、水/有机/钛晶石及氢持续消耗均通过，唯一失败为油库存均值−17.625（原门≥−4）。油P172/C193不是满氢：氢罐5346→5206。raw-c2c01c00的唯一原油源129理论43.4184/min，双炼厂理论60、141缺原油且输出空、满电；下一方案只针对原油供应，不能靠再搬油/清罐掩盖长期负差，也不能因全链未过就重做已通过钛/塑料。accepted8/revision51/J57，原生统计5项通过；未计蓝图/Governor或最终包。
- 正常需求恢复：raw-0b22c7c3于34737719唯一选择运输船引擎3403（各800蓝/红/黄的普通科研预算），J57记录首次升级选择；root raw-9e418cfe核销终态及真实hash17112。raw-ece8b3ba观察778黄糖5973、单600tick P1/C3且无finding，证明产消恢复，不等于原24窗整链门通过。accepted8/revision51、主档34698572保持；科研只选择，不直接解锁。调用端漏算机甲研究缓冲的误报按EXP-239修正并核销原动作，没有再次提交。
- 返航后根因确认：raw-8b59da5d在34686595以独立checkpoint34683198单次正常返104，raw-49a0533b完整2741实体/435配置、16详情及玩家/J56/健康核销，无重建或人工携矿；raw-fd379d1d save34698572，raw-c0b55eef证明返航ticket真实retired且新protected resume可用。778黄糖6000/774输出10满、84研究原料非零而队列空，停产直接边界为没有当前科研消耗，不是旧钛供料复发。3403运输船引擎仅fresh预检允许，各800三色矩阵将由正常科研消费，不搬空仓或新增有限仓作修复。accepted7/revision49保持，选择科研、完整黄糖窗口和重启仍待。
- 钛源持续性：raw-90482c3a单一预声明24个独立600tick窗，raw-320ad850逐窗复算34643300–34658253：102矿P240/60min、三组各80、源仓3000→3000（最低2999），原180/每组60/净差≥−6门通过，双升级/原边/两星抽样电力保持。104原矿C240、钛块1106 P120/C99；物品/星球不能混写。44钛200/订单−200→53/0→116说明原路径继续取货补库，不能仅凭订单或远端全物品库存证明母星精确到料。黄糖P0、774输出缓冲满，后续fresh科研为空队列，不能再归咎旧钛源或借供给修复继续盲建。accepted5/revision45/save34641350保持，原整链和重启门未通过；原生统计5项回归通过，详见日记001。
- 新升级保存：raw-c136fa52唯一正常save34641350；root raw-f771bc50在34644688核对唯一终态、两设备配置/满电、全玩家守恒/原位、J56/56和healthy/new protected resume，accepted5/revision45保持。ActionClient21项通过，重启仍待；随后24窗只作源端持续性诊断，不能用一个60/min正例、理论速度或仓库库存替代整链送达。
- 钛源升级复验：主会话raw-7750d291先核对当前16/123预检、两只2012整套预算及17100/55000J/tick全工作电力边界；Luna raw-f86c1aec于34620849/34620969逐一正常升级，两基础设备返还且两高速设备扣除，周期时长400000/600000→200000/300000，原生周期比例/货物/配置/双端保持，结果ID据实际读回。root raw-569c327a于34629284完成包含最后两Move的四终态、全179实体/28配置/13详情及全玩家/J56/healthy审计；44升级回归通过。accepted4/revision44不归零，主档34525433尚不含升级；理论90/60矿每分钟不是持续出矿或母星到料证据，保存恢复和窗口另验。
- 落地持久性复验：raw-7f5cc014唯一正常save34525433；root raw-2f12bd60在34543302核对原终态/新protected resume及受保护flight ticket的真实`retired`/retiredAtGameTick34525433，原检查点已退役。机甲原位自然回充至400MJ、accepted1/revision18保持，没有为靠近矿线新增未批准动作；原生目标/路径预检和两升级仍未执行。
- 抵达现场复核（2026-09-09）：raw-3e45d04a于34491396单次飞抵102，raw-dcfd1b2e完整审计34506427。矿机1两矿点/输出40、仓15钛3000、16/123原生stt400000/600000且满电，旧45/30理想限速候选现已获fresh身份/拓扑证明；44钛151/200、硅500/500、独立硅口保持。尚未升级，不把站库存或理论容量当实际送达；落地低核心通过格内普通燃料自然回充，不低能量移动。主档34466163，落地正常保存/成功checkpoint retire及两处原地升级另验。
- 钛源备件闭环复验（2026-09-09）：raw-56aee77e三取料/三固定配方成功，root raw-e1007fbb在34400216核对六个原始/fresh终态、两源仓静态端点/配置及全玩家净差仅2012+2，石墨400/燃料格7保持，J56/56/healthy、accepted6/revision11。配方预算铁4/铜1/电机1全部正常取得并消耗，不预支升级返还；无移动/升级/保存，主档仍34182305。下一步原生refuel须按已有格余量逐栈prepare；`CalculateAvailableFlightEnergy`只累计核心、反应堆和已加注燃料，不计背包备用，不能以持有400石墨直接推导起飞预算通过。ActionClient21项通过；实际加注和远端限速修复仍待。
- 2026-09-09塑料恢复后的钛边界：raw-1d182e06同tick34255077证明远端102实际钛矿30/min、理论60/min/满电，母星1657钛0但订单200/工作船1，259与530缺原矿，已升级532/1784原配置/端点/满电保持，918/916缺钛块。不能把有运单的零库存直接判成死锁，也不能用短窗黄糖非零掩盖远端30<原需求45的供给差。站内1733硅不是钛库存；聚合storedItemCount不代表所需物料数量。历史raw-b23b7479的102:16/123原生stt400000/600000按既有Foundry周期得45/30矿每分钟，只可作为赴现场fresh核验的候选，不能凭旧详情远程升级或宣称当前唯一根因。保持独立硅口及正确母星线路；后续仅正常备料/已知同星系检查点飞行/本地预检，accepted9不重置。
- 状态：`observed`；日期：2026-09-08；最近复验：2026-09-09；范围：本档两处钛入口升级后的24个独立600tick诊断窗及后续有限石墨供给修复，不是Governor连续窗口。
...
- 两炉施工/配方后验：raw-716fd4cd按已核销前缀只执行A配方、B建造、B配方，终态33383469/33384217/33384916；原2719保留，2720唯一新增。root raw-8c83d822于33391955完整核销2720结构、四个原始/fresh终态、旧配置、全玩家两炉成本与J56/pre0/healthy；两炉recipe17、network2消费者29/容量65000，accepted7/revision144保持。production完整hash与配方说明预算边界已在真实配置中通过，12项针对性检查通过。两炉建成后的三条带raw-476f82a5完整原生预检再次通过，14项路径边界检查通过；这只是下一有限步骤的场地证据，不替代之后fresh prepare。新炉尚无接料，不宣称90/min实产或整链恢复；主档32930076，三带第10写审计、七sorter及持续产率/恢复仍待。

- 三带/十写后验：raw-343cb378完成11NEW，三路径终态33434163/33435661/33438390；raw-c279236d于33449021完整核销2731结构/427配置和十个原始/fresh终态，13新增身份/位置/自由端、两配方零扣料、115货物保持、全玩家精确净差及三网/J56/pre0/healthy均通过。旧边变更拒绝检查通过；本条落盘后accepted10→0、revision150，主档32930076保持。新带施工完成，不等于附件或持续生产通过。
- 七连接估算的反证边界：raw-16e454f2实际通过111→2721和2723→2719；2719→114设备直连被BUILD_CONNECTION_INVALID拒绝，末个候选TooSkew，单belt偏移回退不支持设备↔设备。原0.09°模板估算不足以证明双方原生姿态/放置条件，撤销对该出口可直连的候选假设，不修改成功的两炉/三路径。零附件commit，不计accepted写入；主会话重新设计出口，不能继续重试同一对实体或以理论90/min结案。当前唯一施工设计缺口在A炉出口，未预检的B侧连接仍须独立原生确认。

- B侧施工后验：raw-b8ed6cfd按下游优先、煤源最后的四个已预检连接正常建成2732–2735，全部精确过滤/双端/满电，四终态33493618/33495020/33496404/33497824；raw-3aa38ff3于33508375核销完整2735结构/39相关配置、原始及fresh终态、全玩家基础−2/高速−2、旧边和三网/J56/pre0/healthy保持。network2消费者33、满工作预算60000<65000，22项固定计划/变更拒绝与原始全结构重建通过。accepted4/revision158保留，主档32930076不变；接通不是实际非零/持续产率证明，A出口及整链窗口/保存恢复继续单独核验。

- A出口修订边界：保留两炉/全部既有线路，撤销尚未施工的2719→114直连，改为两NEW带中继至既有塑料入口553，仍保留114→555旧供给。raw-b3c9ebba完整原生自由路径通过；实际端点模板两附件约2.69/2.52m、1.01°/0.72°只用于候选选择，实建后仍须原生prepare。raw-1c0a66a4证明额外铁1/电路1的两个正常取料计划及recipe85，并用原生吸附后的四端点供电探针核对network2当前满工作60000；剩余基础2/高速2加1800，最终61800<65000，现有基础1需正常再制作1，带7足够新增2。七项配方/变更拒绝/路径/预算检查通过；只批准有限四动作备料+自由带到accepted8，不把位置、供电或理论需求预算外推成附件/持续生产通过。保存32930076及既有结构不变。

- A中继实建/端点复验：raw-f23c705f四动作完成，两NEW有向2737→2736，玩家净基础+1/带−2；raw-c532eadd于33595350核销完整2737结构/31配置、全部唯一终态/物品及原线路、三网/J56/pre0/healthy，accepted8/revision164保持。实际2736→553及2719→2737在raw-73767141和新位置raw-d07d103a均原生通过，双1109/槽位/位姿/零offset一致，12项固定计划检查通过；尚未建附件或启用A煤源。动作后外部位置变化单独按EXP-270处理，没有放宽动作内守恒。B已有实际输出石墨，但715输出满100证明仍需区分产出正例和持续吞吐，不能用B缓冲增长宣布30/min持续供给。

- A出口/十写后验：raw-62d78eaa正常完成2738（2736→553）及2739（2719→2737），双基础/1109过滤/原生槽位互反/满供电通过，终态33608712/33610170。raw-779781e2在33642112完整核销本窗十个唯一原始/fresh终态、2739结构/433配置、八新增对象、全玩家基础−3/高速−2/普通带−2且铁/电路净0，原边/其他物品/三网/J56/pre0/healthy保持；network2仅新增六消费者，现35消费者/65000容量。12项A固定计划检查、十动作成本/完整结构离线重建和ActionClient21项通过，本条落盘后accepted10→0、revision168。尚无A煤输入或A实际产出证明，不以空载出口通路代替持续生产；现有高速2只够固定两煤连接，最终满工作61800预算不是实测吞吐。主档32930076不变，正常保存/独立产量窗/恢复及后续版本门仍待。

- A煤输入后验：raw-13e92f30在当前现场两条原生预检通过后，raw-27bc9847正常建2740（2723→2719）和2741（111→2721），双高速/1006过滤/双端/满电，终态33669230/33670539。raw-96c925db于33676582核销完整2741结构/54配置及两原始/fresh终态、全玩家仅高速−2、旧边与三网/J56/pre0/healthy，accepted2/revision172不归零。A输出24石墨、两出口各持1、553带货2以及552实际油4/石墨2并工作构成首次通料正例；B输出100满缓冲仍不算持续30/min。原12项固定计划检查和完整结构离线重建通过；混合历史raw的无计划错误响应须先识别，不能当成功计划读取或签发信用。正常保存、多独立窗口和恢复继续单独核验，不能把设计90/min或满工作61800<65000当作实际产率。

- 石墨修复保存后验：raw-db734201唯一454faf2a正常保存33689680，revision173/accepted3并签发新protected-resume能力。raw-8dd09d6e于33699547核销完整2741结构、76组原始前后及fresh配置、保存原始/fresh终态、全玩家零物品差和J56/pre0/healthy；六项保存边界与配置/完整结构离线重建通过。没有实际退出/恢复，也没有新的持续产率窗口，accepted3保持。历史主档32930076是此前截面，本次主档已经包含七风机/115升级/两炉/13新带及全部八新附件；不复用旧保存票据或旧token。

- 修复后多窗反证：raw-aae88cbf的24×600tick由raw-5f59ddfb逐原生窗口/481详情独立核销，14,400采样tick不含窗口间隙、不是Governor。油156≥150、塑料79≥60、石墨176均真实生产；油均值−0.125、塑料均值+55.75、氢78产/92耗且罐−16，各原子门通过，network2/4采样均满电。但水/有机晶体全窗零产、钛块后两组零产、钛晶石末组零产，钛来料下界12.3638/min<45；黄糖29通过数量门仍消耗旧库存，不据此宣称联合恢复。accepted3/revision173/save33689680保持，停止同类重采。
- 消费边界复验：760有水2/油3而塑料0，761水20/油340而塑料0，558首样1984→2064；水泵output_blocked是停耗后的局部症状，不能从水0/0推断源泵需扩建。先追塑料到有机厂的真实有向输入，不用再补上游石墨掩盖运输断点。259空、530缺矿和102仍产120矿同时成立，矿机产出/先前入口60矿/min额定值均不证明当前实际到货；钛问题另沿现有物流追，不重做已成功升级。
- 调用端边界：生成PowerShell检查脚本时，含字面量`$'`的替换文本必须使用返回字面量的回调，避免JavaScript替换字符串将它展开为原串后缀。此次只读审计在语法检查阶段发现并修正，随后才读取/核销原始完整样本；无游戏写入，不能把脚本语法通过当成产量门通过。
...
- 2026-09-10科研队列复验：raw-8f9d7ae3在prepare前同样把原生techQueue=[]经函数管道展开成null；没有accepted。固定执行器显式保留集合，空/单项/有序双项/缺字段4类回归和真实失败raw离线复现通过，随后raw-0f6c3561两项选科成功。原契约动作kind是select-research而Bridge方法名用下划线；审计从实际DTO/契约取值，不能混写。旧全厂分页和141条局部详情不是同一覆盖范围，需补用本次明确来源调查；root最终raw-d08c4450仅只读核销，未重放游戏动作。
...
- 冷部署后恢复复验：正常save34182274→同档恢复34182305，raw-705122df三个独立600tick窗全部晚于恢复保存水位，塑料P/C9/10、有机5/3、钛晶石4/4、黄糖4/0；761塑料561→563→566，2268仍闭仓331，760满电工作，三塑料入口和专用油水配置保持。root raw-d3a5b0af/ ec7f168e独立核对原生窗口、fresh终态/配置及完整玩家/J56，accepted9不清零。本档局部塑料送达及保存恢复已通过；未核销钛长期到货/全链供给，不能用短窗P=C替代配平。重启后的第一个计产窗口必须完整位于恢复边界之后，不能给旧原生统计环形缓冲的保存内容新session信用。
- 关闭后送达/保存复验：raw-036e9c7e三个独立600tick窗（34165815/34166443/34167054）塑料P/C9/8、有机4/3，761塑料389→391→394、2268闭仓库存331保持，实际塑料持货和目标满电可读。普通save34182274由root raw-1adc19ef核销原生三窗、唯一save终态/全玩家/J56及三入口/闭仓，accepted8/revision182；未实际恢复、未满足整链持续供应或Governor门。原生统计窗为inclusive `[start,end]`，`end-start+1=elapsedGameTicks=600`，下一窗实际start须大于上窗end才独立；9个离线反例拒绝畸形/重叠，不能把一个字段算错当成Plugin无产量。任何新观察周期都不重置此前失败的临时配置期限。
- 2026-09-09残料收尾复验：raw-5b2efbd1/1ace3b86两原生SetBans终态在34093959/34106092完成2268的30→0→30；root raw-b0339071证明每个动作即时库存/连接守恒，期间正常运入水2/油32/塑料32，氢不变。raw-d360b7db再核销11对象、全玩家/J56/pre0/healthy，accepted7保留，未保存。临时开放实际12133tick超过预声明6000，时限失败必须保留，不能用成功关闭或已送达塑料掩盖。
- 客户端执行教训：成功commit后汇总对象形状错误不是原生动作失败，先核销同一action，不重放。已开放设施持续按游戏tick运转，失败的客户端观察器不能让物理排料“尚未开始”或重置原终态计时；有限临时配置应先保留原生终态/恢复期限，展示层失败不能中断已批准收尾。后续只读观察失败也只修复未完成的读取，不重复已完成配置。两个观察器连续失败后必须由主会话接管方案。原始native窗口须按真实窗口字段验证，不凭临时脚本一句异常丢弃全部可独立核验的证据，亦不能据一个ready截面宣称三窗完成。
- 状态：`observed`
- 日期：2026-09-08
- 最近复验：2026-09-09，塑料与油的饥饿方向对调；不是新工具缺口或仓761再次缺预约。
- 当前最小阻塞证据：石墨修复后塑料上游558持续增加而761塑料0，三轮局部图raw-e7aa5125与12对象raw-33d22dbb确认混流支路/过滤不变、满电；761有11个空塑料预约格，17油格却已340。root用完整raw-8dd09d6e沿唯一出带追到2282：path51真实尾段288/16、无输出，33876849的rearPickup实测油1。2269旁路2280无过滤且持油1，唯一目的仓2268仍bans30，库存水19/油45/塑料153/氢48；冗余入口2218仍滤油、2229仍滤水。这个具体尾端/容量边界应先于继续增加上游产量或重复查水源。
- 当前处置边界：专用油2557和水2299已有独立供给，先fresh核对后，主会话可设计把冗余混流入口2218/2229限定为塑料，只影响未来pickup，既有油/水持货必须继续原目的端。随后才计算带上有限残货的正常去向；不能只重新开放有限溢出仓就宣布长期修复，不重复已成功的专线。该诊断尚无新commit游戏动作或供料恢复信用；三轮实际轮首相隔599/598tick，不能伪称三个独立600tick产量窗口。相关SorterFilter18测试通过不替代实机配置/持续生产。
- 隔离入口实测：raw-0dc8673f两项原生预检零材料/无blocker，随后raw-8b7f6a21以fresh计划完成2218和2229的filter1115，唯一terminal在33958597/33958759。root raw-0cd4ded0复读两原始终态、10对象身份/位姿/连接/配方/供电网络及仓配置、全玩家/J56，于33968632独立核销通过：仅两过滤改变、无材料变化，accepted3→5/revision177，主档仍33689680。持油/水的旧手臂没有被清空或改向；这里只证明未来不再由这两支注入油/水，尚未开放2268、处理尾部残料、保存或证明有机/黄糖持续恢复。
...
- 2026-09-09长施工观察：41带14debac6实际35971937→35987037完成，原180秒调用等待先结束，继续同一action只读取得terminal而非重放。root raw-f97aa5eb核销41个唯一NEW/有向连接、20旧对象、全玩家仅带−41/J58/pre0/3idle/healthy。调用等待预算应参考prepare的estimatedGameTicks及现场无人机进展；观察超时不是游戏终态，也不调整原生watchdog或开放下一写。全局accepted7须由原始接受记录累计，不能采用子任务回报中的局部/错误计数。
- 后续同档入口实测：raw-ad8181ac的31be664b在35938052成功建2748，root raw-964999ad核销2742:4→2748→707:7、原油1007过滤/network1满电、原十六对象及全玩家仅2011−1/J58/pre0/3idle；accepted6/revision61。私有执行器将有界readiness结果与已成功施工分开落盘，不再因后续无人机检查未满足而丢失成功摘要。本次readiness实际为true；这不改变Plugin终态，也不授权在未ready时继续提交。
- 2026-09-09复验：raw-3cbae54b的六NEW带动作9f5d6a71在35906777已成功，私有脚本只因24次250ms读取尚未全部idle而停止。root raw-77faf737于35919061复读原始/fresh成功、六唯一实体/完整双向链、十旧对象不变、全库存仅带−6/J58/pre0/3idle/healthy；accepted5保留，没有重放或把施工成功改判失败。仍只把空闲视为下一动作条件，不据working推断无人机具体阶段；随后新尾2742→707 slot7原生过滤sorter预检通过，尚无该sorter提交或持续供油证据。
- 状态：`validated`
- 日期：2026-09-07
...
- 2026-09-09调用端复验：raw-bff9e4f6的Move已在35330827成功，首次后置Walk仍1.8026，私有执行脚本要求立即speed==0导致误停；raw-0e356b54随后证明Walk0/400MJ。修正私有调用端，沿用包内指南最多八次250ms只读减速复查，不增加Move、不改Plugin成功条件或延长watchdog；非ready仍停止后续写，保留原成功终态。
- 同日连续跨水正例的证据边界：raw-542455a0只到此前实际Walk且当前完整工厂/附近端点复核的落点，35381567成功后首次仍Drift；root raw-6c0fa336在35386629已读到Walk0/400MJ，不再执行返回意图。两次读取间无连续落地记录，不能声称固定落地时长，也不能从一个成功落点外推未知岸线可行。超过32m的unknown预览仍是未知，不被改写为无风险；没有重放或位置写入。
- 状态：`validated`
- 日期：2026-08-31
- 适用范围：当前正常地表移动 coordinator 与后续依赖玩家 hash/位置的动作。
- 当前结论：移动action达到到达容差后可以终结，但物理减速或Drift仍可能存在；arrival成功与后续业务ready分开。按当前指南复读Walk、速度≤约0.1m/s及足够能量，再使用fresh玩家hash；不要要求成功瞬间严格零速度、误报动作失败或重放。观察界限届满仍不ready时重新分析，不能无界等悬停；若复读已稳定Walk则撤销过期返回意图。
- 直接证据：移动 action 已完成，而紧随其后的速度仍约 `4.22`，15 秒后仍有约 `0.19` 的 Drift。
- 限制或反例：具体衰减时长和阈值受地形/帧率影响；早期采用的`speed < 0.05`是当时操作阈值，不是DSP API常量；现行约0.1同样不是物理保证。
...
- 最近复验：2026-09-09（两唯一成功Move与随后ready核销；修正私有调用脚本对旧经验的遗漏，未新增公开动作）。

### EXP-010 — 三台风机只够覆盖当前油井的紧负载

- 状态：`superseded`（2026-09-09纠正先前invalidated的理由，见EXP-274）
- 日期：2026-08-31
- 适用范围：当前普通 1x 世界、实体 `129` 油井和当前版本电力数值。
- 当前结论：先前把14000判为“记录错误”的推断不成立。400是历史瞬时请求，当前原生目录明确工作14000/待机400；完整解释和容量预算由EXP-274替代，不再以EXP-017的轻载截面否定工作值。
- 直接证据：历史十动作复核时实体129的powerDemandPerTick=400，网络3两个消费者energyRequired=550、合网capacity=51000，热电接入前风机capacity=15000。这些实际读数保留，但只证明当时请求，不能推出工作峰值余量。
- 限制或反例：保留原观察与本次修订，不把瞬时值或旧“14000错误”解释用于满负荷规划。
- 复验触发：供电建筑、科技加成、网络拓扑、DSP 版本或油井参数变化。
- 关联：`docs/gameplay-timeline.md`、`docs/research/game-api-m0.md`。
- 最近复验：2026-09-09（目录和读取字段语义反证先前失效判定的理由；按EXP-274重新区分工作/待机/瞬时）。
...
### EXP-017 — 历史油井轻载样本不代表满工作容量余量

- 状态：`superseded`（容量外推由EXP-274替代，历史测量保留）
- 日期：2026-08-31
- 适用范围：当前版本、实体 `129` 油井、实体 `138` 基础分拣器和网络 3 的 DTO 单位。
- 当前结论：400/550是历史瞬时需求，不能外推为油井与sorter的工作需求。当前目录分别为14000/300J/tick；三风机15000仅余700。四风机新方案及实际覆盖/持续供电须按EXP-274另验，不能继续声称14000已被反证。
- 直接证据：旧实体与网络在同一tick附近的结构化读回400/550、serve/consumer ratio1.0均保留；新目录和字段映射证明它们与工作上限是不同口径。
...
- 最近复验：2026-09-09（修正瞬时请求到工作容量的错误外推；不把新目录预算称为新电网实机通过）。
...
- 2026-09-09边界澄清：本条“历史Walk落点”约束适用于长距离或缺少完整采样的跨越。完整≤32m短预览已能区分中途水段和目标端地面；达到EXP-236的末端≥4m/地面高于水面≥0.1m/零unknown条件时，可由外部Agent选择一次明确尝试，不再要求该新落点曾被玩家走到。两种证据都不保证Walk、碰撞净空或免于失败复核。
- 2026-09-09复验/指南修订：两个短段岸线候选停止后，raw-b06364e7重新观察历史Walk落脚点16230并复核2741实体，当前约118.888m连续目标非带建筑最小中心净距5.584m；旧15416已不存在，不能沿历史锚点照搬。原生surfacePreview仍明确unavailable，不是全程陆地/无碰撞保证。Luna raw-4e74e39f唯一Move在35012527成功，root raw-36ed17ab于35058609独立核销原始/fresh终态、稳定Walk0/400MJ及完整玩家材料/J58/healthy；没有水面中点、重试或飞行。accepted6，主档34977364尚不含新Move。精简指南及MCP资源测试吸收“当前复验过的Walk终点＋有限能源/恢复余量＋同一订单到终态”，不是未知海岸的一般放行；2项新测试先红后绿、MCP91项通过，新指南仅源码/Debug，未冷部署/最终打包。
- 状态：`observed`
- 日期：2026-09-01
- 适用范围：密集工厂与水面混合地形中的普通地表移动、已有大量风机/电塔/生产设备可供只读定位、跨区域取料。
- 当前结论：直达球面分段在未知地形中失败后，不应继续猜同一弧线。分页读取 factory entities，排除 belt/inserter 后可用实体位置构造候选锚点链，但建筑存在不证明陆地或可行走。目标选在基座外，容差按当前地形与EXP-246/253选择，不机械沿用历史4–5m岸边提前停靠；每段后复读Walk、速度、能量。密集设备正好落在直线上时，先按最近碰撞体的局部切向背离约5m，或选设备侧面的明确绕点。长距离或没有完整地表采样时，只有已验证的对岸Walk终点在当前owned world重新观察、当前已知障碍与有限跨越/恢复能源均复核后，才可选一个连续原生Move穿越中间水面；缺失地表预览仍是未知，不能外推为任意岸线放行。完整短预览的另一路有限条件见EXP-236。把水面中点当作分段终点会制造不必要的Drift恢复。到达业务prepare已通过的交互范围后直接做原业务，不必再撞建筑中心或穿越密集厂区。
...
- 最近复验：2026-09-09（当前重验16230后单个约119mMove成功；15416失效。历史2026-09-03的旧图和侧绕正例仍只是候选骨架，不是免复验白名单；本次MCP指南修订尚未冷部署）。
...
- 2026-09-09复验：新原油尾段六点正常施工后，raw-77faf737逐计划坐标匹配唯一实体并逐边反向核销2747→2746→2745→2744→2743→2742，头只有输出、尾只有输入、无额外外接，旧707/708/709/710/711/1658/2521/2522/2524/2559保持。原生返回顺序本次恰与拓扑相同不构成ID规律；后续只对实际尾2742的fresh端点哈希预检。材料257→251/原动作成功及J58保持，未把六带或入口预检当供油/保存恢复通过。
- 状态：`observed`
- 日期：2026-09-03
...
- 最近复验：2026-09-17（恢复后84/679/939仍有`research-matrix`点数与既有连接；`recipeId=0`不等于空实验室，不得仅凭此字段改作生产。此前2026-09-06的84/679 points/3600与76/256 items/1证据继续有效；当前科研队列为空也不能抹去已有配置/点数）。
...
- 2026-09-12补充：返回一元逗号包装的ID数组时，`@(函数调用)`可能再增加一层；下一业务尾端转换错误仍属调用方故障。先单次赋值并校验扁平正整数/唯一/数量，再追加到外层路径列表。IFX-107的13项纯表达式复现/正反通过，实机两笔成功前缀已独立核销；新后缀不能据此提前放行，更不得重放前缀。
...
- 2026-09-09线路调查复验：原生长带明确OutOfReach后，raw-2ca1e816按主会话三个固定接近点仅完成前两Move；第三目标11个风险样本且末端仍水下，零commit。另一南向raw-605dc5fe末端同样水下，停止相似候选。其样本6–18虽聚合not_detected，地面仍低于水面约0.18–0.33m；浅水标签不能替代明确陆地证据。root raw-f6758f7b核销前两成功、Walk0/400MJ、全背包和J58，accepted8不归零。后续只能以已读地形重新设计陆地/合格短跨越，不重放成功段或把无commit预检拒绝算成动作失败；完整原油输送仍未证明。
- 2026-09-09调用指南缺口（IFX-078）：raw-76838e13的完整27.972m/29点预览有11个风险点，均在中途；末端约12m地面高于水面，最后六样本跨约4.995m、地水差≤−0.197m。旧指南把任意聚合detected与没有历史Walk目标组合成拒绝，丢掉了现有逐点信息。此为调用方解释过严，不是采样/API错误；观察本身不证明已移动或新油源可施工。
- 现行短跨越解释：完整observed、零unknown、≤32m、实际点距≤1m，并逐点核验末端至少4m的groundHit/waterHit有效、有限非空groundBelowWaterMetres≤−0.1且无风险；结合当前障碍、地表径向差及有限跨越/恢复能量，外部Agent可选择一次fresh normal Move。容差沿用EXP-246/253优先1.5m，并使末端余量≥2×容差+1m；最初强制0.5m的指南遗漏了既有三维距离经验，已修订，不改已失败动作。浅水not_detected、partial/unavailable/缺失样本不满足；不扩大射线预算、不选择下一目标或更改Plugin准入。终态后真正Walk/低速度/能量仍须实读，不重试失败目标，不把指南条件当作保证或自动循环。长距离/不完整预览沿用EXP-093历史Walk要求。
- 材料区续作反例：raw-82963b65的28.8m接近35083138成功Walk0后，raw-1dd87cc4/raw-0664ab7b向北/南两个几何净空候选分别发现11/7风险点，均零commit；raw-61647826复核accepted7/revision14/J58/全玩家保持，停止第三个相似路线尝试。不能以中心净距5.65m或8.69m代替水面预览，也不能因下一段失败重做前段。已在范围内的仓26两份transfer预检正常，先做原业务而非继续撞建筑；尚未转移或扩建，长线路预算和其他材料仍待。
- 连续跨越边界补充：EXP-093当前复验的16230是先前实际Walk且本轮重新观察的终点；raw-4e74e39f约119m单订单35012527稳定Walk0，root raw-36ed17ab复核通过。超过32m仍unavailable，不能伪造dry-ground；不再把“普通短距接近必须检查预览”机械变成“所有跨越都切到水面中点”。外部明确选择一次有限目标、能源/恢复余量、当前障碍和终态/落地复读不可省略。MCP资源91项测试通过只证明新指南可读，新文本尚未安装；原生采样/准入和工具数量不变。
- 2026-09-09复验：原油备料接近raw-d94d6515/raw-f12e37a8的两个首段分别在34937559/34960433正常到达Walk0；两条后续24.85/23.94m预览分别发现6/15个水深风险点，均在commit前停止。既有工厂锚点不能证明整段陆地，成功首段也不能给后续候选信用；两次后续路径被阻后不继续相似尝试，主会话重新设计。root raw-44cfe569核销两唯一终态与物料/J58/400MJ；正常save34977364由raw-2ca7007d独立核销，accepted5保持。37项地表/看门狗测试通过，没有落水、卡停、回档、新工具或地形修改。保存不算恢复验证，也不抵扣原油供给和版本门。
...
- 最近复验：2026-09-09（新油源备料接近的两正常Move及两个未提交的岸线反例，正常保存34977364；前次31761367十写审计与早期1405/恢复证据边界保留，当前未重启）。

### EXP-237 — 调用层参数、配方预算和终态证据必须各用自己的契约

- 七NEW续建实测：raw-c1b3cc40的a9a2077d在36015790正常完成，root raw-59b03479核销2789→2796→2795→2794→2793→2792→2790→2791、210→203带、68对象/原边/全玩家及J58；共享源未收费。仅2789朝向变化，采用已安装coordinator对entity/collider的精确原生旋转证明，不把改变一律当损坏，也不任意豁免其他旧对象。第二桥实际端点2791→2747原生过滤预检通过但尚未施工；accepted8保留，持续供料/保存恢复未证明。
- source-bound预算实测：41带完成后raw-f97aa5eb从真实自由尾2789预检原8点北向段，返回7个NEW、non_removing_belt_cover/whole_path_native_rotation_v1/full_path_stage1；七个吸附点精确等于原free-path去掉共享首点，成本7而非8。当前只完成该预检，尚无续建/跨线连接；源复用及旋转60项离线通过不替代后续原生扣料、旧边/货物/朝向和逐对象终态核验。
- 2026-09-09施工顺序复验：实际尾2742经fresh原生exact_slots接入707:7后，raw-964999ad对41NEW东段完整预检仍通过，零该段commit。相邻41/8点候选共享一端，必须先按原油有向顺序建41段，再从其真实自由尾fresh source reuse延伸；不能先建共享端后的下游，再用free-path掩盖不支持的destination merge。两处独立跨线桥仍须各自实际实体/native槽位验证，原井及持续供应尚未完成。
- 最后一只sorter备料复验：raw-b961eec8按固定顺序两笔各一件正常转移及recipe85手搓完成，原生终态35853779/35853855/35854035；root raw-ef6da73c在35859763独立核销本窗四个成功动作、全玩家净2011+1/其他0、两源同tick扣料/配置、J58及Walk/满核心/healthy。材料现为1井/4风机/257带/3基础sorter/1塔；accepted4/revision57保留，主档35281231未变。ActionClient21项通过；齐料不等于三个新连接或持续供给已经通过。
- 余段复验（2026-09-09）：raw-ceee1ab0的49/41/8/6带与先前7/85/24带均为独立完整free-path正例，合计220点去重为216，四个共享边界必须保留未来source reuse语义，不能按220重复建造。两跨带间距3.4588/3.7736m及三处候选供电覆盖只支持有限施工方案，仍不能代替建成后的原生sorter槽位/角度/双向读回。新塔raw-f51f4861实际吸附后距140为13.9312m、第一桥7.2745m；其他两接点对应1658/711，全部未施工。主网当前满供电/瞬时余量不保证全部旧消费者同时满工作；后续持续窗必须重新核销真实负载。
- 同名作用域复验（2026-09-09）：只读油路脚本的ExpectedRevision=49被dot-source辅助文件的默认参数2覆盖，仅session读取后误停，尚未prepare。采用独立CrudeExpectedRevision后，零Bridge离线对照证明共享名称被覆盖而独立名称保留；随后raw-8a337f28正常完成85带原生预检。此为私有调用端作用域错误，不是陈旧游戏状态或Plugin拒绝；不能因此重复任何已accepted动作，关联IFX-079。几何计算中的foreach先收集成数组再管道，避免将纯解析错误误记为原生失败。
- 分段预算边界复验：raw-8a337f28/40c66dbd/0cb96150分别通过85/24/7带free-path预检；各自真实成本不构成整线原子许可，共用首尾点不能重复计料，未来source-bound接点必须按实际实体/hash重新验证。129→151已知连接/姿态可用于预计同原型井口方向，但新井未建前不能声称该端口已实测；剩余线路/桥/入口和整套供电预算未通过，不先建孤立油井。最近复验2026-09-09，范围仅当前候选和私有调用端。
- 备料续作实测（2026-09-09）：raw-28e18acf的recipe14唯一正常手搓在35262985完成，root raw-c42af587核销完整递归材料净差，实际占格40→37，随后铁280的fresh prepare通过。raw-ee8ed84c仅执行未提交铁料、四风机、252带和正常save35281231，root raw-c5efde54十写审计通过；原三笔成功转移不重做。`inventory`的20个聚合条目不能称20格，实际占格字段与各entry.slotCount合计均37；容量仍须每步fresh预检。
- 调用脚本复核：raw-3767cb71在prepare前因`[ordered]`的整数索引重载越界，只有三次只读、零游戏写。物品ID映射使用普通字典并枚举Key/Value，函数结果先括号再比较；同raw九项与现有ActionClient21项离线通过，随后唯一原生手搓成功。此项只修私有调用端，不是MCP/Plugin缺陷，不新增通用工具；详见IFX-077。
- 批次容量复验（2026-09-09）：raw-07ef5a07四份针对同一初始背包的材料transfer预检各自通过，但raw-83e4e2e前三笔原石24/石材12/钢材12占格后，第四笔铁280的fresh prepare返回INVENTORY_FULL且无commit。不是已接受动作丢料或Plugin失效；独立prepare不预留后续容量，不能相加成整批空间证明。root raw-dc6b02c9核销三个原终态/玩家及J58，accepted5保持；只有完成一次已备料的正常recipe14×1手搓并核销新背包后，才可fresh准备原未接受的铁转移。该截面手搓仅有预检正例，后续执行证据见本条新增复验；不丢弃物品、不重做成功转移，不把直接配方预算冒充递归净投入。
- 状态：`observed`
- 日期：2026-09-07。
...
- 最近复验：2026-09-09（备料续作、调用端字典与占格计数修正、正常保存及完整十写审计通过；不声称新井已施工）。
...
- 2026-09-09反向复验：运输船引擎3403在34737719正常入队后，raw-0b22c7c3于34737681→34737827记录背包蓝77/黄40变为0，机甲缓冲恰增加277200/144000点；自动管理开启，非矩阵和背包加缓冲总量保持。root raw-9e418cfe核对原科研终态itemDeltas=[]和真实hash增长、J57/57。调用端遗漏本条而误报，已修正有限验证文件并以原始证据及缺失缓冲/错误点数/非矩阵变更三个负例验证，原科研没有重放。说明此前公开字段和指南已经够用，不能把重复漏算包装成新的Plugin能力缺口。
- 状态：`observed`
- 日期：2026-09-07。
...
- 最近复验：2026-09-09（原有取回时间仍未知；新增正常入队后整件转入缓冲的独立正例，无点数注入）。
