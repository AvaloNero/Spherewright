...
- 限制或反例：本次只永久化了有机晶体输出到钛晶石输入的短桥；塑料、水和精炼油仍由各自自动产线产出后经玩家做本地守恒搬运，不得把它表述为有机晶体三料无人值守供给。瞬时带快照未必抓到移动货物，故本次以 sorter 携货、下游设备取料、配对金刚石消耗和普通保存组合验收；若目标仓过滤被改动，共享带可能串料。
- 复验触发：目标仓 `768` 过滤或带路变化、第二个新物品汇流、出现带阻塞/串料、上游三料改为全自动配送、DSP 或程序集版本变化。
- 关联：EXP-007、EXP-018、EXP-028、EXP-086、EXP-088、EXP-118、EXP-129、EXP-133、EXP-142、EXP-143。
- 最近复验：2026-09-03（10 件有机晶体批次真实经过 sorter `2032`，钛晶石与黄糖设备依次运行，主档保存到 tick `12996056`）。
