...
- 最近复验：2026-09-02（普通电力感应塔 `915` 未被误当无线充电证据；路径受阻后以守恒燃料回退维持安全余量）。
...
- 最近复验：2026-09-02（新增电塔/研究站/制造台夹缝样本：6 m 自由扇区脱困成功，保守网格近端仍被原生碰撞反证并停止）。
...
- 最近复验：2026-09-02（风机 `82` 的几何中点再次入水并停止；单一已建地面终点跨水直达成功，返程在 fresh 密集工厂碰撞处另由看门狗终止）。
...
- 最近复验：2026-09-01（石墨烯备料往返完成陆地锚点链、两次连续跨水、局部基座绕行和 80 m 内远程守恒投料）。
...
- 限制或反例：专用哈希只排除 `hashUploaded` 自然增长；队列顺序、当前科技、解锁、等级、前置或 `CanEnqueueTech` 变化仍必须 stale。此次 `1703` 因缺少红矩阵没有自然增长，下一次持续上传期间仍应保留复验，但不影响队列与当前科技并存的 live 闭环。
...
- 最近复验：2026-09-02（同档部署后用专用哈希把 `1604` 一次追加到活跃队列，fresh 队列 `[1703,1604]`）。
