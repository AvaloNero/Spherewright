...
- 当前结论：下游设备只接受某种配方输入，不代表上游无过滤分拣器会替整条带筛料；从混料仓接出的所谓专线必须在接触物料前设置并复读过滤项，或直接从只含目标物品的仓/生产设备另建旁路。当前已部署旧 DLL 把完整运动进度哈希和 `stage=Picking,time=0` 都当作过滤前置，活跃源仓上几乎无法获得可提交窗口；在 EXP-102 的修复版部署并实机验收前，仍不得绕过 stale、带货强写或换键重放，只能先使用守恒回收/纯源旁路。
