...
- 最近复验：2026-09-01（电动机按“全链自动产出 → 日记 → 普通保存 → 独立工程提交推送”流程验收）。
...
- 限制或反例：当前正样本只有一座普通储仓和四个一级分拣器；它不证明储液罐、多层仓、物流站或载货分拣器可沿用同一配置时机。过滤只能保证物品选择，不能替代端口占用、距离、电力、带方向和下游背压验收。若共享仓未来加入新物品或新出口，仍需重新做空载/携货风险评估。
...
- 最近复验：2026-09-01（电动机线空仓四过滤出口、分批装仓、全部中间件与最终 39 电动机闭环）。
