...
- 状态：`validated`（两负例离线复现及两NEW续接施工；分拣器准入未验）
- 日期：2026-09-08
...
- 限制：只证明反例复现和两NEW场地/施工，不证明未来分拣器、油/黄糖持续产出或Governor。更优的离线角度本身不授权试建多个候选。
- 复验触发：DSP哈希、段边界、邻居/倾斜、原生投影实现、插值精度或现场改变。
- 关联：EXP-255、包内明确单带段/禁止重试规则、game-api-foundry、存档日记001。
- 最近复验：2026-09-08（两NEW非移除cover施工与独立审计通过；accepted9保留，实际附件仍待）。
