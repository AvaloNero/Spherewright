...
- 日期/最近复验：2026-09-09；状态`validated`，限源码及离线回归，冷部署/实机仍待。
...
- 限制/复验：不修改原生API、600tick窗口、稳定阈值、2×/10%/36000tick或动作协议，64工具/1资源不变；它不会把raw-8b336d9f失败基线改判通过，也不增加实际产能。当前安装仍79be0ae，新的同批冷部署后必须复验“只读重报价保留真实历史/真实源变更仍清除/原声明不放宽”。关联IFX-089及日记001；观察范围、源哈希、声明持久化或预算编译变化时复验。
