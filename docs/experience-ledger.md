...
- 限制：最初诊断为零写，后续目前仅建成一个未供料火电。持续燃料去路、实际炼油/黄糖恢复、多窗口库存趋势及保存恢复尚未通过；不能把诊断或施工结论当解堵。建厂前的路线预览不能覆盖新建筑出现后的场地，后续必须fresh完整带路与真实端口预检。
- 复验触发：氢取料端、满罐输出、火电燃料源、用电需求、仓储混料或session变化。
- 关联：EXP-247、EXP-250、存档日记001及包内buffer单位/有向边指南。
- 最近复验：2026-09-08（原阻塞证据保留；新窗accepted1火电正常施工、实际并网及独立完整阶段审计）。
