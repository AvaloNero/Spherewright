...
最近保存截面：同一owned世界104、fa29180/1488-test开发安装态，最近普通主档仍为f0d72dce的28811558；磁铁专线已送达并保存，详细两写审计见下。当前内存accepted8/revision78：三次备料、空矿机回收、新电塔3和两点矿机2435均已正常完成，尚未新保存。root raw-d37e23f5独立核销回收/两建设的三个唯一终态，2435满电并采出50铜矿，vein9/12合计实际减少50；旧8邻居结构/配置保持、0prebuild/Walk0/J55/55/healthy。矿机未接线，这不是九点铜候选或持续铜供给验收。用户新冻结主线已提交推送为7f93599：下一步先正常保存、冷部署同批Plugin/MCP、protected resume及磁路/缺料诊断复验，再推进九点铜矿完整方案；旧铁/铜下游/磁路不重做。原打包/CI改动保留，未tag/release。1508-test源码尚未冷部署；新路恢复、运行模块复制/取消续建、三级链、两倍十分钟、准备清单与最终双包仍待，无新最终ZIP。
...
- 当前开发安装态fa29180/1488，正常保存28811558；当前窗口accepted2保留。源码9a71250/1508-test的缺料诊断与MCP指南已离线验证，未冷部署新诊断。只用实际当前protected primary恢复，不依据本页旧tick或旧checkpoint选档；新支路的普通重启验证不能冒充Foundry中途续建，不能因提前审计或历史“归零”再次归零。
