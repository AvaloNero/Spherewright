...
关机截面：同一 Spherewright-owned 普通和平 1× 非沙盒世界已普通保存到 tick `8340400`、revision `677`，随后正常关闭；当前没有活跃 DSP 进程。最后一次科研读回 tick `8337127`，write health `healthy`。

## 结论与证据边界

- 记录仍在。本局受保护日记共有 `36` 条，已经持久化到 sequence `36`，`persistencePending=false`，`persistenceError=null`；仓库中的经验账本共有 `96` 条决策/经验，完整保留了每条的状态、证据和复验条件。
...
| 36 | 科技 | 1703 粒子磁力阱 | 2026-09-01 22:49:32.8062890 +08:00 | 8244528 / `001d 14:10:08` | 未完成；关机前最新 `92929/288000` |
...
当前共 `96` 条：`validated=59`、`observed=34`、`invalidated=2`、`superseded=1`。`observed` 表示已有样本但仍需复验；`invalidated` 和 `superseded` 不能继续作为现行规则。每条的适用范围、当前结论、直接证据与复验触发在 [experience-ledger.md](./experience-ledger.md) 中完整保存。
...
- DSP 已正常关闭。下次应在票据到期时间 `2026-09-02T23:16:11.8559867+08:00` 前消费本次保存自动签发的受保护 planned-restart ticket；恢复 exact primary 后验证 tick `>=8340400`、revision 不低于 `677`、planet `104`、和平/非沙盒/1×、write health healthy、journal durable through `>=36`，以及粒子磁力阱进度不低于 `92929/288000`。若票据已过期则停止，不得篡改票据或开新档。
- 当前主线：继续并完成科技 `1703` 粒子磁力阱；解锁后给预建制造拓扑补齐 particle container recipe/filter，验证第一批粒子容器自动产出。
- 接着完成物流运输机和行星物流运输站：无人机输入仓已经备有铁 `50`、处理器 `20`、推进器 `20`；物流站输入仓已有钢 `40`、处理器 `30`，还缺钛块 `40`、处理器 `10`、粒子容器 `20`。
- 推进器主档后的升级、研究选择、过滤器修正、预建、备料和有限混料清理都已经越过 tick `8340400` 的新普通保存边界。
- 第一批粒子容器、物流运输机、行星物流站各自达到自动输出闭环时，分别执行普通保存，并按“一个完成产品线一次 commit+push”落盘。
