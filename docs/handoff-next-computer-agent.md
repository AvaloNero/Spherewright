...
仓库已经具备安全本机 Bridge、34 个结构化 MCP 工具，以及从普通和平 1x、关闭沙盒的新世界推进到红色矩阵所需的读写原语。Gate A、Gate B 已有当前 DSP 版本的实机证据；Gate C 部分完成；Gate D 尚未开始。
...
最重要的边界是：**用户尚未授权创建下一份验证新档。** 本次“整理交接文档、提交并推送”不构成该授权。接手 Agent 在用户再次明确回复“确认继续”或等价指令之前，只能拉取代码、阅读、构建和运行不接触 DSP 存档的自动化测试；不得部署 Plugin、启动或控制 DSP、创建新档、执行游戏写操作，也不得读取任何已有存档。
...
9. 以运行时目录识别出的红色矩阵物品 ID 为准，结构化证明同一会话的目标产出缓冲从 0 变为至少 1；同时保存动作 ID、游戏 tick、配方、输入输出缓冲、功率、资源节点和玩家物品差量。
