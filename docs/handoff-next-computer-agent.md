...
- 当前同一 owned world 位于 planet `104`、和平、1x、关闭沙盒、写入健康；返航第三次从同一 checkpoint 稳定落地后，部署前主档保存到 tick `7198197`，新 Release 的健康 planned restart 只加载 ticket-bound exact primary 并恢复到不低于该 tick。恢复后完成铁矿续采、钢线复产、电磁涡轮、本地高纯硅、微晶元件、硫酸、处理器、石墨烯和推进器自动线；最新普通主档保存到 tick `8123715`（保存后复读 `8123723`）、revision `639`。Steam/Windows 账号信息从未作为授权或存档归属证明。结构矩阵线、科研 sorter `722/860`、新钢炉实体 `791–794`、新铁矿实体 `795–812`、电磁涡轮实体 `814–828`、硅链实体 `841–847`、微晶元件/处理器实体 `848–858`、自动石矿分流 `859`、硫酸实体 `861–867`、蓝矩阵修复 sorter `868`、石墨烯实体 `869–875`、推进器/后续预建实体 `876–905`、临时混料分流实体 `906–909` 与补电风机 `910–914` 均越过保存边界。
...
- 用户要求后续无异常时持续使用同一存档；每完成一种新产物流水线，先结构化复读产出、普通保存，再提交并推送对应代码和经验。红糖、动力引擎、塑料、钛块、金刚石、齿轮、电动机、水、有机晶体、钛晶石、结构矩阵、电磁涡轮、高纯硅、微晶元件、硫酸、处理器、石墨烯和推进器流水线已经完成。氢燃料棒 `1112` 已于 tick `7834800` 完成，推进器 `1113` 已于 tick `8098237` 完成；垂直建造 `3701` 已于 tick `8243611` 完成，粒子磁力阱 `1703` 已在 tick `8244528` 正常选择，最新读回 tick `8281224` 为 `37080/288000`。下一步等待它解锁粒子容器，再完成粒子容器/物流运输机/行星物流站自动线。粒子容器组 `884 -> 887/888/889(filter 1204/1104/1123) -> 883(recipe 0) -> 886 -> 885` 已备 40 涡轮/80 铜/80 石墨烯。物流运输机组 `892 -> 895/896/897 -> 891(recipe 0) -> 894 -> 893` 中，`895/896/897` 现已分别过滤铁块/处理器/推进器。行星物流站组 `899 -> 902/903/904/905 -> 898(recipe 0) -> 901 -> 900` 中，前三条已过滤钢/钛块/处理器，粒子容器 filter `905` 必须等 item `1206` 解锁后补。石墨烯取料已经验证 `82 -> 750 -> 724/723 -> 130 -> 133 -> 129/141 -> 183 -> 713 -> 120/119/262` 陆地锚点链和 `183 <-> 713` 连续跨水；完整总时间线见 `docs/gameplay-timeline.md`，拓扑证据仍以 `docs/m0-status.md` 与 `docs/experience-ledger.md` 为准。
