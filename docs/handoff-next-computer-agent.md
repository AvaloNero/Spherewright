...
- 当前离线源码由 `.local\dotnet\dotnet.exe` SDK `8.0.424` 完整构建，0 warning / 0 error，62 tests passed（Contracts 4、Bridge.Core 45、MCP 13）。新增逐存档日记、tank-fluid、移动 watchdog、同星系飞行与可重复检查点读档均等待能证明同档恢复的安全重启再部署。日记对新档从采用帧开始完整记录；对当前已有进度的档以 `historicalCoverageComplete=false` 迁移，不伪造过去的时间。实际起飞前，flight commit 必须先用 `GameSave.SaveCurrentGame` 创建独立内部档、核对 header tick、持久化票据，三步完成后才允许切换原生飞行；失败时始终重载同一 token 对应档。
- 用户要求后续无异常时持续使用同一存档；每完成一种新产物流水线，先结构化复读产出、普通保存，再提交并推送对应代码和经验。红糖与动力引擎两种产物流水线已经完成。用户已要求本次正常关闭游戏，下次以 tick `4409247` 的同一主档继续。恢复后还需实现“每个新档配套日记”：分别记录每种物品首次手搓/首次产线产出，以及每个科技/升级首次点击时的实际时间与本局游戏时间；关机前只做了初步 API 审计，尚无日记代码提交。之后部署已完成的飞行保护代码，先创建独立起飞检查点，再飞往有钛星球手动带回第一批钛并建立黄矩阵前置链。完整细节以 `docs/m0-status.md` 与 `docs/experience-ledger.md` 为准。
