...
- 最近一次 owned 世界已携钛稳定返回星球 `104`，换用另一个 Steam 账号后仍只通过本地受保护的同档证明恢复；最后 session 为 `ecfdfd5b-e7c2-4976-8053-ed2a8dd7335d`，和平、1x、关闭沙盒、写入健康。关机前普通保存动作 `507870de-2b19-45bc-af7a-1af2dec0d481` 已把同一主档持久化到 tick `5731056`，revision `592`、`ownedSaveState=saved`、`writeHealth=healthy`。随后进程 `39104` 接受正常窗口关闭并退出，没有强杀，live Bridge descriptor 已清理。Steam/Windows 账号信息从未作为授权或存档归属证明；下次只允许恢复这个精确保存点。
...
- 蓝矩阵再次停机的根因已经实读确认：旧分拣器 `551` 没有过滤项，实际从混料仓 `26` 把 item `1102` 磁铁送进所谓电路板专线。活跃分拣器的过滤 prepare 全部因状态逐帧变化而安全返回 `STALE_STATE`，没有强写；旧带现由 `563 -> 562` 守恒回收污染物。新建独立电路板输出 `36 -> 572 -> 571…565 -> 573 -> 76`，并从纯铁块仓接入 `28 -> 594 -> 593…580 -> 595 -> 36`。研究站 `679` 又接入独立蓝/红支线，蓝矩阵站 `76` 增加第二输出；多站交替读回最终把瓶颈定位到红矩阵/H2，而不是蓝矩阵或研究站数量。
- 精炼油现有永久输送：`163 -> 675 -> 596…674 -> 676 -> 557`，电塔 `677` 覆盖塑料仓末端；替换分拣器 `678` 恢复 `163 -> 183` 热电取料，网络 3 回到 `51000` 容量和满供电。第二精炼厂 `707` 按“空设备先接并过滤输出 `708/709`，最后接原油输入 `710`”建成；电塔 `711` 补齐氢出口供电，风机 `712/713` 把下游网络 2 提升到 `30000` 容量，在 `29428` 需求下满供电，红矩阵站 `256` 的氢输入达到 6。
...
- 关机前运行的 DLL SHA-256 为 `EA82C019659549009BB1EC577B015D7A76366C42CE99682145BC9DEC7414566E`，其 flight/稳定落地修复已经完成返航 live 验证。游戏正常退出后，完整构建 0 warning / 0 error、62 tests passed 的新 DLL 已安装，部署 SHA-256 为 `E6B48E498AABFE2A2EFD28789840684226E3C59A0B9219FD17BFA0AEA6044956`；新增 build-catalog `water-pump` 分类已有当前 DLL 字段/原生校验研究，但仍需下次严格恢复同一主档后做 live prepare/build/readback。
- 用户要求后续无异常时持续使用同一存档；每完成一种新产物流水线，先结构化复读产出、普通保存，再提交并推送对应代码和经验。红糖、动力引擎、塑料和钛块流水线已经完成。当前科技为晶体冶炼 `1403`，关机前读到 `16535/90000`，队列后继为电磁驱动 `1701`；高强度晶体 `1123` 尚未入队，因为活跃科研使完整 progression hash 每 tick 变化，prepare 安全返回 stale，未绕过校验。下次优先严格恢复 tick `5731056` 的同一主档，部署并验证抽水站目录支持，然后等待/推进 `1403 -> 1701`，依次完成金刚石、水泵与有机晶体、钛晶石、黄矩阵产线。完整细节以 `docs/m0-status.md` 与 `docs/experience-ledger.md` 为准。
