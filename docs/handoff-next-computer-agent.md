...
- 当前同一 owned 世界已携 1000 钛稳定返回星球 `104`，换用另一个 Steam 账号后仍只通过本地受保护的同档证明恢复，当前 session 为 `ecfdfd5b-e7c2-4976-8053-ed2a8dd7335d`；和平、1x、关闭沙盒、写入健康。去程检查点 tick `4617708` 与返程检查点 tick `4808424` 都由起飞提交在切换 Fly 前独立保存；返程修复期间两次重启、多次失败只重复加载同一 token。最终动作 `d95955e7-cd86-48dd-b79f-4cb54734863c` 连续 600 tick 保持 Walk/速度 0 后才完成，10 秒后位置仍完全一致。正常保存动作 `cb5ced55-36cd-4f80-9d60-6e497bdc732d` 已把修复后的蓝矩阵拓扑持久化到 tick `5407742`，live revision `374`；Steam/Windows 账号信息从未作为授权或存档归属证明。
...
- 仓库 `259` 现已正式作为钛锭线输入仓使用并装入守恒的 1000 钛矿；分拣器 `532/533`、熔炉 `530`、输出仓 `531` 和网络 4 的风机/电塔已经就绪，供电比例 1.0。运行时钛块配方为 `65`，仍等待队列中的钛矿冶炼 `1413` 解锁；在输出仓实际增长前不得把这条线标记完成或提交产物里程碑。
...
- 蓝矩阵再次停机的根因已经实读确认：旧分拣器 `551` 没有过滤项，实际从混料仓 `26` 把 item `1102` 磁铁送进所谓电路板专线。活跃分拣器的过滤 prepare 全部因状态逐帧变化而安全返回 `STALE_STATE`，没有强写；旧带现由 `563 -> 562` 守恒回收污染物。新建独立电路板输出 `36 -> 572 -> 571…565 -> 573 -> 76`，并从纯铁块仓接入 `28 -> 594 -> 593…580 -> 595 -> 36`。两条带分别为 8/20 段，四个端点均独立 prepare 后提交；铁块实际运输、组装机电路板输出、蓝矩阵站六个电路板输入和研究 hash `35643 -> 36180 -> 37800/72000` 均已复读。保存动作 `cb5ced55-36cd-4f80-9d60-6e497bdc732d` 已持久化 tick `5407742`，当前 revision `374`、写入健康，仍等待 `1122` 完成后进入 `1413`。
...
- 用户要求后续无异常时持续使用同一存档；每完成一种新产物流水线，先结构化复读产出、普通保存，再提交并推送对应代码和经验。红糖、动力引擎和塑料流水线已经完成；红矩阵仓 `260` 也已通过源分拣器 `342`、196 条普通带和末端分拣器 `529` 自动接入研究站 `84`。高分子化工 `1122` 已在修复后的纯源蓝矩阵旁路下推进到至少 `37800/72000`，队列仍为 `[1122,1413,1403]`；旧混料线路只作回收，不再视作电路板专线。下一优先级是在 `1413` 解锁后配置熔炉 `530` 为配方 `65` 并验收钛块输出，然后继续有机晶体、钛晶石、金刚石和黄矩阵生产线。完整细节以 `docs/m0-status.md` 与 `docs/experience-ledger.md` 为准。
