...
- 当前源码/MCP 注册面仍为 44 个工具。飞行 checkpoint 现在有 24 小时过期与 `active/recovery_required/flight_succeeded/retired` 生命周期；成功立即移除 reload capability，覆盖成功的 primary save 再 retire，启动时更晚精确主档也会淘汰 legacy ticket。健康恢复只选精确 primary，quarantine 才选 LastExit，两者 commit 前检查 header tick；一次性消费先写 token-hash tombstone。日记新增 `durableThroughSequence/persistencePending/persistenceError`，本次 live 为 `34/false/null`。
...
- 炼油混料分流仍是受控实验，不是完成的永久物流：无过滤 sorter `906` 在中继仓表面满载时仍预取异物并持货，说明普通仓满载是槽位语义。临时仓 `907` 与 sorter `908/909` 已把源仓 `163` 泄压到当前空仓；下游 `790` 恢复精炼油 filter，因此红糖侧只取油，但 `784/907` 仍为混合库存。必须等 `906/908/909` 全部空载后先分别设置油/氢过滤，再守恒清理两仓，不能把当前状态当成分产完成。
...
- 用户要求后续无异常时持续使用同一存档；每完成一种新产物流水线，先结构化复读产出、普通保存，再提交并推送对应代码和经验。红糖、动力引擎、塑料、钛块、金刚石、齿轮、电动机、水、有机晶体、钛晶石、结构矩阵、电磁涡轮、高纯硅、微晶元件、硫酸、处理器、石墨烯和推进器流水线已经完成。氢燃料棒 `1112` 已于 tick `7834800` 完成，推进器 `1113` 已于 tick `8098237` 完成；下一研究优先垂直建造 `3701`，再推进粒子磁力阱/粒子容器与行星物流。粒子容器组 `884 -> 887/888/889(filter 1204/1104/1123) -> 883(recipe 0) -> 886 -> 885` 已备 40 涡轮/80 铜/80 石墨烯。物流运输机组 `892 -> 895/896/897 -> 891(recipe 0) -> 894 -> 893` 中，`895/896` 已过滤铁块/处理器，item `1405` 已解锁，下一写入应 fresh 复读并只补 sorter `897` 的推进器 filter。行星物流站组 `899 -> 902/903/904/905 -> 898(recipe 0) -> 901 -> 900` 中，前三条已过滤钢/钛块/处理器，粒子容器 filter `905` 必须等 item `1206` 解锁后补。石墨烯取料已经验证 `82 -> 750 -> 724/723 -> 130 -> 133 -> 129/141 -> 183 -> 713 -> 120/119/262` 陆地锚点链和 `183 <-> 713` 连续跨水，完整细节以 `docs/m0-status.md` 与 `docs/experience-ledger.md` 为准。
