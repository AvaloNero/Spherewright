...
- 当前 active owned session 为 `905747a6-21cd-4782-81c0-9abeb5b5536a`，planet `104`、和平、1x、关闭沙盒、写入健康；钛晶石里程碑普通保存已把同一主档持久化到 tick `6518917`，revision `270 -> 271`，并再次签发受保护的正常重启票据。换用另一个 Steam 账号后仍只通过本地受保护的同档证明恢复，Steam/Windows 账号信息从未作为授权或存档归属证明。已重建的科研 sorter `722`、晶体冶炼/电磁驱动/高强度晶体解锁，以及金刚石、电动机、水、有机晶体和钛晶石线都已越过最新保存边界。
...
- 精炼油路线 `163 -> 675 -> 596…674 -> 676 -> 557` 只有物理连接与端点供电成立，不能再称为纯油永久线：无过滤 sorter `675` 曾把 `557` 污染为精炼油 33/氢 271。高强度晶体研究期间三次满仓反压均通过正常守恒腾位恢复；纯油仓 `286/773` 现各保存 600 精炼油，另有 200 油进入有机晶体输入仓 `761`。这些库存没有删除，但只是有界缓冲，后续仍需送入经过空载过滤的连续有机晶体/燃料下游。替换分拣器 `678` 恢复 `163 -> 183` 热电取料的证据仍有效；第二精炼厂 `707` 的空载双过滤输出、供电和网络 2 满服务证据也仍有效。
...
- 用户要求后续无异常时持续使用同一存档；每完成一种新产物流水线，先结构化复读产出、普通保存，再提交并推送对应代码和经验。红糖、动力引擎、塑料、钛块、金刚石、齿轮、电动机、水、有机晶体和钛晶石流水线已经完成。结构矩阵科技 `1124` 已正常设为当前唯一研究目标；它完成后才解锁黄矩阵配方 `27`（1 金刚石+1 钛晶石）。下一步优先维持蓝/红矩阵供给并预建黄矩阵的空载过滤输入/输出物流，只在 `1124` 完成后配置 recipe `27`。完整细节以 `docs/m0-status.md` 与 `docs/experience-ledger.md` 为准。
