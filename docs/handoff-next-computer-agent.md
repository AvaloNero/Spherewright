...
- 当前同一 owned world 位于 planet `104`、和平、1x、关闭沙盒、写入健康；返航第三次从同一 checkpoint 稳定落地后，部署前主档保存到 tick `7198197`，新 Release 的健康 planned restart 只加载 ticket-bound exact primary 并恢复到不低于该 tick。恢复后完成铁矿续采、钢线复产、电磁涡轮和本地高纯硅自动线，最新普通主档已保存到 tick `7517473`、revision `169`；Steam/Windows 账号信息从未作为授权或存档归属证明。结构矩阵线、科研 sorter `722`、新钢炉实体 `791–794`、新铁矿实体 `795–812`、电磁涡轮实体 `814–828` 与硅链实体 `841–847` 均越过保存边界。
- 当前源码/MCP 注册面仍为 44 个工具。飞行 checkpoint 现在有 24 小时过期与 `active/recovery_required/flight_succeeded/retired` 生命周期；成功立即移除 reload capability，覆盖成功的 primary save 再 retire，启动时更晚精确主档也会淘汰 legacy ticket。健康恢复只选精确 primary，quarantine 才选 LastExit，两者 commit 前检查 header tick；一次性消费先写 token-hash tombstone。日记新增 `durableThroughSequence/persistencePending/persistenceError`，本次 live 为 `26/false/null`。
...
- 用户要求后续无异常时持续使用同一存档；每完成一种新产物流水线，先结构化复读产出、普通保存，再提交并推送对应代码和经验。红糖、动力引擎、塑料、钛块、金刚石、齿轮、电动机、水、有机晶体、钛晶石、结构矩阵、电磁涡轮和高纯硅流水线已经完成。当前研究为处理器 `1302`，最新现场为 `20340/144000`；下一步直接用仓 `843` 的自动高纯硅接微晶元件，再接处理器，随后推进推进器、粒子容器和行星物流运输站。完整细节以 `docs/m0-status.md` 与 `docs/experience-ledger.md` 为准。
