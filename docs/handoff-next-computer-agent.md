...
- 当前 active owned session 为 `905747a6-21cd-4782-81c0-9abeb5b5536a`，planet `104`、和平、1x、关闭沙盒、写入健康；有机晶体里程碑普通保存已把同一主档持久化到 tick `6315704`，revision `189 -> 190`，并再次签发受保护的正常重启票据。换用另一个 Steam 账号后仍只通过本地受保护的同档证明恢复，Steam/Windows 账号信息从未作为授权或存档归属证明。已重建的科研 sorter `722`、晶体冶炼/电磁驱动解锁、金刚石线、电动机线、水线和有机晶体线都已越过最新保存边界。
...
- 精炼油路线 `163 -> 675 -> 596…674 -> 676 -> 557` 只有物理连接与端点供电成立，不能再称为纯油永久线：恢复后仓 `163` 为精炼油 459/氢 13，无过滤 sorter `675` 已把 `557` 污染为精炼油 33/氢 271。下一写入先守恒清理并改用纯源或空载过滤。替换分拣器 `678` 恢复 `163 -> 183` 热电取料的证据仍有效。第二精炼厂 `707` 的空载双过滤输出与最后接原油顺序、电塔 `711`、风机 `712/713` 和网络 2 满供电证据也仍有效。
...
- 用户要求后续无异常时持续使用同一存档；每完成一种新产物流水线，先结构化复读产出、普通保存，再提交并推送对应代码和经验。红糖、动力引擎、塑料、钛块、金刚石、齿轮、电动机、水和有机晶体流水线已经完成。高强度晶体 `1123` 已正常设为当前科技，最新读数为 `26412/108000`，队列仅含 `1123`。下一步在等待 `1123` 解锁配方 `26` 的同时备好有机晶体与自动钛块的钛晶石输入/输出物流；只在科技完成后配置钛晶石，再接结构矩阵配方 `27`。完整细节以 `docs/m0-status.md` 与 `docs/experience-ledger.md` 为准。
