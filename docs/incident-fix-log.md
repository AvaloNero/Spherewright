...
- 修正/状态：`fixed`仅限私有线段投影，全部参数显式Double；实际绕行仍待。补中点最近对象排序、两端裁剪、本次精确反例，纠正首段旧离线预期；27+95预检及25+122审计离线、三文件解析、原执行脚本导入前退役拒绝通过。初始附加重载反例对首参数显式定型，已经改变了错误调用的语义，按真实动态变量重做后通过；无游戏副作用。公开PowerShell未发现该调用，Plugin/MCP及3m门、地表规则、状态哈希不变。未来使用更正几何重新设计有限绕行，不修改成功工厂或重放七动作。
