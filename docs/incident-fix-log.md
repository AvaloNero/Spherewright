...
- 状态：`fixed_offline_live_pending`，仅铜接线私有调用方；Plugin/Foundry返回正确，未改公共协议或游戏规则。
...
- 验证：9个纯预算正负例覆盖当刻2348余量、少1拒绝、精确边界、分步/完工、错误网/基载/负数/代理；17路径回归及解析通过，0游戏调用。原执行43回复与零写由root核销，旧23带不重做；修正后重进仍须fresh prepare、唯一commit和逐对象终态，实机接线/保存、持续供电与燃料门尚待。见[本档负例](./gameplay-timeline.md#2026-09-17--铜接线在首写前被私有供电预算守卫误阻断)及经验账本。
