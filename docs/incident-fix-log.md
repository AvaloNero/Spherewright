...
- 状态：`mitigated`仅成功prefix核销，顺序修复仍待。Sol负责只对原/fresh终态和新详情已证的单一ID建立临时两端核验范围，再严格双向slot/旧边/配方过滤供电验证后固化已建；不提前增加built，不泛化容忍未知新边。下一调用方只续后三建一存（6→10），既有4640/风机/S1不得重做；旧v3/参数数据不再用于live。持续供应与正常保存仍未验收。关联EXP-299。
