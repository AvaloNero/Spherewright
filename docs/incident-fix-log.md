...
- 首见：2026-09-09，在五对象蓝图数据往返/不稳定石墨基线之后，主会话审计“测量后计算准确2×目标”的调用路径。状态`fixed_offline_live_pending`；原42/36/36不稳定样本不是本缺陷的实机反例，也不因修复改判。
...
- 验证：13 Core+2 MCP新增回归，相关173+4通过；完整Release0警告/错误、1605项通过。未新增DSP调用、工具、观察字段或类型，未改变600tick/10%/36000tick门。包内指南和MCP描述同步；安装79be0ae仍未包含本修复，待同批冷部署和实机只读正例。关联EXP-282、日记001。
