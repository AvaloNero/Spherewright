...
- 状态：`mitigated`，限当前843仓原生配置修复与保存；不是持续硅石/处理器/紫糖供应或重启恢复通过。
- 根因：此前为晶格硅保留首格并开放一格后，仓库仍有高纯硅输入846和通往微晶机的输出850，但没有可接收1105的开放格。五条连接和既有库存不变属于安全不变量，不能代替多物料业务准入；这是规划遗漏，不是SetBans/SetFilter实现绕过原生规则。
- 处置：复用既有storage-capacity，不改公共API。先fresh证明完整投影 `[1113,1105×29] / bans29`，再独立fresh设置bans28，保留原1113正库存、五边和玩家；不清空、不搬货、不新增有限储罐来伪装持续修复。
- 验证：48项仓储回归及入口小测试通过；70330262/70330368两次原生配置即时守恒，正常save70330468/R24/J91。主会话原始/fresh三终态及完整5099实体/9936边独立复核通过，细节与证明哈希见[当档记录](./gameplay-timeline.md#2026-09-18--共享硅仓重新开放高纯硅输入并正常保存)。微晶机有working样本、处理器收到1微晶；硅石持续来源和成品出口仍未满足，不能核销产率门。
- 后续：沿用包内指南已要求的全部输入/输出与格位预算，把业务供需检查前置；不为这次个案添加重复规划框架。现场/原生规则改变时重新预检。关联EXP-221/224/299。
...
- 补料阶段复验：prepare_transfer无terminal专属targetItemId/requestedCount，改核sourceObjectId=723和storage-to-player的1101×54预算；原错误仅3读＋1prepare、零commit，修后fresh取料/递归制造/保存成功。只读摘要误沿用旧70详情集合并索引空预建筑页，也都在原回复已落盘后发生；复用56条并仅补未完成5条。根独立审计核完整5009实体/9754互返边、三终态及库存守恒，见[补料日记](./gameplay-timeline.md#2026-09-18--处理器整桥材料正常生产并保存)。仍标mitigated，不把局部修正宣称为全调用方已类型化或所有准备错误消失。
- 状态：`mitigated`，限本次调用修正、正常保存与同批原证据离线核验；没有新增公共API或宣称所有辅助脚本已修复。
- 根因：保存前把数值为0的待施工/修理计数包装成数组再取Count，误判玩家忙碌；采集摘要读取action不存在的gameTick；独立审计使用缩短的和平/沙盒标签而非真实confirmed_peaceful/confirmed_disabled。均是调用层表示错误，不是原生施工拒绝、游戏状态损坏或恢复失败。
- 处置与证据：首次仅两次只读便停止，修正数值判断后仅执行尚未接受的save；原action成功后不重放。采集128条回复全部已落盘，仅离线重建manifest；根审计修正枚举后复用同批原回复核全部实体/互返边/详情/库存/Journal，证明与边界见[当档部署前保存记录](./gameplay-timeline.md#2026-09-18--高架适配部署前保存与独立核验)。DTO字段与标量/数组的最小离线检查另核，不能用本地脚本错误消耗布局失败次数或重置accepted。
- 后续：沿用共享入口和真实DTO，不为单次保存或摘要新增完整执行模拟器。关联IFX-160、IFX-166；部署、恢复和持续产量尚待各自实机证明。
- 同日续验：坡A原生prepare通过后辅助摘要又误读不存在的preparedAtGameTick，未提交动作；仅继续原预算内尚未请求的B并通过，不重复A。2649aad冷部署/同档恢复及十写审计已另行通过；本条不因此提升为所有调用脚本已根治。

## IFX-167 — 显式坡段缺口及预建筑与完工姿态的不同生命周期

- 同日后续验证：原两坡的固定高位双cover水平合接已正常完成67NEW、88成员完整路径与保存70063762，并由独立整厂审计核销；详见[本档后续证据](./gameplay-timeline.md#2026-09-18--处理器高架水平合接施工保存与独立核验通过)。下述原两坡记录中的“高架水平合接待验”为当时截面，仅这一项已被后续正例覆盖；处理器供料、持续产量及高架重启恢复仍未核销。
- 状态：`fixed`，限当前DLL和2649aad的2001显式free→free单跨度，以及两条0→3/3→0实机施工/正常保存；不是所有地面候选都不可行，也未证明处理器已接通或全部层级组合均已实测。
...
- 验证与边界：详见[当档源码/测试记录](./gameplay-timeline.md#2026-09-18--显式单跨度高架带源码闭环实机待验)与[API研究](./research/game-api-foundry.md#implemented-single-span-proof-2026-09-18-offline-only)。2649aad已冷部署、同档恢复并严格核十写；后续两坡12＋9条完整native预检、正常建成和保存69923507通过，root5030实体/9792互返边/21详情独立核验，见[实机阶段](./gameplay-timeline.md#2026-09-18--两段原生高架坡道施工保存与独立核验通过)。不核销高架水平合接、处理器供料、持续产量或高架重启恢复；未知读回仍隔离且不得重放。

## IFX-166 — 阶段审计误把动态发电容量作为静态等值

- 状态：`mitigated`，限本次离线审计判据修正及同批原证据复核；未改Plugin、原生规则或公共协议。
- 根因：源分拣器施工后网3容量从1560000变为1632000 J/t，本地审计要求跨采样完全相等而停止。当前DLL的`PowerSystem.GameTick`逐tick累加当前发电能力并扣交换量，`energyCapacity`不是不变配置；不能把其变化等同于新增设备、损坏或缺电。
- 处置：保留原失败，核实旧实体配置与所有互返边、202节点/125发电机保持，消费者仅新增1；两网served=required、ratio1，主网当前容量覆盖1549300满负载条件预算。仅重算原持久化证据，不重读游戏或重放动作。完整证明SHA-256 **DC70D36D2F41C23C9EF91639A7251D5E081935581B253234C9F8EB6DA426F9B1**，见[当档日记](./gameplay-timeline.md#2026-09-18--宽带源接入保存与实验室到货通过)及[API依据](./research/game-api-overseer.md#2026-09-18网络容量是动态采样不是静态配置)。
- 边界：没有证明本次72000增量的具体发电来源，也不证明持续供电/燃料；后续仍须fresh容量与原有连续窗口，不能一般性忽略动态异常。采集器另一个manifest字段拼写错误只做离线封存恢复，127条已持久化回复保留，非DTO变化或原生失败。
...
- 修复：仅修改四个既有私有入口的额外实参守卫，用`Get-Variable -ErrorAction SilentlyContinue`读取并排除null，保留显式模式、未知/多余参数拒绝、固定证据及授权范围；不写自动变量，不扩展执行模式。按相同PowerShell/`-File`方式跑纯入口及原函数导入链，确认0游戏调用通过；276送料、83风机、20备料及21共享客户端离线检查通过。之后真实运行进入首次原生连接预检并被TooSkew拒绝，40调用、0commit；这证明入口不再挡住合法调用，不证明连接或新增施工成功，见[本档记录](./gameplay-timeline.md#2026-09-18--宽带真实尾端附件拒绝入口修复未产生新施工)。
- 复验：入口参数、shell版本、调用方式或导入链变化时先做同形态零游戏调用smoke，不把普通纯函数回归当启动路径覆盖；关联EXP-299/303。已接受动作仍按原action核销，禁止借修入口重放。

## IFX-159 — 临时路径投影错误锚点漏掉中段障碍

- 状态：`fixed`限私有几何实现与原证据回归；替代Move/后续施工尚未通过，非Plugin的移动或地表采样错误。
- 根因：Sol未落盘的inline计算用了`AP=entity-target`，而投影向量为`AB=target-origin`，实际比例被减1后由中段筛选排除。原生预览全干为真，但root以完整4930场景独立发现3050中心距线段1.073672m、3047距2.148669m，原“中段无对象”结论撤销；目标在commit前拒绝，无实测碰撞、回档或成功部分修改。
- 修复/证据：主会话把正确origin锚点、有限段Clamp、显式参数和有限坐标检查放进既有私有移动辅助`Roundtrip-GetSegmentProximity`，不另建执行/寻路框架。新增9项回归包含非零origin的中段障碍、前后端Clamp、反向/平移不变及非法输入；66风机/几何和20备料检查均0游戏调用通过。修正函数对原4930实体重算精确复现独立反例，详见[存档日记](./gameplay-timeline.md#2026-09-17--短移地表正例不等于路径畅通投影错误在提交前拦截)。不增加公共工具或将中心距当作原生碰撞体；后续仍须不同候选的完整现场、地表、fresh prepare和真正Walk终态，关联EXP-236/299/303。
...
- 事实：紫糖首段纸面尾点距旧1403为0.472256m，纸面Core检查通过，但fresh prepare拒绝point13/1403，0commit。适配器`ResolveFreeBeltStart`和显式`PathEnd`都先调用`Snap(onTerrain:true)`，然后才调用`SnapLineNonAlloc`；选择geodesic不取消这个顺序。不能把未吸附的球面插值或纸面点序号直接当实际路径。
- 处理：退役该候选，保留旧线、电塔及材料。包内/MCP同源playbook明确先计入两端吸附，再估算整路占位、点数、材料和附件；完整fresh原生路径仍是施工前提。失败未返回路径时不得补造实际点，也不能重试原目标、删端点或改旧线绕过拒绝。
- 验证：3条新增嵌入资源回归及原4条格线指南检查通过；MCP测试项目Release构建0警告/0错误。此前66项Core占位/路由检查及[真实负例](./gameplay-timeline.md#2026-09-17--紫糖首段带路占位拒绝原生端点吸附边界复核)保留。指南为源码态，未热替换安装程序集；不据文本测试声称改线、持续供料或0.4验收已通过。

## IFX-156 — 规划计算未完成被误报为全路清场

- 状态：`mitigated`，限本次私有纳米管纸面规划；不是Plugin漏检、原生建造失败或已施工对象损坏。
- 事实：全厂嵌套PowerShell距离计算没有取得终态成功结果，Agent却报告54点无重叠。随后缩尾检查先发现1395，root复用当前Core策略独立重算53点/4858实体，最早冲突实际为point19/1931；候选没有进入游戏prepare/commit，accepted保持0。
- 处理：撤回清场结论并否决整条候选；不把无输出、观察超时、退出异常或部分局部搜索当成全路径通过。局部几何应直接复用已测试的Core空间索引策略，返回明确布尔结论与首个冲突；必须保留精确完整路径、场景身份、策略版本和终态证据，最终仍fresh原生预检。已有20点私有切片不等于产品native_grid的256点上限，也不应据此凭空增加中继。
- 验证：同一已审计场景的Core实际返回`belt_path_existing_overlap`；81项占位、路径模式与端点几何相关测试通过，保护证据见[本档纠正](./gameplay-timeline.md#2026-09-17--纳米管候选纸面清场误报纠正旧带冲突未施工)。这只证明旧候选不能使用，不证明替代路线、地形、碰撞或供电通过；未放宽任何规则，也未新增公共工具。

## IFX-155 — 固定赤道格点模型误判跨纬带输送路线

- 状态：`fixed`，限当前200m星球、局部等高free-ground的私有纸面模型；不修改Plugin、原生碰撞规则或放行任意布局。
- 事实：纸面28点与旧带最小距离0.539m，实际预检却在point13/3767拒绝。当前DLL的`PlanetGrid`会按纬度切换经度格数、吸附、按0.6/0.99步进及去重；固定经纬都除1000不成立。按源码重构得到26点，point13距3767仅0.00000874m；之前负例未返回完整路径，此根因来自随后源码重构与新正例，不补造旧回复。
- 修复：保留旧横线，采用六条独立路径和一个额外三格跨线附件。首段先被真实`OutOfReach`拒绝；6m接近点的末段浅水仅预检，未提交。改为完整干地3.5m目标，一次Move成功且稳定Walk后重新预检。
- 验证：94项私有路径/边界纯检查、7项短Move地表检查通过；六段12/12/4/12/3/5共48个NEW点全部得到`full_path_stage1`，原生点与模型最大差小于0.000024m。未施工，七个附件的原生合法性、实际供电及持续供流仍待；极区、跨半球、不同高度和source-cover不属于模型证明范围。包内指南加入纬带/距离/地表证据边界，见[本档正例](./gameplay-timeline.md#2026-09-17--修正跨纬带路线并完成短移和六段原生预检)。

## IFX-154 — 完工后无人机未闲置导致调用方停止并误报零提交

- 2026-09-18显式坡A复现：working3/pending0再次让私有入口在A成功后停止；本次执行报告正确保留accepted3→4，没有误报零写。现有包内指南本已禁止把全idle加到每次普通build/save，根因是新入口未复用该经验。root核原12实体/材料/终态，再只续B/save；返航正例和pending负例、保存69923507及5030实体阶段审计通过，见[两坡记录](./gameplay-timeline.md#2026-09-18--两段原生高架坡道施工保存与独立核验通过)。未修改Plugin准入或增加公共工具，不把局部caller修正称为全仓此类问题根治。
- 状态：`mitigated`，只涉及私有执行器与执行报告；Plugin建造结果正确，无公共协议变更。
- 事实：lab4743正常建成、材料已扣，随后玩家仍working drone1但无待施工/维修目标，严格idle守卫抛错；执行报告误写为初始失败且零commit。root读取原始receipt/terminal/stopped证明accepted6→7，并fresh核销唯一实体、整个背包和原动作终态；无人机随后全部idle。不能从脚本exit或最后一条异常推断此前没有游戏写入。
- 处理：保留原caller与证据，退役建造入口，不重播成功前缀。后缀执行只允许在R10/accepted7/旧primary、4743空配置及同一证明下fresh配置r55并保存；完整九动作审计前不归零。后续通用工序如需等待无人机，最多做有界只读settle，并且须先证明原建造已终态、无待施工目标及其余玩家条件；不是重建或忽略不明动作。
- 验证：原31调用与4次fresh read已核，11个后缀边界/坏入口纯检查、解析及21个共享客户端检查通过，后缀尚未实机。原保存64052125不覆盖建造64280269；不得正常重启回退到该旧点。证据与限制见[本档建成核销](./gameplay-timeline.md#2026-09-17--紫糖实验室建成后核销成功前缀不重建)。
- 后续本机复验：仅配置4743及正常保存两动作成功，primary64351298覆盖建造；root完整九动作/4743实体/9274互返边/玩家净0与durable J89核验通过，未重建或重扣料。本次后缀恢复已闭合；未增加自动重试、自动寻路或一般无人机等待机制。见[后缀与阶段审计](./gameplay-timeline.md#2026-09-17--紫糖实验室配置保存与九动作完整审计通过)。

## IFX-153 — 私有施工守卫把瞬时容量和覆盖代理误作真实阶段预算

- 状态：`fixed`，仅铜接线私有调用方；Plugin/Foundry返回正确，未改公共协议或游戏规则。修正版在新fresh容量截面正常接线/保存并九写全审通过；原低容量/代理false分支只作原回执离线回归，不冒称再次实机命中。
- 事实：双sorter原生预检通过后，首次执行在首个prepare前停止。保护原流43次读取、0prepare/commit，R48/accepted6不变；63867728电网容量1548000，10tick后Foundry同网容量1534948、既有满基础负载1532000。4500 J/t假想设备预算缺1552，但实际两只分拣器合计600，仍余2348。旧“1548000稳定下界”被实读反证；不从当前发电输出猜是哪台燃料机造成变化。
- 修复：每步fresh覆盖、网身份和既有满基础负载仍必需，按完整剩余阶段比较容量至少1532600，而不是旧容量常数或较大代理需求。第二步既有负载1532300加剩余300，建完后实读1532600/剩余0；核消费者463、供需相等和serve1。分别记录代理4500的真实结果与实际600预算，不把代理失败改写为成功，不因不足循环等待或用即时低需求代替完整基础负载。
- 验证：9个纯预算正负例覆盖当刻2348余量、少1拒绝、精确边界、分步/完工、错误网/基载/负数/代理；17路径回归及解析通过，原失败Foundry回复重验保留代理false而实际余2348，均0新游戏调用。原43回复/零写已核销；修正后4741/4742正常建成，消费者463/实读基载1532600/满供通过，normal save63905825及4742实体全审通过。持续供电与燃料仍另验。见[本档负例](./gameplay-timeline.md#2026-09-17--铜接线在首写前被私有供电预算守卫误阻断)、[接线正例](./gameplay-timeline.md#2026-09-17--铜接通微晶与处理器复产九写全审及保存通过)及经验账本。
...
- 状态：`fixed`，仅私有原记录审计器及下述一次原流复验。单仓游戏施工已经成功，此项不是游戏超时、失败、回档或再次施工。
- 事实：Luna55290C46/PTY42373 exit0的原流包含第一轮50个action响应，首末相差29.8886524秒，随后一组prebuild/session，再27个action响应（含两成功终态）。原caller按29.5秒或60次观察界限退出一轮；83CC4494审计器移除sleep后却仍用新Stopwatch，可能错误要求继续读取action。该旧审计器未执行本次原流，不能虚构一次实机或审计失败。
- 修正：4F0D63C8只对原Wait的墙钟break做第四处唯一AST替换，使用原record分组；轮数1–8、每轮1–60且连续，提前转进度读取必须证明原首末响应至少29.5秒，或已经到60次上限。该时差是保守下界，证据不够即停止。原Action/Progress、同handle、终态、意图、完整现场和结果比较保持，不改原raw、不生成游戏调用，不重演墙钟节奏。
- 验证：29离线检查/PTY50881 exit0、三坏入口import前exit1/解析0及Sol静态复核通过；4F0D63C8/PTY13993 exit0完整重验226文件/223响应和全部结果，独立封存D63421DE，manifest未变、0新游戏调用。游戏caller5529保持不变，末62017248/R143/primary61814274/J89/accepted9；新4680为空且未连接，连接、持续产出和该链保存恢复仍待。关联EXP-299和本档日记。

## IFX-150 — 仓位预检草稿把纸面坐标误当原生最终落点

- 2026-09-17同类边界后继：Foundry机器预览先吸附、再以机器位置评估功率；实际纳米管功率代理有0.326130890m偏移，不能称为任意slot点的精确功率探针。已撤回这层等同解释，包内指南和定向回归同步，6项指南及40项供电策略通过；具体原回执与证明见[本档记录](./gameplay-timeline.md#2026-09-17--纳米管补电预览区分请求点与真实评估点)。只修解释和指南，不改Plugin；新指南未部署，补塔/实际分拣供电仍待，不扩大原仓位调用方的fixed范围。

- 状态：`fixed`，仅私有仓位预检调用方；正常零写路径已本机复验，拒绝分支仍为离线，无Plugin改动。
- 本机复验：ccac2cb/CI35087534806绿后，Luna C50/raw-8a155208/PTY99440 exit0完成137调用/1prepare/0commit，真实吸附偏移0.506243m；root5118F2CB/PTY17415 exit0核139原文件/137响应及完整结果，独立封存293E979E，manifest不变。末61957615/R141/primary61814274/J89/accepted8，实体/拓扑和建材保留，不外推施工、实际端口、容量或持续生产。
- 根因：草稿要求plannedPosition与请求坐标相距不超过0.001m，但当前`TryPrepareCoreBuild`明确先执行`planet.aux.Snap(requested, onTerrain: true)`。纸面几何不是原生吸附坐标，更不是尚未建成仓库的端口证据；也不能把预检期间玩家hash变化直接归因于prepare。
- 修正：C50F1B21仅一固定候选/1prepare/0commit，分别记录请求点、原生返回点与实际偏移；验证有限坐标、200.0–200.4局部半径、当前79.8m范围及2m外部偏移界限，后者不是原生保证。建造和端口仍需后续fresh核验；玩家变化一律停止且原因未知，不扩大候选或施工权限。
- 验证：26基础、root6E015C81完整18场景/16guard（PTY79977 exit0）、双方六入口/解析及21公共客户端通过；5118F2CB审计器18离线（PTY56932 exit0）与Sol静态复核通过。0真实游戏/证据写入；通过预检不等于施工、容量或持续产出通过。关联EXP-299；当时实机为61924904/R141/primary61814274/J89/accepted8，后继本机复验见上。

## IFX-149 — 材料预检把目录能力误作唯一生产者，并硬编码仓储堆叠上限

- 状态：`fixed`，仅私有材料预检调用方；正常路径已本机复验，拒绝分支为离线证据。不修改Plugin、既有设备或已保存前缀。
- 本机复验：e89924e/CI35079917529绿后，Luna5485158B/raw-2a388539/PTY35136 exit0完成79调用/3prepare/0commit；rootD72190F6/PTY90011 exit0独立核81原文件并封存DE6F9A5C，manifest不变。末61876601/R134/primary61814274/J89/accepted3，三源/玩家hash稳定、空26格大于三项所需3格；未取料或施工，全部token丢弃。23审计离线和三入口/解析另过，不以预检通过声称落点、容量上界或持续产出通过。
- 根因：草稿要求1113目录只有recipe37，但封存运行目录实际还有已解锁的recipe62；目录中的可用配方不等于现场正在生产的设备。另一处把所有仓储行限定100，误拒合法200堆叠电路板。列表的beltCargo又是detail-only/null，不能据整厂列表算出完整带货上界。
- 修正：5485158B按运行目录37/62与完整现场配置相交证明当前唯一101/r37，按每物品stackSize检查混合材料仓，联合检查三种材料所需背包空格；容量上界明确unknown。三次transfer只prepare且全部丢弃，后续必须fresh；自然仓储流动与source/player hash是否变化分别报告，不声称prepare因果或库存冻结。
- 验证：27基础（PTY4194 exit0）、root9CB0B46E的15个真实Lbp-Run模拟场景及8项guard（PTY7053 exit0）、四坏入口/解析和21公共ActionClient检查通过，0游戏/证据写入。完整场景核正常/自然流动与13种具体失败的调用数、prepare数、错误和accepted3保持；额外覆盖200电路板正例及201/string拒绝。旧root DBDACC1/PTY23068因预建筑错误文本期望不符退出1，保留该事实，只修测试期望后整组重跑；5485158B未改。推送绿CI后仅可由Luna做一次零提交预检，不授权取料、施工或生产窗口；这些离线错误不计为物理方案失败，现行Sol三轮升级规则不变。

## IFX-148 — 原记录审计只规范化意图顶层，嵌套request仍为Hashtable

- 后续材料原记录复验：DDA11699/PTY77250 exit0完整核236原文件/224响应、五意图/终态及结果，独立2586CF0E且manifest不变；Luna1C9/PTY25348 exit0、末61924904/R141/primary61814274/J89/accepted8。无新游戏调用或原文件改写，本次只闭合正常备料审计，不外推施工、保存恢复和失败分支实机。
...
- 后继单仓正常路径本机复验：Luna5529/raw-e1a8d554/PTY42373 exit0一次建成4680，root4F0D63C8/PTY13993 exit0核226原文件/223响应与全结果并封存D63421DE；末62017248/R143/primary61814274/J89/accepted9，原raw/manifest不变。只闭合正常施工，不声称过期/replay实机负例、连接、持续产量或保存恢复；原时序分组适配另见IFX-151。

- 2026-09-16 单仓草稿复核：相同边界一度遗漏于新的私有施工caller，尚未实机。55290C46在commit Query及protected intent前重验原expiry；明确拒绝保留accepted8/knownWrites0并清接受不确定性，typed新接受且非replay保留9/1和合法handle，即使后续key异常也不丢已接受前缀；replay/未知flag仍8且停止。38基础、root2B31EADE完整30场景/16guard（PTY11977 exit0）、六入口/解析、21公共ActionClient，以及83CC4494原记录审计器23离线/静态复核通过，不扩大原保存实机结论。旧2D5正例通过后冻结期间被补充修改，旧full-flow在hash门/import前拒绝；以后先通知并解除冻结再改，不把版本漂移算成游戏失败。
...
- 本机复验：7a12ea2/CI35070121802绿后，Luna450F4CA7/raw-64c8fd0b/PTY53670 exit0；88eb0416在61773739完成843 SetBans30→29，同步库存/四边保持、后继正常到仓1113=6。root36428248/PTY3702 exit0独立核146文件/143响应并封存0A498D36，0新游戏调用、原文件未变。末61774204/R133/primary61386568/J89/accepted2，双47页4679/9164与13详情保持，不重放、不保存、不归零；尚无持续产量或恢复结论。
- 根因：旧843基线只有三条连接，当前已经保留新输出4666，实际应核四条；843详情验证返回的是detail/stock包装，最终capture必须取detail。草稿另缺提交前持久化幂等标识，并曾将整个场景仓储配置归一化，可能掩盖非本次允许的过滤变化。
- 修正范围：冻结450F4CA7只允许843的SetBans30→29，filterItemId=0，保留首格1113预约、其他29格及四条原边；双场完整4679实体/9164互返边、13详情、玩家/科研/供电/J89/零prebuild不放宽。commit前先持久化唯一意图，nativeSent在认证后实际发送前计数；收据异常保留原key/handle/accepted，不换键重试。同步终态严格保全库存与连接，后续自然到货另验；最终时间括窗解开843包装、保留原Journal capture，仅归一化本次批准的bans字段。
- 离线证据：30基础检查、root4387AC2D的71独立检查、三坏入口import前exit1/解析0通过；独立审计器36428248的28检查、三坏入口/解析及Sol静态审阅通过。45项公共仓储回归已随d499613通过CI35066037729。19个完整Los-Run场景分组验证：83AC88E1在PTY38124完成前14例后，因第15例预期错误文本不准exit1；生产guard已在正确调用/写入边界拒绝。只修错误期望及测试筛选的3EAD79F0再以PTY98855 exit0通过后五例/10检查，未改mock或冻结caller，不把前一进程改称exit0。全部0游戏/证据写入。
- 完整模拟覆盖两种143调用/唯一模拟写的正例，以及旧三/错误五连接、hash版本、玩家未就绪、原生prepare拒绝、过期计划、post-prepare变化、未知/错key收据、旧terminal、同步库存变化、场景过滤漂移、串料、日记/供电/时间括窗/revision漂移。每个负例锁定实际调用数、prepare/commit、持久化意图、accepted、原handle及不确定性；未知收据不能补造handle或清除意图。
- 测试经验：完整模拟必须使用一致的fresh时间线、值类型终止的有界数据遍历，并断言具体首个拒绝点和精确调用/提交/意图/handle计数，不能把其他早期异常当目标负例通过。旧三连接和新增错误第五连接均应拒绝；实际完整场是先收齐47页再校验，因此关闭场漂移的检测点在第124调用，不是发现该实体对应页的第86调用。
- 当时离线边界：61707511/R131/primary61386568/J89/accepted1，843仍bans30；后继实机见上。该切片始终不搬货、不建造、不移动、不保存/重启/归零，不证明持续1113或保存恢复；关联EXP-299及本档日记。
...
- 状态：`fixed`，仅本批私有十写审计器；离线及当前一次fresh实机审计通过，没有修改 Plugin、游戏规则或已建对象。
- 根因：复用施工后的端口归一化 helper，只能证明其他槽保持，不能替代目标槽当前指向核验；原完整模拟的 wrongSlot 负例未被拒绝。Sol 静态复核另发现 countUnit 未先验证字符串类型，以及无喷涂证据时仍允许正 inc。
- 修正：reader9F8BF372显式核验101/slot1→4678/0，并保留101/slot4→4665/1与843/slot7→4666/0；逐字段检查原生整数和布尔类型。物料单位必须为字符串items，inc必须为整数0。没有取消完整场景、配置或互返边检查。
- 验证：contractCC138D15的66检查、reader173检查和最终完整Tn-Run模拟20检查通过；三坏入口均exit1、解析零错误。完整模拟80读取，0游戏/证据写入且不归零。首轮修正又暴露843单行数组被PowerShell展开，改为显式保留行数组后最终PTY74172 exit0；这是测试捕获的调用方问题，不是原生游戏失败。
- 边界：843保留2900历史高纯硅与一个晶格硅预约格是合法基线，不能误判为串料或已清仓。以上离线结果不替代fresh实机十写审计，也不准许下一个游戏写入。关联EXP-299与本档日记。
- 实机：root9F8BF372/raw-d508903a/PTY2163 exit0，80只读在61602883–61603347通过，82记录封存999ABE20；4678实体/9160边、材料与准确端口保持，0游戏写入、accepted10未归零。不是持续产出或保存恢复验收。
- 后续账本：离线另拒绝数字Hashtable键/JSON顺序比较及把session查询套用action envelope门的错误假设；改为完整物料键集/typed值、精确null查询envelope和真实result session，其余方法不放宽。C0F4637F的31离线/三坏入口通过，0aff551绿CI后唯一PTY68649 exit0生成C29A0AF7，外部accepted10→0，0游戏调用/写入；原82记录不变，不冒充新现场或重启。

## IFX-142 — 早期零连接炉判据误用于已经接好的C输出

- 状态：`fixed`，当前私有H炉端施工caller；2026-09-16对应一写已实机通过，不扩大为持续产出或完整版本通过。
- 症状与根因：完整正例到post-101读回时被La-AssertRecipe101拒绝；该早期Stage-A helper硬要求connections为空，但101当前已经有C输出4665。剥离本次H新边后仍应保留原C边，不能把成功接线当异常。
- 修正：7FFE6284只替换这个旧阶段假设，按当前H基线比较完整配置/端口图、recipe37和缓冲，并保留typed powerServeRatio=1；不改变已建C/H、不放宽材料、原action、双向端点或新ID核验。端点图只还原本次已核销槽的占用三字段，不能替换整个旧数组掩盖多余槽。
- 验证：22基础及root39纯检查、三坏入口/解析、21公共客户端通过；最终7FFE恢复typed满供检查后root PTY35883 exit0/39，A7F536F2全22场景PTY29871 exit0，root核对全部失败原因及计数。正常135调用/一模拟提交，长等待196调用/一模拟提交；负例保留原handle、accepted与未完成读回边界，测试零游戏/证据写入。fixture的返航player、string expiry与错误pendingId预期独立修正，不归因于原生游戏拒绝。
- 实机复验：6cadb45/CI35054040332绿后，Luna 7FFE/raw-3788f356/PTY38811 exit0；4678于61555943正常完成，101原C输出4665保留并新增slot1的H输入，材料2→1。root F72A/PTY55203 exit0核销223原文件/220回复并封存62EB86F9，0新游戏调用；末61556324/R129/accepted10，完整旧场与新增四条互返有向边通过。
- 边界：只接炉端，H源尚未接，未保存、未归零；完整十写fresh审计及交接后才续H源端，持续产出仍待。关联EXP-299、本档日记及IFX-140的阶段判据经验。
...
- 实机后继：26632b5/CI35022424183后，Luna raw-ff312c36的C19确实按waiting→19负ID冻结/现场逐减→19正ID成功完工，61134156→61142640，belt33→14；caller直到读取完19详情及完整47页关闭场景后才遭本地600秒截止，PTY79281 exit1真实保留，不是native失败。root 9F63647F经10离线/三入口/解析及Sol审查，核1135原文件/1132回复后以raw-5e972858补78次有界fresh只读，PTY87748 exit0、末61155160/R120/accepted5，完整4664/9128、库存/三设备/原J88/科研/两网/零prebuild保持。状态`fixed`仅当前C19契约与已接受前缀保全/独立尾审，损坏回执等负例仍只有离线证明；不把原caller改写成passed，不称真实断线已验证。新审计首稿的JSON属性顺序误判仅离线，已改按键集合/类型/字段比对，计划凭据只验脱敏形状，未新增游戏写入或公开接口。

- 首见：2026-09-16，C19私有施工器离线审查。初版完整模拟把成功回执写成queued，并允许零开始tick/空目标；当前Coordinator实际先执行普通预建筑创建，再返回waiting_for_game回执，开始tick为正、目标为完整负ID集合。旧模拟通过不证明该真实路径正确，没有因此执行游戏。
- 修正：仅对本批普通C19绑定当前原生契约，明确waiting的完成tick/after hash为空，before hash等于本次计划；完整负ID保持至终态，现场存活预建筑只可递减并最终替换为19个唯一正ID。复用既有进度观察器，不放宽原生超时，不重试整段。其他动作族仍按各自契约，不能全局禁止queued。
- 同批复验旧EXP-299类型经验：PowerShell动态比较不能替代string/int/bool检查；回执、动作、局部快照、实体和空带货物先验原生类型。收到accepted=true且合法UUID后先保留handle和accepted前缀，再验证其余回执；后置字段错误仍保持不确定并停止，不能在停止摘要中丢掉已接受身份或换键重放。
- 验证：冻结调用方EF7873DC、模拟器4EEF94F2由root实际运行31场景/66检查，PTY99632正常exit0；覆盖19→10→0预建筑、原handle终态、整场4664实体/9128互返边、模糊/损坏已接受回执、类型冒充、实体重复/倒序/缺失、陈旧快照、库存错误及停止后不重放。另13基础、五坏入口、两解析和21公共action-client通过，Sol独立只读审查无C19阻塞；均零游戏调用。初版夹具遗漏端口capture tick的exit1、旧queued合成正例被本轮否决的事实保留，不能把旧模拟通过冒充新契约证明。
- 此前离线截面（上方实机后继之前）：状态`fixed_offline_live_pending`，只修改私有调用方/模拟器；源码Plugin/MCP、原生创建路径、正常材料和无人机规则均未改。当时只授权fresh预检后提交一次C19，尚无新施工或产出实证，不计物理布局设计失败轮次。后续以本条开头的实机范围为准，仍不外推持续产出。关联EXP-299、IFX-139。

## IFX-139 — 私有读取客户端只有连接超时，不能支撑观察的响应截止声明

- 2026-09-16调用方后继，仍离线：A92D3F66的有限七动作复用04CE881B中已有Core framing/envelope函数，另由精确阶段白名单控制请求；每请求≤30秒、总240秒，最多240次调用。提交前保护记录原意图，返回后先保留原回复，未取得接受回执明确标不确定，不更换幂等键；接受后未终态也只保留原handle。97项调用方检查和87项/15场景纯模拟通过，不代表写请求超时已在游戏中验证，不改变旧reader或公共MCP客户端。

- 本机后继：fa0c719绿CI后，最终E57ABD45经74离线/四坏入口/解析复验，由Luna唯一执行raw-a10a7ebc，29次真实请求/回复正常，60466787→60466970/R102/primary60175189/J88/accepted3、0写。原exit1为3052输出余量48不足声明100的正确开场拒绝；完整预约功率余40100满足，非传输或供电故障。root以原29回复重核exit0，0新调用；不覆盖原exit1。该后继只验证正常本机读取，超时取消仍为离线假管道证据，未进入12窗，不能宣布持续生产通过。

- 首见/范围：2026-09-15，root审查Sol的持续CNT方案时发现既有本地Bridge脚本只对Connect设10秒限制，后续同步Stream.Read没有响应截止；循环前后检查Stopwatch不能取消已经阻塞的读取。没有因此发生新的游戏调用或把先前已完成动作改判失败；产品MCP客户端已有异步取消，本缺口限旧私有调用路径。
- 修复/状态：`fixed_offline_live_pending`，限04CE881B任务专用只读reader。复用现有Core FrameCodec及MCP的异步管道/Identification/统一CancellationToken模式，连接、握手、帧写读共享每请求≤30秒且不超过本轮剩余240秒的截止；仅十个既定只读方法、最多248次尝试，原回复先保护落盘再解释，超时停止，不换连接重试或重放游戏动作。57E11607在每次sleep也按剩余预算截断；不改公共CLI/MCP工具或已安装Plugin。
- 验证：51项reader检查含真实本地假管道连接后不响应、150ms取消和单DTO输出；74项调用方检查、91项Sol纯判据、四坏入口/解析通过。原31 proof、311六信号/供电及c678最后原生probe四正例通过，后者真实R99/tick60174122而非R101，未改写DTO。全部零游戏调用；调用方observe入口仍关闭，实际12窗待验。
- 检查器自身修订：异步VoidTaskResult需显式丢弃，不能混进PowerShell返回DTO。首个双工测试夹具同步先写后读挂起，被终止并保留exit1；改为有截止的并行I/O后通过，非游戏卡死。root的19对象草稿还误把既有3667当2011、把872的null过滤当物料0；真实全场证明3667为2012、872为未设filter的869→871输出，现只按已证身份/配方输出核货物、不改设备。Sol复核补齐proof/catalog严格类型与真实间隙计算，避免字符串数字转换冒充证据。关联EXP-299。

## IFX-138 — 保存 helper 更新新主档后，外层适配器仍读取旧缓存

- 2026-09-16调用方后继，仍离线：A92D3F66先以保存原terminal和显式新primary完整核验session，再同步laPrimary、taskExpectedSaveTick和revision，嵌套断言不读取旧外层缓存。97项调用方检查含旧tick/错primary/错revision/unknown/restricted不采用新边界；34FB2478整条七步模拟及root复跑证明R102→112正常保存和准确停止前缀。原09EDD665/原exit1不改写，此修复尚未发生新的实机save。
...
- 验证：219项调用方离线检查，另报告47项干净bootstrap及85个闭包函数；root独立64项真实导入/原终态回放和18项纯入口/解析检查通过。独立源阶段原记录核验器43项离线通过，83项最终状态与42项原生准入检查只证明原记录/合成反例解释，尚无新两源实机结果。所有检查零游戏调用/写入，不把测试数量相加宣称独立用例总数。
- 测试自身修订：root曾误在命令AST而非字面表达式上调用SafeGetValue，并枚举无参数定义的null；均在离线退出后修正。原记录审计范围也明确采用9个S1及4个下游分拣器，不能把ID夹在中间的风机4639当作分拣器。保留这些测试边界，不把测试器异常写成游戏失败。
- 后续许可：仅在独立切片push及绿CI后，由Luna以新prepare、实际位置供电和节点夹读执行871→4505、3622→4574两源及一次正常save，accepted0→3保留；原/fresh同handle终态、逐端互返及扣料均须核验。仍不重放成功对象，不移动、重启、归零或宣称持续CNT/燃料/紫糖供应。没有Plugin/MCP接口或安装变化；关联EXP-299。
...
- 独立实机核销：root02415F3F通过18离线/解析/三坏入口，raw-fcabfed3一次11读、PTY4813正常exit0；同原action及4640、两端真实slot4/4611与4623旧边、过滤1106/net3满供/两带空载保持。全玩家仅一只sorter净减、settled、科研/J88、两网成员和零prebuild保持。末59820632/R90/primary59372664/accepted6，未保存/reset/重启/扩大施工，未重扫完整post-build工厂。
- 首链核销时状态：`mitigated`仅成功prefix核销；随后顺序修复的离线结果见本条当前修复，实机仍待。只对原/fresh终态和新详情已证的单一ID建立临时两端核验范围，再严格双向slot/旧边/配方过滤供电验证后固化已建；不提前增加built，不泛化容忍未知新边。下一调用方只续后三建一存（6→10），既有4640/风机/S1不得重做；旧v3/参数数据不再用于live。持续供应与正常保存仍未验收。关联EXP-299。

## IFX-135 — 把发电设备含实时缓冲的配置哈希当成静态几何证据

- 当前状态：`fixed_live_preflight`仅私有v3/72E822DD的四点预检。ac9a77f/CI34956182216绿后，Luna raw-87ccd681正常exit0，155原成功回复、零写；root193A37CD/raw-0807c7e7独立原记录核验exit0，无补游戏读取。两次47页4639/9068全场和四组166节点夹读、四个实际plan/probe均通过，每组86个合法实时发电变化，静态配置/节点几何不变。最小覆盖余量0.307769m，全六sorter剩余1800 J/t含未来两源600仍余4100；完整4500探针余1400/缺口0。末59785219/R88/primary59372664/J88/accepted5，未施工/保存/打开两源。此项不关闭持续供给；6070cf8运行安装尚未冷部署新指南。
- 后继调用方v3/72E822DD的离线截面：已完成精确矿竭基线续接，见IFX-119；104项调用方及33项独立核验器离线通过，实时buffer、几何和功率规则不改。当时仍为`fixed_offline_live_pending`，不能用矿竭适配的离线结果核销；随后实机结果见本条当前状态。
- 新现场边界：7717252/CI34952526379绿后，Luna仅运行一次82EC3592的v2 preview，原raw-705ad182真实exit1发生在开场第一页的86.resourceNodeIds变化，尚未到达任何四点prepare/probe或本条节点夹读。root已独立核清末个石矿点耗尽，见IFX-119后继复验；不能把这次零写前置拒绝说成IFX-135实机已经通过，也不能重跑旧脚本。
- 私有调用方修复：Sol冻结v2/82EC3592，root独立97项调用方、27项节点/几何、13项原记录预览核验及4项纯函数缺字段回归全部正常exit0，解析零错误、八个坏入口均在导入前拒绝。只允许唯一正确单位的实时发电count变化，两个派生hash必须相应变化；其余完整字段、纯节点hash、四个独立快照的前/probe/后时序和原几何证明保持。原940375B5不改、不再用于live；公共hash/版本、动作和原生准入不变。当前状态`fixed_offline_live_pending`，只准推送绿CI后进行新的四点零写预览，不是施工许可。
- 审查补漏：root纯函数反例证明，两边同时缺真实count且hash不变时，PowerShell scalar Count=1会被早期v2误接受。最终版先通过PSObject.Properties核对精确七字段及非null值，再核数值，保留合法空name。独立4项回归特意保持两边hash相同，避免由不相关的hash差异掩盖缺字段问题。root初次串行测试在27项已返回后因严格模式下未设置LASTEXITCODE导致外层exit1；改为独立进程运行后27/13均正常exit0，原失败保留，不涉及游戏调用。
- 指南产品化（早于本次私有修复）：根因2471be7/CI34949557238已推送并绿；现已把buffer敏感哈希、完整前/probe/后静态证据、唯一合法实时发电count变化及禁止改写hash/复用token写入MCP嵌入的agent-playbook。新资源测试与既有发电单位/无人机边界共3项通过，MCP测试项目Release build零警告错误；仅源码/嵌入资源离线通过，安装6070cf8和原71120字符指南未冷部署更新，工具/资源数量及协议不变。
...
- 首见后的历史状态：`open`。当时Sol仅提出窄修正，尚未改执行器或重试；现在的离线修复边界见上。旧F55/54和940375B5的跨tick哈希相等假设不适用于运行发电机，其live用途保持冻结；原生几何公式本身未被否定。
...
- 边界：本次只证明备料来源和原生配方可观察；不补造风机落点、材料转移、施工或供电结果，原exit1保留。显示/汇总失败时优先重建原证据，不能重放已完成调用来美化退出状态。关联EXP-299与本档日记。
- 独立审核补充：Sol确认四步作用域/净量/计数后提出session时间字段应改名；root以真实750fffe5及SessionState契约反证，该DTO确为gameTick，不能套用玩家DTO的capturedAtGameTick。保留正确字段，并加真实正例及缺失/null/旧tick/错字段负例进入上述89项；未按误报改坏调用方，也不把审核意见计作一次实机方案失败。

## IFX-133 — 分拣器物理槽与Foundry探针设备落点被假定为同一点

- 第3轮有限修复方向：先正常备一台2203，01AE4CFF只允许三transfer与r7一次、accepted0→4后冻结；当前仍R81/primary59372664/J88/accepted0。新增风机须以后单独选址、原生预检和真实并网；5000 J/t额定只支持相对已观察低容量的4100余量草案，不把火电自行恢复当修复，不改S1/燃料成功部分。原调用方覆盖等价的实建验收仍未关闭。
- 有限诊断结果：56b525b绿CI后raw-1d829c14与root原记录审计raw-9e515434均正常exit0，20只读/零写，59532008–59532034/R81/primary59372664/J88开场/accepted0。capacity自行恢复1565000、3060正发电2507，而3965仍H58/G0、抽样G进料与氢仓3065为空；不能称持续供料已修复或直接燃料库存已知。原部署与所有成功对象未改，IFX-133完整修订预览/实建仍待；本次不是第三次S2重试，Sol仍在第3轮方案设计中。
- 后续诊断准备：Sol第3轮不重跑S2，按原全场把燃料检查缩到12对象/20只读，由root实现F4658678、88专用及继承/解析/4入口通过。3060是九火电正输出集合减少的唯一成员，但其0 J/t不能直接证明单机容量或燃料耗尽；原生发电受顺序分配限制。3065为仓而非带的类型假设在离线审查中已纠正，无错误游戏调用。最新实际仍是d4/59491869、accepted0；新只读检查待独立授权，完整S2修订预览/consumer施工与持续供料门均未关闭。
- 第2轮现场边界：b980f6b绿CI后唯一raw-d4e9080e只补index5；原生放置及165节点前侧几何通过，未复发同点断言，随后因当前满负载缺900 J/t而exit1。root72CC8C88/raw-11891258正常exit0、12专用/继承/解析/3入口确认64原响应及零写；网3容量1529000、预约1528100、S2需求1800，不应以瞬时满供放行。开场47页4638/9068保持，原R81/primary59372664/J88/accepted0；probe59491869，后侧节点及结尾未读，完整修订预览和consumer实建仍未证明。此为新增实际供电blocker，不是native build失败；发电容量下降来源未诊断，按EXP-290处理，先记录结论再交Sol第3轮设计，不重放S1或失败预览。
...
- 根因复核：当前Foundry会先Snap制造台再计算供电，而普通sorter的native consumer以实际entity.pos投影到供电球面。Sol第2轮提出完整覆盖节点集合等价证明；root独立504FB195/raw-ee11bfc3以原S1全场165节点和d723五探针复算，第五P/Q均由875/3753覆盖，分别留1.883685/1.701861m保守余量，原0.257892m偏移保留。三座目录外2103/2104由原生componentKind严格station及分支顺序证明非节点，未知类型不能默认排除。22专用、40 Core、解析/4坏入口通过，零游戏调用。状态仍`open`：不同tick的既有场景复放不是fresh现场；第2轮生产脚本修订和新现场检查未完成，不能据此宣称施工已放行。精确原生规则见game-api-foundry。
...
- 修复/状态：`fixed`限私有只读审计。抽出实际生产用来源校验，显式按`Measure-Object -Property count -Sum`统计，并独立检查单位/inc/物品/范围。原native计划不重跑，执行器2E30773D不变；未修改Plugin/MCP、物资或游戏状态。
- 验证/边界：原预检DTO的30格分别合计3000钛和2916石墨烯通过；保持格数但改为不足量的反例被拒绝。107审计（另45/50继承）及28计数辅助offline、入口/解析通过；root raw-5e2a5a5b原记录复放通过，零新增游戏调用。它验证预检证据解释，不证明预约已执行或长期供给；初审exit1不改写为成功，后续数量统计遇到集合属性同名时复验。

## IFX-131 — 私有备料预检把磁铁ID误标为铜

- 首见/根因：2026-09-14，raw-221841da先成功prepare铁、未commit，随后用1102查询“铜”。来源仅8件磁铁，本地保留库存检查拒绝；不是游戏铜供给缺口，未发送第二prepare，没有游戏写入。
- 修复/状态：`fixed`限调用方。按当前原生物料证据固定铜1104、铁1101及两来源，缩为148铁/6铜的明确元件预算；请求守卫检查ID/数量/方向/哈希并拒绝1102替代。未改Plugin/MCP、未更改运行时物品或放宽保留量。
- 验证：36离线、4入口及21客户端检查；修正后的fresh预检和Luna raw-dee8cd2a真实两次转移、root raw-0a4a54e7独立复读通过，来源即时−148/−6、玩家对应增加，57485177/accepted9/primary57449785/J88。尚未施工/保存；不能把备料闭合外推为化工输送可行。EXP-301记录目录身份规则与复验触发。
- 后续预算复核：raw-e36b08f5显示磁铁recipe2输入1001铁矿，不能把剩余铁块当可递归制作磁铁的材料。完整输送包改为144铁块/6铜块/4磁铁，已取148铁块留下4件余量、仍须补4磁铁；实际电路板recipe50×4已沿原生直接材料正常完成。没有错误手搓或注入，EXP-296/301同步更正；其他配方不得类推。
...
- 修复/状态：`fixed`限当前DLL的原生游标转换，完整施工另验。在相同位置补调用原生SnapTropic，仅改工具自己的一点预览数组；不修改玩家、地形、库存、蓝图代码或原生拒绝条件。原有输入/分区限额约束扫描工作量，返回姿态仍进入范围、碰撞、科技、整图预算及审批哈希。
...
- 本机复验：6070cf8同批228文件冷部署，安装MCP64/1/71120字符验证；原主档57243616通过protected resume重存57243648，root raw-11294d8e/57254850核全4386实体/8598互返边、38详情、全玩家/16风机、原J88、两网和完整科技状态保持。raw-b5ef2d8b/57258404使用原导出代码、原请求锚点/方向，16对象由NotAlignTropicAnchor全部变为Ok，实际姿态修正约0.506–0.566m；仍有12个范围阻挡、两处占位，nativeCheckPerformed=false，没有施工。raw-f9b37280/57261463单风机/单分区导出在原位置转换零偏差、Ok，同时准确报告2972占位。
- 边界：离线测试本身未运行Unity，上述独立只读实机证据只核销原生对齐缺失；Ok不等于最终原生建设检查完成，不能忽略范围/占位或宣称16风机已发电。下一按修正后的姿态规划空地及必要接近，不改动完成实体、不重放备料/移动。关联EXP-300与game-api-foundry。

## IFX-129 — PowerShell Clamp整数重载把线段内部最近点错投到端点

- 首见：2026-09-13。Luna raw-ca2863a8已成功完成五段Move及两笔取料，返程在原生prepare前因非带实体中心净空小于3m而退出；accepted8、没有返程动作或保存，不是卡住后的Move失败。
- 根因/更正：03ad6fe初步把2.76973m/fraction0当作整个返回段的最近点，这个解释已被独立复算反证。私有PowerShell函数的动态投影表达式传给Clamp(value,0,1)时选中Int32重载，将真实0.094840445舍入到0。正确Double结果是距仓762为0.721942m、距761为1.749804m，最近点均在返程内部。原2.76973m只是起点到仓762的距离；不能据旧字段批准起点豁免，更不能声称原生碰撞体已经观察。
- 修正/状态：`fixed`限私有线段投影，全部参数显式Double；本次两段有限实机绕行证据见后，不外推通用寻路。补中点最近对象排序、两端裁剪、本次精确反例，纠正首段旧离线预期；27+95预检及25+122审计离线、三文件解析、原执行脚本导入前退役拒绝通过。初始附加重载反例对首参数显式定型，已经改变了错误调用的语义，按真实动态变量重做后通过；无游戏副作用。公开PowerShell未发现该调用，Plugin/MCP及3m门、地表规则、状态哈希不变。后续仍使用更正几何和fresh现场设计有限路径，不修改成功工厂或重放七动作。
- 证据：root raw-da99d4a3在54947167核七原始/fresh成功终态、4225配置/8282互返边/18详情、完整玩家和原J86/两网；84石墨/12钛真实在背包，revision193、primary54777500，零预建筑。43+118有限执行器、25+118只读审计离线及解析/三入口拒绝通过，实际返程/新保存仍待。关联EXP-007/057/135/185/246/253/299。
- 更正后复验：raw-70788080于54978643重新核上述七终态、全配置/拓扑、玩家与日记/两网，exit0、accepted8不归零；旧raw几何字段保留但不再使用。没有返程prepare/commit或新save，不能把辅助计算修复冒充实际绕行通过。
- 有限侧移正例：84石墨/12钛保存55002268后，root仅选择严格远离762的约4m切向候选；raw-a2b300f6完整native地水预检通过，Luna raw-813d8156单次Move终态55066739成功、Walk/速度0、原材料保持。root raw-f0bbd87f于55088826独立核本批十终态、4225配置/8282互返边/28详情及玩家/J86/两网；12+173、13+185、26+198私有离线及解析/入口拒绝通过。此正例仅关闭该精确离仓短移，完整返程仍待；旧穿仓返回线、全局起点豁免和自动寻路均未获批准。
- 完整有限返程正例：从新的实际离仓位置，raw-83d18e6f预检28.382m全干地；正确浮点全段距762/761为3.56258/3.64942m，不是对旧起点的豁免。Luna raw-bc1771d8的唯一返程Move终态55227985成功，随后正常完成既有化工分支的有限加工并保存三聚变成品55240705。root raw-c39cff49于55253680核完整十终态、4225配置/8282互返边/27详情、玩家/原J86/两网；16+238、45+254、25+299离线及解析/入口拒绝通过。此后本次返程不再待验，原始失败/错误字段仍保留，旧起点穿仓路径仍禁止；不据此宣布自动寻路、发电容量或持续燃料通过。

## IFX-128 — 私有审计误把相关详情子集当成完整对象基线

- 首见：2026-09-13。六步备料已成功，root raw-9c0b5a50逐笔复核终态后，在比较3080电塔时读取空基线的objectId而exit1；没有新的游戏写入或实体消失。
- 根因：调用方仅从上次445个inspect详情建索引，误以为包含本次12个目标；3080实际只在同次完整4194实体快照中。通用配置本就由list/inspect共享，缺少一次可选详情不能解释为对象缺失。
- 修正：仅私有只读审计改用完整快照作精确身份/配置基线；读取字段前检查目标存在且ID相符，已有详情必须与基线一致。禁止跳过缺失目标、以空对象默认通过或重放已成功的六步。未改变Plugin/MCP、位置容差或游戏动作语义。
- 验证/状态：`fixed`限本次基线选择。28+60离线及解析/3入口拒绝通过，包含真实3080缺详情正例和缺对象/null/错误身份/冲突详情负例。后续位置守卫另检测批次外7.2943m变化，保留该次exit1，独立确认当前位置稳定及全玩家/工厂不变；不是放宽守卫来通过原失败。最终raw-b34a2c0c于54366418核六终态、4194配置/8220互返边/12详情、原J85和两网，exit0、accepted6不归零。原失败、位置变化来源未知及一次DTO汇总错误均留在存档日记；保存恢复和持续供给仍待。关联EXP-007/018/039/185/299。
...
- 验证/状态：`fixed`限私有调用方计数和只读后缀。34+12原断言/继承、5项后缀检查及解析/3入口拒绝通过；首次复用公共断言时合成fixture漏destinationObjectId导致offline失败、零游戏调用，补齐真实DTO形状后通过。root raw-0427d825于54105240核六相关详情、完整玩家/durable J85/两网及零预建筑，accepted8/revision149/primary53983928保持，原exit1不改写。随后Luna raw-e88c5aab的两段原生形状预检均成功、exit0，root核三份原始计划和12端点前后详情及完整边界，末读54129830；51项私有离线及21项action-client检查通过。没有游戏commit，未来真实父带续接、材料、新入口、实际消耗和持续供给仍待，不能将只读后缀闭合当作施工完成。关联EXP-007/188/219/299。

## IFX-126 — 调用方把一次配方配置误算为一次revision递增

- 2026-09-15钛后继实机：`fixed`限当前私有执行器。raw-9268e07d九次普通build后，真实session59024064为R46/primary58959632；唯一save59024068成功，session59024072变为R47/primary59024068，完整summary真实passed且settled，无原错误守卫再现。root raw-e7b9f009/59026341独立核十原/fresh终态、4629实体/9032边/89详情及完整玩家科研/日记/配置/空货/电网，证明修正落实到实际执行路径。旧G保存后的exit1仍保留，不宣称全局revision规律；其他动作族或新安装源码仍须单独证明。
...
- 首见：2026-09-13。raw-6be33071的唯一配置2a557a2f在53550757成功把3964设为r16，零物料扣减；私有执行器随后按110→111作后置断言而exit1，未执行3965/3966，accepted8保持，没有隔离或原生失败。
- 根因：当前NormalGameActionCoordinator的Complete增加revision，配置调度末尾又增加一次，实际110→112。只看调度末尾或把普通save的一次递增套用于configure都不正确。原42项离线检查覆盖请求/预算/终态，却漏测此执行后计数；不能将其PASS解释为完整执行器已验证。
- 处置与修正：root raw-77fb39be于53557494独立核原始/fresh成功终态、三台16/0/0空机、27空闲端口及原玩家/J85/两网，primary53515875不变。原完整执行入口永久拒绝再运行，单独后缀只允许3965/3966；从当前112 fresh读取和预检，每步依当前已证明配置路径核两次递增，不重放3964或改Plugin版本语义。
- 验证/状态：`fixed_offline_live_pending`限私有调用方。12项后缀专项及54继承检查、解析/3入口拒绝通过，覆盖真实110→112、错误0/1/3增量、首台终态/配方/未知负例、离线零调用及原入口退役。后缀实机与十写审计仍待；本条不是新增MCP能力或三台已完成声明。关联EXP-007/037/299。

## IFX-125 — 只读调查固定数组追加失败遮住已成功的原生窗口

- 后续实机状态：`fixed`仅私有只读后缀。raw-fc641760原脚本单次exit0，保留首个窗口后追加两个600tick窗口、逐窗写保护证据；root raw-de4ed7ac独立核原生响应/16详情及52119677当前healthy边界，accepted5/revision12/J85/primary51984902保持。16离线复验通过；19118/18tick间隙不计连续覆盖，原始两次exit1仍是历史事实，不改写为成功。下列offline待验表述保留修正时的证据层级。
- 首见：2026-09-13。raw-86983ad0先因未括号函数调用中的`-and`参数解析失败；raw-1c877cea再次运行后，资源/矿机快照和首个原生生产窗口均已返回，汇总使用固定数组`.Add()`抛出“Collection was of a fixed size”。两次均exit1，零prepare/commit，没有游戏写入或隔离。
- 处置/修正：到两次同阶段失败即冻结Luna，不允许第三次自行试错。root raw-a7d908b4独立保留52096382–52096981有效窗口、四对象及当前healthy边界，不能抹去原错误或声称完整三窗已完成。私有只读后缀复用既有Roundtrip边界函数，显式使用可增长集合，逐窗先持久化保护证据，只补缺失的两个不重叠600tick窗口；不重扫已成功的资源快照，不改变Plugin/API/游戏状态。
- 验证/状态：`fixed_offline_live_pending`，仅私有调用方。16项离线检查覆盖原生真实窗口、错误会话/宽度/重叠/状态/分页/覆盖/速率与三次追加，解析和3种导入前模式拒绝通过。fresh52110562仍accepted5/revision12/primary51984902，热电回收未执行；后缀实机另验，不能用离线PASS掩盖原始exit1。关联EXP-007/128/299。

## IFX-124 — 整批缓冲上界被当作原生批量出料拒绝阈值

- 当前状态（2026-09-15）：`fixed`，限当前DSP/6070cf8同批安装的精确19棒诊断。Luna raw-4e957fa8以预声明有界只读事件观察，保留20→20样本后自然命中58557430→58557438的19→19；实际MCP原生600tick窗58556835–58557434产0，3403/r41停机/满电/输入齐备，confirmed output_blocked的count及下一批准入阈值均19。root raw-aa09799b独立复核17个原响应，63 Core+2 MCP通过；安装228文件、64工具/1资源/71120字符指南、纯stdout/exit0及J88/accepted5保持。未清库存制造样本、未写游戏或重启。新固定探针raw-c35afb54先读到20而exit1也保留；下面记录仍为各自历史截面。持续燃料供应不随诊断修复一并核销。
...
- 2026-09-16再次复现：Luna 8F3477E1/raw-ac35cc9c已完成七动作、normal save60758779及最后八对象/玩家/J88/科研/两网/零prebuild核验，124原生回复全部success，完整完成报告也已保护落盘；stdout再因嵌套wholePlayerNet整数键失败，原PTY41153 exit1。此前15场景仅测试La-RunSeven返回对象，遗漏La-Start最终输出；不是Sol布局失败或原生动作未完成。root DA9D3B98经10专用离线/三入口/解析，以raw-0120da03重放原124回复、精确复现显示异常并正常exit0，无新游戏调用。58AC1C9E退役已完成入口并让显示走同一安全投影；F4084F6D的15场景/88检查增加真实输出formatter及100高纯硅/3铁/1铜嵌套数量往返。原执行源/exit1保留；accepted10继续冻结待完整审计，不重存或重做，未把本准备阶段称作持续产出。
...
- 2026-09-15复现/修正：CNT备料十写审计raw-38d2a5ae已在58720936耐久记录passed、十收据及4500实体/8802边，末尾kitNet/wholeTenNet直接输出再次触发整数键异常。保留exit1，主会话只读同一成功记录核销，不重采工厂或重放Luna五步；展示复用既有安全投影，30项私有审计离线包含新增嵌套整数键数量往返，完成入口退役。原首轮86/201自然矿竭断言是独立现场变化，已另按EXP-195复读核销，不能混为JSON问题或静默忽略。

- 2026-09-19相关后置断言：高纯硅炉备料后的固定 Move、取石材和 `r6×1` 已有唯一成功终态；`r6` 后 durable J91 仍无新增首件事件。既有 craft helper 已正确允许历史排除导致 0 或 1 条新增，私有入口却在其后又强制 entries 必须＋1，因而本地退出并保留 accepted3/旧调用方本地 R62；不是 native 失败、未知结果或需要重放。其后一次 fresh R64 / 0-write 续接读取又因调用方把旧本地 R62 当当前值、并假设每步 revision＋1 而停止；原回执保留，未发生 world 变化。当前修订将续接收窄为 `r56×1 → normal save`，精确保全 J91 前缀，r56 后只允许 0 或 1 条对应2302的 durable 首件事件，不补写/迟到1202；每个后缀终态后都读取严格前进的实际 revision，并保留字符串键输出投影；当前 runner 已通过 Sol 独立差量审核，实际执行待。此条扩展的是“成功终态后私有显示或断言不能反推动作失败”的同类边界，不新增公共 API 或事故编号。
- 2026-09-19后续实机复验：该窄后缀随后在 R67/save71701504 唯一完成 `r56×1 → normal save`，J91 durable 且无 pending/error，accepted3→5。Terra 61+4 只读比对保持5114实体、9968互返边和零预建筑；首个本地比较仅因误把较早材料封窗当 r56 前玩家基线而停止，改用正确回执并补四个未读项目，未重采工厂、未触发游戏异常。执行、收集与比较证明分别为 **31BD02EE84895C24BF076B7F7998955C511467A568417337BCA01BB82809E78C**、**80F332BABA28C52E1CEA35D7626A7B206B0797DF8E80CC5C3AD8E2AC358EB9F7**、**A315E8DAAACFE36068492193EC24A5B2B23A029966A443D945387F772FD5D360**。同期三条独立空段（6/11/14）仅以8次调用、3次prepare、0写入取得`full_path_stage1`，原证明 **2AAC52FE6FCD0F30F807EDB3905DEF5EACEE3EE66993D1ECD379593BB6EA51EC**、Sol复核 **51414CE4B4C43B9B7986C50B20DB5D4464F81999112F88F5A05B28B9587CDA9B**；它们不是本条调用方修复，也不涵盖余下4 NEW source-cover、2 NEW L3 dual-cover、4附件或任何施工/吞吐结论。不新增事故编号；此修正不证明新炉施工、物流或持续产率。
...
- 根因：私有七步执行器复制2012的100000/200000/300000枚举，用来校验2011。当前GameStateReader的inserter进度字段直接读取原生stt；真实目录2011的inserterSttRaw=200000/grade1，跨三格的600000正确。既有research和Core计算已有此语义，先前65项执行器测试未覆盖实际完工计时条件，不是产品动作失败。
- 修正：私有公共断言按同物品目录及1–3跨度校验STT，保留材料、过滤、精确双端、实际供电和空载条件；加入原始及独立复读3833两份真实样本、三个合成合法跨度及19个拒绝用例。原七步执行入口永久拒绝再次执行，后续只可针对明确未提交五只分拣器fresh施工。包内playbook补充计时单位/等级、理想周期不等于吞吐，以及调用方断言失败后不得重放原生成功动作。
- 验证：24项计时离线、27项部分施工审计离线及7项解析/入口检查通过；Release相关26Core/102MCP测试通过。root raw-74ff7821于49657031独立核两原始/fresh终态、全3833实体/7486互返边/202详情；3778→3833→3799两端实际slot4、network3/ratio1、零预建筑，原J84身份/全部历史/84durable保持，玩家余17带/六普通/零塔。accepted4/revision7分别保留；stopped记录的revision5只是第二次动作完成前的脚本变量，不能当作当前revision。
- 状态：`fixed`限本次私有断言和已建两对象核销。新指南已编译进源码MCP，尚未冷部署或打新包；原安装态不变。primary仍49350923，本段未保存、未通水，不核销持续硫酸/合金/燃料或完整Foundry门。关联EXP-007/188/219/299。

## IFX-119 — 私有资源审计猜矿种/字段并混入失败响应空结果

- 当前86空矿点基线的全场复验（2026-09-15）：ac9a77f绿CI后的v3 raw-87ccd681完成两组47页4639实体/9068边及四点预检，root193A37CD/raw-0807c7e7独立核原155回复exit0。86仍按普通完整配置比较通过，没有复原197、忽略设备或重建旧产线；最新完整全场59784824，末session59785219/R88/primary59372664/J88/accepted5。限当前现场预检，不核销施工、保存或持续供应。

- 矿竭基线续接离线完成（2026-09-15）：Sol v3/72E822DD将原失败705、独立744九读、eb16deb/CI34954218203精确绑定，并以实际86空矿点DTO替换比较基线，保留旧v2/82EC3592及全部原证据。未来86仍经完整Assert-AuditConfig，不忽略整机或所有resourceNodeIds。root104项调用方、193A37CD的33项独立核验器及继承41项纯离线测试、解析/八个导入前拒绝入口正常exit0；覆盖错误矿点、对象、位置、连接、电网及未来漂移。零新游戏调用/写入，末实机59755916/R88/primary59372664/J88/accepted5不变。本切片只关闭私有基线适配离线门，新全厂/四点预览及IFX-135 live仍待。

- 最后一个矿点后继复验（2026-09-15）：v2 raw-705ad182在59747867的第一张100实体页发现86由[197]变[]而exit1。仅8个成功只读回复，session59747835/R88/primary59372664/J88，没有四点prepare/probe/commit，全厂分页未完成。root9C577308以18项离线/解析/三坏入口验证后，只做一次9读：raw-74466c3f正常exit0，miner59755870为空、旧197独立返回INVALID_ENTITY/null result，除精确资源列表外位置/朝向/连接/电网等原配置保持。末59755916/R88/J88/accepted5、全玩家/科研、两网成员满供和零prebuild保持；正常save尚未发生，精确耗尽tick未知。
- 适用界限：CaptureMiner按原生veinCount枚举，已耗尽设备是合法零矿点状态，不沿用旧单点理论产能。该精确证明只允许Sol为后续比较建立86的当前基线，不能整机跳检、全局忽略resourceNodeIds、补回矿点或修改成功工厂。新全场与四点预检仍需fresh核验。root原始诊断包装器曾因未导入ACL helper、未先区分成功响应就读取error.code而本地exit1，均零游戏调用；随后按真实wrapper及response.success分支核清，失败记录保留。未来读失败响应继续先检查envelope，不把null result当节点。
...
- 2026-09-17复验：八只纳米管sorter及save66133149已成功；root阶段审计raw-403b5102完成79次成功读取，最后实体4877真实存在，却再次因Int64取值键得到null。统一详情循环的实际Int32键，缺项先明确拒绝；3项类型/缺项纯检查和解析通过。只读raw-cd5e8f36于66176088完整核4917实体/9618互返边、九terminal、物料/Journal/供电及七条末带纯1124，通过后保持accepted9待显式封存。未重做游戏动作或修改原证据，`fixed`仅此私有阶段审计，不声称全体临时脚本已消除此类问题。

- 2026-09-14复验：石墨源cover调用方raw-ce03cbba在第10项只读详情4422处用Int64索引Int32字典，零prepare/commit即停止；取值处统一显式整数键并增加29个真实选择ID回归，45离线和raw-9a2b8b51实际源完整预检通过。后续11带正常完成与root raw-514044a2全场核销保留；另一个0.906mm玩家后置停止不是键修复失败或原生动作失败，依EXP-293分开记录，不重放已建对象。

- 2026-09-13复验：取料路线私有只读预检raw-4b9c184d在49次读取后，仓库716同样因Int64索引Int32键而被当作空对象，尚无prepare/commit。只在私有查找入口验证整数/正值/Int32范围并规范化实际取值键，补真实Int64/Int32正例和非整数/非数值/null/缺项/越界负例；23+95离线、解析/入口守卫通过。raw-5f5bbd4f随后核完整4225配置、两源各3000并进入原生首段预检，因另一个真实浅水终点条件拒绝commit；此后续拒绝不是查找修复失败，也不把预检负例写成走通。root于54858190确认原玩家/日记/保存边界保持，无新游戏写。

- 首见：2026-09-12。石矿同档恢复成功后，root raw-43b8efb1已核全场和438详情；货物循环随后错误报告缺少beltCargo，Luna没有继续游戏写入。
- 根因：从原始JSON读取的37个石矿带ID为Int64，而详情字典显式使用Int32键。真实3716用原Int64索引得到null，转换为Int32则可读observed cargo；ContainsKey前的转换不能替代取值时的转换。原对象和数据完整，没有观察API缺陷或游戏恢复失败。
- 修正：共用货物函数先拒绝非整数、非正数和超Int32范围，再统一键类型并验证对象存在、2001类型和实际cargo纯度；不删除目标或改成把null当空带。保存前/恢复后438详情、真实类型差异以及缺项/unknown/串料/错误类型负例进入同一函数，恢复离线合计18项通过。
- 验证/状态：root raw-a712991d于48821669重新完成3756实体/7344互返边/438详情全审计，原背包、石矿/油/钛钢环及J84/两网保持；`fixed`仅当前私有审计调用方。accepted1/save48796250，没有重复resume或其他游戏动作，关联EXP-007/219/299和日记001。

## IFX-114 — 私有脚本文本复用展开了替换元字符

- 首见：2026-09-12。适配石矿恢复审计时，JS String.replace的字符串替换参数中包含美元加单引号，被解释为原文匹配后的尾部，导致脚本尾部重复和字符串未闭合；同次机械Steel→Stone命名还造成重复结果键。
- 修正/验证：用回调返回精确替换字面量，去掉尚未执行的重复尾部，按实际物品身份分别保留steel/stone指标。美元加单引号、反引号、与号和美元的四个字面量回归通过，最终脚本解析及三入口拒绝通过；这些检查均在该审计第一次执行前完成。
- 状态：`fixed`仅本次私有文本适配，不改变Plugin/MCP；解析失败未调用游戏，也未据此重放已成功的恢复。JSON键类型问题另见IFX-115；关联EXP-007/299和日记001。

## IFX-113 — 私有角度夹取误选整数重载，产生假零角

- 2026-09-14离线复现：重设计石墨入仓接点的一次性算式再次使用整数Min/Max边界，三个不同点被错误投影为0°，没有游戏调用或据此执行。该结果作废；有界续接调用方使用显式double两边界及归一化向量，已知0/30/60/90/120/150/180°和两种零向量检查防止回归，真实旧末带/拟新直线点中心角为33.415°/9.089°。原生prepare给出的33.413°拒绝证据不受影响；后一中心角仍非实际带插值接点通过，需实体建成后fresh预检。

- 首见：2026-09-12。定位石矿仓口负例时，raw-f3dab6f3的状态/材料/Journal核销有效，但附加角度全为0，与已有端点位移明显冲突；尚未据此prepare或commit替代方案。
- 根因/修正：PowerShell的Math.Min(1, cosine)/Math.Max(-1, ...)选取整数重载，非整数余弦被转换。私有离线分析改用明确double变量和上下界比较，不改变Plugin/MCP或放宽原生11°规则。
- 验证：raw-b5e71e92显式作废旧angle字段；0/30/60/90/120/150/180°非单位向量正例、两种零向量拒绝及实际端口检查共10项通过。实际95空slot9/11→3734约33.4°；slot9→3733为9.67/9.66°。后一项只支持挑选不同候选，必须另做fresh原生预检。
- 状态：`fixed`仅当前私有离线计算，零游戏调用；旧f3dab6f3状态核销仍有效，错误数值保留来源而不再引用。关联EXP-208/219/299及存档日记001；没有额外游戏动作或重新施工。

## IFX-112 — 命令包装器用残留原生命令退出码判断PowerShell脚本

- 首见：2026-09-12。石矿备料审计的offline已输出31项通过，外层却用LASTEXITCODE检查该PowerShell调用而报错，尚未开始read审计。五个游戏动作此前均已成功，没有游戏失败或第二次执行。
- 根因/修正：LASTEXITCODE只代表最近一次原生命令或显式exit，不能当作普通PowerShell脚本的返回值；调用方依赖PowerShell异常/当前调用结果，原生命令才就地检查其退出码。取消该错误判断后仅执行未开始的只读审计，不改游戏或重放备料。
- 验证/状态：31项离线及三入口拒绝真实通过，随后raw-c644da93于48247755完整复核五原始/fresh终态、16详情、玩家/J84/两网，exit0；`fixed`仅当前调用方，不声称所有历史私有脚本都已整改。accepted5保留，关联EXP-299及存档日记001。
- 同日水源续接审计复现：41项离线已通过，外层仍以LASTEXITCODE判断普通PowerShell脚本而误报失败，read尚未开始。改用脚本异常/实际调用结果后离线独立exit0、三入口拒绝通过；raw-6da1c2ee随后于49510555一次只读核全3831实体/161详情通过。原Luna三带终态49496648没有失败或重放，accepted2保持；本次修正仍只属于当前命令包装器。
- 同日酸到合金预检再次复现：52项离线已经正常返回，外层沿用原生命令退出码而误报；此时尚未做六次native prepare。外层改为PowerShell终止异常判断后离线独立完成，随后raw-35e8bd2e只执行一次六个固定端点预检，全部通过于50130899/accepted3。未改Plugin、未增加重试、未产生游戏写入；检查LASTEXITCODE只限紧随真实原生命令之后。
...
2026-09-13十写审计入口再现：私有阶段表只有最终Move含final键，首次offline直接读取stage.final被StrictMode拒绝，未发生游戏调用。改为先ContainsKey再读取，不关闭StrictMode；26+198离线、解析/三入口拒绝通过，随后raw-f0bbd87f真实只读审计于55088826核十终态及全厂/玩家/日记通过。此处`fixed`仅覆盖该可选阶段字段，不改Plugin/MCP、不重放任何游戏动作，也不外推所有旧脚本。
...
2026-09-15写入前复验：Sol新石墨烯执行器的Cg-ReadOrdered再次以一元逗号输出数组、调用方用`@()`收集；root纯表达式证明2对象变outerCount1/innerCount2。尚未执行任何build，不能记成新实机失败。修为生产调用复用的纯排序helper正常逐项输出，89项离线包含2/4对象、逆序terminal IDs的扁平几何排序与负例；零写原生预检raw-a5ed5325另通过，施工尚待。继续保留原成功前缀不可重放和显示/形状错误不能归咎DSP的边界。
...
- 显式入口后继正例（2026-09-15）：cf12834绿CI后Luna实际v3 execute命令转录时把风机审批SHA从40位截成33位，25c990真实exit1发生在任何导入前，无Bridge调用/accepted。原准入没有被放宽；root纯AST两例正常exit0，完整参数通过、实际短串复现原拒绝。已将原14项获批实参固定为data-only/3CF2E393，参数键与源码ParamBlock一致、解析/纯guard通过，执行者须验hash后整体加载，仍明确execute且仅原四建一存。两次root初始本地诊断误拒绝（文本正则匹配了错误消息、猜错键数）均零调用，改为结构AST核验后通过。当前accepted5、末59785219/R88/primary59372664/J88，尚无施工；本条不授权旧token重放、隐式执行或改变已成功部分。
...
- 2026-09-15后缀修正：Sol八路径raw-6c506b90/58818297零commit预检通过，root独立复读原计划及九组玩家/session、J88/两网/4501/零prebuild。保留原成功塔及accepted1，只允许八带+save；两份实际生产核验共用各自被测试的纯节点helper，真实null正例及缺失字段/假网号/身份/连接负例覆盖，电网摘要独立归因。Sol在root审计发现的残留网号3断言也在执行前修正；51后缀+89继承+40独立离线、8错误入口及1 MCP回归通过。尚无修正后续建/保存实机结果，状态仍为mitigated，不重写首轮exit1或重放4501。
- 2026-09-15再现：Sol私有石墨烯执行器经89离线/原生预检后，Luna raw-f300c4a7成功建4501/2201（58803728、4→3），却在null网号后置断言停止。root raw-5bcb5033核4501实体/8802边、旧配置/完整玩家/J88、原/fresh成功终态和两网，只有网3节点+1；原动作成功且不可重放，accepted1/R11/primary58361888保持。并非产品缺失字段或原生拒绝，而是已有指南没有落实到新执行器的实际纯节点测试。旧全批入口退役且两模式零调用拒绝，现有MCP指南1项回归通过；本次记`mitigated`至成功前缀核销，未提交八段带+save的修正后缀另验，不能改写首次exit1。

- 2026-09-12 再现于只读水线预检：raw-10674513在读取882的2201/power-node/null详情后被私有网号断言停止，尚无prepare_build或游戏写入。已有包内规则未落实，不应另造产品字段或称为断电。调用方按实际节点类型只校验几何身份，null不推出网号；保留新消费者真实受电及完整供电待验。28实际/合成检查含该null正例与非法身份，修正后raw-528f59ad六个真实过滤端点原生预检通过，13详情/玩家/J84保持；3入口/解析、144相关Core及21 action-client通过。未建塔、分拣器或实际通水，不把此调用方修正当补电完成。
...
- 2026-09-14第二供源有真实收益、仍open：新增9带/2sorter正常保存58238457后，raw-36f7caed三个原生分钟石墨烯16/16/16、重氢30/25/25、燃料4/2/2；root raw-87735abd/314原始检查、34审计离线/4入口/2退役及135相关Core通过。石墨烯到达原15±10%目标，不能再沿用“当前仍只有10”的旧截面；但燃料首窗超差、石墨烯C=0且871仍有限仓，全星球石墨P59–60/C62–66，未证明长期消费/库存平衡或实际保存恢复。下一只为真实下游需求设计完整原料/成品/预约方案，不重建成功线路、降低目标或重跑有利窗口；后两窗2/min不独立构成稳定验收。

- 2026-09-14接入石墨烯后仍open：67带/五过滤连接已正常保存57954957；raw-d9d0496c三组3600tick窗为石墨烯10/10/10、重氢15/15/15、燃料0/2/2每分钟，未达15石墨烯和2燃料棒所需20重氢目标。3083/3084工作、3965/3966仍满20石墨，root raw-3f3ff256的252原始检查与102 Governor Core通过；不是全线停机、缺电或旧共仓事故。root前案核了sorter额定流量却漏核单源：3110唯一供给3084的r58满速15石墨/min，只能支持10石墨烯/min；全星球统计不等于此支路可分配供给。保留已成功线路，下一仅重设计现有其他石墨支路的有限补源和长期消费，至少缺7.5石墨/min，氢/D2实际送达另验；不得重放施工、降低原目标或拿有限仓增长关闭问题。

- 2026-09-14复验仍未关闭：补电与最终入口已正常保存57449785，raw-bd24943c的三个独立600tick窗口仍证明石墨背压限制氢供应。3083/3965三次均油2、氢4、石墨20、周期完成而停机且满供；第二窗四裂解均被既有Overseer确认output_blocked，重氢诊断路径3073→氢→3083明确落在石墨20/20门。其他窗有批次产出，不能称永久死锁；原氢共仓3074为空、旧种子仓3348为空，未复发585D2占满的旧原因。155项独立原始证据核对、13只读隔离检查与41项容量/燃料Core回归通过。下一由root设计服务紫糖/远征真实需求的石墨消费及完整供给预算；不增加发电机掩盖负载跟随，不清罐、不改已成功线路或授权Luna盲试。
...
- 2026-09-16后继规划错误及替代预检：C19原首端101→4664在raw-65833656报TooFar；第三轮完整连接仍失败后交root，不能重试同对或把米距/端向合格当完整准入。当前DLL另有3.499格距门，root E0BC40EB用原19带坐标和源码重构旧首约4.210155格，明确不是原生instrumentation或对具体失败分支的断言。既有中段4661/4648提供约2.205格的两处新接点，不需新增中继/改建成功对象。Luna66B8D9DA/raw-b2812748的148次读/prepare全部成功、PTY43577 exit0、零写；两处C输出接点均通过exact-slot与独立166节点/未来4sorter1200J/t预算。root E56B92D3审原150文件、重放148回复、5专用检查及入口/解析通过，零新游戏调用。开闭4664实体/9128边及19带保持，末61242765/R120/primary60758779/J88/accepted5。包内指南增加格距独立门、既有内部接点保全和禁止重放规则，2项MCP资源回归通过；指南未冷部署，修正范围仍只到替代接点预检，实际施工、H输入和持续生产另验。
...
- 2026-09-15后继实测补记：2ea146b/CI34920128723成功后，修正辅助的唯一fresh raw-5327ba1d至59316547正常exit0，核完整玩家/科研/J88、五原对象配置/预约、零prebuild和两网满供，accepted10→0、R62/primary59071065不变。capacity1565000与原1601000分别保留，当前required=served=82404及成员完全符合条件；这关闭下条后继修复的live-pending，范围仍仅计数核验，不证明未来负载或持续燃料，不改写前次exit1。
- 2026-09-15复发：共仓十写已由raw-a014a51b独立审计并推送；只读计数交接raw-8d6ffb7d却沿用旧Ta-Power，在主网容量1601000→1565000、成员不变且required=served=328881时exit1。没有游戏commit或reset证明，accepted10保留。仅当前计数辅助改为严格固定成员、完整有限非负供需和实际满供检查，显式保留容量前后值；不放宽后续施工的满负载预算，也不声称已定位本次具体燃料变化来源。50专用（含真实原/后DTO正例及19类缺失/欠电/成员/摘要负例）加107/45/50继承offline、3入口/解析及1项既有MCP指南回归通过；raw-ef7f2277重现旧拒绝、新守卫通过，零新增游戏调用。此后继修复为`fixed_offline_live_pending`，原始exit1保留，fresh计数交接仍待；已执行历史脚本、Plugin和hash均不改。
...
- 2026-09-12复验：合金连接十写只读审计的生成文件再次被AST在任何游戏调用前拒绝；逐处使用字面回调重建后，解析/入口拒绝及105离线检查通过，才执行raw-6755ef9a完整实机只读审计。没有修改Plugin/MCP或重放施工；该次重复说明模板变换本身必须执行既有修正规则，不能只依赖旧模板已合法。
...
2026-09-18后续复用入口修正（`fixed_offline_live_pending`）：Q/4936的回航等待暴露了旧经验再次未进入私有风机入口。现将初始idle、已完成施工后的零pending回航与独立阶段idle核验分层：完整一致的无人机计数/稳定玩家/前动作与实体材料证据成立才继续fresh预检；保存后独立记录readinessPending，保留全部原terminal。没有延长等待或改变Plugin、hash、幂等、正常成本与审计。17新增正反例、83风机/20备料纯检查、原62份玩家回包离线分类及两项MCP指南回归通过，Release零警告/错误；同源包内指南吸收此规则。新执行器BBBDE48C尚未施工、安装程序集未更新，不能将这次离线修正当作后继六风或持续供电通过，关联EXP-242/299/303。

2026-09-18私有风机入口复验（本次成功前缀处置`mitigated`，等待实现未改）：Q/4936已在67698936完成并扣1风机，后续60次玩家复读仍有一架回航无人机，本地等待上限使原两风阶段在第二台与save之前停止。root核原终态和fresh唯一实体/材料，确认稍后全idle后，仅让Luna正常保存到67716265；完整4936实体/9618边/零预建筑独立核验成立，accepted1→2不重置或重放。现行BuildUiIsIdle及普通save并不要求所有无人机idle，不能把私有等待偏好写成原生失败；本轮尚未修改该入口，亦未证明剩余施工或持续供电。相关MCP指南回归2/2通过，见[存档日记](./gameplay-timeline.md#2026-09-18--第十八台风机完成回航等待停止后仅保存成功前缀)。

- 2026-09-15单风机再现：raw-a60c9099的c169c05f已在59638794成功为4639，原184回复包含新实体、完整after47页及四旧见证；7011F776仅在最后player的working1/pending0断言停止，PTY31252真实exit1保留。这是私有调用方未吸收现有返航经验，不是Plugin未完成或新公共缺陷。root首轮原记录摘要又误访问player.objectId而exit1，零游戏调用；其后DE52E5E8独立原响应/ACL/场景/材料核验正常通过。raw-90b8c224另8fresh至59654915/R88/J88，原terminal一致、4639 net3正输出、玩家已idle、两网/空prebuild保持，primary59372664/accepted5不改。原execute退役，C6C72953将完成证明和返航phase分开，64离线含8新就绪反例，独立审计11项/21客户端/2现有MCP指南通过；新有界等待分支未live执行，不扩大为所有旧脚本已修。未重建/保存/移动/清零，关联EXP-007/242/299。
...
2026-09-14私有调用方再次复核：raw-12810dcd三带原action1f36cac9已在57537202成功且raw含terminal；As-Ready在轮询前调用含无人机idle要求的断言，working=3提前结束runner，第二段未prepare。root raw-60f4fd02核全部4406实体/8606边与材料，再把检查顺序改为先保全状态/物品/位置/未完目标、后有界等返回；六个新增就绪正反例通过。原双段入口退役，raw-236002e8仅fresh执行三带南后缀，57546561成功，root raw-dbd3022c核4409实体/8610边及两原terminal，不重放首段、不重启/改门禁。27就绪/2原终态/7后缀隔离检查通过；此前包内指南已经产品化此经验，本次缺陷仍在私有调用方，不称新增公共修复或新增动作域。
...
- 2026-09-14现行安装态负例复验：石墨末带4481→仓870在raw-77e408b0返回36尝试、4插值投影、0朝向匹配、bestFacing33.413°≥11°，nativeChecks/candidateChecks均0；该诊断正确区分了预条件朝向拒绝与原生施工条件检查，未commit。root raw-58940bf4/57813690保持旧六带/仓/塔、玩家/J88/健康写状态，核北向槽0/1已占、槽2空闲，原候选退役。100私有/155 Core/21客户端通过，产品代码及阈值未改；下一按真实空闲槽重设计有限接入，不能把观察正确称作已经修复供料。此项复验不改变下列历史实现截面或持续燃料未通过状态。
