...
- 修正：先同步AGENTS，再在MCP内嵌指南/prepare描述区分两种证据。完整≤32m短预览须明确末端至少4m逐点地面高于水面至少0.1m，结合障碍、精确目标、≤0.5m容差与能源余量，才可外部选择一次正常Move；长距/不完整仍要求当前复验的历史Walk点。未新增工具、DTO字段、原生调用、自动寻路或位置写入。
- 离线验证：新增两项MCP资源文案回归后MCP93项通过，既有地表策略27项通过。私有有限执行器对保留原预览及12个缺失/未知/浅水/非有限值/末端风险变体完成一正十二负检查，AST通过；这是调用方检查，不是新的Plugin准入。尚未执行该短跨越，新指南未冷部署；不能以文案测试或采样正例声称实机通过。关联EXP-093/236及日记001。
