...
- 2026-09-19相关后置断言：高纯硅炉备料后的固定 Move、取石材和 `r6×1` 已有唯一成功终态；`r6` 后 durable J91 仍无新增首件事件。既有 craft helper 已正确允许历史排除导致 0 或 1 条新增，私有入口却在其后又强制 entries 必须＋1，因而本地退出并保留 accepted3/R62；不是 native 失败、未知结果或需要重放。当前修订将续接收窄为 `r56×1 → normal save`，精确保全 J91 前缀，r56 后只允许 0 或 1 条对应2302的 durable 首件事件，不补写/迟到1202，并保留字符串键输出投影；待独立审核/实际执行。此条扩展的是“成功终态后私有显示或断言不能反推动作失败”的同类边界，不新增公共 API 或事故编号。
