...
## IFX-129 — 固定中心净空未单独处理容差内到达的近邻起点

- 首见：2026-09-13。Luna raw-ca2863a8已成功完成五段Move及两笔取料，返程在原生prepare前因非带实体中心净空小于3m而退出；accepted8、没有返程动作或保存，不是卡住后的Move失败。
- 根因：名义取料目标离仓762约3.43m，但真实成功落点在原生1.5m容差内，中心距离为2.76973m。候选评分未为下一段起点预留到达容差；返程线段的最小中心距离出现在起点fraction0，固定全段3m守卫也会拒绝从现有近邻向外离开。不能据此声称已观测到碰撞体或原生路径拒绝。
- 边界/状态：`open`，调用方规划与守卫边界已定位，尚无恢复执行。原脚本只调用一次并保留成功前缀，不能改阈值后整段重跑。下一由root绑定当前实体/玩家位置设计有限背离或返程，保留原生地表、能量、fresh状态哈希、终态和正常Move；不拆改仓762或其他已成功部分，不改Plugin/MCP或开放寻路循环。
