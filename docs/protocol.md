...
When the immediate result is `material_shortage`, each missing item is traced independently from the consumer's exact input sorter. Reverse traversal follows only directed belt/splitter/piler/spraycoater/sorter/storage/tank edges. Every sorter must be unfiltered or filtered to that item. A splitter is entered only through one exact identity-checked output belt: with no filter either output class admits the item; with a filter its priority output admits only the filtered item and every non-priority output admits only other items. A disallowed branch is not traversed, while inconsistent connection-slot/component/belt identity fails the whole read closed. A reached producer is bound by exact planet, object and output-item identity, then classified from the same captured current buffers, power, source and working state. Its per-device historical rate is deliberately marked unknown so an aggregate rate for a different requested item cannot suppress the current-state finding. Traversal is same-planet and limited to eight producer levels and 64 visited producer identities with a cycle guard. If a graph bound or inconsistent producer reference stops a still-expandable path, evidence includes `upstream_trace_stop_reason`; callers must not treat that point as a proven complete root. Physically relevant station evidence is attached only when the same reverse traversal reaches an exact configured station output belt. A single snapshot never invents transport progress: it may confirm a configured route with available source inventory but zero carrier fleet, while an outstanding route with carriers remains a material shortage until a later temporal slice proves no progress. Unsupported immediate fractionator, gamma-receiver, or orbital-collector producers make direct coverage `partial`; they and cross-planet producer edges are not yet recursive. Each planet separately returns at most 16 `infrastructureFindings` for depleted extractors whose item identity has already been cleared, with total/truncation metadata. See [game-api-overseer.md](./research/game-api-overseer.md).
