...
`spherewright_list_factory_entities` with `componentKind=station`, and `spherewright_inspect_factory_entity` for the resulting positive entity ID, include a deep-copied `logisticsStation` object for a completed station. The object cross-binds entity, local station, galactic station and planet identities; reports energy, fleet counts, raw route settings, every configured or empty storage slot, and every station belt port; and includes separate `stateHash` and `configurationStateHash` values. The full hash changes with live energy, inventory, orders and fleet activity. The configuration hash excludes those volatile values but binds identity, station type, slot item/limit/logic, route settings and belt topology. Trip/delivery/warp values remain explicitly named `*Raw` or `*Setting`; their UI transforms are documented locally, but no route-setting writer is adopted yet. The only station write in this slice is the restricted configuration action above; no action directly fills, clears, replaces, or injects station inventory.
