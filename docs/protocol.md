...
Quarantine reconciliation is not a force-unlock: it is available only for the exact retained outcome-unknown build action and clears quarantine only after the item decrement, all new entity/component identities, and directed topology form one unchanged proof. It never repeats the build. Owned-game resume is likewise not a save picker: it accepts only a protected one-time token and first prefers DSP's fresh fixed `LastExit` slot. If a failed shutdown did not refresh LastExit, it may fall back only to the exact fresh primary owned save whose high-entropy name and minimum tick were already sealed inside that ticket. It never enumerates saves or accepts a save name from the caller. Either load is adopted only when the high-entropy owned identity, minimum tick, planet, peaceful/non-sandbox/1x settings, and source-process shutdown proof all match; the token is then consumed.
