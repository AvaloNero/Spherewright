...
- The first normally built PLS exposed the native local-station `planetId == 0` sentinel. The source identity-policy correction builds with 0 warnings / 0 errors and 101 tests pass (Contracts 11, Bridge.Core 73, MCP 17). It has not been hot-deployed into the running process; live station-detail/configuration/fleet validation waits for the next normal save/close boundary.
