...
- MCP Inspector: the 46-tool source surface compiles and is covered by registration tests, including the new two-phase logistics-station fleet transfer. The currently deployed game process still exposes the preceding 44-tool build until a normal save/close boundary. The current owned world runtime-verified explicit save, research-result acknowledgement, normal refuel, exact-proof quarantine reconciliation, planned exact-primary resume, both refinery-output sorter filters, journal durable-through readback, and legacy checkpoint retirement. Station fleet transfer remains pending live deployment and first-tower validation.
