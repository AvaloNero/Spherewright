...
- On 2026-09-04, the conversation-confirmed current-save import was isolated onto a `v0.3.0` maintenance baseline for the `v0.3.1` prerelease. The backport contains the two import tools and their required contracts, safety policy, normal-save/header-proof coordinator, configuration, and tests, but no Overseer types or tools. Against the unchanged DSP `0.10.34.28529` assembly hash recorded above, the full Release solution built with 0 warnings/errors and all 127 tests passed (Contracts 15, Bridge.Core 92, MCP 20). No Plugin deployment, DSP launch, game mutation, or save write occurred in this batch; cross-computer proof that the original header stays unchanged, the owned copy saves/resumes, and re-entering the original remains restricted is still required, so the package is a prerelease.
...
- The full solution builds with 0 warnings and 0 errors. The current suite passes 94 tests: Contracts 11, Bridge.Core 66, MCP 17.
