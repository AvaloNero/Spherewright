...
Warehouse follow-up research, not yet an implemented public action: `UIStorageWindow.OnBansSliderValueChange(float)` calls `StorageComponent.SetBans(int)`; its filter-button handlers set `type=EStorageType.Filtered` and call `SetFilter(int gridIndex,int filterId)` for existing item grids or only empty/same-item grids. Clear-filter loops through `SetFilter(i,0)` before restoring Default. `UIStorageGrid.OnFilterItemPickerReturn(ItemProto)` confirms empty/same-item per-grid filtering. `SetFilter` refuses incompatible occupied contents and resets optimization flags; it does not make capacity by removing items. `SetBans` changes automation capacity and lookup caches, not item counts. A future action must prebind the exact ordered native grids privately: the existing public `buffers` list omits empty grids and its role is just `storage`, so list offsets are NOT a durable grid-index API. Do not infer editable slots from that list. Respect the native ban/filtered-store delivery behavior, and prove inventory/topology unchanged by any configuration.
