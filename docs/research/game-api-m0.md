...
## 2026-09-17 bounded owned-save identity prefix

Current DLL inspection of `GameSave.SaveCurrentGame`, `GameSave.ReadHeader`, `GameData.Export/Import` and `AccountData.Export/Import` proves the uncompressed prefix: six ASCII `VFSAVE` bytes; Int64 finalized file length; Int32 header version7; sandbox/peaceful Boolean bytes; four Int32 game-version components; Int64 game tick; Int64 UTC DateTime ticks; Int32 screenshot length and bytes; AccountData(format0, Int32 platform, UInt64 ID, 7-bit byte-length UTF-8 name); UInt64 cluster generation; GameData(version13, patch22); another AccountData; 7-bit byte-length UTF-8 embedded gameName. The next value belongs to GameDesc and is not needed for identity. Native `ReadHeader` does not return this name; `ReadHeaderAndDescAndProperty` reads and discards it further down. Neither mtime nor a filename prefix proves ownership.

`OwnedSavePrefixReader` reproduces only this bounded layout without game DLL dependencies, skips account values/screenshots, compares the complete embedded name ordinally against a caller-supplied protected identity, and returns no names/accounts. It additionally requires the following GameDesc version9 word without importing its content. Screenshot skip is capped at16MiB, account-name bytes4096, owned-identity bytes1024; malformed/truncated or unsupported layouts fail closed. The26 synthetic offline cases cover exact identity, lookalikes, both sandbox values, bounds, versions, malformed UTF-8/lengths and stopping after the GameDesc version word. This parser alone neither grants ownership nor authorizes a load; the opt-in coordinator integration additionally binds full-file SHA-256 (64MiB/2s budget), length/mtime, exact candidate tick, ticket and Journal. Windows tests prove read sharing while writes/deletes are blocked, and same-prefix/same-mtime body changes alter the fingerprint. Local live recovery remains pending.

For the planned commit-to-load lease, native `LoadCurrentGame` opens its file with `FileStream(path, FileMode.Open, FileAccess.Read)` (default sharing `Read`), compatible with an existing read-only `FileShare.Read` handle that prevents competing writes/deletion. Keep that handle until adoption or explicit failure/cancellation; do not copy the candidate over primary or fabricate quarantine. Full world integrity is still proved only by the normal loader and post-load adoption/Journal checks.
...
Implemented offline: `GetProgressionStateOnMainThread` now passes these exact two fields and `LDB.items.Select(itemId)?.name` to `TechCompletionRewardCatalog.Capture`, after the existing owned-world/main-thread gate. At most32 complete paired entries become independent DTOs; null, unequal lengths, invalid IDs/counts, oversized arrays or unresolved items return unknown for the whole field. Observed empty arrays remain empty, native order and duplicate entries remain intact. No grant method is called. The existing live/selection hashes and write paths are untouched. Full current-DLL Release compilation and1714 offline tests passed; specific technology1501 reward readback and cold-deployed live reconciliation remain pending.

Subsequent same-DLL local validation: source efe12c2 was cold-deployed as a complete228-file cohort. Actual installed MCP raw-22ff6292 exposes1501's single2205/count1 reward and observed-empty lists for1502/1503/1521. After normal save39730515 and protected exact-primary resume39730547, raw-7cc37975 reconciles the original no-panel player snapshot39613814, sole technology completion39617713, nine free slots and exactlyone zero-inc panel with unchanged other inventory/research buffers and retained transfer terminals. Journal69 gains no fabricated first-production event, and the save/resume does not grant a second panel. This closes that concrete local event; prototype metadata alone remains insufficient to accept other gains, unknown metadata or overflow.
...
The assembly/version/hash above were rechecked unchanged. `UIInserterWindow.OnItemPickerReturn(ItemProto)` assigns only `InserterComponent.filter`, `SignData.iconId0` and `SignData.iconType`; `OnResetFilterButtonClick()` clears those same fields. Neither handler checks for empty cargo, changes the held item or changes the destination. `InserterComponent` is a value type with primitive fields, including private `idleTick`; comparing a copied expected struct with `Equals` immediately on the Unity thread covers its complete native state, not only the serialized subset. Spherewright now retains the cargo-free window and additionally allows only ordinary2011/2012, unidirectional, built-to-built `Inserting` sorters with valid native-sized held fields. The current configuration hash still binds held cargo. The write compares complete native inserter/sign copies allowing only the three UI assignments, player package/in-hand state, and endpoint hash normalized for the declared filter change. No take-back, buffer mutation, motion reset or destination change is added. Offline validation is distinct from the still-pending held-cargo live test.

Warehouse configuration implementation now follows these rechecked handlers: `UIStorageWindow.OnBansSliderValueChange(float)` calls `StorageComponent.SetBans(int)`; `OnFilterButton2Click(int)` locks existing item grids, `OnFilterItemPickerReturn(ItemProto)` filters only empty/same-item grids, and `OnFilterButton3Click(int)` clears filters before restoring Default. The first two filter handlers set `type=EStorageType.Filtered` and call `SetFilter(int gridIndex,int filterId)`. `UIStorageGrid.OnFilterItemPickerReturn(ItemProto)` confirms empty/same-item filtering; `BuildingParameters` ordinary storage paste also calls `NotifyStorageChange()` after its settings. `SetFilter` refuses incompatible occupied contents and resets optimization flags; it does not free capacity by removing items. `SetBans` changes the count of disabled final grids and lookup caches, not item counts. `PlanetFactory.InsertInto(int,int,int,byte,byte,out byte)` reaches ordinary `StorageComponent.AddItem(...,useBan:true)`, bounded to `size-bans` grids; this is separate from normal player's `AddItemStacked` and machine needs-based `TakeTailItems` with default useBan=false.

The new `storage-capacity` configuration mode permits only one unstacked2101, at most100 grids, known native stack sizes and Default/Filtered states. Core projects the selected UI operation immutably. Prepare/commit additionally bind exact ordered grids, including empty cells, storage component/link identity and endpoint geometry; the prior public `buffers` list omits empty cells and its role is just `storage`, so list offsets are NOT grid indices. Native apply changes only UI settings, then compares every expected item/count/inc/filter/stack-size cell, entity/link identity, endpoint hash normalized for authorized storage settings and player package/in-hand state. Empty reservation metadata can change itemId while count stays0; no stock is created/deleted. Unsupported/corrupt or stale evidence rejects; unproved after-state quarantines instead of retrying. The1036-test held-sorter slice and subsequent1066-test warehouse slice remain offline evidence until same-cohort live verification.
...
This distinction is specific to the real destination: storage-to-belt/storage must not be assumed to inherit a downstream recipe. The installed1018 cohort required an empty-handed filter assignment; the later cargo-preserving subset above does not alter consumer needs. `StorageComponent.TakeItem(int,int,out int)`, already used by normal transfer, searches forward; removing13 oil from a later partial stack cannot be assumed to free that grid if an earlier20 stack exists. The one-time20-oil transfer experiment did admit water2, but later mixed deliveries reclaimed the space and three incoming sorters held hydrogen. EXP-219 therefore records a failed sustained-recovery hypothesis, not artificial production/demand or permission to repeat transfers. Exact logical grid indices require native ordered evidence, not public buffer offsets.
...
Current-assembly decompilation also proves that connecting an output belt does not select its cargo. `StationComponent.UpdateOutputSlots` computes the public storage array index as `slots[port].storageIdx - 1`, so raw `0` is the native “none” sentinel and raw `1` selects `storage[0]`. Both station UI entry points use the same direct field path:

```text
UIBeltBuildTip.SetOutputEntity / UISlotPicker.SetOutputEntity
  -> picker list [None, storage[0], storage[1], ...]
UIBeltBuildTip.SetFilterToEntity / UISlotPicker.SetFilterToEntity
  -> station.slots[outputSlotId].storageIdx = selectedListIndex
StationComponent.UpdateOutputSlots
  -> storageIndex = slot.storageIdx - 1
```

The adopted `logistics-station-belt` subset accepts only a completed non-collector station, an exact connected `IODir.Output` port with `counter=0`, and an unlocked nonempty storage slot. The belt component must resolve to a current belt entity whose `EntityData.beltId` points back to the same component. Existing nonzero selectors are never cleared or retargeted. Prepare binds the station configuration hash, port index, zero-based public storage index, and resolved item; commit writes only `storageIdx = publicIndex + 1`, then proves target readback while the complete storage/order fingerprint, player package/in-hand fingerprint, station energy/fleet/route fields, port direction/belt/counter, and every unrelated belt selector remain unchanged.
...
Later orbital geometry exposed an additional requirement: origin-only avoidance does not protect a line of sight that crosses another planet. Two attempts from the same protected `104 -> 102` checkpoint followed a line passing only about 174 m from gas giant `103`'s center and were natively captured by that body. Current-assembly Mono.Cecil inspection of `GameData.GetNearestStarPlanet(StarData&, PlanetData&)` shows that an existing local planet is released after its surface distance exceeds `1000 × 0.4 = 400 m`; with no local planet, the nearest planet whose surface distance is below 1000 m is adopted. `PlayerMove_Sail.GameTick` separately applies `SoftLimit` to ordinary planets from `realRadius + 40` through `realRadius + 400`. The controller now scans all non-origin/non-destination planets in the current star, treats `realRadius + max(1000 m, 0.75 × realRadius)` as the minimum direct centerline clearance, and uses a deterministic moving detour waypoint only when that envelope is intersected. Both waypoint legs must retain at least 1.05 times that envelope and detour speed is capped at 200 m/s; once direct clearance is restored, ordinary destination steering resumes. The geometry lives in Bridge.Core and is covered by the observed blocked route plus clear-route and deterministic-selection tests. Live validation remains required after a normal same-batch deployment.
...
Two later `102 -> 104` attempts exposed a distinct ocean-contact case: destination identity was already established, but the player remained in `EMovementState.Drift`; the user observed that ordinary lateral movement of only a few steps reached land. Targeted decompilation of this exact assembly confirms that `PlayerMove_Drift.GameTick` consumes `player.currentOrder`, projects an `OrderNode.MoveTo` target onto the local tangent plane, generates `rtsVelocity` at ordinary `mecha.walkSpeed`, charges it through `UseDriftEnergy`, and leaves collision plus the eventual Drift-to-Walk transition to `DetermineDrift`. `PlayerOrder.ReachTest` measures Move completion on the planet sphere. `PlanetRawData.vertices` and `QueryModifiedHeight` provide current terrain samples, while native `PlanetAlgorithm.CalcLandPercent` treats raw heights near the planet radius as the land boundary. The bounded recovery therefore searches no farther than 120 m, requires a center at least 0.2 m above `realRadius` plus eight 2 m neighborhood probes no lower than -0.05 m, and issues at most three exact-owned native Move orders. Each order retains power-starvation checks, the existing movement-progress watchdog, a 120-tick native transition grace period, the overall 7200-tick landing timeout, and checkpoint recovery on any failure. It never writes player position or synthesizes keyboard input. Live action `f6b60009-3b77-4b34-b14c-fd89d4b13d71`, started from the same tick-`10173149` checkpoint, selected a 24.6 m target with 0.20 m center clearance, reached Walk/0 on its first native order, remained stable for 600 ticks, and preserved all 1100 titanium plus 651 silicon cargo. A further 10-second readback was position-identical, and normal save tick `10182419` retired the checkpoint with healthy writes.
...
2026-09-13 approach-path recheck: current DSP `0.10.34.28529` Assembly-CSharp SHA-256 `AE0BA95F75BD879A62AA4CE253B2AB78EAA4FB3C7C595F5E1FEE75EBE0E0EF85` confirms `PlayerAction_Mine.GameTick(long)` only selects the vein/vegetation from a Mine order after `currentOrder.targetReached`. `OrderNode.MineTarget(Vector3, EObjectType, int, Vector3)` stores that normal approach target; Spherewright computes a surface-projected point 1.2m before the resource. Thus the current 80m preparation range is not remote stationary mining. The current harvest coordinator has its bounded mining/power-starvation timeout, not the Move progress watchdog. Use existing bounded Move and terminal/readback semantics to get out of a known building cluster before attempting harvest; neither range acceptance nor a short terrain preview proves collision-free mining approach. This recheck changes no native call, timeout or action domain.
...
Targeted current-assembly inspection also confirmed the normal dismantle business path used by the build controller:

```text
public bool PlayerAction_Build.DoDismantleObject(int objId)
public void PlanetFactory.DismantleFinally(Player player, int objId, ref int protoId)
public void PlanetFactory.TakeBackItemsInEntity(Player player, int entityId)
public void FactorySystem.TakeBackItems_Miner(Player player, int minerId)
```

`DoDismantleObject` validates the live object, returns one built-item unit through the player's normal package operation, and then calls `DismantleFinally`. In ordinary non-sandbox play, `DismantleFinally` invokes `TakeBackItemsInEntity`; a miner's internal cargo is in turn recovered through `TakeBackItems_Miner` before the component/entity is removed. The public Spherewright subset calls only `DoDismantleObject` for a fully built resource miner and surrounds it with exact precondition, disappearance and inventory-conservation readback; it does not call `DismantleFinally` or mutate pools directly.
...
For a solid resource miner without an explicit preferred pose, prepare now validates the complete bounded radial/yaw candidate set before selecting a plan. Selection is deterministic: maximize the number of unique positive native preview resource-node parameters first, then minimize distance to the caller-bound node, yaw and candidate index. The returned `plannedResourceNodeIds` exposes that exact coverage before commit, and completion requires the constructed miner's native vein set to equal it exactly. A merely legal first candidate is therefore no longer treated as an efficient miner placement.
