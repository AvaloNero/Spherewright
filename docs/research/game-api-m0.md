...
`OwnedSavePrefixReader` reproduces only this bounded layout without game DLL dependencies, skips account values/screenshots, compares the complete embedded name ordinally against a caller-supplied protected identity, and returns no names/accounts. Screenshot skip is capped at16MiB, account-name bytes4096, owned-identity bytes1024; malformed/truncated or unsupported layouts fail closed. The25 synthetic offline cases cover exact identity, lookalikes, both sandbox values, bounds, versions, malformed UTF-8/lengths and stopping before GameDesc. This parser alone neither grants ownership nor authorizes a load; the explicit newer-LastExit recovery integration and local live validation remain pending.
