...
public static class SpherewrightTools
{
    ...
    [McpServerTool(
        ...
        Idempotent = true,
        OpenWorld = false)]
    [Description("Returns a cursor-bound page of all already-created owned factories for up to 64 exact item IDs. Actual rates come from DSP's save-persisted 600-game-tick automatic production and consumption window; theoretical output capacity is independently recomputed from identity-bound current runtime components. Supported assemblers, matrix labs, and resource extractors report bounded same-snapshot findings for material shortage, full output buffers, insufficient power, and exhausted veins; material shortages recursively follow same-planet physical cargo paths whose sorter and splitter filters admit the item. Logistics demand/supply evidence is attached only when physically proven. This is not yet a temporal transport-stall detector.")]
