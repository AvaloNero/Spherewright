...
public static partial class SpherewrightTools
{
    ...
    [McpServerTool(
        ...
        Idempotent = true,
        OpenWorld = false)]
    [Description("Re-reads one built entity (positive objectId) or prebuild (negative objectId) on Unity's main thread. Completed station entities include detailed logisticsStation state and independent live/configuration hashes. Belt cargo is not observed in buffers: use the separate detail-only beltCargo object. state=observed reports unique stacks touching only this belt segment at one game tick; unavailable/null is unknown, never zero. The read is bounded to 512 segment cells, with explicit reasons for unobserved seams or invalid references. Adjacent observations can include the same stack: do not sum them, infer flow from a single sample, or use these counts as an upgrade-preservation proof. Optional detail-only sorterEndpoints exposes at most16 native slot world positions/outward directions with occupancy, or4 belt virtual orientations. Virtual slot=-1 has unknown physical occupancy, not a free-slot promise. Plan from slot geometry, not building centers; an observed endpoint is not a placement approval. Missing/unavailable geometry is unknown. Lists omit this geometry; existing action/endpoint hashes do not bind either optional observation. Follow bounded directed/reciprocal connections to real consumers; an unfinished trace is not proof that no consumer exists.")]
