...
internal sealed partial class NormalGameActionCoordinator
{
    ...
    {
        accepted = null;
        rejection = "No supported native single-belt attachment fits the exact selected endpoints.";
        ...
        if (!TryCaptureBeltAttachment(factory, beltPoints[0].ObjectId, out var geometry)) return false;
        var validations = 0;
        foreach (var devicePoint in devicePoints)
        {
            foreach (var beltPoint in beltPoints)
            {
                ...
                if (!InserterBeltAttachmentPolicy.AcceptsSearchSeed(Snapshot(device.pos), Snapshot(devicePoint.Pose.position),
                        Snapshot(devicePoint.Pose.forward), Snapshot(beltPose.position), Snapshot(beltPose.forward))) continue;
                ...
                for (var i = 0; i < geometry!.Positions.Length - 1; i++)
                {
                    ...
                    if (!NativeInserterEndpointGeometry.AcceptsStraightPair(
                            ...
                            Snapshot(destinationPose.forward), touchesBelt: true)) continue;
                    ...
                    if (!InserterBeltAttachmentPolicy.AcceptsFinalRotations(
                            AttachmentRotation(step.Rotation), AttachmentRotation(step.Rotation2))) break;
                    if (++validations > InserterBeltAttachmentPolicy.MaximumNativeCandidates) return false;
    ...
    private static bool TryCaptureBeltAttachment(PlanetFactory factory, int entityId, out BeltAttachmentGeometry? geometry)
    {
        ...
        if (path is null || path.id != belt.segPathId || path.closed || path.buffer is null
            ...
            || path.pointPos is null || path.pointRot is null
            || path.pathLength > path.buffer.Length || path.pointPos.Length != path.buffer.Length
            || path.pointRot.Length != path.buffer.Length || path.belts is null
            || path.belts.Count > BeltUpgradePathPolicy.MaximumBelts
            || path.belts.Count(id => id == belt.id) != 1
            || !InserterBeltAttachmentPolicy.TryGetWindow(path.pathLength, belt.segIndex, belt.segLength,
    ...
    private static bool RevalidateBeltAttachment(PlanetFactory factory, BuildStepPlan step)
    {
        ...
        if (beltIsSource == beltIsDestination
            ...
            || !TryCaptureBeltAttachment(factory, step.AttachmentBeltObjectId, out var geometry)
